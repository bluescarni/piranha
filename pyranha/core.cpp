...
static inline void cleanup_type_system() { pyranha::et_map.clear(); }
...
BOOST_PYTHON_MODULE(_core) {
  // NOTE: this is a single big lock to avoid registering types/conversions
  // multiple times and prevent contention
  ...
  // Set the inited flag.
  // NOTE: we do it here because if something goes wrong in the rest of the
  // init, we do not have a way
  // to roll back the initialisation, and if the user tries again the init
  // probably a lot of things would
  // go haywire. Like this, we will not re-run any init code in a successive
  // attempt at loading the module.
  inited = true;
  // Piranha init.
  piranha::init();
  ...
  bp::class_<pyranha::generic_type_generator> gtg_class(
      "_generic_type_generator", bp::no_init);
  ...
  std::string types_module_name =
      bp::extract<std::string>(bp::scope().attr("__name__") + ".types");
  // NOTE: the nested namespace is created if not there, otherwise it will be
  // returned.
  ::PyObject *types_module_ptr =
      ::PyImport_AddModule(types_module_name.c_str());
  if (!types_module_ptr) {
    ::PyErr_SetString(PyExc_RuntimeError,
                      "error while creating the 'types' submodule");
    bp::throw_error_already_set();
  }
  // NOTE: I think at one point in the past there was a typo in the Python3 C
  // API documentation
  // which hinted at a difference in behaviour for PyImport_AddModule between 2
  // and 3 (new reference
  // vs borrowed reference). Current documentation states that the reference is
  // always
  ...
  pyranha::expose_generic_type_generator<piranha::monomial,
                                         piranha::rational>();
  ...
  pyranha::generic_translate<&PyExc_ZeroDivisionError,
                             piranha::zero_division_error>();
  pyranha::generic_translate<&PyExc_NotImplementedError,
                             piranha::not_implemented_error>();
  pyranha::generic_translate<&PyExc_OverflowError, std::overflow_error>();
  pyranha::generic_translate<&PyExc_OverflowError,
                             boost::numeric::positive_overflow>();
  pyranha::generic_translate<&PyExc_OverflowError,
                             boost::numeric::negative_overflow>();
  pyranha::generic_translate<&PyExc_OverflowError,
                             boost::numeric::bad_numeric_cast>();
  pyranha::generic_translate<&PyExc_ArithmeticError,
                             piranha::math::inexact_division>();
  // Exposed types list.
  ...
  settings_class
      .def("_get_max_term_output", piranha::settings::get_max_term_output)
      .staticmethod("_get_max_term_output");
  settings_class
      .def("_set_max_term_output", piranha::settings::set_max_term_output)
      .staticmethod("_set_max_term_output");
  settings_class
      .def("_reset_max_term_output", piranha::settings::reset_max_term_output)
      .staticmethod("_reset_max_term_output");
  settings_class.def("_set_n_threads", piranha::settings::set_n_threads)
      .staticmethod("_set_n_threads");
  settings_class.def("_get_n_threads", piranha::settings::get_n_threads)
      .staticmethod("_get_n_threads");
  settings_class.def("_reset_n_threads", piranha::settings::reset_n_threads)
      .staticmethod("_reset_n_threads");
  settings_class
      .def("_set_min_work_per_thread",
           piranha::settings::set_min_work_per_thread)
      .staticmethod("_set_min_work_per_thread");
  settings_class
      .def("_get_min_work_per_thread",
           piranha::settings::get_min_work_per_thread)
      .staticmethod("_get_min_work_per_thread");
  settings_class
      .def("_reset_min_work_per_thread",
           piranha::settings::reset_min_work_per_thread)

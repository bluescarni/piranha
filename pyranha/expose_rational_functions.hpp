...
template <typename T> struct rf_unary_ctor_exposer {
...
template <typename T, typename Tuple> struct rf_binary_ctor_exposer {
  template <typename U> struct inner_functor {
...
template <typename T> struct rf_interop_exposer {
  ...
  template <typename U> void operator()(const U &) const {
    ...
    m_rf_class.def(bp::self /= x);
#else
    m_rf_class.def("__itruediv__", generic_in_place_division_wrapper<T, U>,
                   bp::return_arg<1u>{});
#endif
...
template <typename T> struct rf_eval_exposer {
...
template <typename T> struct rf_subs_exposer {
...
template <typename T> inline typename T::p_type rf_num_wrapper(const T &r) {
  return r.num();
}

template <typename T> inline typename T::p_type rf_den_wrapper(const T &r) {
  return r.den();
}

template <typename Key> inline void expose_rational_functions_impl() {
  ...
  // Binary ctors.
  using binary_ctor_types =
      std::tuple<std::string, piranha::integer, piranha::rational, p_type,
                 q_type, r_type>;
  tuple_for_each(binary_ctor_types{},
                 rf_binary_ctor_exposer<r_type, binary_ctor_types>(rf_class));
  // Copy ctor.
  ...
  rf_class.def(bp::self /= bp::self);
#else
  rf_class.def("__itruediv__",
               generic_in_place_division_wrapper<r_type, r_type>,
               bp::return_arg<1u>{});
#endif
  rf_class.def(bp::self / bp::self);
  rf_class.def(bp::self == bp::self);
  ...
  using interop_types =
      std::tuple<piranha::integer, piranha::rational, p_type, q_type>;
  ...
  rf_class.def("clear_pow_cache", r_type::clear_pow_cache)
  ...
  // Subs.
  using subs_types =
      std::tuple<piranha::integer, piranha::rational, p_type, q_type, r_type>;
  ...
  rf_class
      .def("register_custom_derivative",
           generic_register_custom_derivative_wrapper<r_type>)
      .staticmethod("register_custom_derivative");
  rf_class
      .def("unregister_custom_derivative", r_type::unregister_custom_derivative)

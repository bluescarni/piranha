...
template <typename Series> struct generic_pickle_suite : bp::pickle_suite {
  static bp::tuple getinitargs(const Series &) { return bp::make_tuple(); }
  ...
  static void setstate(Series &s, bp::tuple state) {
    if (bp::len(state) != 1) {
      ::PyErr_SetString(PyExc_ValueError,
                        "the 'state' tuple must have exactly one element");
...
// Expose class with a default constructor and map it into the pyranha type
// system.
template <typename T> inline bp::class_<T> expose_class() {
  const auto t_idx = std::type_index(typeid(T));
  if (et_map.find(t_idx) != et_map.end()) {
    // NOTE: it is ok here and elsewhere to use c_str(), as PyErr_SetString will
    // convert the second argument to a Python
    // string object.
    ::PyErr_SetString(PyExc_RuntimeError, (std::string("the C++ type '") +
                                           demangled_type_name(t_idx) +
                                           "' has already been exposed")
                                              .c_str());
    bp::throw_error_already_set();
  }
  bp::class_<T> class_inst(
      (std::string("_exposed_type_") + std::to_string(exposed_types_counter))
          .c_str(),
      bp::init<>());
  ++exposed_types_counter;
  // NOTE: class_ inherits from bp::object, here the "call operator" of a class
  // type will construct an instance
  // of that object. We then get the Python type out of that. It seems like
  // another possible way of achieving
  ...
  if (!ptr) {
    ::PyErr_SetString(
        PyExc_RuntimeError,
        "cannot extract the Python type of an instantiated class");
    bp::throw_error_already_set();
  }
...
template <typename Tuple, typename Op, std::size_t B = 0u,
          std::size_t E = std::tuple_size<Tuple>::value, typename = void>
static void tuple_for_each(const Tuple &t, const Op &op,
                           typename std::enable_if<B != E>::type * = nullptr) {
  op(std::get<B>(t));
  static_assert(B != std::numeric_limits<std::size_t>::max(),
                "Overflow error.");
  tuple_for_each<Tuple, Op, static_cast<std::size_t>(B + std::size_t(1)), E>(
      t, op);
}

template <typename Tuple, typename Op, std::size_t B = 0u,
          std::size_t E = std::tuple_size<Tuple>::value, typename = void>
static void tuple_for_each(const Tuple &, const Op &,
                           typename std::enable_if<B == E>::type * = nullptr) {}

struct NullHook {
  template <typename T> void operator()(bp::class_<T> &) const {}
};

// Generic copy wrappers.
template <typename S> inline S generic_copy_wrapper(const S &s) { return s; }

template <typename S> inline S generic_deepcopy_wrapper(const S &s, bp::dict) {
  return s;
}
...
inline auto generic_evaluate_wrapper(const S &s, bp::dict dict, const T &)
    -> decltype(piranha::math::evaluate(
        s, std::declval<std::unordered_map<std::string, T>>())) {
...
// NOTE: need to reason about thread safety here. Lambdified objects are not
// thread safe,
// but separate lambdified objects could be used from different threads and we
// need to protect
// access to the python interpreter.
template <typename S, typename U>
inline auto generic_lambdify_wrapper(const S &s, bp::list l, bp::dict d,
                                     const U &) ->
...
inline auto lambdified_call_operator(piranha::math::lambdified<T, U> &l,
                                     bp::object o) -> decltype(l({})) {
...
template <typename S, typename U> inline void generic_expose_lambdified() {
  using l_type = piranha::math::lambdified<S, U>;
  bp::class_<l_type> class_inst(
      (std::string("_lambdified_") + std::to_string(lambdified_counter))
          .c_str(),
      bp::no_init);
...
// NOTE: last param is dummy to let the Boost.Python type system to pick the
// correct type.
template <typename S>
inline bool generic_canonical_wrapper(bp::list new_p, bp::list new_q,
                                      bp::list p_list, bp::list q_list,
                                      const S &) {
  ...
  return piranha::math::transformation_is_canonical(
      std::vector<S>(begin_new_p, end_new_p),
      std::vector<S>(begin_new_q, end_new_q),
      std::vector<std::string>(begin_p, end_p),
      std::vector<std::string>(begin_q, end_q));
}

// Generic Poisson bracket wrapper.
template <typename S>
inline auto generic_pbracket_wrapper(const S &s1, const S &s2, bp::list p_list,
                                     bp::list q_list)
    -> decltype(piranha::math::pbracket(s1, s2, {}, {})) {
  bp::stl_input_iterator<std::string> begin_p(p_list), end_p;
  bp::stl_input_iterator<std::string> begin_q(q_list), end_q;
  return piranha::math::pbracket(s1, s2,
                                 std::vector<std::string>(begin_p, end_p),
                                 std::vector<std::string>(begin_q, end_q));
}
...
template <typename S>
inline auto generic_degree_wrapper(const S &s)
    -> decltype(piranha::math::degree(s)) {
...
template <typename S>
inline auto generic_ldegree_wrapper(const S &s)
    -> decltype(piranha::math::ldegree(s)) {
...
template <typename S> inline std::string generic_latex_wrapper(const S &s) {
  std::ostringstream oss;
  s.print_tex(oss);
  return oss.str();
}

// A simple wrapper for in-place division. We need this because Boost.Python
// does not expose correctly
...
inline void check_callable(bp::object func) {
  ...
  try {
    ...
    // NOTE: it seems like it is ok to overwrite the global error status of
    // Python here,
    // after it has already been set by Boost.Python via the exception thrown
    // above.
    ::PyErr_SetString(PyExc_TypeError, "object is not callable");
    bp::throw_error_already_set();
  }
...
// NOTE: here it is important to keep the member/free distinction because of the
// special semantics of partial.
template <typename S>
inline auto generic_partial_wrapper(const S &s, const std::string &name)
    -> decltype(piranha::math::partial(s, name)) {
  return piranha::math::partial(s, name);
}

template <typename S>
inline auto generic_partial_member_wrapper(const S &s, const std::string &name)
    -> decltype(s.partial(name)) {
  return s.partial(name);
}

// NOTE: here we need to take care of multithreading in the future. We need to
// check at least that:
// - the check and copy are thread-safe,
// - calling the function itself is thread-safe (e.g., if pbracket one day gets
// parallelised we might
//   end up calling func from multiple C++ threads at the same time).
// Note that the issue here is calling the same Python interpreter from multiple
// C++ threads of which
// the interpreter knows nothing.
template <typename S>
inline void generic_register_custom_derivative_wrapper(const std::string &name,
                                                       bp::object func) {
  using partial_type =
      decltype(std::declval<const S &>().partial(std::string()));
  check_callable(func);
  // Make a deep copy.
  bp::object deepcopy = bp::import("copy").attr("deepcopy");
...
class series_exposer {
  ...
  static void expose_ctor(
      ...
      typename std::enable_if<!std::is_constructible<T, U>::value>::type * =
          nullptr) {}
  // Sparsity wrapper.
  template <typename S> static bp::dict table_sparsity_wrapper(const S &s) {
  ...
  // Wrapper to list.
  template <typename S> static bp::list to_list_wrapper(const S &s) {
    bp::list retval;
    for (const auto &t : s) {
  ...
  using common_ops_ic = std::integral_constant<
      bool, piranha::is_addable_in_place<S, T>::value &&
  ...
  template <typename S, typename T,
            typename std::enable_if<common_ops_ic<S, T>::value, int>::type = 0>
  ...
  template <
      ...
      typename std::enable_if<division_ops_ic<S, T>::value, int>::type = 0>
  static void expose_division(bp::class_<S> &series_class, const T &in) {
    namespace sn = boost::python::self_ns;
#if PY_MAJOR_VERSION < 3
    series_class.def(sn::operator/=(bp::self, in));
#else
    series_class.def("__itruediv__", generic_in_place_division_wrapper<S, T>,
                     bp::return_arg<1u>{});
#endif
    series_class.def(sn::operator/(bp::self, in));
    series_class.def(sn::operator/(in, bp::self));
  ...
  template <typename S> struct pow_exposer {
    pow_exposer(bp::class_<S> &series_class) : m_series_class(series_class) {}
    bp::class_<S> &m_series_class;
    template <typename T, typename U>
    static auto pow_wrapper(const T &s, const U &x)
        -> decltype(piranha::math::pow(s, x)) {
      return piranha::math::pow(s, x);
    }
    template <typename T>
    void operator()(
        const T &,
        typename std::enable_if<piranha::is_exponentiable<S, T>::value>::type
            * = nullptr) const {
  ...
  static void
  expose_pow(bp::class_<S> &series_class,
             typename std::enable_if<has_typedef_pow_types<T>::value>::type * =
                 nullptr) {
  ...
  template <typename S, typename T = Descriptor>
  static void
  expose_pow(bp::class_<S> &,
             typename std::enable_if<!has_typedef_pow_types<T>::value>::type * =
                 nullptr) {}
  // Evaluation.
  template <typename S> struct eval_exposer {
    eval_exposer(bp::class_<S> &series_class) : m_series_class(series_class) {}
    bp::class_<S> &m_series_class;
    template <typename T>
    void operator()(
        const T &,
        typename std::enable_if<piranha::is_evaluable<S, T>::value>::type * =
            nullptr) const {
  ...
  template <typename S, typename T = Descriptor>
  static void expose_eval(
      bp::class_<S> &series_class,
      typename std::enable_if<has_typedef_eval_types<T>::value>::type * =
          nullptr) {
  ...
  static void expose_eval(
      ...
      typename std::enable_if<!has_typedef_eval_types<T>::value>::type * =
          nullptr) {}
  // Substitution.
  template <typename S> struct subs_exposer {
    ...
    template <typename T, typename std::enable_if<
                              piranha::has_subs<S, T>::value, int>::type = 0>
    ...
    void impl_subs(const T &) const {}
    template <typename T,
              typename std::enable_if<piranha::has_ipow_subs<S, T>::value,
                                      int>::type = 0>
    ...
    void impl_ipow_subs(const T &) const {}
    template <typename T,
              typename std::enable_if<piranha::has_t_subs<S, T, T>::value,
                                      int>::type = 0>
    ...
    // The actual wrappers.
    template <typename T>
    static auto subs_wrapper(const S &s, const std::string &name, const T &x)
        -> decltype(s.subs(name, x)) {
      return s.subs(name, x);
    }
    template <typename T>
    static auto ipow_subs_wrapper(const S &s, const std::string &name,
                                  const piranha::integer &n, const T &x)
        -> decltype(s.ipow_subs(name, n, x)) {
      return s.ipow_subs(name, n, x);
    }
    template <typename T>
    static auto t_subs_wrapper(const S &s, const std::string &name, const T &x,
                               const T &y) -> decltype(s.t_subs(name, x, y)) {
      return s.t_subs(name, x, y);
    }
  };
  template <typename S, typename T = Descriptor>
  static void expose_subs(
      bp::class_<S> &series_class,
      typename std::enable_if<has_typedef_subs_types<T>::value>::type * =
          nullptr) {
  ...
  template <typename S> struct interop_exposer {
    interop_exposer(bp::class_<S> &series_class)
        : m_series_class(series_class) {}
    bp::class_<S> &m_series_class;
  ...
  // Here we have three implementations of this function. The final objective is
  // to enable construction and arithmetics
  // with respect to the coefficient type, and, recursively, the coefficient
  // type of the coefficient type.
  // 1. Series2 is a series type whose cf is not among interoperable types.
  // In this case, we expose the interoperability with the cf, and we try to
  // expose also the interoperability with the coefficient
  // type of cf, if it is a series type.
  template <typename InteropTypes, typename Series2, typename S>
  static void expose_cf_interop(
      bp::class_<S> &series_class,
      typename std::enable_if<!piranha::detail::type_in_tuple<
          typename Series2::term_type::cf_type, InteropTypes>::value>::type * =
          nullptr) {
  ...
  // 2. Series2 is a series type whose cf is among the interoperable types. Try
  ...
  static void expose_cf_interop(
      ...
      typename std::enable_if<piranha::detail::type_in_tuple<
          typename Series2::term_type::cf_type, InteropTypes>::value>::type * =
          nullptr) {
    expose_cf_interop<InteropTypes, typename Series2::term_type::cf_type>(
        series_class);
  }
  ...
  static void expose_cf_interop(
      bp::class_<S> &,
      typename std::enable_if<!piranha::is_series<Series2>::value>::type * =
          nullptr) {}
  template <typename S, typename T = Descriptor>
  static void expose_interoperable(
      bp::class_<S> &series_class,
      typename std::enable_if<has_typedef_interop_types<T>::value>::type * =
          nullptr) {
    using interop_types = typename Descriptor::interop_types;
    interop_types it;
    tuple_for_each(it, interop_exposer<S>(series_class));
    // Interoperate conditionally with coefficient type, if it is not already in
    // the
  ...
  template <typename S>
  static auto integrate_wrapper(const S &s, const std::string &name)
      -> decltype(piranha::math::integrate(s, name)) {
    return piranha::math::integrate(s, name);
  }
  template <typename S>
  static void expose_integrate(
      bp::class_<S> &series_class,
      typename std::enable_if<piranha::is_integrable<S>::value>::type * =
          nullptr) {
  ...
  // Differentiation.
  template <typename S>
  static void expose_partial(
      bp::class_<S> &series_class,
      typename std::enable_if<piranha::is_differentiable<S>::value>::type * =
          nullptr) {
    // NOTE: we need this below in order to specify exactly the address of the
    // templated
    // static method for unregistering the custom derivatives.
    using partial_type = decltype(
        piranha::math::partial(std::declval<const S &>(), std::string{}));
    ...
    series_class
        .def("register_custom_derivative",
             generic_register_custom_derivative_wrapper<S>)
        .staticmethod("register_custom_derivative");
    series_class
        .def("unregister_custom_derivative",
             S::template unregister_custom_derivative<S, partial_type>)
        .staticmethod("unregister_custom_derivative");
    series_class
        .def("unregister_all_custom_derivatives",
             S::template unregister_all_custom_derivatives<S, partial_type>)
  ...
  static void expose_partial(
      ...
      typename std::enable_if<!piranha::is_differentiable<S>::value>::type * =
          nullptr) {}
  template <typename S>
  static void expose_pbracket(
      bp::class_<S> &,
      typename std::enable_if<piranha::has_pbracket<S>::value>::type * =
          nullptr) {
  ...
  static void expose_pbracket(
      ...
      typename std::enable_if<!piranha::has_pbracket<S>::value>::type * =
          nullptr) {}
  template <typename S>
  static void expose_canonical(
      bp::class_<S> &,
      typename std::enable_if<
          piranha::has_transformation_is_canonical<S>::value>::type * =
          nullptr) {
  ...
  static void expose_canonical(
      ...
      typename std::enable_if<
          ...
          nullptr) {}
  // filter() wrap.
  template <typename S> static S wrap_filter(const S &s, bp::object func) {
  ...
  static void check_tuple_2(bp::object obj) {
    ...
    if (len != 2u) {
      ::PyErr_SetString(PyExc_ValueError, "the tuple to be returned in series "
                                          "transformation must have 2 "
                                          "elements");
      bp::throw_error_already_set();
    }
  }
  // transform() wrap.
  template <typename S> static S wrap_transform(const S &s, bp::object func) {
    typedef typename S::term_type::cf_type cf_type;
    check_callable(func);
    auto cpp_func =
        [func](const std::pair<cf_type, S> &p) -> std::pair<cf_type, S> {
  ...
  static void expose_sin_cos(
      typename std::enable_if<piranha::has_sine<S>::value &&
                              piranha::has_cosine<S>::value>::type * =
          nullptr) {
  ...
  static void expose_degree(
      bp::class_<T> &,
      typename std::enable_if<piranha::has_degree<T>::value &&
                              piranha::has_ldegree<T>::value>::type * =
          nullptr) {
  ...
  static void expose_degree(
      ...
      typename std::enable_if<!piranha::has_degree<T>::value ||
          ...
          nullptr) {}
  // Truncation.
  template <typename S> struct truncate_degree_exposer {
    truncate_degree_exposer(bp::class_<S> &series_class)
        : m_series_class(series_class) {}
    bp::class_<S> &m_series_class;
    template <typename T>
    ...
    void expose_truncate_degree(
        const T &,
        typename std::enable_if<piranha::has_truncate_degree<S, T>::value>::type
            * = nullptr) const {
    ...
    template <typename T>
    void expose_truncate_degree(
        const T &, typename std::enable_if<
                       !piranha::has_truncate_degree<S, T>::value>::type * =
                       nullptr) const {}
    template <typename T> void operator()(const T &x) const {
  ...
  template <typename S, typename T = Descriptor,
            typename std::enable_if<
                has_typedef_degree_truncation_types<T>::value, int>::type = 0>
  ...
  static void expose_trigonometric_series(
      bp::class_<S> &,
      typename std::enable_if<
          piranha::has_t_degree<S>::value && piranha::has_t_ldegree<S>::value &&
  ...
  static void expose_trigonometric_series(
      bp::class_<S> &,
      typename std::enable_if<!piranha::has_t_degree<S>::value ||
                              !piranha::has_t_ldegree<S>::value ||
                              !piranha::has_t_order<S>::value ||
                              !piranha::has_t_lorder<S>::value>::type * =
          nullptr) {}
  template <typename S>
  ...
  template <typename S>
  static auto wrap_partial_t_degree(const S &s, bp::list l)
      -> decltype(s.t_degree(std::vector<std::string>{})) {
    bp::stl_input_iterator<std::string> begin(l), end;
  ...
  template <typename S>
  static auto wrap_partial_t_ldegree(const S &s, bp::list l)
      -> decltype(s.t_ldegree(std::vector<std::string>{})) {
    bp::stl_input_iterator<std::string> begin(l), end;
  ...
  template <typename S>
  static auto wrap_partial_t_order(const S &s, bp::list l)
      -> decltype(s.t_order(std::vector<std::string>{})) {
    bp::stl_input_iterator<std::string> begin(l), end;
  ...
  template <typename S>
  static auto wrap_partial_t_lorder(const S &s, bp::list l)
      -> decltype(s.t_lorder(std::vector<std::string>{})) {
    bp::stl_input_iterator<std::string> begin(l), end;
    return s.t_lorder(std::vector<std::string>(begin, end));
  }
  // Symbol set wrapper.
  template <typename S> static bp::list symbol_set_wrapper(const S &s) {
    bp::list retval;
    for (auto it = s.get_symbol_set().begin(); it != s.get_symbol_set().end();
         ++it) {
  ...
  static void expose_save_load(bp::class_<S> &series_class) {
    // Save.
    typedef void (*s1)(const S &, const std::string &, piranha::file_format,
                       piranha::file_compression);
    typedef void (*s2)(const S &, const std::string &);
    typedef void (*s3)(const S &, const std::string &,
                       piranha::file_compression);
    ...
    // Load.
    typedef S (*l1)(const std::string &, piranha::file_format,
                    piranha::file_compression);
  ...
  template <typename S, typename std::enable_if<
                            piranha::is_invertible<S>::value, int>::type = 0>
  ...
  struct exposer_op {
    ...
    void operator()(const std::tuple<Args...> &) const {
      ...
      series_class.def(bp::self /= bp::self);
#else
      series_class.def("__itruediv__",
                       generic_in_place_division_wrapper<s_type, s_type>,
                       bp::return_arg<1u>{});
#endif
      series_class.def(bp::self / bp::self);
      series_class.def(bp::self == bp::self);
      ...
      series_class.def(-bp::self);
      // NOTE: here this method is available if is_identical() is (that is, if
      // the series are comparable), so
      // put it here - even if logically it belongs to exponentiation. We assume
      // the series are comparable anyway.
      series_class
          .def("clear_pow_cache", s_type::template clear_pow_cache<s_type, 0>)
...
public:
  series_exposer() {
    // NOTE: probably we can avoid instantiating p here, look at the tuple
    // for_each
    // in vargs_to_v_t_idx.
    params p;

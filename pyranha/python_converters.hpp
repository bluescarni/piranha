...
template <typename T>
inline void
construct_from_str(::PyObject *obj_ptr,
                   bp::converter::rvalue_from_python_stage1_data *data,
                   const std::string &name) {
  piranha_assert(obj_ptr);
  // NOTE: here we use str because for int and rational the string
  // representations in Python
  // match the input format for the corresponding C++ objects. This is not true
  // in general
  // for the repr() representations (e.g., in Python 2.x a trailing "L" is added
  // to the repr() representation
  // for long integers).
  ::PyObject *str_obj = ::PyObject_Str(obj_ptr);
  if (!str_obj) {
    ::PyErr_SetString(
        PyExc_RuntimeError,
        (std::string("unable to extract string representation of ") + name)
            .c_str());
...
#else
  ::PyObject *unicode_str_obj =
      ::PyUnicode_AsEncodedString(str_rep.get(), "ascii", "strict");
  if (!unicode_str_obj) {
    ::PyErr_SetString(
        PyExc_RuntimeError,
        (std::string("unable to extract string representation of ") + name)
...
#endif
  // NOTE: the use of reinterpret_cast here comes straight from the sources
  // quoted at the beginning of the file. reinterpret_cast
  // in general looks rather dangerous and unsafe, and it's not clear to me here
  // if this is an idiomatic way of doing things in Boost.Python
  // or instead this should be a static_cast to void * or something like that.
  // Maybe a good question for the Boost Python mailing list.
  void *storage =
      reinterpret_cast<bp::converter::rvalue_from_python_storage<T> *>(data)
          ->storage.bytes;
  ::new (storage) T(s);
...
struct real_converter {
  ...
  struct to_python {
    static ::PyObject *convert(const piranha::real &r) {
      ...
      try {
        ...
        ::PyErr_SetString(PyExc_RuntimeError, "could not convert real number "
                                              "to mpf object - please check "
                                              "the installation of mpmath");
        bp::throw_error_already_set();
        // A fake return value that will never be returned, to make a GCC
        // warning go away.
        return nullptr;
      }
    }
  };
  // Check if obj_ptr is an instance of the class str_class in the module
  // str_mod. In case
  // of import or attr errors, false will be returned.
  static bool is_instance_of(::PyObject *obj_ptr, const char *str_mod,
    ...
    try {
      ...
      // Clear the Python global error status. We don't want some other function
      // to check it later
  ...
  static void construct(::PyObject *obj_ptr,
                        bp::converter::rvalue_from_python_stage1_data *data) {
    // NOTE: here we cannot construct directly from string, as we need to query
    // the precision.
    piranha_assert(obj_ptr);
    // NOTE: here the handle is from borrowed because we are not responsible for
    // the generation of obj_ptr:
    // borrowed will increase the refcount of obj_ptr, so that, when obj is
    // destroyed, the refcount
    ...
    // NOTE: here we use repr instead of str because the repr seems to give the
    // most accurate representation
    // in base 10 for the object.
    ::PyObject *str_obj = ::PyObject_Repr(obj.ptr());
    if (!str_obj) {
      ::PyErr_SetString(PyExc_RuntimeError,
                        "unable to extract string representation of real");
      bp::throw_error_already_set();
    }
    bp::handle<> str_rep(str_obj);
#if PY_MAJOR_VERSION < 3
...
#endif
    // NOTE: the search for "'" is due to the string format of mpmath.mpf
    // objects.
    while (*s != '\0' && *s != '\'') {
      ++s;
    }
    if (*s == '\0') {
      ::PyErr_SetString(PyExc_RuntimeError,
                        "invalid string input converting to real");
      bp::throw_error_already_set();
    }
    ++s;
    auto start = s;
    ...
    void *storage =
        reinterpret_cast<
            bp::converter::rvalue_from_python_storage<piranha::real> *>(data)
            ->storage.bytes;

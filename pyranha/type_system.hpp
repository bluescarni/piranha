...
// Meta/macro programming to connect a template template class to a name
// expressed as a string.
template <typename T> struct tt_namer {
  static_assert(!std::is_same<T, T>::value,
                "No name defined for this template template class.");
};

#define PYRANHA_DECLARE_TT_NAMER(TT, tt_name)                                  \
  template <typename... Args> struct tt_namer<TT<Args...>> {                   \
...
// Names of Python instances of type generators. We keep track because we do not
// want multiple instances
// with the same name on the Python side.
extern std::unordered_set<std::string> tg_names;

// Connection between C++ types (type_index) and Python types (encapsulated in
// bp::object).
extern std::unordered_map<std::type_index, bp::object> et_map;

// Type generator structure. It establishes the connection between a C++ type
// (the m_t_idx member)
// and its exposed Python counterpart via the call operator, which will query
// the et_map archive.
...
// Map of generic type generators. Each item in the map is associated to another
// map, which establishes the
// connection between the concrete set of types used as template parameters for
// the template template class
// and the final concrete instantiated type.
extern std::unordered_map<std::string,
                          std::unordered_map<std::vector<std::type_index>,
                                             std::type_index, v_idx_hasher>>
    gtg_map;

// Small utility to convert a vector of type indices to a string representation
// for error reporting purposes.
std::string v_t_idx_to_str(const std::vector<std::type_index> &);

// Like above, but this instead establishes the connection between a template
// template class instantiated
// with a certain set of params and a type_generator.
struct generic_type_generator {
  explicit generic_type_generator(const std::string &name,
                                  const std::string &orig_name)
      : m_name(name), m_orig_name(orig_name) {}
  // Get the type generator corresponding to the C++ type defined by
  // name<l[0],l[1],...>,
...
inline void expose_type_generator(const std::string &name) {
  ...
  if (tg_names.find(name) != tg_names.end()) {
    ::PyErr_SetString(PyExc_RuntimeError,
                      (std::string("a type generator called '") + name +
...
struct vargs_to_v_t_idx {
  void operator()(std::vector<std::type_index> &v_t_idx) const {
    static_assert(Idx < std::numeric_limits<std::size_t>::max(),
                  "Overflow error.");
    v_t_idx.push_back(
        std::type_index(typeid(typename std::tuple_element<Idx, Tuple>::type)));
...
struct vargs_to_v_t_idx<
    Tuple, Idx,
    typename std::enable_if<Idx == std::tuple_size<Tuple>::value>::type> {
...
inline void expose_generic_type_generator() {
  const std::string name = tt_namer<TT<Args...>>::name,
                    orig_name = tt_namer<TT<Args...>>::orig_name;
  ...
  if (gtg_map[name].find(v_t_idx) != gtg_map[name].end()) {
    ::PyErr_SetString(
        PyExc_RuntimeError,
        (std::string("the generic type generator '") + name +
         std::string("' has already been instantiated with the type pack ") +
         v_t_idx_to_str(v_t_idx))
            .c_str());
    bp::throw_error_already_set();
  }
  gtg_map[name].emplace(
      std::make_pair(v_t_idx, std::type_index(typeid(TT<Args...>))));
}
}

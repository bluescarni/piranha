...
struct ps_custom_hook {
  ...
  template <typename T> struct has_t_integrate : piranha::detail::sfinae_types {
  ...
  // NOTE: here the return type is the same as returned by the other overload of
  // t_integrate().
  template <typename S>
  static auto t_integrate_names_wrapper(const S &s, bp::list l)
      -> decltype(s.t_integrate()) {
    bp::stl_input_iterator<std::string> begin_p(l), end_p;
    return s.t_integrate(std::vector<std::string>(begin_p, end_p));
  }
  template <typename S,
            typename std::enable_if<has_t_integrate<S>::value, int>::type = 0>

...
std::string demangled_type_name(const std::type_index &t_idx) {
  ...
  std::unique_ptr<char, void (*)(void *)> res{
      ::abi::__cxa_demangle(t_idx.name(), nullptr, nullptr, &status),
      std::free};
...
bp::object type_generator::operator()() const {
  ...
  if (it == et_map.end()) {
    ::PyErr_SetString(PyExc_TypeError,
                      (std::string("the type '") +
                       demangled_type_name(m_t_idx) + "' has not been exposed")
                          .c_str());
...
std::string type_generator::repr() const {
  return std::string("Type generator for the C++ type '") +
         demangled_type_name(m_t_idx) + "'";
}

std::size_t v_idx_hasher::
operator()(const std::vector<std::type_index> &v) const {
  ...
  if (it1 == gtg_map[m_name].end()) {
    ::PyErr_SetString(
        ...
        (std::string("the generic type generator '") + m_name +
         std::string("' has not been instantiated with the type pack ") +
...
std::string generic_type_generator::repr() const {
  return std::string("Type generator for the generic C++ type '") +
         m_orig_name + "'";
}

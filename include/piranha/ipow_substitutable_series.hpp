 ...
 * by
 * the piranha::has_ipow_subs and piranha::key_has_ipow_subs type traits). If the requirements outlined above are not
...
class ipow_substitutable_series : public Series, detail::ipow_substitutable_series_tag
{
    ...
    static ret_type_1<T, Term> subs_term_impl(const Term &t, const std::string &name, const integer &n, const T &x,
    ...
    static ret_type_2<T, Term> subs_term_impl(const Term &t, const std::string &name, const integer &n, const T &x,
                                              const symbol_set &s_set)
    {
    ...
    static ret_type_3<T, Term> subs_term_impl(const Term &t, const std::string &name, const integer &n, const T &x,
                                              const symbol_set &s_set)
    {
    ...
    template <typename T>
    using ipow_subs_type = typename std::enable_if<std::is_constructible<subs_type_<T>, int>::value
...
using ipow_subs_impl_ipow_subs_series_enabler =
    typename std::enable_if<std::is_base_of<ipow_substitutable_series_tag, Series>::value

 ...
 * \brief Exceptions.
 *
 * This header contains custom exceptions used within piranha and related
 * utilities.
 */
...
template <typename Exception> struct ex_thrower {
  // Determine the type of the __LINE__ macro.
  typedef std::decay<decltype(__LINE__)>::type line_type;
  explicit ex_thrower(const char *file, line_type line, const char *func)
      : m_file(file), m_line(line), m_func(func) {}
  template <typename... Args,
            typename = typename std::enable_if<
                std::is_constructible<Exception, Args...>::value>::type>
  ...
  template <
      ...
      typename = typename std::enable_if<
          ...
          (std::is_same<typename std::decay<Str>::type, std::string>::value ||
           ...
           std::is_same<typename std::decay<Str>::type,
...
/**
 * By default, this variadic macro will throw an exception of type \p
 exception_type, using the variadic
 * arguments for the construction of the exception object. The macro will check
 if the exception can be constructed
 * from the variadic arguments, and will produce a compilation error in case no
 suitable constructor is found.
 *
 ...
 * - and if the exception can be constructed from the set of arguments
 <tt>[str,arg1,...]</tt>,
 *   where \p str is an instance of \p std::string,
 *
 * then the first argument \p arg0 is interpreted as the error message
 associated to the exception object, and it will be decorated
 * with information about the context in which the exception was thrown (file,
 line, function) before being
 * passed on for construction.
 *
 * Note that, in order to be fully standard-compliant, for use with exceptions
 that take no arguments on construction
...
#define piranha_throw(exception_type, ...)                                     \
  piranha::detail::ex_thrower<exception_type>(__FILE__, __LINE__,              \
...
/// Exception for functionality not implemented or not available on the current
/// platform.

...
/**
 * This allocator will try to allocate memory aligned to the cache line size (as
 * reported by piranha::settings).
 *
 * Exception safety and move semantics are equivalent to
 * piranha::dynamic_aligning_allocator.
 */
template <typename T>
  ...
  static std::size_t determine_alignment() {
    ...
    try {
      const std::size_t alignment =
          boost::numeric_cast<std::size_t>(settings::get_cache_line_size());
...
public:
  ...
  /// Default constructor.
  /**
   * Will invoke the base constructor with an alignment value determined as
   * follows:
   * - if no memory alignment primitives are available on the host platform, the
   * value will be zero;
   * - if the cache line size reported by
   * piranha::settings::get_cache_line_size() passes the checks
   *   performed by piranha::alignment_check() of \p T, it will be used as
   * construction value;
  ...
  /**
   * Will forward the call to the corresponding constructor in
   * piranha::dynamic_aligning_allocator.
   *
   * @param[in] other construction argument.
   */
  template <typename U>
  explicit cache_aligning_allocator(const cache_aligning_allocator<U> &other)
      : base(other) {}
   ...
   */
  template <typename U>
  explicit cache_aligning_allocator(cache_aligning_allocator<U> &&other)
      : base(std::move(other)) {}
  /// Defaulted destructor.
  ~cache_aligning_allocator() = default;
  /// Defaulted copy assignment operator.
  cache_aligning_allocator &
  operator=(const cache_aligning_allocator &) = default;

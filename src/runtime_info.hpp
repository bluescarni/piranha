...
template <typename = int> struct base_runtime_info {
  static const std::thread::id m_main_thread_id;
};

template <typename T>
const std::thread::id
    base_runtime_info<T>::m_main_thread_id = std::this_thread::get_id();
}
...
public:
  /// Main thread ID.
  /**
   * @return const reference to an instance of the ID of the main thread of
   * execution.
   */
  static const std::thread::id &get_main_thread_id() {
    return m_main_thread_id;
  }
  // NOTE: here we do not use boost's as we do not want to depend on Boost
  // thread.
  /// Hardware concurrency.
  /**
   * @return number of concurrent threads supported by the environment
   * (typically equal to the number of logical CPU cores), or 0 if
   * the detection fails.
   */
  static unsigned get_hardware_concurrency() {
    ...
    return ::sysctlbyname("hw.ncpu", &count, &size, NULL, 0)
               ? 0u
  ...
  /**
   * @return data cache line size (in bytes), or 0 if the value cannot be
   * determined.
   */
  static unsigned get_cache_line_size() {
    ...
    // NOTE: here we could iterate over all cpus and take the maximum cache line
    // size.
    if (!ls) {
...
#elif defined(_WIN32) &&                                                       \
    defined(PIRANHA_HAVE_SYSTEM_LOGICAL_PROCESSOR_INFORMATION)
    // Adapted from:
    ...
    try {
      buffer = (::SYSTEM_LOGICAL_PROCESSOR_INFORMATION *)std::malloc(
          boost::numeric_cast<std::size_t>(buffer_size));
      if (unlikely(!buffer)) {
    ...
    // that size_t is used to represent sizes, and we are getting the size of
    // the cache line.
    // Compare to the usage above for the FreeBSD cpu count, were we expect
    // ints.
    std::size_t ls, size = sizeof(ls);
    try {
      return ::sysctlbyname("hw.cachelinesize", &ls, &size, NULL, 0)
                 ? 0u
                 : boost::numeric_cast<unsigned>(ls);
    } catch (...) {
      return 0u;
    }
#else

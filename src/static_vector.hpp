...
template <>
class static_vector<std::int8_t,14u>
{
		template <typename>
	...
	public:
		...
		static const size_type max_size = 14u;
	private:
		template <std::size_t Size, typename Functor, typename Iterator, std::size_t Current = 0u>
		static void unroller(const Functor &f, Iterator it,
			typename std::enable_if<Current != Size>::type * = nullptr)
		{
			f(it + Current);
			unroller<Size,Functor,Iterator,static_cast<std::size_t>(Current + 1u)>(f,it);
		}
		template <std::size_t Size, typename Functor, typename Iterator, std::size_t Current = 0u>
		static void unroller(const Functor &, Iterator,
			typename std::enable_if<Current == Size>::type * = nullptr)
		{}
		template <std::size_t Size, typename Functor, typename Iterator1, typename Iterator2,
			std::size_t Current = 0u>
		static void unroller(const Functor &f, Iterator1 it1, Iterator2 it2,
			typename std::enable_if<Current != Size>::type * = nullptr)
		{
			f(it1 + Current,it2 + Current);
			unroller<Size,Functor,Iterator1,Iterator2,static_cast<std::size_t>(Current + 1u)>(f,it1,it2);
		}
		template <std::size_t Size, typename Functor, typename Iterator1, typename Iterator2,
			std::size_t Current = 0u>
		static void unroller(const Functor &, Iterator1, Iterator2,
			typename std::enable_if<Current == Size>::type * = nullptr)
		{}
		void copy_storage_from(const static_vector &other)
		{
			unroller<max_size>([](iterator it, const_iterator it_other){*it = *it_other;},
				&m_storage[0u],&other.m_storage[0u]);
		}
	public:
		static_vector():m_tag(1u),m_size(0u)
		{
			// Zero out the whole storage.
			unroller<max_size>([](iterator it){*it = 0;},&m_storage[0u]);
		}
		static_vector(const static_vector &other):m_tag(1u),m_size(other.m_size)
		{
			// Copy the whole storage.
			copy_storage_from(other);
		}
		static_vector(static_vector &&other) noexcept : m_tag(1u),m_size(other.m_size)
		{
			// Copy the whole storage.
			copy_storage_from(other);
		}
		explicit static_vector(const size_type &n, const value_type &x):m_tag(1u),m_size(n)
		{
			// Fill in the constant values.
			std::fill(&m_storage[0u],&m_storage[0u] + m_size,x);
			// Fill in to zero the uninited values.
			std::fill(&m_storage[0u] + m_size,&m_storage[0u] + max_size,value_type(0));
		}
		...
		void emplace_back(Args && ... params)
		{
			...
			*(begin() + m_size) = value_type(std::forward<Args>(params)...);
			++m_size;
		}
		void resize(const size_type &new_size)
		{
			...
			// Need to do something only when shrinking size, when increasing the new
			// elements are already set to zero.
			if (new_size < m_size) {
				std::fill(begin() + new_size, begin() + m_size, value_type(0));
			}
			m_size = new_size;
		}
		std::size_t hash() const noexcept
...
/*
			...
			vt a0 = ::_mm_cvtepi8_epi16(::_mm_loadl_epi64((vt *)m_storage)),
				a1 = ::_mm_cvtepi8_epi16(::_mm_loadl_epi64((vt *)(m_storage + 6))),
				b0 = ::_mm_set_epi16(410,-290,70,-1030,370,-710,1030,-30), b1 = ::_mm_set_epi16(50,-110,830,-530,190,-970,410,-290);
			vt ab0 = ::_mm_mullo_epi16(a0,b0), ab1 = ::_mm_mullo_epi16(a1,b1);
			// Erase the first two elements from ab1 (redundant, as they are in ab0 already).
			ab1 = ::_mm_blend_epi16(ab1,::_mm_setzero_si128(),3);
			vt c0 = ::_mm_madd_epi16(ab0,::_mm_set1_epi16(1)), c1 = ::_mm_madd_epi16(ab1,::_mm_set1_epi16(1));
			vt c0c1 = ::_mm_hadd_epi32(c0,c1);
			c0c1 = ::_mm_hadd_epi32(c0c1,c0c1);
			c0c1 = ::_mm_hadd_epi32(c0c1,c0c1);
			alignas(16) std::int32_t tmp[4];
			//std::int32_t __attribute__ ((aligned(16))) tmp[4];
			::_mm_store_si128((vt *)tmp,c0c1);
			return static_cast<std::size_t>(tmp[0]);
*/
			/*std::size_t retval = std::hash<value_type>{}(m_storage[0u]);
	...
	private:
		unsigned char	m_tag;
		size_type	m_size;
		value_type	m_storage[max_size];
};

const static_vector<std::int8_t,14u>::size_type static_vector<std::int8_t,14u>::max_size;

...
// TMP to determine size type big enough to represent Size. The candidate types
// are the fundamental unsigned integer types.
using static_vector_size_types =
    std::tuple<unsigned char, unsigned short, unsigned, unsigned long,
               unsigned long long>;
...
struct static_vector_size_type {
  ...
  using type = typename std::conditional<
      ...
      typename static_vector_size_type<Size, static_cast<std::size_t>(
                                                 Index + 1u)>::type>::type;
};
...
struct static_vector_size_type<
    Size, static_cast<std::size_t>(
              std::tuple_size<static_vector_size_types>::value - 1u)> {
  using type = typename std::tuple_element<
      static_cast<std::size_t>(
          std::tuple_size<static_vector_size_types>::value - 1u),
      static_vector_size_types>::type;
  static_assert(std::numeric_limits<type>::max() >= Size,
...
/// Static vector class.
/**
 * This class represents a dynamic array that avoids runtime memory allocation.
 * Memory storage is provided by
 * a contiguous memory block stored within the class. The size of the internal
 * storage is enough to fit
 ...
 * ## Move semantics ##
 *
 * After a move operation, the container will be left in a state equivalent to a
...
#endif
  template <typename, typename> friend union detail::small_vector_union;
...
private:
  ...
  // NOTE: some notes about the use of raw storage, after some research in the
  // standard:
  // - storage type is guaranteed to be a POD able to hold any object whose size
  // is at most Len
  //   and alignment a divisor of Align (20.9.7.6). Thus, we can store on object
  //   of type T at the
  //   beginning of the storage;
  // - POD types are trivially copyable, and thus they occupy contiguous bytes
  // of storage (1.8/5);
  // - the meaning of "contiguous" seems not to be explicitly stated, but it can
  // be inferred
  //   (e.g., 23.3.2.1/1) that it has to do with the fact that one can do
  //   pointer arithmetics
  //   to move around in the storage;
  // - the footnote in 5.7/6 implies that casting around to different pointer
  // types and then doing
  //   pointer arithmetics has the expected behaviour of moving about with
  //   discrete offsets equal to the sizeof
  //   the pointed-to object (i.e., it seems like pointer arithmetics does not
  //   suffer from
  //   strict aliasing and type punning issues - at least as long as one goest
  //   through void *
  //   conversions to get the address of the storage - note also 3.9.2/3). This
  //   supposedly applies to arrays only,
  //   but, again, it is implied in other places that it also holds true for
  //   contiguous storage;
  // - the no-padding guarantee for arrays (which also consist of contiguous
  // storage) implies that sizeof must be
  //   a multiple of the alignment for a given type;
  // - the definition of alignment (3.11) implies that if one has contiguous
  // storage starting at an address in
  //   which T can be constructed, the offsetting the initial address by
  //   multiples of the alignment value will
  //   still produce addresses at which the object can be constructed;
  // - in general, we are assuming here that we can handle contiguous storage
  // the same way arrays can be handled (e.g.,
  //   to get the end() iterator we get one past the last element);
  // - note that placement new will work as expected (i.e., it will construct
  // the object exactly at the address passed
  //   in as parameter).
  typedef typename std::aligned_storage<sizeof(T) * MaxSize, alignof(T)>::type
      storage_type;
#endif
public:
  /// Maximum size.
  ...
  static_vector(const static_vector &other) : m_tag(1u), m_size(0u) {
    // NOTE: here and elsewhere, the standard implies (3.9/2) that we can use
    // this optimisation
    // for trivially copyable types. GCC does not support the type trait yet, so
    // we restrict the
   ...
   */
  explicit static_vector(const size_type &n, const value_type &x)
      : m_tag(1u), m_size(0u) {
    try {
   ...
   */
  iterator end() { return ptr() + m_size; }
  /// Const begin iterator.
  /**
   * @return const iterator to the first element, or end() if the vector is
   * empty.
   */
  const_iterator begin() const { return ptr(); }
  /// Const end iterator.
  /**
   * @return const iterator to the position one past the last element of the
   * vector.
   */
  const_iterator end() const { return ptr() + m_size; }
  /// Equality operator.
  /**
   * @param[in] other argument for the comparison.
   *
   * @return \p true if the sizes of the vectors are the same and all elements
   * of \p this compare as equal
   * to the elements in \p other, \p false otherwise.
   *
   * @throws unspecified any exception thrown by the comparison operator of the
   * value type.
   */
   ...
   *
   * @throws std::bad_alloc if the insertion of \p x would lead to a size
   * greater than \p MaxSize.
   * @throws unspecified any exception thrown by the copy constructor of \p T.
   */
  ...
  /**
   * \note
   * This method is enabled only if \p value_type is constructible from the
   * variadic arguments pack.
   *
   * Input parameters will be used to construct an instance of \p T at the end
   * of the container.
   *
   * @param[in] params arguments that will be used to construct the new element.
   *
   * @throws std::bad_alloc if the insertion of the new element would lead to a
   * size greater than \p MaxSize.
   * @throws unspecified any exception thrown by the constructor of \p T from
  ...
  void emplace_back(Args &&... params) {
    ...
    ::new (static_cast<void *>(ptr() + m_size))
  ...
  /// Resize.
  /**
   * After this operation, the number of elements stored in the container will
   * be \p new_size. If \p new_size is greater than
   * the size of the object before the operation, the new elements will be
   * value-initialized and placed at the end of the container.
   * If \p new_size is smaller than the size of the object before the operation,
   * the first \p new_size
  ...
  void resize(const size_type &new_size) {
    ...
    if (new_size == old_size) {
      ...
      try {
        for (; i < new_size; ++i) {
          // NOTE: placement-new syntax for value initialization, the same as
          // performed by
  ...
  iterator erase(const_iterator it) {
    // NOTE: on the use of const_cast: the object 'it' points to is *not* const
    // by definition
    // (it is an element of a vector). So the const casting should be ok. See
    // these links:
    ...
    // The return value is the original iterator, which will eventually contain
    // the value
    // following the original 'it'. If 'it' pointed to the last element, then
    // retval
    // will be the new end().
  ...
  /// Clear.
  /**
   * This method will destroy all elements of the vector and set the size to
   * zero.
   */
  void clear() {
    destroy_items();
   ...
   * - in all other cases, the result of iteratively mixing via \p
   * boost::hash_combine the hash
   ...
   *
   * @throws unspecified any exception resulting from calculating the hash
   * value(s) of the
   * individual elements.
   *
  ...
  template <typename U = T,
            typename = typename std::enable_if<is_hashable<U>::value>::type>
   ...
   *
   * @throws unspecified any exception thrown by printing to stream instances of
   * the value type of \p v.
   */
...
private:
  void destroy_items() {
    ...
    // NOTE: here we could have the destructor defined in a base class to be
    // specialised
    // if T is trivially destructible. Then we can default the dtor here and
    // static_vector
  ...
  void *vs() { return static_cast<void *>(&m_storage); }
  const void *vs() const { return static_cast<const void *>(&m_storage); }
  // Getters for T pointer.
  T *ptr() { return static_cast<T *>(vs()); }
  const T *ptr() const { return static_cast<const T *>(vs()); }
...
const typename static_vector<T, MaxSize>::size_type
    static_vector<T, MaxSize>::max_size;
}

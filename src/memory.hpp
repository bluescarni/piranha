...
// Possible implementation of aligned malloc using std::align (not yet available
// in GCC).
...
/**
 * This function will allocate a block of memory of \p size bytes aligned to \p
 * alignment.
 * If \p size is zero, \p nullptr will be returned. If \p alignment is zero, \p
 * std::malloc()
 * will be used for the allocation. Otherwise, the allocation will be deferred
 * to an implementation-defined
 * and platform-dependent low-level routine (e.g., \p posix_memalign()). If such
 * a low level routine is not
 * available, an exception will be raised.
 *
 * @param[in] alignment desired alignment.
 * @param[in] size number of bytes to allocate.
 *
 * @return a pointer to the allocated memory block, or \p nullptr if \p size is
 * zero.
 *
 * @throws std::bad_alloc if the allocation fails for any reason (e.g., bad
 * alignment value, failure
 * in the low-level allocation routine, etc.).
 * @throws piranha::not_implemented_error if \p alignment and \p size are both
 * nonzero and the
 * low-level allocation function is not available on the platform.
 */
inline void *aligned_palloc(const std::size_t &alignment,
  ...
  piranha_throw(not_implemented_error,
                "memory alignment primitives are not available");
#endif
}

/// Free memory allocated via piranha::aligned_alloc.
/**
 * This function must be used to deallocate memory obtained via
 * piranha::aligned_alloc(). If \p ptr is
 * \p nullptr, this function will be a no-op. If \p alignment is zero, \p
 * std::free() will be used. Otherwise,
 * the deallocation will be deferred to an implementation-defined
 * and platform-dependent low-level routine (e.g., \p _aligned_free()). If such
 * a low level routine is not
 * available, an exception will be raised.
 *
 ...
 *
 * @throws piranha::not_implemented_error if \p ptr is not \p nullptr, \p
 * alignment is not zero and the low-level
...
/// Alignment checks.
/**
 * This function will run a series of checks on an alignment value to be used to
 * allocate storage for objects of the decay
 * type of \p T using piranha::aligned_palloc(),
 * and it will retun \p true if the alignment value passes these checks, \p
 * false otherwise.
 ...
 * - the alignment must not be smaller than the default alignment of \p T, as
 * reported by
 *   \p alignas(),
 * - the alignment must satisfy additional platform-dependent checks (e.g., \p
 * posix_memalign() requires
 *   the alignment to be a multiple of <tt>sizeof(void *)</tt>).
 *
 * Note that piranha::aligned_palloc() will not check the alignment via this
 * function, and that even if this function returns \p true on an alignment
 * value,
 * this will not guarantee that the allocation via piranha::aligned_palloc()
 * will succeed.
 *
 * @param[in] alignment alignment value to be checked.
 *
 * @return \p true if the input value is zero or if it passes the alignment
 * checks, \p false otherwise.
 */
template <typename T>
inline bool alignment_check(const std::size_t &alignment) {
  ...
  // Alignment must not be less than the natural alignment of T. We just need
  // the '<' check
  // as we already know that alignment is either a multiple of alignof(T) or a
  // divisor.
...
/**
 * \note
 * This function is enabled only if \p T satisfies the
 * piranha::is_container_element type trait.
 *
 * This function will value-initialise in parallel the array \p ptr
 * of size \p size. The routine will use the first \p n_threads from
 * piranha::thread_pool to perform
 * the operation concurrently. If \p n_threads is 1 or 0, the operation will be
 * performed in the
 * calling thread. If \p ptr is null, this function will be a no-op.
 *
 * This function provides the strong exception safety guarantee: in case of
 * errors, any constructed
 ...
 *
 * @throws std::bad_alloc in case of memory allocation errors in multithreaded
 * mode.
 * @throws unspecified any exception thrown by:
 * - the value initialisation of instances of type \p T,
 * - piranha::thread_pool::enqueue() and piranha::future_list::push_back(), only
 * in multithreaded mode.
 */
template <typename T, typename = typename std::enable_if<
                          is_container_element<T>::value>::type>
inline void parallel_value_init(T *ptr, const std::size_t &size,
  ...
  // Initing functor.
  auto init_function = [](T *start, T *end, const unsigned &thread_idx,
                          ranges_vector *rv) {
  ...
  if (n_threads <= 1) {
    ...
    ranges_vector inited_ranges(static_cast<rv_size_type>(n_threads),
                                std::make_pair(ptr, ptr));
    ...
    try {
      for (auto i = 0u; i < n_threads; ++i) {
        auto start = ptr + i * wpt,
             end = (i == n_threads - 1u) ? ptr + size : ptr + (i + 1u) * wpt;
        f_list.push_back(thread_pool::enqueue(i, init_function, start, end, i,
                                              &inited_ranges));
      }
      f_list.wait_all();
      f_list.get_all();
 ...
 * piranha::is_container_element type trait.
 *
 * This function will destroy in parallel the element of an array \p ptr of size
 * \p size. If \p n_threads
 * is 0 or 1, the operation will be performed in the calling thread, otherwise
 * the first \p n_threads in piranha::thread_pool
 * will be used to perform the operation concurrently.
 *
 * The function is a no-op if \p ptr is null or if \p T has a trivial
 * destructor.
 *
 * @param[in] ptr pointer to the array.
 * @param[in] size size of the array.
...
template <typename T, typename = typename std::enable_if<
                          is_container_element<T>::value>::type>
inline void parallel_destroy(T *ptr, const std::size_t &size,
                             const unsigned &n_threads) {
  using ranges_vector = std::vector<std::pair<T *, T *>>;
  using rv_size_type = typename ranges_vector::size_type;
  ...
  if (n_threads <= 1u) {
    ...
    // A vector of ranges representing elements yet to be destroyed in case
    // something goes wrong
    // in the multithreaded part.
    ranges_vector d_ranges;
    future_list<decltype(destroy_function(ptr, ptr))> f_list;
    try {
      d_ranges.resize(static_cast<rv_size_type>(n_threads),
                      std::make_pair(ptr, ptr));
    ...
    try {
      // Work per thread.
      const auto wpt = static_cast<std::size_t>(size / n_threads);
      // The ranges to destroy in the cleanup phase are, initially, all the
      // ranges.
      for (auto i = 0u; i < n_threads; ++i) {
        auto start = ptr + i * wpt,
             end = (i == n_threads - 1u) ? ptr + size : ptr + (i + 1u) * wpt;
      ...
      for (auto i = 0u; i < n_threads; ++i) {
        ...
        // The range needs not to be destroyed anymore, as the enqueue/push_back
        // was successful. Replace
        // with an empty range.
      ...
      f_list.wait_all();
      // If anything failed in the multithreaded part, just destroy in
      // single-thread the ranges
...
template <typename T> class parallel_deleter {
public:
 ...
 * piranha::is_container_element type trait.
 *
 * This function will create an array whose values will be initialised in
 * parallel using piranha::parallel_value_init().
 * The pointer to the array is returned wrapped inside an \p std::unique_ptr
 * that will take care of
 * destroying the array elements (also in parallel using
 * piranha::parallel_destroy()) and deallocating the memory when the
 * destructor is called.
 *
 * \note
 * Due to the special semantics of the parallel deleter, the returned smart
 * pointer cannot be reset with another arbitrary pointer
 * without supplying a new deleter. To replace the managed object while
 * supplying a new deleter as well, move semantics may be used.
 *
 * @param[in] size size of the array.
 * @param[in] n_threads number of threads to use.
 *
 ...
 *
 * @throws std::bad_alloc if \p size is greater than an implementation-defined
 * value.
 * @throws unspecified any exception thrown by:
...
template <typename T, typename = typename std::enable_if<
                          is_container_element<T>::value>::type>
inline std::unique_ptr<T[], detail::parallel_deleter<T>>
make_parallel_array(const std::size_t &size, const unsigned &n_threads) {
  ...
  auto ptr = static_cast<T *>(
      aligned_palloc(0u, static_cast<std::size_t>(size * sizeof(T))));
  try {
  ...
  return std::unique_ptr<T[], detail::parallel_deleter<T>>(
      ptr, detail::parallel_deleter<T>{size, n_threads});
}
}

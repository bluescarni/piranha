...
template <typename T> struct is_polynomial_key {
  static const bool value = false;
};

template <typename T> struct is_polynomial_key<kronecker_monomial<T>> {
  static const bool value = true;
};

template <typename T, typename U> struct is_polynomial_key<monomial<T, U>> {
  static const bool value = true;
};

// Implementation detail to check if the monomial key supports the
// linear_argument() method.
template <typename Key> struct key_has_linarg : detail::sfinae_types {
  template <typename U>
  static auto test(const U &u)
      -> decltype(u.linear_argument(std::declval<const symbol_set &>()));
  static no test(...);
  static const bool value =
      std::is_same<std::string, decltype(test(std::declval<Key>()))>::value;
};

// Division utilities.
// Return iterator to the leading term of a polynomial, as defined by the
// operator<()
...
template <typename PType>
inline void poly_cf_mult(const typename PType::term_type::cf_type &a,
                         PType &p) {
...
// NOTE: this is always used in exact divisions, we could wrap
// mp_integer::_divexact()
// to improve performance.
template <typename PType>
inline void poly_exact_cf_div(PType &p,
                              const typename PType::term_type::cf_type &a) {
  piranha_assert(!math::is_zero(a));
  const auto it_f = p._container().end();
  for (auto it = p._container().begin(); it != it_f; ++it) {
...
template <typename PType> inline PType gcd_prs_sr(PType a, PType b) {
  using term_type = typename PType::term_type;
  using cf_type = typename term_type::cf_type;
  using key_type = typename term_type::key_type;
  // Only univariate polynomials are allowed.
  piranha_assert(a.get_symbol_set().size() == 1u &&
                 a.get_symbol_set() == b.get_symbol_set());
  ...
  while (fprime.size() != 0u && poly_lterm(fprime)->m_key != zero_key) {
    ...
    // NOTE: we are using the degree here in order to maintain compatibility
    // with
    // all monomials. There is some small overhead in this, but it should not
    // matter too much.
    // NOTE: this will be used only on monomials with integral exponents, so it
    // is always valid.
    integer delta(l2->m_key.degree(args)), l1d(l1->m_key.degree(args));
    piranha_assert(delta >= l1d);
    // NOTE: this is a pseudo-remainder operation here, we don't use the poly
    // method as we need the delta
    // information below.
    delta -= l1d;
    poly_cf_mult(math::pow(l1->m_cf, static_cast<unsigned>(delta + 1)),
                 F[static_cast<size_type>(i - 2u)]);
    fprime = PType::udivrem(F[static_cast<size_type>(i - 2u)],
                            F[static_cast<size_type>(i - 1u)])
                 .second;
    if (fprime.size() != 0u) {
...
// Establish the limits of the exponents of two polynomials. Will throw if a
// negative exponent is encountered.
// Preconditions:
// - equal symbol sets.
// - non-empty polys.
template <typename PType>
...
poly_establish_limits(const PType &n, const PType &d) {
  ...
  for (; it != it_f; ++it) {
    ...
    for (decltype(tmp.size()) i = 0u; i < tmp.size(); ++i) {
      // NOTE: greater-than comparability is ensured for all exponent types on
      // which this
      // method will be used.
  ...
  if (std::any_of(retval.begin(), retval.end(),
                  [](const std::pair<expo_type, expo_type> &p) {
                    return p.first < expo_type(0);
                  })) {
    piranha_throw(std::invalid_argument, "negative exponents are not allowed");
  }
  return retval;
...
inline auto poly_dot_product(It1 begin1, It1 end1, It2 begin2)
    -> decltype((*begin1) * (*begin2)) {
...
inline std::tuple<polynomial<Cf, k_monomial>, polynomial<Cf, k_monomial>,
                  std::vector<typename k_monomial::value_type>>
poly_to_univariate(const polynomial<Cf, Key> &n, const polynomial<Cf, Key> &d) {
  ...
  std::transform(
      limits.begin(), limits.end(), std::back_inserter(max_expos),
      [](const std::pair<expo_type, expo_type> &p) { return p.second; });
  using me_size_type = decltype(max_expos.size());
  // Next we need to check the limits of the codification. We do everything in
  // multiprecision and
  ...
  for (decltype(args.size()) i = 0u; i < args.size(); ++i) {
    mp_cv.push_back(mp_cv.back() *
                    (integer(1) + max_expos[static_cast<me_size_type>(i)]));
  }
  // Determine the max univariate expo.
  integer mp_kM =
      poly_dot_product(max_expos.begin(), max_expos.end(), mp_cv.begin());
  ...
  auto encode_poly =
      [&args, &cv](const polynomial<Cf, Key> &p) -> polynomial<Cf, k_monomial> {
    ...
    for (const auto &t : p._container()) {
      ...
      retval.insert(term_type{
          t.m_cf, k_monomial(static_cast<k_expo_type>(poly_dot_product(
                      tmp_expos.begin(), tmp_expos.end(), cv.begin())))});
    }
    return retval;
  };
...
poly_from_univariate(const polynomial<Cf, k_monomial> &n,
  ...
  auto decode_poly =
      [&args, &cv](const polynomial<Cf, k_monomial> &p) -> polynomial<Cf, Key> {
    ...
    for (const auto &t : p._container()) {
      for (v_size_type i = 0u; i < tmp_expos.size(); ++i) {
        // NOTE: use construction here rather than static_cast as expo_type
        // could be integer.
        tmp_expos[i] = expo_type(
            (t.m_key.get_int() % cv[static_cast<cv_size_type>(i + 1u)]) /
...
// This implementation is taken straight from the Geddes book. We used to have
// an implementation based on the paper
// from Liao & Fateman, which supposedly is (or was) implemented in Maple, but
// that implementation had a bug which showed
// up very rarely. Strangely enough, the sympy implementation of heugcd (also
// based on the Liao paper) has exactly the same
// issue. Thus the motivation to use the implementation from Geddes.
// This function returns:
// - a status flag, to signal if the heuristic failed (true if failure
// occurred),
// - the GCD and the 2 cofactors (a / GCD and b / GCD).
// The function can also fail via throwing gcdheu_failure, which is a different
// type of failure from the status flag (the
// gcdheu_failure signals that too large integers are being generated, the
// status flag signals that too many iterations
// have been run). This difference is of no consequence for the user, as the
// exception is caught in try_gcdheu.
template <typename Poly>
std::pair<bool, std::tuple<Poly, Poly, Poly>>
gcdheu_geddes(const Poly &a, const Poly &b,
              symbol_set::size_type s_index = 0u) {
  static_assert(is_mp_integer<typename Poly::term_type::cf_type>::value,
                "Invalid type.");
  ...
  // This is the case in which the polynomials are reduced to two integers
  // (essentially, the zerovariate case).
  if (a.is_single_coefficient() && b.is_single_coefficient()) {
    piranha_assert(a.size() == 1u && b.size() == 1u);
    Poly retval;
    retval.set_symbol_set(args);
    auto gab = math::gcd(a_cont, b_cont);
    // NOTE: the creation of the gcd retval and of the cofactors can be improved
    // performance-wise
    // (e.g., exact division of coefficients).
    return std::make_pair(false,
                          std::make_tuple(retval + gab, a / gab, b / gab));
  }
  ...
  // NOTE: the values of 6 iterations is taken straight from the original
  // implementation of the algorithm.
  // The bits limit is larger than the original implementation, on the grounds
  // that GMP should be rather efficient
  // at bignum. It might as well be that other tuning params are better, but
  // let's keep them hardcoded for now.
  for (int i = 0; i < 6; ++i) {
    if (integer(xi.bits_size()) * std::max(ap.degree({var}), bp.degree({var})) >
        100000) {
      piranha_throw(gcdheu_failure, );
    }
    auto res = gcdheu_geddes(ap.subs(var, xi), bp.subs(var, xi),
                             static_cast<symbol_set::size_type>(s_index + 1u));
    if (!res.first) {
      ...
      // NOTE: I don't know if this is possible at all, but it's better to be
      // safe than
      ...
      try {
        ...
        auto cf_a = ap / G;
        auto cf_b = bp / G;
        // Need to multiply by the GCD of the contents of a and b in order to
        // produce
        // the GCD with the largest coefficients.
        const auto F = math::gcd(a_cont, b_cont);
        poly_cf_mult(F, G);
        // The cofactors above have been computed with the primitive parts of G
        // and a,b.
        // We need to rescale them in order to find the true cofactors, that is,
        // a / G
        // and b / G.
        ...
        return std::make_pair(
            false,
            std::make_tuple(std::move(G), std::move(cf_a), std::move(cf_b)));
...
template <typename T> using cf_t = typename T::term_type::cf_type;
...
using enabler = typename std::enable_if<
    std::is_base_of<polynomial_tag, T>::value && std::is_same<T, U>::value &&
...
/// Polynomial class.
/**
 * This class represents multivariate polynomials as collections of multivariate
 * polynomial terms.
 * \p Cf represents the ring over which the polynomial is defined, while \p Key
 * represents the monomial type.
 *
 * Polynomials support an automatic degree-based truncation mechanism, disabled
 * by default, which comes into play during
 * polynomial multiplication. It allows to discard automatically all those
 * terms, generated during series multiplication,
 * whose total or partial degree is greater than a specified limit. This
 * mechanism can be configured via a set of
 * thread-safe static methods, and it is enabled if:
 * - the total and partial degree of the series are represented by the same type
 * \p D,
 * - all the truncation-related requirements in piranha::power_series are
 * satsified,
 * - the type \p D is subtractable and the type resulting from the subtraction
 * is still \p D.
 *
 * This class satisfies the piranha::is_series and piranha::is_cf type traits.
 *
 * \warning
 * The division and GCD operations are known to have poor performance,
 * especially with large operands. Performance
 * will be improved in future versions.
 *
 * ## Type requirements ##
 *
 * \p Cf must be suitable for use in piranha::series as first template argument,
 * \p Key must be an instance of either piranha::monomial or
 * piranha::kronecker_monomial.
 *
 * ## Exception safety guarantee ##
 *
 * This class provides the same guarantee as the base series type it derives
 * from.
 *
 * ## Move semantics ##
 *
 * Move semantics is equivalent to the move semantics of the base series type it
 * derives from.
 *
 * ## Serialization ##
 *
 * This class supports serialization if the underlying coefficient and key types
 * do.
 */
template <typename Cf, typename Key>
class polynomial
    : public power_series<
          trigonometric_series<ipow_substitutable_series<
              substitutable_series<
                  t_substitutable_series<series<Cf, Key, polynomial<Cf, Key>>,
                                         polynomial<Cf, Key>>,
                  polynomial<Cf, Key>>,
              polynomial<Cf, Key>>>,
          polynomial<Cf, Key>>,
  ...
  template <typename, typename> friend class polynomial;
  // Make friend with Poisson series.
  template <typename> friend class poisson_series;
  // Make friend with divisor series.
  template <typename, typename> friend class divisor_series;
  // The base class.
  ...
  // String constructor.
  template <typename Str> void construct_from_string(Str &&str) {
  ...
  template <typename T = Key,
            typename std::enable_if<detail::key_has_linarg<T>::value &&
                                        has_safe_cast<integer, Cf>::value,
                                    int>::type = 0>
  std::map<std::string, integer> integral_combination() const {
    try {
      ...
      for (auto it = this->m_container.begin(); it != this->m_container.end();
           ++it) {
        const std::string lin_arg =
            it->m_key.linear_argument(this->m_symbol_set);
  ...
  template <typename T = Key,
            typename std::enable_if<!detail::key_has_linarg<T>::value ||
                                    ...
                                    int>::type = 0>
  std::map<std::string, integer> integral_combination() const {
    piranha_throw(std::invalid_argument, "the polynomial type does not support "
                                         "the extraction of a linear "
                                         "combination");
  }
  // Integration utils.
  // Empty for SFINAE.
  template <typename T, typename = void> struct integrate_type_ {};
  ...
  using key_integrate_type =
      decltype(std::declval<const typename T::term_type::key_type &>()
                   .integrate(std::declval<const symbol &>(),
                              std::declval<const symbol_set &>())
                   .first);
  // Basic integration requirements for series T, to be satisfied both when the
  ...
  using basic_integrate_requirements = typename std::enable_if<
      ...
      has_is_zero<decltype(
          math::partial(std::declval<const typename T::term_type::cf_type &>(),
  ...
  using i_cf_type = decltype(
      math::integrate(std::declval<const typename T::term_type::cf_type &>(),
                      std::declval<const std::string &>()));
  // Type above, multiplied by the type coming out of the derivative of the key.
  template <typename T>
  using i_cf_type_p = decltype(std::declval<const i_cf_type<T> &>() *
                               std::declval<const key_partial_type<T> &>());
  // Final series type.
  template <typename T>
  using ic_res_type = decltype(std::declval<const i_cf_type_p<T> &>() *
                               std::declval<const T &>());
  template <typename T>
  struct integrate_type_<
      T, typename std::enable_if<
             is_integrable<typename T::term_type::cf_type>::value &&
             detail::true_tt<
                 basic_integrate_requirements<T, ic_res_type<T>>>::value &&
             // We need to be able to add the non-integrable type.
             is_addable_in_place<ic_res_type<T>, nic_res_type<T>>::value &&
             // We need to be able to compute the partial degree and cast it to
             // integer.
             has_safe_cast<
                 integer,
                 decltype(
             ...
             // We need to be able to assign the integrated coefficient times
             // key partial.
  ...
  // Final typedef.
  template <typename T>
  using integrate_type = typename std::enable_if<
      is_returnable<typename integrate_type_<T>::type>::value,
      typename integrate_type_<T>::type>::type;
  // Integration with integrable coefficient.
  template <typename T = polynomial>
  integrate_type<T> integrate_impl(const symbol &s,
    ...
    try {
      ...
      piranha_throw(std::invalid_argument, "unable to perform polynomial "
                                           "integration: cannot extract the "
                                           "integral form of an exponent");
    }
    // If the degree is negative, integration by parts won't terminate.
    if (degree.sign() < 0) {
      piranha_throw(std::invalid_argument, "unable to perform polynomial "
                                           "integration: negative integral "
                                           "exponent");
    }
    polynomial tmp;
    ...
    for (integer i(1); i <= degree; ++i) {
      // Update coefficient and key. These variables are persistent across loop
      // iterations.
  ...
  template <typename T = polynomial>
  integrate_type<T> integrate_impl(const symbol &,
                                   const typename base::term_type &,
                                   const std::false_type &) const {
    piranha_throw(std::invalid_argument, "unable to perform polynomial "
                                         "integration: coefficient type is not "
                                         "integrable");
  }
  // Template alias for use in pow() overload. Will check via SFINAE that the
  // base pow() method can be called with argument T
  ...
  template <typename T>
  using pdegree_type =
      decltype(math::degree(std::declval<const T &>(),
                            std::declval<const std::vector<std::string> &>()));
  // Enablers for auto-truncation: degree and partial degree must be the same,
  // series must support math::truncate_degree(), degree type
  // must be subtractable and yield the same type.
  template <typename T>
  using at_degree_enabler = typename std::enable_if<
      std::is_same<degree_type<T>, pdegree_type<T>>::value &&
          ...
          std::is_same<decltype(std::declval<const degree_type<T> &>() -
                       ...
                       degree_type<T>>::value,
      int>::type;
  // For the setter, we need the above plus we need to be able to convert safely
  // U to the degree type.
  template <typename T, typename U>
  using at_degree_set_enabler =
      typename std::enable_if<detail::true_tt<at_degree_enabler<T>>::value &&
                                  has_safe_cast<degree_type<T>, U>::value,
                              int>::type;
  // This needs to be separate from the other static inits because we don't have
  // anything to init
  // if the series does not support degree computation.
  // NOTE: here the important thing is that this method does not
  // return the same object for different series types, as the intent of the
  // truncation mechanism is that each polynomial type
  // has its own settings.
  // We need to keep this in mind if we need static resources that must be
  // unique for the series type, sometimes adding the Derived
  // series as template argument in a toolbox might actually be necessary
  // because of this. Note that, contrary to the, e.g., custom
  // derivatives map in series.hpp here we don't care about the type of T - we
  // just need to be able to extract the term type
  // from it.
  template <typename T = polynomial>
  ...
  using str_enabler = typename std::enable_if<
  ...
  using find_cf_enabler = typename std::enable_if<
      std::is_constructible<typename base::term_type::key_type,
                            decltype(std::begin(std::declval<const T &>())),
                            decltype(std::end(std::declval<const T &>())),
                            const symbol_set &>::value &&
  ...
  Cf find_cf_impl(Iterator begin, Iterator end) const {
    typename base::term_type tmp_term{Cf(0),
                                      Key(begin, end, this->m_symbol_set)};
  ...
  template <typename T> using cf_t = typename T::term_type::cf_type;
  template <typename T> using key_t = typename T::term_type::key_type;
  template <typename T>
  using expo_t = typename T::term_type::key_type::value_type;
  ...
  static std::pair<polynomial, polynomial> udivrem_impl(const T &n,
                                                        const T &d) {
    using term_type = typename base::term_type;
    using cf_type = typename term_type::cf_type;
    using key_type = typename term_type::key_type;
    // Only univariate polynomials are allowed.
    if (unlikely(n.get_symbol_set().size() != 1u ||
                 n.get_symbol_set() != d.get_symbol_set())) {
      piranha_throw(
          std::invalid_argument,
          "only univariate polynomials in the same variable "
          "are allowed in the polynomial division with remainder routine");
    }
    if (unlikely(d.size() == 0u)) {
      piranha_throw(zero_division_error,
                    "univariate polynomial division by zero");
    }
    ...
    while (true) {
      ...
      // it gets to zero. This sounds exactly like the kind of situation in
      // which
      ...
      // We should consider adding a re-hashing every time we do a +/- operation
      // on
      // a series and the load factor gets below a certain threshold. We should
      // also
      // review uses of the insert() method of series to spot other possible
      // places where this could be a problem (although once we have a good
      // behaviour
      // for +/- we should be mostly good - multiplication should already be
      // ok).
      r -= term_mult(t, d);
      q.insert(std::move(t));
    }
  ...
  template <typename T> static T division_impl(const T &n, const T &d) {
    ...
    // NOTE: here we need to check a few things as the mapping to univariate has
    // certain prereqs.
    ...
    // Deal with the case in which the number of arguments is zero. We need to
    // do it here as the mapping
    ...
    if (args.size() == 0u) {
      ...
      retval.set_symbol_set(args);
      Cf tmp_cf;
      math::divexact(tmp_cf, n._container().begin()->m_cf,
                     d._container().begin()->m_cf);
    ...
    auto ures = std::get<0u>(umap).template udivrem_impl<false>(
        std::get<0u>(umap), std::get<1u>(umap));
    ...
    auto retval =
        detail::poly_from_univariate<Key>(ures.first, std::get<2u>(umap), args);
    // Final check: for each variable, the degree of retval + degree of d must
    // be equal to the degree of n.
    auto degree_vector_extractor =
        [&args](const polynomial &p) -> std::vector<expo_type> {
    ...
    const auto n_dv = degree_vector_extractor(n),
               d_dv = degree_vector_extractor(d),
  ...
  // NOTE: the further constraining here that T must be polynomial is to work
  // around a GCC < 5 (?) bug,
  // in which template operators from other template instantiations of
  // polynomial<> are considered. We can
  // remove this once we bump the minimum version of GCC required.
  template <typename T, typename U = T>
  using poly_div_enabler =
      enable_if_t<std::is_same<decay_t<T>, polynomial>::value,
                  detail::ptd::enabler<decay_t<T>, decay_t<U>>>;
  // Enabler for in-place division.
  template <typename T, typename U>
  using poly_in_place_div_enabler =
      enable_if_t<detail::true_tt<poly_div_enabler<T, U>>::value &&
                      !std::is_const<T>::value,
                  int>;
  // Join enabler.
  template <typename T>
  using join_enabler = typename std::enable_if<
      std::is_base_of<detail::polynomial_tag, cf_t<T>>::value &&
  ...
  // Content enabler.
  template <typename T>
  using content_enabler =
      typename std::enable_if<has_gcd3<cf_t<T>>::value, int>::type;
  // Primitive part enabler.
  template <typename T>
  using pp_enabler =
      typename std::enable_if<detail::true_tt<content_enabler<T>>::value &&
                                  has_exact_division<cf_t<T>>::value,
                              int>::type;
  // Prem enabler.
  template <typename T>
  ...
  using gcd_enabler = typename std::enable_if<
      has_gcd<cf_t<T>>::value && has_gcd3<cf_t<T>>::value &&
          ...
          std::is_same<decltype(std::declval<const cf_t<T> &>() *
                       ...
                       cf_t<T>>::value &&
          has_exact_division<cf_t<T>>::value &&
          std::is_copy_assignable<cf_t<T>>::value &&
          detail::true_tt<content_enabler<T>>::value &&
          detail::true_tt<poly_div_enabler<T>>::value,
      int>::type;
  // Height.
  template <typename T>
  ...
  template <typename T>
  using height_type = typename std::enable_if<
      std::is_constructible<height_type_<T>, int>::value &&
      ...
      height_type_<T>>::type;
  // Wrapper around heuristic GCD. It will return false,tuple if the calculation
  // went well,
  // true,tuple otherwise. If run on polynomials with non-integer coefficients,
  // it will throw
  // if the requested algorithm is specifically the heuristic one.
  template <typename T,
            typename std::enable_if<detail::is_mp_integer<cf_t<T>>::value,
                                    int>::type = 0>
  static std::pair<bool, std::tuple<T, T, T>>
  try_gcdheu(const T &a, const T &b, polynomial_gcd_algorithm) {
    try {
  ...
  template <typename T,
            typename std::enable_if<!detail::is_mp_integer<cf_t<T>>::value,
                                    int>::type = 0>
  static std::pair<bool, std::tuple<T, T, T>>
  try_gcdheu(const T &, const T &, polynomial_gcd_algorithm algo) {
    // The idea here is that this is a different kind of failure from the one we
    // throw in the main gcd()
    // function, and we want to be able to discriminate the two.
    if (algo == polynomial_gcd_algorithm::heuristic) {
      piranha_throw(
          std::runtime_error,
          "the heuristic polynomial GCD algorithm was explicitly selected, "
          "but it cannot be applied to non-integral coefficients");
    }
    return std::make_pair(true, std::tuple<T, T, T>{});
  }
  // This is a wrapper to compute and return the cofactors, together with the
  // GCD, when the PRS algorithm
  // is used (the gcdheu algorithm already computes the cofactors).
  template <typename T, typename U>
  static std::tuple<T, T, T> wrap_gcd_cofactors(U &&gcd, const T &a, const T &b,
                                                bool with_cofactors) {
    if (with_cofactors) {
      if (math::is_zero(gcd)) {
...
public:
  /// Series rebind alias.
  template <typename Cf2> using rebind = polynomial<Cf2, Key>;
  ...
  /**
   * \note
   * This template constructor is enabled only if the decay type of \p Str is a
   * C or C++ string.
   *
   * Will construct a univariate polynomial made of a single term with unitary
   * coefficient and exponent, representing
   * the symbolic variable \p name. The type of \p name must be a string type
   * (either C or C++).
   *
   * @param[in] name name of the symbolic variable that the polynomial will
   * represent.
   *
  ...
  /**
   * \note
   * This method is enabled only if piranha::series::pow() can be called with
   * exponent \p x
   * and the key type can be raised to the power of \p x via its exponentiation
   * method.
   *
   * This exponentiation override will check if the polynomial consists of a
   * single-term with non-unitary
   * key. In that case, the return polynomial will consist of a single term with
   * coefficient computed via
   * piranha::math::pow() and key computed via the monomial exponentiation
   * method.
   *
  ...
  pow_ret_type<T, Series> pow(const T &x) const {
    ...
    typedef typename term_type::key_type key_type;
    if (this->size() == 1u &&
        !this->m_container.begin()->m_key.is_unitary(this->m_symbol_set)) {
    ...
    return static_cast<series<Cf, Key, polynomial<Cf, Key>> const *>(this)->pow(
        x);
  }
  /// Inversion.
  /**
   * \note
   * This method is enabled only if <tt>piranha::polynomial::pow(-1)</tt> is a
   * well-formed
   * expression.
   *
   * @return the calling polynomial raised to -1 using
   * piranha::polynomial::pow().
   *
   * @throws unspecified any exception thrown by piranha::polynomial::pow().
   */
  template <typename Series = polynomial> inverse_type<Series> invert() const {
    return this->pow(-1);
  }
  /// Integration.
  /**
   * \note
   * This method is enabled only if the algorithm described below is supported
   * by all the involved types.
   *
   * This method will attempt to compute the antiderivative of the polynomial
   * term by term. If the term's coefficient does not depend on
   * the integration variable, the result will be calculated via the integration
   * of the corresponding monomial.
   * Integration with respect to a variable appearing to the power of -1 will
   * fail.
   *
   * Otherwise, a strategy of integration by parts is attempted, its success
   * depending on the integrability
   * of the coefficient and on the value of the exponent of the integration
   * variable. The integration will
   ...
   * - piranha::math::partial(), piranha::math::is_zero(),
   * piranha::math::integrate(), piranha::safe_cast()
   *   and piranha::math::negate(),
   * - piranha::symbol_set::add(),
   * - term construction,
   * - coefficient construction, assignment and arithmetics,
   * - integration, construction, assignment, differentiation and degree
   * querying methods of the key type,
   * - insert(),
   * - series arithmetics.
   */
  template <typename T = polynomial>
    ...
    for (auto it = this->m_container.begin(); it != it_f; ++it) {
      ...
      if (math::is_zero(math::partial(it->m_cf, name))) {
        ...
        retval += integrate_impl(
            s, *it,
            std::integral_constant<bool, is_integrable<cf_type>::value>());
      }
    }
    return retval;
  }
  ...
  /**
   * \note
   * This method is available only if the requisites outlined in
   * piranha::polynomial are satisfied
   * and if \p U can be safely cast to the degree type.
   *
   * Setup the degree-based auto-truncation mechanism to truncate according to
   * the total maximum degree.
   *
   * @param[in] max_degree maximum total degree that will be retained during
   ...
   */
  template <typename U, typename T = polynomial,
            at_degree_set_enabler<T, U> = 0>
  static void set_auto_truncate_degree(const U &max_degree) {
    ...
    // Initialisation of function-level statics is thread-safe, no need to lock.
    // We get
    // a ref before the lock because the initialisation of the static could
    // throw in principle,
    ...
    // This should not throw (a vector of strings, destructors and deallocation
    // should be noexcept).
   ...
   * the partial degree.
   *
   * @param[in] max_degree maximum partial degree that will be retained during
   * automatic truncation.
   * @param[in] names names of the variables that will be considered during the
   * computation of the
   * partial degree.
   *
   * @throws unspecified any exception thrown by:
   ...
   * piranha::polynomial are satisfied.
   *
   * This method will return a tuple of three elements describing the status of
   * the degree-based auto-truncation mechanism.
   * The elements of the tuple have the following meaning:
   * - truncation mode (0 if disabled, 1 for total-degree truncation and 2 for
   * partial-degree truncation),
   * - the maximum degree allowed,
   * - the list of names to be considered for partial truncation.
   *
   * @return a tuple representing the status of the degree-based auto-truncation
   * mechanism.
   *
   * @throws unspecified any exception thrown by threading primitives or by the
   * involved constructors.
   */
  template <typename T = polynomial, at_degree_enabler<T> = 0>
  static std::tuple<int, degree_type<T>, std::vector<std::string>>
  get_auto_truncate_degree() {
    std::lock_guard<std::mutex> lock(s_at_degree_mutex);
    return std::make_tuple(s_at_degree_mode, get_at_degree_max(),
                           s_at_degree_names);
  }
  /// Find coefficient.
  /**
   ...
   * - \p Key can be constructed from the begin/end iterators of \p c and a
   * piranha::symbol_set.
   *
   * This method will first construct a term with zero coefficient and key
   * initialised from the begin/end iterators
   * of \p c and the symbol set of \p this, and it will then try to locate the
   * term inside \p this.
   * If the term is found, its coefficient will be returned. Otherwise, a
   * coefficient initialised
   * from 0 will be returned.
   *
   * @param[in] c the container that will be used to construct the \p Key to be
   * located.
   *
   * @returns the coefficient of the term whose \p Key corresponds to \p c if
   * such term exists,
   * zero otherwise.
   *
   * @throws unspecified any exception thrown by:
   ...
   */
  template <typename T, find_cf_enabler<T> = 0> Cf find_cf(const T &c) const {
    return find_cf_impl(std::begin(c), std::end(c));
  }
  /// Find coefficient.
  /**
   * \note
   * This method is enabled only if \p Key can be constructed from the begin/end
   * iterators of \p l and a piranha::symbol_set.
   *
   * This method is identical to the other overload with the same name, and it
   * is provided for convenience.
   *
   * @param[in] l the list that will be used to construct the \p Key to be
   * located.
   *
   * @returns the coefficient of the term whose \p Key corresponds to \p l if
   * such term exists,
   * zero otherwise.
   *
  ...
  /// Split polynomial.
  /**
   * This method will split the first symbolic argument of the polynomial. That
   * is, it will return a univariate
   * representation of \p this in which the only symbolic argument is the first
   * symbol in the symbol set of \p this,
   * and the coefficients are multivariate polynomials in the remaining
   * variables.
   *
   * @return a representation of \p this as a univariate polynomial with
   * multivariate polynomial coefficients.
   *
   * @throws std::invalid_argument if \p this does not have at least 2 symbolic
   * arguments.
   * @throws unspecified: any exception thrown by:
  ...
  polynomial<polynomial<Cf, Key>, Key> split() const {
    if (unlikely(this->get_symbol_set().size() < 2u)) {
      piranha_throw(
          std::invalid_argument,
          "a polynomial needs at least 2 arguments in order to be split");
    }
    using r_term_type =
        typename polynomial<polynomial<Cf, Key>, Key>::term_type;
    using term_type = typename base::term_type;
    polynomial<polynomial<Cf, Key>, Key> retval;
    retval.set_symbol_set(symbol_set(this->get_symbol_set().begin(),
                                     this->get_symbol_set().begin() + 1));
    symbol_set tmp_ss(this->get_symbol_set().begin() + 1,
                      this->get_symbol_set().end());
  ...
  /**
   * \note
   * This method is enabled only if the coefficient type \p Cf is a polynomial
   * whose key type is \p Key, and which supports
   * the expression <tt>a += b * c</tt> where \p a, \p b and \p c are all
   * instances of \p Cf).
   *
   * This method is the opposite of piranha::polynomial::split(): given a
   * polynomial with polynomial coefficients,
   * it will produce a multivariate polynomial in which the arguments of \p this
   * and the arguments of the coefficients
   * have been joined.
   *
   ...
   */
  template <typename T = polynomial, join_enabler<T> = 0> Cf join() const {
  ...
  /**
   * \note
   * This method is enabled only if the coefficient type satisfies
   * piranha::has_gcd3.
   *
   * This method will return the GCD of the polynomial's coefficients. If the
   * polynomial
   * is empty, zero will be returned.
   *
   * @return the content of \p this.
   *
   * @throws unspecified any exception thrown by the polynomial constructor from
   * \p int
   ...
   * @throws unspecified any exception thrown by the division operation or by
  ...
  polynomial primitive_part() const {
    ...
    if (unlikely(math::is_zero(c))) {
      piranha_throw(zero_division_error,
                    "the content of the polynomial is zero");
    }
   ...
   * - the polynomial coefficient type \p Cf is multipliable yielding the same
   * type \p Cf, it is multipliable in-place and
   *   divisible exactly, and it has exact ring operations,
   * - the polynomial type is subtractable in-place,
   * - the exponent type of the monomial is a C++ integral type or an instance
   * of piranha::mp_integer.
   *
   * This method will return a pair representing the quotient and the remainder
   * of the division of the univariate polynomials
   * \p n and \p d. The input polynomials must be univariate in the same
   * variable.
   *
   * @param[in] n the numerator.
   * @param[in] d the denominator.
   *
   * @return the quotient and remainder of the division of \p n by \p d,
   * represented as a standard pair.
   *
   * @throws std::invalid_argument if:
   * - the input polynomials are not univariate or univariate in different
   * variables, or
  ...
  static std::pair<polynomial, polynomial> udivrem(const polynomial &n,
                                                   const polynomial &d) {
  ...
  /**
   * \note
   * This static method is enabled only if polynomial division is enabled, and
   * the polynomial
   * coefficient type is exponentiable to \p unsigned, yielding the coefficient
   * type as a result.
   *
   ...
   * \f]
   * where \f$\mathrm{lc}\f$ denotes the leading coefficient (that is, the
   * coefficient of the term with the
   * highest univariate degree), and \f$a\f$ and \f$b\f$ are the degrees of \p n
   * and \p d respectively.
   *
   * This method works only on univariate polyomials in the same variable and it
   * requires the univariate degree of \p n to be not less than
   * the univariate degree of \p d. If \p n is zero, an empty polynomial will be
   * returned.
   *
   * @param[in] n the numerator.
   * @param[in] d the denominator.
   ...
   *
   * @throws std::invalid_argument if the polynomials are not univariate in the
   * same variable, or if the univariate
  ...
  static polynomial uprem(const polynomial &n, const polynomial &d) {
    ...
    const auto &args = n.get_symbol_set();
    // NOTE: this check is repeated in divrem, we do it here as we want to get
    // the leading term in d.
    if (unlikely(d.size() == 0u)) {
      piranha_throw(zero_division_error,
                    "univariate polynomial division by zero");
    }
    ...
    integer dn(detail::poly_lterm(n)->m_key.degree(args)),
        dd(ld->m_key.degree(args));
    if (dd > dn) {
      piranha_throw(
          std::invalid_argument,
          "the degree of the denominator is greater than "
          "the degree of the numerator in the polynomial uprem routine");
    }
    // NOTE: negative degrees will be caught by udivrem.
    auto n_copy(n);
    detail::poly_cf_mult(
        math::pow(ld->m_cf, static_cast<unsigned>(dn - dd + 1)), n_copy);
  ...
  /**
   * \note
   * This operator is enabled only if the decay types of \p T and \p U are the
   * same type \p Td
   * and polynomial::udivrem() is enabled for \p Td.
   *
   * This operator will compute the exact result of <tt>n / d</tt>. If \p d does
   * not divide \p n exactly, an error will be produced.
   *
   * @param[in] n the numerator.
   * @param[in] d the denominator.
   *
   ...
   * @throws unspecified any exception thrown by:
   * - piranha::polynomial::udivrem(),
   * - the public interface of piranha::hash_set, piranha::series,
   * piranha::symbol_set,
  ...
  /**
   * \note
   * This operator is enabled only if operator/() is enabled for the input types
   * and \p T
   * is not const.
   *
  ...
  /**
   * The default value initialised on startup is
   ...
   * This method is thread-safe.
   *
   * @param[in] algo the desired default algorithm to be used for GCD
   * computations.
   */
  ...
  /**
   * This method will set the default algorithm to be used for GCD computations
   * to polynomial_gcd_algorithm::automatic.
   * This method is thread-safe.
   */
   ...
   * - the coefficient type has math::gcd() and math::gcd3(), it is
   * exponentiable to \p unsigned yielding
   *   the coefficient type, it is multipliable in-place and in binary form,
   * yielding the coefficient type,
   *   it has math::divexact(), and it is copy-assignable,
   * - the polynomial type has piranha::polynomial::content() and
   * piranha::polynomial::udivrem().
   *
   * This method will compute the GCD of polynomials \p a and \p b. The
   * algorithm that will be employed
   * for the computation is selected by the \p algo flag. If \p algo is set to
   * polynomial_gcd_algorithm::automatic
   * the heuristic GCD algorithm will be tried first, followed by the PRS SR
   * algorithm in case of failures.
   * If \p algo is set to any other value, the selected algorithm will be used.
   * The heuristic GCD algorithm can be
   * used only when the ceofficient type is an instance of piranha::mp_integer.
   * The default value for \p algo
   * is the one returned by get_default_gcd_algorithm().
   *
   * The \p with_cofactors flag signals whether the cofactors should be returned
   * together with the GCD or not.
   *
   * @param[in] a first argument.
   * @param[in] b second argument.
   * @param[in] with_cofactors flag to signal that cofactors must be returned as
   * well.
   * @param[in] algo the GCD algorithm.
   *
   * @return a tuple containing the GCD \p g of \p a and \p b, and the cofactors
   * (that is, <tt>a / g</tt> and <tt>b / g</tt>),
   * if requested. If the cofactors are not requested, the content of the last
   * two elements of the tuple is unspecified. If both input
   * arguments are zero, the cofactors will be zero as well.
   *
   * @throws std::invalid_argument if a negative exponent is encountered in \p a
   * or \p b.
   * @throws std::overflow_error in case of (unlikely) integral overflow errors.
   * @throws std::runtime_error if \p algo is
   * polynomial_gcd_algorithm::heuristic and the execution
   * of the algorithm fails or it the coefficient type is not an instance of
   * piranha::mp_integer.
   * @throws unspecified any exception thrown by:
   * - the public interface of piranha::series, piranha::symbol_set,
   * piranha::hash_set,
   * - construction of keys,
   * - construction, arithmetics and other operations of coefficients and
   * polynomials,
  ...
  gcd(const polynomial &a, const polynomial &b, bool with_cofactors = false,
    ...
    if (algo == polynomial_gcd_algorithm::automatic ||
        algo == polynomial_gcd_algorithm::heuristic) {
      auto heu_res = try_gcdheu(*real_a, *real_b, algo);
      if (heu_res.first && algo == polynomial_gcd_algorithm::heuristic) {
        // This can happen only if the heuristic fails due to the number of
        // iterations. Calling the heuristic
        // with non-integral coefficients already throws from the try_gcdheu
        // implementation.
        piranha_throw(
            std::runtime_error,
            "the heuristic polynomial GCD algorithm was explicitly selected, "
    ...
    if (args.size() == 1u) {
      return wrap_gcd_cofactors(detail::gcd_prs_sr(*real_a, *real_b), *real_a,
                                *real_b, with_cofactors);
    }
    // Zerovariate case. We need to handle this separately as the use of split()
    // below
    // requires a nonzero number of arguments.
    if (args.size() == 0u) {
      if (real_a->size() == 0u && real_b->size() == 0u) {
        return wrap_gcd_cofactors(polynomial{}, *real_a, *real_b,
                                  with_cofactors);
      }
      if (real_a->size() == 0u) {
      ...
      Cf g(0);
      math::gcd3(g, real_a->_container().begin()->m_cf,
                 real_b->_container().begin()->m_cf);
      return wrap_gcd_cofactors(polynomial(std::move(g)), *real_a, *real_b,
                                with_cofactors);
    }
    // The general multivariate case.
    return wrap_gcd_cofactors(
        detail::gcd_prs_sr(real_a->split(), real_b->split()).join(), *real_a,
        *real_b, with_cofactors);
    // NOTE: an older implementation that replaces the recursive call in the
    // line immediately
   ...
   * - the coefficient type supports piranha::math::abs(), yielding the type
   * <tt>height_type<T></tt>,
   * - <tt>height_type<T></tt> is constructible from \p int, less-than
   * comparable, move-assignable and copy or move constructible.
   *
   * The height of a polynomial is defined as the maximum of the absolute values
   * of the coefficients. If the polynomial
   * is empty, zero will be returned.
   *
   * @return the height of \p this.
   *
   * @throws unspecified any exception thrown by the construction, assignment,
   * comparison, and absolute value calculation
   * of <tt>height_type<T></tt>.
   */
  template <typename T = polynomial> height_type<T> height() const {
...
template <typename Cf, typename Key>
std::atomic<polynomial_gcd_algorithm>
    polynomial<Cf, Key>::s_def_gcd_algo(polynomial_gcd_algorithm::automatic);
...
/**
 * This is a convenience alias that can be used to define multivariate
 * polynomials
 ...
 * @endcode
 * And so on for increasing values of \p N. \p N must be nonzero, or a
 * compile-time error will be
 * generated.
 */
template <unsigned N, typename Cf, typename Key>
...
template <typename T> struct is_kronecker_monomial {
  static const bool value = false;
};

template <typename T> struct is_kronecker_monomial<kronecker_monomial<T>> {
  static const bool value = true;
};

template <typename T> struct is_monomial { static const bool value = false; };

template <typename T, typename S> struct is_monomial<monomial<T, S>> {
  static const bool value = true;
};
...
class has_set_auto_truncate_degree : sfinae_types {
  // NOTE: if we have total degree auto truncation, we also have partial degree
  // truncation.
  template <typename S1, typename T1>
  static auto test(const S1 &, const T1 &t)
      -> decltype(S1::set_auto_truncate_degree(t), void(), yes());
  static no test(...);

public:
  static const bool value =
      std::is_same<yes,
                   decltype(test(std::declval<S>(), std::declval<T>()))>::value;
};

template <typename S, typename T>
const bool has_set_auto_truncate_degree<S, T>::value;

template <typename S> class has_get_auto_truncate_degree : sfinae_types {
  template <typename S1>
  static auto test(const S1 &)
      -> decltype(S1::get_auto_truncate_degree(), void(), yes());
  static no test(...);

public:
  static const bool value =
      std::is_same<yes, decltype(test(std::declval<S>()))>::value;
};

template <typename S> const bool has_get_auto_truncate_degree<S>::value;

// Global enabler for the polynomial multiplier.
template <typename Series>
using poly_multiplier_enabler = typename std::enable_if<
    std::is_base_of<detail::polynomial_tag, Series>::value>::type;

// Enabler for divexact.
template <typename T>
using poly_divexact_enabler =
    typename std::enable_if<true_tt<ptd::enabler<T, T>>::value>::type;

// Enabler for exact ring operations.
template <typename T>
using poly_ero_enabler = typename std::enable_if<
    std::is_base_of<detail::polynomial_tag,
                    typename std::decay<T>::type>::value &&
    has_exact_ring_operations<
        typename std::decay<T>::type::term_type::cf_type>::value &&
    (std::is_integral<typename std::decay<
         T>::type::term_type::key_type::value_type>::value ||
     has_exact_ring_operations<typename std::decay<
         T>::type::term_type::key_type::value_type>::value)>::type;
...
/**
 * This specialisation is enabled if \p T is an instance of piranha::polynomial
 * which supports
 * exact polynomial division.
 */
template <typename T>
struct divexact_impl<T, detail::poly_divexact_enabler<T>> {
   ...
   *
   * @throws unspecified any exception thrown by the polynomial division
   * operator.
   */
  T &operator()(T &out, const T &n, const T &d) const { return out = n / d; }
};
 ...
 * This specialisation is enabled if \p T is an instance of piranha::polynomial
 * that supports
 * piranha::polynomial::gcd(). The piranha::polynomial_gcd_algorithm::automatic
 * algorithm
 * will be used for the computation.
 */
template <typename T> struct gcd_impl<T, T, detail::poly_gcd_enabler<T>> {
  /// Call operator.
  /**
   * @param[in] a first argument.
   * @param[in] b second argument.
   *
   * @return the first element of the result of
   * <tt>piranha::polynomial::gcd(a,b)</tt>.
   *
...
/**
 * This specialisation is enabled if the decay type of \p T is an instance of
 * piranha::polynomial
 * whose coefficient type satisfies piranha::has_exact_ring_operations and whose
 * exponent type is either
 * a C++ integral type or another type which satisfies
 * piranha::has_exact_ring_operations.
 */
template <typename T>
...
/**
 * This specialisation of piranha::series_multiplier is enabled when \p Series
 * is an instance of
 * piranha::polynomial.
 *
 * ## Type requirements ##
 ...
 * ## Move semantics ##
 *
 * Move semantics is equivalent to piranha::base_series_multiplier's move
 * semantics.
 */
template <typename Series>
class series_multiplier<Series, detail::poly_multiplier_enabler<Series>>
  ...
  template <typename T> using key_t = typename T::term_type::key_type;
  // Bounds checking.
  // Functor to return un updated copy of p if v is less than p.first or greater
  // than p.second.
  struct update_minmax {
    template <typename T>
    std::pair<T, T> operator()(const std::pair<T, T> &p, const T &v) const {
      return std::make_pair(v < p.first ? v : p.first,
  ...
  template <typename T = Series,
            typename std::enable_if<
                detail::is_monomial<key_t<T>>::value &&
  ...
  void check_bounds() const {
    ...
    // The function used to determine minmaxs for the two series. This is used
    // both in
    // single-thread and multi-thread mode.
    auto thread_func = [&mut, this, &monomial_checker](
        unsigned t_idx, const v_ptr *vp, mm_vec *mmv) {
      ...
      const auto end = vp->data() + ((t_idx == this->m_n_threads - 1u)
                                         ? vp->size()
                                         : ((t_idx + 1u) * block_size));
      // We need to make sure we have at least 1 element to process. This is
      // guaranteed
      ...
      // Init the mimnax.
      // NOTE: we can use this as we are sure the series has at least one
      // element (start != end).
      std::transform((*start)->m_key.begin(), (*start)->m_key.end(),
                     std::back_inserter(minmax_values),
                     [](const expo_type &v) { return std::make_pair(v, v); });
      // Move to the next element and go with the loop.
      ++start;
      for (; start != end; ++start) {
        piranha_assert(monomial_checker(**start));
        // NOTE: std::transform is allowed to do transformations in-place -
        // i.e., here the output range is the
        ...
        // The important part is that the functor *itself* must not mutate the
        // elements.
        std::transform(minmax_values.begin(), minmax_values.end(),
                       (*start)->m_key.begin(), minmax_values.begin(),
                       update_minmax{});
      }
      if (this->m_n_threads == 1u) {
        // In single thread the output mmv should be written only once, after
        // being def-inited.
        ...
        *mmv = std::move(minmax_values);
        ...
        if (mmv->empty()) {
          ...
          *mmv = std::move(minmax_values);
          ...
          auto updater = [](const std::pair<expo_type, expo_type> &p1,
                            const std::pair<expo_type, expo_type> &p2) {
            return std::make_pair(p1.first < p2.first ? p1.first : p2.first,
                                  p1.second > p2.second ? p1.second
                                                        : p2.second);
          };
          std::transform(minmax_values.begin(), minmax_values.end(),
                         mmv->begin(), mmv->begin(), updater);
        }
      }
    };
    ...
    // Compute the sum of the two minmaxs, using multiprecision to avoid
    // overflow (this is a simple interval addition).
    std::vector<std::pair<integer, integer>> minmax_values;
    std::transform(minmax_values1.begin(), minmax_values1.end(),
                   minmax_values2.begin(), std::back_inserter(minmax_values),
                   [](const std::pair<expo_type, expo_type> &p1,
                      const std::pair<expo_type, expo_type> &p2) {
                     return std::make_pair(
                         integer(p1.first) + integer(p2.first),
  ...
  template <typename T = Series,
            typename std::enable_if<
                detail::is_kronecker_monomial<key_t<T>>::value, int>::type = 0>
  void check_bounds() const {
    ...
    piranha_assert(this->m_v1.size() != 0u && this->m_v2.size() != 0u);
    // NOTE: here we are sure about this since the symbol set in a series should
    // never
    // overflow the size of the limits, as the check for compatibility in
    // Kronecker monomial
    // would kick in.
    piranha_assert(this->m_ss.size() < ka::get_limits().size());
    std::mutex mut;
    auto thread_func = [&mut, this](unsigned t_idx, const v_ptr *vp,
                                    mm_vec *mmv) {
      piranha_assert(t_idx < this->m_n_threads);
      const auto block_size = vp->size() / this->m_n_threads;
      auto start = vp->data() + t_idx * block_size;
      ...
      // NOTE: we need to check that the exponents of the monomials in the
      // result do not
      // go outside the bounds of the Kronecker codification. We need to unpack
      // all monomials
      // in the operands and examine them, we cannot operate on the codes for
      // this.
      ...
      for (; start != end; ++start) {
        ...
        std::transform(minmax_values.begin(), minmax_values.end(),
                       tmp_vec.begin(), minmax_values.begin(), update_minmax{});
      }
      if (this->m_n_threads == 1u) {
        piranha_assert(mmv->empty());
        *mmv = std::move(minmax_values);
        ...
        if (mmv->empty()) {
          *mmv = std::move(minmax_values);
          ...
          piranha_assert(minmax_values.size() == mmv->size());
          auto updater = [](const std::pair<value_type, value_type> &p1,
    ...
    const auto &minmax_vec = std::get<0u>(
        ka::get_limits()[static_cast<decltype(ka::get_limits().size())>(
            this->m_ss.size())]);
    piranha_assert(minmax_values.size() == minmax_vec.size());
    piranha_assert(minmax_values.size() == minmax_values1.size());
    piranha_assert(minmax_values.size() == minmax_values2.size());
    for (decltype(minmax_values.size()) i = 0u; i < minmax_values.size(); ++i) {
      if (unlikely(minmax_values[i].first < -minmax_vec[i] ||
                   minmax_values[i].second > minmax_vec[i])) {
        piranha_throw(std::overflow_error,
                      "Kronecker monomial components are out of bounds");
      }
    }
  }
  // Implementation detail of the bound checking logic. This is common enough to
  // be shared.
  template <typename MmVec, typename Func>
  void check_bounds_impl(MmVec &minmax_values1, MmVec &minmax_values2,
                         Func &thread_func) const {
    if (this->m_n_threads == 1u) {
      ...
      // Series 1.
      {
        ...
        try {
          for (unsigned i = 0u; i < this->m_n_threads; ++i) {
            ff_list.push_back(thread_pool::enqueue(
                i, thread_func, i, &(this->m_v1), &minmax_values1));
          }
      ...
      // Series 2.
      {
        ...
        try {
          for (unsigned i = 0u; i < this->m_n_threads; ++i) {
            ff_list.push_back(thread_pool::enqueue(
                i, thread_func, i, &(this->m_v2), &minmax_values2));
          }
          // First let's wait for everything to finish.
          ff_list.wait_all();
          // Then, let's handle the exceptions.
  ...
  using call_enabler = typename std::enable_if<
      key_is_multipliable<cf_t<T>, key_t<T>>::value &&
          has_multiply_accumulate<cf_t<T>>::value &&
          detail::true_tt<detail::cf_mult_enabler<cf_t<T>>>::value,
      int>::type;
  // Utility helpers for the subtraction of degree types in the truncation
  // routines. The specialisation
...
public:
  /// Constructor.
  /**
   * The constructor will call the base constructor and run these additional
   * checks:
   * - if the key is a piranha::kronecker_monomial, it will be checked that the
   * result of the multiplication does not overflow
   *   the representation limits of piranha::kronecker_monomial;
   * - if the key is a piranha::monomial of a C++ integral type, it will be
   * checked that the result of the multiplication does not overflow
   ...
   */
  explicit series_multiplier(const Series &s1, const Series &s2)
      : base(s1, s2) {
   ...
   * - the coefficient and key type of \p Series satisfy
   * piranha::key_is_multipliable,
   * - the coefficient type of \p Series supports piranha::math::mul3() and
   * piranha::math::multiply_accumulate().
   *
   * This method will perform the multiplication of the series operands passed
   * to the constructor. Depending on
   * the key type of \p Series, the implementation will use either
   * base_series_multiplier::plain_multiplication()
   * with base_series_multiplier::plain_multiplier or a different algorithm.
   *
   * If a polynomial truncation threshold is defined and the degree type of the
   * polynomial is a C++ integral type,
   * the integral arithmetic operations involved in the truncation logic will be
   ...
   * - the conditions for truncated multiplication outlined in
   * piranha::polynomial are satisfied,
   * - the type \p T is the same as the degree type of the polynomial,
   * - the number and types of \p Args is as specified below,
   * - piranha::base_series_multiplier::plain_multiplication() and
   * get_skip_limits() can be called.
   *
   * This method will perform the truncated multiplication of the series
   * operands passed to the constructor.
   * The truncation degree is set to \p max degree, and it is either the total
   * maximum degree (if the number
   * of \p Args is zero) or the partial degree (if \p Args is a single
   * symbol_set::positions parameter
   * representing the positions of the arguments to be considered for the
   * computation of the degree).
   *
   * @param[in] max_degree the maximum degree of the result of the
   * multiplication.
   * @param[in] args either an empty argument, or a single symbol_set::positions
   * argument.
   *
   * @return the result of the truncated multiplication of the operands used for
   * construction.
   *
   * @throws unspecified any exception thrown by:
   * - memory errors in standard containers,
  ...
  Series truncated_multiplication(const T &max_degree,
                                  const Args &... args) const {
    // NOTE: a possible optimisation here is the following: if the sum degrees
    // of the arguments is less than
    // or equal to the max truncation degree, just do the normal multiplication
    // - which can also then take advantage
    // of faster Kronecker multiplication, if the series are suitable.
    using term_type = typename Series::term_type;
    // NOTE: degree type is the same in total and partial.
    using degree_type =
        decltype(detail::ps_get_degree(term_type{}, this->m_ss));
    using size_type = typename base::size_type;
    namespace sph = std::placeholders;
    static_assert(std::is_same<T, degree_type>::value, "Invalid degree type");
    static_assert(detail::has_get_auto_truncate_degree<Series>::value,
                  "Invalid series type");
    // First let's create two vectors with the degrees of the terms in the two
    // series.
    ...
    detail::parallel_vector_transform(this->m_n_threads, this->m_v1, v_d1,
                                      std::bind(term_degree_getter{}, sph::_1,
                                                std::cref(this->m_ss),
                                                std::cref(args)...));
    detail::parallel_vector_transform(this->m_n_threads, this->m_v2, v_d2,
                                      std::bind(term_degree_getter{}, sph::_1,
                                                std::cref(this->m_ss),
                                                std::cref(args)...));
    // Next we need to order the terms in the second series, and also the
    // corresponding degree vector.
    // First we create a vector of indices and we fill it.
    std::vector<size_type> idx_vector(
        safe_cast<typename std::vector<size_type>::size_type>(
            this->m_v2.size()));
    std::iota(idx_vector.begin(), idx_vector.end(), size_type(0u));
    // Second, we sort the vector of indices according to the degrees in the
    // second series.
    std::stable_sort(idx_vector.begin(), idx_vector.end(),
                     [&v_d2](const size_type &i1, const size_type &i2) {
                       return v_d2[static_cast<d_size_type>(i1)] <
                              v_d2[static_cast<d_size_type>(i2)];
                     });
  ...
  /**
   * \note
   * This method can be called only if \p Series supports truncated
   * multiplication (as explained in piranha::polynomial) and
   * \p T is the same type as the degree type.
   *
   * This method assumes that \p v_d1 and \p v_d2 are vectors containing the
   * degrees of each term in the first and second series
   * respectively, and that \p v_d2 is sorted in ascending order.
   * It will return a vector \p v of indices in the second series such that,
   * given an index \p i in the first series,
   * the term of index <tt>v[i]</tt> in the second series is the first term such
   * that the term-by-term multiplication with
   * the <tt>i</tt>-th term in the first series produces a term of degree
   * greater than \p max_degree. That is, terms of index
   * equal to or greater than <tt>v[i]</tt> in the second series will produce
   * terms with degree greater than \p max_degree
   * when multiplied by the <tt>i</tt>-th term in the first series.
   *
   * @param[in] v_d1 a vector containing the degrees of the terms in the first
   * series.
   * @param[in] v_d2 a sorted vector containing the degrees of the terms in the
   * second series.
   ...
   */
  template <typename T>
  std::vector<typename base::size_type>
  get_skip_limits(const std::vector<T> &v_d1, const std::vector<T> &v_d2,
                  const T &max_degree) const {
    // NOTE: this can be parallelised, but we need to check the heuristic
    // for selecting the number of threads as it is pretty fast wrt the
    // multiplication.
    ...
    for (const auto &d1 : v_d1) {
      // Here we will find the index of the first term t2 in the second series
      // such that
      // the degree d2 of t2 is > max_degree - d1, that is, d1 + d2 >
      // max_degree.
      // NOTE: we need to use upper_bound, instead of lower_bound, because we
      // need to find the first
      // element which is *strictly* greater than the max degree, as upper bound
      // of a half closed
      // interval.
      // NOTE: the functor of lower_bound works inversely wrt upper_bound. See
      // the notes on the type
      // requirements for the functor here:
      // http://en.cppreference.com/w/cpp/algorithm/upper_bound
      const T comp = degree_sub(max_degree, d1);
      const auto it =
          std::upper_bound(idx_vector.begin(), idx_vector.end(), comp,
                           [&v_d2](const T &c, const size_type &idx) {
                             return c < v_d2[static_cast<d_size_type>(idx)];
                           });
      retval.push_back(it == idx_vector.end()
                           ? static_cast<size_type>(idx_vector.size())
                           : *it);
    }
    ...
    auto retval_checker = [&retval, &v_d1, &v_d2, &max_degree, this]() -> bool {
      for (decltype(retval.size()) i = 0u; i < retval.size(); ++i) {
        ...
        if (!(v_d2[static_cast<d_size_type>(retval[i])] >
              this->degree_sub(max_degree,
                               v_d1[static_cast<d_size_type>(i)]))) {
          return false;
        }
      }
      return true;
  ...
  //@}
private:
  // NOTE: wrapper to multadd that treats specially rational coefficients. We
  // need to decide in the future
  // if this stays here or if it is better to generalise it.
  template <typename T, typename std::enable_if<
                            !detail::is_mp_rational<T>::value, int>::type = 0>
  ...
  template <typename T = Series,
            typename std::enable_if<
                !detail::has_get_auto_truncate_degree<T>::value, int>::type = 0>
  ...
  Series plain_multiplication_wrapper() const {
    ...
    // Partial degree truncation.
    const symbol_set::positions pos(
        this->m_ss, symbol_set(std::get<2u>(t).begin(), std::get<2u>(t).end()));
    return truncated_multiplication(std::get<1u>(t), std::get<2u>(t), pos);
  }
  // NOTE: the existence of these functors is because GCC 4.8 has troubles
  // capturing variadic arguments in lambdas
  // in truncated_multiplication, and we need to use std::bind instead. Once we
  // switch to 4.9, we can revert
  // to lambdas and drop the <functional> header.
  struct term_degree_sorter {
    using term_type = typename Series::term_type;
    template <typename... Args>
    bool operator()(term_type const *p1, term_type const *p2,
                    const symbol_set &ss, const Args &... args) const {
      return detail::ps_get_degree(*p1, args..., ss) <
             detail::ps_get_degree(*p2, args..., ss);
    }
  };
  struct term_degree_getter {
    using term_type = typename Series::term_type;
    template <typename... Args>
    auto operator()(term_type const *p, const symbol_set &ss,
                    const Args &... args) const
  ...
  template <typename T = Series,
            typename std::enable_if<!detail::is_kronecker_monomial<
  ...
  // Case 2: Kronecker mult, do the special multiplication unless a truncation
  // is active. In that case, run the plain mult.
  template <typename T = Series,
            typename std::enable_if<detail::is_kronecker_monomial<
                                        typename T::term_type::key_type>::value,
                                    int>::type = 0>
  Series execute() const {
    ...
    if (integer(size1) * size2 < integer(e_thr) * e_thr &&
        this->m_n_threads == 1u) {
      estimate = false;
    }
    // If estimation is not worth it, we go with the plain multiplication.
    // NOTE: this is probably not optimal, but we have to do like this as the
    // sparse
    // Kronecker multiplication below requires estimation. Maybe in the future
    // we can
    ...
    // Do not do anything if one of the two series is empty, just return an
    // empty series.
    if (unlikely(!size1 || !size2)) {
      return retval;
    }
    // Rehash the retun value's container accordingly. Check the tuning flag to
    // see if we want to use
    // multiple threads for initing the return value.
    // NOTE: it is important here that we use the same n_threads for
    // multiplication and memset as
    // we tie together pinned threads with potentially different NUMA regions.
    const unsigned n_threads_rehash =
        tuning::get_parallel_memory_set() ? this->m_n_threads : 1u;
    // Use the plain functor in normal mode for the estimation.
    const auto est = this->template estimate_final_series_size<
        1u, typename base::template plain_multiplier<false>>();
    // NOTE: if something goes wrong here, no big deal as retval is still empty.
    retval._container().rehash(
        boost::numeric_cast<typename Series::size_type>(std::ceil(
            static_cast<double>(est) / retval._container().max_load_factor())),
        n_threads_rehash);
  ...
  void sparse_kronecker_multiplication(Series &retval) const {
    ...
    // Task comparator. It will compare the bucket index of the terms resulting
    // from
    // the multiplication of the term in the first series by the first term in
    // the block
    // of the second series. This is essentially the first bucket index of
    // retval in which the task
    // will write.
    // NOTE: this is guaranteed not to overflow as the max bucket size in the
    // hash set is 2**(nbits-1),
    // and the max value of bucket_size_type is 2**nbits - 1.
    auto task_cmp = [&r_bucket, &v1, &v2](const task_type &t1,
                                          const task_type &t2) {
    ...
    const size_type block_size =
        safe_cast<size_type>(tuning::get_multiplication_block_size());
    // Task splitter: split a task in block_size sized tasks and append them to
    // out.
    auto task_split = [block_size](const task_type &t,
                                   std::vector<task_type> &out) {
      size_type start = std::get<1u>(t), end = std::get<2u>(t);
      while (static_cast<size_type>(end - start) > block_size) {
        out.emplace_back(std::get<0u>(t), start,
                         static_cast<size_type>(start + block_size));
    ...
    // Function to perform all the term-by-term multiplications in a task, using
    // tmp_term
    // as a temporary value for the computation of the result.
    auto task_consume = [&v1, &v2, &container, it_end,
                         this](const task_type &task, term_type &tmp_term) {
      // Get the term in the first series.
      term_type const *t1 = v1[std::get<0u>(task)];
      // Get pointers to the second series.
      term_type const **start2 = &(v2[std::get<1u>(task)]),
                      **end2 = &(v2[std::get<2u>(task)]);
      ...
      for (; start2 != end2; ++start2) {
        ...
        tmp_term.m_key.set_int(
            static_cast<int_type>(key1 + cur.m_key.get_int()));
        ...
        if (it == it_end) {
          // NOTE: for coefficient series, we might want to insert with move()
          // below,
          ...
          // NOTE: here we need to decide if we want to give the same treatment
          // to fmp as we did with cf_mult_impl.
    ...
    // Compute the number of zones in which the output container will be
    // subdivided,
    // a multiple of the number of threads.
    // NOTE: zm is a tuning parameter.
    const unsigned zm = 10u;
    const bucket_size_type n_zones =
        static_cast<bucket_size_type>(integer(this->m_n_threads) * zm);
    // Number of buckets per zone (can be zero).
    const bucket_size_type bpz =
        static_cast<bucket_size_type>(bucket_count / n_zones);
    // For each zone, we need to define a vector of tasks that will write only
    // into that zone.
    ...
    // Given the [first,last[ index range in v2, find the first index idx in the
    // v2 range such that the i-th term in v1
    // multiplied by the idx-th term in v2 will be written into retval at a
    // bucket index not less than zb.
    auto l_bound = [&v1, &v2, &r_bucket, &task_split](
        size_type first, size_type last, bucket_size_type zb,
    ...
    // Fill the task table.
    auto table_filler = [&task_table, bpz, zm, this, bucket_count, size1, size2,
                         &l_bound, &task_split,
      ...
      for (unsigned n = 0u; n < zm; ++n) {
        ...
        bucket_size_type a =
            static_cast<bucket_size_type>(thread_idx * bpz * zm + n * bpz);
        bucket_size_type b;
        ...
        for (size_type i = 0u; i < size1; ++i) {
          auto t = std::make_tuple(i, l_bound(0u, size2, a, i),
                                   l_bound(0u, size2, b, i));
        ...
        // Note: we can always compute a,b + bucket_count because of the limits
        // on the maximum value of
        // bucket_count.
        for (size_type i = 0u; i < size1; ++i) {
          auto t = std::make_tuple(
              i, l_bound(0u, size2,
                         static_cast<bucket_size_type>(a + bucket_count), i),
              l_bound(0u, size2,
                      static_cast<bucket_size_type>(b + bucket_count), i));
          if (std::get<1u>(t) == 0u && std::get<2u>(t) == 0u) {
            break;
          }
          task_split(t, cur_tasks);
        ...
        task_table[static_cast<decltype(task_table.size())>(
    ...
    auto table_checker = [&task_table, size1, size2, &r_bucket, bpz,
                          bucket_count, &v1, &v2]() -> bool {
      ...
      for (decltype(task_table.size()) i = 0u; i < task_table.size(); ++i) {
        ...
        for (const auto &t : v) {
          auto idx1 = std::get<0u>(t), start2 = std::get<1u>(t),
               end2 = std::get<2u>(t);
          using int_type = decltype(v1[idx1]->m_key.get_int());
          piranha_assert(start2 <= end2);
          tot_n += end2 - start2;
          for (; start2 != end2; ++start2) {
            tmp_term.m_key.set_int(static_cast<int_type>(
                v1[idx1]->m_key.get_int() + v2[start2]->m_key.get_int()));
    ...
    // Thread functor.
    auto thread_functor = [zm, &task_table, &af, &v1, &v2, &container,
                           &task_consume](const unsigned &thread_idx) {
      ...
      while (true) {
        // If this returns false, it means that the tasks still need to be
        // consumed;

...
/**
 * This class represents multivariate Poisson series as collections of
 * multivariate Poisson series terms,
 * in which the trigonometric monomials are represented by
 * piranha::rtk_monomial.
...
// - once we have a selectable key type, we must take care that in a few places
// we assume that the value type
//   of the key is a C++ integral, but this might not be the case any more
//   (e.g., in the sin/cos implementation
//   we will need a safe cast) -> also in integrate(), there are a few
//   occurrences of this (e.g., == 0 should
//   become math::is_zero() etc.). Will also need the is_integrable check on the
//   key type.
...
class poisson_series
              ...
              substitutable_series<
                  t_substitutable_series<
                      trigonometric_series<
                          series<Cf, rtk_monomial, poisson_series<Cf>>>,
                      poisson_series<Cf>>,
                  poisson_series<Cf>>,
              poisson_series<Cf>>,
          poisson_series<Cf>>,
  ...
  using base = power_series<
      ipow_substitutable_series<
          substitutable_series<
              t_substitutable_series<trigonometric_series<series<
                                         Cf, rtk_monomial, poisson_series<Cf>>>,
                                     poisson_series<Cf>>,
              poisson_series<Cf>>,
          poisson_series<Cf>>,
      poisson_series<Cf>>;
  ...
  template <typename T>
  using has_rf_cf = std::integral_constant<
      bool, std::is_base_of<detail::rational_function_tag,
                            typename T::term_type::cf_type>::value>;
  // Types coming out of sin()/cos() for the base type. These will also be the
  // final types.
  template <typename T>
  using sin_type =
      decltype(math::sin(std::declval<const typename T::base &>()));
  template <typename T>
  using cos_type =
      decltype(math::cos(std::declval<const typename T::base &>()));
  // Case 0: Poisson series is not suitable for special sin() implementation.
  // Just forward to the base one, via casting.
  template <
      typename T = poisson_series,
      typename std::enable_if<
          !detail::poly_in_cf<T>::value && !has_rf_cf<T>::value, int>::type = 0>
  sin_type<T> sin_impl() const {
    return math::sin(*static_cast<const base *>(this));
  }
  // Case 1: Poisson series is suitable for special sin() implementation via
  // poly. This can fail at runtime depending on what is
  // contained in the coefficients. The return type is the same as the base one,
  // as in this routine we only need operations
  // which are supported by all coefficient types, no need for rebinding or
  // anything like that.
  template <
      typename T = poisson_series,
      typename std::enable_if<detail::poly_in_cf<T>::value, int>::type = 0>
  sin_type<T> sin_impl() const {
    return special_sin_cos<false, sin_type<T>>(*this);
  }
  // Case 2: Poisson series is suitable for special sin() implementation via
  // rational function.
  template <typename T = poisson_series,
            typename std::enable_if<has_rf_cf<T>::value, int>::type = 0>
  sin_type<T> sin_impl() const {
  ...
  // Functions to handle the case in which special sin/cos implementation fails.
  // They will just call the base sin/cos, we need
  // them with tag dispatching as the implementation below is generic and needs
  // to cope with potentially different sin/cos return
  // types.
  template <typename T>
  static cos_type<T> special_sin_cos_failure(const T &s,
                                             const std::true_type &) {
    return math::cos(static_cast<const typename T::base &>(s));
  }
  template <typename T>
  static sin_type<T> special_sin_cos_failure(const T &s,
                                             const std::false_type &) {
    return math::sin(static_cast<const typename T::base &>(s));
  }
  // Convert an input polynomial to a Poisson series of type RetT. The
  // conversion
  // will be successful if the polynomial can be reduced to an integral linear
  // combination of
  // symbols.
  ...
  static RetT poly_to_ps(const Poly &poly) {
    ...
    if (!lc.empty() && lc.begin()->second.sign() < 0) {
      std::for_each(
          lc.begin(), lc.end(),
          [](std::pair<const std::string, integer> &p) { p.second.negate(); });
      sign_change = true;
    }
    // Return value.
    RetT retval;
    ...
    for (auto it = lc.begin(); it != lc.end(); ++it) {
      ...
      // The value type here could be anything, and not guaranteed to be
      // castable,
  ...
  // Special sin/cos implementation when we have reached the first polynomial
  // coefficient in the hierarchy.
  ...
  RetT special_sin_cos(const T &s) const {
    ...
    if (s.is_single_coefficient() && !s.empty()) {
      try {
        ...
        // Interpret invalid_argument as a failure in extracting integral
        // combination,
        // and move on.
      }
    }
    return special_sin_cos_failure(*this,
                                   std::integral_constant<bool, IsCos>{});
  }
  // The coefficient is not a polynomial: recurse to the inner coefficient type,
  // if the current coefficient type
  // is suitable.
  template <bool IsCos, typename RetT, typename T,
            typename std::enable_if<
                !std::is_base_of<detail::polynomial_tag,
  ...
  RetT special_sin_cos_rf(const T &s) const {
    if (s.is_single_coefficient() && !s.empty() &&
        math::is_unitary(s._container().begin()->m_cf.den())) {
      try {
  ...
  // NOTE: this used to be a template alias to be used with true_tt in the usual
  // fashion, but there is a pesky bug
  // in GCC < 5 that results in a segfault of the compiler. This is a
  // workaround.
  ...
  using i_cf_only_type = decltype(
      ...
      std::declval<const T &>());
  // Integration of coefficient only is enabled only if the type above is well
  // defined
  ...
  template <typename ResT, typename T>
  struct i_cf_only_enabler<
      ResT, T, typename std::enable_if<
                   std::is_same<ResT, i_cf_only_type<ResT, T>>::value>::type> {
    static const bool value = true;
  };
  template <typename ResT, typename It, typename T = poisson_series>
  void integrate_coefficient_only(
      ResT &retval, It it, const std::string &name,
      typename std::enable_if<i_cf_only_enabler<ResT, T>::value>::type * =
          nullptr) const {
  ...
  template <typename ResT, typename It, typename T = poisson_series>
  void integrate_coefficient_only(
      ResT &, It, const std::string &,
      typename std::enable_if<!i_cf_only_enabler<ResT, T>::value>::type * =
          nullptr) const {
    piranha_throw(std::invalid_argument, "unable to perform Poisson series "
  ...
  // Coefficient type divided by the value coming from the integration of the
  // key.
  template <typename T>
  using i_cf_type =
      decltype(std::declval<const typename T::term_type::cf_type &>() /
               std::declval<const key_integrate_type<T> &>());
  // Derivative of the type above.
  template <typename T>
  using i_cf_type_p =
      decltype(math::partial(std::declval<const i_cf_type<T> &>(),
                             std::declval<const std::string &>()));
  // The final return type.
  template <typename T>
  using pc_res_type = decltype(std::declval<const i_cf_type_p<T> &>() *
                               std::declval<const T &>());
  template <typename T>
  struct integrate_type_<
      T, typename std::enable_if<
  ...
  integrate_type<T> integrate_impl(const symbol &s,
    ...
    try {
      ...
      piranha_throw(std::invalid_argument, "unable to perform Poisson series "
                                           "integration: cannot convert "
                                           "polynomial degree to an integer");
    }
    // If the variable is in both cf and key, and the cf degree is negative, we
    // cannot integrate.
    if (degree.sign() < 0) {
      piranha_throw(std::invalid_argument,
                    "unable to perform Poisson series integration: polynomial "
                    "coefficient has negative integral degree");
    }
  ...
  using ti_type_divisor = typename std::enable_if<
      std::is_constructible<ti_type_divisor_<T>, int>::value &&
  ...
  template <typename T = poisson_series>
  ti_type_divisor<T>
  t_integrate_impl(const std::vector<std::string> &names) const {
    ...
    for (auto it = this->m_container.begin(); it != it_f; ++it) {
      ...
      std::transform(trig_vector.begin(), trig_vector.end(),
                     std::back_inserter(tmp_int),
                     [](const k_value_type &n) { return integer(n); });
      // Determine the common divisor.
      // NOTE: both the divisor and the trigonometric key share the canonical
      // form in which the
      // first nonzero multiplier is positive, so we don't need to account for
      // sign flips when
      // constructing a divisor from the trigonometric part. We just need to
      // take care
      ...
      if (unlikely(math::is_zero(cd))) {
        piranha_throw(std::invalid_argument,
                      "an invalid trigonometric term was encountered while "
      ...
      // Temporary Poisson series from the current term, with the trig flavour
      // flipped.
  ...
  template <typename T = poisson_series,
            typename std::enable_if<
                std::is_base_of<detail::rational_function_tag,
                                typename T::term_type::cf_type>::value,
                int>::type = 0>
  T t_integrate_impl(const std::vector<std::string> &names) const {
    ...
    for (const auto &t : this->_container()) {
      ...
      for (decltype(trig_vector.size()) i = 0u; i < trig_vector.size(); ++i) {
        tmp += p_type{names[static_cast<decltype(names.size())>(i)]} *
               trig_vector[i];
      }
      if (math::is_zero(tmp)) {
        piranha_throw(std::invalid_argument,
...
public:
  /// Series rebind alias.
  template <typename Cf2> using rebind = poisson_series<Cf2>;
  ...
  /// Trivial destructor.
  ~poisson_series() { PIRANHA_TT_CHECK(is_series, poisson_series); }
  ...
  /**
   * \note
   * This template method is enabled only if math::sin() can be called on the
   * class
   * from which piranha::poisson_series derives (i.e., only if the default
   * math::sin()
   * implementation for series is appropriate).
   *
   * In general, this method behaves exactly like the default implementation of
   * piranha::math::sin() for series
   * types. If, however, a polynomial appears in the hierarchy of coefficients,
   * or the coefficient type is an instance
   * of piranha::rational_function, then this method
   * will attempt to extract an integral linear combination of symbolic
   * arguments and use it to construct
   * a Poisson series with a single term, unitary coefficient and the
   * trigonometric key built from the linear
   * combination.
   *
   ...
   * \f]
   *
   * If for any reason it is not possible to extract the linear integral
   * combination,
   * then this method will forward the call to the default implementation of
   * piranha::math::sin() for series
   * types.
   *
   ...
   * - piranha::math::negate(),
   * - piranha::math::sin(),
   * - the extraction of a linear combination of integral arguments from the
   * polynomial coefficient.
   */
  template <typename T = poisson_series> sin_type<T> sin() const {
    return sin_impl();
  }
  /// Cosine.
  /**
   ...
   */
  template <typename T = poisson_series> cos_type<T> cos() const {
    return cos_impl();
  }
  /// Integration.
  /**
   ...
   *
   * This method will attempt to compute the antiderivative of the Poisson
   * series term by term using the
   * following procedure:
   * - if the term's monomial does not depend on the integration variable, the
   * integration will be deferred to the coefficient;
   * - otherwise:
   *   - if the coefficient does not depend on the integration variable, the
   * monomial is integrated;
   *   - if the coefficient is a polynomial, a strategy of integration by parts
   * is attempted, its success depending on whether
   ...
   * piranha::math::integrate(), piranha::safe_cast() and
   *   piranha::math::negate(),
  ...
  integrate_type<T> integrate(const std::string &name) const {
    ...
    for (auto it = this->m_container.begin(); it != it_f; ++it) {
      ...
      // If the variable does not appear in the monomial, try deferring the
      // integration
      // to the coefficient.
      if (key_int.first == 0) {
      ...
      // The variable is in the monomial, let's check if the variable is also in
      // the coefficient.
      if (math::is_zero(math::partial(it->m_cf, name))) {
        // No variable in the coefficient, proceed with the integrated key and
        ...
        // With the variable both in the coefficient and the key, we only know
        // how to proceed with polynomials.
   ...
   * - the coefficient type is an instance of piranha::divisor_series or
   * piranha::rational_function, and
   * - the operations required by the computation of the time integration are
   * supported by all
   *   the involved types.
   *
   * This is a special type of integration in which the trigonometric arguments
   * are considered as linear functions
   * of time, and in which the integration variable is time itself. For
   * instance, if the input series is
   * \f[
   ...
   * \f]
   * where \f$ \nu_{x} \f$ and \f$ \nu_{y} \f$ are the frequencies associated to
   * \f$ x \f$ and \f$ y \f$ (that is,
   * it is understood that \f$ x = \nu_{x}t \f$ and \f$ x = \nu_{y}t \f$).
   *
   * This method will throw an error if any term of the calling series has a
   * unitary key (e.g., in the Poisson series
   * \f$ \frac{1}{5}z \f$ the only trigonometric key is \f$ \cos\left( 0 \right)
   * \f$ and would thus result in a division by zero
   ...
   * - the public interfaces of piranha::symbol_set, piranha::mp_integer and
   * piranha::series,
   ...
   */
  template <typename T = poisson_series> ti_type<T> t_integrate() const {
    std::vector<std::string> names;
    for (auto it = this->m_symbol_set.begin(); it != this->m_symbol_set.end();
         ++it) {
  ...
  /**
   * \note
   * This method is enabled only if the other overload of
   * piranha::poisson_series::t_integrate() is enabled.
   *
   * This method operates exactly like the other overload of
   * piranha::poisson_series::t_integrate(), with the difference
   * that the names of the symbols used to represent the frequencies are passed
   * as the \p names argument, rather
   * than automatically deduced. The \p names argument must be sorted
   * lexicographically, otherwise an error
   * will be produced. Duplicate entries in \p names will be removed.
   *
   * @param[in] names the names of the symbols used to represent the
   * frequencies.
   *
   * @return the result of the time integration.
   *
   * @throws std::invalid_argument if the size of \p names is not equal to the
   * size of the symbol set of \p this
   * (after the removal of duplicate entries), or if \p names is not sorted
   * lexicographically.
  ...
  ti_type<T> t_integrate(std::vector<std::string> names) const {
    ...
    if (unlikely(names.size() != this->m_symbol_set.size())) {
      piranha_throw(
          std::invalid_argument,
          "the number of symbols passed in input must be equal to the "
...
using ps_series_multiplier_enabler = typename std::enable_if<
    std::is_base_of<poisson_series_tag, Series>::value>::type;
}
...
class series_multiplier<Series, detail::ps_series_multiplier_enabler<Series>>
  ...
  using call_enabler = typename std::enable_if<
      key_is_multipliable<typename T::term_type::cf_type,
                          typename T::term_type::key_type>::value,
      int>::type;
  void divide_by_two(Series &s) const {
    // NOTE: if we ever implement multi-threaded series division we most likely
    // need
    // to revisit this.
    piranha_assert(this->m_n_threads > 0u);
    if (this->m_n_threads == 1u) {
      // This is possible, as the requirements of series divisibility and trig
      // key
      ...
      auto divider = [&container, &total_erase_count, this](
          bucket_size_type start_idx, bucket_size_type end_idx) {
      ...
      // Buckets per thread.
      const auto bpt = static_cast<bucket_size_type>(container.bucket_count() /
                                                     this->m_n_threads);
      // Go with the threads.
      ...
      try {
        for (unsigned i = 0u; i < this->m_n_threads; ++i) {
          ...
          const auto end_idx =
              (i == this->m_n_threads - 1u)
                  ? container.bucket_count()
                  : static_cast<bucket_size_type>(bpt * (i + 1u));
          ff_list.push_back(
              thread_pool::enqueue(i, divider, start_idx, end_idx));
        }
...
public:
  ...
  /**
   * \note
   * This operator is enabled only if the coefficient and key types of \p Series
   * satisfy

...
template <typename = void> struct piranha_init_statics {
 ...
 *
 * It is allowed to call this function concurrently from multiple threads: after
 * the first
 * invocation, additional invocations will not perform any action.
 */
inline void init() {
  if (detail::piranha_init_statics<>::s_init_flag.test_and_set()) {
    // If the previous value of the init flag was true, it means we already
    // called

...
/**
 * This toolbox will conditionally augment a \p Series type by adding methods to
 * subtitute integral powers of symbols with generic objects.
 * Such augmentation takes place if the series' coefficient and/or key types
 * expose substitution methods (as established by
 * the piranha::has_ipow_subs and piranha::key_has_ipow_subs type traits). If
 * the requirements outlined above are not satisfied, the substitution
 ...
 * - \p Derived must derive from piranha::ipow_substitutable_series of \p Series
 * and \p Derived.
 *
...
class ipow_substitutable_series : public Series,
                                  detail::ipow_substitutable_series_tag {
  typedef Series base;
  PIRANHA_SERIALIZE_THROUGH_BASE(base)
  // Detect subs term.
  template <typename Term, typename T> struct subs_term_score {
    static const unsigned value =
        static_cast<unsigned>(has_ipow_subs<typename Term::cf_type, T>::value) +
        (static_cast<unsigned>(
             key_has_ipow_subs<typename Term::key_type, T>::value)
         << 1u);
  };
  // Case 1: subs only on cf.
  template <typename T, typename Term>
  using cf_subs_type =
      decltype(math::ipow_subs(std::declval<typename Term::cf_type const &>(),
                               std::declval<std::string const &>(),
                               std::declval<const integer &>(),
                               std::declval<T const &>()));
  template <typename T, typename Term>
  using ret_type_1 = decltype(std::declval<const cf_subs_type<T, Term> &>() *
                              std::declval<Derived const &>());
  template <typename T, typename Term,
            typename std::enable_if<subs_term_score<Term, T>::value == 1u,
                                    int>::type = 0>
  static ret_type_1<T, Term>
  subs_term_impl(const Term &t, const std::string &name, const integer &n,
                 const T &x, const symbol_set &s_set) {
    Derived tmp;
  ...
  template <typename T, typename Term>
  using k_subs_type = typename decltype(
      std::declval<const typename Term::key_type &>().ipow_subs(
          std::declval<const std::string &>(), std::declval<const integer &>(),
          std::declval<const T &>(),
          std::declval<const symbol_set &>()))::value_type::first_type;
  template <typename T, typename Term>
  using ret_type_2_ = decltype(std::declval<Derived const &>() *
                               std::declval<const k_subs_type<T, Term> &>());
  template <typename T, typename Term>
  using ret_type_2 = typename std::enable_if<
      is_addable_in_place<ret_type_2_<T, Term>>::value &&
          std::is_constructible<ret_type_2_<T, Term>, int>::value,
      ret_type_2_<T, Term>>::type;
  template <typename T, typename Term,
            typename std::enable_if<subs_term_score<Term, T>::value == 2u,
                                    int>::type = 0>
  static ret_type_2<T, Term>
  subs_term_impl(const Term &t, const std::string &name, const integer &n,
                 const T &x, const symbol_set &s_set) {
  ...
  template <typename T, typename Term>
  using ret_type_3 = decltype(std::declval<const cf_subs_type<T, Term> &>() *
                              std::declval<const ret_type_2<T, Term> &>());
  template <typename T, typename Term,
            typename std::enable_if<subs_term_score<Term, T>::value == 3u,
                                    int>::type = 0>
  static ret_type_3<T, Term>
  subs_term_impl(const Term &t, const std::string &name, const integer &n,
                 const T &x, const symbol_set &s_set) {
    // Accumulator for the sum below. This is the same type resulting from case
    // 2.
  ...
  template <typename T>
  using subs_type_ = decltype(subs_term_impl(
      std::declval<typename Series::term_type const &>(),
      std::declval<std::string const &>(), std::declval<const integer &>(),
      std::declval<const T &>(), std::declval<symbol_set const &>()));
  // Enable conditionally based on the common requirements in the ipow_subs()
  // method.
  template <typename T>
  using ipow_subs_type = typename std::enable_if<
      std::is_constructible<subs_type_<T>, int>::value &&
          is_addable_in_place<subs_type_<T>>::value,
      subs_type_<T>>::type;
...
public:
  ...
  operator=(const ipow_substitutable_series &) = default;
  ...
  /**
   * \note
   * This method is enabled only if the coefficient and/or key types support
   * integral power substitution,
   * and if the types involved in the substitution support the necessary
   * arithmetic operations
   * to compute the result.
   *
   * This method will return an object resulting from the substitution of the
   * integral power of the symbol called \p name
  ...
  /**
   * \note
   * This method is enabled only if the other ipow_subs() overload is enabled,
   * and \p Int
   * is a C++ integral type.
   *
   * This is a convenience method that will call the other ipow_subs() overload
   * after converting
   ...
   * @return the result of the substitution.
   *
   * @throws unspecified any exception resulting from calling the other
   * ipow_subs() overload,
   * or by constructing a piranha::integer from a C++ integral type.
   */
  template <typename T, typename Int, ipow_subs_int_enabler<Int> = 0>
  ipow_subs_type<T> ipow_subs(const std::string &name, const Int &n,
                              const T &x) const {
...
using ipow_subs_impl_ipow_subs_series_enabler = typename std::enable_if<
    ...
    true_tt<decltype(std::declval<const Series &>().ipow_subs(
        std::declval<const std::string &>(), std::declval<const integer &>(),
...
/// Specialisation of the piranha::math::ipow_subs_impl functor for instances of
/// piranha::ipow_substitutable_series.
/**
 * This specialisation is activated if \p Series is an instance of
 * piranha::ipow_substitutable_series which supports
 * the substitution methods.
 */
template <typename Series, typename T>
struct ipow_subs_impl<
    Series, T, detail::ipow_subs_impl_ipow_subs_series_enabler<Series, T>> {
  /// Call operator.
  /**
   ...
   * method.
   */
  auto operator()(const Series &s, const std::string &name, const integer &n,
                  const T &x) const -> decltype(s.ipow_subs(name, n, x)) {
...
PIRANHA_TEMPLATE_SERIALIZATION_LEVEL(piranha::ipow_substitutable_series,

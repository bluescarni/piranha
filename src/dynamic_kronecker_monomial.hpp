...
private:
    ...
    static const std::size_t max_size = static_cast<std::size_t>((ksize < 255u ? 255u / ksize : 1u) * ksize);
...
public:
    ...
    v_type unpack(const symbol_set &args) const
    {
        ...
        if (unlikely(!std::all_of(retval.begin() + args.size(), retval.end(),
                                  [](const value_type &n) { return n == value_type(0); }))) {
            piranha_throw(std::invalid_argument, "incompatible symbol set");
        }
        return retval;
    }
    ...
    std::size_t hash() const
    {
        ...
        for (std::size_t i = 0u; std::get<1u>(t) != std::get<2u>(t); ++std::get<1u>(t), ++i) {
            retval = static_cast<std::size_t>(retval + static_cast<std::size_t>(*std::get<1u>(t)) * hash_mixer[i]);
        }
        return retval;
    }
...
template <typename SignedInt, int NBits>
const std::array<std::size_t, dynamic_kronecker_monomial<SignedInt, NBits>::max_p_size>
    dynamic_kronecker_monomial<SignedInt, NBits>::hash_mixer

...
template <typename T> struct is_pmappable {
...
template <typename T> const bool is_pmappable<T>::value;
}

/// Symbol set.
/**
 * This class represents an ordered set of piranha::symbol. The individual
 * piranha::symbol instances
...
class symbol_set {
  ...
  using it_ctor_enabler = typename std::enable_if<
      is_input_iterator<Iterator>::value &&
          std::is_constructible<
              Symbol, decltype(*(std::declval<const Iterator &>()))>::value,
      int>::type;
  ...
  template <class Archive> void load(Archive &ar, unsigned int) {
    ...
    // of bogus input from the archive (e.g., out-of-order symbols, duplicate
    // names,
...
public:
   ...
   * in a piranha::symbol_set \p a, of the symbols in the piranha::symbol_set \p
   * b.
   * The positions are stored internally in an \p std::vector which is
   * guaranteed
   * to be sorted in ascending order and which can be accessed via the iterators
   * provided by this class. If a symbol which is present in \p b is not present
   * in
   * \p a, then it will be ignored.
   *
   * For instance, if the set \p a contains the symbols <tt>[B,C,D,E]</tt> and
   * \p b
   * contains <tt>[A,B,D,F]</tt>, then an instance of this class constructed
   * passing
  ...
  public:
     ...
     */
    std::vector<value_type>::size_type size() const { return m_values.size(); }
  ...
  /// Positions map class.
  /**
   * This class is similar to piranha::symbol_set::positions. In addition to
   * storing the positions
   * of the symbols from a map \p b with respect to a reference
   * piranha::symbol_set \p a, it will also store
   * the instances of \p T that were originally mapped to the symbols in \p b.
   *
   * For instance, if \p T is \p int, \p a contains the symbols
   * <tt>[B,C,D,E]</tt> and \p b is the map
   * <tt>[(A,10),(B,20),(D,30),(F,40)]</tt>, then an instance of this class
   * constructed passing
   * \p a and \p b as parameters will contain the vector
   * <tt>[(0,20),(2,30)]</tt>. The internal vector
   * is guaranteed to be sorted according to the position of the symbols.
   *
   * ## Type requirements ##
   *
   * \p T must be destructible, copy-constructible, move-constructible and
   * move-assignable.
   */
  template <typename T> class positions_map {
  ...
  public:
    ...
    explicit positions_map(const symbol_set &,
                           const std::unordered_map<symbol, T> &);
    /// Deleted copy constructor.
  ...
  /**
   * @throws unspecified any exception thrown by memory allocation errors in \p
   * std::vector.
   */
   ...
   *
   * The set will be initialised with symbols constructed from the elements of
   * the range.
   *
   * @param[in] begin begin iterator.
   * @param[in] end end iterator.
   *
   * @throws unspecified any exception thrown by operations on standard
   * containers or by
  ...
  symbol_set &operator=(symbol_set &&other) noexcept {
    // NOTE: here the idea is that in principle we want to be able to
    // move-assign self,
    // and we don't want to rely on std::vector to support this. Hence, the
    // explicit check.
    if (likely(this != &other)) {
  ...
  /**
   * @return const iterator to the first element of the set, or end() if the set
   * is empty.
   */
  const_iterator begin() const { return m_values.begin(); }
  /// End const iterator.
  ...
  void add(const symbol &s) {
    ...
    if (unlikely(it != new_values.end() && *it == s)) {
      piranha_throw(std::invalid_argument,
                    "symbol already present in this set");
    }
  ...
  /// Add symbol to the set.
  /**
   * Equivalent to constructing a piranha::symbol from \p name and then invoking
   * the other overload of this method.
   *
   * @param[in] name name of the piranha::symbol to be inserted.
   *
   * @throws unspecified any exception thrown by the other overload of this
   * method or by the construction
   * of piranha::symbol from \p std::string.
   */
  void add(const std::string &name) { add(symbol(name)); }
   ...
   * of piranha::symbol from \p std::string.
   */
  void remove(const std::string &name) { remove(symbol(name)); }
  /// Set size.
  /**
   ...
   *
   * @return a new set containing the union of the elements present in \p this
   * and \p other.
   *
  ...
  symbol_set merge(const symbol_set &other) const {
    ...
    auto bi_it =
        std::back_insert_iterator<std::vector<symbol>>(retval.m_values);
   ...
   *
   * @return a new set containing the elements of \p this which are not present
   * in \p other.
   *
   * @throws unspecified any exception thrown by \p std::vector::push_back().
   */
  symbol_set diff(const symbol_set &other) const {
    ...
    std::set_difference(begin(), end(), other.begin(), other.end(),
                        std::back_inserter(retval.m_values));
    piranha_assert(retval.check());
    return retval;
  }
  /// Equality operator.
   ...
   *
   * @return \p true if \p this and \p other contain exactly the same symbols,
   * \p false otherwise.
   */
   ...
   */
  bool operator!=(const symbol_set &other) const { return !(*this == other); }
  /// Index of symbol.
  /**
   * This method will return the index in the set of the input symbol \p s. If
   * \p s is not in the set,
   ...
   *
   * @throws std::overflow_error if the size of the set is larger than an
  ...
  size_type index_of(const symbol &s) const {
    ...
    // This is not an exact check, it is conservative by 1 unit in order to
    // simplify the logic.
    if (unlikely(
            m_values.size() >
            static_cast<udiff_type>(std::numeric_limits<diff_type>::max()))) {
      piranha_throw(std::overflow_error,
                    "potential overflow in the computaion of the "
                    "index of a symbol");
    }
...
/**
 * The internal positions vector will contain the positions in \p a of the
 * elements
 ...
 * containers.
 */
// NOTE: it might make sense here in the future to have a constructor from a
// range of strings instead.
// The rationale would be that like this we would avoid going through string ->
// symbol conversions
// when using this class from series.
inline symbol_set::positions::positions(const symbol_set &a,
                                        const symbol_set &b) {
...
/**
 * The internal vector of pairs will contain the positions in \p a of the mapped
 * values
 ...
 */
template <typename T>
inline symbol_set::positions_map<T>::positions_map(
    const symbol_set &a, const std::unordered_map<symbol, T> &map) {
  ...
  std::stable_sort(m_pairs.begin(), m_pairs.end(),
                   [](const value_type &p1, const value_type &p2) {

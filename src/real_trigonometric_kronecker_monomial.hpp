...
/**
 * This class represents a multivariate real trigonometric monomial, i.e.,
 ...
 * \f]
 * where the integers \f$ n_i \f$ are called <em>multipliers</em>. The values of
 * the multipliers
 * are packed in a signed integer using Kronecker substitution, using the
 * facilities provided
 * by piranha::kronecker_array. The boolean <em>flavour</em> of the monomial
 * indicates whether it represents
 * a cosine (<tt>true</tt>) or sine (<tt>false</tt>).
 *
 * Similarly to an ordinary monomial, this class provides methods to query the
 * <em>trigonometric</em> (partial) (low) degree, defined
 * as if the multipliers were exponents of a regular monomial (e.g., the total
 * trigonometric degree is the sum of the multipliers).
 * Closely related is the concept of trigonometric order, calculated by adding
 * the absolute values of the multipliers.
 *
 * This class satisfies the piranha::is_key, piranha::key_has_t_degree,
 * piranha::key_has_t_ldegree,
 * piranha::key_has_t_order, piranha::key_has_t_lorder and
 * piranha::key_is_differentiable type traits.
 *
 * ## Type requirements ##
 *
 * \p T must be suitable for use in piranha::kronecker_array. The default type
 * for \p T is the signed counterpart of \p std::size_t.
 *
 ...
 * ## Move semantics ##
 *
 * The move semantics of this class are equivalent to the move semantics of C++
 ...
 */
// NOTES:
// - it might make sense, for canonicalisation and is_compatible(), to provide a
// method in kronecker_array to get only
//   the first element of the array. This should be quite fast, and it will
//   provide enough information for the canon/compatibility.
// - related to the above: we can embed the flavour as the first element of the
// kronecker array - at that point checking
//   the flavour is just determining if the int value is even or odd.
// - need to do some reasoning on the impact of the codification in a
// specialised fast poisson series multiplier: how do we deal
//   with the canonical form without going through code/decode? if we require
//   the last multiplier to be always positive (instead
...
public:
  ...
  /// Size type.
  /**
   * Used to represent the number of variables in the monomial. Equivalent to
   * the size type of
...
// Doxygen gets terribly confused by the following.
  ...
  static_assert(
      max_size <= std::numeric_limits<static_vector<int, 1u>::size_type>::max(),
      "Invalid max size.");
  // Eval and subs type definition.
  template <typename U, typename = void> struct eval_type_ {};
  template <typename U>
  using e_type =
      decltype(std::declval<U const &>() * std::declval<value_type const &>());
  template <typename U>
  struct eval_type_<
      U,
      typename std::enable_if<
          is_addable_in_place<e_type<U>>::value &&
  ...
  template <typename U> using eval_type = typename eval_type_<U>::type;
  // Substitution utils.
  template <typename U>
  using subs_cos_type = decltype(math::cos(std::declval<const value_type &>() *
                                           std::declval<const U &>()));
  template <typename U>
  using subs_sin_type = decltype(math::sin(std::declval<const value_type &>() *
                                           std::declval<const U &>()));
  template <typename U, typename = void> struct subs_type_ {};
  template <typename U>
  struct subs_type_<
      U, typename std::enable_if<
             std::is_same<subs_cos_type<U>, subs_sin_type<U>>::value &&
...
#undef PIRANHA_TMP_TYPE
  ...
  using init_list_enabler =
      typename std::enable_if<has_safe_cast<value_type, U>::value, int>::type;
  // Enabler for ctor from iterator.
  template <typename Iterator>
  using it_ctor_enabler = typename std::enable_if<
  ...
  using multiply_enabler = typename std::enable_if<
      is_divisible_in_place<Cf, int>::value && has_negate<Cf>::value &&
  ...
  // Order utils.
  using order_type = decltype(math::abs(std::declval<const T &>()) +
                              math::abs(std::declval<const T &>()));
#endif
public:
  /// Default constructor.
  /**
   * After construction all multipliers in the monomial will be zero, and the
   * flavour will be set to \p true.
   */
  real_trigonometric_kronecker_monomial() : m_value(0), m_flavour(true) {}
  /// Defaulted copy constructor.
  real_trigonometric_kronecker_monomial(
      const real_trigonometric_kronecker_monomial &) = default;
  /// Defaulted move constructor.
  real_trigonometric_kronecker_monomial(
      real_trigonometric_kronecker_monomial &&) = default;
  /// Constructor from initalizer list.
  /**
   * \note
   * This constructor is enabled if \p U is safely convertible to \p T.
   *
   * The values in the initializer list are intended to represent the
   * multipliers of the monomial:
   * they will be safely converted to type \p T (if \p T and \p U are not the
   * same type),
   * encoded using piranha::kronecker_array::encode() and the result assigned to
   * the internal integer instance.
  ...
  /**
   * \note
   * This constructor is enabled if \p Iterator is an input iterator whose \p
   * value_type is safely convertible
   * to \p T.
   *
   * Will build internally a vector of values from the input iterators, encode
   * it and assign the result
   * to the internal integer instance. The value type of the iterator is
   * converted to \p T using
  ...
  explicit real_trigonometric_kronecker_monomial(const Iterator &start,
                                                 const Iterator &end)
      : m_value(0), m_flavour(true) {
  ...
  /**
   * After construction all multipliers will be zero and the flavour will be set
   * to \p true.
   *
   ...
   * - piranha::static_vector::push_back().
   */
  explicit real_trigonometric_kronecker_monomial(const symbol_set &args)
      : m_flavour(true) {
    v_type tmp;
   ...
   */
  explicit real_trigonometric_kronecker_monomial(const value_type &n, bool f)
      : m_value(n), m_flavour(f) {}
  /// Converting constructor.
  /**
   * This constructor is for use when converting from one term type to another
   * in piranha::series. It will
   * set the internal integer instance and flavour to the same value of \p
   * other, after having checked that
   ...
   */
  explicit real_trigonometric_kronecker_monomial(
      const real_trigonometric_kronecker_monomial &other,
  ...
  ~real_trigonometric_kronecker_monomial() {
    ...
    PIRANHA_TT_CHECK(key_is_differentiable,
                     real_trigonometric_kronecker_monomial);
  }
  /// Defaulted copy assignment operator.
  real_trigonometric_kronecker_monomial &
  operator=(const real_trigonometric_kronecker_monomial &) = default;
  /// Defaulted move assignment operator.
  real_trigonometric_kronecker_monomial &
  operator=(real_trigonometric_kronecker_monomial &&) = default;
  /// Set the internal integer instance.
  /**
   * @param[in] n value to which the internal integer instance will be set.
   */
  void set_int(const value_type &n) { m_value = n; }
  /// Get internal instance.
  /**
   * @return value of the internal integer instance.
   */
  value_type get_int() const { return m_value; }
  /// Get flavour.
  /**
   * @return flavour of the monomial.
   */
  bool get_flavour() const { return m_flavour; }
  /// Set flavour.
  /**
   * @param[in] f value to which the flavour will be set.
   */
  void set_flavour(bool f) { m_flavour = f; }
  /// Canonicalise.
  /**
   * A monomial is considered to be in canonical form when the first nonzero
   * multiplier is positive.
   * If \p this is not in canonical form, the method will canonicalise \p this
   * by switching
   ...
   * - the internal integer is not within the limits reported by
   * piranha::kronecker_array::get_limits(),
   * - the first nonzero element of the vector of multipliers represented by the
   * internal integer is negative.
   *
  ...
  bool is_compatible(const symbol_set &args) const noexcept {
    ...
    // If we overflow the maximum size available, we cannot use this object as
    // key in series.
    if (s >= limits.size()) {
      return false;
    }
    ...
    // NOTE: here we have already checked all the conditions that could lead to
    // unpack() throwing, so
  ...
  /// Ignorability check.
  /**
   * A monomial is considered ignorable if all multipliers are zero and the
   * flavour is \p false.
   *
  ...
  /// Merge arguments.
  /**
   * Merge the new arguments set \p new_args into \p this, given the current
   * reference arguments set
   * \p orig_args.
   *
   ...
   */
  real_trigonometric_kronecker_monomial
  merge_args(const symbol_set &orig_args, const symbol_set &new_args) const {
    return real_trigonometric_kronecker_monomial(
        detail::km_merge_args<v_type, ka>(orig_args, new_args, m_value),
        m_flavour);
  }
   ...
   * @param[in] args reference set of piranha::symbol.
   *
   * @return \p true if all the multipliers are zero and the flavour is \p true,
   * \p false otherwise.
   *
  ...
  /**
   * The type returned by this method is the type resulting from the addition of
   * two instances
   * of \p T.
   *
   * @param[in] args reference set of piranha::symbol.
  ...
  degree_type t_ldegree(const symbol_set &args) const { return t_degree(args); }
  /// Partial trigonometric degree.
  /**
   * Partial trigonometric degree of the monomial. The \p p argument is used to
   * indicate which multipliers are to be taken into account when
   * computing the partial degree. Multipliers not in \p p will be discarded
   * during the computation of the partial degree.
   *
   * The type returned by this method is the type resulting from the addition of
   * two instances
   * of \p T.
   ...
   * @param[in] args reference set of piranha::symbol.
   *
   * @return the summation of all the multipliers of the monomial corresponding
   * to the symbols at the positions in
   ...
   * @throws unspecified any exception thrown by unpack().
   */
  degree_type t_degree(const symbol_set::positions &p,
  ...
  /// Partial low trigonometric degree (equivalent to the partial trigonometric
  /// degree).
  degree_type t_ldegree(const symbol_set::positions &p,
                        const symbol_set &args) const {
  ...
  order_type t_order(const symbol_set &args) const {
    ...
    order_type retval(0);
    for (const auto &x : tmp) {
      // NOTE: here the k codification is symmetric, we can always take the
      // negative
      // safely.
      detail::safe_integral_adder(retval,
                                  static_cast<order_type>(math::abs(x)));
    }
    return retval;
  }
  /// Low trigonometric order (equivalent to the trigonometric order).
  order_type t_lorder(const symbol_set &args) const { return t_order(args); }
  /// Partial trigonometric order.
  /**
   * Partial trigonometric order of the monomial. The \p p argument is used to
   * indicate which multipliers are to be taken into account when
   * computing the partial order. Multipliers not in \p p will be discarded
   * during the computation of the partial order.
   *
   * The type returned by this method is the type resulting from the addition of
   * the absolute
   * values of two instances of \p T.
   ...
   * @param[in] args reference set of piranha::symbol.
   *
   * @return the summation of the absolute values of all the multipliers of the
   * monomial corresponding to the symbols at the positions in
   * \p p, or <tt>value_type(0)</tt> if no symbols in \p p appear in \p args.
   *
   * @throws std::invalid_argument if \p p is not compatible with \p args.
   * @throws std::overflow_error if the computation of the degree overflows.
   * @throws unspecified any exception thrown by unpack().
   */
  order_type t_order(const symbol_set::positions &p,
                     const symbol_set &args) const {
    const auto tmp = unpack(args);
    piranha_assert(tmp.size() == args.size());
    if (unlikely(p.size() && p.back() >= tmp.size())) {
    ...
    for (const auto &i : p) {
      detail::safe_integral_adder(retval,
                                  static_cast<order_type>(math::abs(cit[i])));
    }
    return retval;
  }
  /// Partial low trigonometric order (equivalent to the partial trigonometric
  /// order).
  order_type t_lorder(const symbol_set::positions &p,
                      const symbol_set &args) const {
   ...
   *
   * This method will compute the result of the multiplication of the two terms
   * \p t1 and \p t2 with trigonometric key.
   * The result is stored in the two terms of \p res and it is computed using
   * basic trigonometric formulae.
   * Note however that this method will **not** perform the division by two
   * implied by Werner's formulae. Also, in case
   * \p Cf is an instance of piranha::mp_rational, only the numerators of the
   ...
   * @param[in] args reference set of piranha::symbol.
   *
   * @throws std::overflow_error if the computation of the result overflows type
   * \p value_type.
   * @throws unspecified any exception thrown by:
   * - piranha::kronecker_array::encode(),
   * - unpack(),
  ...
  static void
  multiply(std::array<term<Cf, real_trigonometric_kronecker_monomial>,
                      multiply_arity> &res,
           const term<Cf, real_trigonometric_kronecker_monomial> &t1,
           const term<Cf, real_trigonometric_kronecker_monomial> &t2,
           const symbol_set &args) {
    // Coefficients first.
    ...
    // Flags to signal if a sign change in the multipliers was needed as part of
    ...
    for (typename v_type::size_type i = 0u; i < size; ++i) {
      ...
      // NOTE: it is safe here to take the negative because in kronecker_array
      // we are guaranteed
      // that the range of each element is symmetric, so if tmp2[i] is
      // representable also -tmp2[i] is.
      // NOTE: the static cast here is because if value_type is narrower than
      // int, the unary minus will promote
      // to int and safe_adder won't work as it expects identical types.
      result_minus.push_back(tmp1[i]);
      detail::safe_integral_adder(result_minus[i],
                                  static_cast<value_type>(-tmp2[i]));
    }
    // Handle sign changes.
    sign_plus = canonicalise_impl(result_plus);
    sign_minus = canonicalise_impl(result_minus);
    // Compute them before assigning, so in case of exceptions we do not touch
    // the return values.
    const auto re_plus = ka::encode(result_plus),
               re_minus = ka::encode(result_minus);
    ...
    // If multiplier sign was changed and the result is a sine, negate the
    // coefficient.
   ...
   */
  std::size_t hash() const { return static_cast<std::size_t>(m_value); }
  /// Equality operator.
  /**
   * @param[in] other comparison argument.
   *
   * @return \p true if the internal integral instance and the flavour of \p
   * this are the same of \p other,
   * \p false otherwise.
   */
  ...
  /**
   * Will decode the internal integral instance into a piranha::static_vector of
   * size equal to the size of \p args.
   *
   * @param[in] args reference set of piranha::symbol.
   *
   * @return piranha::static_vector containing the result of decoding the
   * internal integral instance via
   * piranha::kronecker_array.
   *
   * @throws std::invalid_argument if the size of \p args is larger than the
   * maximum size of piranha::static_vector.
   * @throws unspecified any exception thrown by
   * piranha::kronecker_array::decode().
   */
   ...
   * @param[in] args reference set of piranha::symbol.
   *
   * @throws unspecified any exception thrown by unpack() or by streaming
   * instances of \p value_type.
   */
  void print(std::ostream &os, const symbol_set &args) const {
    ...
    for (decltype(tmp.size()) i = 0u; i < tmp.size(); ++i) {
      if (tmp[i] != zero) {
        // A positive multiplier, in case previous output exists, must be
        // preceded
        // by a "+" sign.
        if (tmp[i] > zero && !empty_output) {
          os << '+';
        }
        // Print the multiplier, unless it's "-1": in that case, just print the
        // minus sign.
        if (tmp[i] == m_one) {
          os << '-';
        } else if (tmp[i] != one) {
   ...
   * the symbol at the position indicated by \p p.
   * The result is a pair consisting of the multiplier associated to \p p and a
   * copy of the monomial.
   * The sign of the multiplier and the flavour of the resulting monomial are
   ...
   * the returned pair will be
   * <tt>(0,real_trigonometric_kronecker_monomial{args})</tt>.
   *
   * @param[in] p position of the symbol with respect to which the
   * differentiation will be calculated.
   * @param[in] args reference set of piranha::symbol.
   *
   * @return result of the differentiation.
   *
   * @throws std::invalid_argument if \p p is incompatible with \p args or it
   * has a size greater than one.
   * @throws unspecified any exception thrown by:
   * - unpack(),
   * - piranha::math::is_zero().
   */
  std::pair<T, real_trigonometric_kronecker_monomial>
  partial(const symbol_set::positions &p, const symbol_set &args) const {
    auto v = unpack(args);
    // Cannot take derivative wrt more than one variable, and the position of
    // that variable
    // must be compatible with the monomial.
    if (p.size() > 1u || (p.size() == 1u && p.back() >= args.size())) {
      piranha_throw(std::invalid_argument,
                    "invalid size of symbol_set::positions");
    }
    // Derivative wrt a variable not in the monomial: position is empty, or
    // refers to a
    // variable with zero multiplier.
    // NOTE: safe to take v.begin() here, as the checks on the positions above
    // ensure
  ...
  /// Integration.
  /**
   * Will return the antiderivative of \p this with respect to symbol \p s. The
   * result is a pair
   ...
   * the returned pair will be
   * <tt>(0,real_trigonometric_kronecker_monomial{})</tt>.
   *
   * @param[in] s symbol with respect to which the integration will be
   * calculated.
   * @param[in] args reference set of piranha::symbol.
   *
   ...
   */
  std::pair<T, real_trigonometric_kronecker_monomial>
  integrate(const symbol &s, const symbol_set &args) const {
    auto v = unpack(args);
    for (min_int<decltype(args.size()), typename v_type::size_type> i = 0u;
         i < args.size(); ++i) {
  ...
  /**
   * \note
   * This template method is activated only if \p U supports the mathematical
   * operations needed to compute
   * the return type, and if it is suitable for use in
   * piranha::symbol_set::positions_map.
   *
   * The return value will be built by applying piranha::math::cos() or
   * piranha::math:sin()
   * to the linear combination of the values in \p pmap with the multipliers.
   * If the size of the monomial is zero, 1 will be returned if the monomial is
   * a cosine, 0 otherwise.
   * If the positions in \p pmap do not reference
   * only and all the multipliers in the monomial, an error will be thrown.
   *
   * @param[in] pmap piranha::symbol_set::positions_map that will be used for
   * substitution.
   * @param[in] args reference set of piranha::symbol.
   *
   * @return the result of evaluating \p this with the values provided in \p
   * pmap.
   *
   ...
   * - piranha::math::cos(), piranha::math::sin(), or the in-place addition and
   * binary multiplication operators of the types
   *   involved in the computation.
   */
  template <typename U>
  eval_type<U> evaluate(const symbol_set::positions_map<U> &pmap,
                        const symbol_set &args) const {
    using return_type = eval_type<U>;
    if (unlikely(pmap.size() != args.size() ||
                 (pmap.size() && pmap.back().first != pmap.size() - 1u))) {
      piranha_throw(std::invalid_argument,
                    "invalid positions map for evaluation");
    }
    // Run the unpack before the checks below in order to check the suitability
    // of args.
    auto v = unpack(args);
    if (args.size() == 0u) {
      if (get_flavour()) {
    ...
    using tmp_type = decltype(std::declval<U const &>() *
                              std::declval<value_type const &>());
    tmp_type tmp(0);
    auto it = pmap.begin();
    for (min_int<decltype(args.size()), typename v_type::size_type> i = 0u;
      ...
      // NOTE: here it might make sense to use multiply_accumulate. There might
      // be a perf gain
      // for things like real. Maybe fast FMA on Haswell too. Remember to adapt
   ...
   * - the value type can be multiplied by \p U, and the result can be passed to
   * piranha::math::cos()
   *   or piranha::math::sin(), yielding a type \p subs_type,
   * - \p subs_type is constructible from \p int, assignable and it supports
   * piranha::math::negate().
   *
   * Substitute the symbol called \p s in the monomial with quantity \p x. The
   * return value is a vector of two pairs
   * computed according to the standard angle sum identities. That is, given a
   ...
   * \left(na + b\right)
   * \f]
   * in which the symbol \f$ a \f$ is to be substituted with \f$ x \f$, the
   * result of the substitution will be
   * one of
   * \f[
   * \begin{array}{c}
   ...
   * \end{array}
   * \f]
   * where \f$ \cos b \f$ and \f$ \sin b \f$ are returned as monomials, and \f$
   * \cos nx \f$ and \f$ \sin nx \f$
   * as the return values of piranha::math::cos() and piranha::math::sin(). If
   * \p s is not in \p args,
   * \f$ \cos nx \f$ will be initialised to 1 and \f$ \sin nx \f$ to 0. If,
   * after the substitution, the first nonzero multiplier
   * in \f$ b \f$ is negative, \f$ b \f$ will be negated and the other signs
   * changed accordingly.
   *
  ...
  subs(const std::string &s, const U &x, const symbol_set &args) const {
    ...
    using ret_type = std::vector<
        std::pair<subs_type<U>, real_trigonometric_kronecker_monomial>>;
    ret_type retval;
    ...
    real_trigonometric_kronecker_monomial cos_key(new_int, true),
        sin_key(new_int, false);
    if (get_flavour()) {
      retval.push_back(
          std::make_pair(std::move(retval_s_cos), std::move(cos_key)));
      retval.push_back(
      ...
      retval.push_back(
          std::make_pair(std::move(retval_s_sin), std::move(cos_key)));
      retval.push_back(
  ...
  /**
   * \note
   * This method is enabled only if \p U supports the mathematical operations
   * needed to compute the result. In particular,
   * the implementation uses piranha::math::binomial() internally (thus, \p U
   * must be interoperable with piranha::integer).
   *
   * This method works in the same way as the subs() method, but the cosine \p c
   * and sine \p s of \p name will be substituted (instead of a direct
   * substitution of \p name).
   * The substitution is performed using standard trigonometric formulae, and it
   * will result in a list of two (substitution result,new monomial) pairs.
   *
   ...
   * @param[in] args reference set of piranha::symbol.
   *
   * @return the result of substituting \p c and \p s for the cosine and sine of
   * \p name.
   *
   * @throws unspecified any exception thrown by:
   * - unpack(),
   * - construction, assignment and arithmetics on the return value and on the
   * intermediary values invovled in the computation,
  ...
  t_subs(const std::string &name, const U &c, const U &s,
         const symbol_set &args) const {
    ...
    res_type cos_nx((cos_phase(abs_n) * math::binomial(abs_n, value_type(0))) *
        ...
        sin_nx((sin_phase(abs_n) * math::binomial(abs_n, value_type(0))) *
               (c_map[value_type(0)] * s_map[abs_n]));
    for (value_type k(0); k < abs_n; ++k) {
      const value_type p = abs_n - (k + value_type(1));
      piranha_assert(p >= value_type(0));
      // NOTE: here the type is slightly different from the decltype() that
      // determines the return type, but as long
      // as binomial(value_type,value_type) returns integer there will be no
      // difference because of the
      // left-to-right associativity of multiplication.
      res_type tmp(math::binomial(abs_n, k + value_type(1)) *
                   (c_map[k + value_type(1)] * s_map[p]));
  ...
  /**
   * This method is used in piranha::series::trim(). The input parameter \p
   * candidates
   * contains a set of symbols that are candidates for elimination. The method
   * will remove
  ...
  /// Trim.
  /**
   * This method will return a copy of \p this with the multipliers associated
   * to the symbols
   ...
   */
  real_trigonometric_kronecker_monomial
  trim(const symbol_set &trim_args, const symbol_set &orig_args) const {
    return real_trigonometric_kronecker_monomial(
        detail::km_trim<v_type, ka>(trim_args, orig_args, m_value), m_flavour);
  }
  /// Comparison operator.
  /**
   * The values of the internal integral instances are used for comparison. If
...
/// Specialisation of \p std::hash for
/// piranha::real_trigonometric_kronecker_monomial.
template <typename T>
struct hash<piranha::real_trigonometric_kronecker_monomial<T>> {
   ...
   *
   * @return hash value of \p a computed via
   * piranha::real_trigonometric_kronecker_monomial::hash().
   */

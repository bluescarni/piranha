...
template <typename UInt> inline UInt clear_top_bits(UInt input, unsigned n) {
...
struct read_uint_opt {
  static const bool value =
      IBits == 0u && RBits == 0u && std::is_same<URet, UIn>::value;
};

// Considering an array of UIn (unsigned integrals) of a certain size as a
// continuous sequence of bits,
// read the index-th URet (unsigned integral) that can be extracted from the
// sequence of bits.
// The parameter IBits is the number of upper bits of UIn that should be
// discarded in the computation
// (i.e., not considered as part of the continuous sequence of bits). RBits has
// the same meaning,
// but for the output value: it's the number of upper bits that are not
// considered as part of the
// return type.
template <typename URet, unsigned IBits = 0u, unsigned RBits = 0u, typename UIn,
          typename std::enable_if<
              !read_uint_opt<URet, IBits, RBits, UIn>::value, int>::type = 0>
inline URet read_uint(const UIn *ptr, std::size_t size, std::size_t index) {
  // We can work only with unsigned integer types.
  static_assert(std::is_integral<UIn>::value && std::is_unsigned<UIn>::value,
                "Invalid type.");
  static_assert(std::is_integral<URet>::value && std::is_unsigned<URet>::value,
                "Invalid type.");
  // Bit size of the return type.
  constexpr unsigned r_bits =
      static_cast<unsigned>(std::numeric_limits<URet>::digits);
  // Bit size of the input type.
  constexpr unsigned i_bits =
      static_cast<unsigned>(std::numeric_limits<UIn>::digits);
  // The ignored bits in the input type cannot be larger than or equal to its
  // bit size.
  ...
  // NOTE: we need to protect against multiplication overflows in the upper
  // level routine.
...
template <typename URet, unsigned IBits = 0u, unsigned RBits = 0u, typename UIn,
          typename std::enable_if<read_uint_opt<URet, IBits, RBits, UIn>::value,
                                  int>::type = 0>
inline URet read_uint(const UIn *ptr, std::size_t size, std::size_t index) {
  (void)size;
  // We can work only with unsigned integer types.
...
static_assert(
    sizeof(expected_mpz_struct_t) == sizeof(mpz_struct_t) &&
        std::is_standard_layout<mpz_struct_t>::value &&
        std::is_standard_layout<expected_mpz_struct_t>::value &&
        offsetof(mpz_struct_t, _mp_alloc) == 0u &&
        offsetof(mpz_struct_t, _mp_size) ==
            offsetof(expected_mpz_struct_t, _mp_size) &&
        offsetof(mpz_struct_t, _mp_d) ==
            offsetof(expected_mpz_struct_t, _mp_d) &&
        std::is_same<mpz_alloc_t,
                     decltype(std::declval<mpz_struct_t>()._mp_alloc)>::value &&
        std::is_same<mpz_size_t,
                     decltype(std::declval<mpz_struct_t>()._mp_size)>::value &&
        std::is_same<::mp_limb_t *,
                     decltype(std::declval<mpz_struct_t>()._mp_d)>::value &&
        // mp_bitcnt_t is used in shift operators, so we double-check it is
        // unsigned. If it is not
        // we might end up shifting by negative values, which is UB.
        std::is_unsigned<::mp_bitcnt_t>::value &&
        // Sanity check on GMP_LIMB_BITS. We use both GMP_LIMB_BITS and
        // numeric_limits interchangeably,
...
inline std::ostream &stream_mpz(std::ostream &os, const mpz_struct_t &mpz) {
  ...
  if (unlikely(size_base10 > std::numeric_limits<std::size_t>::max() -
...
template <int NBits> struct static_integer {
  ...
  static const unsigned total_bits =
      static_cast<unsigned>(std::numeric_limits<limb_t>::digits);
  ...
  static_assert(limb_bits < std::numeric_limits<limb_t>::max() / 2u,
                "Overflow error.");
  ...
  bool attempt_direct_construction(T n) {
    try {
      if (n >= T(0)) {
        // We can attempt conversion of a non-negative signed directly into
        // limb_t.
  ...
  template <typename Integer> void construct_from_integral(Integer n) {
    ...
    while (n != Integer(0)) {
      ...
      // NOTE: in C++11 division will round to zero always (for negative numbers
      // as well).
      // The bit shift operator >> has implementation defined behaviour if n is
      // signed and negative,
      // so we do not use it.
      const Integer quot = static_cast<Integer>(n / Integer(2)),
                    rem = static_cast<Integer>(n % Integer(2));
      if (rem) {
        set_bit(bit_idx);
      }
      n = quot;
  ...
  template <typename Integer, typename = typename std::enable_if<
  ...
  ~static_integer() { piranha_assert(consistency_checks()); }
  ...
  void set_bit(const limb_t &idx) {
    ...
    const auto quot = static_cast<limb_t>(idx / limb_bits),
               rem = static_cast<limb_t>(idx % limb_bits);
    m_limbs[static_cast<size_type>(quot)] =
        static_cast<limb_t>(m_limbs[static_cast<size_type>(quot)] |
                            static_cast<limb_t>(limb_t(1) << rem));
    // Update the size if needed. The new size must be at least quot + 1, as we
    // set a bit
  ...
  public:
    ...
    static_assert(max_tot_nbits / unsigned(GMP_NUMB_BITS) + 1u <=
    ...
    explicit static_mpz_view(const static_integer &n) {
      ...
      static_assert(unsigned(GMP_LIMB_BITS) <=
                        std::numeric_limits<std::size_t>::max() /
                            max_n_gmp_limbs,
                    "Overflow error.");
      ...
      static_assert(max_n_gmp_limbs <=
                        static_cast<std::make_unsigned<mpz_alloc_t>::type>(
    ...
    operator const mpz_struct_t *() const { return &m_mpz; }
  ...
  public:
    ...
    // NOTE: we use the const_cast to cast away the constness from the pointer
    // to the limbs
    // in n. This is valid as we are never going to use this pointer for
    // writing.
    explicit static_mpz_view(const static_integer &n)
        : m_mpz{static_cast<mpz_alloc_t>(2), n._mp_size,
                const_cast<::mp_limb_t *>(n.m_limbs.data())} {}
    static_mpz_view(const static_mpz_view &) = delete;
    static_mpz_view(static_mpz_view &&) = default;
    static_mpz_view &operator=(const static_mpz_view &) = delete;
    static_mpz_view &operator=(static_mpz_view &&) = delete;
  ...
  static_mpz_view<> get_mpz_view() const { return static_mpz_view<>{*this}; }
  ...
  bool is_zero() const { return _mp_size == 0; }
  bool is_unitary() const { return _mp_size == 1 && m_limbs[0u] == 1u; }
  // Compare absolute values of two integers whose sizes are the same in
  // absolute value.
  static int compare(const static_integer &a, const static_integer &b,
                     const mpz_size_t &size) {
    using size_type = typename limbs_type::size_type;
  ...
  bool operator<(const static_integer &other) const {
    ...
    if (size0 < size1) {
      ...
      const mpz_size_t abs_size =
          static_cast<mpz_size_t>(size0 >= 0 ? size0 : -size0);
  ...
  // NOTE: the idea here is that it could happen that limb_bits is smaller than
  // the actual total bits
  // used for the representation of limb_t. In such a case, in arithmetic
  // operations whenever we cast from
  // dlimb_t to limb_t or exploit wrap-around arithmetics, we might have extra
  // bits past limb_bits set
  // that need to be set to zero for consistency.
  template <typename T = static_integer>
  void clear_extra_bits(
      typename std::enable_if<T::limb_bits != T::total_bits>::type * =
          nullptr) {
  ...
  void clear_extra_bits(
      typename std::enable_if<T::limb_bits == T::total_bits>::type * =
          nullptr) {}
  static int raw_add(static_integer &res, const static_integer &x,
                     const static_integer &y) {
    piranha_assert(x.abs_size() <= 2 && y.abs_size() <= 2);
    const dlimb_t lo = static_cast<dlimb_t>(
        static_cast<dlimb_t>(x.m_limbs[0u]) + y.m_limbs[0u]);
    const dlimb_t hi = static_cast<dlimb_t>(
        (static_cast<dlimb_t>(x.m_limbs[1u]) + y.m_limbs[1u]) +
        (lo >> limb_bits));
  ...
  static void raw_sub(static_integer &res, const static_integer &x,
                      const static_integer &y) {
    piranha_assert(x.abs_size() <= 2 && y.abs_size() <= 2);
    ...
    res.m_limbs[1u] = static_cast<limb_t>((x.m_limbs[1u] - y.m_limbs[1u]) -
                                          limb_t(has_borrow));
    res._mp_size = res.calculate_n_limbs();
    res.clear_extra_bits();
  }
  template <bool AddOrSub>
  static int add_or_sub(static_integer &res, const static_integer &x,
                        const static_integer &y) {
    mpz_size_t asizex = x._mp_size,
               asizey =
                   static_cast<mpz_size_t>(AddOrSub ? y._mp_size : -y._mp_size);
  ...
  static int add(static_integer &res, const static_integer &x,
                 const static_integer &y) {
    return add_or_sub<true>(res, x, y);
  }
  static int sub(static_integer &res, const static_integer &x,
                 const static_integer &y) {
    return add_or_sub<false>(res, x, y);
  }
  static void raw_mul(static_integer &res, const static_integer &x,
                      const static_integer &y, const mpz_size_t &asizex,
    ...
    res._mp_size =
        static_cast<mpz_size_t>((asizex + asizey) - mpz_size_t(cy_limb == 0u));
    res.clear_extra_bits();
    piranha_assert(res._mp_size > 0);
  }
  static int mul(static_integer &res, const static_integer &x,
                 const static_integer &y) {
  ...
  static_integer operator+() const { return *this; }
  friend static_integer operator+(const static_integer &x,
                                  const static_integer &y) {
  ...
  int multiply_accumulate(const static_integer &b, const static_integer &c) {
    ...
    if (signa == signtmp) {
      ...
      if (asizea > asizetmp ||
          (asizea == asizetmp && compare(*this, tmp, asizea) >= 0)) {
  ...
  int lshift(::mp_bitcnt_t n) {
    ...
    if (n >= limb_bits) {
      ...
      if (n == limb_bits) {
        ...
        _mp_size = (_mp_size > 0) ? static_cast<mpz_size_t>(2)
                                  : static_cast<mpz_size_t>(-2);
        return 0;
      }
    ...
    mpz_size_t asize = 1;
    asize = static_cast<mpz_size_t>(asize +
                                    static_cast<mpz_size_t>(m_limbs[1u] != 0u));
  ...
  // Division.
  static void div(static_integer &q, static_integer &r, const static_integer &a,
                  const static_integer &b) {
    piranha_assert(!b.is_zero());
    // NOTE: here in principle q/r could overlap with a or b (e.g., in in-place
    // division).
    // We need to first read everything we need from a and b, and only then
    // write into q/r.
    // Store the signs.
    const bool signa = a._mp_size >= 0, signb = b._mp_size >= 0;
    // Compute the result in dlimb_t.
    const dlimb_t ad = static_cast<dlimb_t>(
                      a.m_limbs[0u] +
                      (static_cast<dlimb_t>(a.m_limbs[1u]) << limb_bits)),
                  bd = static_cast<dlimb_t>(
                      b.m_limbs[0u] +
                      (static_cast<dlimb_t>(b.m_limbs[1u]) << limb_bits));
    const dlimb_t qd = static_cast<dlimb_t>(ad / bd),
                  rd = static_cast<dlimb_t>(ad % bd);
  ...
  limb_t test_bit(const limb_t &idx) const {
    ...
    const auto quot = static_cast<limb_t>(idx / limb_bits),
               rem = static_cast<limb_t>(idx % limb_bits);
    return (static_cast<limb_t>(m_limbs[static_cast<size_type>(quot)] &
                                static_cast<limb_t>(limb_t(1u) << rem)) != 0u);
  }
  // Some metaprogramming to make sure we can compute the hash safely. A bit of
  // paranoid defensive programming.
  struct hash_checks {
    // Total number of bits that can be stored. We know already this operation
    // is safe.
    static const limb_t tot_bits = static_cast<limb_t>(limb_bits * 2u);
    static const unsigned nbits_size_t =
    ...
    static const bool value =
        ...
        // Convert n_size_t to std::size_t for use as function param in
        // read_uint.
        (max_n_size_t < std::numeric_limits<std::size_t>::max()) &&
        // Multiplication in read_uint that could overflow. max_n_size_t - 1u is
        // the maximum
        // value of the index param.
        (limb_t(max_n_size_t - 1u) <
         std::size_t(std::numeric_limits<std::size_t>::max() / nbits_size_t));
  };
  std::size_t hash() const {
    ...
    const unsigned nbits_size_t = static_cast<unsigned>(
                       std::numeric_limits<std::size_t>::digits),
                   tot_nbits = static_cast<unsigned>(
                       limb_bits * static_cast<unsigned>(asize)),
                   q = tot_nbits / nbits_size_t, r = tot_nbits % nbits_size_t,
                   n_size_t = q + unsigned(r != 0u);
    for (unsigned i = 0u; i < n_size_t; ++i) {
      boost::hash_combine(retval,
                          read_uint<std::size_t, total_bits - limb_bits>(
                              &m_limbs[0u], 2u, static_cast<std::size_t>(i)));
    }
    return retval;
  }
  mpz_alloc_t _mp_alloc;
...
// Integer union.
template <int NBits> union integer_union {
public:
  ...
  bool is_static() const { return m_st._mp_alloc == 0; }
  static bool fits_in_static(const mpz_struct_t &mpz) {
    // NOTE: sizeinbase returns the index of the highest bit *counting from 1*
    // (like a logarithm).
...
template <typename T> struct is_mp_integer_interoperable_type {
  static const bool value =
...
/**
 * This class is a wrapper around the GMP arbitrary precision \p mpz_t type, and
 * it can represent integer numbers of arbitrary size
 * (i.e., the range is limited only by the available memory).
 *
 * As an optimisation, this class will store in static internal storage a fixed
 * number of digits before resorting to dynamic
 * memory allocation. The internal storage consists of two limbs of size \p
 * NBits bits, for a total of <tt>2*NBits</tt> bits
 * of static storage. The possible values for \p NBits, supported on all
 * platforms, are 8, 16, and 32.
 * A value of 64 is supported on some platforms. The special
 * default value of 0 is used to automatically select the optimal \p NBits value
 * on the current platform.
 *
 * ## Interoperability with other types ##
 *
 * Full interoperability with all integral and floating-point C++ types is
 * provided.
 *
 * Every function interacting with floating-point types will check that the
 * floating-point values are not
 * non-finite: in case of infinities or NaNs, an <tt>std::invalid_argument</tt>
 * exception will be thrown.
 * It should be noted that interoperability with floating-point types is
 * provided for convenience, and it should
 * not be relied upon in case exact results are required (e.g., the conversion
 * of a large integer to a floating-point value
 * might not be exact).
 *
 * ## Exception safety guarantee ##
 *
 * This class provides the strong exception safety guarantee for all operations.
 * In case of memory allocation errors by GMP routines,
 ...
 *
 * This class uses, for certain routines, the internal interface of GMP
 * integers, which is not guaranteed to be stable
 * across different versions. GMP versions 4.x, 5.x and 6.x are explicitly
 ...
 *   - it seems like for a bunch of operations we do not need GMP anymore (e.g.,
 * conversion to float),
 *     we can use mp_integer directly - this could be a performance improvement;
 *   - avoid going through mpz for print to stream.
 * - consider if and how to implement demoting. It looks it could be useful in
 * certain cases, for instance when we
 *   rely on GMP routines (we should demote back to static if possible in those
 * cases). For the elementary operations,
 *   it is less clear: addition, subtraction and division could in principle be
 * considered for demotion. But, OTOH
 *   GMP never scales back the allocated memory and, for instance, also the
 * std:: containers do not normally reduce
 *   their sizes. There might be some lesson in there;
 * - understand the performance implications of implementing the binary operator
 * as += and copy. Might be that creating an
 *   empty retval and then filling it with mpz_add() or a similar free function
 * is more efficient. See how it is done
 *   in Arbpp for instance. This should matter much more for mp_integer and not
 * mp_rational, as there we always
 *   have the canonicalisation to do anyway.
 *   Note that now we have the ternary arithmetic functions implemented, which
 * could be used for this.
 * - apparently, the mpfr devs are considering adding an fma-like functions that
 * computes ab +/- cd. It seems like this would
 *   be useful for both rational and complex numbers, maybe we could implement
 * it here as well.
 * - the speed of conversion to floating-point might matter in series
 * evaluation. Consider what happens when evaluating
 *   cos(x) in which x an mp_integer (passed in from Python, for instance). If
 * we overload cos() to produce a double for int argument,
 *   then we need to convert x to double and then compute cos(x). Note that for
 * 1-limb numbers we actually could do directly
 *   the conversion to double of the limb, this should be quite fast.
 * - when converting to/from Python we can speed up operations by trying casting
 * around to hardware integers, if range is enough.
 * - use a unified shortcut for the possible optimisation when the two limb type
 * coincide (e.g., same_limbs_type = true constexpr).
 * - the conversion operator to C++ integral types could use the same
 * optimisation as the constructor from integral types (e.g,
 *   attempt direct conversion if we have only 1 limb) -> this is now done. We
 * should consider piggybacking on the GMP functions
 *   when possible, and use our generic implementation only as last resort.
 * - more in general, for conversions to/from other types we should consider
 * operating directly with limbs instead of bit-by-bit
 *   for increased performance. We should be able to do this with the new shift
 * operators.
 * - there is some out-of-range conversion handling which is not hit by the test
 * cases, need to proper test it.
 * - fits_in_static() might be made obsolete by the new mpz_t ctor.
 * fits_in_static() is more accurate, but potentially
 *   slower and prone to overflow. See if it makes sense to replace it.
 * - use the GMP facilities via mpz_view and/or the mpz_t constructor when
 * interacting with float and double? Maybe pass through a real
 *   for interaction with long doubles? This might need a thread local
 * mpz_t/real/mpfr_t in order to avoid having to allocate at each
 *   construction, but for thread local we have the usual issue on OSX.
 * - if GMP ever adopts sane behaviour for memory errors or if we ever move away
 * from it, we probably need to review the exception behaviour,
 *   and possibly re-implement a bunch of things. For instance, now the
 * copy-assignment operator is defaulted in mp_rational, but if exceptions
 *   are allowed then we need to change the implementation to the copy+move
 * idiom.
 */
template <int NBits = 0> class mp_integer {
  // Make friend with debugging class, mp_rational and real.
  template <typename> friend class debug_access;
  template <int> friend class mp_rational;
  friend class real;
  ...
  using generic_ctor_enabler =
      typename std::enable_if<is_interoperable_type<T>::value, int>::type;
  // Cast enabler.
  template <typename T> using cast_enabler = generic_ctor_enabler<T>;
  // Enabler for in-place arithmetic operations with interop on the left.
  template <typename T>
  using generic_in_place_enabler = typename std::enable_if<
      is_interoperable_type<T>::value && !std::is_const<T>::value, int>::type;
  ...
  void construct_from_interoperable(
      const Float &x,
      typename std::enable_if<std::is_floating_point<Float>::value>::type * =
          nullptr) {
    if (unlikely(!std::isfinite(x))) {
      piranha_throw(std::invalid_argument, "cannot construct an integer from a "
    ...
    while (exp >= 0) {
      ...
      // get to abs_x == 0, in which case we have to prevent the call to ilogb
      // below.
      if (unlikely(abs_x == Float(0))) {
        break;
      }
      // NOTE: in principle, here ilogb could return funky values if something
      // goes wrong
  ...
  void construct_from_interoperable(
    ...
    try {
      ...
      // Check that everything was cleaned properly, in case the exception was
      // triggered,
      // and continue.
    ...
    while (n != Integer(0)) {
      Integer div = static_cast<Integer>(n / Integer(2)),
              rem = static_cast<Integer>(n % Integer(2));
      if (rem != Integer(0)) {
      ...
      if (unlikely(bit_idx == std::numeric_limits<::mp_bitcnt_t>::max())) {
        piranha_throw(std::invalid_argument,
                      "overflow in the construction from integral type");
      }
      ++bit_idx;
      n = div;
    }
  ...
  static void validate_string(const char *str, const std::size_t &size) {
    if (!size) {
      piranha_throw(std::invalid_argument,
                    "invalid string input for integer type");
    }
    const std::size_t has_minus = (str[0] == '-'),
                      signed_size = static_cast<std::size_t>(size - has_minus);
    if (!signed_size) {
      piranha_throw(std::invalid_argument,
                    "invalid string input for integer type");
    }
    // A number starting with zero cannot be multi-digit and cannot have a
    // leading minus sign (no '-0' allowed).
  ...
  void construct_from_string(const char *str) {
    // NOTE: it seems to be ok to call strlen on a char pointer obtained from
    // std::string::c_str()
    // (as we do in the constructor from std::string). The output of c_str() is
    // guaranteed
    // to be NULL terminated, and if the string is empty, this will still work
    // (21.4.7 and around).
  ...
  template <typename T>
  static void check_mult2(
      const T &n,
      typename std::enable_if<std::is_signed<T>::value>::type * = nullptr) {
    if (n < std::numeric_limits<T>::min() / T(2) ||
        n > std::numeric_limits<T>::max() / T(2)) {
      piranha_throw(std::overflow_error,
                    "overflow in conversion to integral type");
    }
  }
  template <typename T>
  static void check_mult2(
  ...
  // Conversion of 1-limb statics. Can fail depending on T. The flag param is
  // initially
  ...
  void attempt_fast_1_limb_conversion(T &retval, bool &flag) const {
    ...
    if (m_int.g_st()._mp_size == -1) {
      try {
        ...
        // In this case we don't want to do anything as the conversion might
        // still
  ...
  template <typename T>
  T convert_to_impl(typename std::enable_if<std::is_integral<T>::value &&
        ...
        if (m_int.g_st().test_bit(i)) {
          if (negative && retval < std::numeric_limits<T>::min() - tmp) {
            piranha_throw(std::overflow_error,
                     ...
                     retval > std::numeric_limits<T>::max() - tmp) {
            piranha_throw(std::overflow_error,
                          "overflow in conversion to integral type");
          }
  ...
  // Special casing for bool.
  template <typename T>
  T convert_to_impl(typename std::enable_if<std::is_same<T, bool>::value>::type
                        * = nullptr) const {
    return sign() != 0;
  }
  ...
  template <typename T>
  T convert_to_impl(
      typename std::enable_if<std::is_same<double, T>::value>::type * =
          nullptr) const {
  ...
  template <typename T>
  T convert_to_impl(typename std::enable_if<std::is_same<float, T>::value>::type
                        * = nullptr) const {
  ...
  template <typename T>
  T convert_to_impl(
      typename std::enable_if<std::is_same<long double, T>::value>::type * =
          nullptr) const {
    // For long double, create a temporary mpfr, init it with this, then extract
    // the long double.
    // NOTE: here we should really use a thread local mpfr_t wrapped into a RAII
    // holder.
    // Overflow check for the operation below.
    static_assert(std::numeric_limits<long double>::digits10 <
                      (std::numeric_limits<int>::max() - 10) / 3,
                  "Overflow error.");
    // This is the number of digits in base 2 needed to represent exactly any
    // long double.
    // The exact number should be  + 1 instead of + 10, but let's be
    // conservative.
  ...
  mp_integer &in_place_add_or_sub(const mp_integer &other) {
    ...
    if (s1 && s2) {
      ...
      // will be returned. The operation is safe, and m_int.g_st() will be
      // untouched
      // in case of problems.
      int status;
      if (AddOrSub) {
  ...
  static mp_integer binary_plus(
      ...
      typename std::enable_if<std::is_same<T, mp_integer>::value &&
                              std::is_same<U, mp_integer>::value>::type * =
          nullptr) {
  ...
  static mp_integer binary_plus(
    ...
    // NOTE: for binary ops, let's do first the conversion to mp_integer and
    // then
  ...
  static mp_integer binary_plus(
      const T &n1, const U &n2,
      typename std::enable_if<std::is_same<U, mp_integer>::value &&
  ...
  template <typename T>
  static T binary_plus(
      const mp_integer &n, const T &x,
      typename std::enable_if<std::is_floating_point<T>::value>::type * =
          nullptr) {
  ...
  mp_integer &in_place_mul(const mp_integer &other) {
    ...
    if (s1 && s2) {
      ...
      if (likely(!m_int.g_st().mul(m_int.g_st(), m_int.g_st(),
                                   other.m_int.g_st()))) {
        return *this;
      }
    }
  ...
  template <typename T>
  static T
  binary_mul(const mp_integer &n, const T &x,
             typename std::enable_if<std::is_floating_point<T>::value>::type * =
                 nullptr) {
    return (static_cast<T>(n) * x);
  }
  template <typename T>
  static T
  binary_mul(const T &x, const mp_integer &n,
             typename std::enable_if<std::is_floating_point<T>::value>::type * =
                 nullptr) {
  ...
  mp_integer &in_place_div(const mp_integer &other) {
    ...
    if (s1 && s2) {
      mp_integer r;
      m_int.g_st().div(m_int.g_st(), r.m_int.g_st(), m_int.g_st(),
                       other.m_int.g_st());
    } else if (s1 && !s2) {
      // Promote this.
      m_int.promote();
  ...
  template <typename T>
  static T
  binary_div(const mp_integer &n, const T &x,
             typename std::enable_if<std::is_floating_point<T>::value>::type * =
                 nullptr) {
    return (static_cast<T>(n) / x);
  }
  template <typename T>
  static T
  binary_div(const T &x, const mp_integer &n,
             typename std::enable_if<std::is_floating_point<T>::value>::type * =
                 nullptr) {
  ...
  mp_integer &in_place_mod(const mp_integer &other) {
    ...
    if (s1 && s2) {
      mp_integer q;
      m_int.g_st().div(q.m_int.g_st(), m_int.g_st(), m_int.g_st(),
                       other.m_int.g_st());
    } else if (s1 && !s2) {
      // Promote this.
      m_int.promote();
  ...
  mp_integer &in_place_lshift(const T &other) {
    try {
      return in_place_lshift_mp_bitcnt_t(
          boost::numeric_cast<::mp_bitcnt_t>(other));
    } catch (const boost::numeric::bad_numeric_cast &) {
      piranha_throw(std::invalid_argument,
                    "invalid argument for left bit shifting");
    }
  }
  ...
  mp_integer &in_place_rshift(const T &other) {
    try {
      return in_place_rshift_mp_bitcnt_t(
          boost::numeric_cast<::mp_bitcnt_t>(other));
    } catch (const boost::numeric::bad_numeric_cast &) {
      piranha_throw(std::invalid_argument,
                    "invalid argument for right bit shifting");
    }
  }
  ...
  template <typename Integer>
  static bool binary_equality(
      const mp_integer &n1, const Integer &n2,
      typename std::enable_if<std::is_integral<Integer>::value>::type * =
          nullptr) {
  ...
  template <typename Integer>
  static bool binary_equality(
      const Integer &n1, const mp_integer &n2,
      typename std::enable_if<std::is_integral<Integer>::value>::type * =
          nullptr) {
  ...
  static bool binary_equality(
      const mp_integer &n, const FloatingPoint &x,
      typename std::enable_if<
  ...
  template <typename FloatingPoint>
  static bool binary_equality(
      const FloatingPoint &x, const mp_integer &n,
      typename std::enable_if<
          std::is_floating_point<FloatingPoint>::value>::type * = nullptr) {
  ...
  template <typename Integer>
  static bool
  binary_leq(const mp_integer &n1, const Integer &n2,
             typename std::enable_if<std::is_integral<Integer>::value>::type * =
                 nullptr) {
    return n1 <= mp_integer(n2);
  }
  template <typename Integer>
  static bool
  binary_leq(const Integer &n1, const mp_integer &n2,
             typename std::enable_if<std::is_integral<Integer>::value>::type * =
                 nullptr) {
    return mp_integer(n1) <= n2;
  }
  template <typename FloatingPoint>
  static bool binary_leq(
      const mp_integer &n, const FloatingPoint &x,
      typename std::enable_if<
          std::is_floating_point<FloatingPoint>::value>::type * = nullptr) {
    return static_cast<FloatingPoint>(n) <= x;
  ...
  pow_impl(const T &ui,
    ...
    try {
      ...
      piranha_throw(std::invalid_argument,
                    "invalid argument for exponentiation");
    }
    return ebs(exp);
  }
  template <typename T>
  ...
  // mpz view class.
  class mpz_view {
    using static_mpz_view = typename detail::integer_union<
        NBits>::s_storage::template static_mpz_view<>;

  public:
    explicit mpz_view(const mp_integer &n)
        : m_static_view(n.is_static() ? n.m_int.g_st().get_mpz_view()
                                      : static_mpz_view{}),
    ...
    operator const detail::mpz_struct_t *() const { return get(); }
...
public:
   ...
   *
   * @throws std::invalid_argument if the construction fails (e.g., construction
   * from a non-finite
  ...
  /**
   * The string must be a sequence of decimal digits, preceded by a minus sign
   * for
   * strictly negative numbers. The first digit of a non-zero number must not be
   * zero. A malformed string will throw an \p std::invalid_argument
   * exception.
   *
   * Note that if the string is not null-terminated, undefined behaviour will
   * occur.
   *
   * @param[in] str decimal string representation of the number used to
   * initialise the integer object.
   *
   * @throws std::invalid_argument if the string is malformed.
   */
  explicit mp_integer(const char *str) { construct_from_string(str); }
  /// Constructor from C++ string.
  /**
   ...
   *
   * This assignment operator is equivalent to constructing a temporary instance
   * of mp_integer from \p x
   ...
   *
   * @throws unspecified any exception thrown by the generic constructor of
   * mp_integer.
   */
  template <typename T, generic_ctor_enabler<T> = 0>
  ...
  /**
   * Equivalent to the construction and susbequent move to \p this of a
   * temporary mp_integer from \p str.
   *
   * @param[in] str C++ string that will be assigned to \p this.
   *
   * @return reference to \p this.
   *
   * @throws unspecified any exception thrown by the constructor of mp_integer
   * from string.
   */
  ...
  /**
   * This method will return an object of an unspecified type \p mpz_view which
   * is implicitly convertible
   * to a const pointer to an \p mpz struct (and which can thus be used as a
   * <tt>const mpz_t</tt>
   * parameter in GMP functions). In addition to the implicit conversion
   * operator, the \p mpz struct pointer
   * can also be retrieved via the <tt>get()</tt> method of the \p mpz_view
   * class.
   * The pointee will represent a GMP integer whose value is equal to \p this.
   *
   * Note that the returned \p mpz_view instance can only be move-constructed
   * (the other constructors and the assignment operators
   * are disabled). Additionally, the returned object and the pointer might
   * reference internal data belonging to
   * \p this, and they can thus be used safely only during the lifetime of \p
   * this.
   * Any modification to \p this will also invalidate the view and the pointer.
   *
   * @return an \p mpz view of \p this.
   */
  mpz_view get_mpz_view() const { return mpz_view(*this); }
  /// Conversion operator.
  /**
   * \note
   * This operator is enabled only if \p T is an interoperable type.
   *
   * Conversion to integral types, if possible, will always be exact. Conversion
   * to \p bool produces
   * \p true for nonzero values, \p false for zero. Conversion to floating-point
   * types is performed
   * via arithmetic operations and might generate infinities in case the value
   * is too large.
   *
   * @return the value of \p this converted to type \p T.
   *
   * @throws std::overflow_error if the conversion fails (e.g., the range of the
   * target integral type
   ...
   */
  template <typename T, cast_enabler<T> = 0> explicit operator T() const {
    return convert_to_impl<T>();
  }
  /// Overload output stream operator for piranha::mp_integer.
  /**
   * The input \p n will be directed to the output stream \p os as a string of
   * digits in base 10.
   *
   ...
   *
   * @throws std::invalid_argument if the number of digits is larger than an
  ...
  /**
   * Equivalent to extracting a line from the stream and then assigning it to \p
   * n.
   *
   * @param[in] is input stream.
   * @param[in,out] n integer to which the contents of the stream will be
   * assigned.
   *
   * @return reference to \p is.
   *
   * @throws unspecified any exception thrown by the constructor from string of
   * piranha::mp_integer.
   */
  ...
  /**
   * This method will promote \p this to dynamic storage, if \p this is
   * currently stored in static
   * storage. Otherwise, an error will be raised.
   *
   * @throws std::invalid_argument if \p this is not currently stored in static
   * storage.
   */
  void promote() {
  ...
  /// Test storage status.
  /**
   * @return \p true if \p this is currently stored in static storage, \p false
   * otherwise.
   */
  bool is_static() const { return m_int.is_static(); }
  /// Negate in-place.
  void negate() {
    if (is_static()) {
      m_int.g_st().negate();
  ...
  /// Sign.
  /**
   * @return 1 if <tt>this > 0</tt>, 0 if <tt>this == 0</tt> and -1 if <tt>this
   * < 0</tt>.
   */
  int sign() const {
    if (is_static()) {
  ...
  /**
   * \note
   * This operator is enabled only if \p T is an interoperable type or
   * piranha::mp_integer.
   *
   * Add \p x in-place. If \p T is piranha::mp_integer or an integral type, the
   * result will be exact. If \p T is a floating-point type, the following
   * sequence of operations takes place:
   *
   * - \p this is converted to an instance \p f of type \p T via the conversion
   * operator,
   * - \p f is added to \p x,
   ...
   * @return reference to \p this.
   *
   * @throws unspecified any exception thrown by the generic constructor, the
   * conversion operator or the generic assignment operator, if used.
   */
  template <typename T>
   ...
   *
   * Add a piranha::mp_integer in-place. This method will first compute <tt>n +
   * x</tt>, cast it back to \p T via \p static_cast and finally assign the
   * result to \p x.
   *
   ...
   *
   * @throws unspecified any exception thrown by the binary operator or by
   * casting piranha::mp_integer to \p T.
   */
   ...
   *
   * If no floating-point types are involved, the exact result of the operation
   * will be returned as a piranha::mp_integer.
   *
   * If one of the arguments is a floating-point value \p f of type \p F, the
   * other argument will be converted to an instance of type \p F
   * and added to \p f to generate the return value, which will then be of type
   * \p F.
   *
   ...
   */
  template <typename T, typename U>
  friend auto operator+(const T &x, const U &y)
      -> decltype(mp_integer::binary_plus(x, y)) {
   ...
   */
  mp_integer operator+() const { return *this; }
  /// Prefix increment.
  /**
   ...
   */
  mp_integer &operator++() { return operator+=(1); }
  /// Suffix increment.
  /**
   * Increment \p this by one and return a copy of \p this as it was before the
   * increment.
   *
  ...
  /**
   * Sets \p this to <tt>n1 + n2</tt>. This form can be more efficient than the
  ...
  mp_integer &add(const mp_integer &n1, const mp_integer &n2) {
    ...
    if (s0 && s1 && s2) {
      if (likely(!detail::integer_union<NBits>::s_storage::add(
              m_int.g_st(), n1.m_int.g_st(), n2.m_int.g_st()))) {
        return *this;
      }
    }
    // this will have to be mpz in any case, promote it if needed and re-check
    // the
    ...
    // 2**2 possibilities.
    // NOTE: here the 0 flag means that the operand is static and needs to be
    // promoted,
    // 1 means that it is dynamic already.
    const unsigned mask =
        static_cast<unsigned>(!s1) + (static_cast<unsigned>(!s2) << 1u);
    switch (mask) {
    case 0u: {
      auto v1 = n1.m_int.g_st().get_mpz_view(),
           v2 = n2.m_int.g_st().get_mpz_view();
   ...
   * piranha::mp_integer.
   *
   * Subtract \p x in-place. If \p T is piranha::mp_integer or an integral type,
   * the result will be exact. If \p T is a floating-point type, the following
   * sequence of operations takes place:
   *
   * - \p this is converted to an instance \p f of type \p T via the conversion
   * operator,
   ...
   * This operator is enabled only if \p T is a non-const interoperable type.
   *
   * Subtract a piranha::mp_integer in-place. This method will first compute
   * <tt>x - n</tt>, cast it back to \p T via \p static_cast and finally assign
   * the result to \p x.
   *
   * @param[in,out] x first argument.
   * @param[in] n second argument.
   ...
   * If one of the arguments is a floating-point value \p f of type \p F, the
   * other argument will be converted to an instance of type \p F
   * and subtracted from (or to) \p f to generate the return value, which will
   * then be of type \p F.
   *
   * @param[in] x first argument
   * @param[in] y second argument.
  ...
  template <typename T, typename U>
  friend auto operator-(const T &x, const U &y)
      -> decltype(mp_integer::binary_subtract(x, y)) {
  ...
  /// Suffix decrement.
  /**
   * Decrement \p this by one and return a copy of \p this as it was before the
   * decrement.
   *
  ...
  mp_integer &sub(const mp_integer &n1, const mp_integer &n2) {
    bool s0 = is_static(), s1 = n1.is_static(), s2 = n2.is_static();
    if (s0 && s1 && s2) {
      if (likely(!detail::integer_union<NBits>::s_storage::sub(
              m_int.g_st(), n1.m_int.g_st(), n2.m_int.g_st()))) {
        return *this;
      }
    }
   ...
   * piranha::mp_integer.
   *
   * Multiply by \p x in-place. If \p T is piranha::mp_integer or an integral
   * type, the result will be exact. If \p T is a floating-point type, the
   * following
   * sequence of operations takes place:
   *
   * - \p this is converted to an instance \p f of type \p T via the conversion
   ...
   * This operator is enabled only if \p T is a non-const interoperable type.
   *
   * Multiply by a piranha::mp_integer in-place. This method will first compute
   * <tt>x * n</tt>, cast it back to \p T via \p static_cast and finally assign
   * the result to \p x.
   *
   * @param[in,out] x first argument.
   ...
   * If one of the arguments is a floating-point value \p f of type \p F, the
   * other argument will be converted to an instance of type \p F
   * and multiplied by \p f to generate the return value, which will then be of
   * type \p F.
   *
   * @param[in] x first argument
   * @param[in] y second argument.
  ...
  template <typename T, typename U>
  friend auto operator*(const T &x, const U &y)
      -> decltype(mp_integer::binary_mul(x, y)) {
  ...
  mp_integer &multiply_accumulate(const mp_integer &n1, const mp_integer &n2) {
    bool s0 = is_static(), s1 = n1.is_static(), s2 = n2.is_static();
    if (s0 && s1 && s2) {
      if (likely(!m_int.g_st().multiply_accumulate(n1.m_int.g_st(),
                                                   n2.m_int.g_st()))) {
        return *this;
      }
    }
  ...
  mp_integer &mul(const mp_integer &n1, const mp_integer &n2) {
    bool s0 = is_static(), s1 = n1.is_static(), s2 = n2.is_static();
    if (s0 && s1 && s2) {
      if (likely(!detail::integer_union<NBits>::s_storage::mul(
              m_int.g_st(), n1.m_int.g_st(), n2.m_int.g_st()))) {
        return *this;
      }
    }
   ...
   * piranha::mp_integer.
   *
   * Divide by \p x in-place. If \p T is piranha::mp_integer or an integral
   * type, the result will be truncated
   ...
   * @throws unspecified any exception thrown by the generic constructor, the
   * conversion operator or the generic assignment operator, if used.
   * @throws piranha::zero_division_error if \p T is an integral type and \p x
   * is zero (as established by
   ...
   * This operator is enabled only if \p T is a non-const interoperable type.
   *
   * Divide by a piranha::mp_integer in-place. This method will first compute
   * <tt>x / n</tt>, cast it back to \p T via \p static_cast and finally assign
   * the result to \p x.
   *
   * @param[in,out] x first argument.
   ...
   * If one of the arguments is a floating-point value \p f of type \p F, the
   * other argument will be converted to an instance of type \p F
   * and divided by (or used as a dividend for) \p f to generate the return
   * value, which will then be of type \p F.
   *
   * @param[in] x first argument
   * @param[in] y second argument.
   *
   ...
   * - the corresponding in-place operator,
   * - the invoked constructor or the conversion operator, if used.
   * @throws piranha::zero_division_error if both operands are of integral type
   * and a division by zero occurs.
   */
  template <typename T, typename U>
  friend auto operator/(const T &x, const U &y)
      -> decltype(mp_integer::binary_div(x, y)) {
  ...
  mp_integer &div(const mp_integer &n1, const mp_integer &n2) {
    ...
    if (s1 && s2) {
      ...
      mp_integer r;
      // NOTE: the static division routine here does both division and
      // remainder, we
      // need to check if there's performance to be gained by doing only the
      // division.
      m_int.g_st().div(m_int.g_st(), r.m_int.g_st(), n1.m_int.g_st(),
                       n2.m_int.g_st());
      return *this;
    }
    // this will have to be mpz in any case, promote it if needed and re-check
    ...
    switch (mask) {
    // NOTE: case 0 here is not possible as it would mean that n1 and n2 are
    // both static,
  ...
  /**
   * This method will set \p q to the quotient and \p r to the remainder of
   * <tt>n1 / n2</tt>.
   ...
   */
  static void divrem(mp_integer &q, mp_integer &r, const mp_integer &n1,
                     const mp_integer &n2) {
    if (unlikely(math::is_zero(n2))) {
      piranha_throw(zero_division_error, "division by zero");
    }
    ...
    // If n1 and n2 are both statics, we can carry the computation completely in
    // static storage.
    if (s1 && s2) {
      if (!s0) {
        q = mp_integer{};
      ...
      q.m_int.g_st().div(q.m_int.g_st(), r.m_int.g_st(), n1.m_int.g_st(),
                         n2.m_int.g_st());
      return;
    }
    // If either n1 or n2 are static, we need to promote the return values and
    // re-check n1/n2.
    if (s0) {
      q.promote();
      r.promote();
    ...
    case 3u:
      ::mpz_tdiv_qr(&q.m_int.g_dy(), &r.m_int.g_dy(), &n1.m_int.g_dy(),
                    &n2.m_int.g_dy());
    }
  }
  ...
  /**
   * \note
   * This template operator is enabled only if \p T is piranha::mp_integer or an
   * integral type among the interoperable types.
   *
   * Sets \p this to <tt>this % n</tt>. This operator behaves in the way
   * specified by the C++ standard (specifically,
   ...
   * @return reference to \p this.
   *
   * @throws unspecified any exception thrown by the generic constructor, if
   * used.
  ...
  /**
   * \note
   * This operator is enabled only if \p T is a non-const integral interoperable
   * type.
   *
   * Compute the remainder with respect to a piranha::mp_integer in-place. This
   * method will first compute <tt>x % n</tt>,
   * cast it back to \p T via \p static_cast and finally assign the result to \p
   * x.
   *
   * @param[in,out] x first argument.
   * @param[in] n second argument.
  ...
  template <typename T, typename U>
  friend auto operator%(const T &x, const U &y)
      -> decltype(mp_integer::binary_mod(x, y)) {
    if (unlikely(math::is_zero(y))) {
      piranha_throw(zero_division_error, "division by zero in mp_integer");
    }
   ...
   * integral type.
   *
   * Sets \p this to <tt>this << n</tt>. The left shift operation is equivalent
   * to a multiplication
   ...
   * @throws unspecified any exception thrown by the generic constructor, if
   * used.
   * @throws piranha::invalid_argument if <tt>n</tt> is negative or it does not
   * fit in an <tt>mp_bitcnt_t</tt>.
   */
  template <typename T>
   ...
   * type.
   *
   * Compute the left shift with respect to a piranha::mp_integer in-place. This
   * method will first compute <tt>x << n</tt>,
   * cast it back to \p T via \p static_cast and finally assign the result to \p
   * x. The left shift operation is equivalent to a multiplication
   * by 2 to the power of \p n.
   *
   * @param[in,out] x first argument.
   * @param[in] n second argument.
  ...
  template <typename T, typename U>
  friend auto operator<<(const T &x, const U &y)
      -> decltype(mp_integer::binary_lshift(x, y)) {
   ...
   * integral type.
   *
   * Sets \p this to <tt>this >> n</tt>. The right shift operation is equivalent
   * to a truncated division
   * by 2 to the power of \p n.
   *
   ...
   * type.
   *
   * Compute the right shift with respect to a piranha::mp_integer in-place.
   * This method will first compute <tt>x >> n</tt>,
   * cast it back to \p T via \p static_cast and finally assign the result to \p
   * x. The right shift operation is equivalent to a
  ...
  template <typename T, typename U>
  friend auto operator>>(const T &x, const U &y)
      -> decltype(mp_integer::binary_rshift(x, y)) {
   ...
   * - both \p T and \p U are piranha::mp_integer.
   *
   * If no floating-point types are involved, the exact result of the comparison
   * will be returned.
   *
   * If one of the arguments is a floating-point value \p f of type \p F, the
   * other argument will be converted to an instance of type \p F
   ...
   *
   * @throws unspecified any exception thrown by the invoked constructor or the
   * conversion operator, if used.
   */
  template <typename T, typename U>
  friend auto operator==(const T &x, const U &y)
      -> decltype(mp_integer::binary_equality(x, y)) {
  ...
  template <typename T, typename U>
  friend auto operator<(const T &x, const U &y)
      -> decltype(mp_integer::binary_less_than(x, y)) {
  ...
  template <typename T, typename U>
  friend auto operator<=(const T &x, const U &y)
      -> decltype(mp_integer::binary_leq(x, y)) {
  ...
  template <typename T, typename U>
  friend auto operator>(const T &x, const U &y)
      -> decltype(mp_integer::binary_less_than(y, x)) {
  ...
  template <typename T, typename U>
  friend auto operator>=(const T &x, const U &y)
      -> decltype(mp_integer::binary_leq(y, x)) {
    return binary_leq(y, x);
  }
  /// Exponentiation.
  /**
   * \note
   * This template method is activated only if \p T is an integral type or
   * piranha::mp_integer.
   *
   * Return <tt>this ** exp</tt>.  Negative
   * powers are calculated as <tt>(1 / this) ** exp</tt>. Trying to raise zero
   * to a negative exponent will throw a
   * piranha::zero_division_error exception. <tt>this ** 0</tt> will always
   * return 1.
   *
   * The value of \p exp cannot exceed in absolute value the maximum value
   * representable by the <tt>unsigned long</tt> type, otherwise an
   ...
   *
   * @throws std::invalid_argument if <tt>exp</tt>'s value is outside the range
   * of the <tt>unsigned long</tt> type.
   * @throws piranha::zero_division_error if \p this is zero and \p exp is
   * negative.
   * @throws unspecified any exception thrown by the generic constructor, if
   * used.
   */
   ...
   * The returned value is guaranteed to be non-negative if both arguments are
   * non-negative.
   *
...
private:
  struct hash_checks {
    ...
    // Check that the computation of the total number of bits does not overflow
    // when the number
    // of size_t to extract is no more than the corresponding quantity for the
    // static int.
    // This protects again both the computation of tot_nbits, but also the
    // multiplication inside
    // read_uint as return type coincides with std::size_t.
    static const bool value =
        s_storage::hash_checks::max_n_size_t <
        std::numeric_limits<std::size_t>::max() / nbits_size_t;
  };

public:
  /// Hash value
  /**
   * The returned hash value does not depend on how the integer is stored (i.e.,
   * dynamic or static
   * storage).
   *
   ...
   */
  std::size_t hash() const {
    // NOTE: in order to check that this method can be called safely we need to
    // make sure that:
    // - the static hash calculation has no overflows (checked above),
    // - dynamic hash calculation cannot overflow when operating on integers
    // with size (in terms
    ...
    // NOTE: mpz_size gives the number of limbs effectively used by mpz, that
    // is,
    ...
    // Determine the number of std::size_t to extract.
    const std::size_t nbits_size_t = static_cast<std::size_t>(
                          std::numeric_limits<std::size_t>::digits),
                      // NOTE: in case of overflow here, we will underestimate
        // the number of bits
        // used in the representation of dy.
        // NOTE: use GMP_NUMB_BITS in case we are operating with a GMP lib built
        // with nails support.
        ...
        tot_nbits = static_cast<std::size_t>(unsigned(GMP_NUMB_BITS) * size),
                      ...
                      n_size_t = static_cast<std::size_t>(
                          q + static_cast<std::size_t>(r != 0u));
    for (std::size_t i = 0u; i < n_size_t; ++i) {
      boost::hash_combine(
          retval, detail::read_uint<std::size_t,
                                    unsigned(GMP_LIMB_BITS - GMP_NUMB_BITS)>(
                      l_ptr, size, i));
    }
    return retval;
  ...
  /**
   * Compute the next prime number after \p this. The method uses the GMP
   * function <tt>mpz_nextprime()</tt>.
   *
   * @return next prime.
   *
  ...
  mp_integer nextprime() const {
    if (unlikely(sign() < 0)) {
      piranha_throw(std::invalid_argument,
                    "cannot compute the next prime of a negative number");
    }
    mp_integer retval;
    retval.promote();
    auto v = get_mpz_view();
   ...
   *
   * @return 2 if \p this is definitely a prime, 1 if \p this is probably prime,
   * 0 if \p this is definitely composite.
   *
   * @throws std::invalid_argument if \p reps is less than one or if \p this is
   * negative.
   */
  int probab_prime_p(int reps = 25) const {
    ...
    if (unlikely(sign() < 0)) {
      piranha_throw(std::invalid_argument,
                    "cannot run primality tests on a negative number");
    }
    auto v = get_mpz_view();
  ...
  mp_integer sqrt() const {
    if (unlikely(sign() < 0)) {
      piranha_throw(std::invalid_argument,
                    "cannot calculate square root of negative integer");
    }
    // NOTE: here in principle if this is static, we never need to go through
    // dynamic
   ...
   *
   * @throws std::invalid_argument if \p this is negative or larger than an
...
private:
  template <typename T>
  static unsigned long check_choose_k(
      const T &k,
      typename std::enable_if<std::is_integral<T>::value>::type * = nullptr) {
    try {
      ...
      piranha_throw(std::invalid_argument,
                    "invalid k argument for binomial coefficient");
    }
  }
  template <typename T>
  static unsigned long check_choose_k(
      const T &k,
      typename std::enable_if<std::is_same<T, mp_integer>::value>::type * =
          nullptr) {
    try {
...
public:
  ...
  /**
   * \note
   * This method is enabled only if \p T is an integral type or
   * piranha::mp_integer.
   *
   ...
   *
   * @throws std::invalid_argument if \p k is outside an implementation-defined
   * range.
   */
  template <typename T,
            typename std::enable_if<std::is_integral<T>::value ||
                                        std::is_same<mp_integer, T>::value,
                                    int>::type = 0>
  mp_integer binomial(const T &k) const {
    if (k >= T(0)) {
      ...
      // NOTE: demote opportunity.
      ::mpz_bin_ui(&retval.m_int.g_dy(), &retval.m_int.g_dy(),
                   check_choose_k(k));
      return retval;
    } else {
      ...
      if (sign() >= 0) {
        ...
        // n < 0, k < 0.
        if (k <= *this) {
          return mp_integer{-1}.pow(*this - k) *
                 (-mp_integer{k} - 1).binomial(*this - k);
        } else {
          return mp_integer{0};
        }
      }
  ...
  /// Size in bits.
  /**
   * This method will return the number of bits necessary to represent the
   * absolute value of \p this.
  ...
  std::size_t bits_size() const {
    if (is_static()) {
      constexpr auto limb_bits =
          detail::integer_union<NBits>::s_storage::limb_bits;
      static_assert(std::numeric_limits<std::size_t>::max() >= limb_bits * 2u,
                    "Overflow error.");
      return static_cast<std::size_t>(m_int.g_st().bits_size());
    }
    // NOTE: in theory this could overflow. Not sure if it we should put any
    // check here,
  ...
  explicit mp_integer(const ::mpz_t z) {
    ...
    if (unsigned(GMP_NUMB_BITS) > (limb_bits * 2u) / size) {
      ...
      const auto total_bits =
          detail::integer_union<NBits>::s_storage::total_bits;
      // The total number of bits we will need to extract from z. We know we can
      // compute this
      // because we know z fits static, and we can always represent the total
      // number of bits
      // in static.
      const limb_t tot_nbits =
                       static_cast<limb_t>(unsigned(GMP_NUMB_BITS) * size),
                   q = static_cast<limb_t>(tot_nbits / limb_bits),
                   r = static_cast<limb_t>(tot_nbits % limb_bits),
                   n_limbs =
                       static_cast<limb_t>(q + static_cast<limb_t>(r != 0u));
      piranha_assert(n_limbs <= 2u && n_limbs > 0u);
      // NOTE: if here the number of static limbs used will be only 1, the
      // second
      // limb has already been zeroed out by the intial construction.
      for (std::size_t i = 0u; i < n_limbs; ++i) {
        m_int.g_st().m_limbs[i] =
            detail::read_uint<limb_t, unsigned(GMP_LIMB_BITS - GMP_NUMB_BITS),
  ...
  /// Exact division.
  /**
   * This static method will set \p out to the quotient of \p n1 and \p n2. \p
   * n2 must divide
   ...
   * @throws piranha::zero_division_error if \p n2 is zero.
   */
  static void _divexact(mp_integer &out, const mp_integer &n1,
                        const mp_integer &n2) {
    if (unlikely(math::is_zero(n2))) {
      piranha_throw(zero_division_error, "division by zero");
    }
    ...
    if (s1 && s2) {
      ...
      // need to check if there's performance to be gained by doing only the
      // division.
      out.m_int.g_st().div(out.m_int.g_st(), r.m_int.g_st(), n1.m_int.g_st(),
                           n2.m_int.g_st());
      return;
    }
    // out will have to be mpz in any case, promote it if needed and re-check
    // the
  ...
  /**
   * This method will return a pointer to the dynamic storage, mpz_struct_t. If
   * \p this is currently stored in static
   * storage, \p this will be promoted to dynamic storage first
   *
   * Note that \p this will remain stored in dynamic storage even if after
   * operations on \p this, value of \p this can
...
// size in mp_integer.
template <typename T> struct is_mp_integer : std::false_type {};
...
/// Specialisation of the implementation of piranha::math::multiply_accumulate()
/// for piranha::mp_integer.
/**
 * This specialisation is enabled when \p T is an instance of
 * piranha::mp_integer.
 */
template <typename T>
struct multiply_accumulate_impl<
    T, T, T, typename std::enable_if<detail::is_mp_integer<T>::value>::type> {
  /// Call operator.
  /**
...
/// Specialisation of the piranha::math::negate() functor for
/// piranha::mp_integer.
/**
 * This specialisation is enabled when \p T is an instance of
 * piranha::mp_integer.
...
template <typename T>
struct negate_impl<
    T, typename std::enable_if<detail::is_mp_integer<T>::value>::type> {
  /// Call operator.
  /**
   ...
   */
  void operator()(T &n) const { n.negate(); }
};

/// Specialisation of the piranha::math::is_zero() functor for
/// piranha::mp_integer.
/**
 * This specialisation is enabled when \p T is an instance of
 * piranha::mp_integer.
...
struct is_zero_impl<
   ...
   */
  bool operator()(const T &n) const { return n.sign() == 0; }
};

/// Specialisation of the piranha::math::is_unitary() functor for
/// piranha::mp_integer.
/**
 * This specialisation is enabled when \p T is an instance of
 * piranha::mp_integer.
...
struct is_unitary_impl<
   ...
   */
  bool operator()(const T &n) const { return n.is_unitary(); }
};
...
struct abs_impl<
   ...
   */
  T operator()(const T &n) const { return n.abs(); }
};
...
/// Specialisation of the piranha::math::partial() functor for
/// piranha::mp_integer.
/**
 * This specialisation is enabled when \p T is an instance of
 * piranha::mp_integer.
...
struct partial_impl<
   ...
   */
  T operator()(const T &, const std::string &) const { return T{}; }
};
 ...
 */
template <typename T, typename U, typename = void> struct ipow_subs_impl {};

/// Substitution of integral power.
/**
 * Substitute the integral power of a symbolic variable with a generic object.
 * The actual implementation of this function is in the
 * piranha::math::ipow_subs_impl functor.
 *
 ...
 */
template <typename T, typename U>
inline auto ipow_subs(const T &x, const std::string &name, const integer &n,
                      const U &y)
...
/**
 * This specialisation is activated when \p T is an instance of
 * piranha::mp_integer.
 */
template <typename T>
struct add3_impl<
   ...
   */
  auto operator()(T &out, const T &a, const T &b) const
      -> decltype(out.add(a, b)) {
    return out.add(a, b);
  }
};
...
struct divexact_impl<
  ...
  /// Call operator.
  /**
   * The call operator will first determine quotient and remainder via
   * piranha::mp_integer::divrem().
   ...
   *
   * @throws piranha::math::inexact_division if the division of \p a by \p b is
   * not exact.
 ...
 *
 * The result will be calculated via piranha::mp_integer::gcd(), after any
...
struct gcd_impl<T, U, detail::mp_integer_gcd_enabler<T, U>> {
   ...
   */
  template <int NBits>
  mp_integer<NBits> operator()(const mp_integer<NBits> &a,
                               const mp_integer<NBits> &b) const {
...
/**
 * This specialisation is enabled when \p T is an instances of
 * piranha::mp_integer.
 */
template <typename T>
struct gcd3_impl<
   ...
   */
  template <int NBits>
  mp_integer<NBits> &operator()(mp_integer<NBits> &out,
                                const mp_integer<NBits> &a,
                                const mp_integer<NBits> &b) const {
 ...
 *
 * This function is a convenience wrapper that will call the other
 * piranha::math::ipow_subs() overload, with \p n
 ...
 * @return \p x after substitution  of \p name to the power of \p n with \p y.
 *
 * @throws unspecified any exception thrown by the other overload of
 * piranha::math::ipow_subs() or by the construction
 * of piranha::integer from an integral value.
 */
template <typename T, typename U, typename Int,
          detail::ipow_subs_int_enabler<Int> = 0>
inline auto ipow_subs(const T &x, const std::string &name, const Int &n,
                      const U &y)
...
/**
 * The type trait will be \p true if piranha::math::ipow_subs can be
 * successfully called on instances
 * of type \p T, with an instance of type \p U as substitution argument.
 */
template <typename T, typename U> class has_ipow_subs : detail::sfinae_types {
  typedef typename std::decay<T>::type Td;
  typedef typename std::decay<U>::type Ud;
  template <typename T1, typename U1>
  static auto test(const T1 &t, const U1 &u)
      -> decltype(math::ipow_subs(t, std::declval<std::string const &>(),
                                  std::declval<integer const &>(), u),
                  void(), yes());
  static no test(...);
...
public:
  ...
  static const bool value =
      std::is_same<decltype(test(std::declval<Td>(), std::declval<Ud>())),
                   yes>::value;
};

template <typename T, typename U> const bool has_ipow_subs<T, U>::value;

/// Type trait to detect the presence of the integral power substitution method
/// in keys.
/**
 * This type trait will be \p true if the decay type of \p Key provides a const
 * method <tt>ipow_subs()</tt> accepting as const parameters a string,
 * an instance of piranha::integer, an instance of \p T and an instance of
 * piranha::symbol_set. The return value of the method must be an
 * <tt>std::vector</tt>
 * of pairs in which the second type must be \p Key itself. The
 * <tt>ipow_subs()</tt> method represents the substitution of the integral power
 * of a symbol with
...
class key_has_ipow_subs : detail::sfinae_types {
  ...
  static auto test(const Key1 &k, const T1 &t)
      -> decltype(k.ipow_subs(std::declval<const std::string &>(),
                              std::declval<const integer &>(), t,
                              std::declval<const symbol_set &>()));
  static no test(...);
  template <typename T1> struct check_result_type {
...
public:
  /// Value of the type trait.
  static const bool value = check_result_type<decltype(
      test(std::declval<Keyd>(), std::declval<Td>()))>::value;
};

// Static init.
template <typename Key, typename T> const bool key_has_ipow_subs<Key, T>::value;

/// Specialisation of piranha::has_exact_ring_operations for
/// piranha::mp_integer.
/**
...
struct has_exact_ring_operations<
    T, typename std::enable_if<
           detail::is_mp_integer<typename std::decay<T>::type>::value>::type> {
  /// Value of the type trait.
  static const bool value = true;
};

template <typename T>
const bool has_exact_ring_operations<
    T, typename std::enable_if<detail::is_mp_integer<
 ...
 */
inline integer operator"" _z(const char *s) { return integer(s); }
}
}
...
template <int NBits> struct hash<piranha::mp_integer<NBits>> {
  /// Result type.
   ...
   */
  result_type operator()(const argument_type &n) const { return n.hash(); }
};
}

...
struct key_is_multipliable_impl<
  ...
  static auto test(const Cf1 &, const Key1 &) -> decltype(
      Key1::multiply(
          std::declval<std::array<term<Cf1, Key1>, Key1::multiply_arity> &>(),
          std::declval<const term<Cf1, Key1> &>(),
          std::declval<const term<Cf1, Key1> &>(),
          std::declval<const symbol_set &>()),
      void(), yes());
  static no test(...);
  static const bool value =
      std::is_same<yes, decltype(test(std::declval<Cf>(),
 ...
 * - it has a static member of type \p std::size_t called \p multiply_arity with
 * a value greater than zero,
 * - it has a static function called \p multiply() accepting the following
 * arguments:
 *   - a reference to an \p std::array of piranha::term of \p Cf and \p Key of
 * size \p multiply_arity,
 *   - two const references to piranha::term of \p Cf and \p Key,
 *   - a const reference to piranha::symbol_set.
 *
 * The decay types of \p Cf and \p Key are considered by this type trait. \p Cf
 * and \p Key must satisfy
 * piranha::is_cf and piranha::is_key, otherwise a compile-time error will be
 * generated.
 */
template <typename Cf, typename Key> class key_is_multipliable {

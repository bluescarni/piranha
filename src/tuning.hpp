...
template <typename = int> struct base_tuning {
...
/// Performance tuning.
/**
 * This class provides static methods to manipulate global variables useful for
 * performance tuning.
...
public:
  ...
  /**
   * Piranha can use multiple threads when initialising large memory areas
   * (e.g., during the initialisation
   * of the result of the multiplication of two large polynomials). This can
   * improve performance on systems
   * with fast or multiple memory buses, but it could lead to degraded
   * performance on slower/single-socket machines.
   *
   * The default value of this flag is \p true (i.e., Piranha will use multiple
   * threads while initialising
   * large memory areas).
   *
   * @return current value of the \p parallel_memory_set flag.
   */
  static bool get_parallel_memory_set() { return s_parallel_memory_set.load(); }
  /// Set the \p parallel_memory_set flag.
  /**
   * @see piranha::tuning::get_parallel_memory_set() for an explanation of the
  ...
  /**
   * This method will reset the \p parallel_memory_set flag to its default
   * value.
   *
   * @see piranha::tuning::get_parallel_memory_set() for an explanation of the
   * meaning of this flag.
   */
  static void reset_parallel_memory_set() { s_parallel_memory_set.store(true); }
  /// Get the multiplication block size.
  /**
   * The multiplication algorithms for certain series types (e.g., polynomials)
   * divide the input operands in
   * blocks before processing them. This flag regulates the maximum size of
   * these blocks.
   *
   * Larger block have less overhead, but can degrade the performance of memory
   * access. Smaller blocks can promote
  ...
  /**
   * @see piranha::tuning::get_multiplication_block_size() for an explanation of
   * the meaning of this value.
   *
   * @param[in] size desired value for the block size.
   *
   * @throws std::invalid_argument if \p size is outside an
  ...
  /**
   * In series multiplication it can be advantageous to employ a heuristic to
   * estimate the final size
   * of the result before actually performing the multiplication. The cost of
   * estimation is proportionally
   * larger for small operands, and it can result in noticeable overhead for
   * small multiplications.
   * This flag establishes a threshold below which the estimation of the size of
   * the product of a series
   * multiplication will not be performed.
   *
   * The precise way in which this value is used depends on the multiplication
   * algorithm.
  ...
  /**
   * @see piranha::tuning::s_estimate_threshold() for an explanation of the
  ...
  /**
   * This method will reset the series estimation threshold to its default
   * value.
   *
   * @see piranha::tuning::get_estimate_threshold() for an explanation of the
   * meaning of this value.
   */
  static void reset_estimate_threshold() { s_estimate_threshold.store(200u); }
};
}

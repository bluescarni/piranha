...
/**
 * This allocator can be used to allocate memory aligned to a specific boundary,
 * specified at runtime.
 * The alignment value is provided upon construction and it is stored as a
 * member of the allocator object.
 * The allocator is standard-compliant and can hence be used in standard
 * containers.
 */
template <typename T> class dynamic_aligning_allocator {
public:
  ...
  /**
   * This allocator has to be move assigned when the container using it is move
   * assigned.
   */
   ...
   */
  explicit dynamic_aligning_allocator(const std::size_t &alignment)
   ...
   */
  template <typename U>
  explicit dynamic_aligning_allocator(
      const dynamic_aligning_allocator<U> &other)
   ...
   */
  template <typename U>
  explicit dynamic_aligning_allocator(dynamic_aligning_allocator<U> &&other)
      : m_alignment(other.alignment()) {}
  /// Defaulted destructor.
  ~dynamic_aligning_allocator() = default;
  /// Defaulted copy assignment operator.
  dynamic_aligning_allocator &
  operator=(const dynamic_aligning_allocator &) = default;
  /// Defaulted move assignment operator.
  dynamic_aligning_allocator &
  operator=(dynamic_aligning_allocator &&) = default;
  /// Maximum allocatable size.
  /**
   * @return the maximum number of objects of type \p value_type that can be
   * allocated by a single call to
   * allocate().
   */
   ...
   *
   * @param[in] size number of instances of type \p value_type for which the
  ...
  value_type *allocate(const size_type &size) const {
    ...
    return static_cast<value_type *>(aligned_palloc(
        m_alignment, static_cast<size_type>(size * sizeof(value_type))));
  }
   ...
   *
   * @return \p true if the alignments of \p this and \p other coincide, \p
   * false otherwise.
   */

...
/**
 * This type trait will be \p true if \p T can be used as a coefficient type, \p
 * false otherwise.
 * The requisites for \p T are the following:
 *
 * - it must satisfy piranha::is_container_element,
 * - it must satisfy piranha::has_print_coefficient and
 * piranha::has_print_tex_coefficient,
 ...
 */
template <typename T> class is_cf {
  static const bool implementation_defined =
      is_container_element<T>::value && has_print_coefficient<T>::value &&
      has_print_tex_coefficient<T>::value && has_is_zero<T>::value &&
      has_negate<T>::value && is_equality_comparable<T>::value &&
      is_addable<T>::value && is_addable_in_place<T>::value &&
      is_subtractable_in_place<T>::value && is_subtractable<T>::value &&
...
template <typename T> const bool is_cf<T>::value;
}

 ...
 * The values of the exponents are packed in a signed integer using Kronecker
 * substitution, using the facilities provided
 * by piranha::kronecker_array.
 *
 * This class satisfies the piranha::is_key, piranha::key_has_degree,
 * piranha::key_has_ldegree and piranha::key_is_differentiable type traits.
 *
 * ## Type requirements ##
 *
 ...
 */
// TODO:
// - consider abstracting the km_commons in a class and use it both here and in
// rtkm.
...
public:
  ...
  // NOTE: this essentially defines a maximum number of small ints that can be
  // packed in m_value,
  // as we always need to pass through pack/unpack. In practice, it does not
  // matter: in current
  // architectures the bit width limit will result in kronecker array's limits
  // to be smaller than
  // 255 items.
...
private:
  ...
  using e_type = decltype(
      math::pow(std::declval<U const &>(), std::declval<value_type const &>()));
  template <typename U>
  struct eval_type_<
      U, typename std::enable_if<is_multipliable_in_place<e_type<U>>::value &&
  ...
  using pow_enabler = typename std::enable_if<
      has_safe_cast<T, decltype(std::declval<integer &&>() *
  ...
  // Subs utilities.
  template <typename U>
  using subs_type__ = decltype(
      math::pow(std::declval<const U &>(), std::declval<const value_type &>()));
  template <typename U, typename = void> struct subs_type_ {};
  template <typename U>
  struct subs_type_<
      U,
      typename std::enable_if<
          std::is_constructible<subs_type__<U>, int>::value &&
  ...
  // ipow subs utilities.
  template <typename U>
  using ipow_subs_type__ = decltype(
      math::pow(std::declval<const U &>(), std::declval<const integer &>()));
  template <typename U, typename = void> struct ipow_subs_type_ {};
  template <typename U>
  struct ipow_subs_type_<
      U, typename std::enable_if<
             std::is_constructible<ipow_subs_type__<U>, int>::value &&
  ...
  template <typename U>
  using container_ctor_enabler = typename std::enable_if<
      has_begin_end<const U>::value &&
          has_safe_cast<T, typename std::iterator_traits<decltype(std::begin(
                               std::declval<const U &>()))>::value_type>::value,
      int>::type;
  template <typename U>
  using init_list_ctor_enabler =
      container_ctor_enabler<std::initializer_list<U>>;
  template <typename Iterator>
  using it_ctor_enabler = typename std::enable_if<
      is_input_iterator<Iterator>::value &&
          has_safe_cast<value_type, typename std::iterator_traits<
                                        Iterator>::value_type>::value,
      int>::type;
  // Implementation of the ctor from range.
  template <typename Iterator>
  typename v_type::size_type construct_from_range(Iterator begin,
                                                  Iterator end) {
    v_type tmp;
    for (; begin != end; ++begin) {
...
public:
  ...
  /**
   * \note
   * This constructor is enabled only if \p U satisfies piranha::has_begin_end,
   * and the value type
   * of the iterator type of \p U can be safely cast to \p T.
   *
   * This constructor will build internally a vector of values from the input
   * container \p c, encode it and assign the result
   * to the internal integer instance. The value type of the container is
   * converted to \p T using
   * piranha::safe_cast().
   *
   * @param[in] c the input container.
   *
   * @throws std::overflow_error if the container has a size greater than an
  ...
  /**
   * \note
   * This constructor is enabled only if the corresponding constructor from
   * container is enabled.
   *
   * This constructor is identical to the constructor from container. It is
  ...
  /**
   * \note
   * This constructor is enabled only if \p Iterator is an input iterator whose
   * value type
   * is safely convertible to \p T.
   *
   * This constructor will build internally a vector of values from the input
   * iterators, encode it and assign the result
   ...
   *
   * @throws std::overflow_error if the distance between \p begin and \p end is
   * greater than an implementation-defined value.
   * @throws unspecified any exception thrown by:
   * - piranha::kronecker_array::encode(),
   * - piranha::safe_cast(),
   * - piranha::static_vector::push_back(),
  ...
  /**
   * \note
   * This constructor is enabled only if the corresponding range constructor is
   * enabled.
   *
   * This constructor is identical to the constructor from range. In addition,
   * after construction
   * it will also check that the distance between \p begin and \p end is equal
   * to the size of \p s.
   ...
   *
   * @throws std::invalid_argument if the distance between \p begin and \p end
   * is different from
   * the size of \p s.
   * @throws unspecified any exception thrown by the constructor from range.
   */
  template <typename Iterator, it_ctor_enabler<Iterator> = 0>
  explicit kronecker_monomial(Iterator begin, Iterator end, const symbol_set &s)
      : m_value(0) {
  ...
  explicit kronecker_monomial(const symbol_set &args) {
    // NOTE: this does incur in some overhead, but on the other hand it runs all
    // sorts of
    // checks on the size of args, the size of tmp, the encoding limits, etc.
    // Probably it is
    // better to leave it like this at the moment, unless it becomes a serious
    // bottleneck.
    v_type tmp;
   ...
   * set the internal integer instance to the same value of \p other, after
   * having checked that
   ...
   */
  explicit kronecker_monomial(const kronecker_monomial &other,
  ...
  bool is_compatible(const symbol_set &args) const noexcept {
    // NOTE: the idea here is to avoid unpack()ing for performance reasons:
    // these checks
    // are already part of unpack(), and that's why unpack() is used instead of
    // is_compatible()
    // in other methods.
   ...
   * @return \p false.
   */
  bool is_ignorable(const symbol_set &) const noexcept { return false; }
  /// Merge arguments.
  /**
   ...
   * - unpack().
   */
  kronecker_monomial merge_args(const symbol_set &orig_args,
                                const symbol_set &new_args) const {
    return kronecker_monomial(
  ...
  degree_type ldegree(const symbol_set &args) const { return degree(args); }
  /// Partial degree.
  /**
   * Partial degree of the monomial: only the symbols at the positions specified
   * by \p p are considered.
   * The type returned by this method is the type resulting from the addition of
   * two instances
   * of \p T.
   *
   * @param[in] p positions of the symbols to be considered in the calculation
   * of the degree.
   * @param[in] args reference set of piranha::symbol.
   *
   * @return the summation of the exponents of the monomial at the positions
   * specified by \p p.
   *
   ...
   * @throws unspecified any exception thrown by unpack().
   */
  degree_type degree(const symbol_set::positions &p,
  ...
  degree_type ldegree(const symbol_set::positions &p,
                      const symbol_set &args) const {
  ...
  /**
   * \note
   * This method is enabled only if \p Cf satisfies piranha::is_cf and
   * piranha::has_mul3.
   *
   * Multiply \p t1 by \p t2, storing the result in the only element of \p res.
   * This method
   * offers the basic exception safety guarantee. If \p Cf is an instance of
   * piranha::mp_rational, then
   * only the numerators of the coefficients will be multiplied.
   *
   * Note that the key of the return value is generated directly from the
   * addition of the values of the input keys.
   * No check is performed for overflow of either the limits of the integral
   * type or the limits of the Kronecker codification.
   *
  ...
  static void
  multiply(std::array<term<Cf, kronecker_monomial>, multiply_arity> &res,
           const term<Cf, kronecker_monomial> &t1,
   ...
   */
  static void multiply(kronecker_monomial &res, const kronecker_monomial &a,
                       const kronecker_monomial &b, const symbol_set &) {
   ...
   * @param[in] b second argument.
   */
  static void divide(kronecker_monomial &res, const kronecker_monomial &a,
                     const kronecker_monomial &b, const symbol_set &) {
   ...
   *
   * @return \p true if the internal integral instance of \p this is equal to
   * the integral instance of \p other,
   * \p false otherwise.
   */
  ...
  /**
   * If the monomial is linear in a variable (i.e., all exponents are zero apart
   * from a single unitary
   * exponent), the name of the variable will be returned. Otherwise, an error
   * will be raised.
   *
   * @param[in] args reference set of piranha::symbol.
   *
  ...
  /**
   * \note
   * This method is enabled only if \p U is multipliable by piranha::integer and
   * the result type can be
   * safely cast back to \p T.
   *
   * Will return a monomial corresponding to \p this raised to the <tt>x</tt>-th
   * power. The exponentiation
   * is computed via the multiplication of the exponents promoted to
   * piranha::integer by \p x. The result will
  ...
  void print_tex(std::ostream &os, const symbol_set &args) const {
    ...
    for (decltype(tmp.size()) i = 0u; i < tmp.size(); ++i) {
      cur_value = tmp[i];
      if (cur_value != zero) {
        // NOTE: here negate() is safe because of the symmetry in
        // kronecker_array.
        cur_oss = (cur_value > zero)
                      ? std::addressof(oss_num)
                      : (math::negate(cur_value), std::addressof(oss_den));
   ...
   * The result is a pair consisting of the exponent associated to \p p before
   * differentiation and the monomial itself
   * after differentiation. If \p p is empty or if the exponent associated to it
   * is zero,
   ...
   *
   * @throws std::invalid_argument if the computation of the derivative causes a
   * negative overflow,
   ...
   * - piranha::kronecker_array::encode().
   */
  std::pair<T, kronecker_monomial> partial(const symbol_set::positions &p,
                                           const symbol_set &args) const {
    auto v = unpack(args);
    ...
    if (unlikely(n == std::numeric_limits<T>::min())) {
      piranha_throw(std::invalid_argument,
                    "negative overflow error in the calculation of the "
   ...
   * result is a pair
   * consisting of the exponent associated to \p s increased by one and the
   * monomial itself
   * after integration. If \p s is not in \p args, the returned monomial will
   * have an extra exponent
   ...
   *
   * @throws std::invalid_argument if the exponent associated to \p s is -1 or
   * if the value of an exponent overflows.
   * @throws unspecified any exception thrown by:
   * - unpack(),
   * - piranha::math::is_zero(),
   * - piranha::static_vector::push_back(),
   * - piranha::kronecker_array::encode().
   */
  std::pair<T, kronecker_monomial> integrate(const symbol &s,
                                             const symbol_set &args) const {
    v_type v = unpack(args), retval;
    value_type expo(0), one(1);
    for (min_int<typename v_type::size_type, decltype(args.size())> i = 0u;
         i < args.size(); ++i) {
      ...
      if (args[i] == s) {
        // NOTE: here using i is safe: if retval gained an extra exponent in the
        // condition above,
        // we are never going to land here as args[i] is at this point never
        // going to be s.
        if (unlikely(retval[i] == std::numeric_limits<value_type>::max())) {
          piranha_throw(std::invalid_argument, "positive overflow error in the "
                                               "calculation of the integral of "
                                               "a monomial");
        }
        retval[i] = static_cast<value_type>(retval[i] + value_type(1));
        if (math::is_zero(retval[i])) {
          piranha_throw(std::invalid_argument, "unable to perform monomial "
                                               "integration: negative unitary "
                                               "exponent");
        }
        expo = retval[i];
      }
   ...
   * - it can be used in piranha::math::pow() with the monomial exponents as
   * powers, yielding a type \p eval_type,
   * - \p eval_type is constructible from \p int,
   * - \p eval_type is multipliable in place.
   *
   * The return value will be built by iteratively applying piranha::math::pow()
   * using the values provided
   * by \p pmap as bases and the values in the monomial as exponents. If the
   * size of the monomial is zero, 1 will be
   ...
   * - piranha::math::pow() or the in-place multiplication operator of the
   * return type.
   */
  template <typename U>
  eval_type<U> evaluate(const symbol_set::positions_map<U> &pmap,
    ...
    auto it = pmap.begin();
    for (min_int<size_type, decltype(args.size())> i = 0u; i < args.size();
         ++i, ++it) {
   ...
   *
   * The algorithm is equivalent to the one implemented in
   * piranha::monomial::subs().
   *
   ...
   */
  template <typename U>
  std::vector<std::pair<subs_type<U>, kronecker_monomial>>
  subs(const std::string &s, const U &x, const symbol_set &args) const {
    ...
    retval.push_back(std::make_pair(std::move(retval_s),
                                    kronecker_monomial(ka::encode(new_v))));
    return retval;
  }
   ...
   * \note
   * This method is enabled only if:
   * - \p U can be raised to a piranha::integer power, yielding a type \p
   * subs_type,
   ...
   * @param[in] x quantity that will be substituted in place of \p s to the
   * power of \p n.
   * @param[in] args reference set of piranha::symbol.
   *
   ...
   */
  template <typename U>
  std::vector<std::pair<ipow_subs_type<U>, kronecker_monomial>>
  ipow_subs(const std::string &s, const integer &n, const U &x,
            const symbol_set &args) const {
  ...
  /// Trim.
  /**
   * This method will return a copy of \p this with the exponents associated to
   * the symbols
   ...
   * - piranha::static_vector::push_back().
   */
  kronecker_monomial trim(const symbol_set &trim_args,
                          const symbol_set &orig_args) const {
    return kronecker_monomial(
        detail::km_trim<v_type, ka>(trim_args, orig_args, m_value));
  }
  /// Comparison operator.
  /**
   * @param[in] other comparison argument.
   *
   * @return \p true if the internal integral value of \p this is less than the
   * internal
  ...
  /**
   * This method will write into \p out the content of \p this. If necessary, \p
   * out will
   ...
   * - unpack().
   */
  void extract_exponents(std::vector<value_type> &out,
                         const symbol_set &args) const {
  ...
  /// Split.
  /**
   * This method will split \p this into two monomials: the second monomial will
   * contain the exponent
   * of the first variable in \p args, the first monomial will contain all the
   * other exponents.
   *
   * @param[in] args reference arguments set.
   *
   * @return a pair of monomials, the second one containing the first exponent,
   * the first one containing all the
   * other exponents.
   *
   * @throws std::invalid_argument if the size of \p args is less than 2.
   * @throws unspecified any exception thrown by unpack() or by the constructor
   * from a range.
   */
  std::pair<kronecker_monomial, kronecker_monomial>
  split(const symbol_set &args) const {
    if (unlikely(args.size() < 2u)) {
      piranha_throw(std::invalid_argument,
                    "only monomials with 2 or more variables can be split");
    }
    auto tmp = unpack(args);
    return std::make_pair(kronecker_monomial(tmp.begin() + 1, tmp.end()),
                          kronecker_monomial(tmp[0u]));
  }
  /// Detect negative exponents.
  /**
   * This method will return \p true if at least one exponent is less than zero,
   * \p false otherwise.
   *
   * @param[in] args reference arguments set.
   *
   * @return \p true if at least one exponent is less than zero, \p false
   * otherwise.
   *
   * @throws unspecified any exception thrown by unpack().
   */
  bool has_negative_exponent(const symbol_set &args) const {
    ...
    return std::any_of(tmp.begin(), tmp.end(),
                       [](const value_type &e) { return e < value_type(0); });
  }
...
template <typename T> struct hash<piranha::kronecker_monomial<T>> {
  /// Result type.
   ...
   * piranha::kronecker_monomial::hash().
   */

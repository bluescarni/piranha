...
template <typename T> struct is_divisor_series_key {
  static const bool value = false;
};

template <typename T> struct is_divisor_series_key<divisor<T>> {
  static const bool value = true;
};

// See the workaround description below.
template <typename T> struct base_getter { using type = typename T::base; };
}

/// Divisor series.
/**
 * This class represents series in which the keys are divisor (see
 * piranha::divisor) and the coefficient type
 * is generic. This class satisfies the piranha::is_series and piranha::is_cf
 * type traits.
 *
 * ## Type requirements ##
 *
 * \p Cf must be suitable for use in piranha::series as first template argument,
 * \p Key must be an instance
 * of piranha::divisor.
 *
...
class divisor_series
    : public power_series<
          ipow_substitutable_series<
              substitutable_series<series<Cf, Key, divisor_series<Cf, Key>>,
                                   divisor_series<Cf, Key>>,
              divisor_series<Cf, Key>>,
          divisor_series<Cf, Key>>,
      detail::divisor_series_tag {
  // NOTE: this is a workaround for GCC < 5. The enabler condition for the
  // special invert() method is a struct defined
  // within this class, and it needs to access the "base" private typedef via
  // the inverse_type<> alias. The enabler condition
  // is not granted access to the private base typedef (erroneously, since it is
  // defined in the scope of this class),
  // so we use this friend helper in order to reach the base typedef.
  template <typename> friend struct detail::base_getter;
  ...
  static void expo_increase(T &e) {
    if (unlikely(e == std::numeric_limits<T>::max())) {
      piranha_throw(std::overflow_error,
                    "overflow in the computation of the partial derivative "
                    "of a divisor series");
    }
  ...
  using d_partial_type_0 =
      decltype(std::declval<const dv_type &>() *
               std::declval<const dv_type &>() * std::declval<const T &>());
  template <typename T>
  using d_partial_type_1 = decltype(
      std::declval<const T &>() * std::declval<const d_partial_type_0<T> &>());
  // Requirements on the final type for the first stage.
  template <typename T>
  using d_partial_type = typename std::enable_if<
      std::is_constructible<d_partial_type_1<T>, d_partial_type_0<T>>::value &&
  ...
  d_partial_type<T> d_partial_impl(typename T::term_type::key_type &key,
                                   const symbol_set::positions &pos) const {
    ...
    // Extract from the first dependent term the aij and the exponent, and
    // multiply+negate them.
    const auto mult =
        safe_mult(first.e, first.v[static_cast<vs_type>(pos.back())]);
    ...
    if (!key.m_container.empty()) {
      // Build a series with only the first dependent term and unitary
      // coefficient.
      key_type tmp_div_01;
  ...
  template <typename T = divisor_series>
  d_partial_type<T> divisor_partial(const typename T::term_type &term,
                                    const symbol_set::positions &pos) const {
    using term_type = typename base::term_type;
    ...
    // Init the constant part of the derivative: the coefficient and the part of
    // the divisor
  ...
  using partial_type = typename std::enable_if<
      std::is_constructible<partial_type_<T>, int>::value &&
  ...
  using integrate_type = typename std::enable_if<
      std::is_constructible<integrate_type_<T>, int>::value &&
          is_addable_in_place<integrate_type_<T>>::value,
      integrate_type_<T>>::type;
  // Invert utils.
  // Type coming out of invert() for the base type. This will also be the final
  // type.
  template <typename T>
  using inverse_type = decltype(math::invert(
      std::declval<const typename detail::base_getter<T>::type &>()));
  // Enabler for the special inversion algorithm. We need a polynomial in the
  // coefficient hierarchy
  // and the inversion type must support the operations needed in the algorithm.
  template <typename T, typename = void> struct has_special_invert {
    static constexpr bool value = false;
  };
  template <typename T>
  struct has_special_invert<
      T, typename std::enable_if<
             detail::poly_in_cf<T>::value &&
             std::is_constructible<
                 inverse_type<T>,
                 decltype(std::declval<const inverse_type<T> &>() /
                          std::declval<const integer &>())>::value>::type> {
    static constexpr bool value = true;
  };
  // Case 0: Series is not suitable for special invert() implementation. Just
  // forward to the base one, via casting.
  template <
      typename T = divisor_series,
      typename std::enable_if<!has_special_invert<T>::value, int>::type = 0>
  inverse_type<T> invert_impl() const {
    return math::invert(*static_cast<const base *>(this));
  }
  // Case 1: Series is suitable for special invert() implementation. This can
  // fail at runtime depending on what is
  ...
  // Special invert() implementation when we have reached the first polynomial
  ...
  RetT special_invert(const T &s) const {
    if (s.is_single_coefficient() && !s.empty()) {
      try {
        ...
        // NOTE: lc cannot be empty as we are coming in with a non-zero
        // polynomial.
        ...
        for (auto &n : v_int) {
          ...
          // NOTE: the gcd computation here is safe as we are operating on
          // integers.
          math::gcd3(cd, cd, n);
        }
        ...
        // It should never be zero: if all elements in v_int are zero, we would
        // not have been
...
public:
  /// Series rebind alias.
  template <typename Cf2> using rebind = divisor_series<Cf2, Key>;
  ...
  /**
   * \note
   * This template method is enabled only if math::invert() can be called on the
   * class
   * from which piranha::divisor_series derives (i.e., only if the default
   * math::invert()
   * implementation for series is appropriate).
   *
   * This method works exactly like the default implementation of
   * piranha::math::invert() for
   * series types, unless:
   *
   ...
   * - the return type is divisible by piranha::integer, yielding a type which
   * can be used
   *   to construct the return type.
   *
   * Under these circumstances, this method will attempt to construct a divisor
   * from the first polynomial
   * coefficient encountered in the hierarchy, and, if successful, it will
   * return a piranha::divisor_series
   * consisting of a single term with unitary
   * coefficient and with the key containing the divisor built from the
   * polynomial.
   *
   * For example, inverting the following divisor series with polynomial
   * coefficient
   * \f[
   * 2x+4y
   * \f]
   ...
   * \f]
   *
   * In order for the procedure described above to be successful, the polynomial
   * coefficient must be equivalent
   * to an integral linear combination of symbols with no constant term. If this
   * special inversion fails,
   * a call to the default implementation of piranha::math::invert() will be
   * attempted.
   *
   ...
   */
  template <typename T = divisor_series> inverse_type<T> invert() const {
    return invert_impl();
  }
  /// Partial derivative.
  /**
   ...
   * involved types.
   *
   * This method will compute the partial derivative of \p this with respect to
   * the symbol called \p name.
   * The derivative is computed via differentiation of the coefficients and the
   * application of the product
   * rule.
   *
   * @param[in] name name of the variable with respect to which the
   * differentiation will be computed.
   *
   * @return the partial derivative of \p this with respect to \p name.
   *
   * @throws std::overflow_error if the manipulation of integral exponents and
   * multipliers in the divisors
  ...
  partial_type<T> partial(const std::string &name) const {
    ...
    const symbol_set::positions pos(this->m_symbol_set,
  ...
  /**
   * \note
   * This template method is enabled only if the coefficient type is integrable,
   * and the result of the multiplication
   * of an integrated coefficient by a piranha::divisor_series is addable
   * in-place and constructible from \p int.
   *
   * Integration for divisor series is supported only if the coefficient is
   * integrable and no divisor in the calling
   * series depends on the integration variable.
   *
   * @param[in] name name of the variable with respect to which the integration
   * will be performed.
   *
   * @return the antiderivative of \p this with respect to \p name.
   *
   * @throws std::invalid_argument if at least one divisor depends on the
...
using divisor_series_multiplier_enabler = typename std::enable_if<
    std::is_base_of<divisor_series_tag, Series>::value>::type;
}

/// Specialisation of piranha::series_multiplier for piranha::divisor_series.
template <typename Series>
class series_multiplier<Series,
                        detail::divisor_series_multiplier_enabler<Series>>

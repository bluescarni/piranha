...
template <typename T>
inline bool generic_binomial_check_k(
    const T &, const T &,
...
template <typename T>
inline bool generic_binomial_check_k(
    const T &k, const T &zero,
    typename std::enable_if<!std::is_unsigned<T>::value>::type * = nullptr) {
  return k < zero;
}

// Generic binomial implementation using the falling factorial. U must be an
// integer
// type, T can be anything that supports basic arithmetics. k must be
// non-negative.
template <typename T, typename U>
...
// Implementation of the generalised binomial coefficient for floating-point
// types.
template <typename T> inline T fp_binomial(const T &x, const T &y) {
  static_assert(std::is_floating_point<T>::value,
                "Invalid type for fp_binomial.");
  if (unlikely(!std::isfinite(x) || !std::isfinite(y))) {
    piranha_throw(std::invalid_argument, "cannot compute binomial coefficient "
                                         "with non-finite floating-point "
                                         "argument(s)");
  }
  const bool neg_int_x =
                 std::trunc(x + T(1)) == (x + T(1)) && (x + T(1)) <= T(0),
             neg_int_y =
                 std::trunc(y + T(1)) == (y + T(1)) && (y + T(1)) <= T(0),
             neg_int_x_y = std::trunc(x - y + T(1)) == (x - y + T(1)) &&
                           (x - y + T(1)) <= T(0);
  ...
  case 5u: {
    const auto phase =
        std::pow(T(-1), x - y + T(1)) / std::pow(T(-1), x + T(1));
...
// Enabler for the specialisation of binomial for floating-point and arithmetic
// arguments.
template <typename T, typename U>
 ...
 */
template <typename T, typename U, typename = void> struct binomial_impl {};

/// Specialisation of the piranha::math::binomial() functor for floating-point
/// and arithmetic arguments.
/**
...
struct binomial_impl<T, U, detail::binomial_fp_arith_enabler<T, U>> {
  ...
  /// Call operator.
  /**
   * The implementation, accepting any real finite value for \p x and \p y, is
   * described in
   * http://arxiv.org/abs/1105.3689/. Note that, since the implementation uses
   * floating-point
   * arithmetics, the result will - in general - be inexact, even if both \p x
   * and \p y represent
   * integral values.
   *
  ...
  result_type operator()(const T &x, const U &y) const {
    return detail::fp_binomial(static_cast<result_type>(x),
...
using math_binomial_type =
    typename std::enable_if<is_returnable<math_binomial_type_<T, U>>::value,
...
/**
 * \note
 * This function is enabled only if <tt>binomial_impl<T,U>{}(x,y)</tt> is a
 * valid expression,
 ...
 * piranha::math::binomial_impl functor. The body of this
 ...
 * - if the arguments are both piranha::mp_integer, or a piranha::mp_integer and
 * an integral type, then piranha::mp_integer::binomial() is used
 *   to compute the result (after any necessary conversion),
 * - if both arguments are integral types, piranha::mp_integer::binomial() is
 * used after the conversion of the top argument
 *   to piranha::mp_integer,
 * - otherwise, the piranha::mp_integer argument is converted to the
 * floating-point type and \p piranha::math::binomial() is
...
struct binomial_impl<T, U, detail::integer_binomial_enabler<T, U>> {
   ...
   * piranha::mp_integer::binomial().
   */
  template <int NBits>
  mp_integer<NBits> operator()(const mp_integer<NBits> &x,
                               const mp_integer<NBits> &y) const {
    return x.binomial(y);
  }
...
/**
 * The type trait will be \p true if piranha::math::binomial can be successfully
 * called on instances of \p T and \p U respectively,
 * \p false otherwise.
 */
template <typename T, typename U> class has_binomial : detail::sfinae_types {
  template <typename T1, typename U1>
  static auto test(T1 const *t, U1 const *u)
      -> decltype(math::binomial(*t, *u), void(), yes());
  static no test(...);
  static const bool implementation_defined =
      std::is_same<decltype(test((T const *)nullptr, (U const *)nullptr)),
                   yes>::value;
...
// Static init.
template <typename T, typename U> const bool has_binomial<T, U>::value;
}

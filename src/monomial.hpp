...
/// Monomial class.
/**
 * This class extends piranha::array_key to define a series key type
 * representing monomials, that is, objects of the form:
 * \f[
 * x_0^{y_0} \cdot x_1^{y_1} \cdot \ldots \cdot x_n^{y_n}.
 * \f]
 * The type \p T represents the type of the exponents \f$ y_i \f$, which are
 ...
 * ## Type requirements ##
 *
 * \p T and \p S must be suitable for use as first and third template arguments
 * in piranha::array_key. Additionally,
 * \p T must satisfy the piranha::has_is_zero type trait, and it must be
 * copy-assignable.
 *
 * ## Exception safety guarantee ##
 *
 * Unless noted otherwise, this class provides the same exception safety
 * guarantee as piranha::array_key.
 *
 * ## Move semantics ##
 *
...
class monomial : public array_key<T, monomial<T, S>, S> {
  ...
  template <typename U>
  using e_type =
      decltype(math::pow(std::declval<U const &>(), std::declval<T const &>()));
  template <typename U>
  struct eval_type_<
  ...
  template <typename U>
  using init_list_enabler = typename std::enable_if<
      std::is_constructible<base, std::initializer_list<U>>::value, int>::type;
  // Multiplication and division.
  template <typename Cf, typename U>
  using multiply_enabler = typename std::enable_if<
      detail::true_tt<decltype(std::declval<U const &>().vector_add(
  ...
  using monomial_divide_enabler = typename std::enable_if<
      detail::true_tt<decltype(std::declval<U const &>().vector_sub(
          std::declval<U &>(), std::declval<U const &>()))>::value,
      int>::type;
  // Enabler for linear argument.
  template <typename U>
  using linarg_enabler =
      typename std::enable_if<has_safe_cast<integer, U>::value, int>::type;
  ...
  using pow_type = typename std::conditional<std::is_integral<T>::value,
                                             integer &&, const T &>::type;
  template <typename U>
  using pow_enabler = typename std::enable_if<
      has_safe_cast<T, decltype(std::declval<pow_type>() *
                                std::declval<const U &>())>::value,
      int>::type;
  // Machinery to determine the degree type.
  // NOTE: add_type<U> will be promoted to a int/unsigned in case U is a short
  // int.
  template <typename U>
  using add_type =
  ...
  template <typename U>
  using degree_type =
      typename std::enable_if<std::is_constructible<add_type<U>, int>::value &&
  ...
  static void ip_inc(U &x) {
    if (unlikely(x == std::numeric_limits<U>::max())) {
      piranha_throw(std::invalid_argument,
                    "positive overflow error in the calculation of the "
  ...
  template <typename U>
  using integrate_enabler = typename std::enable_if<
      std::is_assignable<U &, decltype(std::declval<U &>() +
                                       std::declval<U>())>::value,
      int>::type;
  // Partial utils.
  ...
  template <typename U>
  using partial_enabler = typename std::enable_if<
      std::is_assignable<U &, decltype(std::declval<U &>() -
                                       std::declval<U>())>::value,
      int>::type;
  ...
  // Subs support.
  // NOTE: this can obviously be compressed and implemented more cleanly with a
  // single enable_if, but unfortunately
  // when this pattern is used both here and in k_monomial, a weird compiler
  // error results with both GCC 4.8 and 4.9.
  // We need to check with GCC 5 what happens, and in case the problem is still
  // there, it needs to be reported.
  template <typename U>
  using subs_type__ =
      decltype(math::pow(std::declval<const U &>(), std::declval<const T &>()));
  ...
  // Less-than operator.
  template <typename U>
  using comparison_enabler =
      typename std::enable_if<is_less_than_comparable<U>::value, int>::type;
...
public:
  ...
  /**
   * \note
   * This constructor is enabled only if the corresponding constructor in
   * piranha::array_key is enabled.
   *
   ...
   *
   * This constructor will copy the elements from the range defined by \p begin
   * and \p end into \p this,
   * using piranha::safe_cast() for any necessary type conversion. If the final
   * size of \p this is different
   * from the size of \p s, a runtime error will be produced. This constructor
   * is used by piranha::polynomial::find_cf().
   *
   * @param[in] begin beginning of the range.
   * @param[in] end end of the range.
   * @param[in] s reference symbol set.
   *
   * @throws std::invalid_argument if the final size of \p this and the size of
   * \p s differ.
  ...
  /// Trivial destructor.
  ~monomial() { PIRANHA_TT_CHECK(is_key, monomial); }
  ...
  /**
   * A monomial is unitary if, for all its elements, piranha::math::is_zero()
   * returns \p true.
   *
  ...
  /**
   * \note
   * This method is enabled only if \p T is addable and the type resulting from
   * the addition is constructible from \p int
   * and monomial::value_type can be added in-place to it.
   *
   * This method will return the degree of the monomial, computed via the
   * summation of the exponents of the monomial.
   * If \p T is a C++ integral type, the addition of the exponents will be
   ...
   *
   * @throws std::invalid_argument if the sizes of \p args and \p this differ.
   * @throws std::overflow_error if the exponent type is a C++ integral type and
   * the computation
   * of the degree overflows.
   * @throws unspecified any exception thrown by the invoked constructor or
   ...
   * and monomial::value_type can be added in-place to it.
   *
   * This method will return the partial degree of the monomial, computed via
   * the summation of the exponents of the monomial.
   * If \p T is a C++ integral type, the addition of the exponents will be
   * checked for overflow.
   *
   * The \p p argument is used to indicate which exponents are to be taken into
   * account when computing the partial degree.
   * Exponents not in \p p will be discarded during the computation of the
   * partial degree.
   *
   ...
   */
  template <typename U = T>
  degree_type<U> degree(const symbol_set::positions &p,
                        const symbol_set &args) const {
    if (unlikely(args.size() != this->size() ||
                 (p.size() && p.back() >= this->size()))) {
      piranha_throw(std::invalid_argument,
                    "invalid arguments set or positions");
    }
    auto cit = this->begin();
    degree_type<U> retval(0);
    for (const auto &i : p) {
  ...
  template <typename U = T>
  degree_type<U> ldegree(const symbol_set::positions &p,
                         const symbol_set &args) const {
  ...
  /**
   * \note
   * This method is enabled only if the exponent type supports
   * piranha::safe_cast() to piranha::integer.
   *
   ...
   *
   * @throws std::invalid_argument if the monomial is not linear or if the sizes
   * of \p args and \p this differ.
   */
  ...
  /**
   * \note
   * This method is enabled if the exponent type (or its promoted
   * piranha::integer counterpart)
   * is multipliable by \p U and the result type can be cast safely back to the
   * exponent type.
   *
   * Will return a monomial corresponding to \p this raised to the <tt>x</tt>-th
   * power. The exponentiation
   * is computed via the multiplication of the exponents by \p x. If the
   * exponent type is a C++
   * integral type, each exponent will be promoted to piranha::integer before
   * the exponentiation
   * takes place.
   *
   * @param[in] x exponent.
  ...
  /**
   * \note
   * This method is enabled only if the exponent type is subtractable and the
   ...
   *
   * If the exponent type is an integral type, then the decrement-by-one
   * operation on the affected exponent is checked
   ...
   *
   * @throws std::invalid_argument if the sizes of \p args and \p this differ,
   * if the size of \p p is
   * greater than one, if the position specified by \p p is invalid or if the
   * computation on integral exponents
  ...
  std::pair<U, monomial> partial(const symbol_set::positions &p,
                                 const symbol_set &args) const {
    if (!is_compatible(args)) {
      piranha_throw(std::invalid_argument, "invalid size of arguments set");
    }
    ...
    // NOTE: safe to take this->begin() here, as the checks on the positions
    // above ensure
  ...
  /**
   * \note
   * This method is enabled only if the exponent type is addable and the result
   * of the operation
   * can be assigned back to the exponent type.
   *
   ...
   *
   * If the exponent type is an integral type, then the increment-by-one
   * operation on the affected exponent is checked
   * for negative overflow.
   *
   ...
   *
   * @throws std::invalid_argument if the sizes of \p args and \p this differ,
   * if the exponent associated to \p s is -1, or if the computation on integral
   * exponents
   * results in an overflow error.
   * @throws unspecified any exception thrown by:
   * - piranha::math::is_zero(),
  ...
  std::pair<U, monomial> integrate(const symbol &s,
                                   const symbol_set &args) const {
    typedef typename base::size_type size_type;
    if (!is_compatible(args)) {
      piranha_throw(std::invalid_argument, "invalid size of arguments set");
  ...
  void print(std::ostream &os, const symbol_set &args) const {
    ...
    for (typename base::size_type i = 0u; i < this->size(); ++i) {
      if ((*this)[i] != zero) {
        // If we are going to print a symbol, and something has been printed
        // before,
   ...
   * returned. If \p args is not compatible with \p this and \p pmap, or the
   * positions in \p pmap do not reference
   ...
   * pmap.
   *
   * @throws std::invalid_argument if there exist an incompatibility between \p
   * this,
   * \p args or \p pmap.
   * @throws unspecified any exception thrown by:
  ...
  eval_type<U> evaluate(const symbol_set::positions_map<U> &pmap,
    ...
    // NOTE: the positions map must have the same number of elements as this,
    // and it must
    // be made of consecutive positions [0,size - 1].
    if (unlikely(pmap.size() != this->size() ||
   ...
   * return value is vector containing one pair in which the first
   * element is the result of substituting \p s with \p x (i.e., \p x raised to
   * the power of the exponent corresponding
   * to \p s), and the second element the monomial after the substitution has
   * been performed (i.e., with the exponent
   * corresponding to \p s set to zero). If \p s is not in \p args, the return
   * value will be <tt>(1,this)</tt> (i.e., the
   ...
   */
  template <typename U>
  std::vector<std::pair<subs_type<U>, monomial>>
  subs(const std::string &s, const U &x, const symbol_set &args) const {
    ...
    retval.push_back(
        std::make_pair(std::move(retval_s), std::move(retval_key)));
    return retval;
  }
   ...
   * - the value type of the monomial can be cast safely to piranha::rational
   * and it supports
   *   in-place subtraction with piranha::integer.
   *
   * Substitute the symbol called \p s to the power of \p n with quantity \p x.
   * The return value is a vector containing a single pair in which the first
   * element is the result of the substitution, and the second element the
   * monomial after the substitution has been performed.
   * If \p s is not in \p args, the return value will be <tt>(1,this)</tt>
   * (i.e., the
   * monomial is unchanged and the substitution yields 1).
   *
   * The method will substitute also \p s to powers higher than \p n in absolute
   * value.
   * For instance, substitution of <tt>y**2</tt> with \p a in <tt>y**7</tt> will
   * produce <tt>a**3 * y</tt>, and
   * substitution of <tt>y**-2</tt> with \p a in <tt>y**-7</tt> will produce
   * <tt>a**3 * y**-1</tt>.
   *
   * @param[in] s name of the symbol that will be substituted.
   ...
   */
  template <typename U>
  std::vector<std::pair<ipow_subs_type<U>, monomial>>
   ...
   * If \p Cf is an instance of piranha::mp_rational, then
   ...
   *
   * @throws std::invalid_argument if the size of \p t1 differs from the size of
   * \p args.
   * @throws unspecified any exception thrown by
   * piranha::array_key::vector_add(), or by piranha::math::mul3().
   */
  // NOTE: it should be ok to use this method (and the one below) with
  // overlapping arguments, as this is allowed
  // in small_vector::add().
  template <typename Cf, typename U = monomial, multiply_enabler<Cf, U> = 0>
  static void multiply(std::array<term<Cf, monomial>, multiply_arity> &res,
                       const term<Cf, monomial> &t1,
  ...
  /**
   * \note
   * This method is enabled only if piranha::array_key::vector_add() is enabled
   * for \p a.
   *
   ...
   * @param[in] args reference set of arguments.
   *
   * @throws std::invalid_argument if the size of \p a differs from the size of
   * \p args.
   * @throws unspecified any exception thrown by
   * piranha::array_key::vector_add().
   */
  template <typename U = monomial, monomial_multiply_enabler<U> = 0>
  static void multiply(monomial &out, const monomial &a, const monomial &b,
  ...
  /**
   * \note
   * This method is enabled only if piranha::array_key::vector_sub() is enabled
   * for \p a.
   *
   ...
   * \p args.
   * @throws unspecified any exception thrown by
   * piranha::array_key::vector_sub().
   */
  template <typename U = monomial, monomial_divide_enabler<U> = 0>
  static void divide(monomial &out, const monomial &a, const monomial &b,
                     const symbol_set &args) {
    if (unlikely(a.size() != args.size())) {
      piranha_throw(std::invalid_argument, "invalid size of arguments set");
    }
   ...
   *
   * @return \p true if \p this is lexicographically less than \p other, \p
   * false otherwise.
   *
   * @throws std::invalid_argument if the sizes of \p this and \p other differ.
   * @throws unspecified any exception thrown by
   * <tt>std::lexicographical_compare()</tt>.
   */
  bool operator<(const monomial &other) const {
    ...
    if (unlikely(std::get<0u>(sbe1) != std::get<0u>(sbe2))) {
      piranha_throw(std::invalid_argument,
                    "mismatched sizes in monomial comparison");
    }
   ...
   *
   * Note that this method does not offer a strong exception safety guarantee,
   * as it uses
   ...
   */
  void extract_exponents(std::vector<typename base::value_type> &out,
                         const symbol_set &args) const {
   ...
   * other exponents.
   *
   * @throws std::invalid_argument if the size of \p args is different from the
   * size of \p this or less than 2.
  ...
  bool has_negative_exponent(const symbol_set &args) const {
    ...
    return std::any_of(std::get<1u>(sbe), std::get<2u>(sbe),
                       [&zero](const value_type &e) { return e < zero; });
  }
};
...
/**
 * Functionally equivalent to the \p std::hash specialisation for
 * piranha::array_key.
 */
template <typename T, typename S>
struct hash<piranha::monomial<T, S>>

...
// Fwd declaration.
template <typename> struct is_mp_rational;

// NOTE: this is a bit complicated: the interoperable types for mp_rational are
// those from mp_integer
// plus mp_integer, but not *any* mp_integer, just the one whose bits value
// matches that of the rational.
...
// The second complication is that we need to cope with the fact that we are
// using this tt in a context
// in which Rational might not actually be a rational (in the pow_impl
// specialsiation). In this case we must prevent
// a hard error to be generated.
template <typename T, typename Rational>
struct is_mp_rational_interoperable_type<
    T, Rational,
    typename std::enable_if<!is_mp_rational<Rational>::value>::type> {
...
/**
 * This class encapsulates two instances of piranha::mp_integer to represent an
 * arbitrary-precision rational number
 * in terms of a numerator and a denominator.
 * The meaning of the \p NBits template parameter is the same as in
 * piranha::mp_integer, that is, it represents the
 * bit width of the two limbs stored statically in the numerator and in the
 * denominator.
 *
 * Unless otherwise specified, rational numbers are always kept in the usual
 * canonical form in which numerator and denominator
 * are coprime, and the denominator is always positive. Zero is uniquely
 * represented by 0/1.
 *
 * ## Interoperability with other types ##
 *
 * This class interoperates with the same types as piranha::mp_integer, plus
 * piranha::mp_integer itself.
 * The same caveats with respect to interoperability with floating-point types
 ...
 */
template <int NBits = 0> class mp_rational {
public:
  /// The underlying piranha::mp_integer type used to represent numerator and
  /// denominator.
...
private:
  ...
  using nd_ctor_enabler = typename std::enable_if<
      (std::is_integral<I0>::value || std::is_same<I0, int_type>::value) &&
  ...
  void construct_from_interoperable(
      const T &x,
      typename std::enable_if<std::is_same<T, int_type>::value>::type * =
          nullptr) {
    m_num = x;
    m_den = 1;
  }
  ...
  void construct_from_interoperable(
    ...
    if (unlikely(!std::isfinite(x))) {
      piranha_throw(std::invalid_argument, "cannot construct a rational from a "
  ...
  template <typename Float>
  Float convert_to_impl(
      typename std::enable_if<std::is_floating_point<Float>::value>::type * =
          nullptr) const {
    // NOTE: there are better ways of doing this. For instance, here we might
    // end up generating an inf even
    // if the result is actually representable. It also would be nice if this
    // routine could short-circuit,
    // that is, for every rational generated from a float we get back exactly
    // the same float after the cast.
    // The approach in GMP mpq might work for this, but it's not essential at
    // the moment.
    return static_cast<Float>(m_num) / static_cast<Float>(m_den);
  }
  template <typename Integral>
  Integral convert_to_impl(
  ...
  MpInteger convert_to_impl(
      typename std::enable_if<std::is_same<MpInteger, int_type>::value>::type
          * = nullptr) const {
    return m_num / m_den;
  }
  // In-place add.
  mp_rational &in_place_add(const mp_rational &other) {
    // NOTE: all this should never throw because we only operate on mp_integer
    // objects,
    ...
    if (u1 && u2) {
      ...
      // NOTE: figure out a way here to use multiply_accumulate(). Most likely
      // we need
  ...
  static mp_rational binary_minus(const mp_rational &q1,
  ...
  static bool binary_greater_than(const mp_rational &q1,
                                  const mp_rational &q2) {
    if (q1.m_den == q2.m_den) {
      return q1.m_num > q2.m_num;
    }
  ...
  public:
    explicit mpq_view(const mp_rational &q)
        : m_n_view(q.num().get_mpz_view()), m_d_view(q.den().get_mpz_view()) {
    ...
    operator mpq_struct_t const *() const { return get(); }
    mpq_struct_t const *get() const { return &m_mpq; }
  ...
  using pow_enabler = typename std::enable_if<
      std::is_same<decltype(std::declval<const int_type &>().pow(
                       std::declval<const T &>())),
...
public:
  /// Default constructor.
  /**
   * This constructor will initialise the rational to zero (that is, the
   * numerator is set to zero, the denominator
   * to 1).
   */
  ...
  /// Move constructor.
  mp_rational(mp_rational &&other) noexcept : m_num(std::move(other.m_num)),
                                              m_den(std::move(other.m_den)) {
    // Fix the denominator of other, as its state depends on the implementation
    // of piranha::mp_integer.
    // Set it to 1, so other will have a valid canonical state regardless of
    // what happens
    // to the numerator.
    other.m_den = 1;
  }
  ...
  /**
   * \note
   * This constructor is enabled only if \p I0 and \p I1 are either integral
   * types or piranha::integer.
   *
   * @param[in] n numerator.
   * @param[in] d denominator.
   *
   * @throws piranha::zero_division_error if the denominator is zero.
   * @throws unspecified any exception thrown by the invoked constructor of
   * piranha::mp_integer.
   */
  ...
  /**
   * The string must represent either a valid single piranha::mp_integer, or two
   * valid piranha::mp_integer
   * separated by "/". The rational will be put in canonical form by this
   * constructor.
   *
   ...
   * @throws piranha::zero_division_error if the denominator, if present, is
   * zero.
  ...
  explicit mp_rational(const char *str) : m_num(), m_den(1) {
    ...
    try {
      ...
      piranha_throw(std::invalid_argument,
                    "invalid string input for rational type");
    }
   ...
   *
   * This operator will construct a temporary piranha::mp_rational from \p x and
   * will then move-assign it
   * to \p this.
   *
   ...
   *
   * @throws unspecified any exception thrown by the generic constructor from
   * interoperable type.
   */
  template <typename T, generic_ctor_enabler<T> = 0>
  ...
  /**
   * This assignment operator will construct a piranha::mp_rational from the
   * string \p str
   ...
   */
  mp_rational &operator=(const char *str) { return (*this = mp_rational(str)); }
  /// Assignment operator from C++ string.
  /**
   * This assignment operator will construct a piranha::mp_rational from the
   * string \p str
   ...
   *
   * @throws unspecified any exception thrown by the streaming operator of
   * piranha::mp_integer.
   */
   ...
   * @param[in,out] q rational to which the contents of the stream will be
   * assigned.
   *
  ...
  const int_type &num() const { return m_num; }
  /// Get const reference to the denominator.
  const int_type &den() const { return m_den; }
  /// Get an \p mpq view of \p this.
  /**
   * This method will return an object of an unspecified type \p mpq_view which
   * is implicitly convertible
   * to a const pointer to an \p mpq struct (and which can thus be used as a
   * <tt>const mpq_t</tt>
   * parameter in GMP functions). In addition to the implicit conversion
   * operator, the \p mpq struct pointer
   * can also be retrieved via the <tt>get()</tt> method of the \p mpq_view
   * class.
   * The pointee will represent a GMP rational whose value is equal to \p this.
   *
   * Note that the returned \p mpq_view instance can only be move-constructed
   ...
   */
  mpq_view get_mpq_view() const { return mpq_view{*this}; }
  /// Canonicality check.
  /**
   ...
   *
   * If low-level methods are not used, this function will always return \p
   * true.
   *
  ...
  bool is_canonical() const {
    ...
    // not throw either.
    // NOTE: there should be no way to set a negative denominator, so no check
    // is performed.
  ...
  void canonicalise() {
    ...
    // NOTE: this could be a nice place to use the demote() method of
    // mp_integer.
  }
  /// Conversion operator.
  /**
   ...
   *
   * The conversion to piranha::mp_integer is computed by dividing the numerator
   * by the denominator.
   * The conversion to integral types is computed by casting first to
   * piranha::mp_integer, then to
   * the target integral type. The conversion to floating-point types might
   ...
   * Low-level methods. These methods allow construction from \p mpq_t and
   * direct mutable access to numerator and
   ...
   * @throws piranha::zero_division_error if the denominator is zero.
   */
  explicit mp_rational(const ::mpq_t q)
      : m_num(mpq_numref(q)), m_den(mpq_denref(q)) {
    if (unlikely(m_den.sign() == 0)) {
      piranha_throw(zero_division_error, "zero denominator");
    }
  }
   ...
   */
  int_type &_num() { return m_num; }
  /// Mutable reference to the denominator.
  /**
   * @return mutable reference to the denominator.
   */
  int_type &_den() { return m_den; }
  /// Set denominator.
  /**
   * This method will set the denominator to \p den without canonicalising the
   * rational.
   *
   ...
   */
  mp_rational operator+() const { return mp_rational{*this}; }
   ...
   */
  mp_rational &operator++() { return operator+=(1); }
   ...
   * piranha::mp_rational.
   *
   * If \p T is not a float, the exact result will be computed. If \p T is a
   ...
   * @return reference to \p this.
   *
   * @throws unspecified any exception thrown by the conversion operator, the
   * generic constructor of piranha::mp_integer,
   ...
   *
   * Add a piranha::mp_rational in-place. This method will first compute <tt>q +
   ...
   * casting piranha::mp_rational to \p T.
   */
   ...
   * will be returned as a piranha::mp_rational.
   *
   ...
   * This operator is enabled only if \p T is a non-const interoperable type.
   *
   * Subtract a piranha::mp_rational in-place. This method will first compute
   * <tt>x - q</tt>, cast it back to \p T via \p static_cast and finally assign
   * the result to \p x.
   *
   * @param[in,out] x first argument.
   * @param[in] q second argument.
   ...
   * This operator is enabled only if \p T is a non-const interoperable type.
   *
   * Multiply by a piranha::mp_rational in-place. This method will first compute
   * <tt>x * q</tt>, cast it back to \p T via \p static_cast and finally assign
   * the result to \p x.
   *
   * @param[in,out] x first argument.
   ...
   * This operator is enabled only if \p T is a non-const interoperable type.
   *
   * Divide by a piranha::mp_rational in-place. This method will first compute
   * <tt>x / q</tt>, cast it back to \p T via \p static_cast and finally assign
   * the result to \p x.
   *
   * @param[in,out] x first argument.
   ...
   * If one of the arguments is a floating-point value \p f of type \p F, the
   * other argument will be converted to an instance of type \p F
   * and divided by \p f (or viceversa) to generate the return value, which will
   * then be of type \p F.
   *
   * @param[in] x first argument
   * @param[in] y second argument.
  ...
  /**
   * \note
   * This method is enabled only if piranha::mp_integer::pow() can be called
   * with an argument of type \p T.
   *
   * This method computes \p this raised to the integral power \p exp.
   * Internally, the piranha::mp_integer::pow()
   * method of numerator and denominator is used. Negative powers will raise an
   * error if the numerator of \p this
   * is zero.
   *
   ...
   *
   * @throws piranha::zero_division_error if \p exp is negative and the
   * numerator of \p this is zero.
  ...
  mp_rational pow(const T &exp) const {
    ...
    if (exp >= T(0)) {
      ...
      // is that the numerator has some value and den is still 1 from the
      // initialisation.
      ...
      if (unlikely(math::is_zero(num()))) {
        piranha_throw(zero_division_error,
                      "zero denominator in rational exponentiation");
      }
  ...
  /// Hash value.
  /**
   * The hash value is calculated by combining the hash values of numerator and
   * denominator.
   *
 ...
 */
inline rational operator"" _q(const char *s) { return rational(s); }
}
...
// size in mp_integer.
template <typename T> struct is_mp_rational : std::false_type {};
...
using rational_pow_enabler = typename std::enable_if<
    (is_mp_rational<T>::value &&
     is_mp_rational_interoperable_type<U, T>::value) ||
    (is_mp_rational<U>::value &&
     is_mp_rational_interoperable_type<T, U>::value) ||
    // NOTE: here we are catching two rational arguments with potentially
    // different
    // bits. BUT this case is not caught in the pow_impl, so we should be ok as
    // long
    // as we don't allow interoperablity with different bits.
    (is_mp_rational<T>::value &&is_mp_rational<U>::value)>::type;
...
/// Specialisation of the piranha::print_tex_coefficient() functor for
/// piranha::mp_rational.
template <typename T>
struct print_tex_coefficient_impl<
    T, typename std::enable_if<detail::is_mp_rational<T>::value>::type> {
  /// Call operator.
  /**
...
struct is_zero_impl<
   ...
   */
  bool operator()(const T &q) const { return is_zero(q.num()); }
};
...
struct negate_impl<
   ...
   */
  void operator()(T &q) const { q.negate(); }
};

///// Specialisation of the piranha::math::pow() functor for
/// piranha::mp_rational.
/**
 * This specialisation is activated when one of the arguments is
 * piranha::mp_rational
 * and the other is either piranha::mp_rational or an interoperable type for
 * piranha::mp_rational.
 *
 * The implementation follows these rules:
 * - if the base is rational and the exponent an integral type or
 * piranha::mp_integer, then
 *   piranha::mp_rational::pow() is used;
 * - if the non-rational argument is a floating-point type, then the rational
 * argument is converted
 *   to that floating-point type and piranha::math::pow() is used;
 * - if both arguments are rational the result will be rational and the success
 * of the operation depends
 *   on the values of the operands;
 * - if the base is an integral type or piranha::mp_integer and the exponent a
 * rational, the result is computed
 *   via the integral--integer specialisation of piranha::math::pow() and the
...
struct pow_impl<T, U, detail::rational_pow_enabler<T, U>> {
  ...
  auto operator()(const mp_rational<NBits> &b, const T2 &e) const
      -> decltype(b.pow(e)) {
    return b.pow(e);
  }
   ...
   */
  template <
      int NBits, typename T2,
      typename std::enable_if<std::is_floating_point<T2>::value, int>::type = 0>
   ...
   */
  template <int NBits>
  mp_rational<NBits> operator()(const mp_rational<NBits> &b,
                                const mp_rational<NBits> &e) const {
    // Special casing.
    if (is_unitary(b)) {
      return b;
    }
    if (is_zero(b)) {
      ...
      piranha_throw(zero_division_error,
                    "unable to raise zero to a negative power");
    }
    if (!e.den().is_unitary()) {
      piranha_throw(std::invalid_argument, "unable to raise rational to a "
                                           "rational power whose denominator "
                                           "is not 1");
    }
    return b.pow(e.num());
  }
   ...
   *
   * @throws std::invalid_argument if the result cannot be computed.
   * @throws unspecified any exception thrown by piranha::math::pow() with
   * integral base and piranha::mp_integer exponent.
   */
  template <int NBits, typename T2,
            typename std::enable_if<std::is_integral<T2>::value ||
                                        detail::is_mp_integer<T2>::value,
                                    int>::type = 0>
  auto operator()(const T2 &b, const mp_rational<NBits> &e) const
      -> decltype(math::pow(b, e.num())) {
    ...
    if (!e.den().is_unitary()) {
      piranha_throw(std::invalid_argument, "unable to raise an integral to a "
                                           "rational power whose denominator "
                                           "is not 1");
    }
...
struct abs_impl<
   ...
   */
  T operator()(const T &q) const { return q.abs(); }
};
...
/// Specialisation of the piranha::math::binomial() functor for
/// piranha::mp_rational.
/**
 * This specialisation is activated when one of the arguments is
 * piranha::mp_rational and the other is either
 * piranha::mp_rational or an interoperable type for piranha::mp_rational.
 *
 * The implementation follows these rules:
 * - if the top is rational and the bottom an integral type or
 * piranha::mp_integer, then
 *   piranha::mp_rational::binomial() is used;
 * - if the non-rational argument is a floating-point type, then the rational
 * argument is converted
 *   to that floating-point type and piranha::math::binomial() is used;
 * - if both arguments are rational, they are both converted to \p double and
 * then piranha::math::binomial()
 *   is used;
 * - if the top is an integral type or piranha::mp_integer and the bottom a
 * rational, then both
...
struct binomial_impl<T, U, detail::rational_binomial_enabler<T, U>> {
   ...
   */
  template <int NBits, typename T2>
  auto operator()(const mp_rational<NBits> &x, const T2 &y) const
      -> decltype(x.binomial(y)) {
    return x.binomial(y);
  }
  /// Call operator, rational--floating-point overload.
  /**
   ...
   */
  template <int NBits>
  double operator()(const mp_rational<NBits> &x,
                    const mp_rational<NBits> &y) const {
...
struct divexact_impl<
  ...
  /// Call operator.
  /**
   * The call is equivalent to piranha::math::div3(). The exact result of the
   * division
   ...
   */
  auto operator()(T &out, const T &q1, const T &q2) const
      -> decltype(div3(out, q1, q2)) {
...
struct has_exact_ring_operations<
    T, typename std::enable_if<
           detail::is_mp_rational<typename std::decay<T>::type>::value>::type> {
...
const bool has_exact_ring_operations<
    T, typename std::enable_if<detail::is_mp_rational<
...
/// Specialisation of piranha::safe_cast() for conversions involving
/// piranha::mp_rational.
/**
 * This specialisation is enabled in the following cases:
 * - \p To is a rational type and \p From is either an arithmetic type or
 * piranha::mp_integer,
 * - \p To is an integral type or piranha::mp_integer, and \p From is
 * piranha::mp_rational.
 */
template <typename To, typename From>
...
private:
  ...
  using to_enabler =
      typename std::enable_if<std::is_arithmetic<T>::value ||
                                  detail::is_mp_integer<T>::value,
                              int>::type;
  template <typename T>
  using from_enabler =
      typename std::enable_if<detail::is_mp_rational<T>::value, int>::type;
...
public:
  ...
  /**
   * The conversion, performed via the conversion operator of
   * piranha::mp_rational,
  ...
  To operator()(const From2 &q) const {
    if (unlikely(!q.den().is_unitary())) {
      piranha_throw(
          ...
          "non-unitary denominator in rational to integral conversion");
    }
...
template <int NBits> struct hash<piranha::mp_rational<NBits>> {
  /// Result type.
   ...
   */
  result_type operator()(const argument_type &q) const { return q.hash(); }
};
}

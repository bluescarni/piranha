...
/**
 * This toolbox will conditionally augment a \p Series type by adding a method
 * to subtitute symbols with generic objects.
 * Such augmentation takes place if the series' coefficient and/or key types
 * expose substitution methods (as established by
 * the piranha::has_subs and piranha::key_has_subs type traits). If the
 * requirements outlined above are not satisfied, the substitution
 ...
 * - \p Derived must derive from piranha::substitutable_series of \p Series and
 * \p Derived.
 *
...
class substitutable_series : public Series, detail::substitutable_series_tag {
  ...
  template <typename Term, typename T> struct subs_term_score {
    static const unsigned value =
        ...
        (static_cast<unsigned>(key_has_subs<typename Term::key_type, T>::value)
         << 1u);
  };
  // Case 1: subs only on cf.
  template <typename T, typename Term>
  using cf_subs_type =
      decltype(math::subs(std::declval<typename Term::cf_type const &>(),
  ...
  template <typename T, typename Term,
            typename std::enable_if<subs_term_score<Term, T>::value == 1u,
                                    int>::type = 0>
  static ret_type_1<T, Term> subs_term_impl(const Term &t,
  ...
  template <typename T, typename Term>
  using k_subs_type =
      typename decltype(std::declval<const typename Term::key_type &>().subs(
  ...
  template <typename T, typename Term,
            typename std::enable_if<subs_term_score<Term, T>::value == 2u,
                                    int>::type = 0>
  static ret_type_2<T, Term> subs_term_impl(const Term &t,
                                            const std::string &name, const T &x,
                                            const symbol_set &s_set) {
  ...
  template <typename T, typename Term,
            typename std::enable_if<subs_term_score<Term, T>::value == 3u,
                                    int>::type = 0>
  static ret_type_3<T, Term> subs_term_impl(const Term &t,
                                            const std::string &name, const T &x,
                                            const symbol_set &s_set) {
  ...
  // plus on the fact that
  // the subs type must be returnable.
  // NOTE: the returnable check here should be enough, as the functions above
  // will not be called if the check
  // here is not passed.
  template <typename T>
  using subs_type = typename std::enable_if<
      std::is_constructible<subs_type_<T>, int>::value &&
          is_addable_in_place<subs_type_<T>>::value &&
          is_returnable<subs_type_<T>>::value,
      subs_type_<T>>::type;
...
public:
   ...
   * to compute the result. Also, the return type must satisfy
   * piranha::is_constructible.
   *
   * This method will return an object resulting from the substitution of the
   * symbol called \p name
...
/// Specialisation of the piranha::math::subs_impl functor for instances of
/// piranha::substitutable_series.
/**
 * This specialisation is activated if \p Series is an instance of
 * piranha::substitutable_series which supports
...
struct subs_impl<Series, T, detail::subs_impl_subs_series_enabler<Series, T>> {
   ...
   * method.
   */
  auto operator()(const Series &s, const std::string &name, const T &x) const
...
PIRANHA_TEMPLATE_SERIALIZATION_LEVEL(piranha::substitutable_series,

...
template <typename = int> struct base_settings {
  ...
  // NOTE: this corresponds to circa 2% overhead from thread management on a
  // common desktop
...
template <typename T> std::mutex base_settings<T>::m_mutex;

template <typename T>
unsigned
    base_settings<T>::m_cache_line_size = runtime_info::get_cache_line_size();

template <typename T>
unsigned long base_settings<T>::m_max_term_output =
    base_settings<T>::m_default_max_term_output;
...
template <typename T>
std::atomic_ullong base_settings<T>::s_min_work_per_thread(
    base_settings<T>::s_default_min_work_per_thread);
}
 ...
 */
template <typename = void> class settings_ : private detail::base_settings<> {
public:
  /// Get the number of threads available for use by piranha.
  /**
   * The initial value is set to the maximum between 1 and
   ...
   */
  static unsigned get_n_threads() { return thread_pool::size(); }
   ...
   */
  static void set_n_threads(unsigned n) { thread_pool::resize(n); }
  /// Reset the number of threads available for use by piranha.
  /**
   * Will set the number of threads to the maximum between 1 and
   * piranha::runtime_info::get_hardware_concurrency().
   *
  ...
  /**
   * The initial value is set to the output of
   * piranha::runtime_info::get_cache_line_size(). The value
   * can be overridden with set_cache_line_size() in case the detection fails
  ...
  /**
   * Overrides the detected cache line size. This method should be used only if
   * the automatic
   * detection fails.
   *
  ...
  /**
   * Will set the value to the output of
   * piranha::runtime_info::get_cache_line_size().
   *
   * @throws std::system_error in case of failure(s) by threading primitives.
   */
...
/**
 * This is the alias through which the methods in piranha::settings_ should be
 * called.
 */

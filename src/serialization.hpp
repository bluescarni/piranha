...
// - think about potentially "malicious" archives being loaded. We have some
// classes that
...
//   - possibly more...
//   Right now the general policy is to just restore class members whatever
//   their value is.
//   We need to decide ultimately if we want to have a layer of safety in the
//   serialization methods
//   or not. It comes at a price in terms of complexity and performance. A
//   possible middle way would
//   be to enable the safety checks in the text archives but disable them in the
//   binary ones,
//   for performance. The safety layer could be checked by crafting bad text
//   archives and storing
//   them as strings from the tests.
// - Should probably test the exception safety of the serialization routines.
// Some examples:
//   - serialization of small vector leaves the object in a consistent state if
//   the deserialization
//     of an element fails;
//   - deserialization of a term leaves cf/key in the original state in case of
//   errors;
//   - more...
//   At the moment the routines are coded to provide at least the basic
//   exception safety, but
...
// Macro to customize the serialization level for a template class. Adapted
// from:

...
// A simple RAII holder for an array of atomic flags. Bare minimum
// functionality.
struct atomic_flag_array {
  using value_type = std::atomic_flag;
  // This constructor will init all the flags in the array to false.
  explicit atomic_flag_array(const std::size_t &size)
      : m_ptr(nullptr), m_size(size) {
    if (unlikely(size > std::numeric_limits<std::size_t>::max() /
                            sizeof(value_type))) {
      piranha_throw(std::bad_alloc, );
    }
    // NOTE: this throws if allocation fails, after this line everything is
    // noexcept.
    m_ptr = static_cast<value_type *>(
        aligned_palloc(0u, size * sizeof(value_type)));
  ...
  value_type &operator[](const std::size_t &i) { return *(m_ptr + i); }
...
// The memory order specification is to squeeze out some extra performance with
// respect to the
...
struct atomic_lock_guard {
  ...
  ~atomic_lock_guard() { m_af.clear(std::memory_order_release); }
  // Delete explicitly all other ctors/assignment operators.

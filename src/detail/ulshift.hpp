...
// This method exists to prevent potential UB when left shifting short unsigned integral type (e.g., unsigned short, unsigned char). In this situation,
// the left operand might be promoted to int because of integral promotions. Then we are left shifting on a signed type, and we could run in undefined

...
inline VType km_unpack(const symbol_set &args, const T &value) {
  if (unlikely(args.size() > VType::max_size)) {
    piranha_throw(std::invalid_argument,
                  "input set of arguments is too large for unpacking");
  }
...
template <typename VType, typename KaType, typename T>
inline T km_merge_args(const symbol_set &orig_args, const symbol_set &new_args,
                       const T &value) {
  using size_type =
      min_int<typename VType::size_type, decltype(new_args.size())>;
...
template <typename VType, typename KaType, typename T>
inline void km_trim_identify(symbol_set &candidates, const symbol_set &args,
                             const T &value) {
  const VType tmp = km_unpack<VType, KaType>(args, value);
  for (min_int<decltype(tmp.size()), decltype(args.size())> i = 0u;
       i < tmp.size(); ++i) {
    if (!math::is_zero(tmp[i]) &&
        std::binary_search(candidates.begin(), candidates.end(), args[i])) {
...
template <typename VType, typename KaType, typename T>
inline T km_trim(const symbol_set &trim_args, const symbol_set &orig_args,
                 const T &value) {
  using size_type =
      min_int<typename VType::size_type, decltype(orig_args.size())>;

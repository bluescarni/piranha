...
template <typename Term> struct term_hasher {
  std::size_t operator()(const Term &term) const { return term.hash(); }
};

// NOTE: this needs to go here, instead of in the series class as private
// method,
// because of a bug in GCC 4.7:
// http://gcc.gnu.org/bugzilla/show_bug.cgi?id=53137
template <typename Term, typename Derived>
inline std::pair<typename Term::cf_type, Derived>
pair_from_term(const symbol_set &s, const Term &t) {
...
// Apply a functor to a single-coefficient series. This is in the spirit of the
// old apply_cf_functor method,
...
inline RetT apply_cf_functor(const T &s) {
  ...
  if (!s.is_single_coefficient()) {
    piranha_throw(std::invalid_argument,
                  std::string("cannot compute ") + Functor::name +
  ...
  if (s.empty()) {
    ...
    retval.insert(
        term_type(f(s._container().begin()->m_cf), key_type(symbol_set{})));
  }
  return retval;
}
}
...
/**
 * This type trait will be true if \p T is an instance of piranha::series which
...
template <typename T> class is_series {
...
template <typename T> const bool is_series<T>::value;
...
struct series_is_rebindable_impl : detail::sfinae_types {
  ...
  template <typename T1> using rebound_type = typename T1::template rebind<Cfd>;
  template <typename T1,
            typename std::enable_if<
                is_series<rebound_type<T1>>::value &&
                    std::is_same<typename rebound_type<T1>::term_type::cf_type,
                                 Cfd>::value,
                int>::type = 0>
  ...
  static const bool value =
      !std::is_same<no, decltype(test(std::declval<Td>()))>::value;
};

template <typename T, typename Cf>
struct series_is_rebindable_impl<
    T, Cf, typename std::enable_if<
               !is_cf<typename std::decay<Cf>::type>::value ||
...
/**
 * Rebinding a series type \p T means to change its coefficient type to \p Cf.
 * The ability of a series
 * to be rebound is signalled by the presence of a <tt>rebind</tt> template
 * alias defined within the series class.
 *
 ...
 *
 * The decay types of \p T and \p Cf are considered in this type trait. If \p T
 * is not an instance of piranha::series
 * or \p Cf does not satisfy piranha::is_cf, then the value of the type trait
 * will be \p false.
 */
// NOTE: the behaviour when T and Cf do not satisfy the requirements is to allow
// this type trait
// to be used an all types. The trait is used in the metaprogramming of generic
// series arithmetics,
// and if we ever move the operators outside the series_operators class it is
// convenient to have this
// class not fire static asserts when non-series arguments are substituted.
template <typename T, typename Cf> struct series_is_rebindable {
  /// Value of the type trait.
...
// Implementation of the series_rebind alias, with SFINAE to soft-disable it in
// case
// the series is not rebindable.
template <typename T, typename Cf>
using series_rebind_ =
    typename std::enable_if<series_is_rebindable<T, Cf>::value,
                            typename std::decay<T>::type::template rebind<
                                typename std::decay<Cf>::type>>::type;
}

/// Rebind series.
/**
 * Utility alias to rebind series \p T to coefficient \p Cf. See
 * piranha::series_is_rebindable for an explanation.
 * In addition to being a shortcut to the \p rebind alias in \p T (if present),
 * the implementation will
 * also check that \p T and \p Cf satisfy the piranha::series_is_rebindable type
 * traits.
 */
template <typename T, typename Cf>
...
/**
 * The recursion index measures how many times the coefficient of a series types
 * is itself a series type.
 * The algorithm works as follows:
 * - if \p T is not a series type, the recursion index is 0,
 * - else, the recursion index of \p T is the recursion index of its coefficient
 * type plus one.
 *
 * The decay type of \p T is considered in this type trait.
 */
template <typename T, typename = void> class series_recursion_index {
...
class series_recursion_index<
    T, typename std::enable_if<std::is_base_of<
           detail::series_tag, typename std::decay<T>::type>::value>::type> {
  using cf_type = typename std::decay<T>::type::term_type::cf_type;
  static_assert(series_recursion_index<cf_type>::value <
                    std::numeric_limits<std::size_t>::max(),
                "Overflow error.");

public:
  static const std::size_t value =
      static_cast<std::size_t>(series_recursion_index<cf_type>::value + 1u);
};
...
const std::size_t series_recursion_index<
    ...
    typename std::enable_if<std::is_base_of<
        detail::series_tag, typename std::decay<T>::type>::value>::type>::value;

#endif

/// Type trait to detect the availability of a series multiplier.
/**
 * This type trait will be \p true if a piranha::series_multiplier of \p Series
 * can be constructed and used
 * (as described in the piranha::series_multiplier documentation), \p false
 * otherwise. The decay type of \p Series,
 * which must satisfy piranha::is_series, is considered by this type trait.
 */
template <typename Series> class series_has_multiplier : detail::sfinae_types {
  ...
  static no test(...);
  static const bool implementation_defined =
      std::is_same<decltype(test(std::declval<Sd>())), Sd>::value;
...
template <typename Series> const bool series_has_multiplier<Series>::value;

namespace detail {

// Some notes on this machinery:
// - this is only for determining the type of the result, but it does not
// guarantee that we can actually compute it.
//   In general we should separate the algorithmic requirements from the
//   determination of the type. Note that we still use
//   the operators on the coefficients to determine the return type, but that's
//   inevitable.

// Alias for getting the cf type from a series. Will generate a type error if S
// is not a series.
// NOTE: the is_series check is an extra safe guard to really assert we are
// operating on series. Without the checks, in theory classes with internal
// term_type and similar typedefs
// could trigger no errors.
template <typename S>
using bso_cf_t = typename std::enable_if<is_series<S>::value,
...
template <typename, typename, int, typename = void> struct op_result {};

template <typename T, typename U>
struct op_result<T, U, 0,
                 typename std::enable_if<is_addable<T, U>::value>::type> {
  using type = decltype(std::declval<const T &>() + std::declval<const U &>());
};

template <typename T, typename U>
struct op_result<T, U, 1,
                 typename std::enable_if<is_subtractable<T, U>::value>::type> {
  using type = decltype(std::declval<const T &>() - std::declval<const U &>());
};

template <typename T, typename U>
struct op_result<T, U, 2,
                 typename std::enable_if<is_multipliable<T, U>::value>::type> {
  using type = decltype(std::declval<const T &>() * std::declval<const U &>());
};

template <typename T, typename U>
struct op_result<T, U, 3,
                 typename std::enable_if<is_divisible<T, U>::value>::type> {
  using type = decltype(std::declval<const T &>() / std::declval<const U &>());
};

// Coefficient type in a binary arithmetic operation between two series with the
// same recursion index.
// Will generate a type error if S1 or S2 are not series with same recursion
// index or if their coefficients
// do not support the operation.
template <typename S1, typename S2, int N>
using bso_cf_op_t = typename std::enable_if<
    series_recursion_index<S1>::value == series_recursion_index<S2>::value &&
        series_recursion_index<S1>::value != 0u,
    typename op_result<bso_cf_t<S1>, bso_cf_t<S2>, N>::type>::type;

// Coefficient type in a mixed binary arithmetic operation in which the first
// operand has recursion index
// greater than the second. Will generate a type error if S does not have a rec.
// index > T, or if the coefficient
// of S cannot be opped to T.
template <typename S, typename T, int N>
using bsom_cf_op_t =
    typename std::enable_if<(series_recursion_index<S>::value >
                             series_recursion_index<T>::value),
...
struct binary_series_op_return_type<
    S1, S2, N,
    typename std::enable_if<
        // NOTE: in all these checks, every access to S1 and S2 should not incur
        // in hard errors:
        // - the series recursion index works on all types safely,
        // - the various _t aliases are SFINAE friendly - if S1 or S2 are not
        // series with the appropriate characteristics,
        //   or the coefficients are not oppable, etc., there should be a soft
        //   error.
        /*a*/ series_recursion_index<S1>::value ==
            series_recursion_index<S2>::value &&
        series_recursion_index<S1>::value != 0u &&
...
// Case 1:
// a. both are series with same recursion index,
// b. the coefficients are not the same and the op results in the first
// coefficient.
template <typename S1, typename S2, int N>
struct binary_series_op_return_type<
    S1, S2, N,
...
// Case 2:
// a. both are series with same recursion index,
// b. the coefficients are not the same and the op results in the second
// coefficient.
template <typename S1, typename S2, int N>
struct binary_series_op_return_type<
    S1, S2, N,
...
// c. both series are rebindable to the new coefficient, yielding the same
// series.
template <typename S1, typename S2, int N>
struct binary_series_op_return_type<
    S1, S2, N,
    typename std::enable_if<
        ...
        /*c*/ std::is_same<series_rebind<S1, bso_cf_op_t<S1, S2, N>>,
                           series_rebind<S2, bso_cf_op_t<S1, S2, N>>>::value>::
        type> {
...
template <typename S1, typename S2, int N>
struct binary_series_op_return_type<
    S1, S2, N, typename std::enable_if<
                   /*a*/ (series_recursion_index<S1>::value >
                          series_recursion_index<S2>::value) &&
                   /*b*/ std::is_same<bsom_cf_op_t<S1, S2, N>,
                                      bso_cf_t<S1>>::value>::type> {
  using type = S1;
...
// Case 5:
// a. the first operand has recursion index greater than the second operand,
// b. the op of the coefficient of S1 by S2 results in a type T different from
// the coefficient of S1,
...
struct binary_series_op_return_type<
    S1, S2, N, typename std::enable_if<
                   /*a*/ (series_recursion_index<S2>::value >
                          series_recursion_index<S1>::value) &&
                   /*b*/ std::is_same<bsom_cf_op_t<S2, S1, N>,
                                      bso_cf_t<S2>>::value>::type> {
  using type = S2;
...
// a. the second operand has recursion index greater than the first operand,
// b. the op of the coefficient of S2 by S1 results in a type T different from
...
/// Series operators.
/**
 * This class contains the arithmetic and comparison operator overloads for
 * piranha::series instances. The operators
 * are implemented as inline friend functions and they will be found via
 * argument-dependent name lookup when at least
 * one of the two operands is an instance of piranha::series.
 *
 * The operators defined here, similarly to the builtin operators in C++,
 * promote one or both operands to a common
 * type, if necessary, before actually performing the operation. The promotion
 * rules are dependent on
 * the recursion indices and coefficient types of the series, and they rely on
 * the series rebinding mechanism to promote a series
 * as needed (see piranha::series_is_rebindable and
 * piranha::series_recursion_index).
 *
 * These are the scenarios handled by the type promotion mechanism:
 * - the two arguments are of the same series type and the operator on the
 * coefficient type of the series results in the same coefficient type.
 *   In this case there is no type promotion;
 * - both series arguments have the same recursion index, different
 * coefficients, and the operator on the coefficient types results in the first
 *   (resp. second) coefficient type. In this case the result of the operations
 * is the first (resp. second) series type;
 * - both series arguments have the same recursion index, different
 * coefficients, and the operator on the coefficient types results in something
 *   other than the first or second coefficient type. In this case both series
 * are promoted to a type resulting from the rebinding of the two
 *   series to the resulting coefficient type;
 * - the first (resp. second) argument has recursion index greater than the
 * second (resp. first) one, and the result type of the operator on the
 * coefficient
 *   type of the first (resp. second) argument and the second (resp. first)
 * argument is the coefficient type of the first (resp. second) argument. In
 * this case,
 ...
 * coefficient
 *   type of the first (resp. second) argument and the second (resp. first)
 * argument is something other than the coefficient type of the first (resp.
 * second) argument.
 *   In this case, both arguments are promoted to a type resulting from the
 * rebinding of the first (resp. second) argument to the resulting coefficient
 * type.
 *
 * If any necessary conversion is not possible, either because the series are
 * not rebindable or they do not support the needed constructors, the operators
 * are disabled. The operators are also disabled if any operation needed by the
 * implementation is not supported, or if an ambiguity arises in the type
 * promotion algorithm (e.g., two series with same recursion index, same
 * coefficient type which does not trigger any promotion, and different key
 * types).
 *
 * A few things to note about the operators implemented within this class:
 * - in case two series arguments have different symbol sets, either one or both
 * series will be copied in a new series in which the symbols
 *   have been merged, and the operation will be performed on those series
 * instead;
 * - in-place arithmetic operators are implemented as binary operators plus
 * move-assignment (and they are thus disabled if either the corresponding
 *   binary operation or the assignment are invalid);
 * - series multiplication requires the coefficient types to be multipliable and
 * a valid specialisation of piranha::series_multiplier for the
 *   promoted series type;
 * - division is implemented only when it reduces to coefficient division (true
 * series division may be implemented in specific series types,
 *   e.g., piranha::polynomial);
 * - the comparison operators will use <tt>operator+()</tt> on the coefficient
 * types to determine if any type promotion is necessary
 *   before performing the comparison.
 *
 * The operators are implemented using variadic templates in order to allow
 * overriding them via non-variadic overloads.
 */
class series_operators {
  // A couple of handy aliases.
  // NOTE: we need both bso_type and common_type because bso gives access to the
  // ::value
  // member to decide on the implementation, common_type is handy because it's
  // shorter
  // than using ::type on the bso.
  template <typename T, typename U, int N>
  using bso_type =
      detail::binary_series_op_return_type<typename std::decay<T>::type,
                                           typename std::decay<U>::type, N>;
  template <typename T, typename U, int N>
  using series_common_type = typename bso_type<T, U, N>::type;
  // Main implementation function for add/sub: all the possible cases end up
  // here eventually, after
  ...
  static series_common_type<T, U, 0> binary_add_impl(T &&x, U &&y) {
    ...
    static_assert(std::is_same<typename std::decay<T>::type, ret_type>::value,
                  "Invalid type.");
    static_assert(std::is_same<typename std::decay<U>::type, ret_type>::value,
                  "Invalid type.");
    // Init the return value from the first operand.
    // NOTE: this is always possible to do, a series is always
    ...
    // This could be problematic in the algorithm below if retval was move-cted
    // from
    // x, since in such a case y will also be erased. This will happen if one
    // does
  ...
  // NOTE: this case has no special algorithmic requirements, the base
  // requirements for cf and series types
  // already cover everything (including the needeed series constructors and cf
  // arithmetic operators).
  template <
      typename T, typename U,
      typename std::enable_if<bso_type<T, U, 0>::value == 0u, int>::type = 0>
  ...
  template <
      ...
      typename std::enable_if<
          (bso_type<T, U, 0>::value == 1u || bso_type<T, U, 0>::value == 4u) &&
              // In order for the implementation to work, we need to be able to
              // build T from U.
  ...
  template <
      typename T, typename U,
      typename std::enable_if<bso_type<T, U, 0>::value == 2u, int>::type = 0>
  ...
  // - first higher recursion, coefficient result is different from first
  // series.
  ...
  template <
      ...
      typename std::enable_if<
          (bso_type<T, U, 0>::value == 3u || bso_type<T, U, 0>::value == 5u) &&
              // In order for the implementation to work, we need to be able to
              // build the common type from T and U.
              std::is_constructible<
                  series_common_type<T, U, 0>,
  ...
  template <typename T, typename U,
            typename std::enable_if<bso_type<T, U, 0>::value == 6u ||
                                        bso_type<T, U, 0>::value == 7u,
                                    int>::type = 0>
  static auto dispatch_binary_add(T &&x, U &&y)
      -> decltype(dispatch_binary_add(std::forward<U>(y), std::forward<T>(x))) {
    return dispatch_binary_add(std::forward<U>(y), std::forward<T>(x));
  ...
  // here is that any overload other than the const references one is an
  // optimisation detail
  // and that for the operator to be enabled the "canonical" form of addition
  // operator must be available.
  // The consequence is that if, for instance, only a move constructor is
  // available, the operator
  // will anyway be disabled even if technically we could still perform the
  // computation.
  template <typename T, typename... U>
  using binary_add_type = decltype(dispatch_binary_add(
      std::declval<const typename std::decay<T>::type &>(),
      std::declval<const typename std::decay<U>::type &>()...));
  // In-place add. Default implementation is to do simply x = x + y, if
  // possible.
  // NOTE: this should also be able to handle int += series, if we ever
  // implement it.
  template <
      typename T, typename U,
      typename std::enable_if<
          std::is_assignable<T &, binary_add_type<T, U>>::value, int>::type = 0>
  ...
  // NOTE: as explained above, here the enabling mechanism essentially recasts U
  // as a const reference,
  // because we require in-place addition to work with the second argument as a
  // const reference (as that is
  // the canonical form of in-place addition). The actual call will however do
  // perfect forwarding, so optimisations
  // are still possible.
  template <typename T, typename... U>
  using in_place_add_type = decltype(dispatch_in_place_add(
      std::declval<T &>(),
      std::declval<const typename std::decay<U>::type &>()...));
  template <typename T, typename... U>
  using in_place_add_enabler = typename std::enable_if<
      detail::true_tt<in_place_add_type<T, U...>>::value, int>::type;
  // Subtraction.
  template <
      typename T, typename U,
      typename std::enable_if<bso_type<T, U, 1>::value == 0u, int>::type = 0>
  ...
  template <
      typename T, typename U,
      typename std::enable_if<
          (bso_type<T, U, 1>::value == 1u || bso_type<T, U, 1>::value == 4u) &&
              std::is_constructible<
                  typename std::decay<T>::type,
                  const typename std::decay<U>::type &>::value,
          int>::type = 0>
  ...
  template <
      typename T, typename U,
      typename std::enable_if<bso_type<T, U, 1>::value == 2u, int>::type = 0>
  ...
  template <typename T, typename U,
            typename std::enable_if<bso_type<T, U, 1>::value == 6u ||
                                        bso_type<T, U, 1>::value == 7u,
                                    int>::type = 0>
  static auto dispatch_binary_sub(T &&x, U &&y)
      -> decltype(dispatch_binary_sub(std::forward<U>(y), std::forward<T>(x))) {
    auto retval = dispatch_binary_sub(std::forward<U>(y), std::forward<T>(x));
  ...
  template <typename T, typename... U>
  using binary_sub_type = decltype(dispatch_binary_sub(
      std::declval<const typename std::decay<T>::type &>(),
      std::declval<const typename std::decay<U>::type &>()...));
  template <
      typename T, typename U,
      typename std::enable_if<
          std::is_assignable<T &, binary_sub_type<T, U>>::value, int>::type = 0>
  static T &dispatch_in_place_sub(T &x, U &&y) {
    x = dispatch_binary_sub(std::move(x), std::forward<U>(y));
    return x;
  }
  template <typename T, typename... U>
  using in_place_sub_type = decltype(dispatch_in_place_sub(
      std::declval<T &>(),
      std::declval<const typename std::decay<U>::type &>()...));
  template <typename T, typename... U>
  using in_place_sub_enabler = typename std::enable_if<
      detail::true_tt<in_place_sub_type<T, U...>>::value, int>::type;
  // Multiplication.
  template <typename T, typename U>
  static series_common_type<T, U, 2> binary_mul_impl(T &&x, U &&y) {
    return series_multiplier<series_common_type<T, U, 2>>(std::forward<T>(x),
                                                          std::forward<U>(y))();
  }
  template <
      typename T, typename U,
      typename std::enable_if<bso_type<T, U, 2>::value == 0u, int>::type = 0>
  static series_common_type<T, U, 2> dispatch_binary_mul(T &&x, U &&y) {
    ...
    if (likely(x.m_symbol_set == y.m_symbol_set)) {
      ...
      const bool need_copy_x = (merge != x.m_symbol_set),
                 need_copy_y = (merge != y.m_symbol_set);
  ...
  template <
      typename T, typename U,
      typename std::enable_if<
          (bso_type<T, U, 2>::value == 1u || bso_type<T, U, 2>::value == 4u) &&
              std::is_constructible<
                  typename std::decay<T>::type,
                  const typename std::decay<U>::type &>::value,
          int>::type = 0>
  ...
  template <
      typename T, typename U,
      typename std::enable_if<bso_type<T, U, 2>::value == 2u, int>::type = 0>
  ...
  template <typename T, typename U,
            typename std::enable_if<bso_type<T, U, 2>::value == 6u ||
                                        bso_type<T, U, 2>::value == 7u,
                                    int>::type = 0>
  static auto dispatch_binary_mul(T &&x, U &&y)
      -> decltype(dispatch_binary_mul(std::forward<U>(y), std::forward<T>(x))) {
    return dispatch_binary_mul(std::forward<U>(y), std::forward<T>(x));
  }
  // NOTE: this is the real type from the multiplication, below we put another
  // enable_if to make it conditional
  // on the presence of a series multiplier.
  template <typename T, typename... U>
  using binary_mul_type_ = decltype(dispatch_binary_mul(
      std::declval<const typename std::decay<T>::type &>(),
      std::declval<const typename std::decay<U>::type &>()...));
  template <typename T, typename... U>
  using binary_mul_type = typename std::enable_if<
      series_has_multiplier<binary_mul_type_<T, U...>>::value,
      binary_mul_type_<T, U...>>::type;
  template <
      typename T, typename U,
      typename std::enable_if<
          std::is_assignable<T &, binary_mul_type<T, U>>::value, int>::type = 0>
  static T &dispatch_in_place_mul(T &x, U &&y) {
    x = dispatch_binary_mul(std::move(x), std::forward<U>(y));
    return x;
  }
  template <typename T, typename... U>
  using in_place_mul_type = decltype(dispatch_in_place_mul(
      std::declval<T &>(),
      std::declval<const typename std::decay<U>::type &>()...));
  template <typename T, typename... U>
  using in_place_mul_enabler = typename std::enable_if<
      detail::true_tt<in_place_mul_type<T, U...>>::value, int>::type;
  // Division.
  // NOTE: the divisibility requirement here is already satisfied in the
  // determination of the return type.
  // The base case: same recursion, no promotion.
  template <
      typename T, typename U,
      typename std::enable_if<bso_type<T, U, 3>::value == 0u, int>::type = 0>
  static series_common_type<T, U, 3> dispatch_binary_div(T &&x, U &&y) {
    ...
    if (!y.is_single_coefficient()) {
      piranha_throw(std::invalid_argument,
                    "divisor in series division does not "
  ...
  template <
      typename T, typename U,
      typename std::enable_if<
          (bso_type<T, U, 3>::value == 2u || bso_type<T, U, 3>::value == 6u) &&
              std::is_constructible<
  ...
  // The implementation of these two cases 4 and 5 is different from the other
  // operations, as we cannot promote to a common
  // type (true series division is not implemented).
  // NOTE: the use of the old syntax for the enable_if with nullptr is because
  // of a likely GCC bug:
  // https://gcc.gnu.org/bugzilla/show_bug.cgi?id=59366
  // In real.hpp, there are a few uses of operator/= on reals (e.g., in
  // binary_div) *before* the is_zero_impl
  // specialisation for real is declared. These operators are immediately
  // instantiatied when parsed because they are not
  // templated. During the overload resolution of operator/=, the operator
  // defined in this class is considered - which
  // is wrong, as the operators here should be found only via ADL and this class
  // is in no way associated to real. What happens
  // then is that is_zero_impl is instantiated for a real type before the real
  // specialisation is seen, and GCC errors out.
  // I *think* the nullptr syntax works because the bso_type enable_if disables
  // the function before has_is_zero is encountered,
  // or maybe because it does not participate in template deduction.
  template <
      typename T, typename U,
      typename std::enable_if<bso_type<T, U, 3>::value == 4u, int>::type = 0>
  static series_common_type<T, U, 3> dispatch_binary_div(
      T &&x, U &&y,
      typename std::enable_if<
          has_is_zero<typename std::decay<U>::type>::value>::type * = nullptr) {
    ...
    try {
      for (auto it = retval.m_container.begin(); it != it_f;) {
        // NOTE: here the original requirement is that cf / y is defined, but we
        // know
        // that cf / y results in another cf, and we assume always that cf /= y
        // is exactly equivalent
        // to cf = cf / y. And cf must be move-assignable. So this should be
        // possible.
        it->m_cf /= y;
        // NOTE: no need to check for compatibility, as it depends only on the
        // key type and here
  ...
  // NOTE: the trailing decltype() syntax is used here to make sure we can
  // actually call the other overload of the function
  // in the body.
  template <typename T, typename U,
            typename std::enable_if<
                bso_type<T, U, 3>::value == 5u &&
                    std::is_constructible<
                        series_common_type<T, U, 3>,
                        const typename std::decay<T>::type &>::value,
                int>::type = 0>
  static auto dispatch_binary_div(T &&x, U &&y) -> decltype(
      dispatch_binary_div(std::declval<const series_common_type<T, U, 3> &>(),
                          std::forward<U>(y))) {
    series_common_type<T, U, 3> x1(std::forward<T>(x));
    return dispatch_binary_div(std::move(x1), std::forward<U>(y));
  }
  template <typename T, typename... U>
  using binary_div_type = decltype(dispatch_binary_div(
      std::declval<const typename std::decay<T>::type &>(),
      std::declval<const typename std::decay<U>::type &>()...));
  template <
      typename T, typename U,
      typename std::enable_if<
          std::is_assignable<T &, binary_div_type<T, U>>::value, int>::type = 0>
  static T &dispatch_in_place_div(T &x, U &&y) {
    x = dispatch_binary_div(std::move(x), std::forward<U>(y));
    return x;
  }
  template <typename T, typename... U>
  using in_place_div_type = decltype(dispatch_in_place_div(
      std::declval<T &>(),
      std::declval<const typename std::decay<U>::type &>()...));
  template <typename T, typename... U>
  using in_place_div_enabler = typename std::enable_if<
      detail::true_tt<in_place_div_type<T, U...>>::value, int>::type;
  // Equality.
  // Low-level implementation of equality.
  template <typename T> static bool equality_impl(const T &x, const T &y) {
    if (x.size() != y.size()) {
      return false;
    }
  ...
  // NOTE: we are using operator+() on the coefficients to determine type
  // conversions.
  template <
      typename T, typename U,
      typename std::enable_if<bso_type<T, U, 0>::value == 0u, int>::type = 0>
  ...
  template <
      typename T, typename U,
      typename std::enable_if<(bso_type<T, U, 0>::value == 1u ||
                               bso_type<T, U, 0>::value == 4u) &&
  ...
  template <
      typename T, typename U,
      typename std::enable_if<bso_type<T, U, 0>::value == 2u, int>::type = 0>
  static auto dispatch_equality(const T &x, const U &y)
      -> decltype(dispatch_equality(y, x)) {
  ...
  template <typename T, typename... U>
  using eq_type = decltype(dispatch_equality(
      std::declval<const typename std::decay<T>::type &>(),
      std::declval<const typename std::decay<U>::type &>()...));
  template <typename T, typename... U>
  using eq_enabler =
...
public:
  ...
  /**
   * \note
   * This operator is enabled only if the algorithm outlined in
   * piranha::series_operators
   ...
   * @return <tt>x / y</tt>.
   *
   * @throws piranha::zero_division_error if \p x is and empty series and \p y
   * is zero.
   * @throws unspecified any exception thrown by:
   ...
   * is supported by the arguments.
   *
   * Two series are considered equal if they have the same number of terms and
   * all terms in one series
   * appear in the other.
   *
   * @param[in] x first argument.
   * @param[in] y second argument.
...
/// File formats.
/**
 * These are the available formats for piranha::series::save(const Derived &,
 * const std::string &, file_format, file_compression) and
...
/**
 * These are the available compression options for piranha::series::save(const
 * Derived &, const std::string &, file_format, file_compression) and
 * piranha::series::load(const std::string &, file_format, file_compression).
 */
 ...
 *
 * Unless otherwise specified, this class provides the strong exception safety
...
* \todo cast operator would allow to define in-place operators with fundamental
* types as first operand.
* \todo filter and transform can probably take arbitrary functors as input,
* instead of std::function. Just assert the function object's signature.
* \todo transform needs sfinaeing.
* TODO new operators:
* - test with mock_cfs that are not addable to scalars.
* - the unary + and - operators should probably follow the type promotion rules
* for consistency.
*/
...
private:
  ...
  template <typename, typename, typename> friend class series;
  // Make friend with debugging class.
  template <typename> friend class debug_access;
  // Make friend with the operators class.
  friend class series_operators;
  // Partial need access to the custom derivatives.
  template <typename, typename> friend struct math::partial_impl;
...
private:
  ...
  typedef decltype(
      std::declval<container_type>().evaluate_sparsity()) sparsity_info_type;
  // Insertion.
  template <bool Sign, typename T>
  void dispatch_insertion(
      T &&term,
      typename std::enable_if<std::is_same<
          typename std::decay<T>::type, term_type>::value>::type * = nullptr) {
    // Debug checks.
...
#pragma GCC diagnostic pop
  ...
  template <bool Sign, typename T> void insertion_impl(T &&term) {
    // NOTE: here we are basically going to reconstruct hash_set::insert() with
    // the goal
    ...
    if (it == m_container.end()) {
      ...
      // Term is new. Handle the case in which we need to rehash because of load
      // factor.
      ...
      const auto new_it =
          m_container._unique_insert(std::forward<T>(term), bucket_idx);
      ...
      if (!Sign) {
        try {
          ...
          // Run the cleanup function also in case of exceptions, as we do not
          // know
      ...
      piranha_assert(!it->is_ignorable(m_symbol_set) &&
                     it->is_compatible(m_symbol_set));
      try {
  ...
  template <typename T>
  using insert_enabler = typename std::enable_if<
      std::is_same<term_type, typename std::decay<T>::type>::value, int>::type;
  // Terms merging
  // =============
  // NOTE: ideas to improve the algorithm:
  // - optimization when merging with self: add each coefficient to itself,
  // instead of copying and merging.
  // - optimization when merging with series with same bucket size: avoid
  // computing the destination bucket,
  //   as it will be the same as the original.
  template <bool Sign, typename T> void merge_terms_impl0(T &&s) {
    // NOTE: here we can take the pointer to series and compare it to this
    // because we know that
    // series derives from the type of this.
    if (unlikely(&s == this)) {
      // If the two series are the same object, we need to make a copy.
      // NOTE: here we are making sure we are doing a real deep copy (as
      // opposed, say, to a move,
      // which could happen if we used std::forward.).
      merge_terms_impl1<Sign>(series<Cf, Key, Derived>(
          static_cast<const series<Cf, Key, Derived> &>(s)));
    } else {
  ...
  static void swap_for_merge(container_type &&c1, container_type &&c2,
                             bool &swap) {
    piranha_assert(!swap);
    // Do not do anything in case of overflows.
    if (unlikely(c1.size() >
                 std::numeric_limits<size_type>::max() - c2.size())) {
      return;
    }
    ...
    try {
      ...
      max_n_buckets = boost::numeric_cast<size_type>(
          std::trunc(static_cast<double>(max_size) / c1.max_load_factor()));
    } catch (...) {
      // Ignore any error on conversions.
      return;
    }
    // Now we swap the containers, if the first one is not enough to contain the
    // expected number of terms
    // and if the second one is larger.
    if (c1.bucket_count() < max_n_buckets &&
        c2.bucket_count() > c1.bucket_count()) {
  ...
  // This overloads the above in the case we are dealing with two different
  // container types:
  ...
  void merge_terms_impl1(
    ...
    try {
      ...
      // If we swapped the operands and a negative merge was performed, we need
      // to change
  ...
  // Merge all terms from another series. Works if s is this (in which case a
  // copy is made). Basic exception safety guarantee.
  template <bool Sign, typename T>
  void merge_terms(
      T &&s,
      typename std::enable_if<
          is_series<typename std::decay<T>::type>::value>::type * = nullptr) {
    static_assert(std::is_base_of<series<Cf, Key, Derived>,
  ...
  template <typename T, typename U = series,
            typename std::enable_if<
                (series_recursion_index<T>::value <
                 series_recursion_index<U>::value) &&
  ...
  // NOTE: here in principle we could have a type U which is not a series but
  // has a term_type typedef defined,
  // which could incur into a hard error (e.g., via a static_assert in U). Maybe
  // for the future we could think of defining
  // a term_type_t template alias in the spirit of series_rebind, that goes into
  // SFINAE if the argument is not a series.
  // Alternatively, the key_is_convertible and other similar typedefs with
  // internal static asserts could be made sfinae-friendly
  // in a similar way (i.e., don't define the ::value member if the input types
  // are not keys).
  template <typename T, typename U = series,
            typename std::enable_if<
  ...
  void dispatch_generic_constructor(const T &s) {
    ...
    try {
      for (auto it = s.m_container.begin(); it != it_f; ++it) {
        insert<true>(term_type(convert_to<cf_type>(it->m_cf),
                               key_type(it->m_key, m_symbol_set)));
      }
    } catch (...) {
      // In case of any insertion error, zero out this series.
      m_container.clear();
  ...
  // NOTE: here we need to make sure that the generic ctor cannot be preferred
  // over the copy constructor,
  // otherwise we pay a performance penalty. We need to make sure of the
  // following things:
  // - this must *not* be a copy constructor for series - this will prevent
  // automatically-generated copy constructors
  //   lower in the class hierarchy to call it;
  // - if we call explicitly a base() copy constructor lower in the hierarchy,
  // the generic constructor must be excluded
  //   from the overload set.
  // Here, U is the calling series object, T the generic object. The is_base_of
  // check makes sure of the above conditions:
  // - if T is U exactly, then clearly std::is_base_of<U,T>::value is true;
  // - if T comes from lower in the hierarchy, then clearly
  // std::is_base_of<U,T>::value is true again.
  // These aspects are tested in series_03.
  template <typename T, typename U>
  using generic_ctor_enabler = typename std::enable_if<
      detail::true_tt<decltype(std::declval<U &>().dispatch_generic_constructor(
          std::declval<const T &>()))>::value &&
          !std::is_base_of<U, T>::value,
      int>::type;
  // Enabler for is_identical.
  template <typename T>
  using is_identical_enabler =
      typename std::enable_if<is_equality_comparable<T>::value, int>::type;
  // Iterator utilities.
  typedef boost::transform_iterator<
      std::function<std::pair<typename term_type::cf_type, Derived>(
          const term_type &)>,
      typename container_type::const_iterator>
      const_iterator_impl;
  // Evaluation utilities.
  // NOTE: here we need the Series template because otherwise, in case of
  // missing eval on key or coefficient, the decltype
  // will fail as series is not a template parameter.
  template <typename Series, typename U, typename = void> struct eval_type_ {};
  template <typename Series, typename U>
  using e_type = decltype(
      math::evaluate(
          std::declval<typename Series::term_type::cf_type const &>(),
  ...
  static std::ostream &print_helper(std::ostream &os, Iterator start,
                                    Iterator end, const symbol_set &args) {
    ...
    auto str = oss.str();
    // If we reached the limit without printing all terms in the series, print
    // the ellipsis.
  ...
  Derived merge_arguments(const symbol_set &new_ss) const {
    ...
    piranha_assert(std::includes(new_ss.begin(), new_ss.end(),
                                 m_symbol_set.begin(), m_symbol_set.end()));
    using cf_type = typename term_type::cf_type;
    using key_type = typename term_type::key_type;
    Derived retval;
  ...
  template <typename T>
  static T
  trim_cf_impl(const T &s,
               typename std::enable_if<is_series<T>::value>::type * = nullptr) {
    return s.trim();
  }
  template <typename T>
  static const T &trim_cf_impl(
      const T &s,
      typename std::enable_if<!is_series<T>::value>::type * = nullptr) {
    return s;
  }
  ...
  using cf_diff_type =
      decltype(math::partial(std::declval<const Cf2 &>(), std::string()));
  ...
  using key_diff_type =
      decltype(std::declval<const Key2 &>()
                   .partial(std::declval<const symbol_set::positions &>(),
                            std::declval<const symbol_set &>())
                   .first);
  // Shortcuts to get cf/key from series.
  template <typename Series> using cf_t = typename Series::term_type::cf_type;
  template <typename Series> using key_t = typename Series::term_type::key_type;
  // We have different implementations, depending on whether the computed
  // derivative type is the same as the original one
  // (in which case we will use faster term insertions) or not (in which case we
  // resort to series arithmetics). Adopt the usual scheme
  // of providing an unspecialised value that sfinaes out if the enabler
  // condition is malformed, and two specialisations
  // that implement the logic above.
  template <typename Series, typename = void> struct partial_type_ {};
// NOTE: the enabler conditions below are one the negation of the other, not
// sure if it is possible to avoid repetition without macro as we need
...
#define PIRANHA_SERIES_PARTIAL_ENABLER                                         \
  (std::is_same<cf_diff_type<cf_t<Series>>, cf_t<Series>>::value &&            \
   std::is_same<decltype(                                                      \
                    std::declval<const cf_t<Series> &>() *                     \
                    std::declval<const key_diff_type<key_t<Series>> &>()),     \
                cf_t<Series>>::value)
  template <typename Series>
  struct partial_type_<
      Series, typename std::enable_if<PIRANHA_SERIES_PARTIAL_ENABLER>::type> {
    using type = Series;
  ...
  using partial_type_1 =
      decltype(std::declval<const cf_diff_type<cf_t<Series>> &>() *
                   std::declval<const Series &>() +
               std::declval<const cf_t<Series> &>() *
                   std::declval<const key_diff_type<key_t<Series>> &>() *
                   std::declval<const Series &>());
  // NOTE: in addition to check that we cannot use the optimised algorithm, we
  // must also check that the resulting type
  // is constructible from int (for the accumulation of retval) and that it is
  // addable in-place.
  template <typename Series>
  struct partial_type_<
      Series,
      typename std::enable_if<
          !PIRANHA_SERIES_PARTIAL_ENABLER &&
...
#undef PIRANHA_SERIES_PARTIAL_ENABLER
  // The final typedef.
  template <typename Series>
  using partial_type = typename std::enable_if<
      is_returnable<typename partial_type_<Series>::type>::value,
      typename partial_type_<Series>::type>::type;
  // The implementation of the two partial() algorithms.
  template <
      typename Series = Derived,
      typename std::enable_if<partial_type_<Series>::algo == 0, int>::type = 0>
  partial_type<Series> partial_impl(const std::string &name) const {
    static_assert(std::is_same<Derived, partial_type<Series>>::value,
                  "Invalid type.");
    // This is the faster algorithm.
    Derived retval;
    retval.m_symbol_set = this->m_symbol_set;
    const auto it_f = this->m_container.end();
    const symbol_set::positions p(retval.m_symbol_set,
                                  symbol_set{symbol(name)});
    for (auto it = this->m_container.begin(); it != it_f; ++it) {
      // NOTE: here t0 cannot be incompatible as it->m_key is coming from a
      // series
      ...
      // Here we are assuming that the partial of the key returns always a
      // compatible key,
  ...
  template <
      typename Series = Derived,
      typename std::enable_if<partial_type_<Series>::algo == 1, int>::type = 0>
  partial_type<Series> partial_impl(const std::string &name) const {
  ...
  // Partial derivative of a series type, as defined by math::partial(). Note
  // that here we cannot use
  // partial_type as Derived series might override the partial() method with
  // their own implementation.
  template <typename Series>
  using series_p_type = decltype(math::partial(
      std::declval<const Series &>(), std::declval<const std::string &>()));
  template <typename Series>
  using cp_map_type =
      std::unordered_map<std::string,
                         std::function<series_p_type<Series>(const Derived &)>>;
  // NOTE: here the initialisation of the static variable inside the body of the
  // function
  // is guaranteed to be thread-safe. It should not matter too much as we always
  // protect the access to it.
  ...
  using custom_partial_enabler = typename std::enable_if<
      std::is_constructible<
          std::function<series_p_type<Series>(const Derived &)>, F>::value,
      int>::type;
  ...
  // The type resulting from the exponentiation of the coefficient of a series U
  // to the power of T.
  template <typename T, typename U>
  using pow_cf_type =
      decltype(math::pow(std::declval<const typename U::term_type::cf_type &>(),
                         std::declval<const T &>()));
  ...
  using pow_expo_checks =
      typename std::enable_if<has_is_zero<T>::value &&
                              has_safe_cast<integer, T>::value>::type;
  // Hashing utils for series.
  struct series_hasher {
    template <typename T> std::size_t operator()(const T &s) const {
      return s.hash();
    }
  };
  ...
  template <typename Series>
  using pow_map_type =
      std::unordered_map<Series, std::vector<pow_m_type<Series>>, series_hasher,
                         series_equal_to>;
  // NOTE: here, as in the custom derivative machinery, we need to pass through
  // a static function
  // to get the cache because Derived is an incomplete type and we cannot thus
  // use a static data member
  // involving Derived in series. Also, we need the Series template argument to
  ...
  // Empty for sfinae.
  template <typename T, typename U, typename = void> struct pow_ret_type_ {};
  // Case 0: the exponentiation of the coefficient does not change its type.
  template <typename T, typename U>
  struct pow_ret_type_<
      T, U, typename std::enable_if<
                std::is_same<pow_cf_type<T, U>,
                             typename U::term_type::cf_type>::value &&
...
public:
  /// Size type.
  /**
   * Used to represent the number of terms in the series. Equivalent to
   * piranha::hash_set::size_type.
   */
  typedef typename container_type::size_type size_type;
  /// Const iterator.
  /**
   * Iterator type that can be used to iterate over the terms of the series.
   * The object returned upon dereferentiation is an \p std::pair in which the
   * first element
   * is a copy of the coefficient of the term, the second element a single-term
   * instance of \p Derived constructed from
   * the term's key and a unitary coefficient.
   *
   * This iterator is an input iterator which additionally offers the multi-pass
   * guarantee.
   *
  ...
  /**
   * @throws unspecified any exception thrown by the copy constructor of
   * piranha::hash_set.
   */
  ...
  /**
   * \note
   * This constructor is enabled only if \p T does not derive from the calling
   * piranha::series instance and
   ...
   *   the symbol set of \p x is copied into \p this and all terms from \p x are
   * inserted into \p this.
   *   The terms of \p x are converted to \p term_type via the binary
   * constructor of piranha::term,
   *   and the keys of \p x are converted to the key type of \p term_type via a
   * converting constructor,
   *   if available (see piranha::key_is_convertible);
   * - else, if the recursion index of \p T is less than the recursion index of
   * \p this:
   ...
   * - the construction of a coefficient from \p x or of a key from
   * piranha::symbol_set,
  ...
  /**
   * \note
   * This operator is enabled only if the corresponding generic constructor from
   * \p x is enabled.
   *
   * Generic assignment is equivalent to assignment to a piranha::series
   * constructed
  ...
  series &operator=(const T &x) {
    static_assert(!std::is_base_of<series, T>::value,
                  "Generic assignment should not be enabled with "
  ...
  /**
   * A series is considered to be <em>single-coefficient</em> when it is
   * symbolically equivalent to a coefficient.
   * That is, the series is either empty (in which case it is considered to be
   * equivalent to a coefficient constructed
   * from zero) or consisting of a single term with unitary key (in which case
   * the series is considered equivalent to
   * its only coefficient).
   *
   * @return \p true in case of single-coefficient series, \p false otherwise.
   *
   * @throws unspecified any exception thrown by the <tt>is_unitary()</tt>
   * method of the key type.
   */
  bool is_single_coefficient() const {
    return (empty() || (size() == 1u &&
                        m_container.begin()->m_key.is_unitary(m_symbol_set)));
  }
  /// Insert term.
  /**
   * \note
   * This method is enabled only if the decay type of \p T is
   * piranha::series::term_type.
   *
   * This method will insert \p term into the series using internally
   * piranha::hash_set::insert.
   *
   * The insertion algorithm proceeds as follows:
   * - if the term is not compatible for insertion, an \p std::invalid_argument
   * exception is thrown;
   * - if the term is ignorable, the method will return without performing any
   * insertion;
   * - if the term is already in the series, then:
   *   - its coefficient is added (if \p Sign is \p true) or subtracted (if \p
   * Sign is \p false)
   *     to the existing term's coefficient;
   *   - if, after the addition/subtraction the existing term is ignorable, it
   * will be erased;
   * - else:
   *   - the term is inserted into the term container and, if \p Sign is \p
   * false, its coefficient is negated.
   *
   * After any modification to an existing term in the series (e.g., via
   * insertion with negative \p Sign or via in-place addition
   * or subtraction of existing coefficients), the term will be checked again
   * for compatibility and ignorability, and, in case
   * the term has become incompatible or ignorable, it will be erased from the
   * series.
   *
   * The exception safety guarantee upon insertion is that the series will be
   * left in an undefined but valid state. Such a guarantee
   * relies on the fact that the addition/subtraction and negation methods of
   * the coefficient type will leave the coefficient in a valid
   ...
   * - piranha::hash_set::find(),
   * - piranha::hash_set::erase(),
   * - piranha::math::negate(), in-place addition/subtraction on coefficient
   * types.
   ...
   * piranha::series::term_type.
   *
   * Convenience wrapper for the generic insert() method, with \p Sign set to \p
   * true.
   *
   * @param[in] term term to be inserted.
   *
   ...
   */
  template <typename T, insert_enabler<T> = 0> void insert(T &&term) {
  ...
  /**
   * This method will call math::negate() on the coefficients of all terms. In
   * case of exceptions,
   * the basic exception safety guarantee is provided.
   *
   * If any term becomes ignorable or incompatible after negation, it will be
   * erased from the series.
   *
   * @throws unspecified any exception thrown by math::negate() on the
   * coefficient type.
   */
  void negate() {
    try {
   ...
   * Methods to query the properties of the internal container used to store the
   * terms.
   */
  //@{
  /// Table sparsity.
  /**
   * Will call piranha::hash_set::evaluate_sparsity() on the internal terms
   * container
   ...
   *
   * @throws unspecified any exception thrown by
   * piranha::hash_set::evaluate_sparsity().
   */
   ...
   */
  double table_load_factor() const { return m_container.load_factor(); }
  /// Table bucket count.
  /**
   * @return the bucket count of the internal container.
   */
  size_type table_bucket_count() const { return m_container.bucket_count(); }
  //@}
  /// Exponentiation.
  /**
   * \note
   * This method is enabled only if the algorithm outlined here is supported by
   * the involved types.
   *
   * Return \p this raised to the <tt>x</tt>-th power. The type of the result is
   * either the calling series type,
   * or the calling series type rebound to the type resulting from the
   * exponentiation of the coefficient of the calling
   * type to the power of \p x. The exponentiation algorithm proceeds as
   * follows:
   * - if the series is single-coefficient, the result is a single-coefficient
   * series in which the coefficient
   *   is the original coefficient (or zero, if the calling series is empty)
   * raised to the power of \p x;
   * - if \p x is zero (as established by piranha::math::is_zero()), a series
   * with a single term
   *   with unitary key and coefficient constructed from the integer numeral "1"
   * is returned (i.e., any series raised to
   *   the power of zero is 1 - including empty series);
   * - if \p x represents a non-negative integral value, the return value is
   * constructed via repeated multiplications;
   * - otherwise, an exception will be raised.
   *
   * An internal thread-safe cache of natural powers of series is maintained in
   * order to improve performance during, e.g., substitution operations.
   ...
   *
   * @throws std::invalid_argument if exponentiation is computed via repeated
   * series multiplications and
  ...
  pow_ret_type<T, U> pow(const T &x) const {
    ...
    if (is_single_coefficient()) {
      ...
      if (empty()) {
        ...
        retval.insert(
            r_term_type(math::pow(cf_type(0), x), key_type(symbol_set{})));
      } else {
        retval.insert(r_term_type(math::pow(m_container.begin()->m_cf, x),
                                  key_type(symbol_set{})));
      }
      return retval;
    }
    ...
    if (n.sign() < 0) {
      piranha_throw(std::invalid_argument, "invalid argument for series "
                                           "exponentiation: negative integral "
                                           "value");
    }
    ...
    while (v.size() <= n) {
      // NOTE: for series it seems like it is better to run the dumb algorithm
      // instead of, e.g.,
      // exponentiation by squaring - the growth in number of terms seems to be
      // slower.
  ...
  /**
   * This method can be used to clear the cache of natural powers of series
   * maintained by piranha::series::pow().
   *
  ...
  /**
   * \note
   * This method is enabled only if the coefficient and key are differentiable
   * (i.e., they satisfy the piranha::is_differentiable
   * and piranha::key_is_differentiable type traits), and if the arithmetic
   * operations
   * needed to compute the partial derivative are supported by all the involved
   * types.
   *
   * This method will return the partial derivative of \p this with respect to
   * the variable called \p name. The method will construct
   * the return value from the output of the differentiation methods of
   * coefficient and key, and via arithmetic and/or term insertion
   * operations.
   *
   * Note that, contrary to the specialisation of piranha::math::partial() for
   * series types, this method will not take into account
   * custom derivatives registered via
   * piranha::series::register_custom_derivative().
   *
   * @param[in] name name of the argument with respect to which the derivative
   * will be calculated.
   *
   * @return partial derivative of \p this with respect to the symbol named \p
   * name.
   *
   * @throws unspecified any exception thrown by:
  ...
  /**
   * \note
   * This method is enabled only if piranha::series::partial() is enabled for \p
   * Derived, and if \p F
   * is a type that can be used to construct
   * <tt>std::function<partial_type(const Derived &)</tt>, where
   * \p partial_type is the type resulting from the partial derivative of \p
   * Derived.
   *
   * Register a copy of a callable \p func associated to the symbol called \p
   * name for use by piranha::math::partial().
   * \p func will be used to compute the partial derivative of instances of type
   * \p Derived with respect to
   * \p name in place of the default partial differentiation algorithm.
   *
   * It is safe to call this method from multiple threads.
   *
   * @param[in] name symbol for which the custom partial derivative function
   * will be registered.
   ...
   */
  template <typename F, typename Series = Derived,
            custom_partial_enabler<F, Series> = 0>
   ...
   * Derived.
   *
   * Unregister the custom partial derivative function associated to the symbol
   * called \p name. If no custom
   * partial derivative was previously registered using
   * register_custom_derivative(), calling this function will be a no-op.
   *
   * It is safe to call this method from multiple threads.
   *
   * @param[in] name symbol for which the custom partial derivative function
   ...
   */
  // NOTE: the additional template parameter is to disable the method in case
  // the partial
  // type is malformed.
   ...
   * Derived.
   *
   * Will unregister all custom derivatives currently registered via
   * register_custom_derivative().
   * It is safe to call this method from multiple threads.
   *
   * @throws unspecified any exception thrown by failure(s) in threading
   * primitives.
   */
  template <typename Series = Derived, typename Partial = partial_type<Series>>
  ...
  /// Begin iterator.
  /**
   * Return an iterator to the first term of the series. The returned iterator
   * will
   * provide, when dereferenced, an \p std::pair in which the first element is a
   * copy of the coefficient of
   * the term, whereas the second element is a single-term instance of \p
   * Derived built from the term's key
   * and a unitary coefficient.
   *
   * Note that terms are stored unordered in the series, hence it is not defined
   * which particular
   * term will be returned by calling this method. The only guarantee is that
   * the iterator can be used to transverse
   * all the series' terms until piranha::series::end() is eventually reached.
   *
   * Calling any non-const method on the series will invalidate the iterators
   * obtained via piranha::series::begin() and piranha::series::end().
   *
  ...
  const_iterator begin() const {
    typedef std::function<std::pair<typename term_type::cf_type, Derived>(
        const term_type &)>
        func_type;
    auto func = [this](const term_type &t) {
      return detail::pair_from_term<term_type, Derived>(this->m_symbol_set, t);
    };
    return boost::make_transform_iterator(m_container.begin(),
                                          func_type(std::move(func)));
  }
  /// End iterator.
  /**
   * Return an iterator one past the last term of the series. See the
   * documentation of piranha::series::begin()
  ...
  const_iterator end() const {
    ...
    return boost::make_transform_iterator(m_container.end(),
                                          func_type(std::move(func)));
  }
  /// Term filtering.
  /**
   * This method will apply the functor \p func to each term in the series, and
   * produce a return series
   * containing all terms in \p this for which \p func returns \p true.
   * Terms are passed to \p func in the format resulting from dereferencing the
   * iterators obtained
   ...
   */
  Derived filter(std::function<
                 bool(const std::pair<typename term_type::cf_type, Derived> &)>
                     func) const {
    Derived retval;
   ...
   * This method will apply the functor \p func to each term in the series, and
   * will use the return
   * value of the functor to construct a new series. Terms are passed to \p func
   * in the same format
   * resulting from dereferencing the iterators obtained via
   * piranha::series::begin(), and \p func is expected to produce
   * a return value of the same type.
   *
   * The return series is first initialised as an empty series. For each input
   * term \p t, the return value
   * of \p func is used to construct a new temporary series from the
   * multiplication of \p t.first and
   * \p t.second. Each temporary series is then added to the return value
   * series.
   *
   ...
   */
  // TODO require multipliability of cf * Derived and addability of the result
  // to Derived in place.
  Derived
   ...
   * - the evaluated types are suitable for use in
   * piranha::math::multiply_accumulate(),
   * - the return type is constructible from \p int and it satisfies
   * piranha::is_returnable.
   *
   * Series evaluation starts with a zero-initialised instance of the return
   * type, which is determined
   * according to the evaluation types of coefficient and key. The return value
   * accumulates the evaluation
   * of all terms in the series via the product of the evaluations of the
   * coefficient-key pairs in each term.
   * The input dictionary \p dict specifies with which value each symbolic
   * quantity will be evaluated.
   *
   * @param[in] dict dictionary of that will be used for evaluation.
   *
   * @return evaluation of the series according to the evaluation dictionary \p
   * dict.
   *
   * @throws unspecified any exception thrown by:
  ...
  eval_type<Series, T>
  evaluate(const std::unordered_map<std::string, T> &dict) const {
    // NOTE: possible improvement: if the evaluation type is less-than
    // comparable,
    // build a vector of evaluated terms, sort it and accumulate (to minimise
    // accuracy loss
    ...
    // First we iterate over all elements of pmap (which could be fewer than the
    // symbols).
    for (const auto &p : pmap) {
      if (unlikely(i != p.first)) {
        piranha_throw(
            std::invalid_argument,
            "the symbol '" + this->m_symbol_set[i].get_name() +
                "' is missing from the series evaluation dictionary'");
      }
      ++i;
    }
    // It could still happen that pmap is missing symbols at the tail of the
    // symbol set.
    ...
    for (const auto &t : this->m_container) {
      math::multiply_accumulate(retval, math::evaluate(t.m_cf, dict),
                                t.m_key.evaluate(pmap, m_symbol_set));
    }
    return retval;
  }
  /// Trim.
  /**
   * This method will return a series mathematically equivalent to \p this in
   * which discardable arguments
   * have been removed from the internal set of symbols. Which symbols are
   * removed depends on the trimming
   * method \p trim_identify() of the key type (e.g., in a polynomial a symbol
   * can be discarded if its exponent
   * is zero in all monomials).
   *
   * If the coefficient type is an instance of piranha::series, trim() will be
   * called recursively on the coefficients
  ...
  Derived trim() const {
    ...
    for (auto it = this->m_container.begin(); it != it_f; ++it) {
      retval.insert(term_type(trim_cf_impl(it->m_cf),
                              it->m_key.trim(trim_ss, m_symbol_set)));
    }
    return retval;
  }
  /// Print in TeX mode.
  /**
   * Print series to stream \p os in TeX mode. The representation is constructed
   * in the same way as explained in
   * piranha::series::operator<<(), but using piranha::print_tex_coefficient()
   * and the key's TeX printing method instead of the plain
   * printing functions.
   *
  ...
  void print_tex(std::ostream &os) const {
    ...
    print_helper<true>(os, m_container.begin(), m_container.end(),
                       m_symbol_set);
  }
   ...
   *   the former via the piranha::print_coefficient() function, the latter via
   * its <tt>print()</tt> method;
   * - terms are separated by a "+" sign.
   *
   * The following additional transformations take place on the printed output:
   *
   * - if the printed output of a coefficient is the string "1" and the printed
   * output of its key
   ...
   * "-" and the sign "*" is not printed;
   * - if the key output is empty, the sign "*" is not printed;
   * - the sequence of characters "+-" is transformed into "-";
   * - at most piranha::settings::get_max_term_output() terms are printed, and
   * terms in excess are
   *   represented with ellipsis "..." at the end of the output; if
   * piranha::settings::get_max_term_output()
  ...
  friend std::ostream &operator<<(std::ostream &os, const series &s) {
    ...
    return print_helper<false>(os, s.m_container.begin(), s.m_container.end(),
                               s.m_symbol_set);
  }
  /// Hash value.
  /**
   * The hash value for a series is zero if the series is empty, otherwise it is
   * computed by adding the hash values
   * of all terms. This ensures that two identical series in which the terms are
   * stored in different order still produce
   * the same hash value.
   *
   * Note, however, that the arguments of the series are not considered in the
   * hash value and that,
   * in general, two series that compare equal according to operator==() will
   * **not** have the same hash value
   * (as the equality operator merges the arguments of two series before
   * actually performing the comparison). Instead
   * of operator==(), is_identical() should be used for storing series as keys
   ...
   */
  // NOTE: hash and is_identical must always be considered together. E.g., two
  // series can be identical even in case
  // of coefficient series which are not identical - but this does not matter,
  // as is_identical implies strict equivalence
  // of the keys and hash considers only the keys. If we wanted to consider also
  // the coefficients for hashing, then we need
   ...
   *
   * This method will return \p true if the symbol sets of \p this and \p other
   * are the same,
   ...
   *
   * @throws unspecified any exception thrown by the comparison operator of \p
   * Derived.
   */
  template <typename T = Derived, is_identical_enabler<T> = 0>
  bool is_identical(const Derived &other) const {
    return m_symbol_set == other.m_symbol_set &&
           other == *static_cast<Derived const *>(this);
  }
  /// Symbol set getter.
  /**
   * @return const reference to the piranha::symbol_set associated to the
   * series.
   */
  const symbol_set &get_symbol_set() const { return m_symbol_set; }
  /// Symbol set setter.
  /**
   ...
   * @throws unspecified any exception thrown by the copy assignment operator of
   * piranha::symbol_set.
   */
  ...
  /// Extend symbol set.
  /**
   * This method will return a series whose symbol set will be \p new_ss and
   * whose terms
   * will be the terms of \p this with new keys obtained from calling the
   * existing keys'
   * <tt>merge_args()</tt> method. That is, the new keys will be compatible with
   * the
   * new extended symbol set \p new_ss.
   *
   * \p new_ss must have a size equal to or greater than the size of the current
   * symbol set of \p this,
   ...
   *
   * @throws std::invalid_argument if \p new_ss is smaller than the current
   * symbol set or if
  ...
  Derived extend_symbol_set(const symbol_set &new_ss) const {
    ...
    if (unlikely(!(new_ss.size() > m_symbol_set.size()) ||
                 !std::includes(new_ss.begin(), new_ss.end(),
                                m_symbol_set.begin(), m_symbol_set.end()))) {
      piranha_throw(
          std::invalid_argument,
          "invalid symbol set passed to extend_symbol_set(): the new "
  ...
  /// Save series to file.
  /**
   * This static method will save the input series \p s to the file named \p
   * filename, using the file format \p f
   * and the compression method \p c. The possible values for \p f are listed in
   * piranha::file_format, the possible
   * values for \p c in piranha::file_compression. The output file \p filename,
   ...
   * @throws unspecified any exception thrown by the serialization and/or
   * compression of \p s.
   */
  static void save(const Derived &s, const std::string &filename, file_format f,
                   file_compression c) {
    std::ofstream ofile(
        filename, (f == file_format::binary)
                      ? (std::ios::out | std::ios::binary | std::ios::trunc)
                      : (std::ios::out | std::ios::trunc));
    if (unlikely(!ofile.good())) {
      piranha_throw(std::runtime_error,
                    std::string("file '") + filename + "' could not be opened");
    }
  ...
  /**
   * This static method will load the series contained in the file named \p
   * filename, using the file format \p f
   * and the compression method \p c. The possible values for \p f are listed in
   * piranha::file_format, the possible
   ...
   *
   * @throws std::runtime_error if the output file cannot be opened.
   * @throws unspecified any exception thrown by the deserialization and/or
   * decompression of the series stored in \p filename.
   */
  static Derived load(const std::string &filename, file_format f,
                      file_compression c) {
    std::ifstream ifile(filename, (f == file_format::binary)
...
/**
 * This specialisation is enabled if \p Series is an instance of
 * piranha::series.
 */
template <typename Series>
struct print_coefficient_impl<
    Series, typename std::enable_if<is_series<Series>::value>::type> {
  /// Call operator.
  /**
   * Equivalent to the stream operator overload of piranha::series, apart from a
   * couple
   * of parentheses '()' enclosing the coefficient series if its size is larger
   * than 1.
   *
   * @param[in] os target stream.
   * @param[in] s coefficient series to be printed.
   *
   * @throws unspecified any exception thrown by the stream operator overload of
   * piranha::series.
   */
...
struct negate_impl<T, typename std::enable_if<is_series<T>::value>::type> {
   ...
   */
  template <typename U> void operator()(U &s) const { s.negate(); }
};

/// Specialisation of the piranha::math::is_zero() functor for piranha::series.
/**
 * This specialisation is activated when \p Series is an instance of
 * piranha::series.
 * The result will be computed via the series' <tt>empty()</tt> method.
 */
template <typename Series>
struct is_zero_impl<Series,
                    typename std::enable_if<is_series<Series>::value>::type> {
  /// Call operator.
  /**
   ...
   */
  bool operator()(const Series &s) const { return s.empty(); }
};
}
...
using pow_series_enabler =
    typename std::enable_if<is_series<Series>::value &&
                            true_tt<decltype(std::declval<const Series &>().pow(
...
struct pow_impl<Series, T, detail::pow_series_enabler<Series, T>> {
   ...
   *
   * @throws unspecified any exception resulting from the series' <tt>pow()</tt>
   * method.
   */
...
template <typename T> class series_has_invert : sfinae_types {
  template <typename U>
...
public:
  static const bool value =
      std::is_same<decltype(test(std::declval<T>())), yes>::value;
};

// 1. Inversion via the custom method.
template <typename T, typename std::enable_if<is_series<T>::value &&
                                                  series_has_invert<T>::value,
                                              int>::type = 0>
...
// 2. Coefficient type supports math::invert(), with return type equal to
// coefficient type.
// NOTE: here we are passing a series<> parameter: this will require a
// conversion from whatever series type (Derived) to its series<> base,
// and thus this overload will never conflict with the overload 1., even if the
// series that we pass in actually has the invert method.
// This is useful because we can now force the use of this "base" implementation
// by simply casting a series to its base type.
// This is used, e.g., in the sin/cos overrides for poisson_series, and it is
// similar to what it is done for the pow() overrides.
template <
        ...
        is_series<Derived>::value &&
            std::is_same<typename Derived::term_type::cf_type,
                         decltype(math::invert(
                             std::declval<const typename Derived::term_type::
                                              cf_type &>()))>::value,
        int>::type = 0>
...
// 3. coefficient type supports math::invert() with a result different from the
// original coefficient type and the series can be rebound to this new type.
template <
    typename Cf, typename Key, typename Derived,
    typename std::enable_if<
        is_series<Derived>::value &&
            !std::is_same<typename Derived::term_type::cf_type,
                          decltype(math::invert(
                              std::declval<const typename Derived::term_type::
                                               cf_type &>()))>::value,
        int>::type = 0>
inline series_rebind<
    Derived, decltype(math::invert(
                 std::declval<const typename Derived::term_type::cf_type &>()))>
series_invert_impl(const series<Cf, Key, Derived> &s) {
  using ret_type = series_rebind<
      Derived,
      decltype(math::invert(
          std::declval<const typename Derived::term_type::cf_type &>()))>;
...
template <typename T>
using series_invert_enabler = typename std::enable_if<true_tt<decltype(
    series_invert_impl(std::declval<const T &>()))>::value>::type;
}
 ...
 * - the series' coefficient type \p Cf supports math::invert() yielding a type
 * \p T and either
 *   \p T is the same as \p Cf, or the series type can be rebound to the type \p
 * T.
 */
template <typename Series>
...
template <typename T> class series_has_sin : sfinae_types {
  template <typename U> static auto test(const U &t) -> decltype(t.sin());
  static no test(...);

public:
  static const bool value =
      is_returnable<decltype(test(std::declval<T>()))>::value;
};
...
template <typename T,
          typename std::enable_if<
              is_series<T>::value && series_has_sin<T>::value, int>::type = 0>
...
// 2. coefficient type supports math::sin() with a result equal to the original
// coefficient type.
// NOTE: this overload and the one below do not conflict with the one above
// because it takes a series
// as input argument: when used on a concrete series type, it will have to go
// through a to-base
...
// 3. coefficient type supports math::sin() with a result different from the
// original coefficient type and the series can be rebound to this new type.
template <
    typename Cf, typename Key, typename Derived,
    typename std::enable_if<
...
template <typename T>
using series_sin_enabler = typename std::enable_if<
    true_tt<decltype(series_sin_impl(std::declval<const T &>()))>::value>::type;

// All of the above, but for cos().
template <typename T> class series_has_cos : sfinae_types {
  template <typename U> static auto test(const U &t) -> decltype(t.cos());
  static no test(...);
...
template <typename T,
          typename std::enable_if<
              is_series<T>::value && series_has_cos<T>::value, int>::type = 0>
...
template <typename T>
using series_cos_enabler = typename std::enable_if<
    true_tt<decltype(series_cos_impl(std::declval<const T &>()))>::value>::type;
}
 ...
 * This specialisation is activated when \p Series is an instance of
 * piranha::series and:
 * - either the series type provides a const <tt>%sin()</tt> method returning a
 * type which satisfies
 *   piranha::is_returnable, or, missing this method,
 * - the series' coefficient type \p Cf supports math::sin() yielding a type \p
 * T and either
 *   \p T is the same as \p Cf, or the series type can be rebound to the type \p
 * T.
 */
...
struct sin_impl<Series, detail::series_sin_enabler<Series>> {
   ...
   */
  auto operator()(const Series &s) const
      -> decltype(detail::series_sin_impl(s)) {
 ...
 * This specialisation is activated when \p Series is an instance of
 * piranha::series and:
 * - either the series type provides a const <tt>%cos()</tt> method returning a
 * type which satisfies
 *   piranha::is_returnable, or, missing this method,
 * - the series' coefficient type \p Cf supports math::cos() yielding a type \p
 * T and either
 *   \p T is the same as \p Cf, or the series type can be rebound to the type \p
 * T.
 */
...
struct cos_impl<Series, detail::series_cos_enabler<Series>> {
   ...
   */
  auto operator()(const Series &s) const
      -> decltype(detail::series_cos_impl(s)) {
...
template <typename Series>
using series_partial_enabler = typename std::enable_if<
    is_series<Series>::value &&
    is_returnable<decltype(std::declval<const Series &>().partial(
        std::declval<const std::string &>()))>::value>::type;

// Enabler for the integrate() specialisation: type needs to be a series which
// supports the integration method.
template <typename Series>
using series_integrate_enabler = typename std::enable_if<
    is_series<Series>::value &&
...
/**
 * This specialisation is activated when \p Series is an instance of
 * piranha::series with a const \p partial() method
 * method taking a const <tt>std::string</tt> as parameter and returning a type
 * which satisfies piranha::is_returnable.
 */
template <typename Series>
struct partial_impl<Series, detail::series_partial_enabler<Series>> {
  /// Call operator.
  /**
   * The call operator will first check whether a custom partial derivative for
   * \p Series was registered
   * via piranha::series::register_custom_derivative(). In such a case, the
   * custom derivative function will be used
   * to compute the return value. Otherwise, the output of
   * piranha::series::partial() will be returned.
   *
   * @param[in] s input series.
   * @param[in] name name of the argument with respect to which the
   * differentiation will be calculated.
   *
   ...
   * - the copy assignment and call operators of the registered custom partial
   * derivative function.
   */
  template <typename T>
  auto operator()(const T &s, const std::string &name) const
      -> decltype(s.partial(name)) {
    using partial_type = decltype(s.partial(name));
    bool custom = false;
    std::function<partial_type(const T &)> func;
    // Try to locate a custom partial derivative and copy it into func, if
    // found.
    {
...
/**
 * This specialisation is activated when \p Series is an instance of
 * piranha::series with a const \p integrate()
 * method taking a const <tt>std::string</tt> as parameter and returning a type
 * which satisfies piranha::is_returnable.
 */
template <typename Series>
struct integrate_impl<Series, detail::series_integrate_enabler<Series>> {
   ...
   * @param[in] s input series.
   * @param[in] name name of the argument with respect to which the
   * antiderivative will be calculated.
   *
...
public:
  ...
  template <typename U>
  eval_type<U>
  operator()(const Series &s,
             const std::unordered_map<std::string, U> &dict) const {
    return s.evaluate(dict);
  }
};
}

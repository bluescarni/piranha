...
template <typename = int> struct real_base {
  ...
  // The precision is the number of bits used to represent the significand of a
  // floating-point number.
  // This default value is equivalent to the IEEE 754 quadruple-precision binary
  // floating-point format.
  static const ::mpfr_prec_t default_prec = 113;
};

template <typename T> const ::mpfr_rnd_t real_base<T>::default_rnd;

template <typename T> const ::mpfr_prec_t real_base<T>::default_prec;

// Types interoperable with real.
template <typename T> struct is_real_interoperable_type {
  static const bool value =
      detail::is_mp_integer_interoperable_type<T>::value ||
...
/**
 * This class represents floating-point ("real") numbers of arbitrary size
 * (i.e., the size is limited only by the available memory).
 * The implementation consists of a C++ wrapper around the \p mpfr_t type from
 * the multiprecision MPFR library. Real numbers
 * are represented in binary format and they consist of an arbitrary-size
 * significand coupled to a fixed-size exponent.
 *
 * Unless noted otherwise, this implementation always uses the \p MPFR_RNDN
 * (round to nearest) rounding mode for all operations.
 *
 * ## Interoperability with other types ##
 *
 * This class interoperates with the same types as piranha::mp_integer and
 * piranha::mp_rational,
 ...
 * In case of memory allocation errors by GMP/MPFR routines, the program will
 * terminate.
 *
 * ## Move semantics ##
 *
 * Move construction and move assignment will leave the moved-from object in a
 * state that is destructible and assignable.
 *
 * ## Serialization ##
 *
...
// - maybe we should have a setter as well for the global default precision. It
// would need to be an atomic
//   variable, and we need perf measures to understand the performance impact of
//   this.
// - For series evaluation, we need to be careful performance-wise with the
// possible conversions that might go
//   on when mixing real with other types. E.g., pow(real,int) when evaluating
//   polynomials. We need to make sure
//   the conversions are as fast as possible.
// - At the moment, this class is technically not sortable because moved-from
// reals cannot be compared. For use in std::sort,
...
class real : public detail::real_base<> {
  ...
  // The idea here is that we use the largest integral and fp types supported by
  // the MPFR api for construction,
  ...
  template <typename T, typename std::enable_if<detail::is_mp_integer<T>::value,
                                                int>::type = 0>
  ...
  void assign_from_string(const char *str) {
    ...
    const int retval = ::mpfr_set_str(m_value, str, 10, default_rnd);
    if (retval != 0) {
      // Reset the internal value, as it might have been changed by
      // ::mpfr_set_str().
  ...
  template <typename T>
  typename std::enable_if<detail::is_mp_integer<T>::value, T>::type
  convert_to_impl() const {
    if (is_nan() || is_inf()) {
  ...
  convert_to_impl() const {
    // NOTE: of course, this can be optimised by avoiding going through the
    // integer conversion and
  ...
  convert_to_impl() const {
    ...
    if (is_inf()) {
      ...
      if (std::numeric_limits<T>::has_infinity && sign() > 0) {
        ...
        return std::copysign(std::numeric_limits<T>::infinity(),
  ...
  template <typename T>
  typename std::enable_if<detail::is_mp_rational<T>::value, T>::type
  convert_to_impl() const {
    if (is_nan()) {
    ...
    if (unlikely(!cptr)) {
      piranha_throw(std::overflow_error, "error in conversion of real to "
                                         "rational: the call to the MPFR "
                                         "function failed");
    }
    ...
    // NOTE: here the only exception that can be thrown is when raising to a
    // power
    // that cannot be represented by unsigned long.
    try {
      T retval(str_ptr.get());
      // NOTE: possible optimizations here include going through direct GMP
      // routines.
  ...
  // In-place addition.
  // NOTE: all sorts of optimisations, here and in binary add, are possible
  // (e.g., steal from rvalue ref,
  // avoid setting precision twice in binary operators, etc.). For the moment we
  // keep it basic.
  ...
  template <typename T, typename std::enable_if<is_interoperable_type<T>::value,
                                                int>::type = 0>
  ...
  // NOTE: we need to handle separately the NaNs as we cannot resort to the
  // inversion of the comparison operators for them.
  static bool check_nan(const real &r) { return r.is_nan(); }
  template <typename T>
  static bool
  check_nan(const T &x,
...
public:
  /// Default constructor.
  /**
   * Will initialize the number to zero, using real::default_prec as significand
   * precision.
   */
  real() {
   ...
   * The expected string format, assuming representation in base 10, is
   * described in the MPFR documentation.
   *
   * @param[in] str string representation of the real number.
   * @param[in] prec desired significand precision.
   *
   * @throws std::invalid_argument if the conversion from string fails or if the
   * requested significand precision
  ...
  /**
   * \p this will be first initialised with precision \p prec, and then \p other
   * will be assigned to \p this.
   *
   * @param[in] other real to be copied.
   * @param[in] prec desired significand precision.
   *
  ...
  /**
   * The assignment operation will deep-copy \p other (i.e., including its
   * precision).
   *
   * @param[in] other real to be copied.
   *
  ...
  real &operator=(const real &other) {
    if (this != &other) {
      ...
      if (m_value->_mpfr_d) {
        ...
        piranha_assert(!m_value->_mpfr_prec && !m_value->_mpfr_sign &&
                       !m_value->_mpfr_exp);
   ...
   * @return reference to \p this.
   *
   * @throws unspecified any exception thrown by the assignment operator from C
   * string.
   */
  real &operator=(const std::string &str) { return operator=(str.c_str()); }
  /// Assignment operator from C string.
  /**
   * The parsing rules are the same as in the constructor from string. The
   * precision of \p this
   * will not be changed by the assignment operation, unless \p this was the
   * target of a move operation that
   * left it in an uninitialised state.
   * In that case, \p this will be re-initialised with the default precision.
   *
   * In case \p str is malformed, before an exception is thrown the value of \p
   ...
   *
   * Conversion to \p bool is always successful, and returns <tt>sign() !=
   * 0</tt>.
   * Conversion to the other integral types is truncated (i.e., rounded to
   * zero), its success depending on whether or not
   * the target type can represent the truncated value.
   *
   * Conversion to floating point types is exact if the target type can
   * represent exactly the current value.
   * If that is not the case, the output value will be the nearest adjacent. If
   * \p this is not finite,
   * corresponding non-finite values will be produced if the floating-point type
   * supports them, otherwise
   * an error will be produced.
   *
   * Conversion of finite values to piranha::mp_rational will be exact.
   * Conversion of non-finite values will result in runtime
   * errors.
   *
   * @return result of the conversion to target type T.
   *
   * @throws std::overflow_error if the conversion fails in one of the ways
   * described above.
   */
   ...
   * < 0</tt>. If \p this is NaN, zero will be returned.
   */
  int sign() const {
    if (is_nan()) {
      return 0;
   ...
   */
  bool is_zero() const { return (mpfr_zero_p(m_value) != 0); }
  /// Test for NaN.
  /**
   * @return \p true if \p this is NaN, \p false otherwise.
   */
  bool is_nan() const { return mpfr_nan_p(m_value) != 0; }
  /// Test for infinity.
  /**
   * @return \p true if \p this represents infinity, \p false otherwise.
   */
  bool is_inf() const { return mpfr_inf_p(m_value) != 0; }
  /// Get precision.
  /**
   * @return the number of bits used to represent the significand of \p this.
   */
  ::mpfr_prec_t get_prec() const { return mpfr_get_prec(m_value); }
  /// Set precision.
  /**
   * Will set the significand precision of \p this to exactly \p prec bits, and
   * reset the value of \p this to NaN.
   *
   * @param[in] prec desired significand precision.
   *
   * @throws std::invalid_argument if the requested significand precision
   ...
   */
  void negate() { ::mpfr_neg(m_value, m_value, default_rnd); }
  /// Truncate in-place.
  /**
   * Set \p this to the next representable integer towards zero. If \p this is
   * infinity or NaN, there will be no effect.
   */
  void truncate() {
    if (is_inf() || is_nan()) {
      return;
   ...
   * the precision of \p this is changed to \p prec before the operation takes
   * place.
   *
   ...
   * @return reference to \p this.
   *
   * @throws unspecified any exception thrown by the contructor of
   * piranha::mp_integer, if invoked.
   */
  template <typename T>
   ...
   * This method will first compute <tt>r + x</tt>, cast it back to \p T via \p
   ...
   *
   * @throws unspecified any exception resulting from the binary operator or by
   * casting piranha::real to \p T.
   */
   ...
   * @return <tt>x + y</tt>.
   *
   * @throws unspecified any exception thrown by the corresponding in-place
   * operator.
   */
  template <typename T, typename U>
   ...
   */
  real &operator++() { return operator+=(1); }
  /// Suffix increment.
  /**
   ...
   * This method will first compute <tt>x - r</tt>, cast it back to \p T via \p
   * static_cast and finally assign the result to \p x.
   *
   * @param[in,out] x first argument.
   * @param[in] r second argument.
  ...
  /**
   * Sets \p this to <tt>this + (r1 * r2)</tt>. If the precision of \p this is
   * less than the maximum precision \p max_prec of the two operands
   * \p r1 and \p r2, the precision of \p this will be set to \p max_prec before
  ...
  real &multiply_accumulate(const real &r1, const real &r2) {
    const ::mpfr_prec_t prec1 =
        std::max<::mpfr_prec_t>(r1.get_prec(), r2.get_prec());
    if (prec1 > get_prec()) {
      *this = real{*this, prec1};
    }
// So the story here is that mpfr_fma has been reported to be slower than the
// two separate
// operations. Benchmarks on fateman1 indicate this is indeed the case (3.6 vs
// 2.7 secs
// on 4 threads). Hopefully it will be fixed in the future, for now adopt the
// workaround.
...
// NOTE: this optimisation requires the thread_local keyword.
    ...
    // NOTE: set the same precision as this, which is now the max precision of
    // the 3 operands.
    // If we do not do this, then tmp has an undeterminate precision. Use the
    // raw MPFR function
    // in order to avoid the checks in get_prec(), as we know the precision has
    // a sane value.
   ...
   * - both \p T and \p U are piranha::real.
   *
   * Note that in all comparison operators, apart from
   * piranha::real::operator!=(), if any operand is NaN \p false will be
   * returned.
   *
   * @param[in] x first argument
   * @param[in] y second argument.
  ...
  /// Exponentiation.
  /**
   * The operation is carried out with the maximum precision between \p this and
   * \p exp.
   *
  ...
  /**
   * The output format for finite numbers is normalised scientific notation,
   * where the exponent is signalled by the letter 'e'
   * and suppressed if null.
   *
   * For non-finite numbers, the string representation is one of "nan", "inf" or
   * "-inf".
   *
   ...
   *
   * @throws std::invalid_argument if the conversion to string via the MPFR API
   * fails.
   * @throws std::overflow_error if the exponent is smaller than an
   * implementation-defined minimum.
   * @throws unspecified any exception thrown by memory allocation errors in
   * standard containers.
   */
  friend std::ostream &operator<<(std::ostream &os, const real &r) {
    ...
    auto it = std::find_if(cpp_str.begin(), cpp_str.end(),
                           [](char c) { return detail::is_digit(c); });
    if (it != cpp_str.end()) {
      ++it;
      cpp_str.insert(it, '.');
   ...
   *
   * @throws unspecified any exception thrown by the assignment operator from
   * string of piranha::real.
   */
  ...
  std::remove_extent<::mpfr_t>::type *get_mpfr_t() { return &m_value[0u]; }
...
template <typename T>
struct negate_impl<
    T, typename std::enable_if<std::is_same<T, real>::value>::type> {
  /// Call operator.
  /**
   * @param[in,out] x piranha::real to be negated.
   */
  void operator()(real &x) const { x.negate(); }
};
...
struct is_zero_impl<
   ...
   */
  bool operator()(const T &r) const { return r.is_zero(); }
};
}
 ...
 * and the other is either piranha::real or an interoperable type for
 * piranha::real.
 *
 * The implementation follows these rules:
 * - if base and exponent are both piranha::real, then piranha::real::pow() is
 * used;
 * - otherwise, the non-real argument is converted to piranha::real and then
 * piranha::real::pow()
 *   is used.
 */
template <typename T, typename U>
struct pow_impl<T, U, detail::real_pow_enabler<T, U>> {
   ...
   */
  real operator()(const real &r, const real &x) const { return r.pow(x); }
   ...
   */
  template <typename T2> real operator()(const real &r, const T2 &x) const {
   ...
   * the invoked piranha::real constructor.
   */
  template <typename T2> real operator()(const T2 &r, const real &x) const {
...
template <typename T>
struct sin_impl<T,
                typename std::enable_if<std::is_same<T, real>::value>::type> {
  /// Call operator.
  /**
   ...
   * @return sine of \p r.
   */
  real operator()(const T &r) const { return r.sin(); }
};
...
struct cos_impl<T,
   ...
   */
  real operator()(const T &r) const { return r.cos(); }
};
...
struct abs_impl<T,
   ...
   */
  T operator()(const T &x) const { return x.abs(); }
};
...
struct partial_impl<
   ...
   */
  real operator()(const real &, const std::string &) const { return real(0); }
};
 ...
 *
 * The implementation follows these rules:
 * - if top and bottom are both piranha::real, then piranha::real::binomial() is
 * used;
 * - otherwise, the non-real argument is converted to piranha::real and then
 * piranha::real::binomial()
 *   is used.
 */
template <typename T, typename U>
struct binomial_impl<T, U, detail::real_binomial_enabler<T, U>> {
   ...
   */
  real operator()(const real &x, const real &y) const { return x.binomial(y); }
   ...
   * @return \p x choose \p y.
   *
   * @throws unspecified any exception thrown by piranha::real::binomial() or by
   * the invoked
   * piranha::real constructor.
   */
  template <typename T2> real operator()(const real &x, const T2 &y) const {
    // NOTE: init with the same precision as r in order
    // to maintain the same precision in the result.
   ...
   * piranha::real constructor.
   */
  template <typename T2> real operator()(const T2 &x, const real &y) const {
...
struct multiply_accumulate_impl<
    T, T, T, typename std::enable_if<std::is_same<T, real>::value>::type> {
  /// Call operator.
  /**
...
// upon destruction has the _mpfr_d set to zero for some reason but the other
// members
// are not zeroed out. This results in the asserts below firing, and probably a
// memory
// leak as well as the variable is not cleared. We just disable the asserts for
// now.
...
// Compute gamma(a)/(gamma(b) * gamma(c)), assuming a, b and c are not negative
// ints. The logarithm
// of the gamma function is used internally.
inline real real_compute_3_gamma(const real &a, const real &b, const real &c,
...
/**
 * This method will return \p this choose \p y. Any combination of real values
 * is supported.
 * The implementation uses the logarithm of the gamma function, thus the result
 * will not be in
 ...
 * @throws unspecified any exception resulting from arithmetic operations on
 * piranha::real.
 */
inline real real::binomial(const real &y) const {
  ...
  const ::mpfr_prec_t max_prec =
      std::max<::mpfr_prec_t>(get_prec(), y.get_prec());
  const bool neg_int_x = truncated() == (*this) && sign() < 0,
             neg_int_y = y.truncated() == y && y.sign() < 0,
             neg_int_x_y = ((*this) - y).truncated() == ((*this) - y) &&
                           ((*this) - y).sign() < 0;
  const unsigned mask = unsigned(neg_int_x) + (unsigned(neg_int_y) << 1u) +
                        (unsigned(neg_int_x_y) << 2u);
  switch (mask) {
  case 0u:
    // Case 0 is the non-special one, use the default implementation.
    return detail::real_compute_3_gamma((*this) + 1, y + 1, (*this) - y + 1,
                                        max_prec);
  ...
  case 3u: {
    // 3 and 5 are the cases with 1 inf in num and 1 inf in den. Use the
    // transformation
    // formula to make them finite.
    // NOTE: the phase here is really just a sign, but it seems tricky to
    // compute this exactly
    // due to potential rounding errors. We are attempting to err on the safe
    // side by using pow()
    // here.
    const auto phase = math::pow(-1, (*this) + 1) / math::pow(-1, y + 1);
    return detail::real_compute_3_gamma(-y, -(*this), (*this) - y + 1,
                                        max_prec) *
           phase;
  }
  case 5u: {
    const auto phase =
        math::pow(-1, (*this) - y + 1) / math::pow(-1, (*this) + 1);
    return detail::real_compute_3_gamma(-((*this) - y), y + 1, -(*this),
                                        max_prec) *
           phase;
  }
  }
  // Case 7 returns zero -> from inf / (inf * inf) it becomes a / (b * inf)
  // after the transform.
  // NOTE: put it here so the compiler does not complain about missing return
...
/// piranha::real.
/**
 * This specialisation is enabled if \p To is an integral type,
 * piranha::mp_integer or piranha::mp_rational, and \p From is
 * piranha::real.
 */
template <typename To, typename From>
...
private:
  template <typename T>
  using integral_enabler = typename std::enable_if<
      std::is_integral<T>::value || detail::is_mp_integer<T>::value, int>::type;
  template <typename T>
  using rational_enabler =
      typename std::enable_if<detail::is_mp_rational<T>::value, int>::type;

public:
  /// Call operator, real to integral overload.
  /**
   * The conversion will succeed if \p r is a finite integral value
   * representable by
   * the target type.
   *
   ...
   * @throws unspecified any exception thrown by the conversion operator of
   * piranha::real.
   */
  template <typename T = To, integral_enabler<T> = 0>
  T operator()(const real &r) const {
    // NOTE: the finiteness check here is repeated in the cast below, but we
    // need it here in order
    // to make sure that we can compute the truncated r.
    if (unlikely(r.is_inf() || r.is_nan() || r.truncated() != r)) {
      piranha_throw(
          std::invalid_argument,
          "the input real does not represent a finite integral value");
    }
 ...
 */
inline real operator"" _r(const char *s) { return real(s); }
}
}

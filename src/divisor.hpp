 ...
 * \f]
 * where \f$ a_{i,j} \f$ are integers, \f$ x_i \f$ are symbols, and \f$ e_j \f$
 * are positive integers. The type
 * of \f$ a_{i,j} \f$ and \f$ e_j \f$ is \p T. The terms of the product are
 * stored in a piranha::hash_set and
 * they are guaranteed to be in a canonical form defined by the following
 * properties:
 * - if \p T is a C++ integral type, the values of \f$ a_{i,j} \f$ and \f$ e_j
 * \f$ are within implementation-defined ranges,
 ...
 * ## Type requirements ##
 *
 * \p T must be either a C++ signed integral type or an instance of
 * piranha::mp_integer.
 *
 ...
 */
// NOTE: if we ever make this completely generic on T, remember there are some
// hard-coded assumptions. E.g.,
// is_zero must be available in split().
// NOTE: the implementation defined range restriction is needed in order to make
// the gcd computations safe.
...
private:
  ...
  struct p_type {
    ...
    explicit p_type(v_type &&v_, value_type &&e_)
        : v(std::move(v_)), e(std::move(e_)) {}
    ~p_type() = default;
    p_type &operator=(const p_type &) = default;
    p_type &operator=(p_type &&) = default;
    bool operator==(const p_type &other) const { return v == other.v; }
    // Serialization support.
    template <class Archive> void serialize(Archive &ar, unsigned int) {
      // NOTE: there's no need here for exception safety, as this is not a
      // public class: we
      // don't care if during deserialization we end up deserializing only part
      // of the pair.
      ar &v;
      ar &e;
    }
  ...
  struct p_type_hasher {
    std::size_t operator()(const p_type &p) const { return p.v.hash(); }
  };
  // Underlying container type.
  using container_type = hash_set<p_type, p_type_hasher>;
  // Canonical term: the first nonzero element is positive and all the gcd of
  // all elements is 1 or -1.
  // NOTE: this also includes the check for all zero elements, as gcd(0,0,...,0)
  // = 0.
  ...
  // Insertion machinery.
  template <typename Term> void insertion_impl(Term &&term) {
  ...
  template <class Archive> void load(Archive &ar, unsigned int) {
    divisor new_d;
    try {
      // NOTE: here we could throw either because the archive is garbage
      // (Boost.Serialization throwing
      // in this case) or because the loaded terms are invalid. In the second
      // case we must make sure
      // to destroy the content of new_d before exiting, otherwise the dtor of
      // the divisor will
      // hit assertion failures in debug mode. We need the second clear() in the
      // catch block
      // because we could be in the situation that the archive contains invaild
      // terms and it is garbage
      // after a while - in thie case invalid terms coud be living in new_d and
      // the first clear() is never
      // hit.
      ...
      if (unlikely(!new_d.destruction_checks())) {
        ...
        piranha_throw(
            std::invalid_argument,
            "error during the deserialization of a divisor, the loaded data "
            "is invalid");
      }
    } catch (...) {
      new_d.clear();
  ...
  // NOTE: here we are not actually requiring that the eval_type has to be
  // destructible, copyable/movable, etc.
  // We just assume it is a sane type of some kind.
  template <typename U>
  using eval_sum_type =
      decltype(std::declval<const value_type &>() * std::declval<const U &>());
  template <typename U>
  using eval_type_ =
      decltype(math::pow(std::declval<const eval_sum_type<U> &>(),
                         std::declval<const value_type &>()));
  template <typename U>
  using eval_type = typename std::enable_if<
      std::is_constructible<eval_type_<U>, int>::value &&
          is_divisible_in_place<eval_type_<U>>::value &&
          std::is_constructible<eval_sum_type<U>, int>::value &&
          is_addable_in_place<eval_sum_type<U>>::value &&
...
public:
  ...
  /**
   * This constructor is used in the generic constructor of piranha::series. It
   * is equivalent
   ...
   */
  explicit divisor(const divisor &other, const symbol_set &args)
      : m_container(other.m_container) {
    if (unlikely(!is_compatible(args))) {
      piranha_throw(std::invalid_argument,
                    "the constructed divisor is incompatible with the "
                    "input symbol set");
    }
  }
   ...
   * - the value type of \p It can be safely cast to
   * piranha::divisor::value_type,
   ...
   * in the range <tt>[begin,end)</tt> will be used to construct the \f$ a_{i,j}
   * \f$ of the term, while \p e
   * will be used to construct the exponent (after a call to
   * piranha::safe_cast()).
   * If no term with the same set of \f$ a_{i,j} \f$ exists, then
   * a new term will be inserted; otherwise, \p e will be added to the exponent
   ...
   * @param[in] e exponent.
   *
   * @throws std::invalid_argument if the term to be inserted is not in
   * canonical form, or if the insertion
   * leads to an overflow in the value of an exponent.
   * @throws std::overflow_error if the insertion results in the container to be
   * resized over
  ...
  /// Size.
  /**
   * @return the size of the internal container - that is, the number of terms
   * in the product.
   */
   ...
   */
  bool operator!=(const divisor &other) const { return !((*this) == other); }
  /// Hash value.
  /**
   * The hash value is computed by combining the hash values of all terms. An
   * empty divisor
  ...
  std::size_t hash() const {
    // NOTE: here we are using a simple sum to compute the hash of a vector of
    // vectors. Indeed,
    // we need some add/multiply like operation in order to make sure that two
    // divisors
    // with the same elements stored in different order hash to the same thing.
    // This seems
    ...
    // as the hash of each subvector should be rather good (it comes from boost
    // hash combine).
    // XOR is also commutative and might be used (see link above).
    // If problem arises, we can consider muliplying the hash of each subvector
    // by a prime
  ...
  /// Compatibility check.
  /**
   * An empty divisor is considered compatible with any set of symbols.
   * Otherwise, a non-empty
   * divisor is compatible if the number of variables in the terms is the same
   * as the number
   ...
   *
   * @throws std::invalid_argument if the number of variables in the divisor is
   * different
  ...
  /// Merge arguments.
  /**
   * This method will merge the new arguments set \p new_args into \p this,
   * given the current reference arguments set
   * \p orig_args. Arguments in \p new_args not appearing in \p orig_args will
   * be inserted in the terms,
   * with the corresponding \f$ a_{i,j} \f$ values constructed from the integral
   * constant 0.
   *
   ...
   * - the number of variables in the terms of \p this is different from the
   * size of \p orig_args,
   ...
   */
  divisor merge_args(const symbol_set &orig_args,
  ...
  /// Print to stream.
  /**
   * This method will print to the stream \p os a text representation of \p
   * this.
   *
   * @param[in] os target stream.
   * @param[in] args reference symbol set.
   *
   * @throws std::invalid_argument if the number of terms of \p this is
   * different from the size of \p args.
   * @throws unspecified any exception thrown by printing to \p os
   * piranha::divisor::value_type, strings or characters.
   */
  void print(std::ostream &os, const symbol_set &args) const {
    ...
    for (auto it = m_container.begin(); it != it_f; ++it) {
      ...
      for (typename v_type::size_type i = 0u; i < it->v.size(); ++i) {
        ...
        // Print the aij, unless it's "-1": in that case, just print the minus
        // sign.
        if (it->v[i] == -1) {
          os << '-';
        } else if (it->v[i] != 1) {
   ...
   * - it supports the arithmetic operations necessary to construct the return
   * type.
   *
   * The return value will be built via multiplications of the \f$ a_{i,j} \f$
   * by the values
   * in the input map, additions, divisions and exponentiations via
   * piranha::math::pow().
   * If the divisor has no terms, 1 will be returned. If \p args is not
   * compatible with \p this and \p pmap,
   * or the positions in \p pmap do not reference only and all the variables in
   * the divisor, an error will be thrown.
   *
   ...
   * this,
   * \p args or \p pmap.
   * @throws unspecified any exception thrown by the construction of the return
   * type.
   */
  template <typename U>
  eval_type<U> evaluate(const symbol_set::positions_map<U> &pmap,
    ...
    for (auto it = m_container.begin(); it != it_f; ++it) {
      ...
      // NOTE: consider rewriting this in terms of multiplications as a
      // performance
   ...
   *
   * @throws std::invalid_argument if the key of \p t1 and/or the key of \p t2
   * are incompatible with \p args, or if the multiplication
   * of the keys results in an exponent exceeding the allowed range.
   * @throws std::overflow_error if the multiplication of the keys results in
   * the container of the result key to be resized over
   * an implementation-defined limit.
   * @throws unspecified any exception thrown by:
   * - piranha::math::mul3(),
   * - the public interface of piranha::hash_set,
  ...
  static void multiply(std::array<term<Cf, divisor>, multiply_arity> &res,
                       const term<Cf, divisor> &t1, const term<Cf, divisor> &t2,
                       const symbol_set &args) {
    term<Cf, divisor> &t = res[0u];
    if (unlikely(!t1.m_key.is_compatible(args) ||
                 !t2.m_key.is_compatible(args))) {
      piranha_throw(std::invalid_argument, "invalid size of arguments set");
    }
    // Coefficient.
    ...
    const divisor &large =
        (t1.m_key.size() >= t2.m_key.size()) ? t1.m_key : t2.m_key;
    const divisor &small =
        (t1.m_key.size() < t2.m_key.size()) ? t1.m_key : t2.m_key;
   ...
   * will remove
   * from \p candidates those symbols whose \f$ a_{i,j} \f$ in \p this are not
   * all zeroes.
   *
   ...
   * @throws unspecified any exception thrown by piranha::math::is_zero() or
  ...
  /// Trim.
  /**
   * This method will return a copy of \p this with the \f$ a_{i,j} \f$
   * associated to the symbols
   ...
   *
   * @throws std::invalid_argument if \p this is not compatible with \p
   * orig_args.
   * @throws unspecified any exception thrown by
   * piranha::small_vector::push_back()
  ...
  /// Split divisor.
  /**
   * This method will split \p this into two parts: the first one will contain
   * the terms of the divisor
   * whose \f$ a_{i,j} \f$ values for the only symbol in \p p are not zero, the
   * second one the terms whose
   * \f$ a_{i,j} \f$  values for the only symbol in \p p are zero.
   *
   * @param[in] p a piranha::symbol_set::positions containing exactly one
   * element.
   * @param[in] args reference set of piranha::symbol.
   *
   * @return the original divisor split into two parts.
   *
   * @throws std::invalid_argument if \p args is not compatible with \p this or
   * \p p, or \p p
   ...
   * - piranha::hash_set::insert().
   */
  std::pair<divisor, divisor> split(const symbol_set::positions &p,
                                    const symbol_set &args) const {
    if (unlikely(!is_compatible(args))) {
      piranha_throw(std::invalid_argument, "invalid size of arguments set");
    }
...
template <typename T> const std::size_t divisor<T>::multiply_arity;
}

namespace std {

template <typename T> struct hash<piranha::divisor<T>> {
  /// Result type.

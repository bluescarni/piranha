...
// This class has been minimally adapted from the barrier class in the Boost
// libraries 1.45.0.
...
/// Thread barrier.
/**
 * Also known as a rendezvous, a barrier is a synchronization point between
 * multiple threads.
 * The barrier is configured for a particular number of threads \p n, and as
 * threads reach the barrier
 * they must wait until all \p n threads have arrived. Once the <tt>n</tt>-th
 * thread has reached the barrier,
 * all the waiting threads can proceed, and the barrier is reset.
 *
 * This class has been minimally adapted from the barrier class available from
 * the Boost libraries.
 *
 * @see
...
public:
   ...
   */
  explicit thread_barrier(unsigned count)
      : m_mutex(), m_cond(), m_threshold(count), m_count(count),
        m_generation(0) {
    if (count == 0) {
  ...
  /// Default destructor.
  /**
   * No threads must be waiting on this when the destructor is called, otherwise
  ...
  /// Wait method.
  /**
   * Block until \p count threads have called wait() on this. When the
   * <tt>count</tt>-th thread calls
   * wait(), all waiting threads are unblocked, and the barrier is reset.
   *
   * @return \p true for exactly one thread from each batch of waiting threads,
   * \p false otherwise.
   */
  bool wait() {
    try {

...
// This is essentially a reduced std::vector replacement that should use less
// storage on most implementations
// (e.g., it has a size of 16 on Linux 64-bit and it _should_ have a size of 8
// on many 32-bit archs).
// NOTE: earlier versions of this class used to have support for custom
// allocation. See, e.g., commit
// 6e1dd235c729ed05eb6e872dceece2f20a624f32
// It seems however that there are some defects currently in the standard C++
// library that make it quite hard
// to implement reasonably strong exception safety and nothrow guarantees. See,
// e.g.,
// http://cplusplus.github.io/LWG/lwg-defects.html#2103
// It seems like in the current standard it is not possible to implemement
// noexcept move assignment not even
// for std::alloc. The crux of the matter is that if
// propagate_on_container_move_assignment is not defined,
...
// propagate_on_container_copy_assignment seems to force one to nuke
// pre-emptively the content of the container
// before doing anything (thus preventing strong forms of exception safety).
// The approach taken here is thus to just hard-code std::allocator, which is
// stateless, and force nothrow
// behaviour when doing assignments. This should work with all stateless
// allocators (including our own allocator).
// Unfortunately it is not clear if the extension allocators from GCC are
// stateless, so better not to muck
...
// NOTE: now we do not use the allocator at all, as we cannot guarantee it has a
// standard layout. Keep the comments
...
private:
  ...
  // NOTE: this bit of TMP is to avoid checking an always-false condition on
  // reserve() on most platforms, which triggers a compiler
  // warning on GCC 4.7.
  static const std::size_t max_alloc_size =
      std::numeric_limits<std::size_t>::max() / sizeof(value_type);
  static const bool need_reserve_check =
      std::numeric_limits<size_type>::max() > max_alloc_size;
  static bool reserve_check_size(const size_type &, const std::false_type &) {
    return false;
  }
  static bool reserve_check_size(const size_type &new_capacity,
                                 const std::true_type &) {
...
public:
  ...
  dynamic_storage(const dynamic_storage &other)
      : m_tag(0u), m_size(0u),
        m_capacity(other.m_size), // NOTE: when copying, we set the capacity to
                                  // the same value of the size.
        m_ptr(nullptr) {
  ...
  ~dynamic_storage() {
    // NOTE: here we should replace with bidirectional tt, if we ever implement
    // it.
  ...
  void push_back(const value_type &x) { push_back_impl(x); }
  void push_back(value_type &&x) { push_back_impl(std::move(x)); }
  ...
  void reserve(const size_type &new_capacity) {
    ...
    // NOTE: the first check is redundant right now, just keep it around in case
    // max_size changes in the future.
    if (unlikely(/*new_capacity > max_size ||*/ reserve_check_size(
            new_capacity,
            std::integral_constant<bool, need_reserve_check>()))) {
      piranha_throw(std::bad_alloc, );
    }
    // Start by allocating the new storage. New capacity is at least one at this
    // point.
    pointer new_storage = allocate(new_capacity);
    assert(new_storage != nullptr);
    // Move in existing elements. Consistency checks ensure
    // that m_size is not greater than m_capacity and, by extension,
    // new_capacity.
  ...
  std::size_t hash() const { return detail::vector_hasher(*this); }
  void resize(const size_type &new_size) {
    ...
    // The storage we are going to operate on is either the old one, if it has
    // enough capacity,
    // or new storage.
    ...
    // - if not new storage, new_size <= m_capacity; m_ptr can be null only if
    // capacity is 0, but then
    //   m_size is zero and new_size is 0 as well, and the function never
    //   arrived here because of the check above.
    piranha_assert(storage != nullptr);
    // Default-construct excess elements. We need to do this regardless of where
    ...
    if (new_storage) {
      // Move in old elements into the new storage. As we had to increase the
      // capacity,
      // we know that new_size has to be greater than the old one, hence all old
      // elements
  ...
  iterator erase(const_iterator it) {
    ...
    for (; it != it_f; ++it) {
      it->~T();
      ::new (static_cast<void *>(const_cast<iterator>(it)))
...
private:
  ...
  static pointer allocate(const size_type &s) {
    return static_cast<pointer>(
        aligned_palloc(0u, static_cast<std::size_t>(s * sizeof(value_type))));
  }
  static void deallocate(pointer p) { aligned_pfree(0u, p); }
  // Common implementation of push_back().
  template <typename U> void push_back_impl(U &&x) {
    piranha_assert(consistency_checks());
  ...
  void increase_capacity() {
    ...
    // NOTE: capacity should double, but without going past max_size, and in
    // case it is zero it should go to 1.
    const size_type new_capacity =
        (m_capacity > max_size / 2u)
            ? max_size
            : ((m_capacity != 0u) ? static_cast<size_type>(m_capacity * 2u)
...
template <typename T> const std::size_t dynamic_storage<T>::max_alloc_size;

// TMP to determine automatically the size of the static storage in
// small_vector.
...
struct auto_static_size<
    T, Size, typename std::enable_if<(sizeof(dynamic_storage<T>) >
                                      sizeof(static_vector<T, Size>))>::type> {
...
template <typename T> struct check_integral_constant {
...
// NOTE: here we are using a special rule from 9.2. If the union is
// standard-layout, and it contains two or more
// standard-layout classes, it will be possible to inspect the value of the
// common initial part of any of them. That is,
// we can access the initial m_tag member of the static and dynamic storage via
// both st and dy, and use it to detect
...
template <typename T, typename S> union small_vector_union {
  using s_storage =
      typename std::conditional<S::value == 0u,
                                static_vector<T, auto_static_size<T>::value>,
  ...
  bool is_static() const { return static_cast<bool>(m_st.m_tag); }
  // Getters.
...
/// Small vector class.
/**
 * This class is a sequence container similar to the standard
 * <tt>std::vector</tt> class. The class will avoid dynamic
 * memory allocation by using internal static storage up to a certain number of
 * stored elements. If \p S is a zero integral constant, this
 * number is calculated automatically (but it will always be at least 1).
 * Otherwise, the limit number is set to \p S::value.
 *
 * ## Type requirements ##
 *
...
// - the m_size member of dynamic and static could be made a signed integer, the
// sign establishing the storage type
//   and drop the m_tag member. This in principle would allow to squeeze some
//   extra space from the static vector
//   but not sure this is worth it;
// - POD optimisations in dynamic storage;
// - in the dynamic storage, it looks like on 64bit we can bump up the size
// member to 16 bit without changing size,
//   thus we could sture ~16000 elements. BUT on 32bit this will change the
//   size.
...
class small_vector {
  ...
  using size_type_impl =
      max_int<typename s_storage::size_type, typename d_storage::size_type>;
  template <typename U> static constexpr U get_max(const U &a, const U &b) {
    return (a > b) ? a : b;
  }

public:
  /// Maximum number of elements that can be stored in static storage.
  static const typename std::decay<decltype(s_storage::max_size)>::type
      max_static_size = s_storage::max_size;
  /// Maximum number of elements that can be stored in dynamic storage.
  static const typename std::decay<decltype(d_storage::max_size)>::type
      max_dynamic_size = d_storage::max_size;
  /// A fundamental unsigned integer type representing the number of elements
  /// stored in the vector.
  using size_type = size_type_impl;

private:
  // If the size type can assume values larger than d_storage::max_size, then we
  // need to run a check in resize().
  static const bool need_resize_check =
      std::numeric_limits<size_type>::max() > d_storage::max_size;
...
public:
  ...
  static const size_type max_size =
      get_max<size_type>(max_static_size, max_dynamic_size);
  /// Alias for \p T.
  using value_type = T;
  /// Iterator type.
...
private:
  ...
  using init_list_enabler =
      typename std::enable_if<has_safe_cast<T, U>::value, int>::type;
  // Enabler for equality operator.
  template <typename U>
  using equality_enabler =
      typename std::enable_if<is_equality_comparable<U>::value, int>::type;
  // Enabler for hash.
  template <typename U>
  using hash_enabler =
      typename std::enable_if<is_hashable<U>::value, int>::type;
  ...
  template <class Archive> void load(Archive &ar, unsigned int) {
    // NOTE: there are faster ways of implementing this, first of all by
    // re-using the storage
    // with resize(). Here we are forcing a re-allocation instead. Keep this in
    // mind for later
    // optimisations.
    // Erase this.
    *this = small_vector();
...
public:
  ...
  /// Copy constructor.
  /**
   * The storage type after successful construction will be the same of \p
   * other.
   *
   ...
   *
   * The elements of \p l will be added to a default-constructed object, after
   * conversion
   * to \p T via piranha::safe_cast().
   *
   * @param[in] l list that will be used for initialisation.
   *
   * @throws unspecified any exception thrown by push_back() or by
   * piranha::safe_cast().
   */
  ...
  /**
   * This constructor will initialise a vector containing \p size copies of \p
   * value.
   *
   * @param[in] size the desired vector size.
   * @param[in] value the value used to initialise all the elements of the
   * vector.
   *
   ...
   */
  const value_type &operator[](const size_type &n) const { return begin()[n]; }
  /// Subscript operator.
  /**
   * @param[in] n index of the element to be accessed.
   *
   ...
   */
  value_type &operator[](const size_type &n) { return begin()[n]; }
  /// Copy-add element at the end.
  /**
   * @param[in] x object that will be added at the end of the vector.
   *
   * @throws std::bad_alloc in case of memory allocation errors or if the size
   * limit is exceeded.
   * @throws unspecified any exception thrown by the copy constructor of \p T.
   */
  void push_back(const value_type &x) { push_back_impl(x); }
  ...
  /// Static storage flag.
  /**
   * @return \p true if the storage being used is the static one, \p false
   * otherwise.
   */
  bool is_static() const { return m_union.is_static(); }
  /// Equality operator.
  /**
   * \note
   ...
   *
   * @return \p true if the sizes of \p this and \p other coincide and the
  ...
  /**
   * \note
   * This method is enabled only if \p value_type satisfies
   * piranha::is_hashable.
   *
   ...
   * @throws std::bad_alloc in case of memory allocation errors or if the size
   * limit is exceeded.
   * @throws unspecified any exception thrown by the default constructor of \p
   * T.
   */
  void resize(const size_type &size) {
    if (m_union.is_static()) {
      if (size <= max_static_size) {
        ...
        // NOTE: this check is a repetition of an existing check in dynamic
        // storage's
        // resize. The reason for putting it here as well is to ensure the
        // safety
        // of the static_cast below.
        if (unlikely(resize_check_size(
        ...
        // NOTE: this will not throw, as tmp_d is guaranteed to be of adequate
        // size
        // and thus push_back() will not fail.
        std::move(m_union.g_st().begin(), m_union.g_st().end(),
                  std::back_inserter(tmp_d));
        // Fill in the missing elements.
        tmp_d.resize(d_size);
        // Destroy static, move in dynamic.
        m_union.g_st().~s_storage();
        ::new (static_cast<void *>(&(m_union.m_dy)))
            d_storage(std::move(tmp_d));
      }
    } else {
      if (unlikely(resize_check_size(
              size, std::integral_constant<bool, need_resize_check>()))) {
   ...
   *
   * Will compute the element-wise addition of \p this and \p other via
   * piranha::math::add3(), storing the result in \p retval.
   * In face of exceptions during the addition of two elements, \p retval will
   * be left in an unspecified
   * but valid state, provided that piranha::math::add3() offers the basic
   * exception
   * safety guarantee.
   *
   * \p this, \p retval and/or \p other are allowed to be the same object,
   * provided that piranha::math::add3()
   * also supports this type of usage.
   *
   * @param[out] retval result of the addition.
   * @param[in] other argument for the addition.
   *
   * @throws std::invalid_argument if the sizes of \p this and \p other do not
   * coincide.
  ...
  void add(small_vector &retval, const small_vector &other) const {
    ...
    // the resize methods don't do anything if the new size is the same as the
    // old one.
    ...
    for (size_type i = 0u; i < std::get<0u>(sbe1); ++i) {
      math::add3(*(std::get<1u>(sbe_out) + i), *(std::get<1u>(sbe1) + i),
                 *(std::get<1u>(sbe2) + i));
    }
  }
  /// Vector subtraction.
   ...
   *
   * Will compute the element-wise subtraction of \p this and \p other via
   * piranha::math::sub3(), storing the result in \p retval.
   * In face of exceptions during the subtraction of two elements, \p retval
   * will be left in an unspecified
   * but valid state, provided that piranha::math::sub3() offers the basic
   * exception
   * safety guarantee.
   *
   * \p this, \p retval and/or \p other are allowed to be the same object,
   * provided that piranha::math::sub3()
   * also supports this type of usage.
   *
  ...
  void sub(small_vector &retval, const small_vector &other) const {
    ...
    auto sbe_out = retval.size_begin_end();
    for (size_type i = 0u; i < std::get<0u>(sbe1); ++i) {
      math::sub3(*(std::get<1u>(sbe_out) + i), *(std::get<1u>(sbe1) + i),
                 *(std::get<1u>(sbe2) + i));
    }
  }
  /// Erase element.
  /**
   * This method will erase the element to which \p it points. The return value
   * will be the iterator
   * following the erased element (which will be the end iterator if \p it
  ...
  std::tuple<size_type, iterator, iterator> size_begin_end() {
    if (m_union.is_static()) {
      return std::make_tuple(size_type(m_union.g_st().size()),
                             m_union.g_st().begin(), m_union.g_st().end());
    } else {
      return std::make_tuple(size_type(m_union.g_dy().size()),
                             m_union.g_dy().begin(), m_union.g_dy().end());
    }
  }
  /// Size, begin and end (const version).
  /**
   * @return a tuple containing the size and the begin and the end const
   * iterators.
   */
  ...
  template <typename U> void push_back_impl(U &&x) {
    if (m_union.is_static()) {
      if (m_union.g_st().size() == m_union.g_st().max_size) {
        ...
        // NOTE: this check ensures that the d_storage::max_size is strictly
        // greater than
        // the current (static) size (equal to max static size). This means we
        // can compute safely
        // current size + 1 while casting to dynamic storage size type and try
        // to allocate enough space for
        // the std::move and push_back() below.
        // NOTE: this check is analogous to current_size + 1 >
        // d_storage::max_size, i.e., the same
        // check in resize(), but it will use static const variables written as
        // this.
        if (unlikely(s_storage::max_size >= d_storage::max_size)) {
          piranha_throw(std::bad_alloc, );
        }
        d_storage tmp_d;
        tmp_d.reserve(static_cast<d_size_type>(
            static_cast<d_size_type>(m_union.g_st().max_size) + 1u));
        std::move(m_union.g_st().begin(), m_union.g_st().end(),
                  std::back_inserter(tmp_d));
        // Push back the new element.
        tmp_d.push_back(std::forward<U>(x));
        // Now destroy the current static storage, and move-construct new
        // dynamic storage.
        // NOTE: in face of custom allocators here we should be ok, as move
        // construction
        // of custom alloc will not throw and it will preserve the ownership of
        // the moved-in elements.
        m_union.g_st().~s_storage();
        ::new (static_cast<void *>(&(m_union.m_dy)))
            d_storage(std::move(tmp_d));
      } else {
...
const typename std::decay<decltype(
    small_vector<T, S>::s_storage::max_size)>::type
    small_vector<T, S>::max_static_size;

template <typename T, typename S>
const typename std::decay<decltype(
    small_vector<T, S>::d_storage::max_size)>::type
    small_vector<T, S>::max_dynamic_size;

...
/// Array key.
/**
 * Key type represented by an array-like sequence of instances of type \p T.
 * Interface and semantics
 * mimic those of \p std::vector. The underlying container used to store the
 * elements is piranha::small_vector.
 * The template argument \p S is passed down as second argument to
 * piranha::small_vector in the definition
 ...
 * ## Serialization ##
 *
 * This class supports serialization if the internal piranha::small_vector is
 * serializable.
 */
template <typename T, typename Derived,
          typename S = std::integral_constant<std::size_t, 0u>>
class array_key {
  // NOTE: the general idea about requirements here is that these are the bare
  // minimum
  // to make a simple array_key suitable as key in a piranha::series. There are
  // additional
  ...
  using init_list_enabler = typename std::enable_if<
      std::is_constructible<container_type, std::initializer_list<U>>::value,
      int>::type;
...
private:
  ...
  // NOTE: here it is not difficult to make this an int enabler, like we usually
  // do elsewhere,
  // but alas the intel compiler chokes on this :/.
  template <typename U>
  using add_enabler = decltype(std::declval<U const &>().add(
      std::declval<U &>(), std::declval<U const &>()));
  template <typename U>
  using sub_enabler = decltype(std::declval<U const &>().sub(
      std::declval<U &>(), std::declval<U const &>()));
...
public:
  ...
  /**
   * \note
   * This constructor is enabled only if piranha::small_vector is constructible
   * from \p list.
   *
   ...
   *
   * @throws unspecified any exception thrown by the corresponding constructor
   * of piranha::small_vector.
   */
  ...
  /**
   * The key will be created with a number of variables equal to
   * <tt>args.size()</tt>
   ...
   * - the construction of instances of type \p value_type from the integral
   * constant 0.
   */
  ...
  /**
   * \note
   * This constructor is enabled only if \p U can be cast safely to the value
   * type.
   *
   * Generic constructor for use in series, when inserting a term of different
   * type.
   * The internal container will be initialised with the
   * contents of the internal container of \p x (possibly converting the
   * individual contained values through piranha::safe_cast(),
   * if the values are of different type). If the size of \p x is different from
   * the size of \p args, a runtime error will
   * be produced.
   *
   ...
   * - piranha::safe_cast().
   */
  template <typename U, typename Derived2, typename S2,
            generic_ctor_enabler<U> = 0>
  explicit array_key(const array_key<U, Derived2, S2> &other,
                     const symbol_set &args) {
    if (unlikely(other.size() != args.size())) {
      piranha_throw(std::invalid_argument,
                    "inconsistent sizes in generic array_key constructor");
    }
    // NOTE: here the requirement is that value_type is move-assignable, which
    // is already
   ...
   */
  void resize(const size_type &new_size) { m_container.resize(new_size); }
  /// Element access.
  /**
   ...
   */
  std::size_t hash() const { return m_container.hash(); }
   ...
   * piranha::small_vector::push_back().
   */
  void push_back(value_type &&x) { m_container.push_back(std::move(x)); }
   ...
   * piranha::small_vector::push_back().
   */
  void push_back(const value_type &x) { m_container.push_back(x); }
  /// Equality operator.
  /**
   ...
   */
  bool operator!=(const array_key &other) const { return !operator==(other); }
   ...
   *
   * @throws std::invalid_argument if the size of \p this differs from the size
   * of \p args.
  ...
  /// Trim.
  /**
   * This method will return a copy of \p this with the elements associated to
   * the symbols
   ...
   * is well-formed.
   *
   * Equivalent to calling piranha::small_vector::add() on the internal
   * containers of \p this
   * and of the arguments.
   *
   * @param[out] retval piranha::array_key that will hold the result of the
   * addition.
   ...
   * is well-formed.
   *
   * Equivalent to calling piranha::small_vector::sub() on the internal
   * containers of \p this
   * and of the arguments.
   *
   * @param[out] retval piranha::array_key that will hold the result of the
   ...
   * be inserted in the internal container,
   ...
   *
   * @return a \p Derived instance resulting from merging \p new_args into \p
   * this.
   *
   ...
   * constant 0.
   */
  Derived merge_args(const symbol_set &orig_args,
                     const symbol_set &new_args) const {
    Derived retval;
    retval.m_container =
        detail::vector_merge_args(m_container, orig_args, new_args);
    return retval;
  }
...
public:
  ...
  /**
   * @return the output of the piranha::small_vector::size_begin_end() method
   * called

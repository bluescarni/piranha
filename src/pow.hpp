 ...
 */
template <typename T, typename U, typename Enable = void> struct pow_impl {};

/// Specialisation of the piranha::math::pow() functor for arithmetic and
...
struct pow_impl<T, U, detail::pow_fp_arith_enabler<T, U>> {
  /// Call operator.
  /**
   * This operator will compute the exponentiation via one of the overloads of
   * <tt>std::pow()</tt>.
   *
   * @param[in] x base.
...
/// Exponentiation.
/**
 * Return \p x to the power of \p y. The actual implementation of this function
 * is in the piranha::math::pow_impl functor's
 * call operator.
 *
 * @param[in] x base.
 * @param[in] y exponent.
...
// NOTE: this specialisation must be here as in the integral-integral overload
// we use mp_integer inside,
// so the declaration of mp_integer must be avaiable. On the other hand, we
// cannot put this in mp_integer.hpp
// as the integral-integral overload is supposed to work without including
// mp_integer.hpp.
 ...
 * an integral type, then piranha::mp_integer::pow() is used
 *   to compute the result (after any necessary conversion),
 * - if both arguments are integral types, piranha::mp_integer::pow() is used
 * after the conversion of the base
 *   to piranha::mp_integer,
 * - otherwise, the piranha::mp_integer argument is converted to the
 * floating-point type and \p piranha::math::pow() is
...
struct pow_impl<T, U, detail::integer_pow_enabler<T, U>> {
   ...
   */
  template <int NBits>
  mp_integer<NBits> operator()(const mp_integer<NBits> &b,
                               const mp_integer<NBits> &e) const {
    return b.pow(e);
  }
   ...
   * @returns <tt>b**e</tt>.
   *
   * @throws unspecified any exception thrown by converting piranha::mp_integer
...
/**
 * The type trait will be \p true if piranha::math::pow() can be successfully
 * called with base \p T and
 * exponent \p U.
 *
 * The call to piranha::math::pow() will be tested with const reference
 * arguments.
 */
template <typename T, typename U>
class is_exponentiable : detail::sfinae_types {
  template <typename Base, typename Expo>
  static auto test(const Base &b, const Expo &e)
      -> decltype(math::pow(b, e), void(), yes());
  static no test(...);
...
// Static init.
template <typename T, typename U> const bool is_exponentiable<T, U>::value;
}

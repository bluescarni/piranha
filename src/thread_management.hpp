...
/**
 * Upon successful completion of this method, the calling thread will be
 * confined on the specified processor.
 * This method requires platform-specific functions and thus might not be
 * available on all configurations.
 *
 * In case of exceptions, the thread will not have been bound to any processor.
 *
 * @param[in] n index of the processor to which the thread will be bound
 ...
 * - piranha::runtime_info::get_hardware_concurrency() returns a nonzero value
 * \p m and <tt>n >= m</tt>.
 * @throws piranha::not_implemented_error if the method is not available on the
 * current platform.
...
inline void bind_to_proc(unsigned n) {
  ...
  piranha_throw(not_implemented_error,
                "bind_to_proc() is not available on this platform");
#endif
}

/// Query if current thread is bound to a processor.
/**
 * The complexity of the operation is at most linear in the maximum number of
 * processors that can be
 * represented on the system (e.g., in Unix-like system this number will
 * typically correspond to the
 * \p CPU_SETSIZE macro).
 * This method requires platform-specific functions and thus might not be
 * available on all configurations.
 * Note that if only a single core/cpu is available, the returned value will
 * always be <tt>(true,0)</tt>.
 *
 * @return the pair <tt>(true,n)</tt> if the calling thread is bound to a single
 * processor with index \p n, <tt>(false,0)</tt>
 * otherwise.
 *
 * @throws piranha::not_implemented_error if the method is not available on the
 * current platform.
...
inline std::pair<bool, unsigned> bound_proc() {
  ...
  for (int i = 0; i < cpu_setsize; ++i) {
    if (CPU_ISSET(i, &cpuset)) {
      // Cast is safe here (verified above that cpu_setsize is representable in
      // int,
  ...
  cpuset_t cpuset;
  CPU_ZERO(&cpuset);
  const int errno_ = ::cpuset_getaffinity(CPU_LEVEL_WHICH, CPU_WHICH_TID, -1,
  ...
  if (::SetThreadAffinityMask(::GetCurrentThread(), original_affinity_mask) ==
      0) {
    piranha_throw(std::runtime_error,
                  "the call to SetThreadAffinityMask() failed");
  }
...
#else
  piranha_throw(not_implemented_error,
                "bound_proc() is not available on this platform");
#endif
}
}

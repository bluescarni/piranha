...
template <typename = int> struct base_symbol {
  static std::mutex m_mutex;
  static std::set<std::string> m_symbol_list;
};

template <typename T> std::mutex base_symbol<T>::m_mutex;

template <typename T> std::set<std::string> base_symbol<T>::m_symbol_list;
}

/// Literal symbol class.
/**
 * This class represents a symbolic variable uniquely identified by its name.
 * Symbol instances
 * are tracked in a global list for the duration of the program, so that
 * different instances of symbols with the same name are always
 * referring to the same underlying object.
 *
 * The methods of this class, unless specified otherwise, are thread-safe: it is
 * possible to create, access and operate on objects of this class
...
public:
   ...
   */
  const std::string &get_name() const { return *m_ptr; }
  /// Equality operator.
  /**
   * @param[in] other equality argument.
   *
   * @return true if \p other refers to the same underlying object as \p this,
   * false otherwise.
   */
  bool operator==(const symbol &other) const { return m_ptr == other.m_ptr; }
  /// Inequality operator.
  /**
   ...
   */
  std::size_t hash() const { return std::hash<std::string const *>()(m_ptr); }
...
template <> struct hash<piranha::symbol> {
  /// Result type.

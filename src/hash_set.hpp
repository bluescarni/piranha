...
/// Hash set.
/**
 * Hash set class with interface similar to \p std::unordered_set. The main
 ...
 * - iterators and iterator invalidation: after a rehash operation, all
 * iterators will be invalidated and existing
 *   references/pointers to the elements will also be invalid; after an
 * insertion/erase operation, all existing iterators, pointers
 *   and references to the elements in the destination bucket will be invalid,
 * - the complexity of iterator traversal depends on the load factor of the
 * table.
 *
 * The implementation employs a separate chaining strategy consisting of an
 * array of buckets, each one a singly linked list with the first node
 * stored directly within the array (so that the first insertion in a bucket
 * does not require any heap allocation).
 *
 * An additional set of low-level methods is provided: such methods are suitable
 * for use in high-performance and multi-threaded contexts,
 * and, if misused, could lead to data corruption and other unpredictable
 * errors.
 *
 * Note that for performance reasons the implementation employs sizes that are
 * powers of two. Hence, particular care should be taken
 ...
 * apart from methods involving insertion,
 * which provide the basic guarantee (after a failed insertion, the set will be
 * left in an unspecified but valid state).
 *
 * ## Move semantics ##
 *
 * Move construction and move assignment will leave the moved-from object
 * equivalent to an empty set whose hasher and
 * equality predicate have been moved-from.
 *
 * ## Serialization ##
 *
 * This class supports serialization if the contained type supports it. Note
 * that the hasher and the comparator
...
* - see if we can reduce the number of branches in the find algorithm (e.g.,
* when traversing the list) -> this should be a general review of the internal
* linked list
* implementation.
* - memory handling: the usage of the allocator object should be more standard,
* i.e., use the pointer and reference typedefs defined within, replace
* positional new with construct even in the list implementation. Then it can be
* made a template parameter with default = std::allocator.
* - use of new: we should probably replace new with new, in case new is
* overloaded -> also, check all occurrences of root new, it is used as well
* in static_vector for instance.
* - inline the first bucket, with the idea of avoiding memory allocations when
* the series consist of a single element (useful for instance
...
* - in the dtor checks, do we still want the shutdown() logic after we rework
* symbol?
*   are we still accessing potentially global variables?
* - maybe a bit more enabling for ctor and other template methods, not really
* essential though.
*/
...
class hash_set {
  ...
  template <typename U> friend class debug_access;
  ...
  struct node {
    ...
    // NOTE: the idea here is that we use these helpers only *after* the object
    // has been constructed,
    // for constructing the object we must always access m_storage directly. The
    // chain of two casts
    // seems to be the only standard-conforming way of getting out a pointer to
    // T.
    ...
    // - pointer casting to/from void * is ok (4.10.2 and 5.2.9.13, via
    // static_cast);
    // - the placement new operator (which we use to construct the object into
    // the storage)
    //   is guaranteed to construct the object at the beginning of the storage,
    //   and taking
    //   a void * out of the storage will return the address of the object
    //   stored within;
    // - the lifetime of the original POD storage_type ends when we reuse its
    // storage via
    //   placement new.
    // This page contains some more info about storage reuse:
    // http://en.cppreference.com/w/cpp/language/lifetime#Storage_reuse
    // See also this link, regarding static_cast vs reinterpret_cast for this
    // very specific
    // usage:
  ...
  // NOTE: in this list implementation the m_next pointer is used as a flag to
  // signal if the current node
  // stores an item: the pointer is not null if it does contain something. The
  // value of m_next pointer in a node is set to a constant
  // &terminator node if it is the last node of the list. I.e., the terminator
  // is end() in all cases
  // except when the list is empty (in that case the m_node member is end()).
  struct list {
    // NOTE: re: std::iterator inheritance. It's netiher encouraged not
    // discouraged according to this:
    // http://stackoverflow.com/questions/6471019/can-should-i-inherit-from-stl-iterator
    // I think we can just keep on using boost iterator_facade for the time
    // being.
    template <typename U>
    class iterator_impl
        : public boost::iterator_facade<iterator_impl<U>, U,
                                        boost::forward_traversal_tag> {
      typedef typename std::conditional<std::is_const<U>::value, node const *,
                                        node *>::type ptr_type;
      template <typename V> friend class iterator_impl;
    ...
    public:
      ...
      template <typename V,
                typename std::enable_if<
                    std::is_convertible<typename iterator_impl<V>::ptr_type,
                                        ptr_type>::value,
                    int>::type = 0>
    ...
    private:
      ...
      template <typename V> bool equal(const iterator_impl<V> &other) const {
      ...
      try {
        ...
        while (other_cur->m_next) {
          if (cur->m_next) {
            ...
            ::new (static_cast<void *>(&new_node->m_storage))
                T(*other_cur->ptr());
    ...
    void steal_from_rvalue(list &&other) {
      ...
      if (other.m_node.m_next) {
        ...
        ::new (static_cast<void *>(&m_node.m_storage))
            T(std::move(*other.m_node.ptr()));
    ...
    iterator begin() { return iterator(&m_node); }
    iterator end() { return iterator((m_node.m_next) ? &terminator : &m_node); }
    const_iterator begin() const { return const_iterator(&m_node); }
    const_iterator end() const {
      return const_iterator((m_node.m_next) ? &terminator : &m_node);
    }
    bool empty() const { return !m_node.m_next; }
    void destroy() {
      node *cur = &m_node;
      while (cur->m_next) {
      ...
      // After destruction, the list should be equivalent to a
  ...
  // Allocator type.
  // NOTE: if we move allocator choice in public interface we need to document
  // the exception behaviour of the allocator. Also, check the validity
  // of the assumptions on the type returned by allocate(): must it be a pointer
  // or just convertible to pointer?
...
private:
  ...
  using ptr_type = list *;
  // Internal pack type, containing the pointer to the objects, the hash/equal
  // functor
  // and the allocator. In many cases these are stateless so we can exploit EBCO
  // if implemented
  // in the tuple type (likely).
  using pack_type = std::tuple<ptr_type, hasher, key_equal, allocator_type>;
  // A few handy accessors.
  ptr_type &ptr() { return std::get<0u>(m_pack); }
  const ptr_type &ptr() const { return std::get<0u>(m_pack); }
  const hasher &hash() const { return std::get<1u>(m_pack); }
  const key_equal &k_equal() const { return std::get<2u>(m_pack); }
  allocator_type &allocator() { return std::get<3u>(m_pack); }
  const allocator_type &allocator() const { return std::get<3u>(m_pack); }
  // Definition of the iterator type for the set.
  template <typename Key>
  class iterator_impl
      : public boost::iterator_facade<iterator_impl<Key>, Key,
                                      boost::forward_traversal_tag> {
    friend class hash_set;
    typedef typename std::conditional<std::is_const<Key>::value, hash_set const,
                                      hash_set>::type set_type;
    typedef typename std::conditional<std::is_const<Key>::value,
  ...
  void init_from_n_buckets(const size_type &n_buckets, unsigned n_threads) {
    ...
    // Proceed to actual construction only if the requested number of buckets is
    // nonzero.
    if (!n_buckets) {
      return;
    }
    ...
    if (n_threads == 1u) {
      ...
      // NOTE: this is a noexcept operation, no need to account for rolling
      // back.
      ...
      crs_type constructed_ranges(
          static_cast<typename crs_type::size_type>(n_threads),
          std::make_pair(size_type(0u), size_type(0u)));
      if (unlikely(constructed_ranges.size() != n_threads)) {
        piranha_throw(std::bad_alloc, );
      }
      // Thread function.
      auto thread_function = [this, new_ptr, &constructed_ranges](
          const size_type &start, const size_type &end,
      ...
      try {
        for (unsigned i = 0u; i < n_threads; ++i) {
          const auto start = static_cast<size_type>(wpt * i),
                     end = static_cast<size_type>(
                         (i == n_threads - 1u) ? size : wpt * (i + 1u));
          f_list.push_back(
              thread_pool::enqueue(i, thread_function, start, end, i));
        }
        f_list.wait_all();
        // NOTE: no need to get_all() here, as we know no exceptions will be
        // generated inside thread_func.
      } catch (...) {
        // NOTE: everything in thread_func is noexcept, if we are here the
        // exception was thrown by enqueue or push_back.
  ...
  void destroy_and_deallocate() {
    // Proceed to destroy all elements and deallocate only if the set is
    // actually storing something.
    if (ptr()) {
  ...
  using insert_enabler = typename std::enable_if<
      std::is_same<key_type, typename std::decay<U>::type>::value, int>::type;
  // Run a consistency check on the set, will return false if something is
  // wrong.
  bool sanity_check() const {
    ...
    // m_log2_size must not be equal to or greater than the number of bits of
    // size_type.
  ...
  // The number of available nonzero sizes will be the number of bits in the
  // size type. Possible nonzero sizes will be in
  // the [2 ** 0, 2 ** (n-1)] range.
  static const size_type m_n_nonzero_sizes =
      static_cast<size_type>(std::numeric_limits<size_type>::digits);
  // Get log2 of set size at least equal to hint. To be used only when hint is
  // not zero.
...
public:
  ...
  /// Default constructor.
  /**
   * If not specified, it will default-initialise the hasher and the equality
   * predicate. The resulting
   * hash set will be empty.
   *
   * @param[in] h hasher functor.
   * @param[in] k equality predicate.
   *
   * @throws unspecified any exception thrown by the copy constructors of
   * <tt>Hash</tt> or <tt>Pred</tt>.
   */
  hash_set(const hasher &h = hasher{}, const key_equal &k = key_equal{})
      : m_pack(nullptr, h, k, allocator_type{}), m_log2_size(0u),
        m_n_elements(0u) {}
  /// Constructor from number of buckets.
  /**
   * Will construct a set whose number of buckets is at least equal to \p
   * n_buckets. If \p n_threads is not 1,
   * then the first \p n_threads threads from piranha::thread_pool will be used
   * concurrently for the initialisation
   * of the set.
   *
   ...
   *
   * @throws std::bad_alloc if the desired number of buckets is greater than an
   * implementation-defined maximum, or in case
   * of memory errors.
   * @throws std::invalid_argument if \p n_threads is zero.
   * @throws unspecified any exception thrown by:
   * - the copy constructors of <tt>Hash</tt> or <tt>Pred</tt>,
   * - piranha::thread_pool::enqueue() or piranha::future_list::push_back(), if
   * \p n_threads is not 1.
   */
  explicit hash_set(const size_type &n_buckets, const hasher &h = hasher{},
                    const key_equal &k = key_equal{}, unsigned n_threads = 1u)
      : m_pack(nullptr, h, k, allocator_type{}), m_log2_size(0u),
        m_n_elements(0u) {
  ...
  hash_set(const hash_set &other)
      : m_pack(nullptr, other.hash(), other.k_equal(), other.allocator()),
        m_log2_size(0u), m_n_elements(0u) {
  ...
  /// Move constructor.
  /**
   * After the move, \p other will have zero buckets and zero elements, and its
   ...
   * @throws unspecified any exception thrown by the copy constructors of
   * <tt>Hash</tt> or <tt>Pred</tt>, or arising from
   * calling insert() on the elements of the range.
   */
  template <typename InputIterator>
  explicit hash_set(const InputIterator &begin, const InputIterator &end,
                    const size_type &n_buckets = 0u, const hasher &h = hasher{},
  ...
  /**
   * Will insert() all the elements of the initializer list, ignoring the return
   ...
   * @throws std::bad_alloc if the desired number of buckets is greater than an
   * implementation-defined maximum.
   * @throws unspecified any exception thrown by either insert() or of the
   * default constructor of <tt>Hash</tt> or <tt>Pred</tt>.
   */
  template <typename U>
  explicit hash_set(std::initializer_list<U> list)
      : m_pack(nullptr, hasher{}, key_equal{}, allocator_type{}),
        m_log2_size(0u), m_n_elements(0u) {
    // We do not care here for possible truncation of list.size(), as this is
  ...
  /**
   * @return hash_set::const_iterator to the first element of the set, or end()
   * if the set is empty.
   */
  const_iterator begin() const {
    // NOTE: this could take a while in case of an empty set with lots of
    // buckets. Take a shortcut
    // taking into account the number of elements in the set - if zero, go
    // directly to end()?
  ...
  /// Const end iterator.
  /**
   * @return hash_set::const_iterator to the position past the last element of
   * the set.
   */
  const_iterator end() const {
  ...
  /// Begin iterator.
  /**
   * @return hash_set::iterator to the first element of the set, or end() if the
   * set is empty.
   */
  iterator begin() { return static_cast<hash_set const *>(this)->begin(); }
  /// End iterator.
  /**
   * @return hash_set::iterator to the position past the last element of the
   * set.
   */
  iterator end() { return static_cast<hash_set const *>(this)->end(); }
  /// Number of elements contained in the set.
  /**
   * @return number of elements in the set.
   */
  size_type size() const { return m_n_elements; }
  /// Test for empty set.
  /**
  ...
  double load_factor() const {
    const auto b_count = bucket_count();
    return (b_count)
               ? static_cast<double>(size()) / static_cast<double>(b_count)
               : 0.;
  }
  /// Index of destination bucket.
  /**
   * Index to which \p k would belong, were it to be inserted into the set. The
   * index of the
  ...
  size_type bucket(const key_type &k) const {
    if (unlikely(!bucket_count())) {
      piranha_throw(zero_division_error,
                    "cannot calculate bucket index in an empty set");
    }
    return _bucket(k);
  }
  /// Find element.
   ...
   *
   * @return hash_set::const_iterator to <tt>k</tt>'s position in the set, or
   * end() if \p k is not in the set.
   *
   ...
   * @param[in] k element to be located.
   *
   * @return hash_set::iterator to <tt>k</tt>'s position in the set, or end() if
   * \p k is not in the set.
   *
  ...
  double max_load_factor() const {
    ...
    // NOTE: if this is ever made configurable, it should never be allowed to go
    // to zero.
    return 1.;
  }
  /// Insert element.
  /**
   * \note
   * This template method is activated only if \p T and \p U are the same type,
   * aside from cv qualifications and references.
   *
   * If no other key equivalent to \p k exists in the set, the insertion is
   * successful and returns the <tt>(it,true)</tt>
   * pair - where \p it is the position in the set into which the object has
   * been inserted. Otherwise, the return value
   * will be <tt>(it,false)</tt> - where \p it is the position of the existing
   * equivalent object.
   *
   * @param[in] k object that will be inserted into the set.
   *
   * @return <tt>(hash_set::iterator,bool)</tt> pair containing an iterator to
   * the newly-inserted object (or its existing
   ...
   * - _find(),
   * - _bucket().
   * @throws std::overflow_error if a successful insertion would result in
   * size() exceeding the maximum
   * value representable by type piranha::hash_set::size_type.
   * @throws std::bad_alloc if the operation results in a resize of the set past
  ...
  std::pair<iterator, bool> insert(U &&k) {
    ...
    // Item is new. Handle the case in which we need to rehash because of load
    // factor.
    if (unlikely(static_cast<double>(m_n_elements + size_type(1u)) /
   ...
   *
   * Erasing an element invalidates all iterators pointing to elements in the
   * same bucket
   * as the erased element.
   *
   * After the operation has taken place, the size() of the set will be
   * decreased by one.
   *
   * @param[in] it iterator to the element of the set to be removed.
   *
   * @return iterator pointing to the element following \p it prior to the
   * element being erased, or end() if
  ...
  iterator erase(const_iterator it) {
    ...
    if (b_it == ptr()[it.m_idx].end()) {
      ...
      // NOTE: in case we reached the end of the container, the end() iterator
      // should be:
      // {this,bucket_count,local_iterator{}}
      // this has been set above already, bucket_count is set by retval.m_idx =
      // idx
      // and the default local_iterator ctor is called by the def ctor of
      // iterator.
    } else {
   ...
   *
   * @throws unspecified any exception thrown by swapping hasher or equality
   * predicate via \p std::swap.
   */
  ...
  /// Rehash set.
  /**
   * Change the number of buckets in the set to at least \p new_size. No rehash
   * is performed
   * if rehashing would lead to exceeding the maximum load factor. If \p
   * n_threads is not 1,
   * then the first \p n_threads threads from piranha::thread_pool will be used
   * concurrently during
   ...
   *
   * @throws std::invalid_argument if \p n_threads is zero.
   * @throws unspecified any exception thrown by the constructor from number of
   * buckets,
  ...
  void rehash(const size_type &new_size, unsigned n_threads = 1u) {
    ...
    // If rehash is requested to zero, do something only if there are no items
    // stored in the set.
    if (!new_size) {
      if (!size()) {
        clear();
    ...
    // Do nothing if rehashing to the new size would lead to exceeding the max
    // load factor.
    if (static_cast<double>(size()) / static_cast<double>(new_size) >
        max_load_factor()) {
      return;
    }
  ...
  /**
   * @return an <tt>std::map<size_type,size_type></tt> in which the key is the
   * number of elements
   * stored in a bucket and the mapped type the number of buckets containing
   * those many elements.
   *
   * @throws unspecified any exception thrown by memory errors in standard
   * containers.
   */
  ...
  /// Mutable iterator.
  /**
   * This iterator type provides non-const access to the elements of the set.
   * Please note that modifications
   * to an existing element of the set might invalidate the relation between the
   * element and its position in the set.
   * After such modifications of one or more elements, the only valid operation
   * is hash_set::clear() (destruction of the
   * set before calling hash_set::clear() will lead to assertion failures in
   * debug mode).
   */
   ...
   * aside from cv qualifications and references.
   *
   * The parameter \p bucket_idx is the index of the destination bucket for \p k
   * and, for a
   * set with a nonzero number of buckets, must be equal to the output
   * of bucket() before the insertion.
   *
   * This method will not check if a key equivalent to \p k already exists in
   * the set, it will not
   * update the number of elements present in the set after the insertion, it
   * will not resize
   ...
   * hash_set::key_type or by memory allocation
   * errors.
   */
  template <typename U, insert_enabler<U> = 0>
  ...
  /**
   * Locate element in the set. The parameter \p bucket_idx is the index of the
   * destination bucket for \p k and, for
   * a set with a nonzero number of buckets, must be equal to the output
   * of bucket() before the insertion. This method will not check if the value
   * of \p bucket_idx is correct.
   *
   * @param[in] k element to be located.
   ...
   *
   * @return index of the destination bucket for an object with hash value \p
   * hash.
   */
  ...
  /**
   * After this call, size() will return \p new_size regardless of the true
   * number of elements in the set.
   *
   * @param[in] new_size new set size.
   */
  void _update_size(const size_type &new_size) { m_n_elements = new_size; }
   ...
   *
   * @return a const reference to the list of items contained in the bucket
   * positioned
   * at index \p idx.
   */
   ...
   * as the erased element.
   *
   * This method will not update the number of elements in the set, nor it will
   * try to access elements
   * outside the bucket to which \p it refers.
   *
   * @param[in] it iterator to the element of the set to be removed.
   *
   * @return local iterator pointing to the element following \p it prior to the
   * element being erased, or local end() if
   * no such element exists.
   */
  local_iterator _erase(const_iterator it) {
    ...
    // If the pointed-to element is the first one in the bucket, we need special
    // care.
    if (&*it == &*bucket.m_node.ptr()) {
      ...
      if (bucket.m_node.m_next == &bucket.terminator) {
        ...
        ::new (static_cast<void *>(&bucket.m_node.m_storage))
            T(std::move(*bucket.m_node.m_next->ptr()));
      ...
      // It will now point to the item past the erased one or to the local
      // end().
      return ++prev_b_it;
    }
  }
...
typename hash_set<T, Hash, Pred>::node
    hash_set<T, Hash, Pred>::list::terminator;

template <typename T, typename Hash, typename Pred>
const typename hash_set<T, Hash, Pred>::size_type
    hash_set<T, Hash, Pred>::m_n_nonzero_sizes;
}

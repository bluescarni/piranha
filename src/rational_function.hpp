...
/// Rational function.
/**
 * This class represents rational functions, that is, mathematical objects of
 * the form
 * \f[
 * \frac{f\left(x_0,x_1,\ldots\right)}{g\left(x_0,x_1,\ldots\right)},
 * \f]
 * where \f$f\left(x_0,x_1,\ldots\right)\f$ and
 * \f$g\left(x_0,x_1,\ldots\right)\f$ are polynomials in the variables
 * \f$x_0,x_1,\ldots\f$ over \f$\mathbb{Z}\f$.
 * The monomial representation is determined by the \p Key template parameter.
 * Only monomial types with integral exponents are allowed; signed exponent
 * types are allowed, but if a negative exponent is ever generated or
 * encountered while operating on a rational function an error will be produced.
 *
 * Internally, a piranha::rational_function consists of a numerator and a
 * denominator represented as piranha::polynomial with piranha::integer
 * coefficients.
 * Rational functions are always kept in a canonical form defined by the
 ...
 * - piranha::integer,
 * - piranha::rational,
 * - the polynomial type representing numerator and denominator (that is,
 * piranha::rational_function::p_type) and its
 *   counterpart with rational coefficients (that is,
 * piranha::rational_function::q_type).
 *
 * ## Type requirements ##
 *
 * \p Key must be usable as second template parameter for piranha::polynomial,
 * and the exponent type must be a C++ integral type or piranha::integer.
 *
 * ## Exception safety guarantee ##
 *
 * Unless noted otherwise, this class provides the strong exception safety
 * guarantee.
 *
 * ## Move semantics ##
 *
 * Move operations will leave objects of this class in a state which is
...
class rational_function : public detail::rational_function_tag {
  ...
  template <typename T>
  using is_integral =
      std::integral_constant<bool, std::is_same<integer, T>::value ||
                                       std::is_integral<T>::value>;
  PIRANHA_TT_CHECK(detail::is_polynomial_key, Key);
  static_assert(is_integral<typename Key::value_type>::value,
                "The exponent type must be an integral type.");
  // Shortcut from C++14.
...
private:
  ...
  using is_interoperable =
      std::integral_constant<bool, is_integral<T>::value ||
                                       ...
                                       std::is_same<T, p_type>::value ||
                                       std::is_same<T, q_type>::value>;
  // Canonicalisation.
  static std::pair<p_type, p_type> canonicalise_impl(const p_type &n,
                                                     const p_type &d) {
    ...
    if (unlikely(math::is_zero(d))) {
      piranha_throw(zero_division_error,
                    "null denominator in rational function");
    }
    // If the numerator is null, return {0,1}.
    if (math::is_zero(n)) {
      return std::make_pair(p_type{}, p_type{1});
    }
    // NOTE: maybe these checks should go directly into the poly GCD routine.
    // Keep it
    // in mind for the future.
    // NOTE: it would make sense here to deal with the single coefficient
    // denominator case as well.
    // This should give good performance when one is using rational_function as
    ...
    auto retval = std::make_pair(std::move(std::get<1u>(tup_res)),
                                 std::move(std::get<2u>(tup_res)));
    // Check if we need to adjust the sign of the leading term
    // of the denominator.
    // NOTE: here we ask for the lterm into something that potentially might be
    // zero
  ...
  // Utility function to convert a q_type to a pair (p_type,integer),
  ...
  using pzs_enabler =
      std::integral_constant<bool,
                             std::is_same<decay_t<T>, p_type>::value ||
  ...
  template <typename T,
            typename std::enable_if<pzs_enabler<T>::value, int>::type = 0>
  ...
  using unary_ctor_enabler = typename std::enable_if<
      ...
      detail::true_tt<decltype(std::declval<U &>().dispatch_unary_ctor(
          std::declval<const decay_t<T> &>()))>::value,
      int>::type;
  // Binary ctor enabler.
  template <typename T, typename U, typename V>
  using binary_ctor_enabler =
      typename std::enable_if<std::is_constructible<V, T &&>::value &&
                                  std::is_constructible<V, U &&>::value,
                              int>::type;
  // Enabler for generic assignment.
  // Here we are re-using the enabler for generic unary construction. This
  ...
  // Addition.
  static rational_function dispatch_binary_add(const rational_function &a,
                                               const rational_function &b) {
    ...
    if (uda && udb) {
      ...
      // NOTE: if the canonicalisation fails for some reason, it is not a
      // problem
      // as retval is a local variable that will just be destroyed. The
      // destructor
  ...
  template <typename T,
            typename std::enable_if<is_interoperable<T>::value, int>::type = 0>
  static rational_function dispatch_binary_add(const rational_function &a,
                                               const T &b) {
  ...
  template <typename T,
            typename std::enable_if<is_interoperable<T>::value, int>::type = 0>
  static rational_function dispatch_binary_add(const T &a,
                                               const rational_function &b) {
    return rational_function{a} + b;
  }
  template <typename T, typename U>
  using binary_add_enabler = typename std::enable_if<
      detail::true_tt<
          // NOTE: here the rational_function:: specifier is apprently needed
          // only in GCC, probably a bug.
          decltype(rational_function::dispatch_binary_add(
              std::declval<const decay_t<T> &>(),
              std::declval<const decay_t<U> &>()))>::value,
      int>::type;
  template <typename T, typename U>
  using in_place_add_enabler = typename std::enable_if<
      detail::true_tt<decltype(std::declval<const decay_t<T> &>() +
                               std::declval<const decay_t<U> &>())>::value,
      int>::type;
  // Subtraction.
  static rational_function dispatch_binary_sub(const rational_function &a,
                                               const rational_function &b) {
    const bool uda = math::is_unitary(a.den()), udb = math::is_unitary(b.den());
    rational_function retval;
    if (uda && udb) {
  ...
  template <typename T,
            typename std::enable_if<is_interoperable<T>::value, int>::type = 0>
  static rational_function dispatch_binary_sub(const T &a,
                                               const rational_function &b) {
    return rational_function{a} - b;
  }
  template <typename T, typename U>
  using binary_sub_enabler = typename std::enable_if<
      detail::true_tt<decltype(rational_function::dispatch_binary_sub(
          std::declval<const decay_t<T> &>(),
          std::declval<const decay_t<U> &>()))>::value,
      int>::type;
  template <typename T, typename U>
  using in_place_sub_enabler = typename std::enable_if<
      ...
      int>::type;
  // Multiplication.
  static rational_function dispatch_binary_mul(const rational_function &a,
                                               const rational_function &b) {
    const bool uda = math::is_unitary(a.den()), udb = math::is_unitary(b.den());
    rational_function retval;
    if (uda && udb) {
  ...
  template <typename T,
            typename std::enable_if<is_interoperable<T>::value, int>::type = 0>
  static rational_function dispatch_binary_mul(const T &a,
                                               const rational_function &b) {
    return rational_function{a} * b;
  }
  template <typename T, typename U>
  using binary_mul_enabler = typename std::enable_if<
      detail::true_tt<decltype(rational_function::dispatch_binary_mul(
          std::declval<const decay_t<T> &>(),
          std::declval<const decay_t<U> &>()))>::value,
      int>::type;
  template <typename T, typename U>
  using in_place_mul_enabler = typename std::enable_if<
      ...
      int>::type;
  // Division.
  static rational_function dispatch_binary_div(const rational_function &a,
                                               const rational_function &b) {
    // NOTE: division is like a multiplication with inverted second argument, so
    // we need
  ...
  template <typename T,
            typename std::enable_if<is_interoperable<T>::value, int>::type = 0>
  static rational_function dispatch_binary_div(const T &a,
                                               const rational_function &b) {
    return rational_function{a} / b;
  }
  template <typename T, typename U>
  using binary_div_enabler = typename std::enable_if<
      detail::true_tt<decltype(rational_function::dispatch_binary_div(
          std::declval<const decay_t<T> &>(),
          std::declval<const decay_t<U> &>()))>::value,
      int>::type;
  template <typename T, typename U>
  using in_place_div_enabler = typename std::enable_if<
      ...
      int>::type;
  // Comparison.
  static bool dispatch_equality(const rational_function &a,
                                const rational_function &b) {
  ...
  template <typename T, typename U>
  using eq_enabler = typename std::enable_if<
      detail::true_tt<decltype(rational_function::dispatch_equality(
  ...
  using has_special_subs =
      std::integral_constant<bool,
                             is_interoperable<T>::value ||
                                 std::is_same<T, rational_function>::value>;
  // Type resulting from the normal substitution method.
  template <typename T>
  using normal_subs_type =
      decltype(math::subs(std::declval<const p_type &>(), std::string{},
                          std::declval<const T &>()) /
               math::subs(std::declval<const p_type &>(), std::string{},
                          std::declval<const T &>()));
  template <typename T>
  ...
  template <typename T,
            typename std::enable_if<has_special_subs<T>::value, int>::type = 0>
  static rational_function subs_impl(const rational_function &r,
                                     const std::string &name, const T &x) {
    return rational_function{math::subs(r.num(), name, x),
                             math::subs(r.den(), name, x)};
  }
  template <typename T,
            typename std::enable_if<has_normal_subs<T>::value, int>::type = 0>
  static normal_subs_type<T> subs_impl(const rational_function &r,
                                       const std::string &name, const T &x) {
    return math::subs(r.num(), name, x) / math::subs(r.den(), name, x);
  }
  template <typename T>
  using subs_type =
      decltype(subs_impl(std::declval<const rational_function &>(),
                         std::string{}, std::declval<const T &>()));
  // ipow_subs().
  template <typename T> using has_special_ipow_subs = has_special_subs<T>;
  // Type resulting from the normal substitution method.
  template <typename T>
  using normal_ipow_subs_type =
      decltype(math::ipow_subs(std::declval<const p_type &>(), std::string{},
                               integer{}, std::declval<const T &>()) /
               math::ipow_subs(std::declval<const p_type &>(), std::string{},
                               integer{}, std::declval<const T &>()));
  template <typename T>
  using has_normal_ipow_subs = std::integral_constant<
      bool, !has_special_ipow_subs<T>::value &&
                is_returnable<normal_ipow_subs_type<T>>::value>;
  template <typename T, typename std::enable_if<has_special_ipow_subs<T>::value,
                                                int>::type = 0>
  static rational_function ipow_subs_impl(const rational_function &r,
                                          const std::string &name,
                                          const integer &n, const T &x) {
    return rational_function{math::ipow_subs(r.num(), name, n, x),
                             math::ipow_subs(r.den(), name, n, x)};
  }
  template <typename T, typename std::enable_if<has_normal_ipow_subs<T>::value,
                                                int>::type = 0>
  static normal_ipow_subs_type<T> ipow_subs_impl(const rational_function &r,
                                                 const std::string &name,
                                                 const integer &n, const T &x) {
    return math::ipow_subs(r.num(), name, n, x) /
           math::ipow_subs(r.den(), name, n, x);
  }
  template <typename T>
  using ipow_subs_type = decltype(
      ipow_subs_impl(std::declval<const rational_function &>(), std::string{},
                     integer{}, std::declval<const T &>()));
  ...
  template <class Archive> void save(Archive &ar, unsigned int) const {
    ...
    // into local variables and the move them in. So if something goes wrong in
    // the
  ...
  using pow_map_type =
      std::unordered_map<RF, std::vector<RF>, rf_hasher, rf_equal_to>;
  ...
  template <typename T>
  using cp_map_type =
      std::unordered_map<std::string, std::function<T(const T &)>>;
  ...
  using custom_partial_enabler = typename std::enable_if<
      std::is_constructible<
          std::function<rational_function(const rational_function &)>,
          F>::value,
      int>::type;
...
public:
   ...
   * rational_function::p_type from \p int.
   */
  ...
  /**
   * \note
   * This constructor is enabled if the decay type of \p T is either an
   * interoperable type or a string type.
   *
   * The constructor operates as follows:
   * - if \p T is rational_function::p_type, a C++ integral type,
   * piranha::integer or a string type, then the numerator
   *   is constructed from \p x, and the denominator is set to 1;
   * - if \p T is piranha::rational, then the numerator is constructed from the
   * numerator of \p x, and the denominator
   *   from the denominator of \p x;
   * - if \p T is rational_function::q_type, then, after construction, \p this
   * will be mathematically equivalent to \p x (that is, the coefficients
   *   of \p x are multiplied by their LCM and the result is used to construct
   * the numerator, and the denominator is constructed from the LCM
   *   of the coefficients).
   *
   * @param[in] x construction argument.
   *
   * @throws std::invalid_argument if, when constructing from
   * rational_function::p_type or rational_function::q_type, a negative exponent
   * is
   * encountered.
   * @throws unspecified any exception thrown by:
   * - the constructors and the assignment operators of
   ...
   */
  template <typename T, typename U = rational_function,
            unary_ctor_enabler<T, U> = 0>
  ...
  /**
   * \note
   * This constructor is enabled only if piranha::rational_function can be
   * constructed from \p T and \p U.
   *
   * The body of this constructor is equivalent to:
   * @code
   * *this = rational_function(std::forward<T>(x)) /
   * rational_function(std::forward<U>(y));
   * @endcode
   *
   ...
   */
  template <typename T, typename U, typename V = rational_function,
            binary_ctor_enabler<T, U, V> = 0>
  ...
  /**
   * \note
   * This operator is enabled only if the corresponding generic unary
   * constructor is enabled.
   *
   * The operator is implemented as the assignment from a
   * piranha::rational_function constructed
   * from \p x.
   *
   ...
   */
  template <typename T, typename U = rational_function,
            generic_assignment_enabler<T, U> = 0>
  rational_function &operator=(T &&x) {
    // Check this is never invoked when T is a rational function.
    static_assert(!std::is_base_of<rational_function, decay_t<T>>::value,
                  "Type error.");
    return *this = rational_function(std::forward<T>(x));
  }
  /// Trivial destructor.
  ~rational_function() { PIRANHA_TT_CHECK(is_cf, rational_function); }
  /// Canonicalisation.
  /**
   * This method will put \p this in canonical form. Normally, it is never
   * necessary to call this method,
   * unless low-level methods that do not keep \p this in canonical form have
   * been invoked.
   *
   ...
   * @throws std::invalid_argument if either the numerator or the denominator
   * have a negative exponent.
   * @throws unspecified any exception thrown by construction, division and GCD
   * operations involving objects of
   ...
   */
  const p_type &num() const { return m_num; }
  /// Denominator.
  /**
   * @return a const reference to the denominator.
   */
  const p_type &den() const { return m_den; }
  /// Stream operator.
  /**
   ...
   *
   * @throws unspecified any exception thrown by the stream opertor of
   * rational_function::p_type.
   */
  friend std::ostream &operator<<(std::ostream &os,
                                  const rational_function &r) {
    if (math::is_zero(r.m_num)) {
      ...
      if (r.m_den.is_single_coefficient() ||
          (r.m_den.size() == 1u &&
           math::is_unitary(r.m_den._container().begin()->m_cf) &&
           integer{r.m_den.degree()} == 1)) {
        // If the denominator is a single coefficient or it has a single term
        // with unitary coefficient
        // and degree 1 (that is, it is of the form "x"), don't print the
        // brackets.
        os << r.m_den;
      } else {
        os << '(' << r.m_den << ')';
  ...
  void print_tex(std::ostream &os) const {
    if (math::is_zero(*this)) {
      ...
      // by copy + in-place negation, and thus the order of the terms is
      // preserved.
   ...
   */
  p_type &_num() { return m_num; }
  /// Mutable reference to the denominator.
  /**
   * @return a mutable reference to the denominator.
   */
  p_type &_den() { return m_den; }
  //@}
  /// Canonicality check.
  /**
   * This method will return \p true if \p this is in canonical form, \p false
   * otherwise.
   * Unless low-level methods are used, non-canonical rational functions can be
   * generated only
   * by move operations (after which the moved-from object is left with zero
  ...
  /**
   * \note
   * This operator is enabled only if one of the arguments is
   * piranha::rational_function
   * and the other argument is either piranha::rational_function or an
   * interoperable type.
   *
   * The numerator and denominator of \p a and \p b will be compared (after any
   ...
   *
   * @return \p true if the numerator and denominator of \p this are equal to
   * the numerator and
   * denominator of \p other, \p false otherwise.
   *
   * @throws unspecified any exception thrown by the generic unary constructor,
   * if a
   ...
   * @throws unspecified any exception thrown by the copy constructor.
   */
  rational_function operator+() const { return *this; }
  /// Binary addition.
  /**
   * \note
   * This operator is enabled only if one of the arguments is
   ...
   *
   * @throws unspecified any exception thrown by:
   * - construction, assignment, multiplication, addition of
   ...
   * @return a reference to \p this.
   *
   * @throws unspecified any exception thrown by the corresponding binary
   * operator.
   */
  template <typename T, typename U = rational_function,
            in_place_add_enabler<T, U> = 0>
  rational_function &operator+=(T &&other) {
    // NOTE: *this + other will always return rational_function.
    // NOTE: we should consider in the future std::move(*this) +
    // std::forward<T>(other) to potentially improve performance,
   ...
   *
   * @throws unspecified any exception thrown by:
   * - construction, assignment, multiplication, subtraction of
   * piranha::rational_function::p_type,
   * - the generic unary constructor of piranha::rational_function,
   * - canonicalise().
   */
   ...
   *
   * @throws unspecified any exception thrown by:
   * - construction, assignment, multiplication of
   * piranha::rational_function::p_type,
   * - the generic unary constructor of piranha::rational_function,
   * - canonicalise().
   */
  ...
  /**
   * \note
   * This method is enabled only if \p T is a C++ integral type or
   * piranha::integer.
   *
   * Similarly to piranha::series::pow(), this method keeps a cache of natural
   * powers of
   * rational functions in order to expedite computations when the same powers
   * of rational
   * functions are requested repeatedly (e.g., during substitution operations).
   * The cache is
   * thread-safe and it can be cleared with
   * piranha::rational_function::clear_pow_cache().
   *
   * @param[in] n an integral exponent.
   *
   * @return \p this raised to the power of \p n.
   *
   * @throws piranha::zero_division_error if \p n is negative and \p this is
   * zero.
   * @throws unspecified any exception thrown by:
   * - threading primitives,
   * - memory errors in standard containers,
   * - construction, assignment and arithmetic operations on
   * piranha::rational_function.
   */
  ...
  rational_function pow(const T &n) const {
    ...
    if (n_int.sign() < 0) {
      if (unlikely(math::is_zero(retval.m_num))) {
        piranha_throw(zero_division_error,
                      "zero denominator in rational_function exponentiation");
      }
      // Swap num/den.
   ...
   * maintained by piranha::rational_function::pow().
   *
   ...
   * - the expression <tt>math::subs(num(),name,x) /
   * math::subs(den(),name,x)</tt>
   *   is well-formed, yielding a type that satisfies piranha::is_returnable.
   *
   * If \p T is an interoperable type or piranha::rational_function, the body of
   * this method is equivalent to:
   * @code
   * return
   * rational_function{math::subs(num(),name,x),math::subs(den(),name,x)};
   * @endcode
   ...
   * This method is enabled only if either:
   * - \p T is an interoperable type or piranha::rational_function, or
   * - the expression <tt>math::ipow_subs(num(),name,n,x) /
   * math::ipow_subs(den(),name,n,x)</tt>
   *   is well-formed, yielding a type that satisfies piranha::is_returnable.
   *
   * If \p T is an interoperable type or piranha::rational_function, the body of
   * this method is equivalent to:
   * @code
   * return
   * rational_function{math::ipow_subs(num(),name,n,x),math::ipow_subs(den(),name,n,x)};
   * @endcode
   * Otherwise, the body of this method is equivalent to:
   * @code
   ...
   * @param[in] x substitution argument.
   *
   * @return the result of substituting \p name to the power of \p n with \p x
   * in \p this.
   *
   * @throws unspecified any exception thrown by:
   ...
   */
  template <typename T>
  ipow_subs_type<T> ipow_subs(const std::string &name, const integer &n,
                              const T &x) const {
    return ipow_subs_impl(*this, name, n, x);
  }
  /// Trim.
  /**
   * This method will return a copy of \p this whose numerator and denominator
   * have been generated
   * by calling piranha::series::trim() on the numerator and denominator of \p
   * this.
   *
  ...
  /// Identical check.
  /**
   * Two rational functions are identical if their numerators and denominators
   * are, via
   ...
   * \p func will be used to compute the partial derivative of
   * piranha::rational_function with respect to
  ...
  /// Partial derivative.
  /**
   * The result is computed via the quotient rule. Internally, this method will
   * call
   * piranha::math::partial() on the numerator and denominator of \p this.
   *
   * @param[in] name name of the variable with respect to which to
   * differentiate.
   *
  ...
  rational_function partial(const std::string &name) const {
    using math::partial;
    return rational_function{partial(num(), name) * den() -
                                 num() * partial(den(), name),
                             den() * den()};
  }
...
template <typename Key> std::mutex rational_function<Key>::s_pow_mutex;

template <typename Key> std::mutex rational_function<Key>::s_cp_mutex;

/// Specialisation of piranha::print_tex_coefficient() for
/// piranha::rational_function.
/**
 * This specialisation is enabled if \p T is an instance of
 * piranha::rational_function.
 */
template <typename T>
struct print_tex_coefficient_impl<
    T, typename std::enable_if<
           std::is_base_of<detail::rational_function_tag, T>::value>::type> {
  /// Call operator.
  /**
   ...
   *
   * @throws unspecified any exception thrown by
   * piranha::rational_function::print_tex().
   */
  void operator()(std::ostream &os, const T &r) const { r.print_tex(os); }
};
 ...
 */
template <typename T>
struct is_zero_impl<T, typename std::enable_if<std::is_base_of<
                           detail::rational_function_tag, T>::value>::type> {
  /// Call operator.
  /**
   * @param[in] r the piranha::rational_function to be tested.
   *
   * @return the result of calling piranha::math::is_zero() on the numerator of
   * \p r.
   */
  bool operator()(const T &r) const { return is_zero(r.num()); }
};
 ...
 */
template <typename T, typename U>
struct pow_impl<T, U, typename std::enable_if<std::is_base_of<
                          detail::rational_function_tag, T>::value>::type> {
private:
  template <typename V>
  using pow_enabler = typename std::enable_if<
...
public:
  ...
  /**
   * \note
   * The call operator is enabled only if <tt>return r.pow(n)</tt> is a valid
   * expression.
   *
 ...
 */
template <typename T, typename U>
struct subs_impl<T, U, typename std::enable_if<std::is_base_of<
                           detail::rational_function_tag, T>::value>::type> {
private:
  template <typename V>
  using subs_type = decltype(
      std::declval<const T &>().subs(std::string{}, std::declval<const V &>()));

public:
  /// Call operator.
  /**
   * \note
   * The call operator is enabled only if <tt>return r.subs(s,x)</tt> is a valid
   * expression.
   *
   ...
   *
   * @throws unspecified any exception thrown by
   * piranha::rational_function::subs().
   */
  template <typename V>
...
private:
  template <typename V>
  using ipow_subs_type = decltype(std::declval<const T &>().ipow_subs(
      std::string{}, integer{}, std::declval<const V &>()));

public:
  /// Call operator.
  /**
   * \note
   * The call operator is enabled only if <tt>return r.ipow_subs(s,n,x)</tt> is
   * a valid expression.
   *
   * @param[in] r the piranha::rational_function argument.
   * @param[in] s the name of the variable to be substituted.
   ...
   *
   * @throws unspecified any exception thrown by
   * piranha::rational_function::ipow_subs().
   */
  template <typename V>
  ipow_subs_type<V> operator()(const T &r, const std::string &s,
                               const integer &n, const V &x) const {
...
template <typename T>
using rf_partial_enabler = typename std::enable_if<
    std::is_base_of<rational_function_tag, T>::value>::type;
}
 ...
 * piranha::rational_function.
 */
template <typename T> struct partial_impl<T, detail::rf_partial_enabler<T>> {
  /// Call operator.
  /**
   * The call operator will first check whether a custom partial derivative for
   * \p T was registered
   * via piranha::rational_function::register_custom_derivative(). In such a
   * case, the custom derivative function will be used
   * to compute the return value. Otherwise, the output of
   * piranha::rational_function::partial() will be returned.
   *
...
/// Specialisation of the piranha::math::integrate() functor for
/// piranha::rational_function.
/**
 * This specialisation is enabled if \p T is an instance of
 * piranha::rational_function.
 */
template <typename T>
struct integrate_impl<T, typename std::enable_if<std::is_base_of<
                             detail::rational_function_tag, T>::value>::type> {
  /// Call operator.
  /**
   * The integration will be successful only if the denominator of \p r does not
   ...
   *
   * @throws std::invalid_argument if the denominator of \p r depends on the
   * integration variable.
   * @throws unspecified any exception thrown by:
   * - piranha::math::integrate(),
   * - the constructor of piranha::rational_function::q_type from
   * piranha::rational_function::p_type,
   * - the binary constructor of piranha::rational_function.
   */
  T operator()(const T &r, const std::string &name) {
    if (!is_zero(degree(r.den(), {name}))) {
      piranha_throw(std::invalid_argument,
                    "cannot compute the integral of a rational function whose "
...
/// Specialisation of the piranha::math::evaluate() functor for
/// piranha::rational_function.
/**
 * This specialisation is enabled if \p T is an instance of
 * piranha::rational_function.
...
private:
  ...
  using eval_type_ = decltype(
      evaluate(std::declval<const T &>().num(),
               std::declval<const std::unordered_map<std::string, V> &>()) /
      evaluate(std::declval<const T &>().den(),
               std::declval<const std::unordered_map<std::string, V> &>()));
  template <typename V>
  using eval_type = typename std::enable_if<is_returnable<eval_type_<V>>::value,
                                            eval_type_<V>>::type;
...
public:
  ...
  /**
   * \note
   * This operator is enabled only if the algorithm described below is supported
   * by the involved types,
   * and it returns a type which satisfies piranha::is_returnable.
   *
   * The evaluation of a rational function is constructed from the ratio of the
   * evaluation of its numerator
   * by the evaluation of its denominator. The return type is the type resulting
   * from this operation.
   *
   * @param[in] r the piranha::rational_function argument.
   ...
   * - the division of the results of the evaluations of numerator and
   * denominator.
   */
  template <typename V>
  eval_type<V> operator()(const T &r,
                          const std::unordered_map<std::string, V> &m) const {
    return evaluate(r.num(), m) / evaluate(r.den(), m);
  }
};

/// Specialisation of the piranha::math::cos() functor for
/// piranha::rational_function.
/**
 * This specialisation is enabled if \p T is an instance of
 * piranha::rational_function.
 */
template <typename T>
struct cos_impl<T, typename std::enable_if<std::is_base_of<
                       detail::rational_function_tag, T>::value>::type> {
  /// Call operator.
  /**
   * @param[in] r the piranha::rational_function argument.
   ...
   * @throws unspecified any exception thrown by the constructor of
   * piranha::rational_function from \p int.
   */
  T operator()(const T &r) const {
    if (!is_zero(r)) {
      piranha_throw(std::invalid_argument,
                    "cannot compute the cosine of a nonzero rational function");
    }
    return T{1};
  }
};

/// Specialisation of the piranha::math::sin() functor for
/// piranha::rational_function.
/**
 * This specialisation is enabled if \p T is an instance of
 * piranha::rational_function.
 */
template <typename T>
struct sin_impl<T, typename std::enable_if<std::is_base_of<
                       detail::rational_function_tag, T>::value>::type> {
  /// Call operator.
  /**
   * @param[in] r the piranha::rational_function argument.
  ...
  T operator()(const T &r) const {
    if (!is_zero(r)) {
      piranha_throw(std::invalid_argument,
                    "cannot compute the sine of a nonzero rational function");
    }
    return T{};
  }
};

/// Specialisation of the piranha::math::degree() functor for
/// piranha::rational_function.
/**
 * This specialisation is enabled if \p T is an instance of
 * piranha::rational_function.
 */
template <typename T>
struct degree_impl<T, typename std::enable_if<std::is_base_of<
                          detail::rational_function_tag, T>::value>::type> {
  /// Call operator.
  /**
   * The (partial) degree of a rational function is defined as the maximum
   * (partial)
   * degree of numerator and denominator.
   *
   * @param[in] r the piranha::rational_function argument.
   * @param[in] args either an empty pack, or a univariate pack containing a
   * vector
  ...
  auto operator()(const T &r, const Args &... args) const
      -> decltype(degree(r.num(), args...)) {
    // NOTE: very important here, std::max returns a reference, so in the
    // decltype()
...
/// Specialisation of the piranha::math::divexact() functor for
/// piranha::rational_function.
/**
 * This specialisation is enabled if \p T is an instance of
 * piranha::rational_function.
 */
template <typename T>
struct divexact_impl<T, typename std::enable_if<std::is_base_of<
                            detail::rational_function_tag, T>::value>::type> {
  /// Call operator.
  /**
   ...
   *
   * @throws unspecified any exception thrown by
   * piranha::rational_function::operator/().
   */
  void operator()(T &retval, const T &n, const T &d) const { retval = n / d; }
};
}
...
template <typename T>
using sri_rf_enabler = typename std::enable_if<std::is_base_of<
    detail::rational_function_tag, typename std::decay<T>::type>::value>::type;
}

/// Specialisation of piranha::series_recursion_index for
/// piranha::rational_function.
/**
 * This specialisation is enabled when \p T is an instance of
 * piranha::rational_function.
 * Although piranha::rational_function is not a piranha::series, it is assigned
 * a recursion index
 * of 1 in order to provide useful type coercion in a number of situations
 * (e.g., adding
 * a Poisson series over rational functions to a polynomial).
 */
// NOTE: this was prompted by the necessity of allowing subs with generic
// objects in subs() and
// ipow_subs(). It seems to work ok and it does not seem to conflict with the
// general series operators
// (which is the only place where the recursion index is used), but we should
// tread carefully:
// rational_function is not a series and does not provide any term/cf/key types
// and the likes.
// The logic in the series operators is SFINAE friendly and does not error out
// due to the lack
// of these typedefs.
template <typename T>

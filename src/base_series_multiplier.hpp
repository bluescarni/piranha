...
struct base_series_multiplier_impl {
  ...
  template <typename Term,
            typename std::enable_if<
                !is_less_than_comparable<typename Term::key_type>::value,
                int>::type = 0>
  void fill_term_pointers(const container_type &c1, const container_type &c2,
                          std::vector<Term const *> &v1,
                          std::vector<Term const *> &v2) {
    // If the key is not less-than comparable, we can only copy over the
    // pointers as they are.
    std::transform(c1.begin(), c1.end(), std::back_inserter(v1),
                   [](const term_type &t) { return &t; });
    std::transform(c2.begin(), c2.end(), std::back_inserter(v2),
                   [](const term_type &t) { return &t; });
  }
  template <typename Term,
            typename std::enable_if<
  ...
  void fill_term_pointers(const container_type &c1, const container_type &c2,
    ...
    // Threading functor.
    auto thread_func = [n_threads](unsigned thread_idx, const container_type *c,
      ...
      // End index.
      const auto end = static_cast<c_size_type>(
      ...
      v_size_type j = 0u;
      for (auto start = static_cast<c_size_type>(bpt * thread_idx); start < end;
           ++start) {
    ...
    auto thread_wrapper = [&thread_func, n_threads](
        const container_type *c, std::vector<term_type const *> *v) {
      // In the multi-threaded case, each thread needs to work on a separate
      // vector.
      ...
      try {
        for (unsigned i = 0u; i < n_threads; ++i) {
          ff_list.push_back(thread_pool::enqueue(
              i, thread_func, i, c, &(vv[static_cast<vv_size_t>(i)])));
        }
...
struct base_series_multiplier_impl<
    Series, Derived, typename std::enable_if<is_mp_rational<
  ...
  void fill_term_pointers(const container_type &c1, const container_type &c2,
    ...
    for (auto it = c1.begin(); it != it_f; ++it) {
      ...
      m_terms1.push_back(term_type(
          rat_type(m_lcm / it->m_cf.den() * it->m_cf.num(), int_type(1)),
          it->m_key));
    }
    it_f = c2.end();
    for (auto it = c2.begin(); it != it_f; ++it) {
      m_terms2.push_back(term_type(
          rat_type(m_lcm / it->m_cf.den() * it->m_cf.num(), int_type(1)),
          it->m_key));
    }
    // Copy over the pointers.
    std::transform(m_terms1.begin(), m_terms1.end(), std::back_inserter(v1),
                   [](const term_type &t) { return &t; });
    std::transform(m_terms2.begin(), m_terms2.end(), std::back_inserter(v2),
                   [](const term_type &t) { return &t; });
...
/**
 * This is a class that provides functionality useful to define a
 * piranha::series_multiplier specialisation. Note that this class
 * alone does *not* fulfill the requirements of a piranha::series_multiplier -
 * it merely provides a set of utilities that can be
...
// - optimisation in case one series has 1 term with unitary key and both series
// same type: multiply directly coefficients;
// - optimisation for coefficient series that merges all args, similar to the
// rational optimisation;
// - optimisation for load balancing similar to the poly multiplier.
template <typename Series>
class base_series_multiplier : private detail::base_series_multiplier_impl<
                                   Series, base_series_multiplier<Series>> {
  PIRANHA_TT_CHECK(is_series, Series);
  // Make friends with the base, so it can access protected/private members of
  // this.
  friend struct detail::base_series_multiplier_impl<
      Series, base_series_multiplier<Series>>;
...
private:
  ...
  struct default_limit_functor {
    default_limit_functor(const base_series_multiplier &m)
        : m_size2(m.m_v2.size()) {}
    size_type operator()(const size_type &) const { return m_size2; }
    const size_type m_size2;
  };
  // The purpose of this helper is to move in a coefficient series during
  // insertion. For series,
  // we know that moves leave the series in a valid state, and series
  // multiplications do not benefit
  // from an already-constructed destination - hence it is convenient to move
  // them rather than copy.
  template <typename Term,
            typename std::enable_if<!is_series<typename Term::cf_type>::value,
                                    int>::type = 0>
  ...
  template <typename T,
            typename std::enable_if<
                detail::is_mp_rational<typename T::term_type::cf_type>::value,
                int>::type = 0>
  void finalise_impl(T &s) const {
    ...
    // NOTE: this has to be the square of the lcm, as in addition to
    // uniformising
    ...
    // Buckets per thread.
    const bucket_size_type bpt =
        static_cast<bucket_size_type>(container.bucket_count() / m_n_threads);
    auto thread_func = [l2, &container, this, bpt](unsigned t_idx) {
      ...
      const bucket_size_type end_idx =
          t_idx == (this->m_n_threads - 1u)
...
public:
  /// Constructor.
  /**
   * This constructor will store in the base_series_multiplier::m_v1 and
   * base_series_multiplier::m_v2 protected members
   * references to the terms of the input series \p s1 and \p s2. \p m_v1 will
   * store references to the larger series,
   * \p m_v2 will store references to the smaller serier. The constructor will
   * also store a copy of the symbol set of
   * \p s1 and \p s2 in the protected member base_series_multiplier::m_ss.
   *
   * If the coefficient type of \p Series is an instance of
   * piranha::mp_rational, then the pointers in \p m_v1 and \p m_v2
   * will refer not to the original terms in \p s1 and \p s2 but to *copies* of
   * these terms, in which all coefficients
   * have unitary denominator and the numerators have all been multiplied by the
   * global least common multiplier. This transformation
   * allows to reduce the multiplication of series with rational coefficients to
   * the multiplication of series with integral coefficients.
   *
  ...
  explicit base_series_multiplier(const Series &s1, const Series &s2)
    ...
    // This is just an optimisation, no troubles if there is a truncation due to
    // static_cast.
...
protected:
  ...
  /**
   * \note
   * If \p MultFunctor or \p LimitFunctor do not satisfy the requirements
   *outlined below,
   ...
   * @endcode
   *
   * \p mf must be a function object with a call operator accepting two
   *instances of
   * base_series_multiplier::size_type and returning \p void. \p lf must be a
   *function object
   * with a call operator accepting and returning a
   *base_series_multiplier::size_type.
   *
   * Internally, the double loops is decomposed in blocks of size
   *tuning::get_multiplication_block_size() in an attempt
   * to optimise cache memory access patterns.
   *
   * This method is meant to be used for series multiplication. \p mf is
   *intended to be a function object that multiplies
   * the <tt>i</tt>-th term of the first series by the <tt>j</tt>-th term of the
   *second series. \p lf is intended to be
   * a functor that establishes how many terms in the second series have to be
   *multiplied by the <tt>i</tt>-th term
   * of the first series.
   *
   ...
   *
   * @throws std::invalid_argument if \p start1 is greater than \p end1 or
   *greater than the size of
   * base_series_multiplier::m_v1, or if \p end1 is greater than the size of
   *base_series_multiplier::m_v1.
   * @throws unspecified any exception thrown by the call operator of \p mf or
   *\p sf, or piranha::safe_cast.
   */
  template <typename MultFunctor, typename LimitFunctor>
  void blocked_multiplication(const MultFunctor &mf, const size_type &start1,
                              const size_type &end1,
                              const LimitFunctor &lf) const {
    PIRANHA_TT_CHECK(is_function_object, MultFunctor, void, const size_type &,
                     const size_type &);
    ...
    const size_type bsize = safe_cast<size_type>(
                        tuning::get_multiplication_block_size()),
    ...
    const size_type i_ir_start =
                        static_cast<size_type>(nblocks1 * bsize + start1),
                    i_ir_end = end1;
    const size_type j_ir_start = static_cast<size_type>(nblocks2 * bsize),
                    j_ir_end = m_v2.size();
  ...
  /**
   * This method is equivalent to the other overload with a limit functor that
  ...
  void blocked_multiplication(const MultFunctor &mf, const size_type &start1,
                              const size_type &end1) const {
    blocked_multiplication(mf, start1, end1, default_limit_functor{*this});
  }
  /// Estimate size of series multiplication.
  /**
   * \note
   * If \p MultArity, \p MultFunctor or \p LimitFunctor do not satisfy the
   * requirements outlined below,
   * a compile-time error will be produced.
   *
   * This method expects a \p MultFunctor type exposing the same inteface as
   * explained in blocked_multiplication().
   * Additionally, \p MultFunctor must be constructible from a const reference
   * to piranha::base_series_multiplier
   * and a mutable reference to \p Series. \p MultFunctor objects will be
   * constructed internally by this method,
   * passing \p this and a temporary local \p Series object as construction
   * parameters.
   * It will be assumed that a call <tt>mf(i,j)</tt> multiplies the
   * <tt>i</tt>-th
   * term of the first series by the <tt>j</tt>-th term of the second series,
   * accumulating the result into the \p Series
   * passed as second parameter for construction.
   *
   * This method will apply a statistical approach to estimate the final size of
   * the result of the multiplication of the first
   * series by the second. It will perform random term-by-term multiplications
   * and deduce the estimate from the number of term multiplications performed
   * before finding the first duplicate term.
   * The \p MultArity parameter represents the arity of term multiplications -
   * that is, the number of terms generated by a single
   * term-by-term multiplication. It must be strictly positive.
   *
   * The \p lf parameter must be a function object exposing the same inteface as
   * explained in blocked_multiplication().
   * This functor establishes how many terms in the second series must be
   * multiplied by the <tt>i</tt>-th term of the first series.
   *
   * The number returned by this function is always at least 1. Multiple threads
   * might be used by this method: in such a case, different
   * instances of \p MultFunctor are constructed in different threads, but \p lf
   * is shared among all threads.
   *
   * @param[in] lf the limit functor.
   *
   * @return the estimated size of the multiplication of the first series by the
   * second, always at least 1.
   *
   * @throws std::overflow_error in case of (unlikely) overflows in integral
   * arithmetics.
   * @throws unspecified any exception thrown by:
  ...
  bucket_size_type estimate_final_series_size(const LimitFunctor &lf) const {
    PIRANHA_TT_CHECK(is_function_object, MultFunctor, void, const size_type &,
                     const size_type &);
    PIRANHA_TT_CHECK(std::is_constructible, MultFunctor,
                     const base_series_multiplier &, Series &);
    PIRANHA_TT_CHECK(is_function_object, LimitFunctor, size_type,
                     const size_type &);
    // Cache these.
    ...
    // NOTE: here consider that in case of extremely sparse series with few
    // terms this will incur in noticeable
    // overhead, since we will need many term-by-term before encountering the
    // first duplicate.
    ...
    // Number of threads to use. If there are more threads than trials, then
    // reduce
    // the number of actual threads to use.
    // NOTE: this is a bit different from usual, where we do not care if the
    // workload per thread is zero.
    // We do like this because n_trials is a small number and there still seems
    // to be benefit in running
    // just 1 trial per thread.
    const unsigned n_threads =
        (n_trials >= m_n_threads) ? m_n_threads : n_trials;
    piranha_assert(n_threads > 0u);
    ...
    auto estimator = [&lf, size1, n_threads, multiplier, tpt, n_trials, this,
                      &c_estimate, &mut, result_size](unsigned thread_idx) {
      piranha_assert(thread_idx < n_threads);
      // Vectors of indices into m_v1.
      std::vector<size_type> v_idx1(
          safe_cast<typename std::vector<size_type>::size_type>(size1));
      ...
      integer acc(0);
      // Number of trials for this thread - usual special casing for the last
      // thread.
      const unsigned cur_trials =
          (thread_idx == n_threads - 1u) ? (n_trials - thread_idx * tpt) : tpt;
      ...
      for (auto n = 0u; n < cur_trials; ++n) {
        // Seed the engine. The seed should be the global trial number,
        // accounting for multiple
        // threads. This way the estimation will not depend on the number of
        // threads.
        engine.seed(
            static_cast<std::mt19937::result_type>(tpt * thread_idx + n));
        // Reset the indices vector and re-randomise it.
        // NOTE: we need to do this as every run inside this for loop must be
        // completely independent
        // of any previous run, we cannot keep any state.
        v_idx1 = v_idx1_copy;
        std::shuffle(v_idx1.begin(), v_idx1.end(), engine);
        // The counter. This will be increased each time a term-by-term
        // multiplication
        ...
        for (; it1 != v_idx1.end(); ++it1) {
          ...
          // to be decreased by one in order to be used in dist. If zero, it
          // means
          // there are no terms in v2 that can be multiplied by the current term
          // in t1.
          if (limit == 0u) {
            continue;
          }
          ...
          const size_type idx2 =
              dist(engine, typename dist_type::param_type(
        ...
        if (it1 == v_idx1.end()) {
          ...
          // which actually participate in the multiplication, while acc_s2 /
          // count
          // is the average number of terms in s2 that participate in the
          // multiplication.
  ...
  /**
   * @return the output of the other overload of estimate_final_series_size(),
   * with a limit
   * functor whose call operator will always return the size of the second
   * series unconditionally.
   *
   * @throws unspecified any exception thrown by the other overload of
   * estimate_final_series_size().
   */
  template <std::size_t MultArity, typename MultFunctor>
  bucket_size_type estimate_final_series_size() const {
    return estimate_final_series_size<MultArity, MultFunctor>(
        default_limit_functor{*this});
  }
  /// A plain multiplier functor.
  /**
   * \note
   * If the key and coefficient types of \p Series do not satisfy
   * piranha::key_is_multipliable, a compile-time error will
   * be produced.
   *
   * This is a functor that conforms to the protocol expected by
   * base_series_multiplier::blocked_multiplication() and
   * base_series_multiplier::estimate_final_series_size().
   *
   * This functor requires that the key and coefficient types of \p Series
   * satisfy piranha::key_is_multipliable, and it will
   * use the key's <tt>multiply()</tt> method to perform term-by-term
   * multiplications.
   *
   * If the \p FastMode boolean parameter is \p true, then the call operator
   * will insert terms into the return value series
   * using the low-level interface of piranha::hash_set, otherwise the call
   * operator will use piranha::series::insert() for
   * term insertion.
   */
  template <bool FastMode> class plain_multiplier {
    using term_type = typename Series::term_type;
    using key_type = typename term_type::key_type;
    PIRANHA_TT_CHECK(key_is_multipliable, typename term_type::cf_type,
                     key_type);
  ...
  public:
     ...
     *
     * @param[in] bsm a const reference to an instance of
     * piranha::base_series_multiplier, from which
     * the vectors of term pointers will be extracted.
     * @param[in] retval the \p Series instance into which terms resulting from
     * multiplications will be inserted.
     */
    explicit plain_multiplier(const base_series_multiplier &bsm, Series &retval)
        : m_v1(bsm.m_v1), m_v2(bsm.m_v2), m_retval(retval),
          m_c_end(retval._container().end()) {}
    /// Deleted copy constructor.
    ...
    /// Call operator.
    /**
     * The call operator will perform the multiplication of the <tt>i</tt>-th
     * term of the first series by the
     * <tt>j</tt>-th term of the second series, and it will insert the result
    ...
    void operator()(const size_type &i, const size_type &j) const {
      ...
      key_type::multiply(m_tmp_t, *m_v1[i], *m_v2[j],
                         m_retval.get_symbol_set());
  ...
  /// Sanitise series.
  /**
   * When using the low-level interface of piranha::hash_set for term insertion,
   * invariants might be violated
   * both in piranha::hash_set and piranha::series. In particular:
   *
   * - terms may not be checked for compatibility or ignorability upon
   * insertion,
   * - the count of elements in piranha::hash_set might not be updated.
   *
   * This method can be used to fix these invariants: each term of \p retval
   * will be checked for ignorability and compatibility,
   * and the total count of terms in the series will be set to the number of
   * non-ignorable terms. Ignorable terms will
   * be erased.
   *
   * Note that in case of exceptions \p retval will likely be left in an
   * inconsistent state which violates internal invariants.
   * Calls to this function should always be wrapped in a try/catch block that
   * makes sure that \p retval is cleared before
   * re-throwing.
   *
   * @param[in] retval the series to be sanitised.
   * @param[in] n_threads the number of threads to be used.
   *
   * @throws std::invalid_argument if \p n_threads is zero, or if one of the
   * terms in \p retval is not compatible
   * with the symbol set of \p retval.
   * @throws std::overflow_error if the number of terms in \p retval overflows
   * the maximum value representable by
  ...
  static void sanitise_series(Series &retval, unsigned n_threads) {
    ...
    if (n_threads == 1u) {
      ...
      for (auto it = container.begin(); it != it_end;) {
        ...
        if (unlikely(container.size() ==
                     std::numeric_limits<bucket_size_type>::max())) {
          piranha_throw(std::overflow_error,
                        "overflow error in the number of terms of a series");
        }
        // First update the size, it will be scaled back in the erase() method
        // if necessary.
        container._update_size(
            static_cast<bucket_size_type>(container.size() + 1u));
    ...
    auto eraser = [b_count, &container, &m, &args, &global_count](
        const bucket_size_type &start, const bucket_size_type &end) {
    ...
    future_list<decltype(eraser(bucket_size_type(), bucket_size_type()))>
        f_list;
    try {
      for (unsigned i = 0u; i < n_threads; ++i) {
        const auto start =
                       ...
                       (i == n_threads - 1u) ? b_count : (b_count / n_threads) *
                                                             (i + 1u));
      ...
      f_list.wait_all();
      // NOTE: there's not need to clear retval here - it was already in an
      // inconsistent
      // state coming into this method. We rather need to make sure
      // sanitise_series() is always
   ...
   * \note
   * If the key and coefficient types of \p Series do not satisfy
   * piranha::key_is_multipliable, or \p LimitFunctor does
   * not satisfy the requirements outlined in
   * base_series_multiplier::blocked_multiplication(), a compile-time error will
   * be produced.
   *
   * This method implements a generic series multiplication routine suitable for
   * key types that satisfy piranha::key_is_multipliable.
   * The implementation is either single-threaded or multi-threaded, depending
   * on the sizes of the input series, and it will use
   * either base_series_multiplier::plain_multiplier or a similar thread-safe
   * multiplier for the term-by-term multiplications.
   * The \p lf functor will be forwarded as limit functor to
   * base_series_multiplier::blocked_multiplication()
   * and base_series_multiplier::estimate_final_series_size().
   *
   * Note that, in multithreaded mode, \p lf will be shared among (and called
   * concurrently from) all the threads.
   *
   * @param[in] lf the limit functor (see
   * base_series_multiplier::blocked_multiplication()).
   *
   * @return the series resulting from the multiplication of the two series used
   * to construct \p this.
   *
   * @throws unspecified any exception thrown by:
   * - piranha::safe_cast(),
  ...
  Series plain_multiplication(const LimitFunctor &lf) const {
    ...
    // Determine if we should estimate the size. We check the threshold, but we
    // always
    // need to estimate in multithreaded mode.
    bool estimate = true;
    const auto e_thr = tuning::get_estimate_threshold();
    if (integer(m_v1.size()) * m_v2.size() < integer(e_thr) * e_thr &&
        n_threads == 1u) {
      estimate = false;
    }
    if (estimate) {
      // Estimate and rehash.
      const auto est =
          estimate_final_series_size<m_arity, plain_multiplier<false>>(lf);
      // NOTE: use numeric cast here as safe_cast is expensive, going through an
      // integer-double conversion,
      // and in this case the behaviour of numeric_cast is appropriate.
      const auto n_buckets = boost::numeric_cast<bucket_size_type>(std::ceil(
    ...
    if (n_threads == 1u) {
      try {
        // Single-thread case.
        if (estimate) {
          blocked_multiplication(plain_multiplier<true>(*this, retval), 0u,
                                 size1, lf);
          // If we estimated beforehand, we need to sanitise the series.
          sanitise_series(retval, static_cast<unsigned>(n_threads));
        } else {
          blocked_multiplication(plain_multiplier<false>(*this, retval), 0u,
                                 size1, lf);
        }
    ...
    detail::atomic_flag_array sl_array(
        safe_cast<std::size_t>(retval._container().bucket_count()));
    ...
    try {
      for (size_type idx = 0u; idx < n_threads; ++idx) {
        // Thread functor.
        auto tf = [idx, this, block_size, n_threads, &sl_array, &retval,
                   &lf]() {
          ...
          // Block functor.
          // NOTE: this is very similar to the plain functor, but it does the
          // bucket locking
          // additionally.
          auto f = [&c_end, &tmp_t, this, &retval,
                    &sl_array](const size_type &i, const size_type &j) {
            // Run the term multiplication.
            key_type::multiply(tmp_t, *(this->m_v1[i]), *(this->m_v2[j]),
                               retval.get_symbol_set());
            for (std::size_t n = 0u; n < key_type::multiply_arity; ++n) {
              ...
              auto bucket_idx = container._bucket(tmp_term);
              // Lock the bucket.
              detail::atomic_lock_guard alg(
                  sl_array[static_cast<std::size_t>(bucket_idx)]);
              const auto it = container._find(tmp_term, bucket_idx);
              if (it == c_end) {
                container._unique_insert(term_insertion(tmp_term), bucket_idx);
              } else {
          ...
          // Thread block limit.
          const auto e1 = (idx == n_threads - 1u)
                              ? this->m_v1.size()
                              : static_cast<size_type>((idx + 1u) * block_size);
          this->blocked_multiplication(
              f, static_cast<size_type>(idx * block_size), e1, lf);
        };
  ...
  /**
   * @return the output of the other overload of plain_multiplication(), with a
   * limit
   * functor whose call operator will always return the size of the second
   * series unconditionally.
   *
  ...
  /// Finalise series.
  /**
   * This method will finalise the output \p s of a series multiplication
   * undertaken via piranha::base_series_multiplier.
   * Currently, this method will not do anything unless the coefficient type of
   * \p Series is an instance of piranha::mp_rational.
   * In this case, the coefficients of \p s will be normalised with respect to
   * the least common multiplier computed in the
   ...
   * - future_list::push_back().
   */
  void finalise_series(Series &s) const { finalise_impl(s); }
...
protected:
  ...
  /// Number of threads.
  /**
   * This value will be set by the constructor, and it represents the number of
   * threads
   * that will be used by the multiplier. The value is always at least 1 and it
   * is calculated

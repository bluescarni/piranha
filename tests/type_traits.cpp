...
template <typename T> struct foo_t { typedef int foo_type; };

template <typename T> struct bar_t : foo_t<T> {};
...
BOOST_AUTO_TEST_CASE(type_traits_is_nonconst_rvalue_ref_test) {
  ...
  BOOST_CHECK_EQUAL(is_nonconst_rvalue_ref<volatile int const &&>::value,
                    false);
  BOOST_CHECK_EQUAL(is_nonconst_rvalue_ref<const int &&>::value, false);
...
struct nontrivial_dtor {
  ...
  ~nontrivial_dtor() noexcept(false) { n = 0; }
  int n;
};
...
// The Intel compiler seems to have some nonstandard extensions to the complex
// class.
...
BOOST_AUTO_TEST_CASE(type_traits_is_subtractable_in_place_test) {
  ...
  BOOST_CHECK(
      (!is_subtractable_in_place<trivial, std::complex<double>>::value));
...
BOOST_AUTO_TEST_CASE(type_traits_is_multipliable_in_place_test) {
  ...
  BOOST_CHECK(
      (!is_multipliable_in_place<trivial, std::complex<double>>::value));
...
template <typename T> struct iio_base {};
...
template <> struct hash<unhashable2> {};

template <> struct hash<unhashable3> { int operator()(const unhashable3 &); };

template <> struct hash<unhashable4> { std::size_t operator()(unhashable4 &); };

template <> struct hash<unhashable5> {
  hash(const hash &) = delete;
  hash &operator=(const hash &) = default;
  std::size_t operator()(const unhashable5 &);
};

template <> struct hash<unhashable6> {
  ~hash() = delete;
  std::size_t operator()(const unhashable6 &);
};

template <> struct hash<unhashable7> {
  std::size_t operator()(const unhashable7 &);
};

template <> struct hash<unhashable8> {
  hash();
  std::size_t operator()(const unhashable8 &);
};

template <> struct hash<unhashable9> {
  std::size_t operator()(const unhashable9 &);
};

template <> struct hash<unhashable10> {
  hash(const hash &) = default;
  std::size_t operator()(const unhashable10 &) const;
  ~hash() noexcept(false) {}
};

template <> struct hash<unhashable11> {
...
template <> struct hash<unhashable12> {
...
BOOST_AUTO_TEST_CASE(type_traits_is_hashable_test) {
  ...
  BOOST_CHECK(is_hashable<const double>::value);
  // This is gonna fail on GCC 4.7.2 at least, depending on whether
  // static_assert() is used
...
struct fo5 {
  template <typename... Args> int operator()(Args &&...);
};
...
BOOST_AUTO_TEST_CASE(type_traits_is_function_object_test) {
  ...
  // Specifically, they are always function objects and they have defaulted
  // constructors.
  auto l1 = []() {};
  auto l2 = [](const int &) {};
  auto l3 = [](int &) {};
  ...
  BOOST_CHECK(
      (is_function_object<const decltype(l2), void, const int &>::value));
  ...
  BOOST_CHECK(
      (!is_function_object<const decltype(l3), void, const int &>::value));
  BOOST_CHECK((!is_function_object<decltype(l3) &, void, int &>::value));
  ...
  BOOST_CHECK(
      (is_function_object<const std::hash<int>, std::size_t, int &&>::value));
  BOOST_CHECK((is_function_object<const std::hash<int>, std::size_t,
                                  const int &>::value));
  BOOST_CHECK(
      (is_function_object<const std::hash<int>, std::size_t, int &>::value));
  BOOST_CHECK(
      (!is_function_object<const std::hash<int> &, std::size_t, int &>::value));
  BOOST_CHECK(
      (!is_function_object<std::hash<int> &, std::size_t, int &>::value));
  BOOST_CHECK((!is_function_object<const std::hash<int>, int, int &>::value));
  BOOST_CHECK((!is_function_object<const std::hash<int>, std::size_t, int &,
                                   int &>::value));
  ...
  BOOST_CHECK(
      (!is_function_object<fo4, std::string, int, double &, int>::value));
  BOOST_CHECK((!is_function_object<fo4, std::string, int>::value));
  BOOST_CHECK((!is_function_object<fo4, std::string &, int, double &>::value));
  BOOST_CHECK(
      (!is_function_object<fo4, std::string, int, double const &>::value));
  BOOST_CHECK((is_function_object<fo5, int>::value));
  BOOST_CHECK((is_function_object<fo5, int, double>::value));
  BOOST_CHECK(
      (is_function_object<fo5, int, double, const std::string &>::value));
  BOOST_CHECK(
      (!is_function_object<fo5, void, double, const std::string &>::value));
...
BOOST_AUTO_TEST_CASE(type_traits_is_hash_function_object_test) {
  ...
  BOOST_CHECK(
      (is_hash_function_object<std::hash<int const *>, int const *>::value));
  BOOST_CHECK((is_hash_function_object<std::hash<int const *>, int *>::value));
  BOOST_CHECK(
      (!is_hash_function_object<const std::hash<int const *>, int *>::value));
  BOOST_CHECK((!is_hash_function_object<std::hash<int> &, int &>::value));
  BOOST_CHECK((!is_hash_function_object<std::hash<int> const &, int &>::value));
  BOOST_CHECK((!is_hash_function_object<std::hash<int> &, const int &>::value));
  BOOST_CHECK(
      (is_hash_function_object<std::hash<std::string>, std::string>::value));
...
BOOST_AUTO_TEST_CASE(type_traits_is_equality_function_object_test) {
  ...
  BOOST_CHECK(
      (!is_equality_function_object<const std::equal_to<int>, short>::value));
  BOOST_CHECK(
      (!is_equality_function_object<std::equal_to<int> &, short>::value));
  BOOST_CHECK(
      (!is_equality_function_object<std::equal_to<int> &&, short>::value));
...
template <> struct hash<key02> { std::size_t operator()(const key02 &) const; };

template <> struct hash<key03> {};

template <> struct hash<key04> { std::size_t operator()(const key04 &) const; };

template <> struct hash<key05> { std::size_t operator()(const key05 &) const; };

template <> struct hash<key06> { std::size_t operator()(const key06 &) const; };

template <> struct hash<key07> { std::size_t operator()(const key07 &) const; };

template <> struct hash<key08> { std::size_t operator()(const key08 &) const; };
}
...
struct enable_noexcept_checks<
    T, typename std::enable_if<std::is_same<cf07, T>::value>::type> {
  static const bool value = false;
};

template <typename T>
const bool enable_noexcept_checks<
    T, typename std::enable_if<std::is_same<cf07, T>::value>::type>::value;
}
...
BOOST_AUTO_TEST_CASE(type_traits_is_cf_test) {
  ...
  // NOTE: the checks on the pointers here produce warnings in clang. The reason
  // is that
  // ptr1 -= ptr2
  // expands to
  // ptr1 = ptr1 - ptr2
  // where ptr1 - ptr2 is some integer value (due to pointer arith.) that then
  // gets assigned back to a pointer. It is not
  // entirely clear if this should be a hard error (GCC) or just a warning
  // (clang) so for now it is better to simply disable
  // the check. Note that the same problem would be in is_subtractable_in_place
  // if we checked for pointers there.
  // BOOST_CHECK(!is_cf<double *>::value);
...
BOOST_AUTO_TEST_CASE(type_traits_min_max_int_test) {
  ...
  BOOST_CHECK(
      (std::is_same<unsigned, max_int<unsigned short, unsigned>>::value));
  if (boost::integer_traits<long long>::const_max >
    ...
    BOOST_CHECK(
        (std::is_same<long long,
                      max_int<short, int, signed char, long long>>::value));
    BOOST_CHECK(
        (std::is_same<long long,
                      max_int<long long, int, signed char, short>>::value));
    BOOST_CHECK(
        (std::is_same<long long,
                      max_int<int, long long, signed char, short>>::value));
    BOOST_CHECK(
        (std::is_same<long long,
                      max_int<short, signed char, long long, int>>::value));
  }
  if (boost::integer_traits<unsigned long long>::const_max >
      boost::integer_traits<unsigned>::const_max) {
    BOOST_CHECK((std::is_same<unsigned long long,
                              max_int<unsigned short, unsigned, unsigned char,
                                      unsigned long long>>::value));
    BOOST_CHECK((std::is_same<unsigned long long,
                              max_int<unsigned long long, unsigned,
                                      unsigned char, unsigned short>>::value));
    BOOST_CHECK((std::is_same<unsigned long long,
                              max_int<unsigned, unsigned long long,
                                      unsigned char, unsigned short>>::value));
    BOOST_CHECK((std::is_same<unsigned long long,
                              max_int<unsigned short, unsigned char,
                                      unsigned long long, unsigned>>::value));
  }
  if (boost::integer_traits<signed char>::const_max <
          boost::integer_traits<short>::const_max &&
      boost::integer_traits<signed char>::const_min >
          boost::integer_traits<short>::const_min) {
    BOOST_CHECK(
        (std::is_same<signed char,
                      min_int<short, int, signed char, long long>>::value));
    BOOST_CHECK(
        (std::is_same<signed char,
                      min_int<long long, int, signed char, short>>::value));
    BOOST_CHECK(
        (std::is_same<signed char,
                      min_int<int, long long, signed char, short>>::value));
    BOOST_CHECK(
        (std::is_same<signed char,
                      min_int<short, signed char, long long, int>>::value));
  }
  if (boost::integer_traits<unsigned char>::const_min <
      boost::integer_traits<unsigned short>::const_max) {
    BOOST_CHECK((std::is_same<unsigned char,
                              min_int<unsigned short, unsigned, unsigned char,
                                      unsigned long long>>::value));
    BOOST_CHECK((std::is_same<unsigned char,
                              min_int<unsigned long long, unsigned,
                                      unsigned char, unsigned short>>::value));
    BOOST_CHECK((std::is_same<unsigned char,
                              min_int<unsigned, unsigned long long,
                                      unsigned char, unsigned short>>::value));
    BOOST_CHECK((std::is_same<unsigned char,
                              min_int<unsigned short, unsigned char,
                                      unsigned long long, unsigned>>::value));
  }
}

// Boilerplate to test the arrow op type trait.
template <typename T> using aot = detail::arrow_operator_type<T>;
...
template <typename T> struct fake_it_traits_forward {
...
template <typename T> struct fake_it_traits_forward_broken_ref {
  using difference_type = std::ptrdiff_t;
  using value_type = T;
  using pointer = T *;
...
template <typename T> struct fake_it_traits_output {
  using difference_type = std::ptrdiff_t;
  using value_type = T;
  using pointer = T *;
  using reference = T &;
...
// Good input iterator.
template <typename T> struct fake_it_traits_input {
  using difference_type = std::ptrdiff_t;
  using value_type = T;
  using pointer = T *;
  using reference = T &;
...
template <typename T> struct fake_it_traits_wrong_tag {
  using difference_type = std::ptrdiff_t;
  using value_type = T;
  using pointer = T *;
  using reference = T &;
...
template <typename T> struct fake_it_traits_missing {
...
#define PIRANHA_DECL_ITT_SPEC(it_type, trait_class)                            \
  namespace std {                                                              \
  template <> struct iterator_traits<it_type> : trait_class {};                \
  }
...
// Bad forward iterator: arrow returns type with different constness from star
// operator.
struct iter21 {
  int &operator*();
  const int *operator->();
...
BOOST_AUTO_TEST_CASE(type_traits_iterator_test) {
  ...
  BOOST_CHECK(
      detail::has_iterator_traits<std::vector<int>::const_iterator>::value);
...
template <typename S> using sai = detail::safe_abs_sint<S>;

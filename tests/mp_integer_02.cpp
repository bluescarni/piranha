...
using integral_types =
    boost::mpl::vector<char, signed char, short, int, long, long long,
...
using size_types = boost::mpl::vector<
    std::integral_constant<int, 0>, std::integral_constant<int, 8>,
...
template <> struct debug_access<mp_integer_access_tag> {
  template <int NBits>
  ...
  template <typename T> void operator()(const T &) {
    ...
    }
    {
      ...
      for (int i = 0; i < ntries; ++i) {
        ...
        math::multiply_accumulate(a, b, c);
        BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(a),
                          mpz_lexcast(m_a));
      }
    }
    // Trigger overflow with three static ints.
    {
      ...
      using limb_t =
          typename detail::integer_union<T::value>::s_storage::limb_t;
      ...
      a.multiply_accumulate(b, c);
      BOOST_CHECK(!a.is_static());
      BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(42 + b * c),
    ...
    }
    {
      ...
      a.multiply_accumulate(b, c);
      BOOST_CHECK(!a.is_static());
      BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(old_a + b * c),
                        boost::lexical_cast<std::string>(a));
    }
    {
      // Promotion bug.
      ...
      get_m(a).g_st().set_bit(
          static_cast<limb_t>(get_m(a).g_st().limb_bits * 2u - 1u));
      ::mpz_setbit(&m_a.m_mpz, static_cast<::mp_bitcnt_t>(
                                   get_m(a).g_st().limb_bits * 2u - 1u));
...
struct in_place_mp_integer_div_tester {
  template <typename T> void operator()(const T &) {
    ...
    for (int i = 0; i < ntries; ++i) {
      ...
      if (tmp2 >= 1) {
        BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(a),
                          boost::lexical_cast<std::string>(tmp1 / tmp2));
      }
    }
  }
};
...
struct in_place_int_div_tester {
  template <typename U> struct runner {
    template <typename T> void operator()(const T &) {
      ...
      BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(n1), "1");
      // Random testing.
      std::uniform_int_distribution<T> int_dist(std::numeric_limits<T>::min(),
      ...
      for (int i = 0; i < ntries; ++i) {
        ...
        int_type n(tmp1);
        n /= tmp2;
        ::mpz_set_str(&m1.m_mpz, boost::lexical_cast<std::string>(
                                     static_cast<int_cast_t>(tmp1))
                                     .c_str(),
                      10);
        ::mpz_set_str(&m2.m_mpz, boost::lexical_cast<std::string>(
                                     static_cast<int_cast_t>(tmp2))
                                     .c_str(),
                      10);
      ...
      for (int i = 0; i < ntries; ++i) {
        ...
        ::mpz_tdiv_q(&m1.m_mpz, &m1.m_mpz, &m2.m_mpz);
        BOOST_CHECK_EQUAL(
            boost::lexical_cast<std::string>(static_cast<int_cast_t>(tmp1)),
            mpz_lexcast(m1));
      }
    }
  };
...
struct in_place_float_div_tester {
  template <typename U> struct runner {
    template <typename T> void operator()(const T &) {
      ...
      BOOST_CHECK_THROW(n1 /= T(0), zero_division_error);
      // Random testing
      std::uniform_real_distribution<T> urd1(T(0),
                                             std::numeric_limits<T>::max()),
      ...
      for (int i = 0; i < ntries / 100; ++i) {
        ...
        int_type n(tmp1);
        n /= tmp1;
        BOOST_CHECK(boost::lexical_cast<std::string>(n) == "0" ||
      ...
      // NOTE: limit the number of times we run this test, as the conversion
      // from int to float
      // is slow as the random values are taken from the entire float range and
      // thus use a lot of digits.
      for (int i = 0; i < ntries / 100; ++i) {
...
struct binary_div_tester {
  template <typename U> struct runner1 {
    template <typename T> void operator()(const T &) {
      typedef mp_integer<U::value> int_type;
      using int_cast_t =
          typename std::conditional<std::is_signed<T>::value, long long,
      ...
      for (int i = 0; i < ntries; ++i) {
        ...
        BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(n / tmp2),
                          mpz_lexcast(res));
        if (tmp1 == T(0)) {
          continue;
        }
        ::mpz_tdiv_q(&res.m_mpz, &m2.m_mpz, &m1.m_mpz);
        BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(tmp2 / n),
                          mpz_lexcast(res));
      }
    }
  };
  template <typename U> struct runner2 {
    template <typename T> void operator()(const T &) {
      typedef mp_integer<U::value> int_type;
      {
        BOOST_CHECK((is_divisible<int_type, T>::value));
  ...
  template <typename T> void operator()(const T &) {
    ...
    for (int i = 0; i < ntries; ++i) {
      ...
      ::mpz_tdiv_q(&m_a.m_mpz, &m_a.m_mpz, &m_b.m_mpz);
      BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(a / b),
                        mpz_lexcast(m_a));
    }
...
struct int_pow_tester {
  template <typename U> struct runner {
    template <typename T> void operator()(const T &) {
      ...
      int_type n;
      BOOST_CHECK(
          (std::is_same<int_type, decltype(math::pow(n, T(0)))>::value));
      ...
      // Random testing.
      const T max_exp = static_cast<T>(std::min(
          int_cast_t(1000), int_cast_t(std::numeric_limits<T>::max())));
      ...
      for (int i = 0; i < ntries; ++i) {
        ...
        ::mpz_pow_ui(&m_base.m_mpz, &m_base.m_mpz,
                     static_cast<unsigned long>(exp_int));
        BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(retval),
                          mpz_lexcast(m_base));
      ...
      BOOST_CHECK(
          (std::is_same<int_type,
                        decltype(math::pow(int_type(1), int_type(1)))>::value));
      ...
      BOOST_CHECK(
          (std::is_same<int_type, decltype(math::pow(int_type(1), 1))>::value));
      BOOST_CHECK((std::is_same<int_type,
                                decltype(math::pow(int_type(1), 1ul))>::value));
      BOOST_CHECK(
          (std::is_same<int_type, decltype(math::pow(int_type(1),
                                                     (signed char)1))>::value));
      BOOST_CHECK_EQUAL(math::pow(int_type(2), 3), 8);
      // Integer -- floating-point.
      BOOST_CHECK((is_exponentiable<int_type, double>::value));
      BOOST_CHECK(
          (std::is_same<double, decltype(math::pow(int_type(1), 1.))>::value));
      BOOST_CHECK_EQUAL(math::pow(int_type(2), 3.), math::pow(2., 3.));
      BOOST_CHECK_EQUAL(math::pow(int_type(2), 1. / 3.),
                        math::pow(2., 1. / 3.));
      // Integral -- integer.
      BOOST_CHECK((is_exponentiable<int, int_type>::value));
      BOOST_CHECK((is_exponentiable<short, int_type>::value));
      BOOST_CHECK(
          (std::is_same<int_type, decltype(math::pow(1, int_type(1)))>::value));
      BOOST_CHECK((std::is_same<int_type, decltype(math::pow(
                                              short(1), int_type(1)))>::value));
      BOOST_CHECK_EQUAL(math::pow(2, int_type(3)), 8.);
      // Floating-point -- integer.
      BOOST_CHECK((is_exponentiable<float, int_type>::value));
      BOOST_CHECK((is_exponentiable<double, int_type>::value));
      BOOST_CHECK(
          (std::is_same<float, decltype(math::pow(1.f, int_type(1)))>::value));
      BOOST_CHECK(
          (std::is_same<double, decltype(math::pow(1., int_type(1)))>::value));
      BOOST_CHECK_EQUAL(math::pow(2.f, int_type(3)), math::pow(2.f, 3.f));
      BOOST_CHECK_EQUAL(math::pow(2., int_type(3)), math::pow(2., 3.));
      BOOST_CHECK_EQUAL(math::pow(2.f / 5.f, int_type(3)),
                        math::pow(2.f / 5.f, 3.f));
      BOOST_CHECK_EQUAL(math::pow(2. / 7., int_type(3)),
                        math::pow(2. / 7., 3.));
    }
  };
  template <typename T> void operator()(const T &) {
...
struct mp_integer_pow_tester {
  template <typename T> void operator()(const T &) {
    ...
    BOOST_CHECK_EQUAL(n.pow(int_type(-1)), 0);
    BOOST_CHECK_THROW(
        n.pow(int_type(std::numeric_limits<unsigned long>::max()) + 1),
    ...
    for (int i = 0; i < ntries; ++i) {
      ...
      BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(retval),
                        mpz_lexcast(m_base));
      BOOST_CHECK_EQUAL(math::pow(int_type(base_int), int_type(exp_int)),
                        retval);
    }
  }
};
...
BOOST_AUTO_TEST_CASE(mp_integer_pow_test) {
  ...
  BOOST_CHECK((std::is_same<integer, decltype(math::pow(
                                         -3ll, (unsigned short)3))>::value));
...
struct abs_tester {
  template <typename T> void operator()(const T &) {
    ...
    // Random testing.
    std::uniform_int_distribution<int> promote_dist(0, 1),
        int_dist(std::numeric_limits<int>::lowest(),
    ...
    for (int i = 0; i < ntries; ++i) {
      ...
      BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(n.abs()),
                        mpz_lexcast(m_tmp));
...
using ru_test_types =
    boost::mpl::vector<std::tuple<unsigned char, unsigned long long>,
                       std::tuple<unsigned long long, unsigned char>,
...
struct read_uint_tester {
  template <typename T> void operator()(const T &) {
    ...
    input_vector.resize(static_cast<size_type>(
        (sizeof(out_type) * 10u) / sizeof(in_type) + 1u));
    std::generate(input_vector.begin(), input_vector.end(), [&bd]() -> in_type {
      in_type retval(0);
      for (unsigned i = 0u;
           i < static_cast<unsigned>(std::numeric_limits<in_type>::digits);
           ++i) {
        retval = static_cast<in_type>(retval + (in_type(bd(rng)) << i));
      }
      return retval;
    });
    // Boilerplate to convert the input vector and return value to a sequence of
    // bits represented
    ...
    auto vec_to_bitset = [&db_in, &input_vector](unsigned ibits) {
      piranha_assert(
          ibits < static_cast<unsigned>(std::numeric_limits<in_type>::digits));
      db_in.clear();
    ...
    auto ret_to_bitset = [&db_out](out_type r, unsigned rbits) {
      ...
      for (unsigned i = 0u;
           i <
           static_cast<unsigned>(std::numeric_limits<out_type>::digits) - rbits;
           ++i) {
    ...
    // Number of rets that can be read from the input vector, ignoring ib bits
    // in input and rbits bits in output.
    auto n_rets = [&input_vector](unsigned ib, unsigned rbits) -> unsigned {
      const unsigned tmp1 = static_cast<unsigned>(
                         input_vector.size() *
                         (static_cast<unsigned>(
                              std::numeric_limits<in_type>::digits) -
                          ib)),
                     tmp2 = static_cast<unsigned>(
                                std::numeric_limits<out_type>::digits) -
                            rbits;
    ...
    vec_to_bitset(0u);
    auto r =
        detail::read_uint<out_type>(&input_vector[0u], input_vector.size(), 0u);
    ret_to_bitset(r, 0u);
    auto out_size = db_out.size();
    BOOST_CHECK(db_in.size() >= db_out.size());
    BOOST_CHECK(std::equal(db_out.begin(), db_out.end(), db_in.begin()));
    // 0 - 1 ignore.
    r = detail::read_uint<out_type, 0u, 1u>(&input_vector[0u],
                                            input_vector.size(), 0u);
    ret_to_bitset(r, 1u);
    ...
    r = detail::read_uint<out_type, 0u, 2u>(&input_vector[0u],
                                            input_vector.size(), 0u);
    ret_to_bitset(r, 2u);
    BOOST_CHECK(db_out.size() <= out_size);
    BOOST_CHECK(db_in.size() >= db_out.size());
    ...
    // 0 - 3 ignore.
    out_size = db_out.size();
    r = detail::read_uint<out_type, 0u, 3u>(&input_vector[0u],
                                            input_vector.size(), 0u);
    ret_to_bitset(r, 3u);
    BOOST_CHECK(db_out.size() <= out_size);
    BOOST_CHECK(db_in.size() >= db_out.size());
    ...
    // 0 - 7 ignore.
    out_size = db_out.size();
    r = detail::read_uint<out_type, 0u, 7u>(&input_vector[0u],
                                            input_vector.size(), 0u);
    ret_to_bitset(r, 7u);
    BOOST_CHECK(db_out.size() <= out_size);
    BOOST_CHECK(db_in.size() >= db_out.size());
    ...
    // 1 - 0 ignore.
    vec_to_bitset(1u);
    r = detail::read_uint<out_type, 1u>(&input_vector[0u], input_vector.size(),
                                        0u);
    ret_to_bitset(r, 0u);
    BOOST_CHECK(db_in.size() >= db_out.size());
    BOOST_CHECK(std::equal(db_out.begin(), db_out.end(), db_in.begin()));
    // 1 - 1 ignore.
    out_size = db_out.size();
    r = detail::read_uint<out_type, 1u, 1u>(&input_vector[0u],
                                            input_vector.size(), 0u);
    ret_to_bitset(r, 1u);
    BOOST_CHECK(db_out.size() <= out_size);
    BOOST_CHECK(db_in.size() >= db_out.size());
    ...
    // 1 - 2 ignore.
    out_size = db_out.size();
    r = detail::read_uint<out_type, 1u, 2u>(&input_vector[0u],
                                            input_vector.size(), 0u);
    ret_to_bitset(r, 2u);
    BOOST_CHECK(db_out.size() <= out_size);
    BOOST_CHECK(db_in.size() >= db_out.size());
    ...
    // 1 - 3 ignore.
    out_size = db_out.size();
    r = detail::read_uint<out_type, 1u, 3u>(&input_vector[0u],
                                            input_vector.size(), 0u);
    ret_to_bitset(r, 3u);
    BOOST_CHECK(db_out.size() <= out_size);
    BOOST_CHECK(db_in.size() >= db_out.size());
    ...
    // 1 - 7 ignore.
    out_size = db_out.size();
    r = detail::read_uint<out_type, 1u, 7u>(&input_vector[0u],
                                            input_vector.size(), 0u);
    ret_to_bitset(r, 7u);
    BOOST_CHECK(db_out.size() <= out_size);
    BOOST_CHECK(db_in.size() >= db_out.size());
    ...
    // 2 - 0 ignore.
    vec_to_bitset(2u);
    r = detail::read_uint<out_type, 2u>(&input_vector[0u], input_vector.size(),
                                        0u);
    ret_to_bitset(r, 0u);
    BOOST_CHECK(db_in.size() >= db_out.size());
    BOOST_CHECK(std::equal(db_out.begin(), db_out.end(), db_in.begin()));
    // 2 - 3 ignore.
    out_size = db_out.size();
    r = detail::read_uint<out_type, 2u, 3u>(&input_vector[0u],
                                            input_vector.size(), 0u);
    ret_to_bitset(r, 3u);
    BOOST_CHECK(db_out.size() <= out_size);
    BOOST_CHECK(db_in.size() >= db_out.size());
    ...
    // 2 - 4 ignore.
    out_size = db_out.size();
    r = detail::read_uint<out_type, 2u, 4u>(&input_vector[0u],
                                            input_vector.size(), 0u);
    ret_to_bitset(r, 4u);
    BOOST_CHECK(db_out.size() <= out_size);
    BOOST_CHECK(db_in.size() >= db_out.size());
    ...
    // 2 - 7 ignore.
    out_size = db_out.size();
    r = detail::read_uint<out_type, 2u, 7u>(&input_vector[0u],
                                            input_vector.size(), 0u);
    ret_to_bitset(r, 7u);
    BOOST_CHECK(db_out.size() <= out_size);
    BOOST_CHECK(db_in.size() >= db_out.size());
    ...
    // 3 - 0 ignore.
    vec_to_bitset(3u);
    r = detail::read_uint<out_type, 3u>(&input_vector[0u], input_vector.size(),
                                        0u);
    ret_to_bitset(r, 0u);
    BOOST_CHECK(db_in.size() >= db_out.size());
    BOOST_CHECK(std::equal(db_out.begin(), db_out.end(), db_in.begin()));
    // 3 - 1 ignore.
    out_size = db_out.size();
    r = detail::read_uint<out_type, 3u, 1u>(&input_vector[0u],
                                            input_vector.size(), 0u);
    ret_to_bitset(r, 1u);
    BOOST_CHECK(db_out.size() <= out_size);
    BOOST_CHECK(db_in.size() >= db_out.size());
    ...
    // 3 - 4 ignore.
    out_size = db_out.size();
    r = detail::read_uint<out_type, 3u, 4u>(&input_vector[0u],
                                            input_vector.size(), 0u);
    ret_to_bitset(r, 4u);
    BOOST_CHECK(db_out.size() <= out_size);
    BOOST_CHECK(db_in.size() >= db_out.size());
    ...
    // 3 - 7 ignore.
    out_size = db_out.size();
    r = detail::read_uint<out_type, 3u, 7u>(&input_vector[0u],
                                            input_vector.size(), 0u);
    ret_to_bitset(r, 7u);
    BOOST_CHECK(db_out.size() <= out_size);
    BOOST_CHECK(db_in.size() >= db_out.size());
    ...
    // 5 - 0 ignore.
    vec_to_bitset(5u);
    r = detail::read_uint<out_type, 5u>(&input_vector[0u], input_vector.size(),
                                        0u);
    ret_to_bitset(r, 0u);
    BOOST_CHECK(db_in.size() >= db_out.size());
    BOOST_CHECK(std::equal(db_out.begin(), db_out.end(), db_in.begin()));
    // 5 - 2 ignore.
    out_size = db_out.size();
    r = detail::read_uint<out_type, 5u, 2u>(&input_vector[0u],
                                            input_vector.size(), 0u);
    ret_to_bitset(r, 2u);
    BOOST_CHECK(db_out.size() <= out_size);
    BOOST_CHECK(db_in.size() >= db_out.size());
    ...
    // 5 - 6 ignore.
    out_size = db_out.size();
    r = detail::read_uint<out_type, 5u, 6u>(&input_vector[0u],
                                            input_vector.size(), 0u);
    ret_to_bitset(r, 6u);
    BOOST_CHECK(db_out.size() <= out_size);
    BOOST_CHECK(db_in.size() >= db_out.size());
    ...
    // 7 - 0 ignore.
    vec_to_bitset(7u);
    r = detail::read_uint<out_type, 7u>(&input_vector[0u], input_vector.size(),
                                        0u);
    ret_to_bitset(r, 0u);
    BOOST_CHECK(db_in.size() >= db_out.size());
    BOOST_CHECK(std::equal(db_out.begin(), db_out.end(), db_in.begin()));
    // 7 - 1 ignore.
    out_size = db_out.size();
    r = detail::read_uint<out_type, 7u, 1u>(&input_vector[0u],
                                            input_vector.size(), 0u);
    ret_to_bitset(r, 1u);
    BOOST_CHECK(db_out.size() <= out_size);
    BOOST_CHECK(db_in.size() >= db_out.size());
    ...
    // 7 - 4 ignore.
    out_size = db_out.size();
    r = detail::read_uint<out_type, 7u, 4u>(&input_vector[0u],
                                            input_vector.size(), 0u);
    ret_to_bitset(r, 4u);
    BOOST_CHECK(db_out.size() <= out_size);
    BOOST_CHECK(db_in.size() >= db_out.size());
    ...
    // 7 - 7 ignore.
    out_size = db_out.size();
    r = detail::read_uint<out_type, 7u, 7u>(&input_vector[0u],
                                            input_vector.size(), 0u);
    ret_to_bitset(r, 7u);
    BOOST_CHECK(db_out.size() <= out_size);
    BOOST_CHECK(db_in.size() >= db_out.size());
    ...
    // Second int.
    vec_to_bitset(0u);
    r = detail::read_uint<out_type, 0u>(&input_vector[0u], input_vector.size(),
                                        1u);
    ret_to_bitset(r, 0u);
    BOOST_CHECK(db_in.size() >= db_out.size());
    BOOST_CHECK(std::equal(db_out.begin(), db_out.end(),
                           &db_in[0u] + std::numeric_limits<out_type>::digits));
    if (n_rets(1u, 0u) > 1u) {
      // NOTE: if we have here enough rets for 0 ignore in the ret, then we have
      // enough
      // rets for higher ignore values.
      // 1 - 0 ignore.
      vec_to_bitset(1u);
      r = detail::read_uint<out_type, 1u>(&input_vector[0u],
                                          input_vector.size(), 1u);
      ret_to_bitset(r, 0u);
      BOOST_CHECK(db_in.size() >= db_out.size());
      BOOST_CHECK(
          std::equal(db_out.begin(), db_out.end(),
                     &db_in[0u] + std::numeric_limits<out_type>::digits));
      // 1 - 1 ignore.
      // NOTE: cannot do the db_out.size() <= out_size check here any more, as
      // now we have some ret preceding
      ...
      BOOST_CHECK(
          std::equal(db_out.begin(), db_out.end(),
                     &db_in[0u] + (std::numeric_limits<out_type>::digits - 1)));
      // 1 - 5 ignore.
      r = detail::read_uint<out_type, 1u, 5u>(&input_vector[0u],
                                              input_vector.size(), 1u);
      ret_to_bitset(r, 5u);
      BOOST_CHECK(db_in.size() >= db_out.size());
      BOOST_CHECK(
          std::equal(db_out.begin(), db_out.end(),
                     &db_in[0u] + (std::numeric_limits<out_type>::digits - 5)));
      // 1 - 7 ignore.
      r = detail::read_uint<out_type, 1u, 7u>(&input_vector[0u],
                                              input_vector.size(), 1u);
      ret_to_bitset(r, 7u);
      ...
      BOOST_CHECK(
          std::equal(db_out.begin(), db_out.end(),
                     &db_in[0u] + (std::numeric_limits<out_type>::digits - 7)));
    }
    if (n_rets(3u, 0u) > 1u) {
      // 3 - 0 ignore.
      vec_to_bitset(3u);
      r = detail::read_uint<out_type, 3u>(&input_vector[0u],
                                          input_vector.size(), 1u);
      ret_to_bitset(r, 0u);
      BOOST_CHECK(db_in.size() >= db_out.size());
      BOOST_CHECK(
          std::equal(db_out.begin(), db_out.end(),
                     &db_in[0u] + std::numeric_limits<out_type>::digits));
      // 3 - 2 ignore.
      r = detail::read_uint<out_type, 3u, 2u>(&input_vector[0u],
                                              input_vector.size(), 1u);
      ret_to_bitset(r, 2u);
      BOOST_CHECK(db_in.size() >= db_out.size());
      BOOST_CHECK(
          std::equal(db_out.begin(), db_out.end(),
                     &db_in[0u] + (std::numeric_limits<out_type>::digits - 2)));
      // 3 - 4 ignore.
      r = detail::read_uint<out_type, 3u, 4u>(&input_vector[0u],
                                              input_vector.size(), 1u);
      ret_to_bitset(r, 4u);
      ...
      BOOST_CHECK(
          std::equal(db_out.begin(), db_out.end(),
                     &db_in[0u] + (std::numeric_limits<out_type>::digits - 4)));
    }
    if (n_rets(5u, 0u) > 1u) {
      // 5 - 0 ignore.
      vec_to_bitset(5u);
      r = detail::read_uint<out_type, 5u>(&input_vector[0u],
                                          input_vector.size(), 1u);
      ret_to_bitset(r, 0u);
      BOOST_CHECK(db_in.size() >= db_out.size());
      BOOST_CHECK(
          std::equal(db_out.begin(), db_out.end(),
                     &db_in[0u] + std::numeric_limits<out_type>::digits));
      // 5 - 1 ignore.
      r = detail::read_uint<out_type, 5u, 1u>(&input_vector[0u],
                                              input_vector.size(), 1u);
      ret_to_bitset(r, 1u);
      BOOST_CHECK(db_in.size() >= db_out.size());
      BOOST_CHECK(
          std::equal(db_out.begin(), db_out.end(),
                     &db_in[0u] + (std::numeric_limits<out_type>::digits - 1)));
      // 5 - 7 ignore.
      r = detail::read_uint<out_type, 5u, 7u>(&input_vector[0u],
                                              input_vector.size(), 1u);
      ret_to_bitset(r, 7u);
      BOOST_CHECK(db_in.size() >= db_out.size());
      BOOST_CHECK(
    ...
    if (n_rets(7u, 0u) > 1u) {
      // 7 - 0 ignore.
      vec_to_bitset(7u);
      r = detail::read_uint<out_type, 7u>(&input_vector[0u],
                                          input_vector.size(), 1u);
      ret_to_bitset(r, 0u);
      BOOST_CHECK(db_in.size() >= db_out.size());
      BOOST_CHECK(
          std::equal(db_out.begin(), db_out.end(),
                     &db_in[0u] + std::numeric_limits<out_type>::digits));
      // 7 - 3 ignore.
      r = detail::read_uint<out_type, 7u, 3u>(&input_vector[0u],
                                              input_vector.size(), 1u);
      ret_to_bitset(r, 3u);
      BOOST_CHECK(db_in.size() >= db_out.size());
      BOOST_CHECK(
          std::equal(db_out.begin(), db_out.end(),
                     &db_in[0u] + (std::numeric_limits<out_type>::digits - 3)));
      // 7 - 7 ignore.
      r = detail::read_uint<out_type, 7u, 7u>(&input_vector[0u],
                                              input_vector.size(), 1u);
      ret_to_bitset(r, 7u);
    ...
    BOOST_CHECK(
        std::equal(db_out.begin(), db_out.end(),
                   &db_in[0u] + 2 * std::numeric_limits<out_type>::digits));
    if (n_rets(1u, 0u) > 2u) {
      // 1 - 0 ignore.
      vec_to_bitset(1u);
      r = detail::read_uint<out_type, 1u>(&input_vector[0u],
                                          input_vector.size(), 2u);
      ret_to_bitset(r, 0u);
      BOOST_CHECK(db_in.size() >= db_out.size());
      BOOST_CHECK(
          std::equal(db_out.begin(), db_out.end(),
      ...
      BOOST_CHECK(std::equal(
          db_out.begin(), db_out.end(),
          &db_in[0u] + 2 * (std::numeric_limits<out_type>::digits - 1)));
      // 1 - 7 ignore.
      r = detail::read_uint<out_type, 1u, 7u>(&input_vector[0u],
                                              input_vector.size(), 2u);
      ret_to_bitset(r, 7u);
      ...
      BOOST_CHECK(std::equal(
          db_out.begin(), db_out.end(),
          &db_in[0u] + 2 * (std::numeric_limits<out_type>::digits - 7)));
    }
    if (n_rets(3u, 0u) > 2u) {
      // 3 - 0 ignore.
      vec_to_bitset(3u);
      ...
      BOOST_CHECK(std::equal(
          db_out.begin(), db_out.end(),
          &db_in[0u] + 2 * (std::numeric_limits<out_type>::digits - 2)));
      // 3 - 5 ignore.
      r = detail::read_uint<out_type, 3u, 5u>(&input_vector[0u],
                                              input_vector.size(), 2u);
      ret_to_bitset(r, 5u);
      BOOST_CHECK(db_in.size() >= db_out.size());
      BOOST_CHECK(std::equal(
          db_out.begin(), db_out.end(),
          &db_in[0u] + 2 * (std::numeric_limits<out_type>::digits - 5)));
    }
    if (n_rets(5u, 0u) > 2u) {
      // 5 - 0 ignore.
      vec_to_bitset(5u);
      ...
      BOOST_CHECK(std::equal(
          db_out.begin(), db_out.end(),
          &db_in[0u] + 2 * (std::numeric_limits<out_type>::digits - 6)));
      // 5 - 7 ignore.
      r = detail::read_uint<out_type, 5u, 7u>(&input_vector[0u],
                                              input_vector.size(), 2u);
      ret_to_bitset(r, 7u);
    ...
    BOOST_CHECK(
        std::equal(db_out.begin(), db_out.end(),
                   &db_in[0u] + 4 * std::numeric_limits<out_type>::digits));
    if (n_rets(1u, 0u) > 4u) {
      // 1 - 0 ignore.
      vec_to_bitset(1u);
      r = detail::read_uint<out_type, 1u>(&input_vector[0u],
                                          input_vector.size(), 4u);
      ret_to_bitset(r, 0u);
      BOOST_CHECK(db_in.size() >= db_out.size());
      BOOST_CHECK(
          std::equal(db_out.begin(), db_out.end(),
      ...
      BOOST_CHECK(std::equal(
          db_out.begin(), db_out.end(),
          &db_in[0u] + 4 * (std::numeric_limits<out_type>::digits - 1)));
      // 1 - 4 ignore.
      r = detail::read_uint<out_type, 1u, 4u>(&input_vector[0u],
                                              input_vector.size(), 4u);
      ret_to_bitset(r, 4u);
      BOOST_CHECK(db_in.size() >= db_out.size());
      BOOST_CHECK(std::equal(
          db_out.begin(), db_out.end(),
          &db_in[0u] + 4 * (std::numeric_limits<out_type>::digits - 4)));
    }
    if (n_rets(3u, 0u) > 4u) {
      // 3 - 0 ignore.
      vec_to_bitset(3u);
      ...
      BOOST_CHECK(
          std::equal(db_out.begin(), db_out.end(),
                     &db_in[0u] + 4 * std::numeric_limits<out_type>::digits));
      // 3 - 3 ignore.
      r = detail::read_uint<out_type, 3u, 3u>(&input_vector[0u],
                                              input_vector.size(), 4u);
      ret_to_bitset(r, 3u);
      BOOST_CHECK(db_in.size() >= db_out.size());
      BOOST_CHECK(std::equal(
          db_out.begin(), db_out.end(),
          &db_in[0u] + 4 * (std::numeric_limits<out_type>::digits - 3)));
      // 3 - 5 ignore.
      r = detail::read_uint<out_type, 3u, 5u>(&input_vector[0u],
                                              input_vector.size(), 4u);
      ret_to_bitset(r, 5u);
      ...
      BOOST_CHECK(std::equal(
          db_out.begin(), db_out.end(),
          &db_in[0u] + 4 * (std::numeric_limits<out_type>::digits - 5)));
    }
    if (n_rets(5u, 0u) > 4u) {
      // 5 - 0 ignore.
      vec_to_bitset(5u);
      ...
      BOOST_CHECK(std::equal(
          db_out.begin(), db_out.end(),
          &db_in[0u] + 4 * (std::numeric_limits<out_type>::digits - 7)));
    }
    if (n_rets(7u, 0u) > 4u) {
      // 7 - 0 ignore.
      vec_to_bitset(7u);
      ...
      BOOST_CHECK(
          std::equal(db_out.begin(), db_out.end(),
                     &db_in[0u] + 4 * std::numeric_limits<out_type>::digits));
      // 7 - 2 ignore.
      r = detail::read_uint<out_type, 7u, 2u>(&input_vector[0u],
                                              input_vector.size(), 4u);
      ret_to_bitset(r, 2u);
      BOOST_CHECK(db_in.size() >= db_out.size());
      BOOST_CHECK(std::equal(
          db_out.begin(), db_out.end(),
          &db_in[0u] + 4 * (std::numeric_limits<out_type>::digits - 2)));
      // 7 - 7 ignore.
      r = detail::read_uint<out_type, 7u, 7u>(&input_vector[0u],
                                              input_vector.size(), 4u);
      ret_to_bitset(r, 7u);
    ...
    BOOST_CHECK(
        std::equal(db_out.begin(), db_out.end(),
                   &db_in[0u] + 6 * std::numeric_limits<out_type>::digits));
    if (n_rets(1u, 0u) > 6u) {
      // 1 - 0 ignore.
      vec_to_bitset(1u);
      r = detail::read_uint<out_type, 1u>(&input_vector[0u],
                                          input_vector.size(), 6u);
      ret_to_bitset(r, 0u);
      BOOST_CHECK(db_in.size() >= db_out.size());
      BOOST_CHECK(
          std::equal(db_out.begin(), db_out.end(),
      ...
      BOOST_CHECK(std::equal(
          db_out.begin(), db_out.end(),
          &db_in[0u] + 6 * (std::numeric_limits<out_type>::digits - 1)));
      // 1 - 5 ignore.
      r = detail::read_uint<out_type, 1u, 5u>(&input_vector[0u],
                                              input_vector.size(), 6u);
      ret_to_bitset(r, 5u);
      ...
      BOOST_CHECK(std::equal(
          db_out.begin(), db_out.end(),
          &db_in[0u] + 6 * (std::numeric_limits<out_type>::digits - 5)));
    }
    if (n_rets(3u, 0u) > 6u) {
      // 3 - 0 ignore.
      vec_to_bitset(3u);
      ...
      BOOST_CHECK(std::equal(
          db_out.begin(), db_out.end(),
          &db_in[0u] + 6 * (std::numeric_limits<out_type>::digits - 3)));
      // 3 - 7 ignore.
      r = detail::read_uint<out_type, 3u, 7u>(&input_vector[0u],
                                              input_vector.size(), 6u);
      ret_to_bitset(r, 7u);
      ...
      BOOST_CHECK(std::equal(
          db_out.begin(), db_out.end(),
          &db_in[0u] + 6 * (std::numeric_limits<out_type>::digits - 7)));
    }
    if (n_rets(5u, 0u) > 6u) {
      // 5 - 0 ignore.
      vec_to_bitset(5u);
      ...
      BOOST_CHECK(std::equal(
          ...
          &db_in[0u] + 6 * (std::numeric_limits<out_type>::digits - 1)));
      // 5 - 5 ignore.
      r = detail::read_uint<out_type, 5u, 5u>(&input_vector[0u],
                                              input_vector.size(), 6u);
      ret_to_bitset(r, 5u);
      BOOST_CHECK(db_in.size() >= db_out.size());
      BOOST_CHECK(std::equal(
    ...
    if (n_rets(7u, 0u) > 6u) {
      ...
      BOOST_CHECK(
          std::equal(db_out.begin(), db_out.end(),
                     &db_in[0u] + 6 * std::numeric_limits<out_type>::digits));
      // 7 - 6 ignore.
      r = detail::read_uint<out_type, 7u, 6u>(&input_vector[0u],
                                              input_vector.size(), 6u);
      ret_to_bitset(r, 6u);
      BOOST_CHECK(db_in.size() >= db_out.size());
      BOOST_CHECK(std::equal(
          db_out.begin(), db_out.end(),
          &db_in[0u] + 6 * (std::numeric_limits<out_type>::digits - 6)));
      // 7 - 7 ignore.
      r = detail::read_uint<out_type, 7u, 7u>(&input_vector[0u],
                                              input_vector.size(), 6u);
      ret_to_bitset(r, 7u);
...
struct ctb_tester {
  ...
  void operator()(
    ...
    const unsigned nbits =
        static_cast<unsigned>(std::numeric_limits<T>::digits);
    ...
    BOOST_CHECK_EQUAL(
        detail::clear_top_bits(static_cast<T>(T(1) << (nbits - 1u)), 1u), T(0));
    BOOST_CHECK_EQUAL(
        detail::clear_top_bits(static_cast<T>(T(1) << (nbits - 2u)), 1u),
        static_cast<T>(T(1) << (nbits - 2u)));
    BOOST_CHECK_EQUAL(
        detail::clear_top_bits(static_cast<T>(T(1) << (nbits - 2u)), 2u), T(0));
    BOOST_CHECK_EQUAL(detail::clear_top_bits(static_cast<T>(67), nbits - 1u),
                      T(1));
  }
  template <typename T>
  void operator()(
...
struct get_mpz_ptr_tester {
  template <typename T> void operator()(const T &) {
    ...
    // Random tests.
    std::uniform_int_distribution<int> ud(std::numeric_limits<int>::min(),
                                          std::numeric_limits<int>::max());
    for (int i = 0; i < ntries; ++i) {
      auto tmp = ud(rng);

...
using key_types =
    boost::mpl::vector<k_monomial, monomial<unsigned char>, monomial<integer>>;
...
struct constructor_tester {
  template <typename Key> void operator()(const Key &) {
    ...
    BOOST_CHECK(
        (std::is_constructible<r_type, const r_type, r_type &&>::value));
    ...
    BOOST_CHECK((std::is_constructible<r_type, p_type &, p_type>::value));
    BOOST_CHECK(
        (std::is_constructible<r_type, p_type &&, const p_type &>::value));
    BOOST_CHECK((std::is_constructible<r_type, q_type, q_type>::value));
    BOOST_CHECK((std::is_constructible<r_type, q_type &, q_type>::value));
    BOOST_CHECK(
        (std::is_constructible<r_type, q_type &&, const q_type &>::value));
    BOOST_CHECK((std::is_constructible<r_type, int, int>::value));
    BOOST_CHECK((std::is_constructible<r_type, char &, char>::value));
    BOOST_CHECK(
        (std::is_constructible<r_type, const integer &, integer>::value));
    BOOST_CHECK((std::is_constructible<r_type, rational, rational>::value));
    BOOST_CHECK(
        (std::is_constructible<r_type, const rational &, rational &&>::value));
    BOOST_CHECK(
        (std::is_constructible<r_type, std::string, std::string>::value));
    ...
    BOOST_CHECK((std::is_constructible<r_type, char *, char *>::value));
    BOOST_CHECK(
        (std::is_constructible<r_type, const char *, const char *>::value));
    ...
    BOOST_CHECK(r.num().get_symbol_set().size() == 0u);
    BOOST_CHECK(r.den().get_symbol_set().size() == 0u);
    BOOST_CHECK_THROW((r = r_type{(x + y) * (x - y), p_type{}}),
                      zero_division_error);
    BOOST_CHECK_THROW((r = r_type{(x + y) * (x - y), x.pow(-1)}),
                      std::invalid_argument);
    BOOST_CHECK_THROW((r = r_type{x.pow(-1), (x + y) * (x - y)}),
                      std::invalid_argument);
    // From rational.
    r = r_type{0_q, -6_q};
    BOOST_CHECK_EQUAL(r.num(), 0);
    ...
    BOOST_CHECK(r.num().get_symbol_set().size() == 0u);
    BOOST_CHECK(r.den().get_symbol_set().size() == 0u);
    r = r_type{(xq / 3 + 3 * xq * yq / 4) * (xq * xq + yq * yq),
               xq.pow(3) * (4 * xq + 9 * xq * yq) * (xq - yq) / 2};
    ...
    BOOST_CHECK_THROW((r = r_type{r_type{1, x + 3 * y}, 0}),
                      zero_division_error);
    BOOST_CHECK_THROW((r = r_type{r_type{1, x + 3 * y}, q_type{}}),
                      zero_division_error);
...
struct stream_tester {
  template <typename Key> void operator()(const Key &) {
    ...
    r = r_type{x, y};
    str_cmp(r, "x/y");
    // This was printed incorrectly (without brackets in den) in a previous
    // version.
    r = r_type{y, x * z};
    str_cmp(r, "y/(x*z)");
  }
...
struct add_tester {
  template <typename Key> void operator()(const Key &) {
    ...
    checker(q_type{"x"} / 2 + r_type{x, y + x},
            r_type{2 * x + x * x + x * y, 2 * (x + y)});
    checker(r_type{x, y + x} + q_type{"x"} / 2,
            r_type{2 * x + x * x + x * y, 2 * (x + y)});
    // Random testing.
    ...
    // Identity operator.
    BOOST_CHECK_EQUAL((+r_type{2 * x * x + y * y, y * x}),
                      (r_type{2 * x * x + y * y, y * x}));
  }
};
...
struct subs_tester {
  template <typename Key> void operator()(const Key &) {
    ...
    BOOST_CHECK((std::is_same<r_type, decltype(math::subs((x + y) / z, "z",
                                                          -x - y))>::value));
    BOOST_CHECK_EQUAL(math::subs((x + y) / z, "x", 123_z), (123 + y) / z);
    BOOST_CHECK((std::is_same<r_type, decltype(math::subs((x + y) / z, "x",
                                                          123_z))>::value));
    BOOST_CHECK_EQUAL(math::subs((x + y) / z, "x", 3 / 2_q),
                      (3 + 2 * y) / (2 * z));
    BOOST_CHECK((std::is_same<r_type, decltype(math::subs((x + y) / z, "x",
                                                          3 / 2_q))>::value));
    BOOST_CHECK_EQUAL(math::subs((x + y) / z, "y", p_type("z") * 3),
                      (x + 3 * z) / z);
    BOOST_CHECK(
        (std::is_same<r_type, decltype(math::subs((x + y) / z, "y",
                                                  p_type("z") * 3))>::value));
    BOOST_CHECK_EQUAL(math::subs((x + y) / z, "z", q_type("z") / 6),
                      6 * (x + y) / z);
    BOOST_CHECK(
        (std::is_same<r_type, decltype(math::subs((x + y) / z, "z",
                                                  q_type("z") / 6))>::value));
    ...
    BOOST_CHECK_THROW(math::subs(x / (z + y), "x", p_type{"x"}.pow(-1)),
                      std::invalid_argument);
  }
};
...
struct ipow_subs_tester {
  template <typename Key> void operator()(const Key &) {
    ...
    BOOST_CHECK(
        (std::is_same<r_type, decltype(x.ipow_subs("x", 1_z, y))>::value));
    BOOST_CHECK_THROW((1 / (x * x)).ipow_subs("x", 2_z, 0),
                      zero_division_error);
    BOOST_CHECK(
        (std::is_same<r_type,
                      decltype((1 / (x * x)).ipow_subs("x", 2_z, 0))>::value));
    BOOST_CHECK_EQUAL((1 / (x * x)).ipow_subs("x", 2_z, y), 1 / y);
    BOOST_CHECK_EQUAL(math::ipow_subs((x + y) / (z * z), "z", 2, -x - y), -1);
    BOOST_CHECK_EQUAL(math::ipow_subs((x * x * x + y) / z, "x", 2, 123_z),
                      (123 * x + y) / z);
    BOOST_CHECK_EQUAL(math::ipow_subs((x + y) / z, "x", 2, 3 / 2_q),
                      (x + y) / z);
    BOOST_CHECK(
        (std::is_same<r_type, decltype(math::ipow_subs((x + y) / z, "x", 2,
                                                       3 / 2_q))>::value));
    BOOST_CHECK_EQUAL(
        math::ipow_subs((x + y * y * y * y) / z, "y", 2, p_type("z") * 3),
        (x + 9 * z * z) / z);
    BOOST_CHECK((std::is_same<r_type, decltype(math::ipow_subs(
                                          (x + y * y * y * y) / z, "y", 2,
                                          p_type("z") * 3))>::value));
    BOOST_CHECK_EQUAL(
        math::ipow_subs((x + y) / (z * z), "z", 2, q_type("z") / 6),
        6 * (x + y) / z);
    BOOST_CHECK((std::is_same<r_type, decltype(math::ipow_subs(
                                          (x + y) / (z * z), "z", 2,
                                          q_type("z") / 6))>::value));
    BOOST_CHECK_EQUAL(math::ipow_subs((x + y) / z, "a", 123, 123_z),
                      (x + y) / z);
    BOOST_CHECK_EQUAL(math::ipow_subs(x / (z + y), "x", 1, 0), 0);
    BOOST_CHECK_EQUAL(math::ipow_subs(x / (z + y), "x", -1, 0), x / (z + y));
    // Check that using negative powers throws.
    BOOST_CHECK_THROW(math::ipow_subs(x / (z + y), "x", 1, p_type{"x"}.pow(-1)),
                      std::invalid_argument);
  }
};
...
struct partial_tester {
  template <typename Key> void operator()(const Key &) {
    ...
    using p_type = typename r_type::p_type;
    {
      ...
      BOOST_CHECK_EQUAL(partial((4 * x - 2) / (x * x + 1), "x"),
                        (-4 * x * x + 4 * x + 4) / pow(x * x + 1, 2));
    }
    // Random testing.
    p_type x{"x"}, y{"y"}, z{"z"};
    std::uniform_int_distribution<int> dist(0, 4), p_dist(-4, 4);
    for (int i = 0; i < ntrials; ++i) {
      ...
      r_type r1{n1, d1};
      BOOST_CHECK_EQUAL(partial(r1, "x"),
                        (r_type{partial(r1.num(), "x") * r1.den() -
                                    partial(r1.den(), "x") * r1.num(),
                                r1.den() * r1.den()}));
    }
  }
};
...
struct integrate_tester {
  template <typename Key> void operator()(const Key &) {
    ...
    BOOST_CHECK_EQUAL(integrate((7 * x * x + y * x) / (2 * y), "x"),
                      (14 * x * x * x + 3 * x * x * y) / (12 * y));
  }
};
...
struct print_tex_tester {
  template <typename Key> void operator()(const Key &) {
    ...
    tex_checker(-x * y / 2, "-\\frac{{x}{y}}{2}");
    tex_checker(-x * y / (x + 2), "-\\frac{{x}{y}}{{x}+2}",
                "-\\frac{{x}{y}}{2+{x}}");
    tex_checker(x * y / 2, "\\frac{{x}{y}}{2}");
    tex_checker(x * y / (x + 2), "\\frac{{x}{y}}{{x}+2}",
                "\\frac{{x}{y}}{2+{x}}");
    tex_checker(x * y / (x - 2), "\\frac{{x}{y}}{{x}-2}",
                "-\\frac{{x}{y}}{2-{x}}");
    tex_checker((x - 3 * y) / x, "\\frac{{x}-3{y}}{{x}}",
                "-\\frac{3{y}-{x}}{{x}}");
    tex_checker((x - 2 * y) / (x - y), "\\frac{{x}-2{y}}{{x}-{y}}",
                "\\frac{2{y}-{x}}{{y}-{x}}");
  }
};
...
struct evaluate_tester {
  template <typename Key> void operator()(const Key &) {
    ...
    r_type x{"x"}, y{"y"};
    BOOST_CHECK((
        std::is_same<decltype(evaluate<rational>(
                         (x + y) / (3 * y), {{"x", 1 / 2_q}, {"y", -3 / 5_q}})),
                     rational>::value));
    BOOST_CHECK((std::is_same<decltype(evaluate<double>(
                                  (x + y) / (3 * y), {{"x", 1.}, {"y", 2.}})),
                              double>::value));
    BOOST_CHECK((std::is_same<decltype(evaluate<integer>(
                                  (x + y) / (3 * y), {{"x", 1_z}, {"y", 2_z}})),
                              integer>::value));
    BOOST_CHECK((std::is_same<decltype(evaluate<r_type>(
                                  (x + y) / (3 * y),
                                  {{"x", r_type{}}, {"y", r_type{}}})),
                              r_type>::value));
    BOOST_CHECK_EQUAL((evaluate<rational>((2 * x * x + y) / (3 * y),
                                          {{"x", 1 / 2_q}, {"y", -3 / 5_q}})),
                      1 / 18_q);
    BOOST_CHECK_EQUAL((evaluate<integer>((2 * x * x + y) / (3 * y),
                                         {{"x", 2_z}, {"y", 3_z}})),
                      1_z);
    BOOST_CHECK_EQUAL(
        (evaluate<r_type>((2 * x * x + y) / (3 * y),
                          {{"x", r_type{"y"}}, {"y", r_type{"x"}}})),
        (2 * y * y + x) / (3 * x));
    BOOST_CHECK_THROW((evaluate<rational>((2 * x * x + y) / (3 * y),
                                          {{"x", 1 / 2_q}, {"y", 0_q}})),
                      zero_division_error);
  }
};
...
struct trim_tester {
  template <typename Key> void operator()(const Key &) {
    ...
    using p_type = typename r_type::p_type;
    {
      ...
      BOOST_CHECK(r.den().get_symbol_set() != r_trim.den().get_symbol_set());
      BOOST_CHECK(
          (r.num().get_symbol_set() == symbol_set{symbol{"x"}, symbol{"y"}}));

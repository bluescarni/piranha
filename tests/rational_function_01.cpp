...
struct ipow_subs_tester {
    ...
    void operator()(const Key &)
    {
        ...
        BOOST_CHECK(
            (std::is_same<r_type, decltype(math::ipow_subs((x + y * y * y * y) / z, "y", 2, p_type("z") * 3))>::value));
        BOOST_CHECK_EQUAL(math::ipow_subs((x + y) / (z * z), "z", 2, q_type("z") / 6), 6 * (x + y) / z);
        BOOST_CHECK(
            (std::is_same<r_type, decltype(math::ipow_subs((x + y) / (z * z), "z", 2, q_type("z") / 6))>::value));
...
struct partial_tester {
    ...
    void operator()(const Key &)
    {
        ...
        for (int i = 0; i < ntrials; ++i) {
            ...
            BOOST_CHECK_EQUAL(
                partial(r1, "x"),
                (r_type{partial(r1.num(), "x") * r1.den() - partial(r1.den(), "x") * r1.num(), r1.den() * r1.den()}));
        }
    }
};
...
struct evaluate_tester {
    ...
    void operator()(const Key &)
    {
        ...
        r_type x{"x"}, y{"y"};
        BOOST_CHECK((std::is_same<decltype(evaluate<rational>((x + y) / (3 * y), {{"x", 1 / 2_q}, {"y", -3 / 5_q}})),
                                  rational>::value));
        BOOST_CHECK(
            (std::is_same<decltype(evaluate<double>((x + y) / (3 * y), {{"x", 1.}, {"y", 2.}})), double>::value));
        BOOST_CHECK(
            (std::is_same<decltype(evaluate<integer>((x + y) / (3 * y), {{"x", 1_z}, {"y", 2_z}})), integer>::value));
        BOOST_CHECK((std::is_same<decltype(evaluate<r_type>((x + y) / (3 * y), {{"x", r_type{}}, {"y", r_type{}}})),
                                  r_type>::value));
        ...
        BOOST_CHECK_EQUAL((evaluate<r_type>((2 * x * x + y) / (3 * y), {{"x", r_type{"y"}}, {"y", r_type{"x"}}})),
                          (2 * y * y + x) / (3 * x));
        BOOST_CHECK_THROW((evaluate<rational>((2 * x * x + y) / (3 * y), {{"x", 1 / 2_q}, {"y", 0_q}})),
                          zero_division_error);
    }
};

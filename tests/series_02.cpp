...
class g_series_type
    : public series<Cf, monomial<Expo>, g_series_type<Cf, Expo>> {
  typedef series<Cf, monomial<Expo>, g_series_type<Cf, Expo>> base;
  PIRANHA_SERIALIZE_THROUGH_BASE(base)
public:
  template <typename Cf2> using rebind = g_series_type<Cf2, Expo>;
...
template <typename Cf, typename Expo>
class g_series_type2
    : public series<Cf, monomial<Expo>, g_series_type2<Cf, Expo>> {
public:
  ...
  g_series_type2 sin() const { return g_series_type2(42); }
  g_series_type2 cos() const { return g_series_type2(-42); }
};
...
class g_series_type3 : public series<Cf, Key, g_series_type3<Cf, Key>> {
  ...
  PIRANHA_SERIALIZE_THROUGH_BASE(base)
public:
  template <typename Cf2> using rebind = g_series_type3<Cf2, Key>;
...
template <typename Cf, typename Key>
class series_multiplier<g_series_type<Cf, Key>, void>
    : public base_series_multiplier<g_series_type<Cf, Key>> {
...
template <typename Cf, typename Key>
class series_multiplier<g_series_type2<Cf, Key>, void>
    : public base_series_multiplier<g_series_type2<Cf, Key>> {
...
template <typename Cf, typename Key>
class series_multiplier<g_series_type3<Cf, Key>, void>
    : public base_series_multiplier<g_series_type3<Cf, Key>> {
...
BOOST_AUTO_TEST_CASE(series_partial_test) {
  init();
  {
    ...
    BOOST_CHECK_EQUAL(math::partial(math::partial(-4 * x.pow(2), "x"), "x"),
                      -8);
    BOOST_CHECK_EQUAL(
        math::partial(math::partial(math::partial(-4 * x.pow(2), "x"), "x"),
                      "x"),
        0);
    BOOST_CHECK_EQUAL(math::partial(-x + 1, "x"), -1);
    BOOST_CHECK_EQUAL(math::partial((1 + 2 * x).pow(10), "x"),
                      20 * (1 + 2 * x).pow(9));
    BOOST_CHECK_EQUAL(math::partial((1 + 2 * x + y).pow(10), "x"),
                      20 * (1 + 2 * x + y).pow(9));
    ...
    p_type1::register_custom_derivative(
        "x", [](const p_type1 &) { return p_type1{rational(1, 314)}; });
    BOOST_CHECK_EQUAL(math::partial(x, "x"), rational(1, 314));
    p_type1::register_custom_derivative(
        "x", [](const p_type1 &) { return p_type1{rational(1, 315)}; });
    ...
    p_type1::register_custom_derivative(
        "x", [](const p_type1 &p) { return p.partial("x"); });
    BOOST_CHECK_EQUAL(math::partial(x + y, "x"), 1);
    ...
    p_type1::register_custom_derivative(
        "y", [](const p_type1 &p) -> p_type1 { return 2 * p; });
  ...
  }
  {
    ...
    BOOST_CHECK(
        (std::is_same<decltype(x2.partial("foo")), p_type2_diff>::value));
    p_type2 x{"x"}, y{"y"};
    BOOST_CHECK_EQUAL(math::partial(x, "x"), 1);
    BOOST_CHECK_EQUAL(math::partial(x, "y"), 0);
    BOOST_CHECK_EQUAL(math::partial(-4 * x.pow(2), "x"), -8 * x);
    ...
    p_type2::register_custom_derivative(
        "x", [](const p_type2 &) { return p_type2_diff{rational(1, 314)}; });
    BOOST_CHECK_EQUAL(math::partial(x, "x"), rational(1, 314));
    p_type2::register_custom_derivative(
        "x", [](const p_type2 &) { return p_type2_diff{rational(1, 315)}; });
    BOOST_CHECK_EQUAL(math::partial(x, "x"), rational(1, 315));
    ...
    BOOST_CHECK_EQUAL(math::partial(x + 2 * y, "y"), 2);
    p_type2::register_custom_derivative(
        "x", [](const p_type2 &p) { return p.partial("x"); });
    BOOST_CHECK_EQUAL(math::partial(x + y, "x"), 1);
    BOOST_CHECK_EQUAL(math::partial(x + y * x, "x"), y + 1);
    p_type2::register_custom_derivative("x", [x](const p_type2 &p) {
      return p.partial("x") + math::partial(p, "y") * 2 * x;
    ...
    p_type2::register_custom_derivative(
        "y", [](const p_type2 &p) { return 2 * p_type2_diff{p}; });
    BOOST_CHECK_EQUAL(math::partial(x + y, "x"), 1 + 4 * x * (x + y));
    BOOST_CHECK_EQUAL(math::partial(x + y, "y"), 2 * (x + y));
  ...
  BOOST_CHECK((!is_differentiable<g_series_type<mock_cf, rational>>::value));
  {
    ...
    BOOST_CHECK_EQUAL(math::partial(s0{"y"} * math::pow(ss0{"x"}, 5), "x"),
                      5 * s0{"y"} * math::pow(ss0{"x"}, 4));
  }
}
...
BOOST_AUTO_TEST_CASE(series_serialization_test) {
  ...
  for (int i = 0; i < ntries; ++i) {
    ...
    for (unsigned j = 0u; j < size; ++j) {
      p += math::pow(x, int_dist(rng)) * math::pow(y, int_dist(rng)) *
           math::pow(z, int_dist(rng));
    }
    p *= int_dist(rng);
  ...
  }
  {
    ...
    const std::string ba = "22 serialization::archive 10 0 0 0 0 0 0 2 1 x 1 y "
                           "2 0 0 0 0 0 0 1 1 1 2 0 0 0 0 0 0 2 2 0 1 1 1 3 3 "
                           "0 2 0";
    ss.str(ba);
    boost::archive::text_iarchive ia(ss);
  ...
  }
  {
    ...
    std::stringstream ss;
    const std::string ba = "22 serialization::archive 10 0 0 0 0 0 0 2 1 x 1 y "
                           "2 0 0 0 0 0 0 1 1 1 2 0 0 0 0 0 0 2 2 0 1 1 1 3 2 "
                           "2 0";
    ss.str(ba);
    boost::archive::text_iarchive ia(ss);
    ia >> tmp;
  ...
  }
  {
    ...
    std::stringstream ss;
    const std::string ba = "22 serialization::archive 10 0 0 0 0 0 0 2 1 x 1 y "
                           "2 0 0 0 0 0 0 1 0 1 2 0 0 0 0 0 0 2 2 0 1 0 1 3 2 "
                           "2 0";
    ss.str(ba);
    boost::archive::text_iarchive ia(ss);
    ia >> tmp;
...
BOOST_AUTO_TEST_CASE(series_evaluate_test) {
  ...
  BOOST_CHECK((std::is_same<rational, decltype(p_type1{}.evaluate(
                                          dict_type_int{}))>::value));
  BOOST_CHECK((std::is_same<rational, decltype(p_type1{}.evaluate(
                                          dict_type_long{}))>::value));
  ...
  BOOST_CHECK_THROW(
      (x + (2 * y).pow(3)).evaluate(dict_type{{"x", rational(1)}}),
      std::invalid_argument);
  BOOST_CHECK_EQUAL(
      (x + (2 * y).pow(3))
          .evaluate(dict_type{{"x", rational(1)}, {"y", rational(2, 3)}}),
      rational(1) + (2 * rational(2, 3)).pow(3));
  BOOST_CHECK_EQUAL(
      (x + (2 * y).pow(3))
          .evaluate(dict_type{{"x", rational(1)}, {"y", rational(2, 3)}}),
      math::evaluate(x + (2 * y).pow(3),
                     dict_type{{"x", rational(1)}, {"y", rational(2, 3)}}));
  BOOST_CHECK((std::is_same<decltype(p_type1{}.evaluate(dict_type{})),
                            rational>::value));
  typedef std::unordered_map<std::string, real> dict_type2;
  BOOST_CHECK((is_evaluable<p_type1, real>::value));
  BOOST_CHECK_EQUAL((x + (2 * y).pow(3))
                        .evaluate(dict_type2{{"x", real(1.234)},
                                             {"y", real(-5.678)},
                                             {"z", real()}}),
  ...
  BOOST_CHECK_EQUAL(
      (x + (2 * y).pow(3))
          .evaluate(dict_type2{
              {"x", real(1.234)}, {"y", real(-5.678)}, {"z", real()}}),
      math::evaluate(
          x + math::pow(2 * y, 3),
          dict_type2{{"x", real(1.234)}, {"y", real(-5.678)}, {"z", real()}}));
  BOOST_CHECK(
      (std::is_same<decltype(p_type1{}.evaluate(dict_type2{})), real>::value));
  typedef std::unordered_map<std::string, double> dict_type3;
  BOOST_CHECK((is_evaluable<p_type1, double>::value));
  BOOST_CHECK_EQUAL(
      (x + (2 * y).pow(3))
          .evaluate(dict_type3{{"x", 1.234}, {"y", -5.678}, {"z", 0.0001}}),
      1.234 + math::pow(2 * -5.678, 3));
  BOOST_CHECK_EQUAL(
      (x + (2 * y).pow(3))
          .evaluate(dict_type3{{"x", 1.234}, {"y", -5.678}, {"z", 0.0001}}),
      math::evaluate(x + math::pow(2 * y, 3),
                     dict_type3{{"x", 1.234}, {"y", -5.678}, {"z", 0.0001}}));
  BOOST_CHECK((
      std::is_same<decltype(p_type1{}.evaluate(dict_type3{})), double>::value));
  BOOST_CHECK((!is_evaluable<g_series_type3<double, mock_key>, double>::value));
  // NOTE: this used to be true before we changed the ctor from int of mock_cf
  // to explicit.
  BOOST_CHECK(
      (!is_evaluable<g_series_type3<mock_cf, monomial<int>>, double>::value));
  BOOST_CHECK(
      (!is_evaluable<g_series_type3<mock_cf, mock_key>, double>::value));
  BOOST_CHECK(
      (is_evaluable<g_series_type3<double, monomial<int>>, double>::value));
  ...
  BOOST_CHECK_EQUAL(
      math::evaluate<double>(p_type1{}, {{"foo", 4.}, {"bar", 7}}), 0);
}

template <typename Expo>
class g_series_type_nr
    : public series<float, monomial<Expo>, g_series_type_nr<Expo>> {
...
template <typename Expo>
class g_series_type_nr2
    : public series<short, monomial<Expo>, g_series_type_nr2<Expo>> {
  typedef series<short, monomial<Expo>, g_series_type_nr2<Expo>> base;

public:
  template <typename Expo2> using rebind = g_series_type_nr2<Expo2>;
...
template <typename Expo>
class g_series_type_nr3
    : public series<float, monomial<Expo>, g_series_type_nr3<Expo>> {
  typedef series<float, monomial<Expo>, g_series_type_nr3<Expo>> base;

public:
  template <typename Expo2> using rebind = void;
...
BOOST_AUTO_TEST_CASE(series_series_is_rebindable_test) {
  ...
  BOOST_CHECK((std::is_same<series_rebind<p_type1, int>,
                            g_series_type<int, int>>::value));
  ...
  BOOST_CHECK(
      (std::is_same<series_rebind<p_type1 &, rational const>, p_type1>::value));
  BOOST_CHECK((series_is_rebindable<p_type1, p_type1>::value));
  BOOST_CHECK((series_is_rebindable<p_type1 &, p_type1>::value));
  BOOST_CHECK((series_is_rebindable<p_type1 &, const p_type1>::value));
  BOOST_CHECK((std::is_same<series_rebind<p_type1, p_type1>,
                            g_series_type<p_type1, int>>::value));
  ...
  BOOST_CHECK(
      (!series_is_rebindable<p_type1, std::vector<std::string>>::value));
  BOOST_CHECK(
      (!series_is_rebindable<p_type1, std::vector<std::string> &>::value));
  BOOST_CHECK((
      !series_is_rebindable<p_type1, const std::vector<std::string> &>::value));
  BOOST_CHECK(
      (!series_is_rebindable<p_type1, std::vector<std::string> &&>::value));
  BOOST_CHECK(
      (!series_is_rebindable<std::string, std::vector<std::string>>::value));
  BOOST_CHECK((!series_is_rebindable<const std::string &,
...
BOOST_AUTO_TEST_CASE(series_series_recursion_index_test) {
  ...
  BOOST_CHECK_EQUAL(
      (series_recursion_index<g_series_type<rational, int>>::value), 1u);
  BOOST_CHECK_EQUAL((series_recursion_index<g_series_type<float, int>>::value),
                    1u);
  BOOST_CHECK_EQUAL((series_recursion_index<g_series_type<double, int>>::value),
                    1u);
  BOOST_CHECK_EQUAL((series_recursion_index<
                        g_series_type<g_series_type<double, int>, int>>::value),
                    2u);
  BOOST_CHECK_EQUAL(
      (series_recursion_index<
          g_series_type<g_series_type<double, int>, long>>::value),
      2u);
  BOOST_CHECK_EQUAL(
      (series_recursion_index<g_series_type<
           g_series_type<g_series_type<double, int>, int>, long>>::value),
      3u);
  BOOST_CHECK_EQUAL(
      (series_recursion_index<g_series_type<
           g_series_type<g_series_type<rational, int>, int>, long>>::value),
      3u);
  BOOST_CHECK_EQUAL(
      (series_recursion_index<g_series_type<
           g_series_type<g_series_type<rational, int>, int>, long> &>::value),
  ...
  BOOST_CHECK_EQUAL(
      (series_recursion_index<
          g_series_type<g_series_type<g_series_type<rational, int>, int>,
                        long> const>::value),
      3u);
  BOOST_CHECK_EQUAL(
      (series_recursion_index<
...
using binary_series_op_return_type =
    detail::binary_series_op_return_type<T, U, 0>;

BOOST_AUTO_TEST_CASE(series_binary_series_op_return_type_test) {
  // Check missing type in case both operands are not series.
  BOOST_CHECK(
      (!has_typedef_type<binary_series_op_return_type<int, int>>::value));
  BOOST_CHECK(
      (!has_typedef_type<binary_series_op_return_type<int, double>>::value));
  BOOST_CHECK(
      (!has_typedef_type<binary_series_op_return_type<float, double>>::value));
  // Case 0.
  // NOTE: this cannot fail in any way as we require coefficients to be addable
  // in is_cf.
  typedef g_series_type<rational, int> p_type1;
  BOOST_CHECK((std::is_same<p_type1, binary_series_op_return_type<
                                         p_type1, p_type1>::type>::value));
  // Case 1 and 2.
  typedef g_series_type<double, int> p_type2;
  BOOST_CHECK((std::is_same<p_type2, binary_series_op_return_type<
                                         p_type2, p_type1>::type>::value));
  BOOST_CHECK((std::is_same<p_type2, binary_series_op_return_type<
                                         p_type1, p_type2>::type>::value));
  // mock_cf supports only multiplication vs mock_cf.
  BOOST_CHECK(
      (!has_typedef_type<binary_series_op_return_type<
           g_series_type<double, int>, g_series_type<mock_cf, int>>>::value));
  BOOST_CHECK(
      (!has_typedef_type<binary_series_op_return_type<
           g_series_type<mock_cf, int>, g_series_type<double, int>>>::value));
  // Case 3.
  typedef g_series_type<short, int> p_type3;
  BOOST_CHECK((std::is_same<
               g_series_type<int, int>,
               binary_series_op_return_type<p_type3, p_type3>::type>::value));
  typedef g_series_type<char, int> p_type4;
  BOOST_CHECK((std::is_same<
               g_series_type<int, int>,
               binary_series_op_return_type<p_type3, p_type4>::type>::value));
  BOOST_CHECK((std::is_same<
               g_series_type<int, int>,
               binary_series_op_return_type<p_type4, p_type3>::type>::value));
  // Wrong rebind implementations.
  BOOST_CHECK((!has_typedef_type<binary_series_op_return_type<
                   g_series_type_nr2<int>, g_series_type<char, int>>>::value));
  BOOST_CHECK((!has_typedef_type<binary_series_op_return_type<
                   g_series_type<char, int>, g_series_type_nr2<int>>>::value));
  // Case 4 and 6.
  BOOST_CHECK(
      (std::is_same<p_type2,
                    binary_series_op_return_type<p_type2, int>::type>::value));
  BOOST_CHECK(
      (std::is_same<p_type2,
                    binary_series_op_return_type<int, p_type2>::type>::value));
  // mock_cf does not support multiplication with int.
  BOOST_CHECK((!has_typedef_type<binary_series_op_return_type<
                   g_series_type<mock_cf, int>, int>>::value));
  BOOST_CHECK((!has_typedef_type<binary_series_op_return_type<
                   int, g_series_type<mock_cf, int>>>::value));
  // Case 5 and 7.
  BOOST_CHECK((std::is_same<p_type2, binary_series_op_return_type<
                                         p_type3, double>::type>::value));
  BOOST_CHECK((std::is_same<p_type2, binary_series_op_return_type<
                                         double, p_type3>::type>::value));
  BOOST_CHECK((
      std::is_same<g_series_type<int, int>,
                   binary_series_op_return_type<p_type4, short>::type>::value));
  BOOST_CHECK((
      std::is_same<g_series_type<int, int>,
                   binary_series_op_return_type<short, p_type4>::type>::value));
  // These need rebinding, but rebind is not supported.
  BOOST_CHECK(
      (!has_typedef_type<
          binary_series_op_return_type<g_series_type_nr<int>, double>>::value));
  BOOST_CHECK(
      (!has_typedef_type<
          binary_series_op_return_type<double, g_series_type_nr<int>>>::value));
  // Wrong implementation of rebind.
  BOOST_CHECK(
      (!has_typedef_type<binary_series_op_return_type<
           g_series_type_nr2<char>, g_series_type<char, char>>>::value));
  BOOST_CHECK(
      (!has_typedef_type<binary_series_op_return_type<
           g_series_type<char, char>, g_series_type_nr2<char>>>::value));
  // Same coefficients, amibguity in series type.
  BOOST_CHECK((!has_typedef_type<binary_series_op_return_type<
                   g_series_type_nr<int>, g_series_type<float, int>>>::value));
}

struct arithmetics_add_tag {};

namespace piranha {
template <> class debug_access<arithmetics_add_tag> {
public:
  template <typename Cf> struct runner {
    template <typename Expo> void operator()(const Expo &) {
      ...
      BOOST_CHECK((std::is_same<
                   decltype(tmp2),
                   g_series_type<g_series_type<double, Expo>, Expo>>::value));
      auto it2 = tmp2.m_container.begin();
      BOOST_CHECK(
          (it2->m_cf == g_series_type<double, Expo>{"y"} || it2->m_cf == 1));
      ...
      tmp += std::move(tmp);
      BOOST_CHECK_EQUAL(tmp.size(), 1u);
      BOOST_CHECK(tmp.m_container.begin()->m_cf ==
                  Cf(1) + Cf(1) + Cf(1) + Cf(1));
      BOOST_CHECK(tmp.m_container.begin()->m_key.size() == 1u);
      BOOST_CHECK(tmp.m_symbol_set == symbol_set{symbol{"x"}});
      // Now with merging.
      ...
      BOOST_CHECK(
          (std::is_same<decltype(it4->m_cf), g_series_type<int, Expo>>::value));
      BOOST_CHECK(
          (it4->m_cf == g_series_type<int, Expo>{"y"} || it4->m_cf == 1));
  ...
  template <typename Cf> void operator()(const Cf &) {
...
BOOST_AUTO_TEST_CASE(series_arithmetics_add_test) {
  ...
  BOOST_CHECK(
      (std::is_same<p_type11, decltype(p_type1{} + p_type11{})>::value));
  BOOST_CHECK(
      (std::is_same<p_type11, decltype(p_type11{} + p_type1{})>::value));
  BOOST_CHECK(
      (std::is_same<p_type21, decltype(p_type1{} + p_type22{})>::value));
  BOOST_CHECK(
      (std::is_same<p_type21, decltype(p_type22{} + p_type1{})>::value));
  BOOST_CHECK(
      (std::is_same<p_type11, decltype(p_type11{} + p_type22{})>::value));
  BOOST_CHECK(
      (std::is_same<p_type11, decltype(p_type22{} + p_type11{})>::value));
  // Type testing for in-place addition.
  // Case 0.
  BOOST_CHECK((std::is_same<p_type1 &, decltype(std::declval<p_type1 &>() +=
                                                p_type1{})>::value));
  // Case 1.
  BOOST_CHECK((std::is_same<p_type1 &, decltype(std::declval<p_type1 &>() +=
                                                p_type2{})>::value));
  // Case 2.
  BOOST_CHECK((std::is_same<p_type2 &, decltype(std::declval<p_type2 &>() +=
                                                p_type1{})>::value));
  // Case 3, symmetric.
  BOOST_CHECK((std::is_same<p_type3 &, decltype(std::declval<p_type3 &>() +=
                                                p_type4{})>::value));
  BOOST_CHECK((std::is_same<p_type4 &, decltype(std::declval<p_type4 &>() +=
                                                p_type3{})>::value));
  // Case 4.
  BOOST_CHECK((std::is_same<p_type1 &,
                            decltype(std::declval<p_type1 &>() += 0)>::value));
  // Case 5.
  BOOST_CHECK((std::is_same<p_type3 &,
                            decltype(std::declval<p_type3 &>() += 0)>::value));
...
namespace piranha {
template <> class debug_access<arithmetics_sub_tag> {
public:
  template <typename Cf> struct runner {
    template <typename Expo> void operator()(const Expo &) {
      typedef g_series_type<Cf, Expo> p_type1;
      ...
      typedef g_series_type<int, Expo> p_type3;
      // Binary sub first.
      // Some type checks - these are not subtractable as they result in an
      // ambiguity
      // between two series with same coefficient but different series types.
...
namespace piranha {
template <> class debug_access<arithmetics_mul_tag> {
public:
  template <typename Cf> struct runner {
    template <typename Expo> void operator()(const Expo &) {
      typedef g_series_type<Cf, Expo> p_type1;
      ...
      typedef g_series_type<int, Expo> p_type3;
      // Binary mul first.
      // Some type checks - these are not multipliable as they result in an
      // ambiguity
      // between two series with same coefficient but different series types.
...
namespace piranha {
template <> class debug_access<arithmetics_div_tag> {
public:
  template <typename Cf> struct runner {
    template <typename Expo> void operator()(const Expo &) {
      typedef g_series_type<Cf, Expo> p_type1;
...
BOOST_AUTO_TEST_CASE(series_arithmetics_div_test) {
  ...
  BOOST_CHECK((std::is_same<decltype(3. / p_type1{}),
...
namespace piranha {
template <> class debug_access<eq_tag> {
public:
  template <typename Cf> struct runner {
    template <typename Expo> void operator()(const Expo &) {
      typedef g_series_type<Cf, Expo> p_type1;
      typedef g_series_type2<Cf, Expo> p_type2;
      typedef g_series_type<int, Expo> p_type3;
      // Some type checks - these are not comparable as they result in an
      // ambiguity
      // between two series with same coefficient but different series types.
...
BOOST_AUTO_TEST_CASE(series_hash_test) {
  ...
  // NOTE: this is not 100% sure as the hash mixing in the monomial could
  // actually lead to identical hashes.
...
// A non-multipliable series, missing a suitable series_multiplier
// specialisation.
template <typename Cf, typename Expo>
class g_series_type_nm
    : public series<Cf, monomial<Expo>, g_series_type_nm<Cf, Expo>> {
  typedef series<Cf, monomial<Expo>, g_series_type_nm<Cf, Expo>> base;
  PIRANHA_SERIALIZE_THROUGH_BASE(base)
public:
  template <typename Cf2> using rebind = g_series_type_nm<Cf2, Expo>;
...
// Mock coefficient, with weird semantics for operator+(integer): the output is
// not a coefficient type.
struct mock_cf2 {
  mock_cf2();
...
// Check that attempting to rebind to an invalid coefficient disables the
// operator, rather
...
BOOST_AUTO_TEST_CASE(series_rebind_failure_test) {
  ...
  BOOST_CHECK((!is_addable<g_series_type<integer, int>,
                           g_series_type<mock_cf2, int>>::value));
  BOOST_CHECK((!is_addable<g_series_type<mock_cf2, int>,
                           g_series_type<integer, int>>::value));
  BOOST_CHECK((is_addable<g_series_type<mock_cf2, int>,
                          g_series_type<mock_cf2, int>>::value));
  BOOST_CHECK((!is_subtractable<g_series_type<integer, int>,
                                g_series_type<mock_cf2, int>>::value));
  BOOST_CHECK((!is_subtractable<g_series_type<mock_cf2, int>,
                                g_series_type<integer, int>>::value));
  BOOST_CHECK((is_subtractable<g_series_type<mock_cf2, int>,
                               g_series_type<mock_cf2, int>>::value));
  BOOST_CHECK((!is_multipliable<g_series_type<integer, int>,
                                g_series_type<mock_cf2, int>>::value));
  BOOST_CHECK((!is_multipliable<g_series_type<mock_cf2, int>,
                                g_series_type<integer, int>>::value));
  BOOST_CHECK((is_multipliable<g_series_type<mock_cf2, int>,
                               g_series_type<mock_cf2, int>>::value));
}

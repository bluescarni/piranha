...
public:
  ...
  // NOTE: strange thing here, move constructor of std::string results in
  // undefined reference?
  ...
  template <typename T> struct runner {
    template <typename U> void operator()(const U &) {
      typedef static_vector<T, U::value> vector_type;
      {
        ...
        // Move constructor.
        BOOST_CHECK_EQUAL(vector_type(std::move(v))[0],
      ...
      BOOST_CHECK_THROW(
          v = vector_type(1u + U::value, boost::lexical_cast<T>(2)),
          std::bad_alloc);
    }
  };
...
BOOST_AUTO_TEST_CASE(static_vector_size_type_test) {
  BOOST_CHECK((std::is_same<detail::static_vector_size_type<10u>::type,
                            unsigned char>::value));
  BOOST_CHECK((std::is_same<detail::static_vector_size_type<255u>::type,
                            unsigned char>::value));
  BOOST_CHECK((std::is_same<detail::static_vector_size_type<10000u>::type,
                            unsigned char>::value) ||
              (std::is_same<detail::static_vector_size_type<10000u>::type,
                            unsigned short>::value));
  BOOST_CHECK((std::is_same<detail::static_vector_size_type<4294967295ul>::type,
                            unsigned char>::value) ||
              (std::is_same<detail::static_vector_size_type<4294967295ul>::type,
                            unsigned short>::value) ||
              (std::is_same<detail::static_vector_size_type<4294967295ul>::type,
                            unsigned>::value) ||
              (std::is_same<detail::static_vector_size_type<4294967295ul>::type,
...
struct emplace_back_tester {
  template <typename T> struct runner {
    template <typename U> void operator()(const U &) {
      ...
      BOOST_CHECK_THROW(v.emplace_back(boost::lexical_cast<T>(1)),
                        std::bad_alloc);
    }
  };
  template <typename T> void operator()(const T &) {

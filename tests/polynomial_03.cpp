...
struct split_tester {
  template <typename Cf> struct runner {
    template <typename Key> void operator()(const Key &) {
      ...
      BOOST_CHECK((2 * x + 3 * y).split().get_symbol_set() ==
                  symbol_set{symbol{"x"}});
      BOOST_CHECK((2 * x + 3 * y).split().join() == 2 * x + 3 * y);
      BOOST_CHECK((
          (2 * x + 3 * y).split()._container().begin()->m_cf.get_symbol_set() ==
          symbol_set{symbol{"y"}}));
      BOOST_CHECK_EQUAL((2 * x * z + 3 * x * x * y - 6 * x * y * z).split(),
                        xx * (2 * z - 6 * y * z) + 3 * xx * xx * y);
      BOOST_CHECK_EQUAL(
          (2 * x * z + 3 * x * x * y - 6 * x * y * z).split().join(),
          2 * x * z + 3 * x * x * y - 6 * x * y * z);
      BOOST_CHECK((2 * x * z + 3 * x * x * y - 6 * x * y * z)
                      .split()
                      .get_symbol_set() == symbol_set{symbol{"x"}});
      BOOST_CHECK(((2 * x * z + 3 * x * x * y - 6 * x * y * z)
                       .split()
                       ._container()
                       .begin()
...
struct udivrem_tester {
  template <typename Key> void operator()(const Key &) {
    ...
    res =
        p_type::udivrem(pow(x, 8) + pow(x, 6) - 3 * pow(x, 4) - 3 * pow(x, 3) +
                            8 * pow(x, 2) + 2 * x - 5,
    ...
    BOOST_CHECK_THROW(p_type::udivrem(x.pow(-1), x.pow(-1)),
    ...
    for (auto i = 0; i < ntrials; ++i) {
      // Little hack to make sure that the symbol set contains always x. It
      // could be
      ...
      for (int j = 0; j < nterms; ++j) {
        num +=
            ((ud(rng) < 5 ? 1 : -1) * ud(rng) * x.pow(ud(rng))) / (ud(rng) + 1);
        den +=
            ((ud(rng) < 5 ? 1 : -1) * ud(rng) * x.pow(ud(rng))) / (ud(rng) + 1);
      }
      if (den.size() == 0u) {
    ...
    BOOST_CHECK_THROW(pp_type::udivrem(x.pow(-1) * xx, xx),
                      std::invalid_argument);
...
struct gcd_prs_sr_tester {
  template <typename Key> void operator()(const Key &) {
    ...
    // Set the default algorithm to PRS, so it is used at all levels of the
    // recursion.
    ...
    BOOST_CHECK_EQUAL(1 + x,
                      gcd_prs_sr(pow(x, 2) + 7 * x + 6, pow(x, 2) - 5 * x - 6));
    BOOST_CHECK_EQUAL(
        gcd_prs_sr(pow(x, 8) + pow(x, 6) - 3 * pow(x, 4) - 3 * pow(x, 3) +
                       8 * pow(x, 2) + 2 * x - 5,
                   3 * pow(x, 6) + 5 * pow(x, 4) - 4 * pow(x, 2) - 9 * x + 21),
        1);
    BOOST_CHECK_EQUAL(gcd_prs_sr(pow(x, 4) - 9 * pow(x, 2) - 4 * x + 12,
                                 pow(x, 3) + 5 * pow(x, 2) + 2 * x - 8),
                      -2 + x * x + x);
    BOOST_CHECK_EQUAL(1, gcd_prs_sr(pow(x, 4) + pow(x, 2) + 1, pow(x, 2) + 1));
    BOOST_CHECK_EQUAL(gcd_prs_sr(x * x + 1, pow(x, 5) + pow(x, 4) + x + 1), 1);
    BOOST_CHECK_EQUAL(gcd_prs_sr(pow(x, 6) + pow(x, 5) + pow(x, 3) + x,
                                 pow(x, 4) + pow(x, 2) + 1),
                      1);
    // With zeroes.
    ...
    for (auto i = 0; i < ntrials; ++i) {
      ...
      if (a.size() != 0u || b.size() != 0u) {
        BOOST_CHECK_EQUAL(
            pq_type::udivrem(pq_type(a), pq_type(tmp)).second.size(), 0u);
        BOOST_CHECK_EQUAL(
            pq_type::udivrem(pq_type(b), pq_type(tmp)).second.size(), 0u);
      }
    ...
    for (auto i = 0; i < ntrials; ++i) {
      ...
      if ((a * b).size() != 0u || b.size() != 0u) {
        BOOST_CHECK_EQUAL(
            pq_type::udivrem(pq_type(a * b), pq_type(tmp)).second.size(), 0u);
        BOOST_CHECK_EQUAL(
            pq_type::udivrem(pq_type(b), pq_type(tmp)).second.size(), 0u);
      }
      ...
      if ((a * b * b).size() != 0u || (b * a).size() != 0u) {
        BOOST_CHECK_EQUAL(
            pq_type::udivrem(pq_type(a * b * b), pq_type(tmp)).second.size(),
            0u);
        BOOST_CHECK_EQUAL(
            pq_type::udivrem(pq_type(b * a), pq_type(tmp)).second.size(), 0u);
      }
      BOOST_CHECK(tmp == gcd_prs_sr(a * b, a * b * b) ||
                  tmp == -gcd_prs_sr(a * b, a * b * b));
    }
    // Restore.
...
struct establish_limits_tester {
  template <typename Key> void operator()(const Key &) {
    ...
    BOOST_CHECK_EQUAL(lims[1].first, 0);
    BOOST_CHECK_EQUAL(lims[1].second, 1);
    BOOST_CHECK_THROW(poly_establish_limits(x + y.pow(-1), y + x.pow(4)),
                      std::invalid_argument);
...
struct mapping_tester {
  template <typename Key> void operator()(const Key &) {
    ...
    BOOST_CHECK_EQUAL(std::get<0u>(res)._container().begin()->m_key.get_int(),
                      1);
    ...
    BOOST_CHECK_EQUAL(std::get<1u>(res)._container().begin()->m_key.get_int(),
                      2);
    BOOST_CHECK_EQUAL(std::get<2u>(res).size(), 2u);
    BOOST_CHECK_EQUAL(std::get<2u>(res)[0u], 1);
    BOOST_CHECK_EQUAL(std::get<2u>(res)[1u], 3);
    // Decodification.
    auto dres = poly_from_univariate<Key>(std::get<0u>(res), std::get<2u>(res),
                                          symbol_set({symbol{"x"}}));
    ...
    overflow_check(x, y);
    dres = poly_from_univariate<Key>(std::get<0u>(res), std::get<2u>(res),
    ...
    for (int i = 0; i < ntrials; ++i) {
      ...
      if (n.get_symbol_set().size() != 3u ||
          n.get_symbol_set() != d.get_symbol_set() || n.size() == 0u ||
          d.size() == 0u) {
        continue;
      }
      res = poly_to_univariate(n, d);
      BOOST_CHECK_EQUAL(std::get<0u>(res).size(), n.size());
      BOOST_CHECK(std::get<0u>(res).get_symbol_set() ==
                  symbol_set({symbol{"x"}}));
      BOOST_CHECK_EQUAL(std::get<1u>(res).size(), d.size());
      BOOST_CHECK(std::get<1u>(res).get_symbol_set() ==
                  symbol_set({symbol{"x"}}));
      BOOST_CHECK_EQUAL(std::get<2u>(res).size(), 4u);
      dres =
          poly_from_univariate<Key>(std::get<0u>(res), std::get<2u>(res), ss);
      BOOST_CHECK_EQUAL(dres, n);
    }
  }
  template <typename T,
            typename std::enable_if<
                detail::is_mp_integer<
                    typename T::term_type::key_type::value_type>::value,
                int>::type = 0>
  void overflow_check(const T &x, const T &y) const {
    BOOST_CHECK_THROW(
        poly_to_univariate(
            x.pow(std::numeric_limits<k_monomial::value_type>::max()), x),
        std::overflow_error);
    BOOST_CHECK_THROW(
        poly_to_univariate(
            x.pow(std::numeric_limits<k_monomial::value_type>::max() / 2) +
...
struct univariate_gcdheu_tester {
  template <typename Key> void operator()(const Key &) {
    ...
    using p_type = polynomial<integer, Key>;
    p_type x{"x"};
    // The idea here is that instead of calling the detail:: implementation we
    // go through
    // the gcd() static function in poly and force the algorithm. This is in
    // order to test
    // the exception throwing paths in the static method.
    auto heu_wrapper = [](const p_type &a,
                          const p_type &b) -> std::pair<bool, p_type> {
      try {
        auto res_heu =
            p_type::gcd(a, b, true, polynomial_gcd_algorithm::heuristic);
    ...
    auto gcd_check = [heu_wrapper](const p_type &a, const p_type &b,
                                   const p_type &g) {
    ...
    gcd_check(pow(x, 8) + pow(x, 6) - 3 * pow(x, 4) - 3 * pow(x, 3) +
                  8 * pow(x, 2) + 2 * x - 5,
              3 * pow(x, 6) + 5 * pow(x, 4) - 4 * pow(x, 2) - 9 * x + 21,
              p_type{1});
    gcd_check(pow(x, 4) - 9 * pow(x, 2) - 4 * x + 12,
              pow(x, 3) + 5 * pow(x, 2) + 2 * x - 8, -2 + x * x + x);
    gcd_check(pow(x, 4) + pow(x, 2) + 1, pow(x, 2) + 1, p_type{1});
    gcd_check(x * x + 1, pow(x, 5) + pow(x, 4) + x + 1, p_type{1});
    gcd_check(pow(x, 6) + pow(x, 5) + pow(x, 3) + x, pow(x, 4) + pow(x, 2) + 1,
              p_type{1});
    // With zeroes.
    ...
    for (auto i = 0; i < ntrials; ++i) {
      ...
      BOOST_CHECK(res_heu.second == res_heu2.second ||
                  res_heu.second == -res_heu2.second);
    ...
    for (auto i = 0; i < ntrials; ++i) {
      ...
      if ((a * b).size() != 0u || b.size() != 0u) {
        BOOST_CHECK_EQUAL(p_type::udivrem(a * b, res_heu.second).second.size(),
                          0u);
        BOOST_CHECK_EQUAL(p_type::udivrem(b, res_heu.second).second.size(), 0u);
      }
      ...
      if ((a * b * b).size() != 0u || (b * a).size() != 0u) {
        BOOST_CHECK_EQUAL(
            p_type::udivrem(a * b * b, res_heu.second).second.size(), 0u);
        BOOST_CHECK_EQUAL(p_type::udivrem(b * a, res_heu.second).second.size(),
                          0u);
      }

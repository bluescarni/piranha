...
BOOST_AUTO_TEST_CASE(trigonometric_series_degree_order_test) {
  ...
  BOOST_CHECK_EQUAL(
      math::t_degree(y * cos(x - y) + cos(x + y), {"y", "x", "y", "z"}), 2);
  BOOST_CHECK_EQUAL(math::t_degree(y * cos(x - y) + cos(x + y), {"x"}), 1);
  BOOST_CHECK_EQUAL(
      math::t_degree(y * sin(x - y) + cos(x + y), std::vector<std::string>{}),
      0);
  ...
  BOOST_CHECK_EQUAL(math::t_ldegree((x * cos(2 * x) - y * cos(x + y)), {"y"}),
                    0);
  ...
  BOOST_CHECK_EQUAL(
      math::t_ldegree((y * cos(x - y) + cos(x + y)), {"y", "x", "y", "z"}), 0);
  BOOST_CHECK_EQUAL(math::t_ldegree((y * cos(x - y) + cos(x + y)), {"x"}), 1);
  BOOST_CHECK_EQUAL(math::t_ldegree((y * sin(x - y) + cos(x + y)),
  ...
  BOOST_CHECK_EQUAL(
      math::t_order((y * cos(x - y) + cos(x + y)), {"y", "x", "y", "z"}), 2);
  BOOST_CHECK_EQUAL(math::t_order((y * cos(x - y) + cos(x + y)), {"x"}), 1);
  BOOST_CHECK_EQUAL(
      math::t_order((y * sin(x - y) + cos(x + y)), std::vector<std::string>{}),
      0);
  ...
  BOOST_CHECK_EQUAL(math::t_lorder((x * cos(2 * x) - y * cos(x + y)), {"y"}),
                    0);
  ...
  BOOST_CHECK_EQUAL(
      math::t_lorder((y * cos(x - y) + cos(x + y)), {"y", "x", "y", "z"}), 2);
  BOOST_CHECK_EQUAL(math::t_lorder((y * cos(x - y) + cos(x + y)), {"x"}), 1);
  BOOST_CHECK_EQUAL(
      math::t_lorder((y * sin(x - y) + cos(x + y)), std::vector<std::string>{}),
      0);
  ...
  BOOST_CHECK_EQUAL(
      math::t_degree(p_type2{math::cos(p_type1{"x"} - p_type1{"y"})}), 0);
  BOOST_CHECK_EQUAL(
      math::t_ldegree(p_type2{1 + math::cos(p_type1{"x"} + p_type1{"y"})}), 0);
  BOOST_CHECK_EQUAL(
      math::t_order(p_type2{math::cos(p_type1{"x"} - p_type1{"y"})}), 2);
  BOOST_CHECK_EQUAL(
      math::t_lorder(p_type2{math::cos(p_type1{"x"} - p_type1{"y"}) +
                             math::cos(p_type1{"x"} + p_type1{"y"})}),
      2);
  // Type traits checks.
  using t_deg_type = decltype(std::make_signed<std::size_t>::type(0) +
                              std::make_signed<std::size_t>::type(0));
  BOOST_CHECK(
      (std::is_same<t_deg_type, decltype(math::t_degree(p_type1{}))>::value));
  BOOST_CHECK(
      (std::is_same<t_deg_type,
                    decltype(math::t_degree(
                        p_type1{}, std::vector<std::string>{}))>::value));
  BOOST_CHECK(
      (std::is_same<t_deg_type, decltype(math::t_ldegree(p_type1{}))>::value));
  BOOST_CHECK(
      (std::is_same<t_deg_type,
                    decltype(math::t_ldegree(
                        p_type1{}, std::vector<std::string>{}))>::value));
  BOOST_CHECK(
      (std::is_same<t_deg_type, decltype(math::t_order(p_type1{}))>::value));
  BOOST_CHECK(
      (std::is_same<t_deg_type,
                    decltype(math::t_order(
                        p_type1{}, std::vector<std::string>{}))>::value));
  BOOST_CHECK(
      (std::is_same<t_deg_type, decltype(math::t_lorder(p_type1{}))>::value));
  BOOST_CHECK(
      (std::is_same<t_deg_type,
                    decltype(math::t_lorder(
                        p_type1{}, std::vector<std::string>{}))>::value));
}
...
struct key02 {
  ...
  template <typename... Args> int t_degree(const Args &...) const;
  template <typename... Args> int t_ldegree(const Args &...) const;
  template <typename... Args> int t_order(const Args &...) const;
  template <typename... Args> int t_lorder(const Args &...) const;
...
struct key03 : key02 {
  ...
  bool operator!=(const key03 &) const;
  template <typename... Args> int t_lorder(const Args &...);
...
struct key04 : key02 {
  ...
  bool operator!=(const key04 &) const;
  template <typename... Args> fake_int_01 t_lorder(const Args &...) const;
...
struct key05 : key02 {
  ...
  bool operator!=(const key05 &) const;
  template <typename... Args> fake_int_02 t_lorder(const Args &...) const;
...
template <> struct hash<key03> { std::size_t operator()(const key03 &) const; };
...
class g_series_type
    : public trigonometric_series<series<Cf, Key, g_series_type<Cf, Key>>> {

...
BOOST_AUTO_TEST_CASE(symbol_constructor_test) {
  ...
  BOOST_CHECK_EQUAL(std::addressof(y2.get_name()),
                    std::addressof(y.get_name()));
  symbol x2("x");
  BOOST_CHECK_EQUAL(x2.get_name(), "x");
  BOOST_CHECK_EQUAL(std::addressof(x2.get_name()),
                    std::addressof(x.get_name()));
  symbol x3("x");
  BOOST_CHECK_EQUAL(x3.get_name(), "x");
  BOOST_CHECK_EQUAL(std::addressof(x3.get_name()),
                    std::addressof(x.get_name()));
  // Copy ctor.
  symbol x4(x3);
  BOOST_CHECK_EQUAL(x4.get_name(), "x");
  BOOST_CHECK_EQUAL(std::addressof(x4.get_name()),
                    std::addressof(x.get_name()));
  // Move ctor - equivalent to copy.
  symbol x5(std::move(x4));
  BOOST_CHECK_EQUAL(x4.get_name(), "x");
  BOOST_CHECK_EQUAL(x5.get_name(), "x");
  BOOST_CHECK_EQUAL(std::addressof(x5.get_name()),
                    std::addressof(x.get_name()));
  // Copy assignment.
  x5 = y;
...
BOOST_AUTO_TEST_CASE(symbol_hash_test) {
  ...
  BOOST_CHECK_EQUAL(
      symbol("x").hash(),
      std::hash<std::string const *>()(std::addressof(symbol("x").get_name())));
...
BOOST_AUTO_TEST_CASE(symbol_streaming_test) {
  BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(symbol("x")),
                    "name = \'x\'");
}

...
struct mock_cf {
  ...
  mock_cf &operator=(mock_cf &&) noexcept { return *this; }
  friend std::ostream &operator<<(std::ostream &, const mock_cf &);
  mock_cf operator-() const;
  bool operator==(const mock_cf &) const { return true; }
  bool operator!=(const mock_cf &) const { return false; }
...
// Copy ctor of base called explicitly: if the generic ctor is invoked, this
// should give a linker error
// as it will need the +/- operators from mock_cf which are declared but not
// defined.
template <typename Cf, typename Expo>
...
template <typename T> class null_toolbox : public T {
...
template <typename T> class null_toolbox2 : public T {
  PIRANHA_SERIALIZE_THROUGH_BASE(T)
public:
...
class g_series_type3
    : public null_toolbox<
          series<Cf, monomial<Expo>, g_series_type3<Cf, Expo>>> {
  using base =
      null_toolbox<series<Cf, monomial<Expo>, g_series_type3<Cf, Expo>>>;
  PIRANHA_SERIALIZE_THROUGH_BASE(base)
public:
...
class g_series_type4
    : public null_toolbox2<
          series<Cf, monomial<Expo>, g_series_type4<Cf, Expo>>> {
  using base =
      null_toolbox2<series<Cf, monomial<Expo>, g_series_type4<Cf, Expo>>>;
  PIRANHA_SERIALIZE_THROUGH_BASE(base)
public:
...
template <typename Cf, typename Expo>
class g_series_type5
    : public series<Cf, monomial<Expo>, g_series_type5<Cf, Expo>> {
...
public:
  ...
  int invert() const { return 42; }
};

// Series type with invalid invert() override.
template <typename Cf, typename Expo>
class g_series_type6
    : public series<Cf, monomial<Expo>, g_series_type6<Cf, Expo>> {
...
BOOST_AUTO_TEST_CASE(series_extend_symbol_set_test) {
  ...
  BOOST_CHECK_THROW(x.extend_symbol_set(symbol_set{symbol{"y"}}),
                    std::invalid_argument);
  BOOST_CHECK_THROW(x.extend_symbol_set(symbol_set{symbol{"y"}, symbol{"z"}}),
                    std::invalid_argument);
  try {
    x.extend_symbol_set(symbol_set{symbol{"y"}});
  } catch (const std::invalid_argument &ia) {
    BOOST_CHECK(std::string(ia.what()).find(
                    "invalid symbol set passed to extend_symbol_set(): the new "
                    "symbol set does not include all the symbols in the old "
                    "symbol set") != std::string::npos);
  }
  BOOST_CHECK((x.extend_symbol_set(symbol_set{symbol{"y"}, symbol{"x"}})
                   .get_symbol_set() == symbol_set{symbol{"y"}, symbol{"x"}}));
  BOOST_CHECK(
      (x.extend_symbol_set(symbol_set{symbol{"y"}, symbol{"x"}, symbol{"z"}})
           .get_symbol_set() ==
  ...
  auto bar = x + y;
  foo =
      bar.extend_symbol_set(symbol_set{symbol{"y"}, symbol{"x"}, symbol{"z"}});
  ...
  st0 null;
  foo =
      null.extend_symbol_set(symbol_set{symbol{"y"}, symbol{"x"}, symbol{"z"}});
  BOOST_CHECK(foo.size() == 0u);
  BOOST_CHECK((symbol_set{symbol{"y"}, symbol{"x"}, symbol{"z"}} ==
               foo.get_symbol_set()));
}

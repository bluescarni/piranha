...
using integral_types =
    boost::mpl::vector<char, signed char, short, int, long, long long,
                       ...
                       unsigned long long
// So the story here is a bit complicated. On Clang, casting a large char16_t
// value
// to unsigned char via numeric_cast does not throw (as it should), whereas on
// GCC it does
// (this is required in the direct constructor of mp_integer from C++ integral).
// This might or might not be related to some missing type-traiting in libstdc++
// (and possibly libc++)
// that was added only around GCC 4.9.1:
// https://gcc.gnu.org/ml/gcc-patches/2014-05/msg01184.html
// Before this patch, e.g., wchar_t is signed but its make_unsigned counterpart
// returns again
// wchar_t :/ Discovered this after investigating re-implementing the
// numeric_cast functionality
// using pure C++11. It seems like instead boost::make_unsigned has the
// functionality implemented
// via a heuristic approach, so that might be the reason why numeric_cast still
// works (at least on GCC).
// In any case, for the time being let's just disable the tests on Clang - it's
// a corner case that hopefully
...
struct constructor_tester {
  template <typename T> void operator()(const T &) {
    ...
    BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(4),
                      boost::lexical_cast<std::string>(int_type(4)));
    BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(-4),
                      boost::lexical_cast<std::string>(int_type(-4)));
    std::uniform_int_distribution<short> short_dist(
        boost::integer_traits<short>::const_min,
        boost::integer_traits<short>::const_max);
    for (int i = 0; i < ntries; ++i) {
      const auto tmp = short_dist(rng);
      try {
        BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(tmp),
                          boost::lexical_cast<std::string>(int_type(tmp)));
...
static inline std::string mpz_lexcast(const mpz_raii &m) {
  ...
  if (unlikely(size_base10 > boost::integer_traits<std::size_t>::const_max -
...
struct set_bit_tester {
  template <typename T> void operator()(const T &) {
    ...
    int_type n1;
    BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(n1),
    ...
    n1.set_bit(1);
    BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(n1),
                      boost::lexical_cast<std::string>(-3));
    n1.negate();
    BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(n1),
                      boost::lexical_cast<std::string>(3));
    mpz_raii m2;
...
struct static_mpz_view_tester {
  template <typename T> void operator()(const T &) {
    ...
    for (int i = 0; i < ntries; ++i) {
      ...
      // There must always be something allocated, and the size must be less
      // than or equal
    ...
    BOOST_CHECK(::mpz_cmp(v, &m.m_mpz) == 0);
    auto v_ptr = static_cast<mpz_struct_t const *>(v);
    BOOST_CHECK(v_ptr->_mp_alloc > 0 && (v_ptr->_mp_alloc >= v_ptr->_mp_size ||
                                         v_ptr->_mp_alloc >= -v_ptr->_mp_size));
    // TT checks.
    BOOST_CHECK(!std::is_copy_constructible<decltype(v)>::value);
    BOOST_CHECK(std::is_move_constructible<decltype(v)>::value);
    BOOST_CHECK(!std::is_copy_assignable<
                typename std::add_lvalue_reference<decltype(v)>::type>::value);
    BOOST_CHECK(!std::is_move_assignable<
                typename std::add_lvalue_reference<decltype(v)>::type>::value);
  }
  ...
  static void check_zero_limbs(const T &, V v_ptr) {
    const std::size_t size = static_cast<std::size_t>(
        v_ptr->_mp_size >= 0 ? v_ptr->_mp_size : -v_ptr->_mp_size);
  ...
  template <typename T> void operator()(const T &) {
    ...
    for (int i = 0; i < ntries; ++i) {
      ...
      try {
        ...
        ::mpz_set_str(&ma.m_mpz, boost::lexical_cast<std::string>(tmp1).c_str(),
                      10);
        ::mpz_set_str(&mb.m_mpz, boost::lexical_cast<std::string>(tmp2).c_str(),
                      10);
...
struct static_addmul_tester {
  template <typename T> void operator()(const T &) {
    ...
    for (int i = 0; i < ntries; ++i) {
      ...
      for (typename int_type::limb_t j = limb_bits / 2u; j < 2u * limb_bits;
           ++j) {
        if (int_dist(rng)) {
          a.set_bit(j);
        }
...
struct float_ctor_tester {
  template <typename T> void operator()(const T &) {
    ...
    mpz_raii m;
    int_type n1;
    BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(0),
                      boost::lexical_cast<std::string>(n1));
    n1.promote();
    BOOST_CHECK_THROW(n1.promote(), std::invalid_argument);
    BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(0),
                      boost::lexical_cast<std::string>(n1));
    std::uniform_int_distribution<int> sign_dist(0, 1);
    // NOTE: the idea here is that we are going to multiply by radix**n, and we
    // need to make sure both long long and the FP
    // type can represent the result.
    std::uniform_int_distribution<int> exp_dist_d(
        0, std::min(get_max_exp(std::numeric_limits<double>::radix),
    ...
    for (int i = 0; i < ntries; ++i) {
      ...
      BOOST_CHECK_EQUAL(
          boost::lexical_cast<std::string>(int_type(tmp)),
          boost::lexical_cast<std::string>(static_cast<long long>(tmp)));
      BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(int_type(tmp)),
                        mpz_lexcast(m));
    }
    BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(int_type(0.)),
                      boost::lexical_cast<std::string>(0));
    BOOST_CHECK(int_type{0.}.is_static());
    if (std::numeric_limits<double>::has_infinity &&
        std::numeric_limits<double>::has_quiet_NaN) {
      BOOST_CHECK_THROW(int_type{std::numeric_limits<double>::infinity()},
                        std::invalid_argument);
      BOOST_CHECK_THROW(int_type{std::numeric_limits<double>::quiet_NaN()},
                        std::invalid_argument);
    }
#if !defined(PIRANHA_RUN_ON_VALGRIND)
    // Long double.
    std::uniform_int_distribution<int> exp_dist_ld(
        0, std::min(get_max_exp(std::numeric_limits<long double>::radix),
    ...
    BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(int_type(0.l)),
                      boost::lexical_cast<std::string>(0));
    ...
    if (std::numeric_limits<long double>::has_infinity &&
      ...
      BOOST_CHECK_THROW(int_type{std::numeric_limits<long double>::infinity()},
                        std::invalid_argument);
      BOOST_CHECK_THROW(int_type{std::numeric_limits<long double>::quiet_NaN()},
                        std::invalid_argument);
    }
#endif
    // Float.
    std::uniform_int_distribution<int> exp_dist_f(
        0, std::min(get_max_exp(std::numeric_limits<float>::radix),
    ...
    BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(int_type(0.f)),
                      boost::lexical_cast<std::string>(0));
    BOOST_CHECK(int_type{0.f}.is_static());
    if (std::numeric_limits<float>::has_infinity &&
        std::numeric_limits<float>::has_quiet_NaN) {
      BOOST_CHECK_THROW(int_type{std::numeric_limits<float>::infinity()},
                        std::invalid_argument);
      BOOST_CHECK_THROW(int_type{std::numeric_limits<float>::quiet_NaN()},
                        std::invalid_argument);
    }
...
struct integral_ctor_tester {
  template <typename U> struct runner {
    template <typename T> void operator()(const T &) {
      ...
      std::uniform_int_distribution<T> int_dist(
      ...
      oss << static_cast<typename std::conditional<
          std::is_signed<T>::value, long long, unsigned long long>::type>(
          std::numeric_limits<T>::lowest());
      BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(
                            int_type(std::numeric_limits<T>::lowest())),
                        oss.str());
      oss.str("");
      oss << static_cast<typename std::conditional<
          std::is_signed<T>::value, long long, unsigned long long>::type>(
      ...
      BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(
                            int_type(std::numeric_limits<T>::max())),
                        oss.str());
    }
  };
  template <typename T> void operator()(const T &) {
...
struct str_ctor_tester {
  template <typename U> struct runner {
    template <typename T> void operator()(const T &) {
      ...
      for (int i = 0; i < ntries; ++i) {
        ...
        BOOST_CHECK_EQUAL(
            boost::lexical_cast<std::string>(int_type(oss.str().c_str())),
            oss.str());
        BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(int_type(oss.str())),
                          oss.str());
        oss.str("");
      }
      oss << static_cast<typename std::conditional<
  ...
  template <typename T> void operator()(const T &) {
    ...
    BOOST_CHECK_EQUAL("-123",
                      boost::lexical_cast<std::string>(int_type("-123")));
    const std::vector<std::string> invalid_strings{
        "-0",     "+0", "01", "+1", "+01", "-01", "123f",  " 123", "123 ",
        "123.56", "-",  "+",  "",   " +0", " -0", "-123 ", "12a",  "-12a"};
...
struct generic_assignment_tester {
  ...
  template <typename T> void operator()(const T &) {
    ...
    if (std::numeric_limits<float>::has_infinity &&
        std::numeric_limits<float>::has_quiet_NaN) {
      BOOST_CHECK_THROW((n = std::numeric_limits<float>::infinity()),
                        std::invalid_argument);
      BOOST_CHECK_THROW((n = std::numeric_limits<float>::quiet_NaN()),
                        std::invalid_argument);
    }
    n = -2.0;
    ...
    if (std::numeric_limits<double>::has_infinity &&
        std::numeric_limits<double>::has_quiet_NaN) {
      BOOST_CHECK_THROW((n = std::numeric_limits<double>::infinity()),
                        std::invalid_argument);
      BOOST_CHECK_THROW((n = std::numeric_limits<double>::quiet_NaN()),
                        std::invalid_argument);
    }
    n = 3.0L;
    ...
    if (std::numeric_limits<long double>::has_infinity &&
        std::numeric_limits<long double>::has_quiet_NaN) {
      BOOST_CHECK_THROW((n = std::numeric_limits<long double>::infinity()),
                        std::invalid_argument);
      BOOST_CHECK_THROW((n = std::numeric_limits<long double>::quiet_NaN()),
                        std::invalid_argument);
    }
#endif
  }
...
BOOST_AUTO_TEST_CASE(mp_integer_ctor_assign_test) {
  ...
  BOOST_CHECK(
      (!std::is_constructible<integer, std::vector<std::string>>::value));
  BOOST_CHECK((
      !std::is_constructible<mp_integer<16>, std::vector<std::string>>::value));
  BOOST_CHECK(
      (!std::is_assignable<integer &, std::vector<std::string>>::value));
  BOOST_CHECK(
      (!std::is_assignable<mp_integer<32> &, std::vector<std::string>>::value));
}
...
struct integral_conversion_tester {
  template <typename U> struct runner {
    template <typename T> void operator()(const T &) {
      ...
      BOOST_CHECK_EQUAL(
          std::numeric_limits<T>::max(),
          static_cast<T>(int_type{std::numeric_limits<T>::max()}));
      BOOST_CHECK_EQUAL(
          std::numeric_limits<T>::min(),
          static_cast<T>(int_type{std::numeric_limits<T>::min()}));
      BOOST_CHECK_EQUAL(
          std::numeric_limits<T>::max() - T(1),
          static_cast<T>(int_type{std::numeric_limits<T>::max() - T(1)}));
      BOOST_CHECK_EQUAL(
          std::numeric_limits<T>::min() + T(1),
          static_cast<T>(int_type{std::numeric_limits<T>::min() + T(1)}));
      BOOST_CHECK_EQUAL(
          std::numeric_limits<T>::max() - T(2),
          static_cast<T>(int_type{std::numeric_limits<T>::max() - T(2)}));
      BOOST_CHECK_EQUAL(
          std::numeric_limits<T>::min() + T(2),
          static_cast<T>(int_type{std::numeric_limits<T>::min() + T(2)}));
      mpz_raii tmp;
      std::ostringstream oss;
      oss << static_cast<typename std::conditional<
          std::is_signed<T>::value, long long, unsigned long long>::type>(
      ...
      BOOST_CHECK_THROW((void)static_cast<T>(int_type{mpz_lexcast(tmp)}),
                        std::overflow_error);
      ::mpz_add_ui(&tmp.m_mpz, &tmp.m_mpz, 1ul);
      BOOST_CHECK_THROW((void)static_cast<T>(int_type{mpz_lexcast(tmp)}),
                        std::overflow_error);
      ::mpz_sub_ui(&tmp.m_mpz, &tmp.m_mpz, 2ul);
      BOOST_CHECK_EQUAL(static_cast<T>(int_type{mpz_lexcast(tmp)}),
                        std::numeric_limits<T>::max());
      oss.str("");
      oss << static_cast<typename std::conditional<
          std::is_signed<T>::value, long long, unsigned long long>::type>(
...
struct float_conversion_tester {
  template <typename U> struct runner {
    template <typename T> void operator()(const T &) {
      ...
      BOOST_CHECK_EQUAL(T(-2), static_cast<T>(int_type{-2}));
      // NOTE: hopefully the radix value is not insane here and we can take
      // negative and +-1.
      ...
      BOOST_CHECK_EQUAL(
          std::numeric_limits<T>::lowest(),
          static_cast<T>(int_type{std::numeric_limits<T>::lowest()}));
      // Random testing.
      std::uniform_int_distribution<int> sign_dist(0, 1);
      std::uniform_int_distribution<int> exp_dist(
          0, std::min(get_max_exp(std::numeric_limits<T>::radix),
      ...
      if (std::numeric_limits<T>::has_infinity) {
        mpz_raii tmp;
        ::mpz_set_str(&tmp.m_mpz, boost::lexical_cast<std::string>(
                                      int_type{std::numeric_limits<T>::max()})
                                      .c_str(),
                      10);
        ::mpz_mul_si(&tmp.m_mpz, &tmp.m_mpz, radix);
        BOOST_CHECK_EQUAL(std::numeric_limits<T>::infinity(),
                          static_cast<T>(int_type{mpz_lexcast(tmp)}));
        ::mpz_neg(&tmp.m_mpz, &tmp.m_mpz);
        BOOST_CHECK_EQUAL(std::copysign(std::numeric_limits<T>::infinity(),
                                        std::numeric_limits<T>::lowest()),
                          static_cast<T>(int_type{mpz_lexcast(tmp)}));
      }
    }
  ...
  template <typename T> void operator()(const T &) {
    ...
    for (int i = 0; i < ntries; ++i) {
      ...
      if (tmp < 0) {
        ...
        tmp_int.negate();
        BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(tmp_int),
  ...
  template <typename T> void operator()(const T &) {
    typedef mp_integer<T::value> int_type;
    {
      ...
      BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(a),
                        boost::lexical_cast<std::string>(int_type{0}));
      BOOST_CHECK(a.is_static());
      a = int_type{"1"};
      b = a;
      a += b;
      BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(a),
                        boost::lexical_cast<std::string>(int_type{2}));
      BOOST_CHECK(a.is_static());
      a = int_type{"1"};
      b = int_type{"-1"};
      a += b;
    ...
    int_type c;
    mpz_raii m_c;
    ::mpz_setbit(&m_c.m_mpz, static_cast<::mp_bitcnt_t>(
                                 get_m(c).g_st().limb_bits * 2u - 1u));
    BOOST_CHECK(c.is_static());
    get_m(c).g_st().set_bit(
        static_cast<limb_t>(get_m(c).g_st().limb_bits * 2u - 1u));
    c += c;
...
struct in_place_int_add_tester {
  template <typename U> struct runner {
    template <typename T> void operator()(const T &) {
      ...
      BOOST_CHECK_THROW(n2 += int_type(std::numeric_limits<T>::max()),
                        std::overflow_error);
      BOOST_CHECK_EQUAL(n2, T(1));
      for (int i = 0; i < ntries; ++i) {
        T tmp1 = int_dist(rng), tmp2 = int_dist(rng);
    ...
    template <typename T> void operator()(const T &) {
      ...
      for (int i = 0; i < ntries; ++i) {
        ...
        BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(n),
                          boost::lexical_cast<std::string>(int_type{tmp1}));
        n = 0;
        const T tmp2 = urd2(rng);
        n += tmp2;
        BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(n),
                          boost::lexical_cast<std::string>(int_type{tmp2}));
      }
      // float += mp_integer.
      BOOST_CHECK((is_addable_in_place<T, int_type>::value));
      BOOST_CHECK((!is_addable_in_place<const T, int_type>::value));
...
struct in_place_mp_integer_mul_tester {
  template <typename T> void operator()(const T &) {
    typedef mp_integer<T::value> int_type;
    {
      ...
      a *= b;
      BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(a),
                        boost::lexical_cast<std::string>(int_type{1}));
      BOOST_CHECK(a.is_static());
      a = int_type{"1"};
      b = int_type{"-1"};
      a *= b;

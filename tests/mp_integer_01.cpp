...
struct constructor_tester {
    ...
    void operator()(const T &) const
    {
        ...
        BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(-4), boost::lexical_cast<std::string>(int_type(-4)));
        std::uniform_int_distribution<short> short_dist(boost::integer_traits<short>::const_min,
        ...
        std::uniform_int_distribution<int> int_dist(boost::integer_traits<int>::const_min,
        ...
        std::uniform_int_distribution<long> long_dist(boost::integer_traits<long>::const_min,
        ...
        std::uniform_int_distribution<long long> llong_dist(boost::integer_traits<long long>::const_min,
...
struct static_mpz_view_tester {
    ...
    void operator()(const T &)
    {
        ...
        for (int i = 0; i < ntries; ++i) {
            ...
            BOOST_CHECK(v_ptr->_mp_alloc > 0
                        && (v_ptr->_mp_alloc >= v_ptr->_mp_size || v_ptr->_mp_alloc >= -v_ptr->_mp_size));
    ...
    template <typename T, typename V, typename std::enable_if<std::is_same<::mp_limb_t, typename T::limb_t>::value
...
struct float_ctor_tester {
    ...
    void operator()(const T &)
    {
        ...
        // NOTE: the idea here is that we are going to multiply by radix**n, and we need to make sure both long long and
        // the FP
        // type can represent the result.
        std::uniform_int_distribution<int> exp_dist_d(
            0, std::min(get_max_exp(std::numeric_limits<double>::radix), std::numeric_limits<double>::max_exponent));
        ...
        std::uniform_int_distribution<int> exp_dist_f(
            0, std::min(get_max_exp(std::numeric_limits<float>::radix), std::numeric_limits<float>::max_exponent));
...
struct integral_ctor_tester {
    ...
    struct runner {
        ...
        void operator()(const T &)
        {
            ...
            oss << static_cast<
                typename std::conditional<std::is_signed<T>::value, long long, unsigned long long>::type>(
...
struct str_ctor_tester {
    ...
    void operator()(const T &)
    {
        ...
        BOOST_CHECK_EQUAL("-123", boost::lexical_cast<std::string>(int_type("-123")));
        const std::vector<std::string> invalid_strings{"-0",   "+0",   "01",   "+1",     "+01", "-01",
                                                       "123f", " 123", "123 ", "123.56", "-",   "+",
                                                       "",     " +0",  " -0",  "-123 ",  "12a", "-12a"};
...
struct integral_conversion_tester {
    ...
    struct runner {
        ...
        void operator()(const T &)
        {
            ...
            BOOST_CHECK_EQUAL(std::numeric_limits<T>::max() - T(1),
                              static_cast<T>(int_type{std::numeric_limits<T>::max() - T(1)}));
            BOOST_CHECK_EQUAL(std::numeric_limits<T>::min() + T(1),
                              static_cast<T>(int_type{std::numeric_limits<T>::min() + T(1)}));
            BOOST_CHECK_EQUAL(std::numeric_limits<T>::max() - T(2),
                              static_cast<T>(int_type{std::numeric_limits<T>::max() - T(2)}));
            BOOST_CHECK_EQUAL(std::numeric_limits<T>::min() + T(2),
...
struct float_conversion_tester {
    ...
    struct runner {
        ...
        void operator()(const T &)
        {
            ...
            BOOST_CHECK_EQUAL(T(-radix - 1), static_cast<T>(int_type{-radix - 1}));
            BOOST_CHECK_EQUAL(std::numeric_limits<T>::max(), static_cast<T>(int_type{std::numeric_limits<T>::max()}));
            BOOST_CHECK_EQUAL(std::numeric_limits<T>::lowest(),
                              static_cast<T>(int_type{std::numeric_limits<T>::lowest()}));
            // Random testing.
            std::uniform_int_distribution<int> sign_dist(0, 1);
            std::uniform_int_distribution<int> exp_dist(
                0, std::min(get_max_exp(std::numeric_limits<T>::radix), std::numeric_limits<T>::max_exponent));
            ...
            if (std::numeric_limits<T>::has_infinity) {
                mpz_raii tmp;
                ::mpz_set_str(&tmp.m_mpz,
                              boost::lexical_cast<std::string>(int_type{std::numeric_limits<T>::max()}).c_str(), 10);

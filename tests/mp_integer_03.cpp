...
struct lshift_tester {
  template <typename T> void operator()(const T &) {
    ...
    std::uniform_int_distribution<limb_t> sdist(limb_t(0u),
                                                limb_t(limb_bits * 2u));
    ...
    // Throwing conditions.
    BOOST_CHECK_THROW(
        int_type{1} << (int_type(std::numeric_limits<::mp_bitcnt_t>::max()) +
                        1),
    ...
    unsigned n = 1;
    BOOST_CHECK_THROW(n <<=
                      (int_type(std::numeric_limits<unsigned>::digits) + 10),
                      std::overflow_error);
...
struct rshift_tester {
  template <typename T> void operator()(const T &) {
    ...
    BOOST_CHECK_THROW(
        int_type{1} >>
            (int_type(std::numeric_limits<::mp_bitcnt_t>::max()) + 1),
        std::invalid_argument);
    BOOST_CHECK_THROW(int_type{1} >> int_type(-1), std::invalid_argument);
    BOOST_CHECK_THROW(int_type{1} >> -1, std::invalid_argument);
    // A couple of tests with C++ integral on the left.
...
struct divexact_tester {
  template <typename T> void operator()(const T &) {
    using int_type = mp_integer<T::value>;
    {
      ...
      // Try the exception.
      BOOST_CHECK_THROW(int_type::_divexact(n1, n1, int_type{}),
                        zero_division_error);
    }
    // Random testing.
    std::uniform_int_distribution<int> int_dist(
...
struct gcd_tester {
  template <typename T> void operator()(const T &) {
    ...
    std::uniform_int_distribution<int> ndist(std::numeric_limits<int>::min(),
...
struct divrem_tester {
  template <typename T> void operator()(const T &) {
    typedef mp_integer<T::value> int_type;
    {
      ...
      int_type q, r;
      BOOST_CHECK_THROW(int_type::divrem(q, r, int_type(1), int_type(0)),
                        zero_division_error);
      BOOST_CHECK_THROW(int_type::divrem(q, q, int_type(1), int_type(1)),
                        std::invalid_argument);
      BOOST_CHECK_THROW(int_type::divrem(r, r, int_type(1), int_type(1)),
                        std::invalid_argument);
    }
    // Randomised testing.
    std::uniform_int_distribution<int> pdist(0, 1);
...
struct next_prime_tester {
  template <typename T> void operator()(const T &) {
    ...
    // Random tests.
    std::uniform_int_distribution<int> ud(std::numeric_limits<int>::lowest(),
                                          std::numeric_limits<int>::max());
    ...
    for (int i = 0; i < ntries; ++i) {
      ...
      BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(n.nextprime()),
                        mpz_lexcast(m));
    }
  }
};
...
struct integer_sqrt_tester {
  template <typename T> void operator()(const T &) {
    ...
    for (int i = 0; i < ntries; ++i) {
      ...
      BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(n.sqrt()),
                        mpz_lexcast(m));
    }
  }
};
...
struct factorial_tester {
  template <typename T> void operator()(const T &) {
    ...
    for (int i = 0; i < ntries; ++i) {
      ...
      BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(n.factorial()),
                        mpz_lexcast(m));
      BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(math::factorial(n)),
                        mpz_lexcast(m));
    }
  }
};
...
struct binomial_tester {
  template <typename T> void operator()(const T &) {
    ...
    BOOST_CHECK((std::is_same<int_type,
                              decltype(math::binomial(int_type{}, 0))>::value));
    BOOST_CHECK((std::is_same<decltype(math::binomial(int_type{}, 0)),
                              int_type>::value));
    BOOST_CHECK((has_binomial<int_type, double>::value));
    BOOST_CHECK((has_binomial<double, int_type>::value));
    BOOST_CHECK((
        std::is_same<double, decltype(math::binomial(int_type{}, 0.))>::value));
    BOOST_CHECK((
        std::is_same<decltype(math::binomial(int_type{}, 0.)), double>::value));
    BOOST_CHECK((has_binomial<int_type, int_type>::value));
    BOOST_CHECK((std::is_same<int_type, decltype(math::binomial(
    ...
    for (int i = 0; i < ntries; ++i) {
      ...
      BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(n.binomial(tmp2)),
                        mpz_lexcast(m));
      // int -- integral.
      ...
      BOOST_CHECK_EQUAL(n.binomial((long long)(tmp2)),
                        n.binomial(int_type(tmp2)));
      BOOST_CHECK_EQUAL(n.binomial((unsigned long)(tmp2)),
                        n.binomial(int_type(tmp2)));
      BOOST_CHECK_EQUAL(n.binomial((unsigned long long)(tmp2)),
                        n.binomial(int_type(tmp2)));
    }
    BOOST_CHECK_THROW(
        n.binomial(std::numeric_limits<unsigned long>::max() + int_type(1)),
        std::invalid_argument);
    // Negative k.
...
BOOST_AUTO_TEST_CASE(mp_integer_binomial_test) {
  ...
  BOOST_CHECK_EQUAL(math::binomial(char(4), 2u),
                    math::binomial(int_type(4), 2));
  BOOST_CHECK((has_binomial<long long, int>::value));
  BOOST_CHECK_EQUAL(math::binomial(7ll, 4), math::binomial(int_type(7), 4));
  BOOST_CHECK(
      (std::is_same<decltype(math::binomial(7ll, 4)), int_type>::value));
...
struct sin_cos_tester {
  template <typename T> void operator()(const T &) {
    ...
    BOOST_CHECK(
        (std::is_same<int_type, decltype(math::cos(int_type{}))>::value));
    BOOST_CHECK(
        (std::is_same<int_type, decltype(math::sin(int_type{}))>::value));
...
struct math_divexact_tester {
  template <typename T> void operator()(const T &) {
    ...
    BOOST_CHECK_EQUAL(out, 0);
    BOOST_CHECK_THROW(math::divexact(out, int_type(0), int_type(0)),
                      zero_division_error);
    BOOST_CHECK_THROW(math::divexact(out, int_type(3), int_type(2)),
...
struct static_hash_tester {
  template <typename U> struct runner {
    template <typename T> void operator()(const T &) {
      ...
      for (int i = 0; i < ntries; ++i) {
        // Build randomly two identical integers wide as much as the narrowest
        // of the two int types,
        // and compare their hashes.
        int_type1 a(1);
        int_type2 b(1);
        while (a.m_limbs[1u] < (lt1(1) << (lbits1 - 1u)) &&
               b.m_limbs[1u] < (lt2(1) << (lbits2 - 1u))) {
          int tmp = udist(rng);
...
struct hash_tester {
  template <typename T> void operator()(const T &) {
    ...
    std::uniform_int_distribution<long long> udll(
    ...
    std::uniform_int_distribution<long> udl(std::numeric_limits<long>::lowest(),
    ...
    std::uniform_int_distribution<unsigned long> udul(
    ...
    std::uniform_int_distribution<unsigned long long> udull(
...
struct mpz_view_tester {
  template <typename T> void operator()(const T &) {
    ...
    int_type n0;
    {
      ...
      BOOST_CHECK_EQUAL(mpz_sgn(static_cast<detail::mpz_struct_t const *>(v0)),
                        0);
      // TT checks.
      ...
      BOOST_CHECK(
          !std::is_copy_assignable<
              typename std::add_lvalue_reference<decltype(v0)>::type>::value);
      BOOST_CHECK(
    ...
    n0 = -1;
    {
      auto v0 = n0.get_mpz_view();
      BOOST_CHECK_EQUAL(
          mpz_cmp_si(static_cast<detail::mpz_struct_t const *>(v0), long(-1)),
          0);
    }
    n0 = 2;
    {
      auto v0 = n0.get_mpz_view();
      BOOST_CHECK_EQUAL(
          mpz_cmp_si(static_cast<detail::mpz_struct_t const *>(v0), long(2)),
          0);
    }
    // Random tests.

...
class g_series_type
    : public substitutable_series<series<Cf, Key, g_series_type<Cf, Key>>,
                                  g_series_type<Cf, Key>> {
  using base = substitutable_series<series<Cf, Key, g_series_type<Cf, Key>>,
                                    g_series_type<Cf, Key>>;
...
BOOST_AUTO_TEST_CASE(subs_series_subs_test) {
  ...
  BOOST_CHECK((!has_subs<stype1, std::string>::value));
  {
    ...
    BOOST_CHECK(tmp4.is_identical(math::subs(3 * x + y * y / 7, "y", 2.123_r)));
    BOOST_CHECK((std::is_same<decltype(tmp4),
                              g_series_type<g_series_type<real, monomial<int>>,
                                            new_monomial<int>>>::value));
    BOOST_CHECK_EQUAL(tmp4, 3 * x + math::pow(2.123_r, 2) / 7);
    auto tmp5 = (3 * x + y * y / 7).subs("y", -2_z);
    BOOST_CHECK(tmp5.is_identical(math::subs(3 * x + y * y / 7, "y", -2_z)));
  ...
  BOOST_CHECK((!has_subs<stype2, std::string>::value));
  {
    ...
    BOOST_CHECK(
        tmp3.is_identical(math::subs((3 * x + y * y / 7) * z, "z", 2 / 5_q)));
    ...
    BOOST_CHECK(tmp4.is_identical(math::subs(
        math::subs((3 * x + y * y / 7) * z, "y", 2 / 3_q), "z", 4_z)));
    ...
    BOOST_CHECK(
        tmp5.is_identical(math::subs((3 * x + y * y / 7) * z, "y", -2.123_r)));
    BOOST_CHECK((std::is_same<decltype(tmp5),
                              g_series_type<g_series_type<real, monomial<int>>,
                                            monomial<int>>>::value));
    BOOST_CHECK_EQUAL(tmp5, (3 * x + math::pow(-2.123_r, 2) / 7) * z);
    auto tmp6 = ((3 * x + y * y / 7) * z).subs("z", 2 * t);
    BOOST_CHECK(
        tmp6.is_identical(math::subs((3 * x + y * y / 7) * z, "z", 2 * t)));
  ...
  }
  {
    ...
    BOOST_CHECK_EQUAL((x1 * x2 * y * 4 / 3_q + 2 * y).subs("x", 4),
                      16 * y * 4 / 3_q + 2 * y);
  }
}

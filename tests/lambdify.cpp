...
struct callable_42 {
  double operator()(const std::vector<double> &) const { return 42.; }
};

struct callable_12 {
  double operator()(const std::vector<double> &) const { return 12.; }
};

struct callable_generic {
  template <typename T> T operator()(const std::vector<T> &) const {
    return T{0};
  }
};
...
BOOST_AUTO_TEST_CASE(lambdify_test_00) {
  init();
  {
    ...
    BOOST_CHECK_THROW(
        lambdify<integer>(x + 2 * y + 3 * z, {"y", "z", "x", "x"}),
        std::invalid_argument);
    BOOST_CHECK((has_lambdify<p_type, rational>::value));
    auto l2 =
        lambdify<rational>(x * x - 2 * y + 3 * z * z * z, {"x", "y", "z", "a"});
  ...
  }
  {
    ...
    auto l1 = lambdify<integer>(
        x + y + z, {"x"}, {{"z", [](const std::vector<integer> &v) -> integer {
                              BOOST_CHECK_EQUAL(v.size(), 1u);
                              return 3_z;
                            }}});
    ...
    BOOST_CHECK_EQUAL(
        lambdify<double>(x + y, {"x"}, {{"y", callable_42{}}})({1.}), 43.);
    BOOST_CHECK_EQUAL(
        lambdify<double>(x + y, {"x"}, {{"y", callable_generic{}}})({1.}), 1.);
    BOOST_CHECK_EQUAL(
        lambdify<integer>(x + y, {"x"}, {{"y", callable_generic{}}})({2_z}), 2);
    BOOST_CHECK_EQUAL(
        lambdify<double>(x + y, {"x"}, {{"y", callable_12{}}})({-1.}), 11.);
    BOOST_CHECK_EQUAL(
        lambdify<double>(x + y + z, {"x"},
                         {{"y", callable_12{}}, {"z", callable_42{}}})({-1.}),
        -1 + 42. + 12.);
  }
}
...
BOOST_AUTO_TEST_CASE(lambdify_test_01) {
  {
    ...
    for (int i = 0; i < ntrials; ++i) {
      ...
      BOOST_CHECK_EQUAL(
          l({yn, xn, zn}),
          evaluate<integer>(tmp, {{"x", xn}, {"y", yn}, {"z", zn}}));
    }
  }
  {
    using p_type = polynomial<integer, k_monomial>;
    p_type x{"x"}, y{"y"}, z{"z"};
    auto l0 =
        lambdify<integer>(x + y + z, {"x", "y"},
                          {{"z", [](const std::vector<integer> &v) -> integer {
                              BOOST_CHECK_EQUAL(v.size(), 2u);
                              return v[0] * v[1];
                            }}});
    auto l1(l0);
    ...
    std::uniform_int_distribution<int> dist(-10, 10);
    const auto tmp = x * x - 6 * y + z * y * x;
    auto l = lambdify<integer>(
        tmp, {"y", "x"}, {{"z", [](const std::vector<integer> &v) -> integer {
                             BOOST_CHECK_EQUAL(v.size(), 2u);
                             return v[0] * v[1];
                           }}});
    for (int i = 0; i < ntrials; ++i) {
      ...
      BOOST_CHECK_EQUAL(
          l({yn, xn}),
          evaluate<integer>(tmp, {{"x", xn}, {"y", yn}, {"z", xn * yn}}));
    }
  }
}
...
BOOST_AUTO_TEST_CASE(lambdify_test_02) {
  ...
  BOOST_CHECK_EQUAL_COLLECTIONS(v.begin(), v.end(), l0.get_names().begin(),
                                l0.get_names().end());
  ...
  auto l2 = lambdify<integer>(
      x + y + z, {"z", "y", "x"},
      {{"t", [](const std::vector<integer> &) { return 1_z; }},
       {"a", [](const std::vector<integer> &) { return 1_z; }}});
  en = l2.get_extra_names();
  BOOST_CHECK((en == std::vector<std::string>{"t", "a"} ||
               en == std::vector<std::string>{"a", "t"}));
}

...
BOOST_AUTO_TEST_CASE(dynamic_aligning_allocator_general_test) {
  ...
  if (sizeof(int) > 1u) {
    BOOST_CHECK_THROW(
        daa4.allocate(boost::integer_traits<std::size_t>::const_max),
        std::bad_alloc);
  }
  ...
  BOOST_CHECK(dynamic_aligning_allocator<int>(std::move(daa1_foo)) ==
              dynamic_aligning_allocator<int>(daa1_bar));
...
struct std_container_tester {
  template <typename T> void operator()(const T &) {
    ...
    BOOST_CHECK(std::all_of(v.begin(), v.end(), [](const T &x) {
      return x == boost::lexical_cast<T>(1);
    }));
    std::vector<T, a_type2> l;
    std::generate_n(std::back_inserter(l), 100,
                    []() { return boost::lexical_cast<T>(1); });
    BOOST_CHECK(std::all_of(l.begin(), l.end(), [](const T &x) {
      return x == boost::lexical_cast<T>(1);
    }));
    std::vector<T, a_type2> va(a_type2(alignof(T)));
    std::generate_n(std::back_inserter(va), 100,
                    []() { return boost::lexical_cast<T>(2); });
    BOOST_CHECK(std::all_of(va.begin(), va.end(), [](const T &x) {
      return x == boost::lexical_cast<T>(2);
    }));
    // NOTE: there seems to be a problem currently here with GCC's stdlib - it
    // tries to use directly
    // the allocator's typedefs instead of using allocator_traits.
    // std::list<T,a_type2> la(a_type2(alignof(T)));
    // std::generate_n(std::back_inserter(la),100,[](){return
    // boost::lexical_cast<T>(2);});
    // BOOST_CHECK(std::all_of(la.begin(),la.end(),[](const T &x) {return x ==
    // boost::lexical_cast<T>(2);}));
    if (alignof(T) > boost::integer_traits<std::size_t>::const_max / 2u) {
      return;
    }
    std::vector<T, a_type2> va2(a_type2(alignof(T) * 2u));
    std::generate_n(std::back_inserter(va2), 100,
                    []() { return boost::lexical_cast<T>(3); });
    auto va3(va2);
    va2 = va;
    BOOST_CHECK(std::all_of(va2.begin(), va2.end(), [](const T &x) {
      return x == boost::lexical_cast<T>(2);
    }));
    va3 = std::move(va);
    BOOST_CHECK(std::all_of(va3.begin(), va3.end(), [](const T &x) {
      return x == boost::lexical_cast<T>(2);
    }));
    BOOST_CHECK(std::allocator_traits<
                a_type2>::propagate_on_container_move_assignment::value);
  }
};

...
struct hash_tester {
  template <typename Key> void operator()(const Key &) const {
...
struct partial_tester {
  template <typename Key> void operator()(const Key &) const {
    ...
    using p_type = typename r_type::p_type;
    r_type x{"x"}, y{"y"}, z{"z"};
    r_type::register_custom_derivative(
        "x", [](const r_type &) { return r_type{42}; });
    ...
    BOOST_CHECK_EQUAL(
        math::subs(
            math::subs(math::partial((x + y) / (x - z + y), "x"), "y", x * x),
            "z", 1 / x),
        ((1 + 2 * x) * (x - 1 / x + x * x) -
         (x + x * x) * (1 + 1 / (x * x) + 2 * x)) /
            math::pow(x - 1 / x + x * x, 2));
    r_type::unregister_all_custom_derivatives();
...
struct subs_tester {
  template <typename Key> void operator()(const Key &) const {
    ...
    BOOST_CHECK((std::is_same<polynomial<double, Key>,
                              decltype(math::subs(x / 2, "x", 3.))>::value));
    BOOST_CHECK_EQUAL(math::subs((x + y) / (x - y), "x", zp),
                      (z + y) / (z - y));
    BOOST_CHECK((std::is_same<ps_type, decltype(math::subs((x + y) / (x - y),
                                                           "x", zp))>::value));
    BOOST_CHECK_EQUAL(math::subs((xp + yp) / (xp - yp), "x", zp),
                      (zp + yp) / (zp - yp));
    BOOST_CHECK(
        (std::is_same<ps_type, decltype(math::subs((xp + yp) / (xp - yp), "x",
                                                   zp))>::value));
  }
};
...
struct ipow_subs_tester {
  template <typename Key> void operator()(const Key &) const {
    ...
    BOOST_CHECK(
        (std::is_same<polynomial<double, Key>,
                      decltype(math::ipow_subs(x / 2, "x", 1, 3.))>::value));
    BOOST_CHECK_EQUAL(math::ipow_subs((x * x + y) / (x - y), "x", 2, zp),
                      (z + y) / (x - y));
    BOOST_CHECK(
        (std::is_same<ps_type, decltype(math::ipow_subs((x * x + y) / (x - y),
                                                        "x", 2, zp))>::value));
    BOOST_CHECK_EQUAL(
        math::ipow_subs((xp + yp) / (xp * xp * xp - yp), "x", 2, zp),
        (xp + yp) / (zp * xp - yp));
    BOOST_CHECK((std::is_same<ps_type, decltype(math::ipow_subs(
                                           (xp + yp) / (xp * xp * xp - yp), "x",
                                           2, zp))>::value));
  }
};
...
struct sri_tester {
  template <typename Key> void operator()(const Key &) const {
    ...
    BOOST_CHECK(
        (std::is_same<psr_type, decltype(r_type{} + psr_type{})>::value));
    BOOST_CHECK(
        (std::is_same<psr_type, decltype(psr_type{} + r_type{})>::value));
    // This works thanks to the fact that rational_function has a sri of 1.
    BOOST_CHECK(
        (std::is_same<psr_type, decltype(p_type{} + psr_type{})>::value));
    BOOST_CHECK(
        (std::is_same<psr_type, decltype(psr_type{} + p_type{})>::value));

...
struct hash_tester {
    ...
    void operator()(const Key &) const
    {
        ...
        BOOST_CHECK_EQUAL((x / y).hash(), static_cast<std::size_t>((x + y - y).hash() + (y + x - x).hash()));
...
struct partial_tester {
    ...
    void operator()(const Key &) const
    {
        ...
        r_type::register_custom_derivative("x",
                                           [&x](const r_type &r) { return r.partial("x") + r.partial("y") * 2 * x; });
        BOOST_CHECK_EQUAL(math::partial((x + y) / (x - z + y), "x"),
                          ((1 + 2 * x) * (x - z + y) - (x + y) * (1 + 2 * x)) / math::pow(x - z + y, 2));
        r_type::unregister_all_custom_derivatives();
        BOOST_CHECK_EQUAL(math::partial((x + y) / (x - z + y), "x"), ((x - z + y) - (x + y)) / math::pow(x - z + y, 2));
        ...
        BOOST_CHECK_EQUAL(math::subs(math::subs(math::partial((x + y) / (x - z + y), "x"), "y", x * x), "z", 1 / x),
                          ((1 + 2 * x) * (x - 1 / x + x * x) - (x + x * x) * (1 + 1 / (x * x) + 2 * x))
                              / math::pow(x - 1 / x + x * x, 2));
        r_type::unregister_all_custom_derivatives();
        // Try also with custom diffs on the poly type.
        p_type::register_custom_derivative(
            "x", [](const p_type &p) { return p.partial("x") + p.partial("y") * 2 * p_type{"x"}; });
        BOOST_CHECK_EQUAL(math::partial((x + y) / (x - z + y), "x"),
                          ((1 + 2 * x) * (x - z + y) - (x + y) * (1 + 2 * x)) / math::pow(x - z + y, 2));
...
struct ipow_subs_tester {
    ...
    void operator()(const Key &) const
    {
        ...
        BOOST_CHECK(
            (std::is_same<ps_type, decltype(math::ipow_subs((xp + yp) / (xp * xp * xp - yp), "x", 2, zp))>::value));
    }
};

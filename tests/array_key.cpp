...
public:
  ...
  explicit g_key_type(Args2 &&... params)
      : base(std::forward<Args2>(params)...) {}
};

namespace std {

template <typename T, typename... Args>
struct hash<g_key_type<T, Args...>>
    : public hash<array_key<T, g_key_type<T, Args...>, Args...>> {};
}
...
// NOTE: the debug access is not really necessary after moving merge_args from
// protected
// to public.
template <> class debug_access<merge_args_tag> {
public:
  template <typename T> struct runner {
    template <typename U> void operator()(const U &) {
      typedef g_key_type<T, U> key_type;
...
template <> class debug_access<add_tag> {
public:
  template <typename T> struct runner {
    template <typename U> void operator()(const U &) {
      typedef g_key_type<T, U> key_type;
...
// NOTE: the debug access is not actually needed any more.
template <> class debug_access<sub_tag> {
public:
  template <typename T> struct runner {
    template <typename U> void operator()(const U &) {
      typedef g_key_type<T, U> key_type;
...
template <> struct hash<fvt> {
  typedef size_t result_type;
  typedef fvt argument_type;
  result_type operator()(const argument_type &) const;
};

template <> struct hash<fvt2> {
  typedef size_t result_type;
  typedef fvt2 argument_type;
  result_type operator()(const argument_type &) const;
};

template <> struct hash<fvt3> {
  typedef size_t result_type;
...
template <typename T>
using add_type = decltype(std::declval<T const &>().vector_add(
    std::declval<T &>(), std::declval<T const &>()));

template <typename T, typename = void> struct has_add {
  static const bool value = false;
};

template <typename T>
struct has_add<T, typename std::enable_if<
                      std::is_same<add_type<T>, add_type<T>>::value>::type> {
...
struct sbe_tester {
  template <typename T> void operator()(const T &) {
    ...
    BOOST_CHECK(
        (std::is_same<decltype(sbe1), std::tuple<typename key_type::size_type,
                                                 int *, int *>>::value));
    ...
    BOOST_CHECK((std::is_same<decltype(sbe2),
                              std::tuple<typename key_type::size_type,

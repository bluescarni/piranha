...
template <> struct debug_access<construction_tag> {
  template <typename Cf> struct runner {
    template <typename Expo> void operator()(const Expo &) {
      ...
      BOOST_CHECK(s3o.m_container.begin()->m_cf.size() ==
                  series_type(5.).m_container.size());
      ...
      BOOST_CHECK(
          (!std::is_constructible<series_type, series_type, int>::value));
...
namespace piranha {
template <> class debug_access<insertion_tag> {
public:
  template <typename Cf> struct runner {
    template <typename Expo> void operator()(const Expo &) {
      typedef term<Cf, monomial<Expo>> term_type;
      ...
      BOOST_CHECK_THROW(s.insert(term_type(Cf(1), key_type())),
...
namespace piranha {
template <> class debug_access<merge_terms_tag> {
public:
  template <typename Cf> struct runner {
    template <typename Expo> void operator()(const Expo &) {
      typedef term<Cf, monomial<Expo>> term_type;
      ...
      auto check_neg_merge = [&it]() {
        BOOST_CHECK(it->m_cf == value_type(1) || it->m_cf == value_type(2) ||
                    it->m_cf == value_type(3) || it->m_cf == value_type(-4) ||
                    it->m_cf == value_type(-5) || it->m_cf == value_type(-6) ||
      ...
      BOOST_CHECK(it->m_cf == value_type(1) + value_type(1) ||
                  it->m_cf == value_type(2) + value_type(2) ||
      ...
      BOOST_CHECK(
          it->m_cf ==
              value_type(1) + value_type(1) + value_type(1) + value_type(1) ||
          it->m_cf ==
              value_type(2) + value_type(2) + value_type(2) + value_type(2) ||
          it->m_cf ==
              value_type(3) + value_type(3) + value_type(3) + value_type(3));
      ++it;
      BOOST_CHECK(
          it->m_cf ==
              value_type(1) + value_type(1) + value_type(1) + value_type(1) ||
...
namespace piranha {
template <> class debug_access<merge_arguments_tag> {
public:
  template <typename Cf> struct runner {
    template <typename Expo> void operator()(const Expo &) {
      typedef term<Cf, monomial<Expo>> term_type;
      ...
      typename series_type::base &s =
          static_cast<typename series_type::base &>(s_derived);
      symbol_set ed1, ed2;
      ...
      BOOST_CHECK(
          merge_out.m_container.find(term_type(Cf(1), key_type{Expo(0)})) !=
          merge_out.m_container.end());
      auto compat_check = [](const typename series_type::base &series) {
        for (auto it = series.m_container.begin();
             it != series.m_container.end(); ++it) {
      ...
      BOOST_CHECK(merge_out.m_container.find(
                      term_type(Cf(1), key_type{Expo(0), Expo(0)})) !=
                  merge_out.m_container.end());
      BOOST_CHECK(merge_out.m_container.find(
                      term_type(Cf(1), key_type{Expo(1), Expo(0)})) !=
                  merge_out.m_container.end());
      BOOST_CHECK(merge_out.m_container.find(
                      term_type(Cf(2), key_type{Expo(2), Expo(0)})) !=
                  merge_out.m_container.end());
      compat_check(merge_out);
    }
  };
...
struct stream_tester {
  template <typename Cf> struct runner {
    template <typename Expo> void operator()(const Expo &) {
      ...
      const std::string tmp_out = boost::lexical_cast<std::string>(
                            3 * poly_type{"x"} + 1 +
                            poly_type{"x"} * poly_type{"x"} +
...
template <typename T>
struct is_zero_impl<
    T, typename std::enable_if<std::is_same<T, fake_int_01>::value>::type> {
  bool operator()(const T &) const;
};

template <typename T>
struct is_zero_impl<
    T, typename std::enable_if<std::is_same<T, fake_int_02>::value>::type> {
  bool operator()(const T &) const;
};
}
...
struct safe_cast_impl<
    integer, T,
    typename std::enable_if<std::is_same<T, fake_int_01>::value>::type> {
...
struct safe_cast_impl<
    integer, T,
    typename std::enable_if<std::is_same<T, fake_int_02>::value>::type> {
  integer operator()(const T &) const;
};
}
...
BOOST_AUTO_TEST_CASE(series_pow_test) {
  ...
  // These are a couple of checks for the new pow() code, which is now able to
  // deal with
  ...
  BOOST_CHECK((std::is_same<decltype(g_series_type<short, int>{}.pow(3)),
                            g_series_type<integer, int>>::value));
  BOOST_CHECK((std::is_same<decltype(g_series_type<int, int>{}.pow(3)),
                            g_series_type<integer, int>>::value));
  BOOST_CHECK_EQUAL(
      (g_series_type<int, int>{"x"}.pow(2)),
  ...
  BOOST_CHECK((std::is_same<decltype(g_series_type<real, int>{}.pow(3.)),
                            g_series_type<real, int>>::value));
  BOOST_CHECK_EQUAL(
      (g_series_type<real, int>{"x"}.pow(2.)),
      (g_series_type<real, int>{"x"} * g_series_type<real, int>{"x"}));
  BOOST_CHECK((std::is_same<decltype(g_series_type<rational, int>{}.pow(3_z)),
                            g_series_type<rational, int>>::value));
  BOOST_CHECK_EQUAL(
      (g_series_type<rational, int>{"x"}.pow(2_z)),

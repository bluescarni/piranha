...
// - we can generally go a few elements beyond the numerical limits of sizes
...
// These seem pretty safe in any concievable situation, but just keep it in
// mind. Note that the implementation
...
struct dynamic_tester {
  template <typename T> void operator()(const T &) {
    ...
    BOOST_CHECK(static_cast<const d1 &>(ds1).begin() ==
                static_cast<const d1 &>(ds1).end());
    ...
    std::vector<T> tmp_vec = {T(0), T(1), T(2), T(3), T(4), T(5),
    ...
    ds10.push_back(time_bomb{});
    ds10.push_back(time_bomb{});
    BOOST_CHECK_THROW((detail::dynamic_storage<time_bomb>{ds10}),
                      std::runtime_error);
    // Assignment.
    d1 ds11, ds12;
    ds11.push_back(T(42));
    ...
    ds15.resize(100u);
    BOOST_CHECK(
        std::equal(ds15.begin(), ds15.end(), std::vector<T>(100u).begin()));
    ds15.resize(200u);
    BOOST_CHECK(
        std::equal(ds15.begin(), ds15.end(), std::vector<T>(200u).begin()));
    ds15.resize(199u);
    BOOST_CHECK(
        std::equal(ds15.begin(), ds15.end(), std::vector<T>(199u).begin()));
    d1 ds16;
    std::vector<T> cmp;
    int n = 0;
...
struct constructor_tester {
  template <typename T> struct runner {
    template <typename U> void operator()(const U &) {
      ...
      BOOST_CHECK(static_cast<v_type const &>(v1).begin() ==
                  static_cast<v_type const &>(v1).end());
      ...
      int n = 0;
      std::generate_n(std::back_inserter(v1),
                      integer(v_type::max_static_size) * 8 + 3,
                      [&n]() { return T(n++); });
      BOOST_CHECK(!v1.is_static());
      v_type v2(v1);
      ...
      n = 0;
      v_type v4;
      std::generate_n(std::back_inserter(v4), integer(v_type::max_static_size),
                      [&n]() { return T(n++); });
...
struct assignment_tester {
  template <typename T> struct runner {
    template <typename U> void operator()(const U &) {
      ...
      v_type v2;
      // This will be static vs static (there is always enough static storage
      // for at least 1 element).
      v2 = v1;
      BOOST_CHECK(v2.size() == 1u);
      BOOST_CHECK(v2[0] == v1[0]);
      // Push enough into v1 to make it dynamic.
      int n = 0;
      std::generate_n(std::back_inserter(v1), integer(v_type::max_static_size),
                      [&n]() { return T(n++); });
      BOOST_CHECK(!v1.is_static());
      BOOST_CHECK(v2.is_static());
      // Static vs dynamic.
      ...
      // Dynamic vs dynamic.
      v_type v4(v2), v5(v2);
      std::transform(v5.begin(), v5.end(), v5.begin(),
...
struct push_back_tester {
  template <typename T> struct runner {
    template <typename U> void operator()(const U &) {
      ...
      std::vector<T> check;
      BOOST_CHECK(v1.size() == 0u);
      for (typename std::decay<decltype(v_type::max_static_size)>::type i = 0u;
           i < v_type::max_static_size; ++i) {
        v1.push_back(T(i));
        check.push_back(T(i));
      }
      v1.push_back(T(5));
...
template <> struct safe_cast_impl<time_bomb2, int, void> {
  time_bomb2 operator()(int n) const { return time_bomb2(n); }
};
}
...
struct init_list_tester {
  template <typename T> struct runner {
    template <typename U> void operator()(const U &) {
      ...
      BOOST_CHECK(
          (std::is_constructible<v_type, std::initializer_list<int>>::value));
      BOOST_CHECK(
          (!std::is_constructible<v_type,
                                  std::initializer_list<time_bomb2>>::value));
...
struct add_tester {
  template <typename T> struct runner {
    template <typename U> void operator()(const U &) {
      ...
      v3.add(v3, v2);
      BOOST_CHECK((v3 == v_type{8 * 2 + 7, 10 * 2 + 8, 12 * 2 + 9, 4 * 2,
                                6 * 2 + 1, 8 * 2 + 2}));
    }
  };
  template <typename T> void operator()(const T &) {
...
BOOST_AUTO_TEST_CASE(small_vector_print_sizes) {
  std::cout << "Signed char: " << sizeof(small_vector<signed char>) << ','
            << detail::prepare_for_print(
                   small_vector<signed char>::max_static_size)
            << ',' << detail::prepare_for_print(
                          small_vector<signed char>::max_dynamic_size)
  ...
  std::cout << "Int        : " << sizeof(small_vector<int>) << ','
            ...
            << ',' << alignof(small_vector<int>) << '\n';
  std::cout << "Long       : " << sizeof(small_vector<long>) << ','
            << detail::prepare_for_print(small_vector<long>::max_static_size)
            << ','
            << detail::prepare_for_print(small_vector<long>::max_dynamic_size)
            << ',' << alignof(small_vector<long>) << '\n';
  std::cout << "Long long  : " << sizeof(small_vector<long long>) << ','
            << detail::prepare_for_print(
                   small_vector<long long>::max_static_size)
            << ',' << detail::prepare_for_print(
                          small_vector<long long>::max_dynamic_size)
...
struct move_tester {
  template <typename T> struct runner {
    template <typename U> void operator()(const U &) {
      ...
      int n = 0;
      std::generate_n(std::back_inserter(v1),
                      integer(v_type::max_static_size) + 1,
                      [&n]() { return T(n++); });
      BOOST_CHECK(!v1.is_static());
      v_type v3(std::move(v1));
      BOOST_CHECK_EQUAL(integer(v3.size()),
                        integer(v_type::max_static_size) + 1);
      BOOST_CHECK_EQUAL(v1.size(), 0u);
      BOOST_CHECK(v1.begin() == v1.end());
      BOOST_CHECK(!v1.is_static());
      ...
      v1 = std::move(v3);
      BOOST_CHECK_EQUAL(integer(v1.size()),
                        integer(v_type::max_static_size) + 1);

...
typedef boost::mpl::vector<std::allocator<char>,malloc_allocator<char>> allocator_types;
...
struct dynamic_tester
{
	...
	struct runner
	{
		...
		void operator()(const U &)
		{
			...
			detail::dynamic_storage<time_bomb,U> ds10;
			ds10.push_back(time_bomb{});
			ds10.push_back(time_bomb{});
			ds10.push_back(time_bomb{});
			ds10.push_back(time_bomb{});
			BOOST_CHECK_THROW((detail::dynamic_storage<time_bomb,U>{ds10}),std::runtime_error);
		}
	};
	template <typename T>
	void operator()(const T &)
	{
		boost::mpl::for_each<allocator_types>(runner<T>());
	}
};

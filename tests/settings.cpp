...
BOOST_AUTO_TEST_CASE(settings_thread_number_test) {
  ...
  BOOST_CHECK_PREDICATE([](unsigned n) { return n != 0; },
...
BOOST_AUTO_TEST_CASE(settings_cache_line_size_test) {
  ...
  BOOST_CHECK_EQUAL(settings::get_cache_line_size(),
                    runtime_info::get_cache_line_size());
...
BOOST_AUTO_TEST_CASE(settings_min_work_per_thread_test) {
  ...
  BOOST_CHECK_THROW(settings::set_min_work_per_thread(0u),

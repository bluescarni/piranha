...
using k_types =
    boost::mpl::vector<monomial<int>, monomial<integer>, monomial<rational>,
                       // This should ensure in the overflow tests below we have
                       // enough bit width (we
...
BOOST_AUTO_TEST_CASE(polynomial_multiplier_multiplier_finalise_test) {
  ...
  // Let's do a check with a fateman1-like: first compute the exact result using
  // integers,
  // then with rationals, then check the two are consistent. Do it with 1 and 4
  // threads.

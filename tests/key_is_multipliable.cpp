...
struct mock_key_00 {
  ...
  static void multiply(std::array<term<Cf, mock_key_00>, 1u> &,
                       term<Cf, mock_key_00> &, const term<Cf, mock_key_00> &,
                       const symbol_set &);
  void trim_identify(symbol_set &, const symbol_set &) const;
...
struct mock_key_01 {
  ...
  void print(std::ostream &, const symbol_set &) const;
  void print_tex(std::ostream &, const symbol_set &) const;
  static void multiply(std::array<term<double, mock_key_01>, 4u> &,
                       const term<double, mock_key_01> &,
...
template <> struct hash<mock_key_00> {
  std::size_t operator()(const mock_key_00 &) const;
};

template <> struct hash<mock_key_01> {

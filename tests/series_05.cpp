...
BOOST_AUTO_TEST_CASE(series_division_test) {
  ...
  }
  {
    ...
    BOOST_CHECK(
        (!is_divisible_in_place<g_series_type<mock_cf, int>, s_type2>::value));
  }
  {
    ...
    BOOST_CHECK(
        (!is_divisible_in_place<s_type2, g_series_type<mock_cf, int>>::value));
  }
  {
  ...
  }
  {
    ...
    BOOST_CHECK(
        (std::is_same<g_series_type<int, int>, decltype(1 / x)>::value));
...
// Check that sin/cos methods that return unreturnable types on a series are
// disabled.
...
// Some evaluation tests after we added the improved checking + error message
// logic in series.
BOOST_AUTO_TEST_CASE(series_evaluation_test) {
  ...
  p_type x{"x"}, y{"y"}, z{"z"};
  BOOST_CHECK_EQUAL(
      math::evaluate<integer>(x + y + z, {{"x", 1_z}, {"y", 2_z}, {"z", 3_z}}),
      1 + 2 + 3);
  BOOST_CHECK_EQUAL(
      math::evaluate<integer>(x + y + z,
                              {{"x", 1_z}, {"y", 2_z}, {"z", 3_z}, {"t", 4_z}}),
      1 + 2 + 3);
  BOOST_CHECK_THROW(
      math::evaluate<integer>(x + y + z, {{"x", 1_z}, {"y", 2_z}}),
      std::invalid_argument);
  try {
    (void)math::evaluate<integer>(x + y + z, {{"x", 1_z}, {"y", 2_z}});
  } catch (const std::invalid_argument &ia) {
    BOOST_CHECK(std::string(ia.what()).find("the symbol 'z' is missing from "
                                            "the series evaluation "
                                            "dictionary") != std::string::npos);
  }
  try {
    (void)math::evaluate<integer>(x + y + z,
                                  {{"x", 1_z}, {"y", 2_z}, {"a", 4_z}});
  } catch (const std::invalid_argument &ia) {
    BOOST_CHECK(std::string(ia.what()).find("the symbol 'z' is missing from "
                                            "the series evaluation "
  ...
  try {
    ...
    BOOST_CHECK(std::string(ia.what()).find("the symbol 'x' is missing from "
                                            "the series evaluation "
                                            "dictionary") != std::string::npos);
  }
  try {
    ...
    BOOST_CHECK(std::string(ia.what()).find("the symbol 'y' is missing from "
                                            "the series evaluation "
                                            "dictionary") != std::string::npos);
  }
  try {

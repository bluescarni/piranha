...
static auto max_nt = []() {
  ...
  if (boost::unit_test::framework::master_test_suite().argc > 1) {
    retval = boost::lexical_cast<unsigned>(
...
struct multiplier : base_series_multiplier<p_type> {
  ...
  struct lf {
    ...
    lf(const multiplier *m, int limit) : m_m(m), m_limit(limit) {
      ...
      std::sort(m_m->m_v2.begin(), m_m->m_v2.end(),
                [m](const term_type *t1, const term_type *t2) {
                  return detail::ps_get_degree(*t1, m->m_ss) <
                         detail::ps_get_degree(*t2, m->m_ss);
                });
      // Create the degree vectors.
      std::transform(m_m->m_v1.begin(), m_m->m_v1.end(),
                     std::back_inserter(m_v_d1), [m](const term_type *t) {
                       return detail::ps_get_degree(*t, m->m_ss);
                     });
      std::transform(m_m->m_v2.begin(), m_m->m_v2.end(),
                     std::back_inserter(m_v_d2), [m](const term_type *t) {
                       return detail::ps_get_degree(*t, m->m_ss);
                     });
    ...
    size_type operator()(const size_type &i) const {
      ...
      const auto it =
          std::upper_bound(m_idx_vector.begin(), m_idx_vector.end(), comp,
                           [this](const d_type &c, const size_type &idx) {
  ...
  p_type::size_type estimate_final_series_size(Args &&... args) const {
    return base::estimate_final_series_size<MultArity, MultFunctor>(
...
BOOST_AUTO_TEST_CASE(fateman1_test) {
  ...
  for (unsigned nt = 1u; nt <= max_nt(); ++nt) {
    ...
    multiplier m(f, g);
    std::cout << real_size /
                     m.estimate_final_series_size<1u, multiplier::p_mult>()
              << '\n';
  }
  std::cout << "\n\n";
}
...
BOOST_AUTO_TEST_CASE(fateman1_truncated_test) {
  ...
  for (unsigned nt = 1u; nt <= max_nt(); ++nt) {
    ...
    std::cout << real_size /
                     m.estimate_final_series_size<1u, multiplier::p_mult>(lf)
              << '\n';
  }
  std::cout << "\n\n";
}

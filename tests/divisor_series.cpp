...
using cf_types = boost::mpl::vector<double, integer, real, rational,
...
struct partial_tester {
  template <typename T> void operator()(const T &) {
    ...
    BOOST_CHECK(
        (std::is_same<s_type, decltype(math::partial(s0, "x"))>::value));
    ...
    auto s5 = s0 * s2 * s4;
    BOOST_CHECK_EQUAL(s5.partial("x"), -s0 * s0 * s2 * s4 - s0 * s2 * s2 * s4 -
                                           s0 * s2 * s4 * s4);
    BOOST_CHECK_EQUAL(s5.partial("z"), 2 * s0 * s0 * s2 * s4);
    auto s6 = s0 * s0 * s2 * s4;
    BOOST_CHECK_EQUAL(s6.partial("x"), -2 * s0 * s0 * s0 * s2 * s4 -
                                           s0 * s0 * s2 * s2 * s4 -
                                           s0 * s0 * s2 * s4 * s4);
    ...
    BOOST_CHECK_EQUAL((x * x * math::invert(z)).partial("x"),
                      2 * x * math::invert(z));
    // This excercises the presense of an additional divisor variable with a
    // zero multiplier.
    BOOST_CHECK_EQUAL((x * x * math::invert(z) + s4 - s4).partial("x"),
                      2 * x * math::invert(z));
    ...
    auto s10 = x * s2 * s4;
    BOOST_CHECK_EQUAL(s10.partial("x"),
                      s2 * s4 + x * (-s2 * s2 * s4 - s2 * s4 * s4));
    auto s11 = math::invert(-3 * x - y);
    auto s12 = math::invert(z);
    auto s13 = x * s11 * s4 + x * y * z * s2 * s2 * s2 * s12;
    BOOST_CHECK_EQUAL(s13.partial("x"),
                      s11 * s4 + x * (3 * s11 * s11 * s4 - s11 * s4 * s4) +
                          y * z * s2 * s2 * s2 * s12 +
                          x * y * z * (-3 * s2 * s2 * s2 * s2 * s12));
    BOOST_CHECK_EQUAL(math::partial(s13, "x"),
                      s11 * s4 + x * (3 * s11 * s11 * s4 - s11 * s4 * s4) +
                          y * z * s2 * s2 * s2 * s12 +
                          x * y * z * (-3 * s2 * s2 * s2 * s2 * s12));
    auto s15 = x * s11 * s4 + x * y * z * s2 * s2 * s2 * s12 + s4 * s12;
    BOOST_CHECK_EQUAL(s15.partial("x"),
                      s11 * s4 + x * (3 * s11 * s11 * s4 - s11 * s4 * s4) +
                          y * z * s2 * s2 * s2 * s12 +
                          x * y * z * (-3 * s2 * s2 * s2 * s2 * s12) -
                          s4 * s4 * s12);
    ...
    BOOST_CHECK_EQUAL(s17.partial("x"),
                      -5 * s2 * s2 * s2 * s2 * s2 * s2 * s16 * s16 * s16 * s12 -
                          3 * s2 * s2 * s2 * s2 * s2 * s16 * s16 * s16 * s16 *
                              s12);
    ...
    BOOST_CHECK_EQUAL(s21.partial("x"),
                      s18.partial("x") * s17 + s18 * s17.partial("x") +
                          s19.partial("x") * s2 * s11 * s12 +
                          s19 * (s2 * s11 * s12).partial("x") +
                          s20.partial("x") * s16 * s2 * s3 +
                          s20 * (s16 * s2 * s3).partial("x"));
    BOOST_CHECK_EQUAL(s21.partial("y"),
                      s18.partial("y") * s17 + s18 * s17.partial("y") +
                          s19.partial("y") * s2 * s11 * s12 +
                          s19 * (s2 * s11 * s12).partial("y") +
                          s20.partial("y") * s16 * s2 * s3 +
                          s20 * (s16 * s2 * s3).partial("y"));
    BOOST_CHECK_EQUAL(s21.partial("z"),
                      s18.partial("z") * s17 + s18 * s17.partial("z") +
                          s19.partial("z") * s2 * s11 * s12 +
                          s19 * (s2 * s11 * s12).partial("z") +
                          s20.partial("z") * s16 * s2 * s3 +
  ...
  static void overflow_check() {
    ...
    s14.insert(
        typename s_type::term_type(typename s_type::term_type::cf_type(1), k0));
    BOOST_CHECK_THROW(s14.partial("x"), std::overflow_error);
    // Skip this overflow test if T is short, as short * short will become int *
    // int and it will
    // not overflow.
    ...
    s15.insert(
        typename s_type::term_type(typename s_type::term_type::cf_type(1), k1));
...
BOOST_AUTO_TEST_CASE(divisor_series_partial_test) {
  ...
  using s_type = divisor_series<p_type, divisor<short>>;
  {
    ...
    BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(p1.t_integrate()),
                      "a*b*1/[(\\nu_{c})]*sin(3*c)");
    BOOST_CHECK_EQUAL(
        boost::lexical_cast<std::string>(p1.t_integrate().partial("a")),
        "b*1/[(\\nu_{c})]*sin(3*c)");
    BOOST_CHECK_EQUAL(
        boost::lexical_cast<std::string>(p1.t_integrate().partial("b")),
        "a*1/[(\\nu_{c})]*sin(3*c)");
    BOOST_CHECK_EQUAL(
        boost::lexical_cast<std::string>(p1.t_integrate().partial("c")),
        "3*a*b*1/[(\\nu_{c})]*cos(3*c)");
    BOOST_CHECK_EQUAL(
        boost::lexical_cast<std::string>(p1.t_integrate().partial("\\nu_{c}")),
        "-a*b*1/[(\\nu_{c})**2]*sin(3*c)");
  }
...
BOOST_AUTO_TEST_CASE(divisor_series_integrate_test) {
  ...
  BOOST_CHECK_EQUAL(math::integrate(x + y.invert(), "x"),
                    x * x / 2 + x * y.invert());
  BOOST_CHECK_THROW(math::integrate(x + y.invert() + x.invert(), "x"),
                    std::invalid_argument);
  BOOST_CHECK_EQUAL(
      math::integrate(x + y.invert() + x.invert() - x.invert(), "x"),
      x * x / 2 + x * y.invert());
}
...
BOOST_AUTO_TEST_CASE(divisor_series_invert_test) {
  ...
  BOOST_CHECK_EQUAL(math::invert(s_type1{2 / 3_q}), 3 / 2_q);
  {
    ...
    BOOST_CHECK(
        (std::is_same<s_type, decltype(math::invert(s_type{}))>::value));
    BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(math::invert(x)),
                      "1/[(x)]");
    BOOST_CHECK_EQUAL(math::invert(s_type{2}), 1 / 2_q);
    BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(math::pow(x, -1)),
                      "x**-1");
    ...
    BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(math::invert(x - y)),
                      "1/[(x-y)]");
    BOOST_CHECK_EQUAL(
        boost::lexical_cast<std::string>(math::invert(2 * x - 4 * y)),
        "1/2*1/[(x-2*y)]");
    BOOST_CHECK_EQUAL(
        boost::lexical_cast<std::string>(math::invert(-2 * x + 4 * y)),
        "-1/2*1/[(x-2*y)]");
    BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(math::invert(x + y + z)),
                      "1/[(x+y+z)]");
    BOOST_CHECK_EQUAL(
        boost::lexical_cast<std::string>(math::invert(x + y + z - z)),
        "1/[(x+y)]");
    ...
    BOOST_CHECK_THROW(
        math::invert((std::numeric_limits<short>::max() + 1_q) * x + y),
        std::invalid_argument);
    // Check, if appropriate, construction from outside the bounds defined in
    // divisor.
    if (detail::safe_abs_sint<short>::value <
        std::numeric_limits<short>::max()) {
      BOOST_CHECK_THROW(
          math::invert((detail::safe_abs_sint<short>::value + 1_q) * x + y),
          std::invalid_argument);
    }
    if (-detail::safe_abs_sint<short>::value >
  ...
  {
    using s_type =
        divisor_series<polynomial<rational, k_monomial>, divisor<short>>;
    s_type x{"x"}, y{"y"}, z{"z"}, null;
    BOOST_CHECK(is_invertible<s_type>::value);
    BOOST_CHECK(
        (std::is_same<s_type, decltype(math::invert(s_type{}))>::value));
  ...
  {
    using s_type = divisor_series<polynomial<rational, monomial<rational>>,
                                  divisor<short>>;
    s_type x{"x"}, y{"y"}, z{"z"}, null;
    BOOST_CHECK(is_invertible<s_type>::value);
    BOOST_CHECK(
  ...
  }
  {
    ...
    using s_type =
        divisor_series<poisson_series<polynomial<rational, monomial<short>>>,
                       divisor<short>>;
    BOOST_CHECK(is_invertible<s_type>::value);
    BOOST_CHECK(
        (std::is_same<s_type, decltype(math::invert(s_type{}))>::value));
    s_type x{"x"}, y{"y"}, null;
    BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(math::pow(2 * x, -1)),
                      "1/2*x**-1");
    BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(math::invert(2 * x)),
                      "1/2*1/[(x)]");
...
BOOST_AUTO_TEST_CASE(divisor_series_rational_multiplication_test) {
  // Test that we handle correctly rational coefficients wrt the lcm computation
  // in the multiplier.

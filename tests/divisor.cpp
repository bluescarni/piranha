...
typedef boost::mpl::vector<signed char, short, int, long, long long, integer>
    value_types;
...
struct insert_tester {
  template <typename T> void operator()(const T &) {
    ...
    std::vector<T> tmp;
    T exponent(0);
    BOOST_CHECK_THROW(d0.insert(tmp.begin(), tmp.end(), exponent),
  ...
  static void range_checks() {
    ...
    if (std::numeric_limits<T>::min() < -detail::safe_abs_sint<T>::value &&
      ...
      BOOST_CHECK_THROW(d.insert(tmp.begin(), tmp.end(), exponent),
                        std::invalid_argument);
    ...
    if (std::numeric_limits<T>::max() < long_e) {
      tmp = {T(1)};
      BOOST_CHECK_THROW(d.insert(tmp.begin(), tmp.end(), long_e),
                        std::invalid_argument);
      BOOST_CHECK_EQUAL(d.size(), 0u);
      std::vector<long long> tmp2 = {long_e, long_e};
      BOOST_CHECK_THROW(d.insert(tmp2.begin(), tmp2.end(), exponent),
                        std::invalid_argument);
      BOOST_CHECK_EQUAL(d.size(), 0u);
    }
...
struct hash_tester {
  template <typename T> void operator()(const T &) {
    ...
    tmp = {T(2), T(1)};
    d0.insert(tmp.begin(), tmp.end(), exponent);
    BOOST_CHECK_EQUAL(d0.hash(),
                      detail::vector_hasher(tmp) +
                          detail::vector_hasher(std::vector<T>{T(1), T(2)}));
    BOOST_CHECK_EQUAL(d0.hash(), hasher(d0));
    ...
    BOOST_CHECK_EQUAL(d0.hash(),
                      detail::vector_hasher(tmp) +
                          detail::vector_hasher(std::vector<T>{T(2), T(1)}));
    BOOST_CHECK_EQUAL(d0.hash(), hasher(d0));
    BOOST_CHECK_EQUAL(d0.size(), 2u);
  }
};
...
struct serialization_tester {
  template <typename T> void operator()(const T &) {
    ...
    {
      std::stringstream ss1;
      // NOTE: this results in a Boost serialization exception that gets thrown
      // (also?) when
      // the ia object is destroyed.
      ss1.str("22 serialization::archive 10 0 0 0 0 1 0 0 0 0 3 2 -2 a 1");
      BOOST_CHECK_THROW(boost::archive::text_iarchive ia(ss1);
                        ia >> dtmp, std::exception);
    }
    {
      std::stringstream ss1;
      ss1.str("22 serialization::archive 10 0 0 0 0 1 0 0 0 0 3 1 -2 3 0");
      BOOST_CHECK_THROW(boost::archive::text_iarchive ia(ss1);
                        ia >> dtmp, std::invalid_argument);
    }
  }
};
...
struct print_tester {
  template <typename T> void operator()(const T &) {
    ...
    d.insert(tmp.begin(), tmp.end(), exponent);
    d.print(oss, v);
    BOOST_CHECK(oss.str() == "1/[(x-2*y)**2*(3*x+4*y)]" ||
                oss.str() == "1/[(3*x+4*y)*(x-2*y)**2]");
    v.add("z");
    tmp = {T(1), T(0), T(-1)};
    d.clear();
    oss.str("");
    ...
    d.insert(tmp.begin(), tmp.end(), exponent);
    d.print(oss, v);
    BOOST_CHECK(oss.str() == "1/[(x-z)*(4*y-z)**3]" ||
                oss.str() == "1/[(4*y-z)**3*(x-z)]");
    tmp = {T(1), T(0), T(0)};
    exponent = 1;
    d.clear();
    oss.str("");
    ...
    d.insert(tmp.begin(), tmp.end(), exponent);
    d.print(oss, v);
    BOOST_CHECK(oss.str() == "1/[(x)*(4*y-z)**3]" ||
                oss.str() == "1/[(4*y-z)**3*(x)]");
    // Check throwing.
    v.add("t");
...
struct print_tex_tester {
  template <typename T> void operator()(const T &) {
    ...
    d.print_tex(oss, v);
    BOOST_CHECK(
        oss.str() == "\\frac{1}{\\left(x-2y\\right)^{2}\\left(3x+4y\\right)}" ||
    ...
    d.print_tex(oss, v);
    BOOST_CHECK(
        oss.str() == "\\frac{1}{\\left(x-z\\right)\\left(4y-z\\right)^{3}}" ||
    ...
    d.print_tex(oss, v);
    BOOST_CHECK(
        oss.str() == "\\frac{1}{\\left(x\\right)\\left(4y-z\\right)^{3}}" ||
...
struct evaluate_tester {
  template <typename T> void operator()(const T &) {
    ...
    BOOST_CHECK(
        (std::is_same<rational, decltype(d.evaluate(pmap_type1(v, dict_type1{}),
                                                    v))>::value));
    BOOST_CHECK(
        (std::is_same<double, decltype(d.evaluate(pmap_type2(v, dict_type2{}),
                                                  v))>::value));
    BOOST_CHECK(
        (std::is_same<real, decltype(d.evaluate(pmap_type3(v, dict_type3{}),
                                                v))>::value));
    T exponent(2);
    std::vector<T> tmp;
    tmp = {T(1), T(-2)};
    ...
    BOOST_CHECK_THROW(d.evaluate(pmap_type1(v, dict_type1{}), v),
                      std::invalid_argument);
    v.add("x");
    // Same as above.
    BOOST_CHECK_THROW(
        d.evaluate(pmap_type1(v, dict_type1{{symbol("x"), 1_q}}), v),
        std::invalid_argument);
    v.add("y");
    // Some numerical checks.
    BOOST_CHECK_EQUAL(d.evaluate(pmap_type1(v, dict_type1{{symbol("x"), -1_q},
                                                          {symbol("y"), 2_q}}),
                                 v),
                      1 / 43200_q);
    BOOST_CHECK_EQUAL(
        d.evaluate(pmap_type1(v, dict_type1{{symbol("x"), 2 / 3_q},
                                            {symbol("y"), -4 / 5_q}}),
                   v),
        -759375_z / 303038464_q);
    BOOST_CHECK_THROW(d.evaluate(pmap_type1(v, dict_type1{{symbol("x"), 2_q},
                                                          {symbol("y"), 1_q}}),
                                 v),
                      zero_division_error);
    ...
    BOOST_CHECK_THROW(
        d.evaluate(
            pmap_type1(symbol_set{symbol{"x"}, symbol{"y"}, symbol{"z"}},
                       dict_type1{{symbol("x"), 2_q}, {symbol("z"), 1_q}}),
            v),
        std::invalid_argument);
    // A simple test with real.
    BOOST_CHECK_EQUAL(
        d.evaluate(pmap_type3(v, dict_type3{{symbol("x"), -1.5_r},
                                            {symbol("y"), 2.5_r}}),
                   v),
        1 / (math::pow(-1.5_r - 2.5_r * 2, 2) *
             math::pow(-1.5_r * 2 + 7 * 2.5_r, 3)));
  }
};
...
struct multiply_tester {
  template <typename T> void operator()(const T &) {
    ...
    res[0u].m_key.print(ss, v);
    BOOST_CHECK(ss.str() == "1/[(x-2*y)**2*(4*x-3*y)**3]" ||
                ss.str() == "1/[(4*x-3*y)**3*(x-2*y)**2]");
...
struct trim_tester {
  template <typename T> void operator()(const T &) {
    ...
    BOOST_CHECK_EQUAL(d1.size(), 2u);
    BOOST_CHECK(ss.str() == "1/[(x-z)**2*(3*x-5*z)]" ||
                ss.str() == "1/[(3*x-5*z)*(x-z)**2]");
    ...
    d1.print(ss, v);
    BOOST_CHECK_EQUAL(d1.size(), 2u);
    BOOST_CHECK(ss.str() == "1/[(x-z)*(3*x-5*z)**4]" ||
                ss.str() == "1/[(3*x-5*z)**4*(x-z)]");
    // Check first throwing condition.
    BOOST_CHECK_THROW(d0.trim(v2, symbol_set{symbol{"x"}, symbol{"z"}}),
                      std::invalid_argument);
...
struct split_tester {
  template <typename T> void operator()(const T &) {
    ...
    BOOST_CHECK_THROW(k1.split(s_to_pos(vs, symbol("y")), vs),
                      std::invalid_argument);
    T exponent(1);
    ...
    BOOST_CHECK_THROW(k1.split(s_to_pos(vs, symbol("y")), vs),
                      std::invalid_argument);
    vs.add("y");
    BOOST_CHECK_THROW(k1.split(s_to_pos(vs, symbol("z")), vs),
                      std::invalid_argument);
    s1 = k1.split(s_to_pos(vs, symbol("x")), vs);
    BOOST_CHECK_EQUAL(s1.first.size(), 1u);

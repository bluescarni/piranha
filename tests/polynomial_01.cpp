...
BOOST_AUTO_TEST_CASE(polynomial_is_evaluable_test) {
  ...
  BOOST_CHECK(
      (is_evaluable<polynomial<mock_cf, monomial<int>>, double>::value));
}
...
namespace piranha {
template <> class debug_access<integral_combination_tag> {
public:
  template <typename Cf> struct runner {
    template <typename Expo> void operator()(const Expo &) {
      ...
      p1 += 2 * p_type{"y"};
      BOOST_CHECK((p1.integral_combination() ==
                   map_type{{"y", integer(2)}, {"x", integer(1)}}));
      p1 = p_type{"x"} + 1;
      ...
      BOOST_CHECK((p1.integral_combination() ==
                   map_type{{"x", integer(2)}, {"z", integer(-3)}}));
    }
  };
  template <typename Cf> void operator()(const Cf &) {
    boost::mpl::for_each<expo_types>(runner<Cf>());
    ...
    if (std::numeric_limits<double>::is_iec559 &&
      ...
      BOOST_CHECK((p2.integral_combination() ==
                   map_type{{"x", integer(2)}, {"y", integer(4)}}));
    ...
    BOOST_CHECK((p3.integral_combination() ==
                 map_type{{"x", integer(2)}, {"y", integer(4)}}));
...
struct pow_tester {
  template <typename Cf> struct runner {
    template <typename Expo> void operator()(const Expo &) {
      typedef polynomial<Cf, monomial<Expo>> p_type;
      p_type p{"x"};
      BOOST_CHECK_EQUAL((2 * p).pow(4),
                        p_type{math::pow(Cf(1) * 2, 4)} * p * p * p * p);
      p *= p_type{"y"}.pow(2);
      BOOST_CHECK_EQUAL((3 * p).pow(4),
                        p_type{math::pow(Cf(1) * 3, 4)} * p * p * p * p);
      if (!std::is_unsigned<Expo>::value) {
        BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(p.pow(-1)),
                          "x**-1*y**-2");
      }
...
BOOST_AUTO_TEST_CASE(polynomial_pow_test) {
  ...
  BOOST_CHECK((std::is_same<decltype(p_type1{"x"}.pow(2.)),
                            polynomial<double, monomial<int>>>::value));
  BOOST_CHECK_EQUAL((p_type1{"x"}.pow(2.)),
                    (polynomial<double, monomial<int>>{"x"} *
                     polynomial<double, monomial<int>>{"x"}));

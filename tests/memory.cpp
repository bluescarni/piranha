...
BOOST_AUTO_TEST_CASE(memory_aligned_malloc_test) {
  ...
  // posix_memalign requires power of two and multiple of sizeof(void *) for
  // alignment value.
  BOOST_CHECK_THROW(aligned_palloc(3, 1), std::bad_alloc);
  BOOST_CHECK_THROW(aligned_palloc(7, 1), std::bad_alloc);
  // For these tests, require that the alignment is valid for int and that it is
  // a power of 2.
  if (sizeof(void *) % alignof(int) == 0 &&
      !(sizeof(void *) & (sizeof(void *) - 1u))) {
...
// NOTE: here we are assuming we can do some basic arithmetics on the alignment
// and size values.
BOOST_AUTO_TEST_CASE(memory_alignment_check_test) {
  ...
  if (alignof(std::string) > 1u) {
    BOOST_CHECK(
        !piranha::alignment_check<std::string>(alignof(std::string) / 2u));
  }
#endif
}
...
BOOST_AUTO_TEST_CASE(memory_parallel_init_destroy_test) {
  for (unsigned i = 0u; i <= settings::get_n_threads(); ++i) {
    ...
    BOOST_CHECK(std::all_of(ptr1.get(), ptr1.get() + small_alloc_size,
                            [](int n) { return n == 0; }));
    auto ptr2 = make_parallel_array<custom_string>(small_alloc_size, i);
    BOOST_CHECK(
        std::all_of(ptr2.get(), ptr2.get() + small_alloc_size,
                    [](const std::string &str) { return str == "hello"; }));
    BOOST_CHECK_THROW(make_parallel_array<faulty_string>(small_alloc_size, i),
                      std::runtime_error);
    // Check zero sizes.
    ...
    if (sizeof(int) > 1u) {
      BOOST_CHECK_THROW(
          make_parallel_array<int>(std::numeric_limits<std::size_t>::max(), i),
          std::bad_alloc);
    }

...
struct constructor_tester {
  template <typename T> void operator()(const T &) {
    ...
    BOOST_CHECK_THROW(k1 = k_type(v2.begin(), v2.end(), symbol_set{}),
    ...
    BOOST_CHECK_EQUAL(k1.get_int(), -3);
    BOOST_CHECK_THROW(k1 = k_type(l2.begin(), l2.end(), symbol_set{}),
                      std::invalid_argument);
    l2 = {-1, 0};
    ...
    struct foo {};
    BOOST_CHECK(
        (std::is_constructible<k_type, int *, int *, symbol_set>::value));
    BOOST_CHECK(
        (!std::is_constructible<k_type, foo *, foo *, symbol_set>::value));
...
struct degree_tester {
  template <typename T> void operator()(const T &) {
    ...
    if (std::is_same<T, signed char>::value) {
      BOOST_CHECK((std::is_same<decltype(k5.degree(ss_to_pos(vs1, {"a"}), vs1)),
                                int>::value));
    } else {
      BOOST_CHECK((std::is_same<decltype(k5.degree(ss_to_pos(vs1, {"a"}), vs1)),
                                T>::value));
    ...
    // NOTE: here it seems the compilation error arising when only {} is used
    // (as opposed
    ...
    BOOST_CHECK_THROW(k5.degree(ss_to_pos(v2, {"c"}), vs1),
                      std::invalid_argument);
    BOOST_CHECK_THROW(k5.ldegree(ss_to_pos(v2, {"c"}), vs1),
                      std::invalid_argument);
...
struct pow_tester {
  template <typename T> void operator()(const T &) {
    ...
    BOOST_CHECK_THROW(k1.pow(std::numeric_limits<T>::max(), vs),
                      std::invalid_argument);
    k1 = k_type{1};
    if (std::get<0u>(limits[1u])[0u] < std::numeric_limits<T>::max()) {
      BOOST_CHECK_THROW(k1.pow(std::get<0u>(limits[1u])[0u] + T(1), vs),
                        std::invalid_argument);
    }
    k1 = k_type{2};
...
struct partial_tester {
  template <typename T> void operator()(const T &) {
    ...
    BOOST_CHECK_THROW(ret = k1.partial(s_to_pos(vs, symbol("x")), vs),
                      std::invalid_argument);
...
struct evaluate_tester {
  template <typename T> void operator()(const T &) {
    ...
    BOOST_CHECK_THROW(k1.evaluate(pmap_type1(vs, dict_type1{}), vs),
                      std::invalid_argument);
    BOOST_CHECK_EQUAL(
        k1.evaluate(pmap_type1(vs, dict_type1{{symbol("x"), integer(1)}}), vs),
        1);
    ...
    k1 = k_type({T(2)});
    BOOST_CHECK_EQUAL(
        k1.evaluate(pmap_type1(vs, dict_type1{{symbol("x"), integer(3)}}), vs),
        9);
    BOOST_CHECK_EQUAL(
    ...
    BOOST_CHECK_EQUAL(
        k1.evaluate(
            pmap_type2(vs, dict_type2{{symbol("y"), -4.3}, {symbol("x"), 3.2}}),
            vs),
    ...
    BOOST_CHECK_EQUAL(
        k1.evaluate(pmap_type3(vs, dict_type3{{symbol("y"), rational(1, 2)},
...
BOOST_AUTO_TEST_CASE(kronecker_monomial_evaluate_test) {
  ...
  BOOST_CHECK(
      (!key_is_evaluable<kronecker_monomial<>, std::vector<int>>::value));
...
struct integrate_tester {
  template <typename T> void operator()(const T &) {
    ...
    const auto &limits = ka::get_limits();
    k1 = k_type{std::get<0u>(limits[2u])[0u], std::get<0u>(limits[2u])[0u]};
    BOOST_CHECK_THROW(ret = k1.integrate(symbol("b"), vs),
                      std::invalid_argument);
  }
};
...
BOOST_AUTO_TEST_CASE(kronecker_monomial_kic_test) {
  ...
  BOOST_CHECK((!key_is_convertible<kronecker_monomial<int>,
                                   kronecker_monomial<long>>::value));

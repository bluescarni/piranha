...
using key_types =
    boost::mpl::vector<monomial<short>, monomial<integer>, k_monomial>;

template <typename Poly>
inline static Poly rn_poly(const Poly &x, const Poly &y, const Poly &z,
                           std::uniform_int_distribution<int> &dist) {
  int nterms = dist(rng);
  Poly retval;
  for (int i = 0; i < nterms; ++i) {
    int m = dist(rng);
    retval += m * (m % 2 ? 1 : -1) * x.pow(dist(rng)) * y.pow(dist(rng)) *
              z.pow(dist(rng));
  }
  return retval;
}
...
template <typename Poly>
inline static Poly rq_poly(const Poly &x, const Poly &y, const Poly &z,
                           std::uniform_int_distribution<int> &dist) {
  int nterms = dist(rng);
  Poly retval;
  for (int i = 0; i < nterms; ++i) {
...
struct division_tester {
  template <typename Key> void operator()(const Key &) {
    ...
    BOOST_CHECK_EQUAL((2 * x * z * (x - y) * (x * x - y)) / (x - y),
                      2 * x * z * (x * x - y));
    BOOST_CHECK_EQUAL((2 * x * z * (x - y) * (x * x - y)) / (z * (x - y)),
                      2 * x * (x * x - y));
    BOOST_CHECK_THROW((2 * x * z * (x - y) * (x * x - y)) / (4 * z * (x - y)),
                      math::inexact_division);
    BOOST_CHECK_EQUAL((2 * x * z * (x - y) * (x * x - y)) / (2 * z * (x - y)),
                      x * (x * x - y));
    BOOST_CHECK_EQUAL((2 * xq * zq * (xq - yq) * (xq * xq - yq)) /
                          (4 * zq * (xq - yq)),
                      xq * (xq * xq - yq) / 2);
...
BOOST_AUTO_TEST_CASE(polynomial_division_test) {
  ...
  BOOST_CHECK(
      (!has_exact_ring_operations<polynomial<double, k_monomial>>::value));
  ...
  BOOST_CHECK((has_exact_ring_operations<
               polynomial<integer, monomial<rational>>>::value));
  BOOST_CHECK(
      (!has_exact_division<polynomial<integer, monomial<rational>>>::value));
  BOOST_CHECK((is_divisible<polynomial<integer, monomial<rational>>>::value));
  BOOST_CHECK(
      (is_divisible_in_place<polynomial<integer, monomial<rational>>>::value));
}
...
struct pp_tester {
  template <typename Key> void operator()(const Key &) {
    ...
    BOOST_CHECK_EQUAL((12 * x + 8 * y + 6 * x * y).primitive_part(),
                      6 * x + 4 * y + 3 * x * y);
  }
};
...
struct gcd_tester {
  template <typename Key> void operator()(const Key &) {
    ...
    auto a = -30 * x.pow(3) * y + 90 * x * x * y * y + 15 * x * x - 60 * x * y +
             45 * y * y;
    auto b = 100 * x * x * y - 140 * x * x - 250 * x * y * y + 350 * x * y -
             150 * y * y * y + 210 * y * y;
    BOOST_CHECK(math::gcd(a, b) == -15 * y + 5 * x ||
                -math::gcd(a, b) == -15 * y + 5 * x);
    // Random testing.
    std::uniform_int_distribution<int> dist(0, 4);
    for (int i = 0; i < ntrials; ++i) {
      auto n = rn_poly(x, y, z, dist), m = rn_poly(x, y, z, dist),
           r = rn_poly(x, y, z, dist);
    ...
    auto explicit_check = [](const p_type &n1, const p_type &n2,
                             const p_type &cmp) -> void {
      auto g = math::gcd(n1, n2);
      BOOST_CHECK(g == cmp || g == -cmp);
    };
    explicit_check(pow(x, 2) * pow(y, 2) - 2 * x * pow(y, 3) * pow(z, 8) +
                       pow(x, 3) * y * y * pow(z, 4) -
                       2 * pow(y, 3) * pow(z, 4),
                   pow(y, 3) * z - 2 * x * pow(z, 3) +
                       x * pow(y, 3) * pow(z, 5) - 2 * pow(x, 2) * pow(z, 7),
                   1 + x * z * z * z * z);
    explicit_check(4 * pow(x, 3) * pow(y, 5) * pow(z, 5) +
                       8 * pow(x, 6) * pow(y, 5) * pow(z, 6) -
                       2 * pow(x, 2) * pow(y, 4) * pow(z, 5) -
                       4 * pow(x, 5) * pow(y, 4) * pow(z, 6),
                   8 * pow(x, 4) * pow(y, 5) * pow(z, 3) -
                       2 * pow(x, 2) * pow(y, 5) * pow(z, 5) -
                       2 * pow(x, 4) * pow(y, 4) * pow(z, 3) -
                       6 * pow(x, 5) * pow(y, 2) * pow(z, 4) +
                       16 * pow(x, 7) * pow(y, 5) * pow(z, 4) -
                       4 * pow(x, 5) * pow(y, 5) * pow(z, 6) -
                       12 * pow(x, 8) * pow(y, 2) * pow(z, 5) -
                       4 * pow(x, 7) * pow(y, 4) * pow(z, 4),
                   -4 * pow(x, 5) * pow(y, 2) * pow(z, 4) -
                       2 * pow(x, 2) * pow(y, 2) * pow(z, 3));
    explicit_check(
        -4 * pow(x, 5) * pow(y, 8) * pow(z, 4) +
            3 * pow(x, 3) * pow(y, 6) * pow(z, 3) +
            9 * pow(x, 2) * pow(y, 5) * pow(z, 3) -
            6 * pow(x, 2) * pow(y, 6) * pow(z, 7) +
            3 * pow(x, 4) * pow(y, 6) * pow(z, 7) -
            2 * pow(x, 6) * pow(y, 5) * pow(z, 3) -
            6 * pow(x, 5) * pow(y, 4) * pow(z, 3) - 4 * pow(x, 4) * pow(y, 8) +
            4 * pow(x, 5) * pow(y, 5) * pow(z, 7) - 12 * pow(x, 3) * pow(y, 7) +
            8 * pow(x, 3) * pow(y, 8) * pow(z, 4) -
            2 * pow(x, 7) * pow(y, 5) * pow(z, 7),
        -6 * pow(x, 8) * y * pow(z, 3) - 12 * pow(x, 6) * pow(y, 4) +
            9 * pow(x, 5) * pow(y, 2) * pow(z, 3),
        2 * pow(x, 5) * y * pow(z, 3) + 4 * pow(x, 3) * pow(y, 4) -
            3 * pow(x, 2) * pow(y, 2) * pow(z, 3));
    explicit_check(
        -4 * pow(x, 3) * pow(y, 3) * pow(z, 6) - 12 * pow(x, 3) * pow(z, 3) +
            16 * pow(x, 4) * pow(z, 7) + 8 * pow(x, 7) * y * pow(z, 4),
        4 * pow(x, 5) * pow(y, 4) * pow(z, 5) +
            8 * pow(x, 2) * pow(y, 3) * pow(z, 8) -
            2 * x * pow(y, 6) * pow(z, 7) +
            3 * pow(x, 2) * pow(y, 3) * pow(z, 7) + 9 * pow(x, 2) * pow(z, 4) -
            6 * pow(x, 6) * y * pow(z, 5) - 12 * pow(x, 3) * pow(z, 8) -
            6 * x * pow(y, 3) * pow(z, 4),
        2 * pow(x, 5) * y * pow(z, 4) + 4 * pow(x, 2) * pow(z, 7) -
            x * pow(y, 3) * pow(z, 6) - 3 * x * pow(z, 3));
  }
};
...
struct gcd_prs_tester {
  template <typename Key> void operator()(const Key &) {
    using p_type = polynomial<integer, Key>;
    using math::pow;
    // Set the default algorithm to PRS, so that it is used at all levels of the
    // recursion.
    BOOST_CHECK(p_type::get_default_gcd_algorithm() ==
    ...
    auto gcd_check = [](const p_type &a, const p_type &b, const p_type &g) {
      try {
        ...
        auto ret = std::get<0u>(
            p_type::gcd(a, b, false, polynomial_gcd_algorithm::heuristic));
    ...
    auto b = 100 * x * x * y - 140 * x * x - 250 * x * y * y + 350 * x * y -
             150 * y * y * y + 210 * y * y;
    BOOST_CHECK(gcd_f(a, b) == -15 * y + 5 * x ||
                -gcd_f(a, b) == -15 * y + 5 * x);
    // Random testing.
    std::uniform_int_distribution<int> dist(0, 4);
    for (int i = 0; i < ntrials; ++i) {
    ...
    // Some explicit tests manually verified via sympy.
    auto explicit_check = [gcd_f](const p_type &n1, const p_type &n2,
                                  const p_type &cmp) -> void {
      auto g = gcd_f(n1, n2);
      BOOST_CHECK(g == cmp || g == -cmp);
    };
...
struct gcd_heu_tester {
  template <typename Key> void operator()(const Key &) {
    ...
    auto gcd_check = [](const p_type &a, const p_type &b, const p_type &g) {
      auto ret = std::get<0u>(
          p_type::gcd(a, b, false, polynomial_gcd_algorithm::prs_sr));
      BOOST_CHECK(ret == g || ret == -g);
    };
    // Some zero tests.
    BOOST_CHECK_EQUAL(gcd_f(p_type{}, p_type{}), 0);
    ...
    for (int i = 0; i < ntrials; ++i) {
      auto n = rn_poly(x, y, z, dist), m = rn_poly(x, y, z, dist),
           r = rn_poly(x, y, z, dist);
      auto tup_res =
          p_type::gcd(n * r, m * n, true, polynomial_gcd_algorithm::heuristic);
      const auto &g = std::get<0u>(tup_res);
      gcd_check(n * r, m * n, g);
      if (math::is_zero(m * n)) {
        BOOST_CHECK_EQUAL(g, n * r);
...
BOOST_AUTO_TEST_CASE(polynomial_gcd_heu_test) {
  ...
  using math::pow;
  auto gcdheu = [](const p_type &a,
                   const p_type &b) -> std::pair<bool, p_type> {
    try {
      return std::make_pair(
          false, std::move(std::get<0u>(detail::gcdheu_geddes(a, b).second)));
    } catch (const detail::gcdheu_failure &) {
  ...
  g_checker(-2 * x + 4 * y - 6 * z, 3 * x - 6 * y + 9 * z, x - 2 * y + 3 * z);
  g_checker((x + y) * (x - y) * z, x * (2 * x - 2 * y) * y * z * z,
            z * (x - y));
  g_checker((x + y) * (x - y) * z, x * (2 * x - 2 * y) * y * z * z,
            z * (x - y));
  // The test slow with PRS_SR.
  auto n = 9 * x * pow(y, 8) + 5 * x * x * pow(y, 9) * pow(z, 7) +
           9 * pow(x, 3) * pow(z, 3) + 5 * pow(x, 6) * pow(y, 8) * pow(z, 8) -
           8 * pow(x, 8) * y * y * pow(z, 7) -
           8 * pow(x, 7) * pow(y, 7) * pow(z, 5) -
           8 * pow(x, 9) * pow(y, 7) * pow(z, 5);
  auto m = 5 * pow(x, 6) * pow(y, 5) * pow(z, 6) +
           9 * pow(x, 4) * pow(y, 3) * pow(z, 8) +
           pow(x, 5) * pow(y, 5) * pow(z, 8) - 8 * pow(x, 9) * y * pow(z, 3) -
           2 * pow(x, 5) * pow(y, 8) * pow(z, 5) -
           2 * pow(x, 7) * pow(y, 9) * pow(z, 2) +
           5 * pow(x, 9) * pow(y, 4) * pow(z, 9) -
...
// This was a specific GCD computation that was very slow before changing the
...
BOOST_AUTO_TEST_CASE(polynomial_gcd_bug_00_test) {
  ...
  auto c =
      -4 * z.pow(3) - 2 * x.pow(3) * y.pow(3) - 4 * x.pow(4) * y * z.pow(4);
  auto d = 3 * x.pow(4) * y.pow(3) * z.pow(2) -
           2 * x.pow(3) * y.pow(4) * z.pow(3) -
           4 * x.pow(3) * y.pow(2) * z.pow(2) - 2 * x.pow(3) * y * z.pow(4);
  auto g = math::gcd(a * d + b * c, b * d);
  BOOST_CHECK(g == y || g == -y);
}

// This failed due to a division by zero by the cofactors cf_p/cf_q in gcdheu.
// The divisibility
// test now also checks that the dividends are not zero.
// NOTE: this does not apply anymore since the most recent implementation of
// gcdheu, but let's keep it around.
...
// This specific computation resulted in a bug in a previous gcdheu
// implementation.
BOOST_AUTO_TEST_CASE(polynomial_gcd_bug_02_test) {
  using p_type = polynomial<integer, k_monomial>;
  p_type x{"x"}, y{"y"}, z{"z"};
  auto num = 12 * x.pow(6) * y.pow(7) * z.pow(3) +
             12 * x.pow(3) * y.pow(5) * z.pow(4) -
             3 * x.pow(4) * y.pow(9) * z.pow(3);
  auto den =
      36 * x.pow(4) * y.pow(7) * z - 48 * x.pow(7) * y.pow(8) * z.pow(3) -
      48 * x.pow(5) * y.pow(7) * z.pow(2) + 36 * x * y.pow(5) * z.pow(2) -
      48 * x.pow(2) * y.pow(5) * z.pow(3) -
      48 * x.pow(4) * y.pow(6) * z.pow(4) - 48 * x.pow(3) * y.pow(4) -
      9 * x.pow(2) * y.pow(9) * z + 12 * x.pow(3) * y.pow(9) * z.pow(2) +
      12 * x.pow(5) * y.pow(10) * z.pow(3) - 48 * y.pow(2) * z +
      12 * x * y.pow(6);
  auto correct =
      -12 * y.pow(2) * z - 12 * x.pow(3) * y.pow(4) + 3 * x * y.pow(6);
  p_type::set_default_gcd_algorithm(polynomial_gcd_algorithm::prs_sr);

...
static inline std::string mpq_lexcast(const mpq_raii &m) {
  ...
  const std::size_t size_base10_num =
                        ::mpz_sizeinbase(mpq_numref(&m.m_mpq), 10),
                    size_base10_den =
                        ::mpz_sizeinbase(mpq_denref(&m.m_mpq), 10),
                    max = std::numeric_limits<std::size_t>::max();
  if (size_base10_den > max - 3u ||
      size_base10_num > max - (size_base10_den + 3u)) {
...
struct constructor_tester {
  template <typename T> void operator()(const T &) {
    ...
    std::cout << "NBits,size,align: " << T::value << ',' << sizeof(q_type)
              << ',' << alignof(q_type) << '\n';
    q_type q;
    ...
    for (int i = 0; i < ntries; ++i) {
      ...
      // The mpq set function only works with unsigned type at the denom. Bypass
      // using mpz directly.
      if (b > 0) {
        ::mpq_set_si(&m.m_mpq, static_cast<long>(a),
      ...
      q_type tmp_q{a, b};
      BOOST_CHECK_EQUAL(mpq_lexcast(m),
                        boost::lexical_cast<std::string>(tmp_q));
      // Check that it is canonical and that canonicalisation doest not change
      // the value.
    ...
    q_type q3(1, -15);
    BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(q3),
                      boost::lexical_cast<std::string>(q_type(q3)));
    q3._num() *= std::numeric_limits<int>::max();
    q3.canonicalise();
    auto q5(q3);
    q_type q4(std::move(q3));
    BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(q4),
                      boost::lexical_cast<std::string>(q_type(q5)));
    q_type q6;
    q6 = q5;
    BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(q6),
                      boost::lexical_cast<std::string>(q_type(q5)));
    q6 = std::move(q5);
    BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(q6),
                      boost::lexical_cast<std::string>(q_type(q4)));
    ...
    // Floating-point.
    const unsigned dradix =
        static_cast<unsigned>(std::numeric_limits<double>::radix);
    {
      double tmp(1);
      tmp /= dradix;
      BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(q_type{tmp}),
                        std::string("1/") +
                            boost::lexical_cast<std::string>(dradix));
      tmp /= dradix;
      BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(q_type{tmp}),
                        std::string("1/") + boost::lexical_cast<std::string>(
                                                int_type(dradix).pow(2)));
      tmp = 0.;
      BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(q_type{tmp}), "0");
      const unsigned ldradix =
          static_cast<unsigned>(std::numeric_limits<long double>::radix);
      long double ltmp(1);
      ltmp /= ldradix;
      BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(q_type{ltmp}),
                        std::string("1/") +
                            boost::lexical_cast<std::string>(ldradix));
      ltmp /= ldradix;
      BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(q_type{ltmp}),
                        std::string("1/") + boost::lexical_cast<std::string>(
                                                int_type(ldradix).pow(2)));
      ltmp = 0.;
      BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(q_type{ltmp}), "0");
      // This was a bug: we did not fix the sign when the input was a negative
      // integral value.
    ...
    if (std::numeric_limits<double>::has_infinity) {
      BOOST_CHECK_THROW(q_type{std::numeric_limits<double>::infinity()},
                        std::invalid_argument);
    }
    if (std::numeric_limits<double>::has_quiet_NaN) {
      BOOST_CHECK_THROW(q_type{std::numeric_limits<double>::quiet_NaN()},
                        std::invalid_argument);
    }
    ...
    BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(q_type{"-4/-2"}), "2");
    BOOST_CHECK_EQUAL(
        boost::lexical_cast<std::string>(q_type{"-432132131123/-289938282"}),
        boost::lexical_cast<std::string>(
            q_type{int_type{"-432132131123"}, int_type{"-289938282"}}));
    ...
    for (int i = 0; i < ntries; ++i) {
      ...
      q_type tmp_q{boost::lexical_cast<std::string>(tmp_num) + "/" +
                   boost::lexical_cast<std::string>(tmp_den)};
    ...
    BOOST_CHECK((std::is_same<decltype(q11 = static_cast<signed char>(-3)),
                              q_type &>::value));
    q11 = int_type{45};
    ...
    if (std::numeric_limits<float>::has_infinity) {
      BOOST_CHECK_THROW((q11 = std::numeric_limits<float>::infinity()),
                        std::invalid_argument);
    }
    ...
    BOOST_CHECK(
        (std::is_same<q_type &, decltype(q11 = std::string("1/0"))>::value));
    BOOST_CHECK((std::is_same<q_type &, decltype(q11 = "1/0")>::value));
    // Check move semantics.
    // A largish rational that forces to use GMP integers, at least on some
    // platforms. The move operation
    // of a GMP int will reset the original value to zero, hence we need to take
    // care of the denominator going to zero.
    q_type tmp00{"231238129381029380129830980980198109890381238120398190/"
                 "2312093812093812903809128310298301928309128390128390128390128"
                 "390183"};
    q11 = std::move(tmp00);
...
struct conversion_tester {
  template <typename T> void operator()(const T &) {
    ...
    if (std::numeric_limits<long double>::has_infinity) {
      ...
      q4._num() *= q4._num();
      BOOST_CHECK_EQUAL(static_cast<long double>(q4),
...
struct plus_tester {
  template <typename T> void operator()(const T &) {
    ...
    using int_type = typename q_type::int_type;
    {
      ...
      if (std::numeric_limits<double>::has_infinity) {
        const auto old_a(a);
        BOOST_CHECK_THROW((a += std::numeric_limits<double>::infinity()),
                          std::invalid_argument);
        BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(a),
                          boost::lexical_cast<std::string>(old_a));
    ...
    for (int i = 0; i < ntries; ++i) {
      ...
      // using mpz directly.
      if (b > 0) {
        ::mpq_set_si(&m0.m_mpq, static_cast<long>(a),
                     static_cast<unsigned long>(b));
      } else {
        ::mpq_set_si(&m0.m_mpq, static_cast<long>(a),
                     static_cast<unsigned long>(1));
      ...
      if (d > 0) {
        ::mpq_set_si(&m1.m_mpq, static_cast<long>(c),
      ...
      BOOST_CHECK_EQUAL(mpq_lexcast(m2),
                        boost::lexical_cast<std::string>(q0 + q1));
      BOOST_CHECK_EQUAL(mpq_lexcast(m2),
                        boost::lexical_cast<std::string>(q1 + q0));
      // With int_type.
      ::mpz_set_si(&z.m_mpz, e);
      ...
      BOOST_CHECK_EQUAL(mpq_lexcast(m2),
                        boost::lexical_cast<std::string>(q0 + int_type{e}));
      BOOST_CHECK_EQUAL(mpq_lexcast(m2),
                        boost::lexical_cast<std::string>(int_type{e} + q0));
      // With int.
      ::mpz_set_si(&z.m_mpz, f);
      ::mpq_set(&m2.m_mpq, &m0.m_mpq);
      ::mpz_addmul(mpq_numref(&m2.m_mpq), mpq_denref(&m2.m_mpq), &z.m_mpz);
      ::mpq_canonicalize(&m2.m_mpq);
      BOOST_CHECK_EQUAL(mpq_lexcast(m2),
                        boost::lexical_cast<std::string>(q0 + f));
      BOOST_CHECK_EQUAL(mpq_lexcast(m2),
                        boost::lexical_cast<std::string>(f + q0));
    }
    // Random testing.
    ...
    for (int i = 0; i < ntries; ++i) {
      ...
      BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(tmp_q),
                        boost::lexical_cast<std::string>(
                            q_type{static_cast<double>(tmp_copy) + y}));
...
struct minus_tester {
  template <typename T> void operator()(const T &) {
    ...
    using int_type = typename q_type::int_type;
    {
      ...
      math::negate(tmp00);
      BOOST_CHECK_EQUAL(
          boost::lexical_cast<std::string>(tmp00),
          std::string("-") +
              boost::lexical_cast<std::string>(
      ...
      BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(q_type{-6, 4}),
                        boost::lexical_cast<std::string>(-q_type{6, 4}));
      BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(q_type{}),
                        boost::lexical_cast<std::string>(-q_type{}));
      // Decrement operators.
      q_type q0;
      BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(--q0), "-1");
      q0 = -1;
...
struct mpq_view_tester {
  template <typename T> void operator()(const T &) {
    ...
    for (int i = 0; i < ntries; ++i) {
      ...
      // using mpz directly.
      if (b > 0) {
        ::mpq_set_si(&mq.m_mpq, static_cast<long>(a),
                     static_cast<unsigned long>(b));
      } else {
        ::mpq_set_si(&mq.m_mpq, static_cast<long>(a),
                     static_cast<unsigned long>(1));
...
struct less_than_tester {
  template <typename T> void operator()(const T &) {
    ...
    for (int i = 0; i < ntries; ++i) {
      ...
      BOOST_CHECK_EQUAL((q_type{a, 1} < q_type{c, 1}),
                        int_type{a} < int_type{c});
      BOOST_CHECK_EQUAL((q_type{a, 5} < q_type{c, 5}),
                        int_type{a} < int_type{c});
      BOOST_CHECK_EQUAL((q_type{a, 2} < q_type{c, 2}),
                        int_type{a} < int_type{c});
      BOOST_CHECK_EQUAL((q_type{a, 10} < q_type{c, 10}),
                        int_type{a} < int_type{c});
      q_type q0{a, b}, q1{c, d};
      // The mpq set function only works with unsigned type at the denom. Bypass
      // using mpz directly.
...
struct pow_tester {
  template <typename T> void operator()(const T &) {
    ...
    BOOST_CHECK_EQUAL(q_type(23, 45).pow(7),
                      q_type(3404825447ull, 373669453125ull));
    BOOST_CHECK_EQUAL(q_type(-23, 45).pow(7),
                      q_type(-3404825447ll, 373669453125ull));
    BOOST_CHECK_EQUAL(q_type(-23, 45).pow(-7),
                      q_type(373669453125ull, -3404825447ll));
    // Random testing.
    std::uniform_int_distribution<int> dist(std::numeric_limits<int>::min(),
                                            std::numeric_limits<int>::max());
    ...
    for (int i = 0; i < ntries; ++i) {
      ...
      auto q1 = q0.pow(exp);
      if (exp >= 0) {
        ::mpz_pow_ui(mpq_numref(&m1.m_mpq), mpq_numref(&m0.m_mpq),
                     static_cast<unsigned long>(exp));
        ::mpz_pow_ui(mpq_denref(&m1.m_mpq), mpq_denref(&m0.m_mpq),
                     static_cast<unsigned long>(exp));
      } else {
        ::mpz_pow_ui(mpq_numref(&m1.m_mpq), mpq_denref(&m0.m_mpq),
                     static_cast<unsigned long>(-exp));
        ::mpz_pow_ui(mpq_denref(&m1.m_mpq), mpq_numref(&m0.m_mpq),
                     static_cast<unsigned long>(-exp));
        ::mpq_canonicalize(&m1.m_mpq);
      }
    ...
    BOOST_CHECK_EQUAL(math::pow(q_type(1, radix), 2.),
                      std::pow(1. / radix, 2.));
    BOOST_CHECK(
        (std::is_same<float, decltype(math::pow(q_type(1), 1.f))>::value));
    BOOST_CHECK((std::is_same<long double, decltype(math::pow(q_type(1, radix),
                                                              2.l))>::value));
    // Rational-rational.
    BOOST_CHECK((is_exponentiable<q_type, q_type>::value));
    BOOST_CHECK_THROW(math::pow(q_type(1, radix), q_type(1, radix)),
                      std::invalid_argument);
    BOOST_CHECK_EQUAL(math::pow(q_type(2, 3), q_type(2)), q_type(4, 9));
    BOOST_CHECK_EQUAL(math::pow(q_type(2, 3), q_type(-2)), q_type(9, 4));
    BOOST_CHECK(
        (std::is_same<q_type, decltype(math::pow(q_type(1, radix),
                                                 q_type(1, radix)))>::value));
    // Special cases.
    ...
    BOOST_CHECK((std::is_same<decltype(math::pow(2., q_type(1, radix))),
                              double>::value));
    BOOST_CHECK((std::is_same<decltype(math::pow(2.f, q_type(1, radix))),
                              float>::value));
    BOOST_CHECK_EQUAL(math::pow(2., q_type(1, radix)),
                      std::pow(2., 1. / radix));
    // Integral-rational.
    ...
    BOOST_CHECK((std::is_same<int_type, decltype(math::pow(
                                            2, q_type(1, radix)))>::value));
    BOOST_CHECK(
        (std::is_same<int_type, decltype(math::pow(int_type(2),
                                                   q_type(1, radix)))>::value));
    BOOST_CHECK_THROW(math::pow(2, q_type(1, radix)), std::invalid_argument);
    BOOST_CHECK_THROW(math::pow(int_type(2), q_type(1, radix)),
                      std::invalid_argument);
    ...
    BOOST_CHECK_THROW(math::pow(int_type(0), q_type(-1, 3)),
                      zero_division_error);
  }
};
...
struct hash_tester {
  template <typename T> void operator()(const T &) {
    ...
    BOOST_CHECK_EQUAL(q_type(4, 5).hash(),
                      std::hash<q_type>()(q_type(-8, -10)));
    q_type q0{123, -456};
...
struct sep_tester {
  template <typename T> using edict = std::unordered_map<std::string, T>;
  template <typename T> void operator()(const T &) {
    using q_type = mp_rational<T::value>;
    BOOST_CHECK_EQUAL(math::partial(q_type{1}, ""), 0);
    BOOST_CHECK(
        (std::is_same<q_type, decltype(math::partial(q_type{1}, ""))>::value));
    BOOST_CHECK(is_differentiable<q_type>::value);
    BOOST_CHECK_EQUAL(math::evaluate(q_type{12}, edict<int>{{"", 1}}), 12);
    BOOST_CHECK_EQUAL(math::evaluate(q_type{10}, edict<double>{{"", 1.321}}),
                      10);
    ...
    BOOST_CHECK(
        (std::is_same<q_type,
                      decltype(math::evaluate(
                          q_type{10}, edict<double>{{"", 1.321}}))>::value));
...
struct binomial_tester {
  template <typename T> void operator()(const T &) {
    ...
    BOOST_CHECK(
        (std::is_same<q_type, decltype(math::binomial(q_type{}, 4))>::value));
    BOOST_CHECK((std::is_same<q_type, decltype(math::binomial(
                                          q_type{}, int_type{4}))>::value));
    BOOST_CHECK((std::is_same<q_type, decltype(math::binomial(
                                          q_type{}, char(4)))>::value));
    BOOST_CHECK((has_binomial<q_type, double>::value));
    BOOST_CHECK(
        (std::is_same<double, decltype(math::binomial(q_type{}, 4.))>::value));
    BOOST_CHECK((has_binomial<q_type, q_type>::value));
    BOOST_CHECK((std::is_same<double, decltype(math::binomial(
                                          q_type{}, q_type{}))>::value));
    ...
    BOOST_CHECK(
        (std::is_same<double, decltype(math::binomial(4, q_type{}))>::value));
    BOOST_CHECK((std::is_same<double, decltype(math::binomial(
                                          int_type(4), q_type{}))>::value));
    BOOST_CHECK((std::is_same<double, decltype(math::binomial(
                                          (long long)4, q_type{}))>::value));
    // Some simple checks.
    ...
    BOOST_CHECK_EQUAL(math::binomial(1, q_type{3, 4}),
                      math::binomial(1., 3. / 4.));
    BOOST_CHECK_EQUAL(math::binomial(q_type{2, 3}, q_type{3, 4}),
                      math::binomial(2. / 3., 3. / 4.));
    BOOST_CHECK_EQUAL(math::binomial(1.2, q_type{3, 4}),
                      math::binomial(1.2, 3. / 4.));
    BOOST_CHECK_EQUAL(math::binomial(q_type{3, 4}, 1.2),
                      math::binomial(3. / 4., 1.2));
    // NOTE: cannot really do random testing at the moment as the implementation
    // of
...
struct safe_cast_tester {
  template <typename T> void operator()(const T &) {
    ...
    BOOST_CHECK_THROW(
        safe_cast<int>(q_type{std::numeric_limits<int>::max()} + 1),
        std::invalid_argument);
    BOOST_CHECK_THROW(
        safe_cast<int>(q_type{std::numeric_limits<int>::min()} - 1),
        std::invalid_argument);
    ...
    BOOST_CHECK_THROW(safe_cast<unsigned>(q_type{4} / 3),
                      std::invalid_argument);
    ...
    BOOST_CHECK_EQUAL(safe_cast<q_type>(-13. / (r * r * r)),
                      q_type(-13, r * r * r));
    if (std::numeric_limits<double>::has_infinity &&
        std::numeric_limits<double>::has_quiet_NaN) {
      BOOST_CHECK_THROW(
          safe_cast<q_type>(std::numeric_limits<double>::infinity()),
          std::invalid_argument);
      BOOST_CHECK_THROW(
          safe_cast<q_type>(std::numeric_limits<double>::quiet_NaN()),
          std::invalid_argument);
    }
  }
};
...
struct serialization_tester {
  template <typename T> void operator()(const T &) {
    ...
    for (int i = 0; i < ntries; ++i) {
      ...
      // NOTE: we don't check for the static/dynamic character here as the
      // following could happen:
      // when initing q, a GCD computation is run which could turn num/den into
      // dynamic from static
      // even if the final result fits into static. Then when deserializing, we
      // would have a static
      // vs dynamic mismatch.
    }
  }
...
struct divexact_tester {
  template <typename T> void operator()(const T &) {
    ...
    BOOST_CHECK_THROW(math::divexact(out, q_type{8}, q_type{0}),
                      zero_division_error);

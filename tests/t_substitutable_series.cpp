...
struct key02 {
  ...
  std::vector<std::pair<std::string, key02>>
  t_subs(const std::string &, const T &, const U &, const symbol_set &) const;
...
class g_series_type
    : public t_substitutable_series<series<Cf, Key, g_series_type<Cf, Key>>,
                                    g_series_type<Cf, Key>> {
  typedef t_substitutable_series<series<Cf, Key, g_series_type<Cf, Key>>,
                                 g_series_type<Cf, Key>>
      base;
...
BOOST_AUTO_TEST_CASE(t_subs_series_t_subs_test) {
  ...
  BOOST_CHECK_EQUAL(
      ...
      c * c * c * c * c);
  BOOST_CHECK_EQUAL((math::t_subs((10 * math::sin(2 * x) -
                                   5 * math::sin(6 * x) + math::sin(10 * x)) /
                                      512,
                                  "x", c, s))
                        .subs("c", math::cos(x))
                        .subs("s", math::sin(x)),
                    math::pow(math::cos(x), 5) * math::pow(math::sin(x), 5));
  BOOST_CHECK_EQUAL((math::cos(x) * math::cos(y)).t_subs("x", c, s),
                    c * math::cos(y));
  BOOST_CHECK_EQUAL((math::sin(x) * math::sin(y)).t_subs("x", c, s),
                    s * math::sin(y));
  BOOST_CHECK_EQUAL((math::sin(x) * math::cos(y)).t_subs("x", c, s),
                    s * math::cos(y));
  BOOST_CHECK_EQUAL((math::cos(x) * math::sin(y)).t_subs("x", c, s),
                    c * math::sin(y));
  BOOST_CHECK_EQUAL(4 * math::sin(2 * x).t_subs("x", c, s), 8 * s * c);
  BOOST_CHECK_EQUAL(5 * math::cos(2 * x).t_subs("x", c, s),
                    5 * (c * c - s * s));
  BOOST_CHECK_EQUAL((2 * math::sin(x + y) * math::cos(x - y)).t_subs("x", c, s),
                    2 * c * s + math::sin(2 * y));
  BOOST_CHECK_EQUAL(math::sin(x + p_type1{"pi2"}).t_subs("pi2", 0, 1),
                    math::cos(x));
  BOOST_CHECK_EQUAL(math::cos(x + p_type1{"pi2"}).t_subs("pi2", 0, 1),
                    -math::sin(x));
  BOOST_CHECK_EQUAL(math::sin(x + p_type1{"pi"}).t_subs("pi", -1, 0),
                    -math::sin(x));
  BOOST_CHECK_EQUAL(math::cos(x + p_type1{"pi"}).t_subs("pi", -1, 0),
                    -math::cos(x));
  BOOST_CHECK_EQUAL(math::sin(-x + p_type1{"pi"}).t_subs("pi", -1, 0),
                    math::sin(x));
  BOOST_CHECK_EQUAL(math::cos(-x + p_type1{"pi"}).t_subs("pi", -1, 0),
                    -math::cos(x));
  BOOST_CHECK_EQUAL(
      (math::cos(-x + p_type1{"pi"}) + math::cos(y)).t_subs("pi", -1, 0),
      -math::cos(x) + math::cos(y));
  BOOST_CHECK_EQUAL(
      (math::cos(-x + p_type1{"pi"}) + math::cos(y + p_type1{"pi"}))
          .t_subs("pi", -1, 0),
      -math::cos(x) - math::cos(y));
  BOOST_CHECK_EQUAL(math::cos(x + p_type1{"2pi"}).t_subs("2pi", 1, 0),
                    math::cos(x));
  BOOST_CHECK_EQUAL(math::sin(x + p_type1{"2pi"}).t_subs("2pi", 1, 0),
                    math::sin(x));
  BOOST_CHECK_EQUAL(math::cos(-x + p_type1{"2pi"}).t_subs("2pi", 1, 0),
                    math::cos(x));
  BOOST_CHECK_EQUAL(math::sin(-x + p_type1{"2pi"}).t_subs("2pi", 1, 0),
                    -math::sin(x));
  BOOST_CHECK_EQUAL(math::t_subs(math::sin(-x + p_type1{"2pi"}), "2pi", 1, 0),
                    -math::sin(x));
  // Real and mixed-series subs.
  typedef poisson_series<polynomial<real, monomial<short>>> p_type3;
  BOOST_CHECK(
      (std::is_same<decltype(p_type3{"x"}.t_subs("x", c, s)), p_type3>::value));
  BOOST_CHECK((std::is_same<decltype(math::t_subs(p_type3{"x"}, "x", c, s)),
                            p_type3>::value));
  BOOST_CHECK_EQUAL(p_type3{"x"}.cos().t_subs("x", c, s), c);
  BOOST_CHECK_EQUAL(p_type3{"x"}.cos().t_subs("x", real(.5), real(1.)),
                    real(.5));
  BOOST_CHECK((std::is_same<decltype(x.t_subs("x", p_type3{"c"}, p_type3{"s"})),
                            p_type3>::value));
  BOOST_CHECK_EQUAL(x.t_subs("x", p_type3{"c"}, p_type3{"s"}), p_type3{"x"});
  BOOST_CHECK_EQUAL(math::sin(x).t_subs("x", p_type3{"c"}, p_type3{3.}),
                    p_type3{3.});
  BOOST_CHECK_EQUAL(
      math::pow(math::cos(p_type3{"x"}), 7)
          .t_subs("x", real(.5), real(math::pow(real(3), .5)) / 2),
      math::pow(real(.5), 7));
  BOOST_CHECK_EQUAL(
      math::pow(math::sin(p_type3{"x"}), 7)
          .t_subs("x", real(math::pow(real(3), .5)) / 2, real(.5)),
      math::pow(real(.5), 7));
  BOOST_CHECK_EQUAL(math::t_subs(math::pow(math::sin(p_type3{"x"}), 7), "x",
                                 real(math::pow(real(3), .5)) / 2, real(.5)),
                    math::pow(real(.5), 7));
  BOOST_CHECK(
      math::abs(((math::pow(math::sin(p_type3{"x"}), 5) *
                  math::pow(math::cos(p_type3{"x"}), 5))
                     .t_subs("x", real(math::pow(real(3), .5)) / 2, real(.5)) -
                 math::pow(real(.5), 5) *
                     math::pow(real(math::pow(real(3), .5)) / 2, 5))
                    .trim()
                    .evaluate(std::unordered_map<std::string, real>{})) < 1E-9);
  ...
  BOOST_CHECK(
      (std::is_same<decltype(p_type2{}.t_subs("x", 1, 2)), p_type2>::value));
  BOOST_CHECK(
      (std::is_same<decltype(p_type2{}.t_subs("x", rational(1), rational(2))),
                    p_type2>::value));
  ...
  BOOST_CHECK(
      (!has_t_subs<g_series_type<double, key02>, double, double>::value));
}

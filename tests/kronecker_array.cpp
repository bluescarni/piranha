...
typedef boost::mpl::vector<std::int_least8_t, std::int_least16_t,
                           std::int_least32_t,
  ...
  template <typename T> void operator()(const T &) {
    ...
    for (size_type i = 1u; i < l.size(); ++i) {
      ...
      if (std::is_same<T, std::make_signed<std::size_t>::type>::value) {
        ...
        std::cout << "] " << std::get<1u>(l[i]) << ',' << std::get<2u>(l[i])
                  << ',' << std::get<3u>(l[i]) << '\n';
      }
    }
  }
...
struct coding_tester {
  template <typename T> void operator()(const T &) {
    ...
    // Exceptions tests.
    BOOST_CHECK_THROW(ka_type::encode(std::vector<T>(l.size())),
                      std::invalid_argument);
    BOOST_CHECK_THROW(ka_type::encode(std::vector<T>{
                          T(0), boost::integer_traits<T>::const_min}),
                      std::invalid_argument);
    BOOST_CHECK_THROW(ka_type::encode(std::vector<T>{
                          T(0), boost::integer_traits<T>::const_max}),
                      std::invalid_argument);

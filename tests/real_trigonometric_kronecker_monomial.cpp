...
struct constructor_tester {
  template <typename T> void operator()(const T &) {
    ...
    BOOST_CHECK(
        (std::is_constructible<k_type, std::initializer_list<int>>::value));
    BOOST_CHECK(
        (std::is_constructible<k_type, std::initializer_list<integer>>::value));
    BOOST_CHECK((
        std::is_constructible<k_type, std::initializer_list<rational>>::value));
    BOOST_CHECK(
        (std::is_constructible<k_type, std::initializer_list<double>>::value));
    k_type k1;
    ...
    BOOST_CHECK(
        (std::is_constructible<k_type, typename std::vector<T>::iterator,
                               typename std::vector<T>::iterator>::value));
    BOOST_CHECK(
        (!std::is_constructible<k_type, typename std::vector<T>::const_iterator,
                                typename std::vector<T>::iterator>::value));
    BOOST_CHECK(
        (!std::is_constructible<k_type, typename std::vector<T>::iterator,
                                int>::value));
...
struct merge_args_tester {
  template <typename T> void operator()(const T &) {
    ...
    vs2.add(symbol("b"));
    k_type k2({-1});
    BOOST_CHECK(k2.merge_args(vs1, vs2).get_int() ==
                ka::encode(std::vector<int>({-1, 0})));
    ...
    BOOST_CHECK(k3.merge_args(vs1, vs2).get_int() ==
                ka::encode(std::vector<int>({-1, 0, -1, 0})));
    BOOST_CHECK(!(k3.merge_args(vs1, vs2).get_flavour()));
    vs1 = symbol_set({symbol("c")});
    k_type k4({-1});
    BOOST_CHECK(k4.merge_args(vs1, vs2).get_int() ==
                ka::encode(std::vector<int>({0, 0, -1, 0})));
    vs1 = symbol_set({});
    k_type k5;
    k5.set_flavour(false);
    BOOST_CHECK(k5.merge_args(vs1, vs2).get_int() ==
                ka::encode(std::vector<int>({0, 0, 0, 0})));
...
struct t_degree_tester {
  template <typename T> void operator()(const T &) {
    ...
    if (std::is_same<signed char, T>::value) {
      ...
      BOOST_CHECK(
          (std::is_same<decltype(k1.t_degree(ss_to_pos(vs1, {"a"}), vs1)),
                        int>::value));
      BOOST_CHECK(
          (std::is_same<decltype(k1.t_ldegree(ss_to_pos(vs1, {"a"}), vs1)),
                        int>::value));
    } else {
    ...
    BOOST_CHECK(k5.t_ldegree(ss_to_pos(vs1, std::set<std::string>{}), vs1) ==
                0);
    ...
    BOOST_CHECK_THROW(k5.t_degree(ss_to_pos(v2, {"d"}), vs1),
                      std::invalid_argument);
    BOOST_CHECK_THROW(k5.t_ldegree(ss_to_pos(v2, {"d"}), vs1),
                      std::invalid_argument);
...
struct t_order_tester {
  template <typename T> void operator()(const T &) {
    ...
    if (std::is_same<T, signed char>::value) {
      ...
      BOOST_CHECK(
          (std::is_same<decltype(k1.t_order(ss_to_pos(vs1, {"a"}), vs1)),
                        int>::value));
      BOOST_CHECK(
          (std::is_same<decltype(k1.t_lorder(ss_to_pos(vs1, {"a"}), vs1)),
                        int>::value));
    } else {
    ...
    // Try with bogus positions.
    symbol_set v2({symbol("a"), symbol("b"), symbol("c"), symbol("d")});
    BOOST_CHECK_THROW(k5.t_order(ss_to_pos(v2, {"d"}), vs1),
                      std::invalid_argument);
    BOOST_CHECK_THROW(k5.t_lorder(ss_to_pos(v2, {"d"}), vs1),
                      std::invalid_argument);
    // Wrong symbol set, will not throw because positions are empty.
...
struct multiply_tester {
  template <typename T> void operator()(const T &) {
    ...
    BOOST_CHECK(tmp[0u] == 3);
    BOOST_CHECK(tmp[1u] == 1);
    // Multiplication that produces first multiplier zero, second negative, in
    // the plus.
    t1 = term_type{1, key_type({1, -1})};
...
struct partial_tester {
  template <typename T> void operator()(const T &) {
    ...
    BOOST_CHECK_THROW(
        k1.partial(
            symbol_set::positions(vs2, symbol_set({symbol("x"), symbol("y")})),
            vs),
        std::invalid_argument);
  }
};
...
struct evaluate_tester {
  template <typename T> void operator()(const T &) {
    ...
    BOOST_CHECK_THROW(k1.evaluate(pmap_type1(vs, dict_type1{}), vs),
                      std::invalid_argument);
    BOOST_CHECK_EQUAL(
        k1.evaluate(pmap_type1(vs, dict_type1{{symbol("x"), integer(0)}}), vs),
        1);
    ...
    BOOST_CHECK(
        (std::is_same<integer,
                      decltype(k1.evaluate(
                          pmap_type1(vs, dict_type1{{symbol("x"), integer(1)}}),
                          vs))>::value));
    // NOTE: here the return type depends on the integral type considered, char
    // * char for instance gives int as result
    // according to the standard integral promotions.
    BOOST_CHECK((
        std::is_same<
            T, decltype(k1.evaluate(
                   symbol_set::positions_map<int>(
                       vs, std::unordered_map<symbol, int>{{symbol("x"), 1}}),
                   vs))>::value ||
        std::is_same<
            int, decltype(k1.evaluate(
                     symbol_set::positions_map<int>(
                         vs, std::unordered_map<symbol, int>{{symbol("x"), 1}}),
                     vs))>::value));
    BOOST_CHECK((std::is_same<real, decltype(k1.evaluate(
                                        symbol_set::positions_map<real>(
                                            vs,
                                            std::unordered_map<symbol, real>{
                                                {symbol("x"), real(1)}}),
                                        vs))>::value));
    BOOST_CHECK(
        (std::is_same<
            double,
            decltype(k1.evaluate(
                symbol_set::positions_map<double>(
    ...
    BOOST_CHECK_EQUAL(
        k1.evaluate(pmap_type2(vs, dict_type2{{symbol("y"), real(-4.3)},
                                              {symbol("x"), real(3.2)}}),
                    vs),
    ...
    k1 = k_type({T(-2), T(-3)});
    BOOST_CHECK_EQUAL(
        k1.evaluate(pmap_type2(vs, dict_type2{{symbol("y"), real(1.234)},
                                              {symbol("x"), real(5.678)}}),
                    vs),
    ...
    k1 = k_type({T(3), T(-2)});
    BOOST_CHECK_EQUAL(
        k1.evaluate(pmap_type3(vs, dict_type3{{symbol("y"), rational(2, 2)},
                                              {symbol("x"), rational(2, 3)}}),
                    vs),
        1);
    k1.set_flavour(false);
    BOOST_CHECK_EQUAL(
...
BOOST_AUTO_TEST_CASE(rtkm_evaluate_test) {
  ...
  BOOST_CHECK((!key_is_evaluable<real_trigonometric_kronecker_monomial<>,
...
BOOST_AUTO_TEST_CASE(rtkm_key_has_t_subs_test) {
  BOOST_CHECK((key_has_t_subs<real_trigonometric_kronecker_monomial<int>, int,
                              int>::value));
  BOOST_CHECK((key_has_t_subs<real_trigonometric_kronecker_monomial<int>, int,
                              int>::value));
  BOOST_CHECK((key_has_t_subs<real_trigonometric_kronecker_monomial<int>,
                              fake_int_01, fake_int_01>::value));
  BOOST_CHECK((!key_has_t_subs<real_trigonometric_kronecker_monomial<int>,
                               fake_int_02, fake_int_02>::value));
  // This fails because the cos and sin replacements must be the same type.
  BOOST_CHECK((!key_has_t_subs<real_trigonometric_kronecker_monomial<short>,
                               int, long>::value));
  BOOST_CHECK((key_has_t_subs<real_trigonometric_kronecker_monomial<short>,
                              long, long>::value));
  BOOST_CHECK((key_has_t_subs<real_trigonometric_kronecker_monomial<long> &,
                              long, const long &>::value));
  BOOST_CHECK(
      (key_has_t_subs<const real_trigonometric_kronecker_monomial<short> &,
                      char, const char &>::value));
  BOOST_CHECK(
      (!key_has_t_subs<const real_trigonometric_kronecker_monomial<long long> &,
                       char, int>::value));
  BOOST_CHECK(
      !key_has_degree<real_trigonometric_kronecker_monomial<int>>::value);
  BOOST_CHECK(
      !key_has_ldegree<real_trigonometric_kronecker_monomial<int>>::value);
  BOOST_CHECK(
      key_has_t_degree<real_trigonometric_kronecker_monomial<int>>::value);
  BOOST_CHECK(
      key_has_t_ldegree<real_trigonometric_kronecker_monomial<int>>::value);
  BOOST_CHECK(
      key_has_t_order<real_trigonometric_kronecker_monomial<int>>::value);
  BOOST_CHECK(
      key_has_t_lorder<real_trigonometric_kronecker_monomial<int>>::value);
}
...
struct t_subs_tester {
  ...
  void operator()(
    ...
    BOOST_CHECK((
        std::is_same<typename res_type1::value_type::first_type, real>::value));
    ...
    BOOST_CHECK((std::is_same<typename res_type2::value_type::first_type,
                              rational>::value));

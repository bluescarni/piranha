...
// NOTE: here we define a custom string class base on std::string that respects
// nothrow requirements in hash_set:
// in the current GCC (4.6) the destructor of std::string does not have nothrow,
// so we cannot use it.
...
namespace std {
template <> struct hash<custom_string> {
...
template <typename T> static inline hash_set<T> make_hash_set() {
  struct lc_func_type {
    T operator()(int n) const { return boost::lexical_cast<T>(n); }
  };
  lc_func_type lc_func;
  return hash_set<T>(
      boost::make_transform_iterator(boost::counting_iterator<int>(0), lc_func),
...
struct initializer_list_tester {
  template <typename T> void operator()(const T &) {
    hash_set<T> h({boost::lexical_cast<T>("1"), boost::lexical_cast<T>("2"),
                   boost::lexical_cast<T>("3"), boost::lexical_cast<T>("4"),
                   boost::lexical_cast<T>("4")});
...
struct random_failure {
  ...
  bool operator==(const random_failure &rf) const { return m_str == rf.m_str; }
...
namespace std {
template <> struct hash<random_failure> {
  typedef size_t result_type;
  typedef random_failure argument_type;
  result_type operator()(const random_failure &rf) const { return rf.hash(); }
};
}
...
BOOST_AUTO_TEST_CASE(hash_set_constructors_test) {
  ...
  BOOST_CHECK_THROW(ht6 = hash_set<custom_string>(
                        boost::integer_traits<std::size_t>::const_max),
                    std::bad_alloc);
  // Check unwind on throw.
  // NOTE: prepare table with large number of buckets, so we are sure the first
  // copy of random_failure will be performed
...
// NOTE: this test had a meaning in a previous implementation of hash_set, now
// it is jut a simple
// insertion test.
...
BOOST_AUTO_TEST_CASE(hash_set_erase_test) {
  ...
  // Tests various possibilities of bucket setup for erase(). This first one
  // triggered a bug
  // where erase() would return not the item after the erased member, but the
  // item before.
  ...
  // Reset the h, and try erasing the first element with 0, 1 and 2 other
  // elements.
  h.clear();
  h.rehash(4u);
  h.insert(0u);
...
struct evaluate_sparsity_tester {
  template <typename T> void operator()(const T &) {
    ...
    h.insert(tmp);
    BOOST_CHECK(
        (h.evaluate_sparsity() == std::map<size_type, size_type>{{1u, 1u}}));
  }
};
...
BOOST_AUTO_TEST_CASE(hash_set_mt_test) {
  ...
  BOOST_CHECK_THROW(
      hash_set<int>(10000, std::hash<int>(), std::equal_to<int>(), 0u),
  ...
  for (int i = 0; i < ntries; ++i) {
    ...
    hash_set<int> h(bcount, std::hash<int>(), std::equal_to<int>(),
                    thread_dist(rng));

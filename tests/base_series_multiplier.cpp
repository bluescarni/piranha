...
template <typename Cf> using p_type = polynomial<Cf, monomial<int>>;
...
struct m_checker : public base_series_multiplier<Series> {
  ...
  template <typename T,
            typename std::enable_if<
                std::is_same<typename T::term_type::cf_type, integer>::value,
                int>::type = 0>
  void term_pointers_checker(const T &s1_, const T &s2_) const {
    ...
    std::copy(this->m_v1.begin(), this->m_v1.end(),
              std::inserter(h1, h1.begin()));
    std::copy(this->m_v2.begin(), this->m_v2.end(),
              std::inserter(h2, h2.begin()));
  ...
  template <typename T,
            typename std::enable_if<
                std::is_same<typename T::term_type::cf_type, rational>::value,
                int>::type = 0>
  void term_pointers_checker(const T &s1_, const T &s2_) const {
    const T &s1 = s1_.size() < s2_.size() ? s2_ : s1_;
    const T &s2 = s1_.size() < s2_.size() ? s1_ : s2_;
    ...
    BOOST_CHECK(s2.size() == this->m_v2.size());
    std::unordered_set<const typename T::term_type *> h1, h2;
    std::transform(s1._container().begin(), s1._container().end(),
                   std::inserter(h1, h1.begin()),
                   [](const typename T::term_type &t) { return &t; });
    std::transform(s2._container().begin(), s2._container().end(),
                   std::inserter(h2, h2.begin()),
                   [](const typename T::term_type &t) { return &t; });
  ...
  typename Series::size_type estimate_final_series_size(Args &&... args) const {
    return base::template estimate_final_series_size<N, MultFunctor>(
        std::forward<Args>(args)...);
  }
  template <typename... Args> static void sanitise_series(Args &&... args) {
  ...
  template <typename... Args> void finalise_series(Args &&... args) const {
    base::finalise_series(std::forward<Args>(args)...);
  }
  unsigned get_n_threads() const { return this->m_n_threads; }
};
...
struct p_sorter {
  bool operator()(const std::pair<unsigned, unsigned> &p1,
                  const std::pair<unsigned, unsigned> &p2) const {
...
struct m_functor_0 {
  ...
  template <typename T> void operator()(const T &i, const T &j) const {
...
struct l_functor_0 {
  ...
  template <typename T> T operator()(const T &) const { return T(m_n); }
  const unsigned m_n;
};
...
BOOST_AUTO_TEST_CASE(base_series_multiplier_blocked_multiplication_test) {
  ...
  // Test error throwing.
  BOOST_CHECK_THROW(m0.blocked_multiplication(mf0, 3u, 2u),
                    std::invalid_argument);
  BOOST_CHECK_THROW(m0.blocked_multiplication(mf0, 101u, 102u),
                    std::invalid_argument);
  BOOST_CHECK_THROW(m0.blocked_multiplication(mf0, 1u, 102u),
                    std::invalid_argument);
...
BOOST_AUTO_TEST_CASE(base_series_multiplier_estimate_final_series_size_test) {
  ...
  for (auto nt = 1u; nt < 4u; ++nt) {
    ...
    }
    {
      ...
      m_checker<pt> m0(e1, e2);
      BOOST_CHECK_EQUAL(
          (m0.estimate_final_series_size<1u, m_functor_0>(l_functor_0{0u})),
          1u);
    }
    // Just a couple of simple tests using polynomials, we can't really know
    // what to expect as the method
    // works in a statistical fashion.
    {
      pt x{"x"}, y{"y"};
      auto a = (x + 2 * y + 4), b = (x * x - 2 * y * x - 3 - 4 * y);
      m_checker<pt> m0(a, b);
      // Here the multiplier functor does nothing, the estimation will exit
      // immediately yielding 1.
      BOOST_CHECK_EQUAL((m0.estimate_final_series_size<1u, m_functor_0>()), 1u);
    }
    // A reduced fateman1 benchmark, just to test a bit more.
    {
      ...
      auto b = f + 1;
      auto retval = f * b;
      std::cout << "Bucket count vs actual size: "
                << retval.table_bucket_count() << ',' << retval.size() << '\n';
    }
  }
...
struct multiplication_tester {
  template <typename T> void operator()(const T &) {
    // NOTE: this test is going to be exact in case of coefficients
    // cancellations with double
    // precision coefficients only if the platform has ieee 754 format (integer
...
BOOST_AUTO_TEST_CASE(base_series_multiplier_plain_multiplication_test) {
  ...
  // Testing ported over from the previous series_multiplier tests. Just use
  // polynomial directly.
  using pt1 = p_type<double>;
  using pt2 = p_type<integer>;
  pt1 p1{"x"}, p2{"x"};
  // Check that the merged symbol set is returned when one of the series is
  // empty.
  BOOST_CHECK(e1 * p1 == 0);
  ...
  BOOST_CHECK(retval._container().begin()->m_cf ==
              (double(3) * double(1)) * (double(2) * double(1)));
  pt2 p3{"x"};
  ...
  BOOST_CHECK(retval._container().begin()->m_key[0] == 2);
  BOOST_CHECK(retval._container().begin()->m_cf ==
              double((double(2) * double(1)) * (integer(1) * 4)));
...
BOOST_AUTO_TEST_CASE(base_series_multiplier_finalise_test) {
  {
    ...
    BOOST_CHECK_EQUAL((x * 4 / 3_q + y * 5 / 2_q) *
                          (x.pow(2) * 4 / 13_q - y * 5 / 17_q),
                      16 * x.pow(3) / 39 + 10 / 13_q * y * x * x -
                          20 * x * y / 51 - 25 * y * y / 34);

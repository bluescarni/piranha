...
using k_types = boost::mpl::vector<monomial<int>, monomial<rational>,
                                   // This should ensure in the overflow tests
                                   // below we have enough bit width (we
...
struct bounds_tester {
  template <typename Cf> struct runner {
    ...
    void operator()(const Key &) {
      ...
      for (unsigned nt = 1u; nt <= 20u; ++nt) {
        ...
        BOOST_CHECK_THROW(math::pow(x, std::numeric_limits<int_type>::max()) *
                          ...
                          std::overflow_error);
        BOOST_CHECK_THROW(
            (math::pow(x, std::numeric_limits<int_type>::max()) + 1) * (x + 1),
            std::overflow_error);
        BOOST_CHECK_THROW(math::pow(x, std::numeric_limits<int_type>::min()) *
                              x.pow(-1),
                          std::overflow_error);
        BOOST_CHECK_THROW(
            (math::pow(x, std::numeric_limits<int_type>::min()) + 1) *
                (x.pow(-1) + 1),
            std::overflow_error);
        BOOST_CHECK_EQUAL(
        ...
        BOOST_CHECK_THROW(
            x * math::pow(y, std::numeric_limits<int_type>::max()) * y,
            std::overflow_error);
        BOOST_CHECK_THROW(
            (x + 1) *
                (math::pow(y, std::numeric_limits<int_type>::max()) * y + 1),
            std::overflow_error);
        BOOST_CHECK_THROW(
            math::pow(x, std::numeric_limits<int_type>::max()) *
            ...
            std::overflow_error);
        BOOST_CHECK_THROW(
            (math::pow(x, std::numeric_limits<int_type>::max()) + 1) *
        ...
        BOOST_CHECK_EQUAL(
            math::pow(y, std::numeric_limits<int_type>::max()) * 0, 0);
        BOOST_CHECK_EQUAL(
            math::pow(y, std::numeric_limits<int_type>::min()) * 0, 0);
    ...
    template <typename Key,
              typename std::enable_if<detail::is_kronecker_monomial<Key>::value,
                                      int>::type = 0>
    void operator()(const Key &) {
      settings::set_min_work_per_thread(1u);
      for (unsigned nt = 1u; nt <= 20u; ++nt) {
        ...
        BOOST_CHECK_THROW(x.pow(limits[0u]) * y.pow(limits[1u]) *
                              z.pow(limits[2u]) * x,
                          std::overflow_error);
        BOOST_CHECK_THROW((x.pow(limits[0u]) + 1) * (y.pow(limits[1u]) + 1) *
                              (z.pow(limits[2u]) + 1) * (x + 1),
                          std::overflow_error);
        BOOST_CHECK_THROW(x.pow(limits[0u]) * y.pow(limits[1u]) *
                              z.pow(limits[2u]) * y,
                          std::overflow_error);
        BOOST_CHECK_THROW((x.pow(limits[0u]) + 1) * (y.pow(limits[1u]) + 1) *
                              (z.pow(limits[2u]) + 1) * (y + 1),
                          std::overflow_error);
        BOOST_CHECK_THROW(x.pow(limits[0u]) * y.pow(limits[1u]) *
                              z.pow(limits[2u]) * z,
                          std::overflow_error);
        BOOST_CHECK_THROW((x.pow(limits[0u]) + 1) * (y.pow(limits[1u]) + 1) *
                              (z.pow(limits[2u]) + 1) * (z + 1),
                          std::overflow_error);
        BOOST_CHECK_THROW(x.pow(-limits[0u]) * y.pow(limits[1u]) *
                              z.pow(limits[2u]) * x.pow(-1),
                          std::overflow_error);
        BOOST_CHECK_THROW((x.pow(-limits[0u]) + 1) * (y.pow(limits[1u]) + 1) *
                              (z.pow(limits[2u]) + 1) * (x.pow(-1) + 1),
                          std::overflow_error);
        BOOST_CHECK_THROW(x.pow(limits[0u]) * y.pow(-limits[1u]) *
                              z.pow(limits[2u]) * y.pow(-1),
                          std::overflow_error);
        BOOST_CHECK_THROW((x.pow(limits[0u]) + 1) * (y.pow(-limits[1u]) + 1) *
                              (z.pow(limits[2u]) + 1) * (y.pow(-1) + 1),
                          std::overflow_error);
        BOOST_CHECK_THROW(x.pow(limits[0u]) * y.pow(limits[1u]) *
                              z.pow(-limits[2u]) * z.pow(-1),
                          std::overflow_error);
        BOOST_CHECK_THROW((x.pow(limits[0u]) + 1) * (y.pow(limits[1u]) + 1) *
                              (z.pow(-limits[2u]) + 1) * (z.pow(-1) + 1),
                          std::overflow_error);
        BOOST_CHECK_EQUAL(
            x.pow(limits[0u] - 1) * y.pow(limits[1u]) * z.pow(limits[2u]) * x,
            x.pow(limits[0u]) * y.pow(limits[1u]) * z.pow(limits[2u]));
        BOOST_CHECK_EQUAL(
            x.pow(limits[0u]) * y.pow(limits[1u] - 1) * z.pow(limits[2u]) * y,
            x.pow(limits[0u]) * y.pow(limits[1u]) * z.pow(limits[2u]));
        BOOST_CHECK_EQUAL(
            x.pow(limits[0u]) * y.pow(limits[1u]) * z.pow(limits[2u] - 1) * z,
            x.pow(limits[0u]) * y.pow(limits[1u]) * z.pow(limits[2u]));
        BOOST_CHECK_EQUAL(x.pow(-limits[0u] + 1) * y.pow(-limits[1u]) *
                              z.pow(-limits[2u]) * x.pow(-1),
                          x.pow(-limits[0u]) * y.pow(-limits[1u]) *
                              z.pow(-limits[2u]));
        BOOST_CHECK_EQUAL(x.pow(-limits[0u]) * y.pow(-limits[1u] + 1) *
                              z.pow(-limits[2u]) * y.pow(-1),
                          x.pow(-limits[0u]) * y.pow(-limits[1u]) *
                              z.pow(-limits[2u]));
        BOOST_CHECK_EQUAL(x.pow(-limits[0u]) * y.pow(-limits[1u]) *
                              z.pow(-limits[2u] + 1) * z.pow(-1),
                          x.pow(-limits[0u]) * y.pow(-limits[1u]) *
                              z.pow(-limits[2u]));
        // Check with constant polys.
        BOOST_CHECK_EQUAL(pt{2} * pt{3}, 6);
...
struct multiplication_tester {
  template <typename Cf> struct runner {
    template <typename Key> void operator()(const Key &) {
      ...
      // precision coefficients only if the platform has ieee 754 format

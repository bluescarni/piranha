...
using key_types =
    boost::mpl::vector<monomial<int>, monomial<rational>, k_monomial>;
...
struct main_tester {
  template <typename Cf> struct runner {
    template <typename Key> void operator()(const Key &) {
      ...
      BOOST_CHECK(
          (!detail::has_set_auto_truncate_degree<pt, std::string>::value));
      ...
      for (unsigned nt = 1u; nt <= 4u; ++nt) {
        ...
        BOOST_CHECK_EQUAL((x + y + 1) * (x + y + 1),
                          2 * x + 2 * y + 1 + 2 * x * y + x * x + y * y);
        ...
        if (std::is_same<Key, monomial<rational>>::value) {
          ...
          BOOST_CHECK_THROW(pt::set_auto_truncate_degree(1 / 2_q),
        ...
        if (std::is_same<monomial<int>, Key>::value) {
          ...
          BOOST_CHECK_THROW(
              pt::set_auto_truncate_degree(
                  integer(std::numeric_limits<long long>::max()) + 1),
              std::invalid_argument);
          ...
          BOOST_CHECK_NO_THROW((x.pow(max / 2) * y.pow(max / 2) + 1) *
                               (x.pow(max / 2) * y.pow(2)));
          // This is what would happen if we used d1 + d2 <= M instead of d1 <=
          // M - d2 in
          // the truncation logic.
          auto check = max / 2 + max / 2;
          BOOST_CHECK_THROW(detail::safe_integral_adder(check, max / 2 + 2),
                            std::overflow_error);
        }
        // Now partial degree.
        ...
        BOOST_CHECK((std::get<2u>(tup) == std::vector<std::string>{"x"}));
        BOOST_CHECK_EQUAL((x + y + z + t) * (x + y + z + t),
                          t * t + 2 * t * x + 2 * t * y + 2 * t * z +
                              2 * x * y + 2 * x * z + y * y + 2 * y * z +
                              z * z);
        // Check that for another series type the truncation settings are
        // untouched.

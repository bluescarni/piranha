...
#include "../src/ipow_substitutable_series.hpp"
...
class g_series_type
    : public ipow_substitutable_series<series<Cf, Key, g_series_type<Cf, Key>>,
                                       g_series_type<Cf, Key>> {
  using base =
      ipow_substitutable_series<series<Cf, Key, g_series_type<Cf, Key>>,
                                g_series_type<Cf, Key>>;
  PIRANHA_SERIALIZE_THROUGH_BASE(base)
public:
  template <typename Cf2> using rebind = g_series_type<Cf2, Key>;
  ...
  explicit g_series_type(const char *name) : base() {
    ...
    this->insert(term_type(
        Cf(1), typename term_type::key_type{typename Key::value_type(1)}));
  }
...
template <class T> class new_monomial : public monomial<T> {
  using base = monomial<T>;

public:
  using base::base;
  new_monomial merge_args(const symbol_set &orig_args,
  ...
  new_monomial trim(const symbol_set &trim_args,
  ...
  static void
  multiply(std::array<term<Cf, new_monomial>, base::multiply_arity> &res,
           const term<Cf, new_monomial> &t1, const term<Cf, new_monomial> &t2,
...
template <typename T> struct hash<new_monomial<T>> {
  std::size_t operator()(const new_monomial<T> &m) const { return m.hash(); }
};
}
...
BOOST_AUTO_TEST_CASE(ipow_subs_series_subs_test) {
  ...
  BOOST_CHECK((!has_ipow_subs<stype0, std::string>::value));
  {
    ...
    BOOST_CHECK((std::is_same<decltype(tmp2),
                              g_series_type<double, monomial<int>>>::value));
    ...
    BOOST_CHECK(
        tmp3.is_identical(math::ipow_subs(3 * x + y * y / 7, "y", 1, 2 / 5_q)));
    ...
    BOOST_CHECK(
        tmp3.is_identical(math::ipow_subs(3 * x + y * y / 7, "x", 2, 2 / 5_q)));
    BOOST_CHECK_EQUAL(tmp3, 3 * x + y * y / 7);
    tmp3 = (3 * x + y * y / 7).ipow_subs("y", 2, 2 / 5_q);
    BOOST_CHECK(
        tmp3.is_identical(math::ipow_subs(3 * x + y * y / 7, "y", 2, 2 / 5_q)));
    BOOST_CHECK_EQUAL(tmp3, 3 * x + 2 / 5_q / 7);
    tmp3 = (3 * x + y * y * y / 7).ipow_subs("y", 2, 2 / 5_q);
    BOOST_CHECK(tmp3.is_identical(
        math::ipow_subs(3 * x + y * y * y / 7, "y", 2, 2 / 5_q)));
    BOOST_CHECK_EQUAL(tmp3, 3 * x + 2 / 5_q / 7 * y);
    auto tmp4 = (3 * x + y * y / 7).ipow_subs("y", 1, 2.123_r);
    BOOST_CHECK(
        tmp4.is_identical(math::ipow_subs(3 * x + y * y / 7, "y", 1, 2.123_r)));
    BOOST_CHECK((std::is_same<decltype(tmp4),
                              g_series_type<real, monomial<int>>>::value));
    BOOST_CHECK_EQUAL(tmp4, 3 * x + math::pow(2.123_r, 2) / 7);
    tmp4 = (3 * x + y * y / 7).ipow_subs("x", 2, 2.123_r);
    BOOST_CHECK(
        tmp4.is_identical(math::ipow_subs(3 * x + y * y / 7, "x", 2, 2.123_r)));
    BOOST_CHECK_EQUAL(tmp4, 3 * x + y * y / 7);
    tmp4 = (3 * x + y * y / 7).ipow_subs("y", 2, 2.123_r);
    BOOST_CHECK(
        tmp4.is_identical(math::ipow_subs(3 * x + y * y / 7, "y", 2, 2.123_r)));
    BOOST_CHECK_EQUAL(tmp4, 3 * x + 2.123_r / 7);
    tmp4 = (3 * x + y * y * y / 7).ipow_subs("y", 2, 2.123_r);
    BOOST_CHECK(tmp4.is_identical(
        math::ipow_subs(3 * x + y * y * y / 7, "y", 2, 2.123_r)));
    BOOST_CHECK_EQUAL(tmp4, 3 * x + y * 2.123_r / 7);
    auto tmp5 = (3 * x + y * y / 7).ipow_subs("y", 1, -2_z);
    BOOST_CHECK(
        tmp5.is_identical(math::ipow_subs(3 * x + y * y / 7, "y", 1, -2_z)));
  ...
  BOOST_CHECK(
      (key_is_multipliable<rational, stype1::term_type::key_type>::value));
  BOOST_CHECK(
      (!key_has_ipow_subs<stype1::term_type::key_type, rational>::value));
  ...
  BOOST_CHECK((!has_ipow_subs<stype1, std::string>::value));
  {
    ...
    BOOST_CHECK(
        (std::is_same<decltype(tmp2),
                      g_series_type<g_series_type<double, monomial<int>>,
                                    new_monomial<int>>>::value));
    tmp2 = (x + y).ipow_subs("x", 2, 2.);
    ...
    BOOST_CHECK(
        tmp6.is_identical(math::ipow_subs(3 * x + y * y / 7, "y", 1, -2 * z)));
    BOOST_CHECK((std::is_same<decltype(tmp6), stype1>::value));
    BOOST_CHECK_EQUAL(tmp6, 3 * x + 4 * z * z / 7);
    tmp6 = (3 * x + y * y / 7).ipow_subs("x", 2, -2 * z);
    BOOST_CHECK(
        tmp6.is_identical(math::ipow_subs(3 * x + y * y / 7, "x", 2, -2 * z)));
  ...
  BOOST_CHECK(
      (key_is_multipliable<rational, stype2::term_type::key_type>::value));
  BOOST_CHECK(
      (key_has_ipow_subs<stype2::term_type::key_type, rational>::value));
  ...
  BOOST_CHECK((!has_ipow_subs<stype2, std::string>::value));
  {
    ...
    BOOST_CHECK(
        tmp.is_identical(math::ipow_subs((x * x * x + y) * z, "x", 2, 2)));
    ...
    BOOST_CHECK_EQUAL(tmp2, (2. + y) * z);
    BOOST_CHECK(
        tmp2.is_identical(math::ipow_subs((x * x + y) * z, "x", 2, 2.)));
    tmp2 = ((x * x * x + y) * z).ipow_subs("x", 2, 2.);
    BOOST_CHECK_EQUAL(tmp2, (2. * x + y) * z);
    BOOST_CHECK(
        tmp2.is_identical(math::ipow_subs((x * x * x + y) * z, "x", 2, 2.)));
    auto tmp3 = ((3 * x + y * y / 7) * z).ipow_subs("z", 1, 2 / 5_q);
    BOOST_CHECK(tmp3.is_identical(
        math::ipow_subs((3 * x + y * y / 7) * z, "z", 1, 2 / 5_q)));
  ...
  }
  {
    ...
    auto tmp =
        (x1.pow(3) * x2.pow(2) * y * z * 4 / 3_q + 2 * t).ipow_subs("x", 2, 3);

...
typedef boost::mpl::vector<double, rational, real,
                           polynomial<rational, monomial<int>>,
...
BOOST_AUTO_TEST_CASE(poisson_series_stream_test) {
  ...
  BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(p_type2{rational(1, 2)}),
                    "1/2");
  BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(p_type2{real("-0.5")}),
                    "-1/2");
  ...
  BOOST_CHECK_EQUAL(
      boost::lexical_cast<std::string>(rational(3, -2) * p_type3{"x"}),
      "-3/2*x");
  BOOST_CHECK_EQUAL(
      boost::lexical_cast<std::string>(rational(3, -2) * p_type3{"x"}.pow(2)),
      "-3/2*x**2");
}
...
BOOST_AUTO_TEST_CASE(poisson_series_sin_cos_test) {
  ...
  BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(math::sin(-p1)),
                    "-sin(x)");
  ...
  BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(math::sin(-p1)),
                    "-sin(x-2*y)");
  BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(math::cos(-p1)),
                    "cos(x-2*y)");
  BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(3 * p1.sin()),
                    "3*sin(x-2*y)");
  ...
  BOOST_CHECK_THROW(math::sin(p_type1{"x"} * rational(1, 2)),
                    std::invalid_argument);
  BOOST_CHECK_THROW(math::cos(p_type1{"x"} * rational(1, 2)),
                    std::invalid_argument);
  BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(
                        math::sin(p_type1{"x"} * rational(4, -2))),
                    "-sin(2*x)");
  BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(
                        -math::cos(p_type1{"x"} * rational(4, 2))),
                    "-cos(2*x)");
  ...
  BOOST_CHECK_THROW(math::sin(p_type2{"x"} * real(rational(1, 2))),
                    std::invalid_argument);
  BOOST_CHECK_THROW(math::cos(p_type2{"x"} * real(rational(1, 2))),
                    std::invalid_argument);
  ...
  // Check with eps.
  using p_type5 = poisson_series<
      divisor_series<polynomial<rational, monomial<short>>, divisor<short>>>;
  ...
  BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(math::cos(p_type5{"x"})),
                    "cos(x)");
  BOOST_CHECK_EQUAL(
      boost::lexical_cast<std::string>(math::cos(p_type5{"x"} + p_type5{"y"})),
      "cos(x+y)");
  BOOST_CHECK_EQUAL(
      boost::lexical_cast<std::string>(math::cos(-p_type5{"x"} + p_type5{"y"})),
      "cos(x-y)");
  BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(math::sin(p_type5{"x"})),
                    "sin(x)");
  BOOST_CHECK_EQUAL(
      boost::lexical_cast<std::string>(math::sin(p_type5{"x"} + p_type5{"y"})),
      "sin(x+y)");
  BOOST_CHECK_EQUAL(
      boost::lexical_cast<std::string>(math::sin(-p_type5{"x"} + p_type5{"y"})),
      "-sin(x-y)");
  ...
  using p_type6 = poisson_series<
      divisor_series<polynomial<double, monomial<short>>, divisor<short>>>;
  BOOST_CHECK_EQUAL(math::cos(p_type6{1.23}), std::cos(1.23));
  BOOST_CHECK_EQUAL(math::sin(p_type6{-4.56}), std::sin(-4.56));
  // Double divisor.
  using p_type7 = poisson_series<divisor_series<
      divisor_series<polynomial<rational, monomial<short>>, divisor<short>>,
      divisor<short>>>;
  ...
  BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(math::cos(p_type7{"x"})),
                    "cos(x)");
  BOOST_CHECK_EQUAL(
      boost::lexical_cast<std::string>(math::cos(p_type7{"x"} + p_type7{"y"})),
      "cos(x+y)");
  BOOST_CHECK_EQUAL(
      boost::lexical_cast<std::string>(math::cos(-p_type7{"x"} + p_type7{"y"})),
      "cos(x-y)");
  BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(math::sin(p_type7{"x"})),
                    "sin(x)");
  BOOST_CHECK_EQUAL(
      boost::lexical_cast<std::string>(math::sin(p_type7{"x"} + p_type7{"y"})),
      "sin(x+y)");
  BOOST_CHECK_EQUAL(
      boost::lexical_cast<std::string>(math::sin(-p_type7{"x"} + p_type7{"y"})),
      "-sin(x-y)");
...
BOOST_AUTO_TEST_CASE(poisson_series_arithmetic_test) {
  ...
  typedef poisson_series<polynomial<rational, monomial<short>>> p_type1;
  p_type1 x{"x"}, y{"y"};
  BOOST_CHECK_EQUAL(math::cos(x) * math::cos(y),
                    (math::cos(x - y) + math::cos(x + y)) / 2);
  BOOST_CHECK_EQUAL(math::cos(-x) * math::cos(y),
                    (math::cos(x - y) + math::cos(x + y)) / 2);
  BOOST_CHECK_EQUAL(math::cos(x) * math::cos(-y),
                    (math::cos(x - y) + math::cos(x + y)) / 2);
  BOOST_CHECK_EQUAL(math::cos(-x) * math::cos(-y),
                    (math::cos(x - y) + math::cos(x + y)) / 2);
  BOOST_CHECK_EQUAL(math::sin(x) * math::sin(y),
                    (math::cos(x - y) - math::cos(x + y)) / 2);
  BOOST_CHECK_EQUAL(math::sin(-x) * math::sin(y),
                    -(math::cos(x - y) - math::cos(x + y)) / 2);
  BOOST_CHECK_EQUAL(math::sin(x) * math::sin(-y),
  ...
  BOOST_CHECK_EQUAL(math::sin(-x) * math::sin(-y),
                    (math::cos(x - y) - math::cos(x + y)) / 2);
  BOOST_CHECK_EQUAL(math::sin(x) * math::cos(y),
                    (math::sin(x + y) + math::sin(x - y)) / 2);
  BOOST_CHECK_EQUAL(math::sin(-x) * math::cos(y),
                    -(math::sin(x + y) + math::sin(x - y)) / 2);
  BOOST_CHECK_EQUAL(math::sin(x) * math::cos(-y),
                    (math::sin(x + y) + math::sin(x - y)) / 2);
  BOOST_CHECK_EQUAL(math::sin(-x) * math::cos(-y),
                    -(math::sin(x + y) + math::sin(x - y)) / 2);
  BOOST_CHECK_EQUAL(math::cos(x) * math::sin(y),
                    (math::sin(x + y) - math::sin(x - y)) / 2);
  BOOST_CHECK_EQUAL(math::cos(-x) * math::sin(y),
                    (math::sin(x + y) - math::sin(x - y)) / 2);
  BOOST_CHECK_EQUAL(math::cos(x) * math::sin(-y),
  ...
  using math::cos;
  using math::pow;
  BOOST_CHECK_EQUAL(pow(sin(x), 5),
                    (10 * sin(x) - 5 * sin(3 * x) + sin(5 * x)) / 16);
  BOOST_CHECK_EQUAL(pow(cos(x), 5),
                    (10 * cos(x) + 5 * cos(3 * x) + cos(5 * x)) / 16);
  BOOST_CHECK_EQUAL(pow(cos(x), 5) * pow(sin(x), 5),
  ...
  typedef poisson_series<polynomial<real, monomial<short>>> p_type2;
  BOOST_CHECK_EQUAL(pow(p_type2(real("1.234")), real("-5.678")),
                    pow(real("1.234"), real("-5.678")));
...
BOOST_AUTO_TEST_CASE(poisson_series_degree_test) {
  ...
  using math::pow;
  {
    ...
    BOOST_CHECK(math::ldegree(p_type1{"x"} * p_type1{"y"} + p_type1{"x"},
                              {"x", "y"}) == 1);
    BOOST_CHECK(
        math::ldegree(p_type1{"x"} * p_type1{"y"} + p_type1{"x"}, {"x"}) == 1);
    BOOST_CHECK(
        math::ldegree(p_type1{"x"} * p_type1{"y"} + p_type1{"x"}, {"y"}) == 0);
    p_type1 x{"x"}, y{"y"};
  ...
  // Try also with eps.
  {
    ...
    BOOST_CHECK_EQUAL(ldegree((invert(x) * x + y * x * z) * cos(x) + cos(y)),
                      0);
  }
}
...
BOOST_AUTO_TEST_CASE(poisson_series_partial_test) {
  ...
  BOOST_CHECK_EQUAL(partial(x * cos(2 * x), "x"),
                    cos(2 * x) - 2 * x * sin(2 * x));
  BOOST_CHECK_EQUAL(partial(x * cos(2 * x + y), "y"), -x * sin(2 * x + y));
  BOOST_CHECK_EQUAL(partial(rational(3, 2) * cos(2 * x + y), "x"),
                    -3 * sin(2 * x + y));
  BOOST_CHECK_EQUAL(partial(rational(3, 2) * x * cos(y), "y"),
                    -rational(3, 2) * x * sin(+y));
  BOOST_CHECK_EQUAL(partial(pow(x * cos(y), 5), "y"),
                    5 * sin(-y) * x * pow(x * cos(y), 4));
  ...
  BOOST_CHECK_EQUAL(partial(x + x * cos(y), "x"),
                    1 + cos(y) + x * sin(y) * sin(x));
  BOOST_CHECK((!is_differentiable<
               poisson_series<polynomial<mock_cf, monomial<short>>>>::value));
  BOOST_CHECK((!has_pbracket<
               poisson_series<polynomial<mock_cf, monomial<short>>>>::value));
  BOOST_CHECK((!has_transformation_is_canonical<
               poisson_series<polynomial<mock_cf, monomial<short>>>>::value));
}
...
BOOST_AUTO_TEST_CASE(poisson_series_transform_filter_test) {
  ...
  typedef std::decay<decltype(*(p_type1{}.begin()->first.begin()))>::type
      pair_type2;
  p_type1 x{"x"}, y{"y"};
  auto s = pow(1 + x + y, 3) * cos(x) + pow(y, 3) * sin(x);
  auto s_t = s.transform([](const pair_type &p) {
    return std::make_pair(p.first.filter([](const pair_type2 &p2) {
      return math::degree(p2.second) < 2;
    }),
                          p.second);
  });
...
BOOST_AUTO_TEST_CASE(poisson_series_evaluate_test) {
  ...
  using math::pow;
  typedef poisson_series<polynomial<rational, monomial<short>>> p_type1;
  std::unordered_map<std::string, real> dict{{"x", real(1.234)},
                                             {"y", real(5.678)}};
  p_type1 x{"x"}, y{"y"};
  auto s1 = (x + y) * cos(x + y);
  ...
  // NOTE: here it seems to be quite a brittle test: if one changes the order of
  // the operands s1 and s2,
  // the test fails on my test machine due to differences of order epsilon. Most
  // likely it's a matter
  // of ordering of the floating-point operations and it will depend on a ton of
  // factors. Better just disable it,
...
BOOST_AUTO_TEST_CASE(poisson_series_subs_test) {
  ...
  using math::subs;
  {
    ...
    BOOST_CHECK_EQUAL(s.subs("x", real(1.234)),
                      (real(1.234) + y) * cos(real(1.234)) +
                          pow(y, 3) * sin(real(1.234)));
    BOOST_CHECK(
        (std::is_same<decltype(s.subs("x", real(1.234))), p_type1>::value));
    BOOST_CHECK(
        (std::is_same<decltype(s.subs("x", rational(1.234))), p_type1>::value));
    s = (x + y) * cos(x + y) + pow(y, 3) * sin(x + y);
    real r(1.234);
    BOOST_CHECK_EQUAL(s.subs("x", r),
                      (r + y) * (cos(r) * cos(y) - sin(r) * sin(y)) +
                          pow(y, 3) * (sin(r) * cos(y) + cos(r) * sin(y)));
    BOOST_CHECK_EQUAL(subs(s, "x", r),
                      (r + y) * (cos(r) * cos(y) - sin(r) * sin(y)) +
                          pow(y, 3) * (sin(r) * cos(y) + cos(r) * sin(y)));
    ...
    BOOST_CHECK_EQUAL(subs(t, "a", a + b),
                      (a + b) * cos(a + b + b) + b * sin(a + b));
    t = a * cos(-3 * a + b) + b * sin(-5 * a - b);
    BOOST_CHECK_EQUAL(subs(t, "a", a + b), (a + b) * cos(-3 * (a + b) + b) +
                                               b * sin(-5 * (a + b) - b));
    BOOST_CHECK_EQUAL(subs(t, "a", 2 * (a + b)),
                      2 * (a + b) * cos(-6 * (a + b) + b) +
                          b * sin(-10 * (a + b) - b));
    ...
    p_type2 c{"c"}, d{"d"};
    BOOST_CHECK_EQUAL(math::subs(a + math::cos(b) - math::cos(b), "b", c + d),
                      a);
    // This was a manifestation of the bug in the rational ctor from float.
    BOOST_CHECK_EQUAL(math::subs(-3 * math::pow(c, 4), "J_2", 0_z),
                      -3 * math::pow(c, 4));
    // Test substitution with integral after math::sin/cos additional overload.
    BOOST_CHECK_EQUAL(math::subs(-3 * math::pow(c, 4), "J_2", 0),
                      -3 * math::pow(c, 4));
  }
  {
    // Test with eps.
    ...
    BOOST_CHECK_EQUAL(math::subs(x * math::pow(z, -1), "z", x * y),
                      x * math::pow(x * y, -1));
    BOOST_CHECK_EQUAL(math::subs(x * math::cos(z + y), "z", x - 2 * y),
                      x * math::cos(x - y));
    BOOST_CHECK_EQUAL(math::subs(x * math::cos(x + y), "x", 2 * x),
                      2 * x * math::cos(2 * x + y));
    BOOST_CHECK_EQUAL(math::subs(x * math::cos(x + y), "y", 2 * x),
                      x * math::cos(x + 2 * x));
    // No subs on divisors implemented (yet?).
    BOOST_CHECK_EQUAL(
        math::subs(x * math::cos(x + y) * math::invert(x), "x", 2 * x),
...
BOOST_AUTO_TEST_CASE(poisson_series_print_tex_test) {
  ...
  oss.str("");
  ((3 * x * y.pow(-1)) * cos(x + y) - (2 * x.pow(2) * y.pow(-2)) * cos(-3 * x))
      .print_tex(oss);
  BOOST_CHECK(oss.str() == s1 + "-" + s2 ||
              oss.str() == std::string("-") + s2 + "+" + s1);
  std::string s3 = "\\left({x}+{y}\\right)";
  std::string s4 = "\\left({y}+{x}\\right)";
  oss.str("");
  ((x + y) * cos(x)).print_tex(oss);
  BOOST_CHECK(oss.str() == s3 + "\\cos{\\left({x}\\right)}" ||
              oss.str() == s4 + "\\cos{\\left({x}\\right)}");
}
...
BOOST_AUTO_TEST_CASE(poisson_series_integrate_test) {
  ...
  BOOST_CHECK_EQUAL(math::integrate((x + y) * cos(x) + cos(y), "x"),
                    (x + y) * sin(x) + x * cos(y) + cos(x));
  BOOST_CHECK_EQUAL(math::integrate((x + y) * cos(x) + cos(y), "y"),
                    y / 2 * (2 * x + y) * cos(x) + sin(y));
  BOOST_CHECK_EQUAL(math::integrate((x + y) * cos(x) + cos(x), "x"),
                    (x + y + 1) * sin(x) + cos(x));
  BOOST_CHECK_THROW(math::integrate(x.pow(-1) * cos(x), "x"),
                    std::invalid_argument);
  BOOST_CHECK_THROW(math::integrate(x.pow(-2) * cos(x + y) + x, "x"),
                    std::invalid_argument);
  // Some examples computed with Wolfram alpha for checking.
  BOOST_CHECK_EQUAL(math::integrate(x.pow(-2) * cos(x + y) + x, "y"),
                    sin(x + y) * x.pow(-2) + x * y);
  BOOST_CHECK_EQUAL(
      math::integrate(
          x.pow(5) * y.pow(4) * z.pow(3) * cos(5 * x + 4 * y + 3 * z), "x"),
      y.pow(4) * z.pow(3) / 3125 *
          (5 * x * (125 * x.pow(4) - 100 * x * x + 24) *
               sin(5 * x + 4 * y + 3 * z) +
           (625 * x.pow(4) - 300 * x * x + 24) * cos(5 * x + 4 * y + 3 * z)));
  BOOST_CHECK_EQUAL(
      math::integrate(x.pow(5) / 37 * y.pow(4) * z.pow(3) *
                          cos(5 * x - 4 * y + 3 * z),
                      "y"),
  ...
  BOOST_CHECK_EQUAL(
      math::partial(math::integrate(x.pow(5) / 37 * y.pow(4) * z.pow(3) *
                                        cos(5 * x - 4 * y + 3 * z),
                                    "y"),
                    "y"),
      x.pow(5) / 37 * y.pow(4) * z.pow(3) * cos(5 * x - 4 * y + 3 * z));
  BOOST_CHECK_EQUAL(
      math::partial(math::partial(math::integrate(
                                      math::integrate(
                                          x.pow(5) / 37 * y.pow(4) * z.pow(3) *
                                              cos(5 * x - 4 * y + 3 * z),
                                          "y"),
                                      "y"),
                                  "y"),
                    "y"),
      x.pow(5) / 37 * y.pow(4) * z.pow(3) * cos(5 * x - 4 * y + 3 * z));
  BOOST_CHECK_EQUAL(math::integrate(rational(1, 37) * y.pow(4) * z.pow(3) *
                                        cos(5 * x - 4 * y + 3 * z),
                                    "x"),
                    rational(1, 185) * y.pow(4) * z.pow(3) *
                        sin(5 * x - 4 * y + 3 * z));
  BOOST_CHECK_EQUAL(
      math::integrate(
          rational(1, 37) * x.pow(4) * z.pow(3) * cos(4 * y - 3 * z), "x"),
      rational(1, 185) * x.pow(5) * z.pow(3) * cos(4 * y - 3 * z));
  BOOST_CHECK_EQUAL(
      math::integrate(
          y.pow(-5) * cos(4 * x - 3 * z) - x * y * y * sin(y).pow(4), "x"),
      (sin(4 * x - 3 * z) - 2 * x * x * y.pow(7) * sin(y).pow(4)) *
          (4 * y.pow(5)).pow(-1));
  BOOST_CHECK_EQUAL((x * x * cos(x)).integrate("x"),
                    (x * x - 2) * sin(x) + 2 * x * cos(x));
  BOOST_CHECK_EQUAL(((x * x + y) * cos(x) - y * cos(x)).integrate("x"),
                    (x * x - 2) * sin(x) + 2 * x * cos(x));
  BOOST_CHECK_EQUAL(
      ((x * x + y) * cos(x) + y * cos(x) - x * sin(y)).integrate("x"),
      -(x * x) / 2 * sin(y) + (x * x + 2 * y - 2) * sin(x) + 2 * x * cos(x));
  BOOST_CHECK_EQUAL(
      ((x * x * x + y * x) * cos(2 * x - 3 * y) + y * x.pow(4) * cos(x) -
       (x.pow(-5) * sin(y)))
          .integrate("x"),
      x.pow(-4) / 8 *
          (32 * (x * x - 6) * x.pow(5) * y * cos(x) +
           x.pow(4) * (6 * x * x + 2 * y - 3) * cos(2 * x - 3 * y) +
           2 * (x.pow(5) * (2 * x * x + 2 * y - 3) * sin(2 * x - 3 * y) +
                4 * (x.pow(4) - 12 * x * x + 24) * x.pow(4) * y * sin(x) +
                sin(y))));
  BOOST_CHECK_EQUAL(
      math::integrate((x.pow(-1) * cos(y) + x * y * cos(x)).pow(2), "x"),
      x.pow(-1) / 24 *
          (4 * x.pow(4) * y * y + 6 * x.pow(3) * y * y * sin(2 * x) +
           6 * x * x * y * y * cos(2 * x) - 3 * x * y * y * sin(2 * x) +
           24 * x * y * sin(x - y) + 24 * x * y * sin(x + y) - 12 * cos(2 * y) -
           12));
  BOOST_CHECK_EQUAL(
      math::integrate((cos(y) * x.pow(-2) + x * x * y * cos(x)).pow(2), "x"),
  ...
  BOOST_CHECK_EQUAL(math::integrate((x * cos(y) + y * cos(x)).pow(2), "x"),
                    rational(1, 6) * x *
                            (x * x * cos(2 * y) + x * x + 3 * y * y) +
                        rational(1, 4) * y * y * sin(2 * x) +
                        2 * y * cos(x) * cos(y) + 2 * x * y * sin(x) * cos(y));
  BOOST_CHECK_EQUAL(math::integrate((x * y * cos(y) + y * cos(x)).pow(2), "x"),
                    rational(1, 12) * y * y *
                        (2 * x * (x * x * cos(2 * y) + x * x + 3) +
                         24 * cos(x) * cos(y) + 24 * x * sin(x) * cos(y) +
                         3 * sin(2 * x)));
  BOOST_CHECK_EQUAL(
      math::integrate((x * y * cos(y) + y * cos(x) + x * x * cos(x)).pow(2),
                      "x"),
      rational(1, 60) *
          (15 * x * (2 * x * x + 2 * y - 3) * cos(x).pow(2) +
           x * (6 * x.pow(4) + 5 * x * x * y * y +
                10 * x * x * y * y * cos(y).pow(2) +
                5 * x * x * y * y * cos(2 * y) + 20 * x * x * y -
                15 * (2 * x * x + 2 * y - 3) * sin(x).pow(2) +
                120 * y * (x * x + y - 6) * sin(x) * cos(y) + 30 * y * y) +
           15 * cos(x) *
               (8 * y * (3 * x * x + y - 6) * cos(y) +
                (2 * x.pow(4) + x * x * (4 * y - 6) + 2 * y * y - 2 * y + 3) *
                    sin(x))));
  // This would require sine/cosine integral special functions.
  BOOST_CHECK_THROW(
      math::integrate(
          (x * y.pow(-1) * cos(y) + y * cos(x) + x * x * cos(x)).pow(2), "y"),
      std::invalid_argument);
  // Check type trait.
  ...
  BOOST_CHECK((
      std::is_same<decltype(math::integrate(p_type3{}, "x")), p_type3>::value));
  BOOST_CHECK((
      std::is_same<decltype(math::integrate(p_type4{}, "x")), p_type3>::value));
  BOOST_CHECK((std::is_same<
               decltype(math::integrate(p_type5{}, "x")),
               poisson_series<polynomial<integer, monomial<integer>>>>::value));
  BOOST_CHECK_EQUAL(math::integrate(p_type4{"x"}.pow(3 / 4_q), "x"),
                    4 / 7_q * p_type3{"x"}.pow(7 / 4_q));
  BOOST_CHECK_EQUAL(
      math::integrate(p_type3{"x"}.pow(8 / 4_q) * math::cos(p_type3{"x"}), "x"),
      (p_type3{"x"} * p_type3{"x"} - 2) * math::sin(p_type3{"x"}) +
          2 * p_type3{"x"} * math::cos(p_type3{"x"}));
  BOOST_CHECK_THROW(
      math::integrate(p_type3{"x"}.pow(3 / 4_q) * math::cos(p_type3{"x"}), "x"),
      std::invalid_argument);
  // Check about eps.
  using p_type6 = poisson_series<
      divisor_series<polynomial<rational, monomial<short>>, divisor<short>>>;
  ...
  using math::invert;
  BOOST_CHECK(
      (std::is_same<p_type6, decltype(math::integrate(a, "a"))>::value));
  ...
  BOOST_CHECK_EQUAL(math::integrate(invert(b) + a, "a"),
                    a * a / 2 + a * invert(b));
  BOOST_CHECK_EQUAL(math::integrate(math::cos(b) * a, "a"),
                    a * a / 2 * math::cos(b));
  BOOST_CHECK_EQUAL(math::integrate(math::cos(b) * a, "b"), a * math::sin(b));
  BOOST_CHECK_EQUAL(math::integrate(math::cos(b) * a, "b"), a * math::sin(b));
  BOOST_CHECK_EQUAL(math::integrate(math::cos(b) * a * invert(c), "b"),
                    a * math::sin(b) * invert(c));
  BOOST_CHECK_EQUAL(math::integrate(math::cos(b) * a * invert(c), "a"),
                    math::cos(b) * a * a / 2 * invert(c));
  // This will fail because we do not know how to integrate with respect to
  // divisors.
  BOOST_CHECK_THROW(math::integrate(math::cos(b) * a * invert(c), "c"),
                    std::invalid_argument);
  // This will fail because, at the moment, eps cannot deal with mixed poly/trig
  // variables (though
  // normal Poisson series can, this is something we need to fix in the future).
  BOOST_CHECK_THROW(math::integrate(math::cos(a) * a * invert(c), "a"),
                    std::invalid_argument);
  BOOST_CHECK_EQUAL(
      math::integrate(math::cos(b - a + a) * (a - c + c) * invert(c - b + b),
                      "b"),
      a * math::sin(b) * invert(c));
  BOOST_CHECK_EQUAL(
      math::integrate(math::cos(b + c - c) * (a + b - b) * invert(c - a + a),
                      "a"),
      math::cos(b) * a * a / 2 * invert(c));
}

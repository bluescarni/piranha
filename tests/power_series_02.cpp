...
BOOST_AUTO_TEST_CASE(power_series_test_02) {
  ...
  BOOST_CHECK((std::is_same<decltype(math::degree(std::declval<stype0>())),
                            rational>::value));
  BOOST_CHECK((std::is_same<decltype(math::ldegree(std::declval<stype0>())),
                            rational>::value));
  BOOST_CHECK((std::is_same<decltype(math::degree(std::declval<stype0>(),
  ...
  BOOST_CHECK((std::is_same<decltype(math::degree(std::declval<stype1>())),
                            int>::value));
  BOOST_CHECK((std::is_same<decltype(math::ldegree(std::declval<stype1>())),
                            int>::value));
  BOOST_CHECK((std::is_same<decltype(math::degree(std::declval<stype1>(),
                                                  std::vector<std::string>{})),
                            int>::value));
  ...
  BOOST_CHECK((std::is_same<decltype(math::degree(std::declval<stype2>())),
                            long>::value));
  BOOST_CHECK((std::is_same<decltype(math::ldegree(std::declval<stype2>())),
                            long>::value));
  BOOST_CHECK((std::is_same<decltype(math::degree(std::declval<stype2>(),
                                                  std::vector<std::string>{})),
                            long>::value));
  ...
  typedef g_series_type2<g_series_type<g_series_type<double, int>, integer>>
      stype4;
  BOOST_CHECK((has_degree<stype4>::value));
  BOOST_CHECK((has_ldegree<stype4>::value));
  BOOST_CHECK((std::is_same<decltype(math::degree(std::declval<stype4>())),
                            integer>::value));
  BOOST_CHECK((std::is_same<decltype(math::ldegree(std::declval<stype4>())),
                            integer>::value));
  BOOST_CHECK((std::is_same<decltype(math::degree(std::declval<stype4>(),
                                                  std::vector<std::string>{})),
                            integer>::value));
  ...
  BOOST_CHECK((std::is_same<decltype(math::degree(std::declval<stype5>())),
                            fake_int>::value));
  BOOST_CHECK((std::is_same<decltype(math::ldegree(std::declval<stype5>())),
                            fake_int>::value));
  BOOST_CHECK((std::is_same<decltype(math::degree(std::declval<stype5>(),
                                                  std::vector<std::string>{})),
                            fake_int>::value));
  ...
  // This does not have a degree type because fake_int cannot be added to
  // integer.
...
BOOST_AUTO_TEST_CASE(power_series_serialization_test) {
  typedef g_series_type<polynomial<rational, monomial<rational>>, rational>
      stype;
...
BOOST_AUTO_TEST_CASE(power_series_truncation_test) {
  // A test with polynomial, degree only in the key.
  {
    ...
    BOOST_CHECK_EQUAL(s0.truncate_degree(3),
                      z.pow(-5) / 2 * x * y + x * y * z / 4);
    BOOST_CHECK_EQUAL(math::truncate_degree(s0, -1), z.pow(-5) / 2 * x * y);
    BOOST_CHECK_EQUAL(math::truncate_degree(s0, 2, {"x"}),
                      z.pow(-5) / 2 * x * y + x * y * z / 4);
    BOOST_CHECK_EQUAL(math::truncate_degree(s0, 5, {"x", "y"}),
                      z.pow(-5) / 2 * x * y + x * y * z / 4);
    BOOST_CHECK_EQUAL(math::truncate_degree(s0, 5, {"y", "x", "y"}),
                      z.pow(-5) / 2 * x * y + x * y * z / 4);
  ...
  }
  {
    ...
    st1 z{"z"};
    auto s0 =
        (x * y + x * x + x + 1_q / 4) * z + (x + y * y + x * x * y) * z * z + 3;
    ...
    BOOST_CHECK_EQUAL(math::truncate_degree(s0, 3_q),
                      (x * y + x * x + x + 1_q / 4) * z + x * z * z + 3);
    BOOST_CHECK_EQUAL(math::truncate_degree(s0, 1, {"x"}),
                      (x * y + x + 1_q / 4) * z + (x + y * y) * z * z + 3);
    BOOST_CHECK_EQUAL(math::truncate_degree(s0, 1ll, {"x", "y"}),
                      (x + 1_q / 4) * z + x * z * z + 3);
    BOOST_CHECK_EQUAL(math::truncate_degree(s0, 1, {"x", "z"}),
                      1_q / 4 * z + 3);
    BOOST_CHECK_EQUAL(math::truncate_degree(s0, 2, {"x", "z"}),
                      (x * y + x + 1_q / 4) * z + y * y * z * z + 3);
    BOOST_CHECK_EQUAL(math::truncate_degree(s0, 3, {"x", "z"}),
                      (x * y + x * x + x + 1_q / 4) * z + (x + y * y) * z * z +
                          3);
    // Test with non-existing variable.
    BOOST_CHECK_EQUAL(math::truncate_degree(s0, 0, {"foo", "bar"}), s0);
  }
  {
    // Recursive poly, integers and rational exponent mixed, same example as
    // above.
...
BOOST_AUTO_TEST_CASE(power_series_degree_overflow_test) {
  ...
  p_type x{"x"};
  pp_type y{"y"};
  BOOST_CHECK_THROW((x * y.pow(std::numeric_limits<int>::max())).degree(),
                    std::overflow_error);
  BOOST_CHECK_THROW(
      (x.pow(-1) * y.pow(std::numeric_limits<int>::min())).degree(),
      std::overflow_error);
  BOOST_CHECK_EQUAL((x * y.pow(std::numeric_limits<int>::min())).degree(),
...
BOOST_AUTO_TEST_CASE(power_series_mixed_degree_test) {
  ...
  using pp_type4 =
      polynomial<polynomial<rational, monomial<rational>>, monomial<long long>>;
  p_type x{"x"};
  pp_type y{"y"};
  pp_type2 z{"z"};
  pp_type3 a{"a"};

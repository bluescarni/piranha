...
BOOST_AUTO_TEST_CASE(print_tex_coefficient_main_test) {
  ...
  BOOST_CHECK_EQUAL(oss.str(),
                    boost::lexical_cast<std::string>(real("-1.2345")));
  BOOST_CHECK((std::is_same<decltype(print_tex_coefficient(oss, 42)),
                            std::ostream &>::value));
  BOOST_CHECK((std::is_same<decltype(print_tex_coefficient(oss, integer(-5))),
                            std::ostream &>::value));
}

...
BOOST_AUTO_TEST_CASE(cache_aligning_allocator_constructor_test) {
  ...
  BOOST_CHECK(cache_aligning_allocator<
              int>::propagate_on_container_move_assignment::value);
...
BOOST_AUTO_TEST_CASE(cache_aligning_allocator_construct_destroy_test) {
  ...
  caa2.destroy(static_cast<std::string *>(static_cast<void *>(&st1)));
  BOOST_CHECK(cache_aligning_allocator<std::string>::rebind<char>::other{} ==
              caa1);
  BOOST_CHECK(
      has_typedef_pointer<cache_aligning_allocator<std::string>>::value);
  BOOST_CHECK(
      has_typedef_const_pointer<cache_aligning_allocator<std::string>>::value);
  BOOST_CHECK(
      has_typedef_reference<cache_aligning_allocator<std::string>>::value);
  BOOST_CHECK(has_typedef_const_reference<
              cache_aligning_allocator<std::string>>::value);
}

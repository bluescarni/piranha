...
template <typename Cf, typename Expo>
class series_multiplier<polynomial_alt<Cf, Expo>, void>
    : public base_series_multiplier<polynomial_alt<Cf, Expo>> {
...
struct multiplication_tester {
  template <typename Cf, typename std::enable_if<
                             detail::is_mp_rational<Cf>::value, int>::type = 0>
...
BOOST_AUTO_TEST_CASE(polynomial_subs_test) {
  {
    ...
    p_type1 x{"x"}, y{"y"}, z{"z"};
    BOOST_CHECK_EQUAL((x.pow(2) + x * y + z).subs("x", integer(3)),
                      9 + 3 * y + z);
    BOOST_CHECK_EQUAL((x.pow(2) + x * y + z).subs("y", rational(3, 2)),
                      x * x + x * rational(3, 2) + z);
    BOOST_CHECK_EQUAL((x.pow(2) + x * y + z).subs("k", rational(3, 2)),
                      x * x + x * y + z);
    BOOST_CHECK_EQUAL(x.pow(-1).subs("x", x.pow(-1)), x);
    BOOST_CHECK_EQUAL((x.pow(2) + x * y + z)
                          .subs("x", rational(3, 2))
                          .subs("y", rational(4, 5))
                          .subs("z", -rational(6, 7)),
                      (x.pow(2) + x * y + z)
                          .evaluate(std::unordered_map<std::string, rational>{
                              ...
                              {"y", rational(4, 5)},
                              {"z", -rational(6, 7)}}));
    BOOST_CHECK_EQUAL(math::subs(x.pow(2) + x * y + z, "x", rational(3, 2))
                          .subs("y", rational(4, 5))
                          .subs("z", -rational(6, 7)),
                      (x.pow(2) + x * y + z)
                          .evaluate(std::unordered_map<std::string, rational>{
                              ...
                              {"y", rational(4, 5)},
                              {"z", -rational(6, 7)}}));
    BOOST_CHECK((std::is_same<decltype(p_type1{"x"}.subs("x", integer(1))),
                              p_type1>::value));
    BOOST_CHECK((std::is_same<decltype(p_type1{"x"}.subs("x", rational(1))),
                              p_type1>::value));
    BOOST_CHECK_EQUAL((x.pow(2) + x * y + z).subs("k", rational(3, 2)),
                      x * x + x * y + z);
    BOOST_CHECK_EQUAL(((y + 4 * z).pow(5) * x.pow(-1)).subs("x", rational(3)),
                      ((y + 4 * z).pow(5)) / 3);
  }
  {
    ...
    p_type2 x{"x"}, y{"y"};
    BOOST_CHECK_EQUAL((x * x * x + y * y).subs("x", real(1.234)),
                      y * y + math::pow(real(1.234), 3));
    BOOST_CHECK_EQUAL(
        (x * x * x + y * y).subs("x", real(1.234)).subs("y", real(-5.678)),
        math::pow(real(-5.678), 2) + math::pow(real(1.234), 3));
    BOOST_CHECK_EQUAL(
        math::subs(x * x * x + y * y, "x", real(1.234)).subs("y", real(-5.678)),
        math::pow(real(-5.678), 2) + math::pow(real(1.234), 3));
  }
  ...
  BOOST_CHECK_EQUAL((x * x * x + y * y + z * y * x)
                        ...
                        .subs("z", integer(4))
                        .subs("k", integer()),
                    integer(2).pow(3) + integer(-3).pow(2) +
...
BOOST_AUTO_TEST_CASE(polynomial_integrate_test) {
  ...
  BOOST_CHECK_EQUAL(
      (x + 3 * y * x * x + z * y * x / 4).integrate("x").partial("x"),
      x + 3 * y * x * x + z * y * x / 4);
  BOOST_CHECK_EQUAL(
      (x + 3 * y * x * x + z * y * x / 4).integrate("y").partial("y"),
      x + 3 * y * x * x + z * y * x / 4);
  BOOST_CHECK_EQUAL(
      (x + 3 * y * x * x + z * y * x / 4).integrate("z").partial("z"),
      x + 3 * y * x * x + z * y * x / 4);
  BOOST_CHECK_EQUAL(p_type1{4}.integrate("z"), 4 * z);
  BOOST_CHECK_EQUAL((x * y * z).pow(-5).integrate("x"),
                    (y * z).pow(-5) * x.pow(-4) * rational(1, -4));
  ...
  BOOST_CHECK_THROW((a * (x).pow(-1) + b * x / 15 - a * y).integrate("x"),
  ...
  BOOST_CHECK_EQUAL((x * xx).integrate("x"),
                    x * x * xx / 2 - math::integrate(x * x / 2, "x"));
  BOOST_CHECK_EQUAL(((3 * x + y) * xx).integrate("x"),
                    (3 * x * x + 2 * x * y) * xx / 2 -
                        math::integrate((3 * x * x + 2 * x * y) / 2, "x"));
  BOOST_CHECK_EQUAL((x * xx * xx).integrate("x"), x * x * xx * xx / 2 -
                                                      2 * xx * x * x * x / 6 +
                                                      2 * x * x * x * x / 24);
  BOOST_CHECK_EQUAL(math::partial((x * xx * xx).integrate("x"), "x"),
                    x * xx * xx);
  BOOST_CHECK_THROW((x.pow(-1) * xx * xx).integrate("x"),
                    std::invalid_argument);
  BOOST_CHECK_THROW((x.pow(-2) * xx * xx).integrate("x"),
                    std::invalid_argument);
  BOOST_CHECK_THROW((x.pow(-3) * xx * xx).integrate("x"),
                    std::invalid_argument);
  BOOST_CHECK_EQUAL((x.pow(-4) * xx * xx).integrate("x"),
                    -x.pow(-3) / 3 * xx * xx - x.pow(-2) * 2 * xx / 6 -
                        2 * x.pow(-1) / 6);
  BOOST_CHECK_EQUAL((x.pow(-4) * xx).integrate("x"),
                    -x.pow(-3) / 3 * xx - x.pow(-2) / 6);
  BOOST_CHECK_EQUAL((y * x.pow(-4) * xx * xx).integrate("x"),
                    y * (-x.pow(-3) / 3 * xx * xx - x.pow(-2) * 2 * xx / 6 -
                         2 * x.pow(-1) / 6));
  BOOST_CHECK_EQUAL(((y + z.pow(2) * y) * x.pow(-4) * xx * xx).integrate("x"),
                    (y + z.pow(2) * y) *
                        (-x.pow(-3) / 3 * xx * xx - x.pow(-2) * 2 * xx / 6 -
                         2 * x.pow(-1) / 6));
  BOOST_CHECK_EQUAL(((y + z.pow(2) * y) * x.pow(-4) * xx * xx - x.pow(-4) * xx)
                        .integrate("x"),
                    (y + z.pow(2) * y) *
                            (-x.pow(-3) / 3 * xx * xx - x.pow(-2) * 2 * xx / 6 -
                             2 * x.pow(-1) / 6) -
                        (-x.pow(-3) / 3 * xx - x.pow(-2) / 6));
  // Misc tests.
  BOOST_CHECK_EQUAL(math::partial((x + y + z).pow(10).integrate("x"), "x"),
                    (x + y + z).pow(10));
  BOOST_CHECK_EQUAL(math::partial((x + y + z).pow(10).integrate("y"), "y"),
                    (x + y + z).pow(10));
  BOOST_CHECK_EQUAL(math::partial((x + y + z).pow(10).integrate("z"), "z"),
                    (x + y + z).pow(10));
  ...
  BOOST_CHECK_THROW(
      math::integrate(p_type_alt{polynomial_alt<rational, int>{"m"}}, "m"),
      std::invalid_argument);
  BOOST_CHECK_EQUAL(
      math::integrate(p_type_alt{polynomial_alt<rational, int>{"n"}}, "m"),
      (polynomial_alt<rational, int>{"n"} * m));
  BOOST_CHECK_EQUAL(
      math::integrate(p_type_alt{polynomial_alt<rational, int>{"m"}}, "n"),
  ...
  BOOST_CHECK((std::is_same<decltype(p_type2{}.integrate("x")),
                            polynomial<rational, monomial<rational>>>::value));
  BOOST_CHECK((std::is_same<decltype(p_type3{}.integrate("x")),
                            polynomial<rational, monomial<rational>>>::value));
  BOOST_CHECK((std::is_same<decltype(math::integrate(p_type2{}, "x")),
                            polynomial<rational, monomial<rational>>>::value));
  BOOST_CHECK((std::is_same<decltype(math::integrate(p_type3{}, "x")),
                            polynomial<rational, monomial<rational>>>::value));
  BOOST_CHECK_EQUAL(math::integrate(p_type2{"x"}.pow(3 / 4_q), "x"),
                    4 / 7_q * p_type2{"x"}.pow(7 / 4_q));
  BOOST_CHECK_EQUAL(math::integrate(3 * p_type3{"x"}.pow(3 / 4_q), "x"),
                    12 / 7_q * p_type3{"x"}.pow(7 / 4_q));
}
...
BOOST_AUTO_TEST_CASE(polynomial_ipow_subs_test) {
  ...
  BOOST_CHECK((has_ipow_subs<p_type1, integer>::value));
  {
    ...
    BOOST_CHECK_EQUAL((x.pow(-5) * y * z).ipow_subs("x", -4, 5),
                      x.pow(-1) * 5 * z * y);
  }
  {
    typedef polynomial<real, monomial<int>> p_type2;
  ...
  }
  {
    ...
    p_type4 x{"x"}, y{"y"}, z{"z"};
    BOOST_CHECK_EQUAL(x * y * 2 * z.pow(7 / 3_q).ipow_subs("z", 2, 4),
                      4 * z.pow(1 / 3_q) * y * 2 * x);
    BOOST_CHECK_EQUAL(x * y * 2 * z.pow(-7 / 3_q).ipow_subs("z", -1, 4),
                      16 * z.pow(-1 / 3_q) * y * 2 * x);
  }
}
...
BOOST_AUTO_TEST_CASE(polynomial_rebind_test) {
  ...
  BOOST_CHECK((std::is_same<series_rebind<stype, float>,
                            polynomial<float, monomial<long>>>::value));
  BOOST_CHECK((std::is_same<series_rebind<stype, rational>,
                            polynomial<rational, monomial<long>>>::value));
  BOOST_CHECK((std::is_same<series_rebind<stype, long double>,
                            polynomial<long double, monomial<long>>>::value));
}
...
BOOST_AUTO_TEST_CASE(polynomial_r_polynomial_test) {
  BOOST_CHECK((std::is_same<polynomial<double, k_monomial>,
                            r_polynomial<1, double, k_monomial>>::value));
  BOOST_CHECK(
      (std::is_same<polynomial<polynomial<double, k_monomial>, k_monomial>,
                    r_polynomial<2, double, k_monomial>>::value));
  BOOST_CHECK(
      (std::is_same<
          polynomial<polynomial<polynomial<double, k_monomial>, k_monomial>,
                     k_monomial>,
...
BOOST_AUTO_TEST_CASE(polynomial_find_cf_test) {
  ...
  BOOST_CHECK_THROW((3 * pt1{"x"} + 4 * pt1{"y"}).find_cf({2}),
                    std::invalid_argument);
  ...
  BOOST_CHECK_EQUAL(
      (3 * pt1{"x"} + 4 * pt1{"y"}).find_cf(std::vector<integer>{1_z, 1_z}), 0);
  BOOST_CHECK_EQUAL((3 * pt1{"x"} + 4 * pt1{"y"}).find_cf(std::list<int>{0, 1}),
                    4);
  ...
  BOOST_CHECK_THROW((3 * pt2{"x"} + 4 * pt2{"y"}).find_cf({2}),
                    std::invalid_argument);
  ...
  BOOST_CHECK_EQUAL(
      (3 * pt2{"x"} + 4 * pt2{"y"}).find_cf(std::vector<integer>{1_z, 1_z}), 0);
  BOOST_CHECK_EQUAL((3 * pt2{"x"} + 4 * pt2{"y"}).find_cf(std::list<int>{0, 1}),
                    4);
  BOOST_CHECK_EQUAL(
      (3 * pt2{"x"} + 4 * pt2{"y"}).find_cf(std::list<signed char>{0, 1}), 4);
  if (std::numeric_limits<long>::max() > std::numeric_limits<int>::max()) {
    BOOST_CHECK_THROW(
        pt2{"x"}.find_cf(std::list<long>{std::numeric_limits<long>::max()}),
        std::invalid_argument);
  }
}

...
public:
  ...
  g_series_type sin() { return g_series_type(42); }
  int cos() const { return -42; }
};
...
public:
  template <typename Cf2> using rebind = g_series_type2<Cf2, Expo>;
...
template <typename Cf, typename Expo>
class g_series_type3
    : public series<Cf, monomial<Expo>, g_series_type3<Cf, Expo>> {
public:
  template <typename Cf2> using rebind = g_series_type3<Cf2, Expo>;
...
template <typename Cf, typename Expo>
class g_series_type4
    : public series<Cf, monomial<Expo>, g_series_type4<Cf, Expo>> {
public:
  template <typename Cf2> using rebind = g_series_type4<Cf2, Expo>;
...
template <typename Cf, typename Key>
class series_multiplier<g_series_type4<Cf, Key>, void>
    : public base_series_multiplier<g_series_type4<Cf, Key>> {
...
template <typename T>
struct sin_impl<
    T, typename std::enable_if<std::is_same<T, mock_cf>::value>::type> {
...
template <typename T>
struct sin_impl<
    T, typename std::enable_if<std::is_same<T, mock_cf2>::value>::type> {
...
// NOTE: here:
// - g_series_type has a wrong sin() overload but a good cos() one, and
// g_series_type2 have suitable sin/cos members,
...
BOOST_AUTO_TEST_CASE(series_sin_cos_test) {
  ...
  // - g_series_type<mock_cf,int> has no sine because math::sin() on mock_cf is
  // wrong,
  ...
  BOOST_CHECK((std::is_same<decltype(math::sin(p_type4{})),
  ...
  BOOST_CHECK((std::is_same<decltype(math::sin(p_type5{})),
                            g_series_type3<mock_cf, int>>::value));
  BOOST_CHECK((std::is_same<decltype(math::cos(p_type5{})),
                            g_series_type3<mock_cf, int>>::value));
  // Check that casting a series type to its base type and then calling sin/cos
  // still gets out the original
  ...
  BOOST_CHECK(
      (std::is_same<p_type6,
                    decltype(math::sin(
                        std::declval<const p_type6::base &>()))>::value));
  BOOST_CHECK(
      (std::is_same<p_type6,
                    decltype(math::cos(
                        std::declval<const p_type6::base &>()))>::value));
  ...
  BOOST_CHECK(
      (std::is_same<p_type7,
                    decltype(math::sin(
                        std::declval<const p_type7::base &>()))>::value));
  BOOST_CHECK(
      (std::is_same<p_type7,
                    decltype(math::cos(
                        std::declval<const p_type7::base &>()))>::value));
  ...
  BOOST_CHECK(
      (std::is_same<p_type1,
                    decltype(math::sin(
                        std::declval<const p_type1::base &>()))>::value));
  BOOST_CHECK(
      (std::is_same<p_type1,
                    decltype(math::cos(
                        std::declval<const p_type1::base &>()))>::value));
...
BOOST_AUTO_TEST_CASE(series_iterator_test) {
  ...
  BOOST_CHECK((std::is_same<typename p_type1::term_type::cf_type,
                            decltype(it->first)>::value));
...
BOOST_AUTO_TEST_CASE(series_filter_test) {
  ...
  BOOST_CHECK_EQUAL(
      x, (x + 2 * y).filter([](const pair_type &p) { return p.first < 2; }));
  BOOST_CHECK_EQUAL(x + 2 * y, (x + 2 * y).filter([](const pair_type &p) {
    return p.second.size();
  }));
  BOOST_CHECK_EQUAL(0, (x + 2 * y).filter([](const pair_type &p) {
    return p.second.size() == 0u;
  }));
  BOOST_CHECK_EQUAL(
      -y, (x - y + 3).filter([](const pair_type &p) { return p.first < 0; }));
  BOOST_CHECK_EQUAL(-y - 3, (x - y - 3).filter([](const pair_type &p) {
    return p.first < 0;
  }));
  BOOST_CHECK_EQUAL(
      x, (x - y - 3).filter([](const pair_type &p) { return p.first > 0; }));
}
...
BOOST_AUTO_TEST_CASE(series_transform_test) {
  ...
  BOOST_CHECK_EQUAL(0,
                    x.transform([](const pair_type &) { return pair_type(); }));
  BOOST_CHECK_EQUAL(rational(1, 2), x.transform([](const pair_type &) {
    return pair_type(rational(1, 2), p_type1(1));
  }));
  BOOST_CHECK_EQUAL(2 * (x + y), (x + y).transform([](const pair_type &p) {
    return pair_type(p.first * 2, p.second);
  }));
  ...
  BOOST_CHECK_EQUAL(y2.transform([](const pair_type2 &p) {
    return std::make_pair(
        p.first.filter([](const pair_type &q) { return q.first < 2; }),
        p.second);
  }),
...
struct print_tex_tester {
  template <typename Cf> struct runner {
    template <typename Expo> void operator()(const Expo &) {
      ...
      BOOST_CHECK(
          std::equal(tmp_cmp.rbegin(), tmp_cmp.rend(), tmp_out.rbegin()));
      oss.str("");
...
struct trim_tester {
  template <typename Cf> struct runner {
    template <typename Expo> void operator()(const Expo &) {
      ...
      BOOST_CHECK_EQUAL((1 + x * y - y * x + x).trim().get_symbol_set().size(),
                        1u);
      BOOST_CHECK_EQUAL(
          (1 + x * y - y * x + x + y).trim().get_symbol_set().size(), 2u);
      p_type11 xx(x), yy(y);
      BOOST_CHECK_EQUAL(((1 + xx) - xx).begin()->first.get_symbol_set().size(),
                        1u);
      BOOST_CHECK_EQUAL(
          ((1 + xx) - xx).trim().begin()->first.get_symbol_set().size(), 0u);
      BOOST_CHECK_EQUAL(((1 + xx * yy) - xx * yy + xx)
                            .trim()
                            .begin()
                            ->first.get_symbol_set()
                            .size(),
                        1u);
      BOOST_CHECK_EQUAL(((1 + xx * yy) - xx * yy + xx + yy)
                            .trim()
                            .begin()
                            ->first.get_symbol_set()
                            .size(),
                        2u);
      BOOST_CHECK_EQUAL((1 + x * xx + y * yy - x * xx)
                            .trim()
                            .begin()
                            ->first.get_symbol_set()
                            .size(),
                        1u);
      BOOST_CHECK_EQUAL(
          (1 + x * p_type11{"x"} + y * p_type11{"y"} - x * p_type11{"x"})
              .trim()
              .get_symbol_set()
              .size(),
          1u);
...
struct type_traits_tester {
  template <typename Cf> struct runner {
    template <typename Expo> void operator()(const Expo &) {
      ...
      BOOST_CHECK((std::is_same<void, decltype(print_coefficient(
                                          *(std::ostream *)nullptr,
                                          std::declval<p_type1>()))>::value));
      BOOST_CHECK((std::is_same<void, decltype(print_coefficient(
                                          *(std::ostream *)nullptr,
                                          std::declval<p_type11>()))>::value));
      BOOST_CHECK((std::is_same<void, decltype(print_tex_coefficient(
                                          *(std::ostream *)nullptr,
                                          std::declval<p_type1>()))>::value));
      BOOST_CHECK((std::is_same<void, decltype(print_tex_coefficient(
                                          *(std::ostream *)nullptr,
      ...
      BOOST_CHECK(!has_negate<const p_type1>::value);
      BOOST_CHECK((std::is_same<decltype(math::negate(*(p_type1 *)nullptr)),
                                void>::value));
      BOOST_CHECK(has_negate<p_type11>::value);
      BOOST_CHECK(has_negate<p_type11 &>::value);
      BOOST_CHECK(!has_negate<const p_type11 &>::value);
      BOOST_CHECK(!has_negate<const p_type11>::value);
      BOOST_CHECK((std::is_same<decltype(math::negate(*(p_type11 *)nullptr)),
                                void>::value));
    }
  };
  template <typename Cf> void operator()(const Cf &) {

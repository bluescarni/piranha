...
const boost::fusion::vector<char, short, int, long, long long, unsigned char,
                            unsigned short, unsigned, unsigned long,
                            unsigned long long, float, double, long double>
arithmetic_values((char)-42, (short)42, -42, 42L, -42LL, (unsigned char)42,
                  (unsigned short)42, 42U, 42UL, 42ULL, 23.456f, -23.456,
                  23.456L);
...
const boost::fusion::vector<char, short, int, long, long long, unsigned char,
                            unsigned short, unsigned, unsigned long,
                            unsigned long long, float, double, long double>
zeroes((char)0, (short)0, 0, 0L, 0LL, (unsigned char)0, (unsigned short)0, 0U,
       0UL, 0ULL, 0.f, -0., 0.L);
...
template <> struct is_zero_impl<trivial_a, void> {
  char operator()(const trivial_a &);
};

template <> struct is_zero_impl<trivial_b, void> {
  char operator()(const trivial_a &);
};

template <> struct is_zero_impl<trivial_c, void> {
  std::string operator()(const trivial_c &);
};

template <> struct is_zero_impl<trivial_d, void> {
...
struct check_multiply_accumulate {
  ...
  // NOTE: avoid testing with char and short types, the promotion rules will
  // generate warnings about assigning int to
  // narrower types.
  template <typename T>
  void operator()(
      const T &,
...
BOOST_AUTO_TEST_CASE(math_pow_test) {
  ...
  BOOST_CHECK((std::is_same<decltype(math::pow(2.f, 2)), double>::value));
  BOOST_CHECK(
      (std::is_same<decltype(math::pow(2.f, 2.L)), long double>::value));
  BOOST_CHECK(math::pow(2., 2) == std::pow(2., 2));
  BOOST_CHECK(math::pow(2.f, 2) == std::pow(2.f, 2));
  BOOST_CHECK((std::is_same<decltype(math::pow(2., 2)), double>::value));
  BOOST_CHECK((std::is_same<decltype(math::pow(2.f, 2)), double>::value));
...
template <> struct cos_impl<cos_00, void> {
  cos_00 operator()(const cos_00 &) const;
};

template <> struct cos_impl<cos_01, void> {
  cos_01 operator()(const cos_01 &) const;
};

template <> struct sin_impl<sin_00, void> {
  sin_00 operator()(const sin_00 &) const;
};

template <> struct sin_impl<sin_01, void> {
...
BOOST_AUTO_TEST_CASE(math_sin_cos_test) {
  ...
  BOOST_CHECK((std::is_same<unsigned short,
                            decltype(math::sin((unsigned short)0))>::value));
  BOOST_CHECK((std::is_same<unsigned short,
                            decltype(math::cos((unsigned short)0))>::value));
...
BOOST_AUTO_TEST_CASE(math_evaluate_test) {
  BOOST_CHECK_EQUAL(
      math::evaluate(5, std::unordered_map<std::string, double>{}), 5);
  BOOST_CHECK_EQUAL(math::evaluate(std::complex<float>(5., 4.),
                                   std::unordered_map<std::string, double>{}),
                    std::complex<float>(5., 4.));
  BOOST_CHECK_EQUAL(math::evaluate(std::complex<double>(5., 4.),
                                   std::unordered_map<std::string, double>{}),
                    std::complex<double>(5., 4.));
  BOOST_CHECK_EQUAL(math::evaluate(std::complex<long double>(5., 4.),
                                   std::unordered_map<std::string, double>{}),
                    std::complex<long double>(5., 4.));
  BOOST_CHECK((std::is_same<decltype(math::evaluate(
                                5, std::unordered_map<std::string, double>{})),
                            int>::value));
  BOOST_CHECK_EQUAL(math::evaluate(5., std::unordered_map<std::string, int>{}),
                    5.);
  BOOST_CHECK((std::is_same<decltype(math::evaluate(
                                5., std::unordered_map<std::string, short>{})),
                            double>::value));
  BOOST_CHECK_EQUAL(
      math::evaluate(5ul, std::unordered_map<std::string, double>{}), 5ul);
  BOOST_CHECK((std::is_same<decltype(math::evaluate(
                                5ul, std::unordered_map<std::string, short>{})),
                            unsigned long>::value));
...
BOOST_AUTO_TEST_CASE(math_pbracket_test) {
  ...
  BOOST_CHECK_EQUAL(math::pbracket(p_type{}, p_type{},
                                   std::vector<std::string>{},
                                   std::vector<std::string>{}),
                    p_type(0));
  BOOST_CHECK_THROW(
      math::pbracket(p_type{}, p_type{}, {"p"}, std::vector<std::string>{}),
      std::invalid_argument);
  BOOST_CHECK_THROW(math::pbracket(p_type{}, p_type{}, {"p"}, {"q", "r"}),
                    std::invalid_argument);
  BOOST_CHECK_THROW(math::pbracket(p_type{}, p_type{}, {"p", "p"}, {"q", "r"}),
                    std::invalid_argument);
  BOOST_CHECK_THROW(math::pbracket(p_type{}, p_type{}, {"p", "q"}, {"q", "q"}),
                    std::invalid_argument);
  BOOST_CHECK_EQUAL(math::pbracket(p_type{}, p_type{}, {"x", "y"}, {"a", "b"}),
                    p_type(0));
  ...
  auto m = ps_type{"m"}, p = ps_type{"p"}, l = ps_type{"l"}, g = ps_type{"g"},
       th = ps_type{"theta"};
  ...
  auto vx = ps_type{"vx"}, vy = ps_type{"vy"}, vz = ps_type{"vz"},
       r = ps_type{"r"};
  ...
  BOOST_CHECK_EQUAL(
      math::pbracket(H_2, H_2, {"vx", "vy", "vz"}, {"x", "y", "z"}), 0);
  // Angular momentum integral.
  auto Gx = y * vz - z * vy, Gy = z * vx - x * vz, Gz = x * vy - y * vx;
  BOOST_CHECK_EQUAL(
      math::pbracket(H_2, Gx, {"vx", "vy", "vz"}, {"x", "y", "z"}), 0);
  BOOST_CHECK_EQUAL(
      math::pbracket(H_2, Gy, {"vx", "vy", "vz"}, {"x", "y", "z"}), 0);
  BOOST_CHECK_EQUAL(
      math::pbracket(H_2, Gz, {"vx", "vy", "vz"}, {"x", "y", "z"}), 0);
  BOOST_CHECK(
      math::pbracket(H_2, Gz + x, {"vx", "vy", "vz"}, {"x", "y", "z"}) != 0);
}
...
BOOST_AUTO_TEST_CASE(math_fp_binomial_test) {
  ...
  for (int i = 0; i < ntries; ++i) {
    ...
    BOOST_CHECK(std::isfinite(
        detail::fp_binomial(static_cast<double>(x), static_cast<double>(y))));
    BOOST_CHECK(std::isfinite(detail::fp_binomial(
...
template <> struct binomial_impl<b_00, b_00, void> {
  b_00 operator()(const b_00 &, const b_00 &) const;
};

template <> struct binomial_impl<b_01, b_01, void> {
...
BOOST_AUTO_TEST_CASE(math_binomial_test) {
  ...
  BOOST_CHECK(
      (std::is_same<long double, decltype(math::binomial(0.l, 0))>::value));
  BOOST_CHECK((std::is_same<long double,
                            decltype(math::binomial(0.l, char(0)))>::value));
  BOOST_CHECK((std::is_same<long double,
                            decltype(math::binomial(0.l, short(0)))>::value));
  ...
  if (std::numeric_limits<double>::has_quiet_NaN &&
    ...
    BOOST_CHECK_THROW(
        math::binomial(1., std::numeric_limits<double>::quiet_NaN()),
        std::invalid_argument);
    BOOST_CHECK_THROW(
        math::binomial(1., std::numeric_limits<double>::infinity()),
        std::invalid_argument);
    BOOST_CHECK_THROW(
        math::binomial(std::numeric_limits<double>::quiet_NaN(), 1.),
        std::invalid_argument);
    BOOST_CHECK_THROW(
        math::binomial(std::numeric_limits<double>::infinity(), 1.),
        std::invalid_argument);
    BOOST_CHECK_THROW(math::binomial(std::numeric_limits<double>::quiet_NaN(),
                                     std::numeric_limits<double>::infinity()),
                      std::invalid_argument);
    BOOST_CHECK_THROW(math::binomial(std::numeric_limits<double>::infinity(),
...
BOOST_AUTO_TEST_CASE(math_canonical_test) {
  ...
  BOOST_CHECK_THROW(
      math::transformation_is_canonical({p_type1{"p"}, p_type1{"p"}},
                                        {p_type1{"q"}}, {"p"}, {"q"}),
      std::invalid_argument);
  BOOST_CHECK_THROW(math::transformation_is_canonical(
                        {p_type1{"p"}}, {p_type1{"q"}}, {"p", "x"}, {"q"}),
                    std::invalid_argument);
  BOOST_CHECK_THROW(math::transformation_is_canonical(
                        {p_type1{"p"}}, {p_type1{"q"}}, {"p", "x"}, {"q", "y"}),
                    std::invalid_argument);
  BOOST_CHECK((math::transformation_is_canonical({p_type1{"p"}}, {p_type1{"q"}},
                                                 {"p"}, {"q"})));
  p_type1 px{"px"}, py{"py"}, x{"x"}, y{"y"};
  BOOST_CHECK((math::transformation_is_canonical({py, px}, {y, x}, {"px", "py"},
                                                 {"x", "y"})));
  BOOST_CHECK((!math::transformation_is_canonical({py, px}, {x, y},
                                                  {"px", "py"}, {"x", "y"})));
  BOOST_CHECK((math::transformation_is_canonical({-x, -y}, {px, py},
                                                 {"px", "py"}, {"x", "y"})));
  BOOST_CHECK((!math::transformation_is_canonical({x, y}, {px, py},
                                                  {"px", "py"}, {"x", "y"})));
  BOOST_CHECK((math::transformation_is_canonical({px, px + py}, {x - y, y},
                                                 {"px", "py"}, {"x", "y"})));
  BOOST_CHECK((!math::transformation_is_canonical({px, px - py}, {x - y, y},
                                                  {"px", "py"}, {"x", "y"})));
  BOOST_CHECK((math::transformation_is_canonical(
      std::vector<p_type1>{px, px + py}, std::vector<p_type1>{x - y, y},
      {"px", "py"}, {"x", "y"})));
  // Linear transformation.
  p_type1 L{"L"}, G{"G"}, H{"H"}, l{"l"}, g{"g"}, h{"h"};
  BOOST_CHECK((math::transformation_is_canonical(
      {L + G + H, L + G, L}, {h, g - h, l - g}, {"L", "G", "H"},
      {"l", "g", "h"})));
  // Unimodular matrices.
  BOOST_CHECK((math::transformation_is_canonical(
      {L + 2 * G + 3 * H, -4 * G + H, 3 * G - H},
      {l, 11 * l - g - 3 * h, 14 * l - g - 4 * h}, {"L", "G", "H"},
      {"l", "g", "h"})));
  BOOST_CHECK((math::transformation_is_canonical(
      {2 * L + 3 * G + 2 * H, 4 * L + 2 * G + 3 * H, 9 * L + 6 * G + 7 * H},
      {-4 * l - g + 6 * h, -9 * l - 4 * g + 15 * h, 5 * l + 2 * g - 8 * h},
      {"L", "G", "H"}, {"l", "g", "h"})));
  BOOST_CHECK((!math::transformation_is_canonical(
      {2 * L + 3 * G + 2 * H, 4 * L + 2 * G + 3 * H, 9 * L + 6 * G + 7 * H},
      {-4 * l - g + 6 * h, -9 * l - 4 * g + 15 * h, 5 * l + 2 * g - 7 * h},
      {"L", "G", "H"}, {"l", "g", "h"})));
  BOOST_CHECK((math::transformation_is_canonical(
      std::vector<p_type1>{2 * L + 3 * G + 2 * H, 4 * L + 2 * G + 3 * H,
                           9 * L + 6 * G + 7 * H},
      std::vector<p_type1>{-4 * l - g + 6 * h, -9 * l - 4 * g + 15 * h,
                           5 * l + 2 * g - 8 * h},
      {"L", "G", "H"}, {"l", "g", "h"})));
  BOOST_CHECK((!math::transformation_is_canonical(
      std::vector<p_type1>{2 * L + 3 * G + 2 * H, 4 * L + 2 * G + 3 * H,
  ...
  p_type2::register_custom_derivative("P", [&P2](const p_type2 &arg) {
    return arg.partial("P") + arg.partial("P2") * math::pow(P2, -1);
  });
  p_type2::register_custom_derivative("Q", [&Q2](const p_type2 &arg) {
    return arg.partial("Q") + arg.partial("Q2") * math::pow(Q2, -1);
  });
  BOOST_CHECK((math::transformation_is_canonical(
      {P2 * math::cos(p), Q2 * math::cos(q)},
      {P2 * math::sin(p), Q2 * math::sin(q)}, {"P", "Q"}, {"p", "q"})));
  BOOST_CHECK((!math::transformation_is_canonical(
      {P * Q * math::cos(p) * q,
       Q * P * math::sin(3 * q) * p * math::pow(q, -1)},
      {P * math::sin(p), Q * math::sin(q)}, {"P", "Q"}, {"p", "q"})));
  BOOST_CHECK((!math::transformation_is_canonical(
      std::vector<p_type2>{P2 * math::cos(p) * q, Q2 * math::cos(q) * p},
      std::vector<p_type2>{P2 * math::sin(p), Q2 * math::sin(q)}, {"P", "Q"},
      {"p", "q"})));
...
template <> struct truncate_degree_impl<double, double, void> {
...
template <> struct truncate_degree_impl<double, float, void> {
  double operator()(const double &, const float &) const;
};

// float-double, wrong return type.
template <> struct truncate_degree_impl<float, double, void> {
...
struct mock_key {
  ...
  std::vector<std::pair<int, mock_key>> subs(const std::string &, int,
...
template <typename T>
struct gcd_impl<
    T, T, typename std::enable_if<std::is_same<T, mock_type>::value>::type> {
...
BOOST_AUTO_TEST_CASE(math_gcd_test) {
  ...
  BOOST_CHECK(
      (!std::is_same<decltype(gcd(short(54), short(27))), short>::value));
  BOOST_CHECK(
      (!std::is_same<decltype(gcd(short(54), char(27))), short>::value));
  BOOST_CHECK_EQUAL(gcd(short(54), short(27)), short(27));
  BOOST_CHECK_EQUAL(gcd(short(27), short(53)), short(1));
  BOOST_CHECK(gcd(short(27), short(-54)) == short(27) ||
              gcd(short(27), short(-54)) == short(-27));
  BOOST_CHECK(gcd(short(-54), short(27)) == short(27) ||
              gcd(short(-54), short(27)) == short(-27));
  ...
  std::uniform_int_distribution<int> int_dist(
      -detail::safe_abs_sint<int>::value, detail::safe_abs_sint<int>::value);
  for (int i = 0; i < ntries; ++i) {

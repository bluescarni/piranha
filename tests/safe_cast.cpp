...
// Good specialisation.
template <> struct safe_cast_impl<conv2, conv1, void> {
  conv2 operator()(const conv1 &) const;
};

// Bad specialisation.
template <> struct safe_cast_impl<conv3, conv1, void> {
  int operator()(const conv1 &) const;
};

// Bad specialisation.
template <> struct safe_cast_impl<conv4, conv1, void> {
...
struct safe_cast_int_float_tester {
  template <typename T> void operator()(const T &) {
    ...
    BOOST_CHECK_THROW(
        safe_cast<int>(int_type{std::numeric_limits<int>::max()} + 1),
        std::invalid_argument);
    BOOST_CHECK_THROW(
        safe_cast<int>(int_type{std::numeric_limits<int>::min()} - 1),
        std::invalid_argument);
    BOOST_CHECK_THROW(
        safe_cast<unsigned>(int_type{std::numeric_limits<unsigned>::max()} + 1),
        std::invalid_argument);
    ...
    BOOST_CHECK_THROW(
        safe_cast<int_type>(1. / std::numeric_limits<double>::radix),
        std::invalid_argument);
    BOOST_CHECK_THROW(
        safe_cast<int_type>(1.f / std::numeric_limits<float>::radix),
        std::invalid_argument);
    BOOST_CHECK_THROW(
        safe_cast<int_type>((1. + std::numeric_limits<double>::radix) /
    ...
    if (std::numeric_limits<double>::has_infinity &&
      ...
      BOOST_CHECK_THROW(
          safe_cast<int_type>(std::numeric_limits<double>::infinity()),
          std::invalid_argument);
      BOOST_CHECK_THROW(
          safe_cast<int_type>(std::numeric_limits<double>::quiet_NaN()),
          std::invalid_argument);
    }
  }
};
...
BOOST_AUTO_TEST_CASE(safe_cast_int_float_test) {
  ...
  BOOST_CHECK_EQUAL(safe_cast<int>(-2.), -2);
  BOOST_CHECK_THROW(safe_cast<int>(1. / std::numeric_limits<double>::radix),
                    std::invalid_argument);
  BOOST_CHECK_THROW(safe_cast<int>(1.f / std::numeric_limits<float>::radix),
                    std::invalid_argument);
  BOOST_CHECK_THROW(safe_cast<int>((1. + std::numeric_limits<double>::radix) /
                                   std::numeric_limits<double>::radix),
                    std::invalid_argument);
  ...
  if (std::numeric_limits<double>::has_infinity &&
      std::numeric_limits<double>::has_quiet_NaN) {
    BOOST_CHECK_THROW(safe_cast<int>(std::numeric_limits<double>::infinity()),
                      std::invalid_argument);
    BOOST_CHECK_THROW(safe_cast<long>(std::numeric_limits<double>::quiet_NaN()),
                      std::invalid_argument);
  }
}

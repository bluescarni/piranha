...
BOOST_AUTO_TEST_CASE(thread_pool_test) {
  ...
  BOOST_CHECK_THROW(thread_pool::enqueue(initial_size, adder, 4, -5),
                    std::invalid_argument);
#if !defined(__APPLE_CC__)
  BOOST_CHECK(thread_pool::enqueue(0, []() { return bound_proc(); }).get() ==
              std::make_pair(true, 0u));
#endif
  BOOST_CHECK_THROW(
      thread_pool::enqueue(0, []() { throw std::runtime_error(""); }).get(),
      std::runtime_error);
  auto fast_task = [](int n) -> int {
    std::this_thread::sleep_for(std::chrono::milliseconds(1));
    return n;
  ...
  if (initial_size != boost::integer_traits<unsigned>::const_max) {
    ...
    std::all_of(list.begin(), list.begin() + initial_size,
                [](decltype(thread_pool::enqueue(0u, func)) &f) {
                  return f.get().first;
                });
...
BOOST_AUTO_TEST_CASE(thread_pool_use_threads_test) {
  ...
  BOOST_CHECK_THROW(thread_pool::use_threads(100_z, 0_z),
                    std::invalid_argument);
  BOOST_CHECK_THROW(thread_pool::use_threads(0_z, 100_z),
                    std::invalid_argument);
  BOOST_CHECK_THROW(thread_pool::use_threads(0_z, 0_z), std::invalid_argument);
  BOOST_CHECK_THROW(thread_pool::use_threads(100_z, -1_z),
                    std::invalid_argument);
  BOOST_CHECK_THROW(thread_pool::use_threads(-1_z, 100_z),
                    std::invalid_argument);
  BOOST_CHECK_THROW(thread_pool::use_threads(-1_z, -1_z),
                    std::invalid_argument);
  BOOST_CHECK(thread_pool::use_threads(100u, 30u) == 3u);
  auto f1 = thread_pool::enqueue(
      0u, []() { return thread_pool::use_threads(100u, 3u); });
  auto f2 = thread_pool::enqueue(
      0u, []() { return thread_pool::use_threads(100u, 1u); });
  auto f3 = thread_pool::enqueue(
      0u, []() { return thread_pool::use_threads(100u, 0u); });
  ...
  auto f4 = thread_pool::enqueue(
      0u, []() { return thread_pool::use_threads(100u, 3u); });
  auto f5 = thread_pool::enqueue(
      0u, []() { return thread_pool::use_threads(100u, 1u); });
  auto f6 = thread_pool::enqueue(
      0u, []() { return thread_pool::use_threads(100u, 0u); });
  ...
  BOOST_CHECK_THROW(thread_pool::use_threads(integer(100u), integer(0u)),
                    std::invalid_argument);

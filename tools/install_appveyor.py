...
# Configuration step.
if is_python_build:
    run_command(r'cmake -G "MinGW Makefiles" ..  -DPIRANHA_BUILD_PYRANHA=yes -DCMAKE_BUILD_TYPE=Release -DPIRANHA_INSTALL_HEADERS=no -DCMAKE_CXX_FLAGS=-s ' + common_cmake_opts + r' -DBoost_PYTHON_LIBRARY_RELEASE=c:\\local\\lib\\libboost_python' +
    ...
    cmake_opts = r'-DCMAKE_BUILD_TYPE=' + BUILD_TYPE + (r' -DPIRANHA_BUILD_TESTS=yes' if BUILD_TYPE == 'Debug' else r' -DPIRANHA_BUILD_BENCHMARKS=yes') + r' -DPIRANHA_TEST_NSPLIT=' + \
        ...
        (r' -DCMAKE_CXX_FLAGS_DEBUG="-g0 -Os"' if BUILD_TYPE == 'Debug' else r'')
